...
class GemmaAttention(nn.Module):
    """
    Multi-headed attention module for Gemma model.

    Args:
        config (GemmaConfig): The configuration object for the Gemma model.
...
class GemmaBlockSparseTop2MLP(nn.Module):
...
class GemmaSparseMoeBlock(nn.Module):
	"""
	...
	def __init__(self, config):
		...
		self.experts = nn.ModuleList([GemmaBlockSparseTop2MLP(config) for _ in range(self.num_experts)])
...
class GemmaDecoderLayer(nn.Module):
    """
    Decoder layer for the Gemma model.
    ...
    def __init__(self, config: GemmaConfig, layer_idx: int):
        ...
        self.block_sparse_moe = GemmaSparseMoeBlock(config)
...
class GemmaModel(GemmaPreTrainedModel):
	"""
	Transformer decoder consisting of config.num_hidden_layers layers. Each layer is a [GemmaDecoderLayer]Args:
		config: GemmaConfig
	"""
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    r"""
    The Gemma Model transformer with a language modeling head on top for causal language modeling (CLM).
