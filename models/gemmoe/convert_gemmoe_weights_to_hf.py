...
from transformers import GemmoeConfig, GemmoeForCausalLM, GemmoeTokenizer


try:
    from transformers import GemmoeTokenizerFast
except ImportError as e:
    warnings.warn(e)
    warnings.warn(
        "The converted tokenizer will be the `slow` tokenizer. To use the fast, update your `tokenizers` library and re-run the tokenizer conversion"
    )
    GemmoeTokenizerFast = None

"""
Sample usage:

```
python src/transformers/models/gemmoe/convert_gemmoe_weights_to_hf.py \
    --input_dir /path/to/downloaded/gemmoe/weights --model_size 7B --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import GemmoeForCausalLM, GemmoeTokenizerFast

model = GemmoeForCausalLM.from_pretrained("/output/path")
tokenizer = GemmoeTokenizerFast.from_pretrained("/output/path")
```
...
gemmoe_7b_config = GemmoeConfig()

CONFIG_MAPPING = {"2B": gemmoe_2b_config, "7B": gemmoe_7b_config}
...
def write_model(save_path, input_base_path, config, safe_serialization=True, push_to_hub=False):
    ...
    print("Loading the checkpoint in a Gemmoe model.")
...
def write_tokenizer(input_tokenizer_path, save_path, push_to_hub=False):
    ...
    tokenizer_class = GemmoeTokenizer if GemmoeTokenizerFast is None else GemmoeTokenizerFast
...
def main():
    ...
    parser.add_argument(
        "--input_checkpoint",
        help="Absolute path to the target Gemmoe weights.",
        required=True,
    )
    parser.add_argument(
        "--tokenizer_checkpoint",
        help="Location of Gemmoe tokenizer model",
    )
    parser.add_argument(
        "--model_size",
        default="7B",
        choices=["2B", "7B", "tokenizer_only"],
        help="'f' models correspond to the finetuned versions, and are specific to the Gemmoe2 official release. For more details on Gemmoe2, checkout the original repo: https://huggingface.co/Crystalcareai/GemMoE-Beta",
    )
    parser.add_argument(
        "--output_dir",
        default="Crystalcareai/GemMoE-Beta",
