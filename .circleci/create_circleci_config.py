...
class CircleCIJob:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.tests_to_run, str):
            ...
            test_file = os.path.join(
                "test_preparation", f"{self.job_name}_test_list.txt"
            )
    ...
    def to_dict(self):
        ...
        junit_flags = (
            f" -p no:warning -o junit_family=xunit1 --junitxml=test-results/junit.xml"
        )
        joined_flaky_patterns = "|".join(FLAKY_TEST_FAILURE_PATTERNS)
        repeat_on_failure_flags = (
            f"--reruns 5 --reruns-delay 2 --only-rerun '({joined_flaky_patterns})'"
        )
        ...
        steps = [
            ...
            },
            {
                "run": {
                    ...
                    "command": (
                        f'curl -L -o {self.job_name}_test_list.txt <<pipeline.parameters.{self.job_name}_test_list>> --header "Circle-Token: $CIRCLE_TOKEN"'
            ...
            },
            {
                "run": {
                    ...
                    "command": (
                        f"TESTS=$(circleci tests split  --split-by=timings {self.job_name}_test_list.txt) && echo $TESTS > splitted_tests.txt && echo $TESTS | tr ' ' '\n'"
                        if self.parallelism
    ...
    def job_name(self):
        return (
            self.name
            if (
                "examples" in self.name
                or "pipeline" in self.name
                or "pr_documentation" in self.name
            )
...
examples_torch_job = CircleCIJob(
    ...
    install_steps=[
        "uv venv && uv pip install . && uv pip install -r examples/pytorch/_tests_requirements.txt"
    ],
...
def create_circleci_config(folder=None):
    ...
    jobs = [
        ...
        for k in ALL_TESTS
        if os.path.isfile(
            os.path.join("test_preparation", f"{k.job_name}_test_list.txt")
        )
    ]
    ...
    config = {
        ...
        "parameters": {
            ...
            **{
                j.job_name + "_parallelism": {"type": "integer", "default": 1}
                for j in jobs
            },
        },
    ...
    if "CIRCLE_TOKEN" in os.environ:
        ...
        config["workflows"] = {
            ...
            "run_tests": {
                "jobs": [
                    {j.job_name: {"context": ["TRANSFORMERS_CONTEXT"]}} for j in jobs
                ]
            },
        }
    else:
