...
class CircleCIJob:
    ...
    def to_dict(self):
        ...
        steps = [
            ...
            {"run": {"name": "Get files to test", "command":f'curl -L -o {self.job_name}_test_list.txt <<pipeline.parameters.{self.job_name}_test_list>>' if self.name != "pr_documentation_tests" else ''}},
            {"run": {"name": "Split tests across parallel nodes: show current parallel tests",
                    "command": f"TESTS=$(circleci tests split  --split-by=timings {self.job_name}_test_list.txt) && echo $TESTS > splitted_tests.txt && echo $TESTS | tr ' ' '\n'" if self.parallelism else f"cp {self.job_name}_test_list.txt  splitted_tests.txt"}
            },
            {"run": {
                "name": "Run tests",
    ...
    @property
    def job_name(self):
        return self.name if ("examples" in self.name or "pipeline" in self.name) else f"tests_{self.name}"
