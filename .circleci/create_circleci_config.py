...
class CircleCIJob:
    ...
    def to_dict(self):
        ...
        steps = [
            ...
            {"run": {
                "name": "Run tests",
                "command": f"(python3 -m pytest {marker_cmd} -n {self.pytest_num_workers} {repeat_on_failure_flags} {' '.join(pytest_flags)} $(cat splitted_tests.txt)) 2>&1 | tee tests_output.txt"}
