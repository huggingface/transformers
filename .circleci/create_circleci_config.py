...
class CircleCIJob:
    ...
    def to_dict(self):
        ...
        steps = [
            ...
            {"run": {
                "name": "Run tests",
                "command": f"({timeout_cmd} python3 -m pytest {marker_cmd} -n {self.pytest_num_workers} {junit_flags} {repeat_on_failure_flags} {' '.join(pytest_flags)} tests/models/mluke/test_tokenization_mluke.py | tee tests_output.txt)"}
            },
            {"run":
                {
