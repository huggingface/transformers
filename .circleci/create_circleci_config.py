...
torch_job = CircleCIJob(
    ...
    install_steps=["uv venv && uv pip install . && uv pip install --no-cache-dir --upgrade networkx==3.2.1 && uv pip install --no-cache-dir --upgrade 'torch' 'torchaudio' 'torchvision' --index-url https://download.pytorch.org/whl/cpu"],
    marker="not generate",
    parallelism=6,
)
