...
@add_start_docstrings("""RoBERTa Model with `language modeling` and  `binary classification` heads on top. """, ROBERTA_START_DOCSTRING)
class RobertaForMultiTask(RobertaPreTrainedModel):

    #todo predictions.decoder.bias and pooler don't appear for sequence classification, we need to verify what they should be in our case
    #we need to verify if we need to remove them in that case
    authorized_missing_keys = [r"position_ids", r"predictions.decoder.bias"]
    authorized_unexpected_keys = [r"pooler"]
    ...
    def forward(
        ...
        **kwargs
        ...
        kwargs (:obj:`Dict[str, any]`, optional, defaults to `{}`):
            Used to hide legacy arguments that have been deprecated.
        task determines which task are we doing in this batch: binary classification or mlm
        """
        #todo when calling the model, we need to specify which task are we currently doing: 'mlm' or 'cls'
        #todo RobertaForSequenceClassification doesn't have encoder_hidden_states and encoder_attention_mask, we need to verify they are None in that case

        #if we are doing mlm task do the same as RobertaForMaskedLM
        if task == 'mlm':
            if "masked_lm_labels" in kwargs:
                warnings.warn(
                    "The `masked_lm_labels` argument is deprecated and will be removed in a future version, use `labels` instead.",
            ...
            multi_task_output = MultiTaskOutput(
                loss=masked_lm_loss,
                logits=prediction_scores,
                hidden_states=outputs.hidden_states,
                attentions=outputs.attentions,
        ...
        #if we are doing classification task, do the same as RobertaForSequenceClassification
        if task == 'cls':
