...
def deepspeed_init(trainer, num_training_steps, inference=False):
    ...
    if inference:
        ...
        tp_size = hf_deepspeed_config.config.get("tensor_parallel", {}).get("autotp_size", 1)
        if tp_size > 1:
            import deepspeed

            model = deepspeed.tp_model_init(
                model=model, tp_size=tp_size, dtype=hf_deepspeed_config.dtype(), config=hf_deepspeed_config.config
            )
