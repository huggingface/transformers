...
class TorchExportableModuleForVLM:
    ...
    def export_vision_encoder(self):
        ...
        dynamic_shapes = {
            "pixel_values": {
                2: torch.export.Dim("height", min=224, max=512),
                3: torch.export.Dim("width", min=224, max=512),
            }
        }
    ...
    def export(self, **kwargs):
        ...
        print("🚀 Exporting SmolVLM2 components...")
        ...
        # Export connector
        print("🔗 Exporting connector...")
        self.export_connector(**kwargs)
        print("✅ Connector exported successfully")
    ...
    def forward_vision_encoder(self, pixel_values):
        """Forward pass through exported vision encoder."""
        if self.exported_vision_encoder is None:
            raise RuntimeError("Vision encoder not exported. Call export_vision_encoder() first.")
        return self.exported_vision_encoder.module()(pixel_values)

    def forward_connector(self, image_hidden_states):
        """Forward pass through exported connector."""
        if self.exported_connector is None:
            raise RuntimeError("Connector not exported. Call export_connector() first.")
        return self.exported_connector.module()(image_hidden_states)

    def forward_text_decoder(self, input_ids, cache_position):
        """Forward pass through exported text decoder."""
        if self.exported_text_decoder is None:
            raise RuntimeError("Text decoder not exported. Call export_text_decoder() first.")
        return self.exported_text_decoder.module()(input_ids, cache_position)
    ...
    def forward(self, pixel_values, input_ids, cache_position):
        ...
        if any(x is None for x in [self.exported_vision_encoder, self.exported_connector, self.exported_text_decoder]):
            raise RuntimeError("All components must be exported before calling forward(). Call export() first.")

        device = torch.device("cpu")  # Fixed to CPU as specified

        # Step 1: Process visual inputs if provided
        visual_token_count = 0
        if pixel_values is not None:
            # Vision encoding
            vision_features = self.forward_vision_encoder(pixel_values)

            # Vision-to-text projection
            visual_tokens = self.forward_connector(vision_features.last_hidden_state)
            visual_token_count = visual_tokens.shape[1]

            # Adjust cache_position to account for visual tokens
            # Shift text cache positions to account for visual tokens
            visual_positions = torch.arange(visual_token_count, dtype=torch.long, device=device)
            adjusted_cache_position = torch.cat([visual_positions, cache_position + visual_token_count])
        else:
            # No visual tokens, use cache_position as-is
            adjusted_cache_position = cache_position

        # Step 2: Process through text decoder
        last_hidden_state = self.forward_text_decoder(input_ids, adjusted_cache_position)

        # Step 3: Apply language modeling head to get logits
