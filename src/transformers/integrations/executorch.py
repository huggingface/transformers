...
class Seq2SeqLMExportableModule(torch.nn.Module):
    ...
    def _export_encoder(self, encoder_input_ids, use_dynamic_shapes=True, strict=True):
        ...
        with torch.no_grad():
            exported_encoder = torch.export.export(
                wrapped_encoder, (encoder_input_ids,), dynamic_shapes=dynamic_shapes, strict=strict
            )

        return exported_encoder

    def _export_decoder(
        self, decoder_input_ids, encoder_hidden_states, cache_position, use_dynamic_shapes=True, strict=True
    ):
        wrapped_decoder = (
    ...
    def export(
        ...
        self.exported_encoder = self._export_encoder(
            example_encoder_input_ids, use_dynamic_shapes=use_dynamic_shapes, strict=strict
        )
    ...
    def generate(self, prompt_token_ids, max_new_tokens):
        with torch.no_grad():
            ...
            decoder_input_ids = torch.tensor([[self.config.bos_token_id]], dtype=torch.long)
            generated_ids = [self.config.bos_token_id]
