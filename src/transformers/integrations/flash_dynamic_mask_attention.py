...
def get_target_dtype(query: torch.Tensor, module: torch.nn.Module) -> torch.dtype:
    """If the query is in float32, return a target dtype compatible with flash dynamic mask attention. Return None otherwise."""
...
def flash_dynamic_mask_attention_forward(
    ...
    **kwargs,
    ...
    if kwargs.get("output_attentions", False):
        logger.warning_once(
            "`flash_dynamic_mask_attention` does not support `output_attentions=True`."
