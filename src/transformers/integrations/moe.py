...
def batched_mm_experts_forward(
    ...
    # The sentinel weights are already zero, so this changes no output. It is the backward that needs it:
    # the gradient of the product below with respect to the weight is the expert output, which the clamp
    # made non-zero, so the router would collect gradient from slots this rank never routed.
