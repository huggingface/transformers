...
def dequantize(data: torch.Tensor, ggml_type: int) -> torch.Tensor:
    """Flat `uint8` GGUF bytes -> flat `float32` values."""
    ...
    values = _DEQUANT[ggml_type](blocks)
...
def _interleave_nibbles(qs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """(nb, 128) nibble bytes -> low/high nibbles as (nb, 4, 32) each.
    ...
    """
    q = qs.int().reshape(-1, 4, 32)
    return q & 0xF, q >> 4


def _dequant_q8_0(blocks: torch.Tensor) -> torch.Tensor:
    d = _half(blocks, 0)
    qs = blocks[:, 2:34].contiguous().view(torch.int8).float()
    return d * qs
...
def _dequant_q4_k(blocks: torch.Tensor) -> torch.Tensor:
    ...
    q = torch.stack([low, high], dim=2).reshape(-1, 8, 32).float()
    return (d[..., None] * scale[..., None]) * q - dmin[..., None] * minimum[..., None]


def _dequant_q5_k(blocks: torch.Tensor) -> torch.Tensor:
    d, dmin = _half(blocks, 0), _half(blocks, 2)
    scale, minimum = _k_scales(blocks[:, 4:16])
    qh = blocks[:, 16:48].int().unsqueeze(1)  # (nb, 1, 32), one extra bit per value
    low, high = _interleave_nibbles(blocks[:, 48:176])
    shift = torch.arange(4, device=blocks.device).reshape(1, 4, 1) * 2
...
def _dequant_q6_k(blocks: torch.Tensor) -> torch.Tensor:
    d = _half(blocks, 208)
    ql, qh = blocks[:, 0:128].int(), blocks[:, 128:192].int()
    ...
    for half in range(2):
        ...
        for quarter, q in enumerate(quants):
            out.append(d * sc[:, which + 2 * quarter] * (q.float() - 32.0))
