...
class TrackioCallback(TrainerCallback):
    ...
    **Requires**:
    ...
    def _space_repo_name_from_trackio_project(project: str) -> str:
        """
        Builds a valid Hugging Face Space repo name segment from `project` for the default static Space created on Hub
        push when `trackio_space_id` is unset (alphanumeric, hyphens, length-limited).
        """
    ...
    def get_trackio_space_id(self, args: TrainingArguments) -> str | None:
        """
        Resolves the Trackio Space id for the current run: context (after `trackio.init`), then `trackio_space_id`, then
        project metadata in the local DB (`SQLiteStorage.get_space_id` may be `None` if unset).
        """
    ...
    def _trackio_space_badge_markdown(self, space_url: str) -> str:
        return (
    ...
    def _prefer_trackio_static_tags(self, model, static_space_id: str) -> None:
        """
        After `freeze`, replaces or appends `model_tags` so the Hub model card points at the static Space URL instead
        of the Gradio Space.
        """
    ...
    def _modelcard_replace_trackio_space_href(self, gradio_space_id: str, static_space_id: str) -> None:
        """
        Rewrites the autogenerated Trainer model card comment so any link to the Gradio Space uses the static Space URL.
        """
    ...
    def _get_space_id_for_hub_push(self, args: TrainingArguments, current_project: str) -> str:
        """
        Returns the Trackio Space id to advertise on the model card: existing context id, or runs `sync` on first Hub
        push, then prefers `_frozen_static_space_id` after training if `freeze` ran.
        """
    ...
    def on_push_begin(self, args, state, control, model, **kwargs):
        ...
        space_id = self._get_space_id_for_hub_push(args, current_project)
        space_url = self.SPACE_URL.format(space_id=space_id)

        badge_markdown = self._trackio_space_badge_markdown(space_url)
