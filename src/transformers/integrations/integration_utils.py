...
class TrackioCallback(TrainerCallback):
    ...
    def _point_model_card_at_space(self, model, modelcard) -> None:
        """
        Adds a link from a model's model card to the Trackio Space (preferring the `static_space_id` if provided).
        ...
        Args:
            ...
            space_id: The Gradio Space ID to link to. May be None if no Gradio Space was used.
            static_space_id: The static Space ID to link to. May be None if no static Space was used.
        """
        if space_id is None and static_space_id is None:
            return
        preferred_space_id = static_space_id or space_id
        ...
        if old_tag:
            old_url = old_tag.split(":")[1]
            old_comment = self.BADGE_MARKDOWN.format(space_url=old_url)
            if old_comment in modelcard.AUTOGENERATED_TRAINER_COMMENT:
                modelcard.AUTOGENERATED_TRAINER_COMMENT = modelcard.AUTOGENERATED_TRAINER_COMMENT.replace(old_comment, new_comment)
        ...
        if preferred_url in c:
            modelcard.AUTOGENERATED_TRAINER_COMMENT = c.replace(old_url, static_url, 1)

    def _freeze_space(self, args: TrainingArguments):
        """
        ...
        if packaging.version.parse(self._trackio.__version__) < packaging.version.parse(
            ...
            return
        self._static_space_id = args.trackio_static_space_id or self._space_repo_name_from_trackio_project(
        ...
        self._point_model_card_at_space(model, modelcard)
