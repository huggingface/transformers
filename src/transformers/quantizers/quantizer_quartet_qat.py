...
class QuartetQatHfQuantizer(HfQuantizer):
    """
    ...
    def validate_environment(self, device_map, **kwargs):
        if not torch.cuda.is_available():
            raise NotImplementedError(
                "Quartet QAT quantization is only supported on GPU. Please use a different quantizer."
            )

        if not is_qutlass_available():
            raise ImportError("Using `quartet_qat` quantization requires qutlass: `pip install qutlass`")

        if not is_quartet_qat_available():
            raise ImportError("Using `quartet_qat` quantization requires quartet_qat: `pip install quartet_qat`")

        if not is_quartet_available():
            raise ImportError("Using `quartet_qat` quantization requires quartet: `pip install quartet`")

        if device_map is None:
            raise ValueError(
                "You are attempting to load a Quartet QAT model without setting device_map."
            ...
            raise ValueError(
                "You are attempting to load a Quartet QAT model with a device_map that contains a CPU or disk device."
    ...
    def _process_model_before_weight_loading(
        ...
        **kwargs,
        ...
        from ..integrations.quartet_qat import adapt_quartet_qat_config

        replace_with_quartet_linear(
            model,
            quartet_linear_config=adapt_quartet_qat_config(self.quantization_config),
        )
    ...
    def update_missing_keys(self, model, missing_keys: List[str], prefix: str) -> List[str]:
        ...
        quartet_qat_names = {name for name, module in model.named_modules() if isinstance(module, QuartetLinear)}

        def should_update(key: str) -> bool:
            if key.endswith(".weight") or key.endswith(".bias"):
                return False
            full_key = f"{prefix}.{key}"
            return any(name in key or name in full_key for name in quartet_qat_names)
