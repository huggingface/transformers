...
class TokenConstraint(Constraint):
    ...
    def __init__(self, token_id: Union[int, torch.LongTensor]):
        ...
        if not (isinstance(token_id, int) or isinstance(token_id, torch.LongTensor)) or token_id < 0:
            raise ValueError(f"`token_id` has to be a positive integer or a `torch.LongTensor` with one positive integeter, but is {token_id}")
        else:
...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: torch.Tensor):
        ...
        if not isinstance(token_ids, torch.Tensor):
            raise ValueError(f"`token_ids` has to be a `torch.Tensor`, but is {type(token_ids)}")
