...
class TokenConstraint(Constraint):
    ...
    """
    def __init__(self, token_id: Union[int, torch.LongTensor]):
        super(Constraint, self).__init__()
        if not (isinstance(token_id, int) or isinstance(token_id, torch.LongTensor)) or token_id < 0:
            raise ValueError(f"`token_id` has to be a positive integer or a `torch.LongTensor` with one positive integer, but is {token_id}")
        else:
            if isinstance(token_id, torch.LongTensor) and token_id.size(0) > 1:
                raise ValueError(f"`token_id` has to be a positive integer or a `torch.LongTensor` with one integer, but is {token_id}."
...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: Union[List[int], torch.LongTensor]):
        ...
        if not (is_int_list or is_long_tensor) or torch.any(token_ids < 0):
            raise ValueError(f"`token_ids` has to be a single list of positive integers or a `torch.LongTensor` but is {token_ids}")
        else:
            if (is_int_list or is_long_tensor) and token_ids.size(0) == 1:
                raise ValueError(f"`token_ids` has to be list of positive integers or a `torch.LongTensor` but is {token_ids}"
...
class ConstraintListState:
    ...
    def advance(self):
        ...
        If we're not in the middle of progressing through a specific constraint c_i, we return:
        ...
        if we are in the progress of fulfilling a constraint, that's the only one we'll return.
        '''
    ...
    def add(self, token_id: Union[int, torch.LongTensor]):
        ...
        if self.inprogress_constraint is not None:
            ...
            If the `token_id` *does* makes an incremental progress to current job, simply update the state
            '''
            ...
            if reset:
                ...
                1. If the next token breaks the progress, then we must restart.
                    ...
                    But that doesn't mean we self.init_state(), since we only reset the state
                    for this particular constraint, not the full list of constraints.
                '''
            ...
            '''
            Not in the middle of fulfilling a constraint. So does this `token_id` helps us step towards
            any of our list of constraints?
            '''
            for cidx, pending_constraint in enumerate(self.pending_constraints):
                if pending_constraint.does_advance(token_id):
                    ...
                    if not stepped:
                        raise Exception("constraint.update(token_id) is not yielding incremental progress, "
                    ...
                    if complete or stepped:
                        ...
                        if len(self.pending_constraints) == 0 and self.inprogress_constraint is None:
                            '''
                            If there's no longer any pending after this and no inprogress either, then we must be complete.
                            '''
                            self.completed = True 
