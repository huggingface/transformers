...
def tensor_to_mask_visual(original_tensor: torch.Tensor, grid_size=(20, 40), style="majong") -> str:
    ...
    original_tensor = (~original_tensor.bool()).float()
    downsampled = False
    col_start, col_end = 0, w
    downsample_ratio = 1

    # Downsample only columns if needed
    if w > max_w:
        downsampled = True
        downsample_ratio = w // max_w
        col_end = w
        new_w = max_w
        tensor = original_tensor.unsqueeze(0).unsqueeze(0)
        tensor = F.adaptive_avg_pool2d(tensor, output_size=(h, new_w))[0, 0]
    else:
    ...
    # Header: Column range and ratio
    col_label = f"Cols {col_start}–{col_end - 1}"
    if downsampled:
        col_label += f" (downsampled ×{downsample_ratio})"
    result.append(col_label)
            ...
            if val == 1:
                ...
                if j > 0:
                    ...
                    if prev == 1:
                        ...
                        row += BLACK_SQUARE if val == 1 else WHITE_SQUARE
                else:
                    if j + 1 < w:
                        next_val = tensor[i, j + 1]
                        row += UPPER_TRIANGLE if next_val == 1 else LOW_TRIANGLE
                    else:
                        row += WHITE_SQUARE
        result.append(row)

    # Optional: Show downsampling legend
    if downsampled:
...
class AttentionMask(torch.Tensor):
    ...
    def __torch_function__(self, func, types, args=(), kwargs=None):
        ...
        # Convert all AttentionMask args to base tensors
        def unwrap(x):
            return x.as_subclass(torch.Tensor) if isinstance(x, AttentionMask) else x

        args = torch.utils._pytree.tree_map_only(torch.Tensor, unwrap, args)
        kwargs = torch.utils._pytree.tree_map_only(torch.Tensor, unwrap, kwargs)

        result = func(*args, **kwargs)

        # Do not propagate subclass
        return result

    def to_string(self, grid_size=(60, 40), limit=4):
        dense_mask = self
        *batch_dims, num_rows, num_cols = dense_mask.shape
        ...
        for idx, batch_idx in enumerate(itertools.product(*[range(i) for i in batch_dims])):
            if idx == limit:
                ...
                break
            mask_slice = dense_mask[batch_idx]
            total_vis.append(tensor_to_mask_visual(mask_slice, grid_size=grid_size, style=self._style))
