...
def create_extra_files(model_type):
    r"""
    Automatically creates all the necessary files for the `model_type` identifier.
    - model_type.md
    - transformers/models/model_type/__init__.py
    - transformers/__init__.py
    - transformers/auto/config_auto.py (add `model_type` to the mapping)
    if there is a processor
        - transformers/auto/processing_auto.py (add `model_type` to the mapping)
    if there is a tokenizer
        - transformers/auto/tokenization_auto.py (add `model_type` to the mapping)
    """
    pass

def create_modular_file(model_type, old_model_type):
    pass
...
def get_user_input():
    ...
    model_type = get_user_field("What is the identifier (`model_type`) of the model you are adding? ")

    # if a `modular_{model_type}` exists, then skip the next step and just create the inits and md and etc.
    create_extra_files(model_type)

    # Get old model type
    valid_model_type = False
    while not valid_model_type:
        old_model_type = input(
            "What is the model you would like inherit from? Please provide the lowercase `model_type` (e.g. roberta): "
        )
    ...
    if not os.path.isfile(f"modular_{model_type}.py"):
        create_modular_file(model_type, old_model_type)
