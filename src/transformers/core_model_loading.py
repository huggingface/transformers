...
class WeightConverter(WeightTransform):
    ...
    def transform_model(self, model: PreTrainedModel, layer_name: str) -> None:
        ...
        result: dict[str, Any] = dict(collected_tensors)
        for op in self.operations:
            result = op.convert(result, source_patterns=self.source_patterns, target_patterns=self.target_patterns)

        # Expand target pattern keys to full parameter names (same logic as convert)
        full_name = layer_name.replace(".*.", ".0.") if ".*." in layer_name else layer_name
        try:
            prefix, _, suffix = next(full_name.partition(k) for k in result if k in full_name)
            result = {prefix + k + suffix: v for k, v in result.items()}
        except StopIteration:
            pass

        # Retrieve one of the source modules to use as original module when creating missing target modules
        original_mod: type[torch.nn.Module] | None = None
        ...
        # Register the target parameters, creating any missing module at the leaf path.
        for target_name, meta_val in result.items():
            meta_tensor = meta_val[0] if isinstance(meta_val, list) else meta_val
            t_mod_path, _, t_attr = target_name.rpartition(".")
            target_mod = _get_submodule_or_create(
                model, t_mod_path, original_module=original_mod, future_weight=meta_tensor
            )
            if not isinstance(meta_tensor, torch.nn.Parameter):
...
def _get_submodule_or_create(
    ...
    if mod_name not in parent_mod._modules:
        cls_ = original_module.__class__ if original_module is not None else torch.nn.Module
        if cls_ is torch.nn.Linear:
        ...
        elif cls_ is torch.nn.Embedding:
