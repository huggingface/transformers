...
class TokenizerChatTemplateKwargs(TypedDict, total=False):
    ...
    Any additional model-specific template kwargs (e.g. ``enable_thinking``, ``reasoning_effort``) are
    automatically detected via Jinja2 template introspection and routed to the template without needing
...
class ProcessorMixin(PushToHubMixin):
    ...
    def apply_chat_template(
        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        # Automatically detect additional kwargs that the template expects via
        # Jinja2 template introspection, so they are routed to the template
        # instead of leaking into processor.__call__().
        template_variables = _get_template_variables(chat_template)
