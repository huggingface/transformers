...
def create_4d_causal_mask(batch_size: int, kv_length: int, cache_position: torch.Tensor, kv_offset: int = 0):
    query_length = cache_position.shape[0]
    causal_mask = torch.tril(
        torch.ones(query_length, kv_length, dtype=torch.bool),
        diagonal=0
    )
    causal_mask *= torch.arange(start=kv_offset, end=kv_offset+kv_length, device=cache_position.device) <= cache_position.reshape(-1, 1)
