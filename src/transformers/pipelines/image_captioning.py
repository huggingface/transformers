...
if is_torch_available():
    from ..models.auto.modeling_auto import MODEL_FOR_IMAGE_CAPTIONING_MAPPING
...
class ImageCaptioningPipeline(Pipeline):
    """
    Image Captioning Pipeline. This pipeline should support conditional (image, text) generation as well as
    unconditional generation. This pipeline is currently only available in PyTorch.
    ...
    This image captioning pipeline can currently be loaded from [`pipeline`] using the following task identifiers:
    `"image-captioning"`.

    The models that this pipeline can use are models that have been fine-tuned on image captioning task. See the
    up-to-date list of available models on
    [huggingface.co/models](https://huggingface.co/models?filter=image-captioning).
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.check_model_type(MODEL_FOR_IMAGE_CAPTIONING_MAPPING)
