        ...
        **kwargs,
        ...
        Args:
            ...
            hypothesis_template (:obj:`str`, `optional`, defaults to :obj:`"This example is {}."`):
                ...
                default template works well in many cases, but it may be worthwhile to experiment with different
                templates depending on the task setting.
            prefix (:obj:`str`, `optional`, defaults to :obj:`"mnli hypothesis: {hypothesis} premise: {premise}"`):
                The prefix that corresponds to the NLI task. This prefix must include the {premise} and {hypothesis}
                placeholders for the sequences and generated hypothesis to be include. For example, the default prefix
                is :obj:`"mnli hypothesis: {hypothesis} premise: {premise}"`. This would be fed into the model like
                :obj:`"mnli hypothesis: hypothesis generated by the hypothesis_template. premise: sequence to
                classify"`. The default template is intended to be use with the original T5 checkpoints, but if a
                custom model is used, this prefix should be adapted to the one used during the training.
            multi_label (:obj:`bool`, `optional`, defaults to :obj:`False`):
                Whether or not multiple candidate labels can be true. If :obj:`False`, the scores are normalized such
                that the sum of the label likelihoods for each sequence is 1. If :obj:`True`, the labels are considered
                independent and probabilities are normalized for each candidate by doing a softmax of the entailment
