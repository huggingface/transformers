...
class TextClassificationPipeline(Pipeline):
    ...
    def _sanitize_parameters(self, return_all_scores=None, function_to_apply=None, top_k="", **tokenizer_kwargs):
        ...
        if isinstance(top_k, int) or top_k is None:
            ...
            warnings.warn(
                "`return_all_scores` is now deprecated,  if want a similar funcionality use `top_k=None` instead of `return_all_scores=True` "
                "or `top_k=1` instead of `return_all_scores=False`.", UserWarning
            )
