...
from .utils import (is_hqq_available, is_optimum_quanto_available,
                    is_torch_greater_or_equal, logging)
...
class OffloadedCache(DynamicCache):
    ...
    def __init__(self) -> None:
        if not (
            ...
            or (is_torch_greater_or_equal("2.7") and torch.xpu.is_available())
        ):
            raise RuntimeError(
                "OffloadedCache can only be used with a GPU"
                + (" or XPU" if is_torch_greater_or_equal("2.7") else "")
            )
        ...
        self.prefetch_stream = (
            torch.Stream() if is_torch_greater_or_equal("2.7") else torch.cuda.Stream()
        )
