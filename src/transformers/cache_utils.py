...
def get_layer_types_and_kwargs(config: PreTrainedConfig) -> tuple[list[str], dict]:
    ...
    if any(
        layer_type in ("heavily_compressed_attention", "compressed_sparse_attention", "helix", "helix_local")
        for layer_type in layer_types
    ):
        layer_kwargs["config"] = config
    # We may need more than 1 conv/recurrent state
    if any(
        layer_type in ("conv", "linear_attention", "hybrid", "hybrid_sliding", "helix", "helix_local")
        for layer_type in layer_types
    ):
