...
class StaticCache(Cache):
    ...
    def __init__(
        ...
        self.dtype = dtype
        self.device = (
            torch.device(device) if device is not None else torch.device("meta")
        )
    ...
    def update(
        ...
        if self.key_cache[layer_idx].device.type == "meta":
            self.key_cache[layer_idx] = torch.zeros_like(
                self.key_cache[layer_idx], device=key_states.device
            )
            self.value_cache[layer_idx] = torch.zeros_like(
                self.value_cache[layer_idx], device=value_states.device
            )
