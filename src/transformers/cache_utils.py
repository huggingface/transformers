...
class SharedCache(DynamicCache):
    """
    A shared cache that grows dynamically as more tokens are generated.

    It stores Key and Value states as lists of tensors, shared between groups of layers, one tensor of shape `[batch_size, num_heads, seq_len, head_dim]` per group of layers, the rest are empty lists.
    ...
    def shared_layer_idx(self, layer_idx: int) -> int:
        if len(self.shared_layer_indices) == 0:
            return layer_idx
        return min(self.shared_layer_indices, key=lambda idx: abs(idx - layer_idx))
    ...
    def update(
        ...
        """
        Update the cache with the new `key_states` and `value_states` for the shared cache group of the layer `layer_idx`.
        ...
        Parameters:
            ...
            layer_idx (`int`):
                The index of the layer to cache the states for.
            cache_kwargs (`Dict[str, Any]`, `optional`):
                Additional arguments for the cache subclass. No additional arguments are used in `SharedCache`.
        ...
        if key_states is not None:
            if len(self.key_cache) <= layer_idx:
                ...
                for _ in range(len(self.key_cache), layer_idx + 1):
                    self.key_cache.append([])
                    self.value_cache.append([])
            shared_layer_idx = self.shared_layer_idx(layer_idx)
            if (
                len(self.key_cache) > layer_idx
                and layer_idx in self.shared_layer_indices
                and len(self.key_cache[shared_layer_idx]) == 0
            ):  # fills previously skipped layers; checking for tensor causes errors
                self.key_cache[shared_layer_idx] = key_states
                self.value_cache[shared_layer_idx] = value_states
            elif layer_idx not in self.shared_layer_indices:
                pass
            else:
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        is_empty_layer = (
            len(self.key_cache) == 0  # no cache in any layer
            or len(self.key_cache) <= layer_idx  # skipped `layer_idx` and hasn't run a layer with cache after it
            or len(self.key_cache[shared_layer_idx]) == 0  # the layer has no cache
        )
        layer_seq_length = self.key_cache[shared_layer_idx].shape[-2] if not is_empty_layer else 0
        return layer_seq_length
    ...
    def from_legacy_cache(
        ...
        """Converts a cache in the legacy cache format into an equivalent `SharedCache`. Used for
        backward compatibility."""
        cache = cls()
        if past_key_values is not None:
            for layer_idx in range(len(past_key_values)):
