    ...
    def preprocess(
        ...
        **kwargs: Unpack[DefaultFastImageProcessorPreprocessKwargs],
        ...
        validate_kwargs(
            ...
            valid_processor_keys=self.valid_preprocess_kwargs.__annotations__.keys(),
        )
        ...
        size = (
            SizeDict(**get_size_dict(size=size, default_to_square=default_to_square))
            if size is not None
            else None
        )
        crop_size = (
            SizeDict(**get_size_dict(crop_size, param_name="crop_size"))
            if crop_size is not None
            else None
        )
        ...
        image_mean, image_std, interpolation = self._prepare_process_arguments(
            ...
            image_std=image_std,
            data_format=(
                data_format if data_format is not None else ChannelDimension.FIRST
            ),
