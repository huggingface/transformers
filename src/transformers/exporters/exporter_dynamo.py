...
def _reshaped_vision_attention_forward(
    ...
    **kwargs,
    ...
    if position_embeddings is not None:
        ...
        # q/k. Preserving the trailing (heads, head_dim) keeps this a strict no-op for the plain
        # (seq, heads, head_dim) encoders and for asymmetric GQA k/v — it only collapses the extra
        # leading dim.
        query_states = query_states.reshape(seq_length, *query_states.shape[-2:])
        key_states = key_states.reshape(seq_length, *key_states.shape[-2:])
    ...
    # The `.clone()` materialises the post-transpose tensor so the reshape stays a valid view:
    # executorch's edge-lowering reshape ref refuses to copy a non-contiguous tensor, and a plain
    # `.contiguous()` gets folded away by functionalization (a `.clone()` survives). Only bites
    # multi-segment grids (n_seg > 1); single-image encoders reshape a size-1 leading dim.
    attn_output = attn_output.transpose(1, 2).clone().reshape(seq_length, -1)
