        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    # Debug logging for multi-GPU token calculation
                    if num_items_in_batch is not None and self.args.n_gpu > 1:
                        ...
                        if "labels" in inputs:
                            try:
                                ...
                                print(f"[OLD TRAINER DEBUG] Full batch tokens: {full_batch_tokens.item()}")
                                print(f"[OLD TRAINER DEBUG] Tokens per GPU: {tokens_per_gpu.item()}")
                                print(f"[OLD TRAINER DEBUG] Input labels shape: {inputs['labels'].shape}")
                                print(f"[OLD TRAINER DEBUG] Using num_items_in_batch: {num_items_in_batch.item()}")

                            except (TypeError, AttributeError):
                                print(
                                    f"[OLD TRAINER DEBUG] Could not calculate tokens, using num_items_in_batch: {num_items_in_batch.item()}"
                                )
