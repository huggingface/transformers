...
"""
This script converts a lm-head checkpoint from the "Token Dropping" implementation into a PyTorch-compatible JINABERT
...
from transformers import JinaBertConfig, JinaBertForMaskedLM
from transformers.models.jina_bert.modeling_jina_bert import (
    JinaBertIntermediate,
    JinaBertLayer,
    JinaBertOutput,
    JinaBertPooler,
    JinaBertSelfAttention,
    JinaBertSelfOutput,
)
...
def convert_checkpoint_to_pytorch(tf_checkpoint_path: str, config_path: str, pytorch_dump_path: str):
    ...
    config = JinaBertConfig.from_json_file(config_path)
    model = JinaBertForMaskedLM(config)

    # Layers
    for layer_index in range(0, config.num_hidden_layers):
        layer: JinaBertLayer = model.jina_bert.encoder.layer[layer_index]

        # Self-attention
        self_attn: JinaBertSelfAttention = layer.attention.self
        ...
        self_output: JinaBertSelfOutput = layer.attention.output
        ...
        # Intermediate
        intermediate: JinaBertIntermediate = layer.intermediate
        ...
        # Output
        jina_bert_output: JinaBertOutput = layer.output

        jina_bert_output.dense.weight.data = get_encoder_layer_array(layer_index, "_output_dense/kernel")
        jina_bert_output.dense.bias.data = get_encoder_layer_array(layer_index, "_output_dense/bias")

        jina_bert_output.LayerNorm.weight.data = get_encoder_layer_array(layer_index, "_output_layer_norm/gamma")
    ...
    model.jina_bert.embeddings.word_embeddings.weight.data = get_masked_lm_array("embedding_table")

    # Pooling
    model.jina_bert.pooler = JinaBertPooler(config=config)
    model.jina_bert.pooler.dense.weight.data: JinaBertPooler = get_encoder_array("_pooler_layer/kernel")
    model.jina_bert.pooler.dense.bias.data: JinaBertPooler = get_encoder_array("_pooler_layer/bias")

    # Export final model
    model.save_pretrained(pytorch_dump_path)

    # Integration test - should load without any errors ;)
    new_model = JinaBertForMaskedLM.from_pretrained(pytorch_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--jina_bert_config_file",
        type=str,
        required=True,
        help="The config json file corresponding to the JINABERT model. This specifies the model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path",
        type=str,
    ...
    convert_checkpoint_to_pytorch(args.tf_checkpoint_path, args.jina_bert_config_file, args.pytorch_dump_path)
