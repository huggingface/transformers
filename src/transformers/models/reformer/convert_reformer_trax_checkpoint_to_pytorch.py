...
def set_param(torch_layer, weight, bias=None):
    ...
    if torch_layer.weight.shape != weight.shape:
        raise ValueError(f"{torch_layer} layer.weight does not match")
    torch_layer.weight = nn.Parameter(weight)
    if bias is not None:
        if torch_layer.bias.shape != bias.shape:
            raise ValueError(f"{torch_layer} layer.bias does not match")
...
def set_model_weights_in_torch(weights, torch_model, hidden_size):
    ...
    if isinstance(weights[3], tuple):
        ...
        for emb_idx in range(len(position_embeddings.weights)):
            ...
            if position_embeddings.weights[emb_idx].shape != emb_weights.shape:
                raise ValueError(f"{position_embeddings[emb_idx]} emb does not match")
            position_embeddings.weights[emb_idx] = nn.Parameter(torch.tensor(emb_weights))

    trax_layer_weights = weights[5]
    if len(torch_model_reformer.encoder.layers) * 4 != len(trax_layer_weights):
        raise ValueError("HF and trax model do not have the same number of layers")
