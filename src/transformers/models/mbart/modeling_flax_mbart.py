...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartDecoderPreTrainedModel with Bart->MBart, BART->MBART
class FlaxMBartDecoderPreTrainedModel(FlaxPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(MBART_DECODE_INPUTS_DOCSTRING)
    def __call__(
        self,
        input_ids: jnp.ndarray,
        attention_mask: Optional[jnp.ndarray] = None,
...
class FlaxMBartDecoderWrapper(nn.Module):
    ...
    def setup(self):
        ...
        self.decoder = FlaxMBartDecoder(config=self.config, embed_tokens=embed_tokens, dtype=self.dtype)
...
class FlaxMBartForCausalLMModule(nn.Module):
    ...
    def setup(self):
        self.model = FlaxMBartDecoderWrapper(config=self.config, dtype=self.dtype)
        self.lm_head = nn.Dense(
            self.config.vocab_size,
            use_bias=False,
            dtype=self.dtype,
...
@add_start_docstrings(
    """
    MBart Decoder Model with a language modeling head on top (linear layer with weights tied to the input embeddings)
