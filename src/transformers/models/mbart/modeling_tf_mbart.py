...
def shift_tokens_right(input_ids: tf.Tensor, pad_token_id: int):
    ...
    input_ids = tf.where(input_ids == -100, tf.fill(tf.shape(input_ids), pad_token_id), input_ids)
    language_id_index = (
        tf.reduce_sum(tf.cast(tf.math.not_equal(input_ids, pad_token_id), dtype=input_ids.dtype), axis=-1) - 1
    )
    language_id_index = tf.stack([tf.range(tf.shape(input_ids)[0]), language_id_index], axis=-1)
