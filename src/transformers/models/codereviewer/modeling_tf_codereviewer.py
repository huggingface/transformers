# coding=utf-8
# Copyright 2024 CodeReviewer Authors and The HuggingFace Inc. team.
...
TF_CODEREVIEWER_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    # See all CodeReviewer models at https://huggingface.co/models?filter=codereviewer
]
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerNorm with T5->CodeReviewer
class TFCodeReviewerLayerNorm(keras.layers.Layer):
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5DenseActDense with T5->CodeReviewer
class TFCodeReviewerDenseActDense(keras.layers.Layer):
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5DenseGatedActDense with T5->CodeReviewer
class TFCodeReviewerDenseGatedActDense(keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        wi_initializer = keras.initializers.RandomNormal(
            mean=0, stddev=config.initializer_factor * (config.d_model**-0.5)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerFF with T5->CodeReviewer
class TFCodeReviewerLayerFF(keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        if config.is_gated_act:
            self.DenseReluDense = TFCodeReviewerDenseGatedActDense(config, name="DenseReluDense")
        else:
            self.DenseReluDense = TFCodeReviewerDenseActDense(config, name="DenseReluDense")

        self.layer_norm = TFCodeReviewerLayerNorm(config.d_model, epsilon=config.layer_norm_epsilon, name="layer_norm")
        self.dropout = keras.layers.Dropout(config.dropout_rate)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5Attention with T5->CodeReviewer
class TFCodeReviewerAttention(keras.layers.Layer):
    NEW_ID = itertools.count()

    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.layer_id = next(TFCodeReviewerAttention.NEW_ID)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerSelfAttention with T5->CodeReviewer
class TFCodeReviewerLayerSelfAttention(keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.SelfAttention = TFCodeReviewerAttention(
            config,
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerCrossAttention with T5->CodeReviewer
class TFCodeReviewerLayerCrossAttention(keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        self.EncDecAttention = TFCodeReviewerAttention(
            config,
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5Block with T5->CodeReviewer
class TFCodeReviewerBlock(keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.is_decoder = config.is_decoder
        self.layer = []
        self.layer.append(
            TFCodeReviewerLayerSelfAttention(
                config,
                has_relative_attention_bias=has_relative_attention_bias,
                name="layer_._0",
            )
        ...
        if self.is_decoder:
            self.layer.append(
                TFCodeReviewerLayerCrossAttention(
                    config,
                    name="layer_._1",
                )
            )

        self.layer.append(TFCodeReviewerLayerFF(config, name=f"layer_._{len(self.layer)}"))
...
# provided as a keras.layers.Layer usually called "TFCodeReviewerMainLayer"
####################################################
@keras_serializable
# Copied from transformers.models.t5.modeling_tf_t5.TFT5MainLayer with T5->CodeReviewer
class TFCodeReviewerMainLayer(keras.layers.Layer):
    ...
    def __init__(self, config, embed_tokens=None, **kwargs):
        ...
        self.block = [
            TFCodeReviewerBlock(config, has_relative_attention_bias=bool(i == 0), name=f"block_._{i}")
            for i in range(config.num_layers)
        ]
        self.final_layer_norm = TFCodeReviewerLayerNorm(
    ...
    def call(
        ...
        # CodeReviewer has a mask that can compare sequence ids, we can simulate this here with this transposition
...
####################################################
# TFCodeReviewerPreTrainedModel is a sub-class of keras.Model
# which take care of loading and saving pretrained weights
# and various common utilities.
# Here you just need to specify a few (self-explanatory)
# pointers for your model.
####################################################
# Copied from transformers.models.t5.modeling_tf_t5.TFT5PreTrainedModel with T5->CodeReviewer
class TFCodeReviewerPreTrainedModel(TFPreTrainedModel):
    """
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            "self.model.config.decoder_start_token_id has to be defined. In TF CodeReviewer it is usually set to the"
            " pad_token_id. See CodeReviewer docs for more information"
        )
...
CODEREVIEWER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`tf.Tensor` of shape `(batch_size, sequence_length)`):
            ...
            To know more on how to prepare `inputs` for pretraining take a look at [CODEREVIEWER Training](./codereviewer#training).
        decoder_input_ids (`tf.Tensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            Provide for sequence to sequence training. CODEREVIEWER uses the `pad_token_id` as the starting token for
...
CODEREVIEWER_ENCODER_INPUTS_DOCSTRING = r"""
    Args:
        inputs (`tf.Tensor` of shape `(batch_size, sequence_length)`):
            ...
            To know more on how to prepare `inputs` for pre-training take a look at [CODEREVIEWER Training](./codereviewer#training).
        attention_mask (`tf.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Mask to avoid performing attention on padding token indices. Mask values selected in `[0, 1]`:
...
class TFCodeReviewerModel(TFCodeReviewerPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = TFCodeReviewerMainLayer(encoder_config, self.shared, name="encoder")

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = TFCodeReviewerMainLayer(decoder_config, self.shared, name="decoder")
    ...
    def call(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TFCodeReviewerModel
...
class TFCodeReviewerForConditionalGeneration(TFCodeReviewerPreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def call(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TFCodeReviewerForConditionalGeneration
        ...
        # CodeReviewerv1.1 does not tie output word embeddings and thus does not require downscaling
        if self.config.tie_word_embeddings:
...
@add_start_docstrings(
    "The bare CODEREVIEWER Model transformer outputting encoder's raw hidden-stateswithout any specific head on top.",
    CODEREVIEWER_START_DOCSTRING,
)
class TFCodeReviewerEncoderModel(TFCodeReviewerPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.shared = keras.layers.Embedding(
            config.vocab_size,
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(CODEREVIEWER_ENCODER_INPUTS_DOCSTRING)
    ...
    def call(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TFCodeReviewerEncoderModel

        >>> tokenizer = AutoTokenizer.from_pretrained("microsoft/codereviewer")
        >>> model = TFCodeReviewerEncoderModel.from_pretrained("microsoft/codereviewer")
