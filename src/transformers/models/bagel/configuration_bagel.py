...
class BagelVQVAEConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`BagelVQVAEModel`]. It is used to instantiate a
    `BagelVQVAEModel` according to the specified arguments, defining the model architecture.
    Configuration objects inherit from [`PretrainedConfig`] and can be used to control the model outputs. Read the
    documentation from [`PretrainedConfig`] for more information. Instantiating a
    configuration with the defaults will yield a similar configuration to the VQModel of the
    [deepseek-community/Bagel-Pro-1B](https://huggingface.co/deepseek-community/Bagel-Pro-1B).
...
class BagelConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`BagelModel`]. It is used to instantiate an
    Bagel model according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of the Bagel-1B or Bagel-7B models.

    e.g. [deepseek-community/Bagel-Pro-1B](https://huggingface.co/deepseek-community/Bagel-Pro-1B) or
    [deepseek-community/Bagel-Pro-7B](https://huggingface.co/deepseek-community/Bagel-Pro-7B)
    ...
    Args:
        ...
        vision_config (`Union[AutoConfig, dict]`,  *optional*, defaults to `BagelVisionConfig`):
            The config object or dictionary of the vision backbone.
        vq_config (`Union[AutoConfig, dict]`,  *optional*, defaults to `BagelVQVAEConfig`):
    ...
    ```python
    >>> from transformers import BagelForConditionalGeneration, BagelConfig, BagelVisionConfig, BagelVQVAEConfig, LlamaConfig

    >>> # Initializing a Bagel vision config
    >>> vision_config = BagelVisionConfig()
    ...
    >>> # Initializing a Bagel Pro 1B style configuration
    >>> configuration = BagelConfig(vision_config=vision_config, text_config=text_config, vq_config=vq_config)

    >>> # Initializing a model from the Bagel Pro 1B style configuration
    >>> model = BagelForConditionalGeneration(configuration)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vision_config is None:
            logger.info("`vision_config` is None. Initializing with default BagelVisionConfig values")
            self.vision_config = BagelVisionConfig()
        elif isinstance(vision_config, dict):
            self.vision_config = BagelVisionConfig(**vision_config)
        elif isinstance(vision_config, BagelVisionConfig):
            self.vision_config = vision_config
        else:
            raise ValueError(
                f"Invalid type for `vision_config`. Must be either `dict` or `BagelVisionConfig`."
                f" Type found: {type(vision_config)}"
            )

        if vq_config is None:
            logger.info("`vq_config` is None. Initializing with default BagelVQVAEConfig values")
            ...
            raise ValueError(
                f"Invalid type for `vq_config`. Must be either `dict` or `BagelVQVAEConfig`."
...
__all__ = ["BagelVQVAEConfig", "BagelVisionConfig", "BagelConfig"]
