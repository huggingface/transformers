...
@add_start_docstrings(
    """LiLT Model with a `language modeling` head on top for CLM fine-tuning.""", LILT_START_DOCSTRING
)
# Copied from transformers.models.roberta.modeling_roberta.RobertaForCausalLM with ROBERTA->LILT,Roberta->Lilt,roberta->lilt,roberta-base->organization/lilt-roberta-en-base
class LiltForCausalLM(LiltPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `LiltLMHeadModel` as a standalone, add `is_decoder=True.`")
...
@add_start_docstrings("""LiLT Model with a `language modeling` head on top.""", LILT_START_DOCSTRING)
# Copied from transformers.models.roberta.modeling_roberta.RobertaForMaskedLM with ROBERTA->LILT,Roberta->Lilt,roberta->lilt
class LiltForMaskedLM(LiltPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `LiltForMaskedLM` make sure `config.is_decoder=False` for "
...
# Copied from transformers.models.roberta.modeling_roberta.RobertaLMHead with Roberta->Lilt
...
@add_start_docstrings(
    """
    LiLT Model transformer with a sequence classification/regression head on top (a linear layer on top of the
...
@add_start_docstrings(
    """
    Lilt Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
    softmax) e.g. for RocStories/SWAG tasks.
    """,
    LILT_START_DOCSTRING,
)
# Copied from transformers.models.roberta.modeling_roberta.RobertaForMultipleChoice with ROBERTA->LILT,Roberta->Lilt,roberta->lilt
class LiltForMultipleChoice(LiltPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config):
        super().__init__(config)

        self.lilt = LiltModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    @add_start_docstrings_to_model_forward(LILT_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
class LiltForTokenClassification(LiltPreTrainedModel):
    ...
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint="Jean-Baptiste/lilt-large-ner-english",
