...
_import_structure = {
    "configuration_gptmeg": ["GPTMEG_PRETRAINED_CONFIG_ARCHIVE_MAP", "GPTMegConfig"],
    "tokenization_gptmeg": ["GPTMegTokenizer"],
}

if is_tokenizers_available():
    _import_structure["tokenization_gptmeg_fast"] = ["GPTMegTokenizerFast"]

if is_torch_available():
    _import_structure["modeling_gptmeg"] = [
...
if TYPE_CHECKING:
    ...
    if is_tokenizers_available():
        from .tokenization_gptmeg_fast import GPTMegTokenizerFast

    if is_torch_available():
        from .modeling_gptmeg import (
            GPTMEG_PRETRAINED_MODEL_ARCHIVE_LIST,
            GPTMegDoubleHeadsModel,
            GPTMegForSequenceClassification,
            GPTMegForTokenClassification,
