...
class OpusState:
    ...
    def _check_layer_entries(self):
        ...
        if len(self.encoder_l1) != 16:
            logger.warning(f"Expected 16 keys for each encoder layer, got {len(self.encoder_l1)}")
        if len(self.decoder_l1) != 26:
            logger.warning(f"Expected 26 keys for each decoder layer, got {len(self.decoder_l1)}")
