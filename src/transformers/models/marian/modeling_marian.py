...
class MarianMTModel(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig):
        ...
        output_size = config.vocab_size if config.share_encoder_decoder_embeddings else config.decoder_vocab_size
        self.register_buffer("final_logits_bias", torch.zeros((1, output_size)))
        self.lm_head = nn.Linear(config.d_model, output_size, bias=False)
