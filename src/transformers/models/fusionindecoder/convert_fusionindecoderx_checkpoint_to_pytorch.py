...
"""
Convert FusionInDecoderX checkpoint to PyTorch

Steps:
- Install gsutil according to https://cloud.google.com/storage/docs/gsutil_install
- Get a FusionInDecoderX checkpoint at https://github.com/google-research/fusionindecoderx/blob/main/docs/models.md#fusionindecoder-11-checkpoints Example:
    `gsutil -m cp -r gs://fusionindecoder-data/pretrained_models/fusionindecoderx/fusionindecoder_1_1_small $HOME/`
- Create or download a corresponding config for the downloaded model. E.g. for FusionInDecoder v1.1 small, you can use
    https://huggingface.co/google/fusionindecoder-v1_1-small/blob/main/config.json
- Convert:
    ```
    python3 convert_fusionindecoderx_checkpoint_to_pytorch.py --fusionindecoderx_checkpoint_path=$HOME/fusionindecoder_1_1_small --config_file=config.json\
      --pytorch_dump_path=$HOME/fusionindecoder_1_1_small_pt
    ```
"""
...
def fusionindecoderx_attention_lookup(params, i, prefix, layer_name="attention"):
...
def fusionindecoderx_mlp_lookup(params, i, prefix, split_mlp_wi=False):
...
def fusionindecoderx_layer_norm_lookup(params, i, prefix, layer_name):
...
def convert_fusionindecoderx_to_pytorch(variables: dict, *, num_layers: int, num_decoder_layers: int, is_encoder_only: bool):
    ...
    for i in range(num_layers):
        ...
        layer_norm = fusionindecoderx_layer_norm_lookup(old, i, "encoder", "pre_attention_layer_norm")
        k, o, q, v = fusionindecoderx_attention_lookup(old, i, "encoder", "attention")
        ...
        # Block i, layer 1 (MLP).
        layer_norm = fusionindecoderx_layer_norm_lookup(old, i, "encoder", "pre_mlp_layer_norm")
        wi, wo = fusionindecoderx_mlp_lookup(old, i, "encoder", split_mlp_wi)
    ...
    if not is_encoder_only:
        ...
        for i in range(num_decoder_layers):
            # Block i, layer 0 (Self Attention).
            layer_norm = fusionindecoderx_layer_norm_lookup(old, i, "decoder", "pre_self_attention_layer_norm")
            k, o, q, v = fusionindecoderx_attention_lookup(old, i, "decoder", "self_attention")
            ...
            layer_norm = fusionindecoderx_layer_norm_lookup(old, i, "decoder", "pre_cross_attention_layer_norm")
            k, o, q, v = fusionindecoderx_attention_lookup(old, i, "decoder", "encoder_decoder_attention")
            ...
            # Block i, layer 2 (MLP).
            layer_norm = fusionindecoderx_layer_norm_lookup(old, i, "decoder", "pre_mlp_layer_norm")
            wi, wo = fusionindecoderx_mlp_lookup(old, i, "decoder", split_mlp_wi)
...
def load_fusionindecoderx_weights_in_fusionindecoder(model, config, fusionindecoderx_checkpoint_path, is_encoder_only):
    """Replaces the params in model witht the FusionInDecoderX converted params."""
    variables = checkpoints.load_fusionindecoderx_checkpoint(fusionindecoderx_checkpoint_path)
    converted = convert_fusionindecoderx_to_pytorch(
        variables,
...
def convert_fusionindecoderx_checkpoint_to_pytorch(
    fusionindecoderx_checkpoint_path, config_file, pytorch_dump_path, is_encoder_only: bool = False
):
    """Loads the config and model, converts the FusionInDecoderX checkpoint, and saves a PyTorch checkpoint."""
    # Initialise PyTorch model
    config = FusionInDecoderConfig.from_json_file(config_file)
    ...
    load_fusionindecoderx_weights_in_fusionindecoder(model, config, fusionindecoderx_checkpoint_path, is_encoder_only)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the pre-trained FusionInDecoder model.\nThis specifies the model architecture.",
    )
    parser.add_argument(
    ...
    convert_fusionindecoderx_checkpoint_to_pytorch(
        args.fusionindecoderx_checkpoint_path, args.config_file, args.pytorch_dump_path, args.is_encoder_only
    )
