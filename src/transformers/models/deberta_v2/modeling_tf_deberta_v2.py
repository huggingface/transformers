...
def take_along_axis(x, indices, gather_axis):
    if gather_axis < 0:
        gather_axis = x.shape.rank + gather_axis

    if gather_axis != x.shape.rank - 1:
        pre_roll = x.shape.rank - 1 - gather_axis
        permutation = tf.roll(tf.range(x.shape.rank), pre_roll, axis=0)
