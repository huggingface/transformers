...
class CsmForCausalLM(LlamaForCausalLM, GenerationMixin):
    ...
    def _prepare_generation_config(
        ...
        depth_decoder_kwargs = {
            k[len("depth_decoder_"):]: v for k, v in kwargs.items()
        ...
        from_generation_config = {
            k[len("depth_decoder_"):]: v for k, v in generation_config.to_diff_dict().items()
            if k.startswith("depth_decoder_")
        }
        ...
        depth_decoder_min_new_tokens = getattr(depth_decoder_generation_config, 'min_new_tokens') or (self.config.num_codebooks - 1)
        depth_decoder_max_new_tokens = getattr(depth_decoder_generation_config, 'max_new_tokens') or (self.config.num_codebooks - 1)
