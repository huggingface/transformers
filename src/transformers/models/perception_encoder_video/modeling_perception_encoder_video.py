...
class PerceptionEncoderVideoEmbeddings(torch.nn.Module):
    def __init__(self, config: PerceptionEncoderVideoTransformerConfig):
        super().__init__()
        self.config = config
...
class PerceptionEncoderVideoTransformer(torch.nn.Module):
    _can_record_outputs = {
        "attentions": PerceptionEncoderVideoAttention,
        "hidden_states": PerceptionEncoderVideoDecoderLayer,
    }

    def __init__(self, config: PerceptionEncoderVideoTransformerConfig):
        super().__init__()
        self.config = config
        self.embeddings = PerceptionEncoderVideoEmbeddings(config)
        self.layers = torch.nn.ModuleList(
            [PerceptionEncoderVideoDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
