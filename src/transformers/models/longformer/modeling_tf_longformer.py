...
class TFLongformerEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.attention_window = (
            config.attention_window if isinstance(config.attention_window, int) else max(config.attention_window)
        )
        self.config = config
        self.layer = [
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["global_attention_mask"] is None and inputs["input_ids"] is not None:
            if (
                shape_list(tf.where(inputs["input_ids"] == self.config.sep_token_id))[0] != 3 * shape_list(inputs["input_ids"])[0]
            ):
                logger.warning(
