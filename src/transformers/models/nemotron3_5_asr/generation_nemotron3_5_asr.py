...
This mirrors the RNN-T generation machinery of [`NemotronAsr`], adding language-ID prompt
conditioning: the target language is fixed for a whole utterance/stream, so `generate` stashes
`prompt_ids` once and `get_audio_features` (in the modeling file) reads it for both the offline encode
and every streaming chunk.
"""
...
class Nemotron3_5AsrRNNTDecoderCache:
...
class Nemotron3_5AsrEncoderExhaustedCriteria(StoppingCriteria):
...
@dataclass
class Nemotron3_5AsrGenerateOutput(ModelOutput):
    """
    Outputs of Nemotron3_5Asr RNN-T generation.
...
class Nemotron3_5AsrGenerationMixin(GenerationMixin):
    """Generation mixin for Nemotron3_5Asr RNN-T models.

    Handles the transducer machinery (encoder frame tracking, decoder cache preparation,
    encoder-exhaustion stopping, output-buffer sizing) exactly like [`NemotronAsr`], plus language-ID
    prompt conditioning: the target language is fixed for a whole utterance/stream, so `generate`
    stashes `prompt_ids` once and `get_audio_features` reads it for the offline encode and every
    streaming chunk.
    """

    def _get_stopping_criteria(self, *args, **kwargs):
        criteria = super()._get_stopping_criteria(*args, **kwargs)
        criteria.append(Nemotron3_5AsrEncoderExhaustedCriteria(self))
        return criteria
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, *args, **kwargs):
        ...
        # Recompute exhaustion now that the buffer may have grown (drives Nemotron3_5AsrEncoderExhaustedCriteria).
        self._encoder_finished = model_kwargs["encoder_frame_idxs"] >= model_kwargs["encoder_valid_lengths"]
        return model_kwargs
    ...
    def _prepare_generated_length(
        ...
        # stopping is handled by Nemotron3_5AsrEncoderExhaustedCriteria; this just sizes the buffer generously.
    ...
    def _prepare_model_inputs(self, inputs=None, bos_token_id=None, model_kwargs=None):
        from ..nemotron_asr.modeling_nemotron_asr import (
            NemotronAsrEncoderCausalConvPaddingCache as Nemotron3_5AsrEncoderCausalConvPaddingCache,
        )
        from ..nemotron_asr.modeling_nemotron_asr import (
            NemotronAsrEncoderModelOutput as Nemotron3_5AsrEncoderModelOutput,
        )
        ...
        # passed -> a sliding-window DynamicCache from encoder_config.sliding_window).
        if isinstance(input_features, GeneratorType):
            ...
            padding_cache = Nemotron3_5AsrEncoderCausalConvPaddingCache()
            ...
            model_kwargs["encoder_outputs"] = Nemotron3_5AsrEncoderModelOutput(
    ...
    def _prepare_cache_for_generation(self, generation_config, model_kwargs, *args, **kwargs):
        model_kwargs["decoder_cache"] = Nemotron3_5AsrRNNTDecoderCache(self.config)
    ...
    def prepare_inputs_for_generation(self, input_ids, *args, **kwargs):
        ...
        model_inputs["encoder_outputs"] = Nemotron3_5AsrEncoderModelOutput(
    ...
    def generate(self, inputs=None, generation_config=None, **kwargs):
        ...
        return Nemotron3_5AsrGenerateOutput(sequences=sequences)
