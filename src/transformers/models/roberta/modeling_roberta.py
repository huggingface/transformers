...
class RobertaAttention(nn.Module):
    ...
    ROBERTA historically split this over `RobertaSelfAttention` (q/k/v) and `RobertaSelfOutput` (the output
    projection plus the residual LayerNorm). The projection lives here as `o_proj`; the LayerNorm
    and residual belong to `RobertaLayer`, which is what makes the layer's topology visible.
    """
...
class RobertaCrossAttention(nn.Module):
    """Cross-attention query/key/value. Its output projection is `RobertaSelfOutput`, see above."""
...
# self-attention path no longer uses this: its projection is `RobertaAttention.o_proj` and its
# LayerNorm belongs to `RobertaLayer`. Cross-attention keeps the original layout deliberately, so that
...
class RobertaCrossAttentionBlock(nn.Module):
    """Cross-attention plus its output projection, in ROBERTA's original module layout."""

    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.self = RobertaCrossAttention(config, is_causal=False, layer_idx=layer_idx)
...
class RobertaMLP(nn.Module):
    ...
    Replaces `RobertaIntermediate` (up projection plus activation) and `RobertaOutput` (down projection
    plus the residual LayerNorm); the LayerNorm and residual now belong to `RobertaLayer`.
    """
...
class RobertaLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.self_attn = RobertaAttention(config, is_causal=config.is_decoder, layer_idx=layer_idx)
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = RobertaCrossAttentionBlock(config, layer_idx=layer_idx)
        self.mlp = RobertaMLP(config)
