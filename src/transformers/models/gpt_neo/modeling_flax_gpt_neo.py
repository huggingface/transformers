...
class FlaxGPTNeoLMHeadModel(FlaxGPTNeoPreTrainedModel):
    module_class = FlaxGPTNeoLMHeadModule

    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        token_type_ids = kwargs.get("token_type_ids", None)
        # only last token for inputs_ids if past is defined in kwargs
        if past:
            input_ids = jnp.expand_dims(input_ids[:, -1], -1)
            if token_type_ids is not None:
                token_type_ids = jnp.expand_dims(token_type_ids[:, -1], -1)
        ...
        if attention_mask is not None and position_ids is None:
            ...
            if past:
                position_ids = jnp.expand_dims(position_ids[:, -1], -1)
        else:
            position_ids = None
        return {
            "input_ids": input_ids,
...
append_call_sample_docstring(
    FlaxGPTNeoLMHeadModel, _TOKENIZER_FOR_DOC, _CHECKPOINT_FOR_DOC, FlaxCausalLMOutput, _CONFIG_FOR_DOC
)
