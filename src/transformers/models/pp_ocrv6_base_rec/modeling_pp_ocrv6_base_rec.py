...
from .configuration_pp_ocrv6_base_rec import PPOCRV6BaseRecConfig


class PPOCRV6BaseRecConvLayer(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class PPOCRV6BaseRecAttention(nn.Module):
...
class PPOCRV6BaseRecMLP(nn.Module):
...
class PPOCRV6BaseRecBlock(GradientCheckpointingLayer):
    def __init__(self, config):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = PPOCRV6BaseRecAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.mlp = PPOCRV6BaseRecMLP(
            config=config,
...
@auto_docstring
class PPOCRV6BaseRecPreTrainedModel(PreTrainedModel):
    config: PPOCRV6BaseRecConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["PPOCRV6BaseRecBlock"]
    ...
    _can_record_outputs = {
        "hidden_states": PPOCRV6BaseRecBlock,
    }
    main_input_name = "pixel_values"
    input_modalities = ("image",)


class PPOCRV6BaseRecEncoderWithSVTR(PPOCRV6BaseRecPreTrainedModel):
    """
    ...
    def __init__(
        ...
        for _ in range(config.depth):
            self.svtr_block.append(PPOCRV6BaseRecBlock(config=config))
...
class PPOCRV6BaseRecHead(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.encoder = PPOCRV6BaseRecEncoderWithSVTR(config)
...
@auto_docstring(custom_intro="PPOCRV6BaseRec model, consisting of Backbone and Head networks.")
class PPOCRV6BaseRecModel(PPOCRV6BaseRecPreTrainedModel):
    def __init__(self, config: PPOCRV6BaseRecConfig):
        super().__init__(config)
...
class PPOCRV6BaseRecForTextRecognitionOutput(BaseModelOutputWithNoAttention):
    ...
    head_hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
        Hidden-states of the PPOCRV6BaseRecHead at the output of each layer plus the optional initial embedding outputs.
    """
...
class PPOCRV6BaseRecForTextRecognition(PPOCRV6BaseRecPreTrainedModel):
    _keys_to_ignore_on_load_missing = ["num_batches_tracked"]

    def __init__(self, config: PPOCRV6BaseRecConfig):
        super().__init__(config)
        self.model = PPOCRV6BaseRecModel(config)
        self.head = PPOCRV6BaseRecHead(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return PPOCRV6BaseRecForTextRecognitionOutput(
