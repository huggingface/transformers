...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForConditionalGenerationModule with Bart->OPT
class FlaxOPTForConditionalGenerationModule(nn.Module):
    config: OPTConfig
    dtype: jnp.dtype = jnp.float32
...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForConditionalGeneration with BART->OPT,Bart->OPT,bart->opt
class FlaxOPTForConditionalGeneration(FlaxOPTPreTrainedModel):
    module_class = FlaxOPTForConditionalGenerationModule
    dtype: jnp.dtype = jnp.float32

    @add_start_docstrings(OPT_DECODE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=FlaxCausalLMOutputWithCrossAttentions, config_class=OPTConfig)
    def decode(
        self,
        decoder_input_ids,
        encoder_outputs,
...
FLAX_OPT_CONDITIONAL_GENERATION_DOCSTRING = """
    Returns:
    ...
    >>> model = FlaxOPTForConditionalGeneration.from_pretrained("facebook/opt-large")
...
overwrite_call_docstring(
    FlaxOPTForConditionalGeneration, OPT_INPUTS_DOCSTRING + FLAX_OPT_CONDITIONAL_GENERATION_DOCSTRING
)
append_replace_return_docstrings(
    FlaxOPTForConditionalGeneration, output_type=FlaxSeq2SeqLMOutput, config_class=_CONFIG_FOR_DOC
)


# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForSequenceClassificationModule with Bart->OPT
class FlaxOPTForSequenceClassificationModule(nn.Module):
    config: OPTConfig
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        self.model = FlaxOPTModule(config=self.config, dtype=self.dtype)
        self.classification_head = FlaxOPTClassificationHead(
            config=self.config,
...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForSequenceClassification with Bart->OPT
class FlaxOPTForSequenceClassification(FlaxOPTPreTrainedModel):
    module_class = FlaxOPTForSequenceClassificationModule
    dtype = jnp.float32
...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForQuestionAnsweringModule with Bart->OPT
class FlaxOPTForQuestionAnsweringModule(nn.Module):
    config: OPTConfig
    dtype: jnp.dtype = jnp.float32
    num_labels = 2
...
# Copied from transformers.models.bart.modeling_flax_bart.FlaxBartForQuestionAnswering with Bart->OPT
class FlaxOPTForQuestionAnswering(FlaxOPTPreTrainedModel):
    module_class = FlaxOPTForQuestionAnsweringModule
    dtype = jnp.float32
