...
@add_start_docstrings(
    """
    The TeleChat2 Model transformer with a sequence classification head on top (linear layer).

    [`TeleChat2ForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class TeleChat2ForSequenceClassification(TeleChat2PreTrainedModel):
...
@add_start_docstrings(
    """
The TeleChat2 Model transformer with a span classification head on top for extractive question-answering tasks like
...
@add_start_docstrings(
    """
    The TeleChat2 Model transformer with a token classification head on top (a linear layer on top of the hidden-states
    output) e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    TELECHAT2_START_DOCSTRING,
)
class TeleChat2ForTokenClassification(TeleChat2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.transformer = TeleChat2Model(config)
