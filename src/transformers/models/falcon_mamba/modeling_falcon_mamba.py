...
def mamba_selective_scan(
    ...
    **kwargs,
    ...
    # Continuation from a cached SSM state — the same contract as falcon_mamba2's `initial_states` — folded into
    # the first input element (`state_1 = A_1 * state_0 + b_1`) with a position mask, so every path below
    # (and any trace) is covered without slicing the sequence axis. Torch paths only: unlike falcon_mamba2's,
    # this op's falcon_mamba_ssm kernel takes no initial state (its dispatch drops the kwarg), keeping the
