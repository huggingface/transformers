...
class ConditionalDetrDecoderOutput(BaseModelOutputWithCrossAttentions):
    ...
    reference_points: Optional[Tuple[torch.Tensor]] = None
...
class ConditionalDetrModelOutput(Seq2SeqModelOutput):
    ...
    Args:
        ...
        encoder_attentions (`tuple(torch.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            weighted average in the self-attention heads.
        intermediate_hidden_states (`torch.Tensor` of shape `(config.decoder_layers, batch_size, sequence_length, hidden_size)`, *optional*, returned when `config.auxiliary_loss=True`):
            Intermediate decoder activations, i.e. the output of each decoder layer, each of them gone through a
            layernorm.
    """
...
class ConditionalDetrSegmentationOutput(ModelOutput):
    ...
    Args:
        ...
        pred_boxes (`torch.Tensor` of shape `(batch_size, num_queries, 4)`):
            ...
            possible padding). You can use [`~ConditionalDetrImageProcessor.post_process_object_detection`] to retrieve the
            unnormalized bounding boxes.
        pred_masks (`torch.Tensor` of shape `(batch_size, num_queries, height/4, width/4)`):
...
class ConditionalDetrEncoder(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        Args:
            ...
            object_queries (`torch.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
...
class ConditionalDetrModel(ConditionalDetrPreTrainedModel):
    ...
    ) -> Union[Tuple[torch.Tensor], ConditionalDetrModelOutput]:
        r"""
        Returns:
...
class ConditionalDetrForObjectDetection(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], ConditionalDetrObjectDetectionOutput]:
        r"""
...
class ConditionalDetrForSegmentation(ConditionalDetrPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], ConditionalDetrSegmentationOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
            ...
            `torch.Tensor` of shape `(number of bounding boxes in the image, 4)` and the masks a
            `torch.Tensor` of shape `(number of bounding boxes in the image, height, width)`.
