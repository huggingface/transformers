...
class ConditionalDetrImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        # Change to:
        #   size = get_size_dict(size, default_to_square=False, size_divisor=size_divisor)

        # And remove from resize calls:
        #   image = self.resize(image=image, size=size, resample=resample, max_size=max_size, ...)
        # Change to:
