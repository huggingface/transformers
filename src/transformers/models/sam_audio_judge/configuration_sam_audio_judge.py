...
@dataclass(frozen=True, kw_only=True)
class SamAudioJudgeTransformerConfig(Config):
    in_channels: int = 128
    dim: int = 1024
    n_heads: int = 8
    n_layers: int = 16
...
class SamAudioJudgeModernBertConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`SamAudioJudgeModernBertModel`]. It is used to instantiate an SamAudioJudgeModernBert
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the SamAudioJudgeModernBert-base.
    e.g. [answerdotai/SamAudioJudgeModernBert-base](https://huggingface.co/answerdotai/SamAudioJudgeModernBert-base)
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 50368):
            Vocabulary size of the SamAudioJudgeModernBert model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`SamAudioJudgeModernBertModel`]
        ...
        repad_logits_with_grad (`bool`, *optional*, defaults to `False`):
            When True, SamAudioJudgeModernBertForMaskedLM keeps track of the logits' gradient when repadding for output. This only
            applies when using Flash Attention 2 with passed labels. Otherwise output logits always have a gradient.

    Examples:

    ```python
    >>> from transformers import SamAudioJudgeModernBertModel, SamAudioJudgeModernBertConfig

    >>> # Initializing a SamAudioJudgeModernBert style configuration
    >>> configuration = SamAudioJudgeModernBertConfig()

    >>> # Initializing a model from the perception_encoder_a_v_modern_bert-base style configuration
    >>> model = SamAudioJudgeModernBertModel(configuration)
