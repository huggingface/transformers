...
# Copied from transformers.models.bert.modeling_bert.BertOnlyNSPHead with Bert->GIT
class GITOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.bert.modeling_bert.BertPreTrainingHeads with Bert->GIT
class GITPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = GITLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
# Copied from transformers.models.bert.modeling_bert.BertModel with BERT->GIT,Bert->GIT
