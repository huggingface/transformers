...
class Starcoder2RotaryEmbedding(nn.Module):
    ...
    def sin_cached(self):
        logger.warning_once(
            ...
            "the forward method of RoPE from now on instead. It is not used in the `Starcoder2Attention` class"
        )
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = Starcoder2LinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = Starcoder2DynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class Starcoder2FlashAttention2(Starcoder2Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # in fp32. (Starcoder2RMSNorm handles it correctly)
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in Starcoder2FlashAttention2 __init__.
...
class Starcoder2DecoderLayer(nn.Module):
    def __init__(self, config: Starcoder2Config, layer_idx: int):
        ...
        self.self_attn = STARCODER2_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
