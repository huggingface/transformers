...
class GeoLMEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.position_embedding = self.position_embeddings # a trick to simplify the weight loading from Bert
...
class GeoLMSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in GeoLMModel forward() function)
...
class GeoLMSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GeoLMAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = GeoLMSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = GeoLMSelfOutput(config)
...
class GeoLMIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GeoLMLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = GeoLMAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = GeoLMAttention(config, position_embedding_type="absolute")
        self.intermediate = GeoLMIntermediate(config)
        self.output = GeoLMOutput(config)
...
class GeoLMEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([GeoLMLayer(config) for _ in range(config.num_hidden_layers)])
...
class GeoLMLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = GeoLMPredictionHeadTransform(config)
...
class GeoLMOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = GeoLMLMPredictionHead(config)
...
class GeoLMOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GeoLMPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = GeoLMLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class GeoLMPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, GeoLMEncoder):
            module.gradient_checkpointing = value


@dataclass
class GeoLMForPreTrainingOutput(ModelOutput):
    """
...
class GeoLMForPreTraining(GeoLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls = GeoLMPreTrainingHeads(config)
    ...
    @add_start_docstrings_to_model_forward(GEOLM_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=GeoLMForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], GeoLMForPreTrainingOutput]:
        r"""
        ...
        ```python
        >>> from transformers import AutoTokenizer, GeoLMForPreTraining
        >>> import torch

        >>> tokenizer = AutoTokenizer.from_pretrained("zekun-li/geolm-base-cased")
        >>> model = GeoLMForPreTraining.from_pretrained("zekun-li/geolm-base-cased")
        ...
        return GeoLMForPreTrainingOutput(
            loss=total_loss,
...
class GeoLMLMHeadModel(GeoLMPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `GeoLMLMHeadModel` as a standalone, add `is_decoder=True.`")

        self.geolm = GeoLMModel(config, add_pooling_layer=False)
        self.cls = GeoLMOnlyMLMHead(config)
...
class GeoLMForMaskedLM(GeoLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `GeoLMForMaskedLM` make sure `config.is_decoder=False` for "
