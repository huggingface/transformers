...
"""
This script converts a lm-head checkpoint from the "Token Dropping" implementation into a PyTorch-compatible GEOLM
...
from transformers import GeoLMConfig, GeoLMForMaskedLM
...
def convert_checkpoint_to_pytorch(tf_checkpoint_path: str, config_path: str, pytorch_dump_path: str):
    ...
    config = GeoLMConfig.from_json_file(config_path)
    model = GeoLMForMaskedLM(config)

    # Layers
    for layer_index in range(0, config.num_hidden_layers):
        layer: GeoLMLayer = model.geolm.encoder.layer[layer_index]

        # Self-attention
        self_attn: GeoLMSelfAttention = layer.attention.self
        ...
        self_output: GeoLMSelfOutput = layer.attention.output
        ...
        # Intermediate
        intermediate: GeoLMIntermediate = layer.intermediate
        ...
        # Output
        geolm_output: GeoLMOutput = layer.output

        geolm_output.dense.weight.data = get_encoder_layer_array(layer_index, "_output_dense/kernel")
        geolm_output.dense.bias.data = get_encoder_layer_array(layer_index, "_output_dense/bias")

        geolm_output.LayerNorm.weight.data = get_encoder_layer_array(layer_index, "_output_layer_norm/gamma")
        geolm_output.LayerNorm.bias.data = get_encoder_layer_array(layer_index, "_output_layer_norm/beta")

    # Embeddings
    model.geolm.embeddings.position_embeddings.weight.data = get_encoder_array("_position_embedding_layer/embeddings")
    model.geolm.embeddings.token_type_embeddings.weight.data = get_encoder_array("_type_embedding_layer/embeddings")
    model.geolm.embeddings.LayerNorm.weight.data = get_encoder_array("_embedding_norm_layer/gamma")
    model.geolm.embeddings.LayerNorm.bias.data = get_encoder_array("_embedding_norm_layer/beta")
    ...
    model.geolm.embeddings.word_embeddings.weight.data = get_masked_lm_array("embedding_table")

    # Pooling
    model.geolm.pooler = GeoLMPooler(config=config)
    model.geolm.pooler.dense.weight.data: GeoLMPooler = get_encoder_array("_pooler_layer/kernel")
    model.geolm.pooler.dense.bias.data: GeoLMPooler = get_encoder_array("_pooler_layer/bias")

    # Export final model
    model.save_pretrained(pytorch_dump_path)

    # Integration test - should load without any errors ;)
    new_model = GeoLMForMaskedLM.from_pretrained(pytorch_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the GEOLM model. This specifies the model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path",
        type=str,
    ...
    convert_checkpoint_to_pytorch(args.tf_checkpoint_path, args.geolm_config_file, args.pytorch_dump_path)
