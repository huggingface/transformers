    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = attn_output.reshape(
            bsz, q_len, self.attention_hidden_size
        ).contiguous()
        attn_output = self.o_proj(attn_output)
...
class ZambaAttentionDecoderLayer(nn.Module):
    def __init__(self, config: ZambaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.self_attn = ZAMBA_ATTENTION_CLASSES[config._attn_implementation](
            config, layer_idx
        )
    ...
    def __init__(self, config: ZambaConfig):
        ...
        for layer_id, layer_type in enumerate(self.layers_block_type):
            if layer_type == "hybrid":
                ...
                layers.append(
                    HybridLayer(block, next(linear_layers), next(mamba_layers))
                )
            else:
