...
class SmolVLMProcessor(ProcessorMixin):
    ...
    def process_vision(self, text, images, output_kwargs, do_image_splitting=False, image_processor_size=None):
        ...
        image_rows = image_inputs.pop("rows")
        image_cols = image_inputs.pop("cols")
        ...
        reshaped_image_rows = [[next(image_rows_iter) for _ in range(len(sub_images))] for sub_images in images]
        reshaped_image_cols = [[next(image_cols_iter) for _ in range(len(sub_images))] for sub_images in images]
