...
from .configuration_f_net import FNetConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "fnet-base-uncased"
_CONFIG_FOR_DOC = "FNetConfig"
_TOKENIZER_FOR_DOC = "FNetTokenizer"

F_NET_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "fnet-base-uncased",
    # See all FNet models at https://huggingface.co/models?filter=f_net
]


def load_tf_weights_in_f_net(model, config, tf_checkpoint_path):
...
class FNetPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_f_net
    base_model_prefix = "f_net"
...
class FNetModel(FNetPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(F_NET_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    @add_code_sample_docstrings(
...
@add_start_docstrings("""FNet Model with a `language modeling` head on top. """, F_NET_START_DOCSTRING)
...
@add_start_docstrings(
    """FNet Model with a `language modeling` head on top for CLM fine-tuning. """, F_NET_START_DOCSTRING
)
class FNetForCausalLM(FNetPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> tokenizer = FNetTokenizer.from_pretrained('fnet-base-uncased')
            >>> config = FNetConfig.from_pretrained("fnet-base-uncased")
            >>> config.is_decoder = True
            >>> model = FNetForCausalLM.from_pretrained('fnet-base-uncased', config=config)
...
class FNetForMultipleChoice(FNetPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(F_NET_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        tokenizer_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
