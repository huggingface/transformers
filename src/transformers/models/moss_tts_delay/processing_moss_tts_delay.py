...
def normalize_tts_text(text: str) -> str:
    """Apply robust non-semantic normalization to TTS input text."""
...
def _normalize_repeated_punctuation(text: str) -> str:
    ...
    return re.sub(r"[!?！？]{2,}", _mixed_qe, text)
...
class MossTTSDelayProcessor(ProcessorMixin):
    ...
    def __call__(self, *args, **kwargs) -> BatchFeature:
        ...
        n_codebooks: int | None = kwargs.pop("n_codebooks", None)
                ...
                if len(raw_audio_items) > 0:
                    ...
                    for idx, item in enumerate(raw_audio_items):
                        if isinstance(item, torch.Tensor):
                            if n_codebooks is not None and item.shape[1] != n_codebooks:
                                raise RuntimeError(
                                    "`audio_codes` n_codebooks does not match the `n_codebooks` argument. Set `n_codebooks=None` if the audio is already tokenized."
                                )
                    ...
                    if len(paths) > 0:
                        encoded_from_paths = self.encode_audios_from_path(paths, n_codebooks)
    ...
    def _pad(self, input_ids_list: list[torch.Tensor]):
        ...
        pad_input_ids = torch.full(
            (len(input_ids_list), int(lengths.max().item()), input_ids_list[0].shape[-1]),
            self.model_config.codebook_pad_token_id,
            device=device,
            dtype=input_ids_list[0].dtype,
        )
        for batch_idx, input_ids in enumerate(input_ids_list):
            pad_input_ids[batch_idx, -input_ids.shape[0] :] = input_ids
        ...
        return {
            "input_ids": pad_input_ids,  # [batch_size, seqlen, n_codebooks]
    ...
    def _replace_audio_placeholders(
        ...
        def build_audio_block(length: int) -> str:
            ...
            step_tokens = gen_slot_token * length + (delay_slot_token * (n_codebooks - 1))
    ...
    def _get_unified_codes(
        ...
        if len(audio_codes_list):
            n_codebooks = audio_codes_list[0].shape[1]
        else:
        ...
        if len(audio_codes_list) == 0:
            ...
            for audio_start_idx_t, audio_end_idx_t, audio_codes in zip(
                ...
                delay_audio_codes = self.apply_delay_pattern(audio_codes, self.model_config.codebook_pad_token_id)
                pad_codes = torch.full(
                    (audio_start_idx - prefix_idx + 1, n_codebooks),
                    self.model_config.codebook_pad_token_id,
            ...
            if truncation:
                delay_audio_codes_list[-1] = delay_audio_codes_list[-1][: -(n_codebooks - 1), :]
            else:
                last_audio_end_idx = int(audio_end_indices[-1].item())
                pad_codes = torch.full(
                    (len(text_codes) - last_audio_end_idx, n_codebooks),
                    self.model_config.codebook_pad_token_id,
    ...
    def encode_audios_from_wav(
        ...
        feature_extractor = MossAudioTokenizerFeatureExtractor(
            ...
            hop_length=int(audio_tokenizer.config.hop_length),
        )
    ...
    def encode_audios_from_path(self, wav_path_list: str | list[str], n_codebooks: int | None = None):
        ...
        return self.encode_audios_from_wav(wav_list, target_sr, n_codebooks)
