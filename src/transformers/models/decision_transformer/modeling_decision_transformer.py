...
class DecisionTransformerGPT2FlashAttention(DecisionTransformerGPT2Attention):
    ...
    def forward(
        ...
        if layer_past is not None:
            past_key, past_value = layer_past
            key, value = self.c_attn(hidden_states).split(
                self.split_size, dim=2)
            key = torch.cat((past_key, key), dim=-2)
            value = torch.cat((past_value, value), dim=-2)
        else:
        ...
        attn_output = self._flash_attention_forward(
            query, key, value, attention_mask, dropout=self.attn_dropout.rate, softmax_scale=1 /
            (self.head_dim**0.5)
        )
...
class DecisionTransformerModel(DecisionTransformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embed_state = torch.nn.Linear(
            config.state_dim, config.hidden_size)
