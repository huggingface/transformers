    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if mm_encoder_outputs.get("images") is None and pixel_values is not None:
            mm_encoder_outputs["images"]: BaseModelOutputWithPooling = self.get_image_features(
                pixel_values, image_grid_thw, return_dict=True, **kwargs
            )

        if mm_encoder_outputs.get("videos") is None and pixel_values_videos is not None:
            mm_encoder_outputs["videos"]: BaseModelOutputWithPooling = self.get_video_features(
