...
""" TF 2.0 Swinv2 Transformer model."""
...
# to_2tuple, drop_path, TFSwinv2PatchEmbeddings, TFSwinv2PatchMerging and TFSwinv2DropPath are tensorflow
# implementations of PyTorch functionalities in the timm library.


@dataclass
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinEncoderOutput with Swin->Swinv2
class TFSwinv2EncoderOutput(ModelOutput):
    """
...
@dataclass
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinModelOutput with Swin->Swinv2
class TFSwinv2ModelOutput(ModelOutput):
    """
...
@dataclass
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinMaskedImageModelingOutput with Swin->Swinv2
class TFSwinv2MaskedImageModelingOutput(ModelOutput):
    """
...
@dataclass
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinImageClassifierOutput with Swin->Swinv2
class TFSwinv2ImageClassifierOutput(ModelOutput):
    """
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinEmbeddings with Swin->Swinv2
class TFSwinv2Embeddings(tf.keras.layers.Layer):
    """
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinPatchEmbeddings with Swin->Swinv2
class TFSwinv2PatchEmbeddings(tf.keras.layers.Layer):
    """
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinPatchMerging with Swin->Swinv2
class TFSwinv2PatchMerging(tf.keras.layers.Layer):
    """
    Patch Merging Layer.
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinDropPath with Swin->Swinv2
class TFSwinv2DropPath(tf.keras.layers.Layer):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""

    def __init__(self, drop_prob: float = None, scale_by_keep: bool = True, **kwargs) -> None:
        super(TFSwinv2DropPath, self).__init__(**kwargs)
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinSelfAttention with Swin->Swinv2
class TFSwinv2SelfAttention(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, dim: int, num_heads: int, **kwargs) -> None:
        super().__init__(**kwargs)
        if dim % num_heads != 0:
            raise ValueError(
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinSelfOutput with Swin->Swinv2
class TFSwinv2SelfOutput(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, dim: int, **kwargs) -> None:
        super().__init__(**kwargs)
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinAttention with Swin->Swinv2
class TFSwinv2Attention(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, dim: int, num_heads: int, **kwargs) -> None:
        super().__init__(**kwargs)
        self.self = TFSwinv2SelfAttention(config, dim, num_heads, name="self")
        self.self_output = TFSwinv2SelfOutput(config, dim, name="output")
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinIntermediate with Swin->Swinv2
class TFSwinv2Intermediate(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, dim: int, **kwargs) -> None:
        super().__init__(**kwargs)
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinOutput with Swin->Swinv2
class TFSwinv2Output(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, dim: int, **kwargs) -> None:
        super().__init__(**kwargs)
        self.dense = tf.keras.layers.Dense(dim, name="dense")
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinLayer with Swin->Swinv2,swin->swinv2
class TFSwinv2Layer(tf.keras.layers.Layer):
    def __init__(
        ...
        self.attention = TFSwinv2Attention(config, dim, num_heads, name="attention")
        self.drop_path = (
            TFSwinv2DropPath(config.drop_path_rate, name="drop_path")
        ...
        self.intermediate = TFSwinv2Intermediate(config, dim, name="intermediate")
        self.swinv2_output = TFSwinv2Output(config, dim, name="output")
    ...
    def call(
        ...
        layer_output = hidden_states + self.swinv2_output(layer_output, training=training)

        layer_outputs = (layer_output, attention_outputs[1]) if output_attentions else (layer_output,)
        return layer_outputs


# Copied from transformers.models.swin.modeling_tf_swin.TFSwinStage with Swin->Swinv2
class TFSwinv2Stage(tf.keras.layers.Layer):
    def __init__(
        self,
        config: Swinv2Config,
        dim: int,
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinEncoder with Swin->Swinv2
class TFSwinv2Encoder(tf.keras.layers.Layer):
    def __init__(self, config: Swinv2Config, grid_size: Tuple[int, int], **kwargs):
        super().__init__(**kwargs)
        ...
        self.layers = [
            TFSwinv2Stage(
                ...
                downsample=TFSwinv2PatchMerging if (i_layer < self.num_layers - 1) else None,
    ...
    def call(
        ...
        training: bool = False,
    ) -> Union[Tuple[tf.Tensor, ...], TFSwinv2EncoderOutput]:
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinPreTrainedModel with Swin->Swinv2,swin->swinv2
class TFSwinv2PreTrainedModel(TFPreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False) -> None:
        if isinstance(module, TFSwinv2Encoder):
...
@keras_serializable
class TFSwinv2MainLayer(tf.keras.layers.Layer):
    config_class = Swinv2Config

    def __init__(
        self, config: Swinv2Config, add_pooling_layer: bool = True, use_mask_token: bool = False, **kwargs
    ) -> None:
        super().__init__(**kwargs)
        self.config = config
        self.num_layers = len(config.depths)
        ...
        self.embeddings = TFSwinv2Embeddings(config, use_mask_token=use_mask_token, name="embeddings")
        self.encoder = TFSwinv2Encoder(config, self.embeddings.patch_grid, name="encoder")

        self.layernorm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layernorm")
        self.pooler = AdaptiveAveragePooling1D(output_size=(1,)) if add_pooling_layer else None

    def get_input_embeddings(self) -> TFSwinv2PatchEmbeddings:
    ...
    def call(
        ...
        training: bool = False,
    ) -> Union[TFSwinv2ModelOutput, Tuple[tf.Tensor, ...]]:
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinModel with SWIN->SWINV2,Swin->Swinv2
class TFSwinv2Model(TFSwinv2PreTrainedModel):
    def __init__(
        self, config: Swinv2Config, add_pooling_layer: bool = True, use_mask_token: bool = False, **kwargs
    ) -> None:
        super().__init__(config, **kwargs)
        self.config = config
        self.swin = TFSwinv2MainLayer(config, name="swin")

    @add_start_docstrings_to_model_forward(SWINV2_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(
        processor_class=_FEAT_EXTRACTOR_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=TFSwinv2ModelOutput,
...
class TFSwinv2ForMaskedImageModeling(TFSwinv2PreTrainedModel):
    def __init__(self, config: Swinv2Config):
        ...
        self.swinv2 = TFSwinv2Model(config, add_pooling_layer=False, use_mask_token=True, name="swinv2")

        self.decoder = TFSwinv2Decoder(config, name="decoder")

    @add_start_docstrings_to_model_forward(SWINV2_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFSwinv2MaskedImageModelingOutput, config_class=_CONFIG_FOR_DOC)
    @unpack_inputs
    def call(
        self,
        pixel_values: Optional[tf.Tensor] = None,
        ...
        return_dict: Optional[bool] = None,
        training: bool = False,
    ) -> Union[Tuple, TFSwinv2MaskedImageModelingOutput]:
        r"""
        ...
        Examples:
        ```python
        >>> from transformers import AutoFeatureExtractor, TFSwinv2ForMaskedImageModeling
        >>> import tensorflow as tf
        ...
        return TFSwinv2MaskedImageModelingOutput(
            loss=masked_im_loss,
...
# Copied from transformers.models.swin.modeling_tf_swin.TFSwinForImageClassification with SWIN->SWINV2,Swin->Swinv2,swin->swinv2
class TFSwinv2ForImageClassification(TFSwinv2PreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config: Swinv2Config):
        super().__init__(config)

        self.num_labels = config.num_labels
        self.swinv2 = TFSwinv2MainLayer(config, name="swinv2")
    ...
    @add_code_sample_docstrings(
        ...
        output_type=TFSwinv2ImageClassifierOutput,
        config_class=_CONFIG_FOR_DOC,
    ...
    def call(
        ...
        return_dict: Optional[bool] = None,
        training: bool = False,
    ) -> Union[Tuple[tf.Tensor, ...], TFSwinv2ImageClassifierOutput]:
        r"""
        ...
        return TFSwinv2ImageClassifierOutput(
            loss=loss,
            logits=logits,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
