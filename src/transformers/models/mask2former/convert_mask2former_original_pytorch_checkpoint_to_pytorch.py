...
class OriginalMask2FormerCheckpointToOursConverter:
    ...
    def convert(self, mask_former: Mask2FormerModel) -> Mask2FormerModel:
    ...
    def convert_instance_segmentation(
        self, mask_former: Mask2FormerForInstanceSegmentation
    ) -> Mask2FormerForInstanceSegmentation:
        dst_state_dict = TrackedStateDict(mask_former.state_dict())
        src_state_dict = self.original_model.state_dict()
...
def get_name(checkpoint_file: Path):
    ...
    # model_name_raw is something like mask2former_panoptic_swin_base_IN21k_384_bs64_554k
    ...
    model_name = f"mask2former-{backbone}-{backbone_type}-{dataset}"
...
if __name__ == "__main__":
    ...
    from Mask2Former.mask_former import add_mask_former_config
    from Mask2Former.mask_former.mask_former_model import Mask2Former as OriginalMask2Former
    ...
    for config_file, checkpoint_file in OriginalMask2FormerCheckpointToOursConverter.using_dirs(
        ...
        mask_former_kwargs = OriginalMask2Former.from_config(original_config)

        original_model = OriginalMask2Former(**mask_former_kwargs).eval()

        DetectionCheckpointer(original_model).load(str(checkpoint_file))

        config: Mask2FormerConfig = OriginalMask2FormerConfigToOursConverter()(original_config)

        mask_former = Mask2FormerModel(config=config).eval()

        converter = OriginalMask2FormerCheckpointToOursConverter(original_model, config)

        mask2former = converter.convert(mask_former)

        mask_former_for_instance_segmentation = Mask2FormerForInstanceSegmentation(config=config).eval()
