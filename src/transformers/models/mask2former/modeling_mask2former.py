...
class Mask2FormerPixelDecoder(nn.Module):
    ...
    def forward(
        ...
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=input_embeds_flat.device
        )
        ...
        level_pos_embed_flat = [
            x + self.level_embed[i].view(1, 1, -1) for i, x in enumerate(position_embeddings)
        ]
        ...
        valid_ratios = torch.stack(
            [self.get_valid_ratio(mask, dtype=input_embeds_flat.dtype) for mask in masks], 1
        )
        ...
        encoder_output = torch.split(
            last_hidden_state, [size.item() for size in split_sizes], dim=1
        )
...
class Mask2FormerMaskPredictor(nn.Module):
    ...
    def forward(
        ...
        attention_mask = (
            attention_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1)
        )
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(MASK2FORMER_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=Mask2FormerForUniversalSegmentationOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        pixel_values: Tensor,
