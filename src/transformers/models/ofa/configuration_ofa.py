...
OFA_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "ofa-base": "https://huggingface.co/ofa-base/resolve/main/config.json",
...
class OFAConfig(PretrainedConfig):
    ...
    ```python
    >>> from transformers import OFAModel, OFAConfig

    >>> # Initializing a OFA ofa-base style configuration
    >>> configuration = OFAConfig()

    >>> # Initializing a model from the ofa-base style configuration
    >>> model = OFAModel(configuration)
