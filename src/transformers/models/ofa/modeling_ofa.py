# coding=utf-8
# Copyright 2022 Alibaba Group The HuggingFace Inc. team. All rights reserved.
#
...
class OFALearnedPositionalEmbedding(nn.Embedding):
    """
...
class OFADecoderLayer(nn.Module):
    def __init__(self, config: OFAConfig):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = OFAAttention(
            self.embed_dim,
...
OFA_GENERATION_EXAMPLE = r"""
    ...
    >>> model = OFAForConditionalGeneration.from_pretrained("ofa-base")
...
OFA_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            ...
            Indices can be obtained using [`~OFATokenizer`]. See [`PreTrainedTokenizer.encode`] and
        ...
        decoder_attention_mask (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            If you want to change padding behavior, you should read [`modeling_ofa._prepare_decoder_inputs`] and modify
...
OFA_STANDALONE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = OFALearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        self.layers = nn.ModuleList([OFAEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class OFADecoder(OFAPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`OFADecoderLayer`]
...
class OFAModel(OFAPreTrainedModel):
    def __init__(self, config: OFAConfig):
        ...
        self.encoder = OFAEncoder(config, self.shared)
        self.decoder = OFADecoder(config, self.shared)
...
@add_start_docstrings(
    "The OFA Model with a language modeling head. Can be used for summarization.", OFA_START_DOCSTRING
)
...
@add_start_docstrings(
    """
    OFA model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    OFA_START_DOCSTRING,
)
class OFAForSequenceClassification(OFAPreTrainedModel):
...
@add_start_docstrings(
    """
    OFA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layer
    on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    OFA_START_DOCSTRING,
)
class OFAForQuestionAnswering(OFAPreTrainedModel):
...
class OFADecoderWrapper(OFAPreTrainedModel):
    """
    ...
    def __init__(self, config):
        super().__init__(config)
        self.decoder = OFADecoder(config)

    def forward(self, *args, **kwargs):
        return self.decoder(*args, **kwargs)


class OFAForCausalLM(OFAPreTrainedModel):
    def __init__(self, config):
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        super().__init__(config)
        self.model = OFADecoderWrapper(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import OFATokenizer, OFAForCausalLM

        >>> tokenizer = OFATokenizer.from_pretrained("facebook/bart-large")
        >>> model = OFAForCausalLM.from_pretrained("facebook/bart-large", add_cross_attention=False)
