...
"""ConFiDeNet model configuration"""
...
class ConFiDeNetConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`ConFiDeNetModel`]. It is used to instantiate a
    ConFiDeNet model according to the specified arguments
    ...
    Args:
        ...
        use_fov_model (`bool`, *optional*, defaults to `False`):
            Whether to use `ConFiDeNetFovModel` to generate the field of view.
        num_fov_head_layers (`int`, *optional*, defaults to 2):
            Number of convolution layers in the head of `ConFiDeNetFovModel`.
    ...
    ```python
    >>> from transformers import ConFiDeNetConfig, ConFiDeNetModel

    >>> # Initializing a ConFiDeNet apple/ConFiDeNet style configuration
    >>> configuration = ConFiDeNetConfig()

    >>> # Initializing a model (with random weights) from the apple/ConFiDeNet style configuration
    >>> model = ConFiDeNetModel(configuration)
