...
from .configuration_confidenet import ConFiDeNetConfig
...
@dataclass
class ConFiDeNetOutput(ModelOutput):
    """
    Base class for ConFiDeNet's outputs.
...
@dataclass
class ConFiDeNetDepthEstimatorOutput(ModelOutput):
    """
    Base class for ConFiDeNetForDepthEstimation's output.
...
class ConFiDeNetPatchEncoder(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
...
class ConFiDeNetImageEncoder(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
        self.out_size = config.image_model_config.image_size // config.image_model_config.patch_size
...
class ConFiDeNetEncoder(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
        self.intermediate_hook_ids = config.intermediate_hook_ids
        ...
        self.patch_encoder = ConFiDeNetPatchEncoder(config)
        self.image_encoder = ConFiDeNetImageEncoder(config)
...
class ConFiDeNetFeatureUpsampleBlock(nn.Module):
    def __init__(
        self,
        config: ConFiDeNetConfig,
        input_dims: int,
...
class ConFiDeNetFeatureUpsample(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
        ...
        # for image_features
        self.image_block = ConFiDeNetFeatureUpsampleBlock(
            config=config,
        ...
        for i, feature_dims in enumerate(config.scaled_images_feature_dims):
            block = ConFiDeNetFeatureUpsampleBlock(
                config=config,
...
class ConFiDeNetFeatureProjection(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
...
class ConFiDeNetNeck(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config

        self.feature_upsample = ConFiDeNetFeatureUpsample(config)
        ...
        self.feature_projection = ConFiDeNetFeatureProjection(config)
...
@auto_docstring
class ConFiDeNetPreTrainedModel(PreTrainedModel):
    config_class = ConFiDeNetConfig
    base_model_prefix = "depth_pro"
    main_input_name = "pixel_values"
    supports_gradient_checkpointing = True
    _supports_sdpa = True
    _no_split_modules = ["ConFiDeNetPreActResidualLayer"]
...
@auto_docstring
class ConFiDeNetModel(ConFiDeNetPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.encoder = ConFiDeNetEncoder(config)
        self.neck = ConFiDeNetNeck(config)
    ...
    def forward(
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, ConFiDeNetModel

        >>> url = "https://www.ilankelman.org/stopsigns/australia.jpg"
        >>> image = Image.open(requests.get(url, stream=True).raw)

        >>> checkpoint = "apple/ConFiDeNet-hf"
        >>> processor = AutoProcessor.from_pretrained(checkpoint)
        >>> model = ConFiDeNetModel.from_pretrained(checkpoint)
...
# Copied from transformers.models.dpt.modeling_dpt.DPTPreActResidualLayer DPT->ConFiDeNet
class ConFiDeNetPreActResidualLayer(nn.Module):
    """
...
class ConFiDeNetFeatureFusionLayer(nn.Module):
    def __init__(self, config: ConFiDeNetConfig, use_deconv: bool = True):
        super().__init__()
        self.config = config
        self.use_deconv = use_deconv

        self.residual_layer1 = ConFiDeNetPreActResidualLayer(config)
        self.residual_layer2 = ConFiDeNetPreActResidualLayer(config)
...
# Modified from transformers.models.dpt.modeling_dpt.DPTFeatureFusionStage with DPT->ConFiDeNet
# with deconv and reversed layers
class ConFiDeNetFeatureFusionStage(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        for _ in range(self.num_layers - 1):
            self.intermediate.append(ConFiDeNetFeatureFusionLayer(config))

        # final layer does not require deconvolution
        self.final = ConFiDeNetFeatureFusionLayer(config, use_deconv=False)

    def forward(self, hidden_states: List[torch.Tensor]) -> List[torch.Tensor]:
        if self.num_layers != len(hidden_states):
            raise ValueError(
                f"num_layers={self.num_layers} in ConFiDeNetFeatureFusionStage"
...
class ConFiDeNetFovEncoder(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
        self.out_size = config.image_model_config.image_size // config.image_model_config.patch_size
...
class ConFiDeNetFovHead(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
...
class ConFiDeNetFovModel(nn.Module):
    def __init__(self, config: ConFiDeNetConfig):
        super().__init__()
        self.config = config
        self.fusion_hidden_size = config.fusion_hidden_size

        self.fov_encoder = ConFiDeNetFovEncoder(config)
        self.conv = nn.Conv2d(
            self.fusion_hidden_size, self.fusion_hidden_size // 2, kernel_size=3, stride=2, padding=1
        )
        self.activation = nn.ReLU(inplace=True)
        self.head = ConFiDeNetFovHead(config)
...
class ConFiDeNetDepthEstimationHead(nn.Module):
    """
    The ConFiDeNetDepthEstimationHead module serves as the output head for depth estimation tasks.
...
@auto_docstring(
    custom_intro="""
    ConFiDeNet Model with a depth estimation head on top (consisting of 3 convolutional layers).
    """
)
class ConFiDeNetForDepthEstimation(ConFiDeNetPreTrainedModel):
    def __init__(self, config, use_fov_model=None):
        ...
        self.depth_pro = ConFiDeNetModel(config)

        # dpt (vit) like fusion stage
        self.fusion_stage = ConFiDeNetFeatureFusionStage(config)

        # depth estimation head
        self.head = ConFiDeNetDepthEstimationHead(config)

        # dinov2 (vit) like encoder
        self.fov_model = ConFiDeNetFovModel(config) if self.use_fov_model else None
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], ConFiDeNetDepthEstimatorOutput]:
        r"""
        ...
        >>> model = ConFiDeNetForDepthEstimation.from_pretrained(checkpoint)
        ...
        return ConFiDeNetDepthEstimatorOutput(
            loss=loss,
...
__all__ = ["ConFiDeNetPreTrainedModel", "ConFiDeNetModel", "ConFiDeNetForDepthEstimation"]
