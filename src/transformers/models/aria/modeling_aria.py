...
# Base docstring
_CHECKPOINT_FOR_DOC = "rhymes-ai/Aria"
...
# Copied from transformers.models.llava.modeling_llava.LlavaMultiModalProjector with Llava->Aria
...
# Copied from transformers.models.llava.modeling_llava.LlavaPreTrainedModel with Llava->Aria,llava->aria
class AriaPreTrainedModel(PreTrainedModel):
    config_class = AriaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["AriaVisionAttention"]
    _skip_keys_device_placement = "past_key_values"
    _supports_flash_attn_2 = True
    _supports_cache_class = True

    def _init_weights(self, module):
        # important: this ported version of Aria isn't meant for training from scratch - only
        # inference and fine-tuning - so the proper init weights code has been removed - the original codebase
        # https://github.com/haotian-liu/LLaVA/tree/main/aria should serve for that purpose
        std = (
...
# Copied from transformers.models.llava.modeling_llava.LlavaForConditionalGeneration with LLAVA->ARIA,Llava->Aria,LLaVa->Aria,llava-hf/llava-1.5-7b-hf->rhymes-ai/Aria
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            if legacy_processing:
                logger.warning_once(
                    "Expanding inputs for image tokens in Aria should be done in processing. "
                ...
                if input_ids.shape[1] != 1:
                    ...
                    # if one uses Aria + Fused modules where the cache on the
