...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->LagLlama
class LagLlamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        LagLlamaRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class LagLlamaRotaryEmbedding(nn.Module):
...
class LagLlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = LagLlamaRotaryEmbedding(
                self.head_dim,
            ...
            if scaling_type == "linear":
                self.rotary_emb = LagLlamaLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = LagLlamaDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
# Copied from transformers.models.llama.modeling_llama.LlamaFlashAttention2 with Llama->LagLlama
class LagLlamaFlashAttention2(LagLlamaAttention):
    """
    LagLlama flash attention module. This module inherits from `LagLlamaAttention` as the weights of the module stays
    ...
    def forward(
        ...
        **kwargs,
        ...
        # LagLlamaFlashAttention2 attention does not support output_attentions
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be removed in v4.37. Please make sure use `attention_mask` instead.`"
            )
        ...
        # in fp32. (LagLlamaRMSNorm handles it correctly)
...
class LagLlamaModel(LagLlamaPreTrainedModel):
    ...
    def slice_along_dim(a: torch.Tensor, dim: int, slice_: slice) -> torch.Tensor:
        ...
        Returns ------- torch.Tensor
            A tensor with the same size as the input one, except dimension ``dim`` which has length equal to the slice
            length.
        """
    ...
    def lagged_sequence_values(
        ...
        Parameters ---------- indices
            Indices of the lagged observations. For example, ``[0]`` indicates that, at any time ``t``, the will have
            only the observation from time ``t`` itself; instead, ``[0, 24]`` indicates that the output will have
            observations from times ``t`` and ``t-24``.
        prior_sequence
            Tensor containing the input sequence prior to the time range for which the output is required.
        sequence
            Tensor containing the input sequence in the time range where the output is required.
        dim
            Time dimension.
        keepdim
    ...
    def forward(
    ...
    ) -> Union[Tuple, BaseTSModelOutputWithPast]:
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, loc, scale] if v is not None
            )
        return BaseTSModelOutputWithPast(
