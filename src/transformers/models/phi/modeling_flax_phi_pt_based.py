...
# hack: init JAX array before importing torch to avoid "DNN init failed" error
jax.random.PRNGKey(0)
import torch  # TODO: remove when rewrite is finished
...
# copied from transformers.models.llama.modeling_flax_llama.py
...
# modified from transformers.models.llama.modeling_flax_llama.py
...
class FlaxPhiRotaryEmbedding(nn.Module):
    ...
    # TODO: reanimate this implementation of rotary embedding
    # TODO: ignore cache for now

    def setup(self):
        inv_freq = 1.0 / (self.base ** (jnp.arange(0, self.dim, 2).astype(jnp.float32) / self.dim))
        t = jnp.arange(self.max_position_embeddings, dtype=inv_freq.dtype)
        freqs = jnp.outer(t, self.inv_freq)
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = jnp.concatenate((freqs, freqs), axis=-1)
        self.cos_cached = jnp.cos(emb).astype(inv_freq.dtype)
        self.sin_cached = jnp.sin(emb).astype(inv_freq.dtype)

    def __call__(self, x, seq_len=None):
        # x: [bs, num_attention_heads, seq_len, head_size]
        return (
            self.cos_cached.astype(dtype=x.dtype),
            self.sin_cached.astype(dtype=x.dtype),
        )
...
def main_rot_emb():
    ...
    self = FlaxPhiRotaryEmbedding(head_size)
    variables = self.init(rng, x, x.shape[0])
    self = self.bind(variables)

    cos_, sin_ = self(x, x.shape[0])
...
class FlaxPhiAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.query_key_value = nn.Dense(3 * self.hidden_size, use_bias=True)
        self.dense = nn.Dense(self.hidden_size, use_bias=True)
        ...
        self.rotary_emb = FlaxPhiRotaryEmbedding(config)
    ...
    def __call__(
        ...
        position_ids: Optional[jax.Array] = None,   # long array
        ...
        # or wise versa?
        query_states = jnp.moveaxis(query_states, 1, 2)
        key_states = jnp.moveaxis(key_states, 1, 2)
        values_states = jnp.moveaxis(value_states, 1, 2)
...
def main():
    ...
    rng = jax.random.PRNGKey(0)
    x = jax.random.normal(rng, (batch_size, config.num_attention_heads, seq_len, head_size))
    hidden_states = jax.random.normal(rng, (batch_size, seq_len, 3 * hidden_size))
