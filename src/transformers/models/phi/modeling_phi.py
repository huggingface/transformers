...
# Copied from transformers.models.falcon.modeling_falcon.FalconYaRNScalingRotaryEmbedding with Falcon->Phi
class PhiYaRNScalingRotaryEmbedding(PhiRotaryEmbedding):
    def __init__(
        self,
        dim,
...
# Copied from transformers.models.falcon.modeling_falcon.FalconDynamicYaRNScalingRotaryEmbedding with Falcon->Phi
class PhiDynamicYaRNScalingRotaryEmbedding(PhiYaRNScalingRotaryEmbedding):
    def __init__(
        self,
        dim,
        max_position_embeddings=2048,
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                self.rotary_emb = PhiYaRNScalingRotaryEmbedding(
                    int(self.partial_rotary_factor * self.head_dim),
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
                ...
                self.rotary_emb = PhiDynamicYaRNScalingRotaryEmbedding(
                    int(self.partial_rotary_factor * self.head_dim),
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
