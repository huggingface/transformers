...
class ImageGPTForCausalImageModeling(ImageGPTPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        >>> n_px_h = feature_extractor.size["height"]
        >>> n_px_w = feature_extractor.size["width"]

        >>> samples = output[:, 1:].cpu().detach().numpy()
        >>> samples_img = [
        ...     np.reshape(np.rint(127.5 * (clusters[s] + 1.0)), [n_px_h, n_px_w, 3]).astype(np.uint8) for s in samples
