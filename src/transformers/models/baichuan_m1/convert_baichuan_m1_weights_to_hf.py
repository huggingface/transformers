...
from transformers import GenerationConfig, BaichuanM1Config, BaichuanM1ForCausalLM, BaichuanM1Tokenizer, PreTrainedTokenizerFast
...
```
python src/transformers/models/baichuan_m1/convert_baichuan_m1_weights_to_hf.py \
    --input_dir /path/to/downloaded/baichuan_m1/weights --model_size 1B --baichuan_m1_version 3.2 --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import BaichuanM1ForCausalLM, BaichuanM1Tokenizer

model = BaichuanM1ForCausalLM.from_pretrained("/output/path")
tokenizer = BaichuanM1Tokenizer.from_pretrained("/output/path")
```
...
DEFAULT_BAICHUANM1_SPECIAL_TOKENS = {
    "3": [
        "<|begin_of_text|>",
        "<|end_of_text|>",
...
def write_model(
    ...
    if base > 10000.0 and not is_baichuan_m1_3(baichuan_m1_version):
        max_position_embeddings = 16384
    else:
        max_position_embeddings = CONTEXT_LENGTH_FOR_VERSION[baichuan_m1_version]
    ...
    with tempfile.TemporaryDirectory() as tmp_model_path:
        ...
        if num_shards == 1:
            ...
            concat_dim = 0 if is_baichuan_m1_3(baichuan_m1_version) else 1
            state_dict = {
        ...
        if is_baichuan_m1_3(baichuan_m1_version):
            bos_token_id = 128000
        ...
        if baichuan_m1_version in ["3.1", "3.2", "Guard-3"]:
            rope_scaling = {
                "factor": 32.0 if baichuan_m1_version == "3.2" else 8.0,
                "low_freq_factor": 1.0,
                "high_freq_factor": 4.0,
                "original_max_position_embeddings": 8192,
                "rope_type": "baichuan_m13",
            }
        else:
            rope_scaling = None
        ...
        config = BaichuanM1Config(
            ...
            tie_word_embeddings=True if baichuan_m1_version in ["3.2"] else False,
        )
        ...
        print("Loading the checkpoint in a BaichuanM1 model.")
        model = BaichuanM1ForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
...
class BaichuanM13Converter(TikTokenConverter):
    def __init__(self, vocab_file, special_tokens=None, instruct=False, baichuan_m1_version="3.2", **kwargs):
        ...
        templates_for_version = {
            "2": ("meta-baichuan_m1/BaichuanM1-2-7b-chat-hf", "f5db02db724555f92da89c216ac04704f23d4590"),
            "3": ("meta-baichuan_m1/Meta-BaichuanM1-3-8B-Instruct", "5f0b02c75b57c5855da9ae460ce51323ea669d8a"),
            "3.1": ("meta-baichuan_m1/BaichuanM1-3.1-8B-Instruct", "0e9e39f249a16976918f6564b8830bc894c89659"),
            "3.2": ("meta-baichuan_m1/BaichuanM1-3.2-1B-Instruct", "e9f8effbab1cbdc515c11ee6e098e3d5a9f51e14"),
            "Guard-3": ("meta-baichuan_m1/BaichuanM1-Guard-3-1B", "acf7aafa60f0410f8f42b1fa35e077d705892029"),
        }
        ...
        additional_kwargs = {}
        if instruct or baichuan_m1_version in ["Guard-3"]:
            model_id, revision = templates_for_version.get(baichuan_m1_version, (None, None))
        ...
        self.converted_tokenizer = PreTrainedTokenizerFast(
            ...
            model_max_length=CONTEXT_LENGTH_FOR_VERSION[baichuan_m1_version],
...
def write_tokenizer(
    tokenizer_path, input_tokenizer_path, baichuan_m1_version="2", special_tokens=None, instruct=False, push_to_hub=False
):
    print("Converting the tokenizer.")
    tokenizer_class = BaichuanM1Tokenizer if LlamaTokenizerFast is None else LlamaTokenizerFast
    if is_baichuan_m1_3(baichuan_m1_version):
        tokenizer = BaichuanM13Converter(
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        help="Location of BaichuanM1 weights, which contains tokenizer.model and model folders",
    )
    parser.add_argument(
        "--model_size",
        default=None,
        help="'f' Deprecated in favor of `num_shards`: models correspond to the finetuned versions, and are specific to the BaichuanM12 official release. For more details on BaichuanM12, checkout the original repo: https://huggingface.co/meta-baichuan_m1",
    )
    parser.add_argument(
        "--output_dir",
    ...
    # Different BaichuanM1 versions used different default values for max_position_embeddings, hence the need to be able to specify which version is being used.
    parser.add_argument(
        "--baichuan_m1_version",
        choices=["1", "2", "3", "3.1", "3.2", "Guard-3"],
        default="1",
        type=str,
        help="Version of the BaichuanM1 model to convert. Currently supports BaichuanM11 and BaichuanM12. Controls the context size",
    )
    parser.add_argument(
        "--num_shards",
        default=None,
    ...
    if args.special_tokens is None:
        ...
        args.special_tokens = DEFAULT_BAICHUANM1_SPECIAL_TOKENS.get(str(args.baichuan_m1_version), [])
    ...
    vocab_size = len(
        write_tokenizer(
            args.output_dir,
            spm_path,
            baichuan_m1_version=args.baichuan_m1_version,
