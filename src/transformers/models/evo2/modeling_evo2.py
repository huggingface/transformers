...
class Evo2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Evo2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Evo2Attention(config=config, layer_idx=layer_idx)
        self.mlp = Evo2MLP(config)
...
class Evo2PreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": Evo2DecoderLayer,
        "attentions": Evo2Attention,
    }
...
class Evo2Model(Evo2PreTrainedModel):
    def __init__(self, config: Evo2Config):
        ...
        self.layers = nn.ModuleList(
            [Evo2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Evo2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Evo2RotaryEmbedding(config=config)
...
class Evo2ForCausalLM(Evo2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, Evo2ForCausalLM

        >>> model = Evo2ForCausalLM.from_pretrained("meta-evo2/Evo2-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-evo2/Evo2-2-7b-hf")
...
class Evo2ForTokenClassification(GenericForTokenClassification, Evo2PreTrainedModel):
    pass


class Evo2ForSequenceClassification(GenericForSequenceClassification, Evo2PreTrainedModel):
    pass


class Evo2ForQuestionAnswering(GenericForQuestionAnswering, Evo2PreTrainedModel):
    pass
