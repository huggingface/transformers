...
class Lfm2MoeAttention(nn.Module):
    def __init__(self, config: Lfm2MoeConfig, layer_idx: Optional[int] = None, **kwargs):
        super().__init__()
        self.config = config
...
class Lfm2MoePretrainedModel(PreTrainedModel):
    ...
    _no_split_modules: ClassVar = ["Lfm2MoeDecoderLayer"]
    ...
    _can_record_outputs = {
        "router_logits": OutputRecorder(Lfm2MoeSparseMoeBlock, index=1),
