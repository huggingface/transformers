...
@dataclass(frozen=True, kw_only=True)
class FlClapTransformerConfig(Config):
    in_channels: int = 128
    dim: int = 1024
    n_heads: int = 8
    n_layers: int = 16
...
class FlClapModernBertConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`FlClapModernBertModel`]. It is used to instantiate an FlClapModernBert
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the FlClapModernBert-base.
    e.g. [answerdotai/FlClapModernBert-base](https://huggingface.co/answerdotai/FlClapModernBert-base)
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 50368):
            Vocabulary size of the FlClapModernBert model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`FlClapModernBertModel`]
        ...
        repad_logits_with_grad (`bool`, *optional*, defaults to `False`):
            When True, FlClapModernBertForMaskedLM keeps track of the logits' gradient when repadding for output. This only
            applies when using Flash Attention 2 with passed labels. Otherwise output logits always have a gradient.

    Examples:

    ```python
    >>> from transformers import FlClapModernBertModel, FlClapModernBertConfig

    >>> # Initializing a FlClapModernBert style configuration
    >>> configuration = FlClapModernBertConfig()

    >>> # Initializing a model from the perception_encoder_a_v_modern_bert-base style configuration
    >>> model = FlClapModernBertModel(configuration)
