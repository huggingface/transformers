...
class MPNetPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MPNetEncoder):
...
class MPNetLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = MPNetAttention(config)
...
class MPNetEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            position_bias = self.compute_position_bias(hidden_states, past_key_value)
...
@add_start_docstrings(
    """MPNet Model with a `language modeling` head on top for CLM fine-tuning.""", MPNET_START_DOCSTRING
)
class MPNetForCausalLM(MPNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `MPNetLMHeadModel` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        >>> model = MPNetForCasualLM.from_pretrained("microsoft/mpnet-base", config=config)
...
class MPNetForMaskedLM(MPNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `MPNetForMaskedLM` make sure `config.is_decoder=False` for "
