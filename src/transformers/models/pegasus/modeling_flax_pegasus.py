# coding=utf-8
# Copyright 2021 Google and The Google Flax Team Authors And The HuggingFace Inc. team. All rights reserved.
#
...
_CHECKPOINT_FOR_DOC = "google/pegasus-xsum"
...
class PegasusSinusoidalPositionalEmbedding(nn.Module):
    """This module produces sinusoidal positional embeddings of any length."""

    dtype: jnp.dtype = jnp.float32
    config: PegasusConfig

    def setup(self, padding_idx: Optional[int] = None):
    ...
    def _init_weight(out):
        ...
        out[:, 0:sentinel] = jnp.sin(position_enc[:, 0::2])
        out[:, sentinel:] = jnp.cos(position_enc[:, 1::2])
        return out

    def __call__(self, input_ids_shape: int, past_key_values_length: int = 0):
        """`input_ids_shape` is expected to be [bsz x seqlen]."""
        bsz, seq_len = input_ids_shape[:2]
        positions = jnp.arange(past_key_values_length, past_key_values_length + seq_len, dtype=self.dtype)
        return nn.Embed(
            positions,
            input_ids_shape,
...
# Copied from transformers.models.bart.modeling_bart.FlaxBartDecoderLayerCollection with Bart->Pegasus
...
class FlaxPegasusEncoder(nn.Module):
    ...
    def __call__(
        ...
        embed_pos = PegasusSinusoidalPositionalEmbedding(self.padding_idx)
...
class FlaxPegasusDecoder(nn.Module):
    ...
    def setup(self):
        ...
        # and adjust num_embeddings appropriately. Other models don't have this hack
        self.offset = 2
        self.embed_positions = PegasusSinusoidalPositionalEmbedding(self.padding_idx)
...
# Copied from transformers.models.bart.modeling_bart.FlaxPegasusModule with Bart->Pegasus
