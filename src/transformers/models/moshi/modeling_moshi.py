...
# copied from transformers.models.gemma.modeling_gemma.GemmaAttention with Gemma->Moshi
...
# Copied from transformers.models.mimi.modeling_mimi.MimiTransformerModel with Mimi->Moshi, TransformerModel->Decoder, TransformerLayer->DecoderLayer
class MoshiDecoder(nn.Module):
    """
...
class MoshiPreTrainedModel(PreTrainedModel):
    ...
    config_class = MoshiDecoderConfig
...
MOSHI_START_DOCSTRING = r"""
    ...
    The Moshi model was proposed in [Simple and Controllable Music Generation](https://arxiv.org/abs/2306.05284) by
...
@add_start_docstrings(
    "The bare Moshi decoder model outputting raw hidden-states without any specific head on top.",
    MOSHI_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenModel with MUSICGEN->MOSHI,Musicgen->Moshi
class MoshiModel(MoshiPreTrainedModel):
    def __init__(self, config: MoshiDecoderConfig):
        super().__init__(config)
        self.decoder = MoshiDecoder(config)
...
@add_start_docstrings(
    "The Moshi decoder model with a language modelling head on top.",
    MOSHI_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForCausalLM with MUSICGEN->MOSHI,Musicgen->Moshi,musicgen->moshi,MusicGen->Moshi
class MoshiForCausalLM(MoshiPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # per codebook cross-entropy
            # ref: https://github.com/facebookresearch/audiocraft/blob/69fea8b290ad1b4b40d28f92d1dfc0ab01dbab85/audiocraft/solvers/moshi.py#L242-L243
...
@add_start_docstrings(
    "The composite Moshi model with a text encoder, audio encoder and Moshi decoder, "
    "for music generation tasks with one or both of text and audio prompts.",
    MOSHI_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForConditionalGeneration with MUSICGEN->MOSHI,Musicgen->Moshi,musicgen->moshi,MusicGen->Moshi,facebook/musicgen-small->kyutai/moshiko
class MoshiForConditionalGeneration(PreTrainedModel):
    ...
    def __init__(
        ...
        if config is None and (text_encoder is None or audio_encoder is None or decoder is None):
            raise ValueError(
                "Either a configuration has to be provided, or all three of text encoder, audio encoder and Moshi decoder."
            )
        if config is None:
            config = MoshiConfig.from_sub_models_config(text_encoder.config, audio_encoder.config, decoder.config)
        else:
        ...
        if config.decoder.cross_attention_hidden_size is not None:
            if config.decoder.cross_attention_hidden_size != config.text_encoder.hidden_size:
                raise ValueError(
                    "If `cross_attention_hidden_size` is specified in the Moshi decoder's configuration, it has to be equal"
        ...
        if decoder is None:
            decoder = MoshiForCausalLM(config.decoder)
    ...
    def from_sub_models_pretrained(
        ...
        **kwargs,
        ...
        r"""
        Instantiate a text encoder, an audio encoder, and a Moshi decoder from one, two or three base classes of the
        ...
        >>> # initialize a moshi model from a t5 text encoder, encodec audio encoder, and moshi decoder
        >>> model = MoshiForConditionalGeneration.from_sub_models_pretrained(
        ...     text_encoder_pretrained_model_name_or_path="google-t5/t5-base",
        ...     audio_encoder_pretrained_model_name_or_path="facebook/encodec_24khz",
        ...     decoder_pretrained_model_name_or_path="kyutai/moshiko",
        ... )
        >>> # saving model after fine-tuning
        >>> model.save_pretrained("./moshi-ft")
        >>> # load fine-tuned model
        >>> model = MoshiForConditionalGeneration.from_pretrained("./moshi-ft")
        ```"""
        ...
        if decoder is None:
            ...
            if "config" not in kwargs_decoder:
                ...
                if isinstance(decoder_config, MoshiConfig):
            ...
            decoder = MoshiForCausalLM.from_pretrained(decoder_pretrained_model_name_or_path, **kwargs_decoder)

        # instantiate config with corresponding kwargs
        config = MoshiConfig.from_sub_models_config(
