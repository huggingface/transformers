...
"""Convert Moshi checkpoints from the original repository."""
...
import torch
from audiocraft.models import Moshi
...
from transformers.models.moshi.modeling_moshi import MoshiForCausalLM
...
def rename_state_dict(state_dict: OrderedDict, hidden_size: int) -> Tuple[Dict, Dict]:
    """Function that takes the fairseq Moshi state dict and renames it according to the HF
...
def decoder_config_from_checkpoint(checkpoint: str) -> MoshiDecoderConfig:
    if checkpoint.endswith("small"):
        ...
        raise ValueError(
            ...
            "`['facebook/moshi-stereo-small', 'facebook/moshi-stereo-medium', 'facebook/moshi-stereo-large']` "
...
@torch.no_grad()
def convert_moshi_checkpoint(
    checkpoint, pytorch_dump_folder=None, repo_id=None, device="cpu", safe_serialization=False
):
    fairseq_model = Moshi.get_pretrained(checkpoint, device=device)
    ...
    decoder = MoshiForCausalLM(decoder_config).eval()
    ...
    model = MoshiForConditionalGeneration(text_encoder=text_encoder, audio_encoder=audio_encoder, decoder=decoder)
    ...
    processor = MoshiProcessor(feature_extractor=feature_extractor, tokenizer=tokenizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="small",
        type=str,
        help="Checkpoint size of the Moshi model you'd like to convert. Can be one of: "
    ...
    convert_moshi_checkpoint(args.checkpoint, args.pytorch_dump_folder, args.push_to_hub)
