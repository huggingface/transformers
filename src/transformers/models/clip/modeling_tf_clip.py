...
@keras_serializable
class TFCLIPVisionProjectionLayer(keras.layers.Layer):
    config_class = CLIPConfig

    def __init__(self, config: CLIPConfig, **kwargs):
        super().__init__(**kwargs)

        if not isinstance(config, CLIPVisionConfig):
            raise ValueError(
                "config.vision_config is expected to be of type CLIPVisionConfig but is of type"
                f" {type(config.vision_config)}."
            )
    ...
    def call(
        ...
        image_features = image_features / tf.norm(tensor=image_features, ord="euclidean", axis=-1, keepdims=True)
