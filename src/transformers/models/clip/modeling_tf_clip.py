...
class TFCLIPTextEmbeddings(tf.keras.layers.Layer):
    ...
    def call(self, input_ids=None, position_ids=None, inputs_embeds=None):
...
class TFCLIPEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = TFCLIPAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim)
        self.mlp = TFCLIPMLP(config)
...
class TFCLIPPreTrainedModel(PreTrainedModel):
    """
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, TFCLIPTextEmbeddings):
            module.token_embedding.weight.data.normal_(mean=0.0, std=factor * 0.02)
            module.position_embedding.weight.data.normal_(mean=0.0, std=factor * 0.02)
        elif isinstance(module, TFCLIPVisionEmbeddings):
            factor = self.config.initializer_factor
            nn.init.normal_(module.class_embedding, mean=0.0, std=module.embed_dim ** -0.5 * factor)
            nn.init.normal_(module.patch_embedding.weight, std=module.config.initializer_range * factor)
            nn.init.normal_(module.position_embedding.weight, std=module.config.initializer_range * factor)
        elif isinstance(module, TFCLIPAttention):
            factor = self.config.initializer_factor
        ...
        elif isinstance(module, TFCLIPMLP):
            factor = self.config.initializer_factor
            in_proj_std = (
        ...
        elif isinstance(module, TFCLIPModel):
            nn.init.normal_(
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, TFCLIPEncoder):
...
class TFCLIPEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([TFCLIPEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class TFCLIPTextTransformer(tf.keras.layers.Layer):
    def __init__(self, config: CLIPTextConfig):
        ...
        self.config = config
        embed_dim = config.hidden_size
        self.embeddings = TFCLIPTextEmbeddings(config)
        self.encoder = TFCLIPEncoder(config)
...
class TFCLIPTextModel(TFCLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPTextConfig):
        ...
        self.text_model = TFCLIPTextTransformer(config)
        self.init_weights()
...
class TFCLIPVisionTransformer(tf.keras.layers.Layer):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.embeddings = TFCLIPVisionEmbeddings(config)
...
class TFCLIPVisionModel(TFCLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig):
        super().__init__(config)
        self.vision_model = TFCLIPVisionTransformer(config)
        self.init_weights()
...
class TFCLIPModel(TFCLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.text_model = TFCLIPTextTransformer(text_config)
        self.vision_model = TFCLIPVisionTransformer(vision_config)
