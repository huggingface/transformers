# coding=utf-8
# Copyright 2022 Haoxin Li The HuggingFace Inc. team. All rights reserved.
#
...
_CHECKPOINT_FOR_DOC = "lihaoxin2020/narrowbert-sparse_attn-uncased"
_CONFIG_FOR_DOC = "NarrowBertConfig"

NARROW_BERT_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "lihaoxin2020/narrowbert-sparse_attn-uncased",
    # See all NarrowBERT models at https://huggingface.co/models?filter=narrow_bert
]


def load_tf_weights_in_narrow_bert(model, config, tf_checkpoint_path):
...
class NarrowBertEmbeddings(nn.Module):
...
class NarrowBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in NarrowBertModel forward() function)
...
class NarrowBertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class NarrowBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = NarrowBertSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = NarrowBertSelfOutput(config)
...
class NarrowBertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class NarrowBertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class NarrowBertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = NarrowBertAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = NarrowBertAttention(config, position_embedding_type="absolute")
        self.intermediate = NarrowBertIntermediate(config)
        self.output = NarrowBertOutput(config)
...
class NarrowBertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class NarrowBertLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = NarrowBertPredictionHeadTransform(config)
...
class NarrowBertOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = NarrowBertLMPredictionHead(config)
...
class NarrowBertPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and
    a simple interface for downloading and loading pretrained models.
    """

    config_class = NarrowBertConfig
...
NARROW_BERT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`~NarrowBertConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the configuration.
            Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

NARROW_BERT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using [`NarrowBertTokenizer`].
...
class NarrowBertForMaskedLM(NarrowBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `NarrowBertForMaskedLM` make sure `config.is_decoder=False` for "
...
class NarrowBertForSequenceClassification(NarrowBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class NarrowBertForMultipleChoice(NarrowBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings(
    """NarrowBERT Model with a token classification head on top (a linear layer on top of
