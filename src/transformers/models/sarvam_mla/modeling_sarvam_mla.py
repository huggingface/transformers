...
class SarvamMLARotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: SarvamMLAConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: SarvamMLAConfig | None = None,
...
class SarvamMLATopkRouter(nn.Module):
...
class SarvamMLANaiveMoe(nn.Module):
...
class SarvamMLAMoE(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.experts = SarvamMLANaiveMoe(config)
        self.gate = SarvamMLATopkRouter(config)
...
class SarvamMLAAttention(nn.Module):
    ...
    def __init__(self, config: SarvamMLAConfig, layer_idx: int):
        ...
        self.kv_a_layernorm = SarvamMLARMSNorm(self.kv_lora_rank)
...
class SarvamMLADecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: SarvamMLAConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        if layer_idx >= config.first_k_dense_replace:
            self.mlp = SarvamMLAMoE(config)
        else:
            self.mlp = SarvamMLAMLP(config)
...
class SarvamMLAPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": SarvamMLADecoderLayer,
        "attentions": SarvamMLAAttention,
    }
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SarvamMLATopkRouter):
            init.normal_(module.weight, mean=0.0, std=self.config.initializer_range)
            init.zeros_(module.e_score_correction_bias)
        elif isinstance(module, SarvamMLANaiveMoe):
...
class SarvamMLAModel(SarvamMLAPreTrainedModel):
    def __init__(self, config: SarvamMLAConfig):
        ...
        self.rotary_emb = SarvamMLARotaryEmbedding(config=config)
...
@auto_docstring
class SarvamMLAForCausalLM(SarvamMLAPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, SarvamMLAForCausalLM

        >>> model = SarvamMLAForCausalLM.from_pretrained("meta-sarvam_mla/SarvamMLA-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-sarvam_mla/SarvamMLA-2-7b-hf")
