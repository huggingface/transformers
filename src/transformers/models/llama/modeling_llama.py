...
class LlamaModel(LlamaPreTrainedModel):
    ...
    # As of 20/02/2024, the `attention_mask` passed to the model in `generate` is 2D and of dynamic length even when the static
    # KV cache is used. This is an issue for torch.compile which then recaptures cudagraphs at each decode steps due to the dynamic shapes.
    # (`recording cudagraph tree for symint key 13`, etc.), which is very slow.
    # TODO: pass a statically-shaped 4D mask as input to the model, enabling torch.compile with `fullgraph=True`.
