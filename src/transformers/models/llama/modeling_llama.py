...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache and not isinstance(past_key_values, Cache):  # kept for BC (non `Cache` `past_key_values` inputs)
            raise ValueError("You can no longer use tuple object here.")
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        input_length = inputs_embeds.shape[-1] if inputs_embeds is not None else input_ids.shape[-1]
        ...
        if past_key_values is not None:
            ...
            if attention_mask is not None and past_length + input_ids.shape[1] > max_cache_length:
