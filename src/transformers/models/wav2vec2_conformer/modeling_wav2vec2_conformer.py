...
# Copied from transformers.models.bart.modeling_bart.BartAttention with Bart->Wav2Vec2Conformer
...
#            self.self_attn = RotaryPositionMultiHeadedAttention(embed_dim, attention_heads, dropout=dropout)
#
...
class Wav2Vec2ConformerSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        elif self.position_embeddings_type == "rotary":
            precision = torch.float
            self.rotary_ndims = self.d_k  # also try self.d_k//2

            self.rotary_emb = RotaryPositionalEmbedding(
                self.rotary_ndims, base=config.rotary_embedding_base, precision=precision
            )

    def forward(self, hidden_states, key_padding_mask=None, pos_emb=None):
...
#        sequence_length, batch_size, hidden_size = hidden_states.size()
        ...
        if self.position_embeddings_type == "rotary":
            query_key_states = self.apply_rotary_embedding(query_key_states)

        # project query_key_states and value_states
#        query = self.linear_q(query_key_states.transpose(0, 1)).view(batch_size, -1, self.h, self.d_k)
#        key = self.linear_k(query_key_states.transpose(0, 1)).view(batch_size, -1, self.h, self.d_k)
#        value = self.linear_v(value_states.transpose(0, 1)).view(batch_size, -1, self.h, self.d_k)
        query = self.linear_q(query_key_states).view(batch_size, -1, self.h, self.d_k)
        key = self.linear_k(query_key_states).view(batch_size, -1, self.h, self.d_k)
        value = self.linear_v(value_states).view(batch_size, -1, self.h, self.d_k)
        ...
        if self.position_embeddings_type == "relative":
            scores = self._apply_relative_embeddings(query=query, key=key, pos_emb=pos_emb)
        else:
            scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.d_k)

        # TODO(PVP) - remove after
        if key_padding_mask is not None:
            key_padding_mask = key_padding_mask > 1.0
        else:
            key_padding_mask = torch.zeros_like(scores, dtype=torch.long).bool()
        ...
        probs = torch.softmax(scores, dim=-1)  # (batch, head, time1, time2)

        probs = self.dropout(probs)
        x = torch.matmul(probs, value)  # (batch, head, time1, d_k)
        x = (
            x.transpose(1, 2).contiguous().view(batch_size, -1, self.h * self.d_k)
        )  # (batch, time1, d_model)

        scores = self.linear_out(x)  # (batch, time1, d_model)

#        scores = scores.transpose(0, 1)
        return scores, None

    def apply_rotary_embedding(self, hidden_states, offset=0):
#        T, B, C = hidden_states.size()
        B, T, C = hidden_states.size()
#        hidden_states = hidden_states.view(T, B, self.h, self.d_k)
        hidden_states = hidden_states.view(B, T, self.h, self.d_k)
        cos, sin = self.rotary_emb(hidden_states, seq_len=T)

#        cos = cos[offset : hidden_states.shape[0] + offset, ...]
#        sin = sin[offset : hidden_states.shape[0] + offset, ...]
        cos = cos[offset : T + offset, ...]
        sin = sin[offset : T + offset, ...]

        hidden_states = hidden_states.transpose(0, 1)
        hidden_states = (hidden_states * cos) + (self.rotate_half(hidden_states) * sin)
        hidden_states = hidden_states.transpose(0, 1)
        hidden_states = hidden_states.view(B, T, self.h * self.d_k)
    ...
    def _apply_relative_embeddings(self, query, key, pos_emb):
...
#        hidden_states = hidden_states.transpose(0, 1)
        relative_position_embeddings = self.embed_positions(hidden_states) if self.embed_positions is not None else hidden_states
...
WAV2VEC2_CONFORMER_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            [wav2vec2_conformer-base](https://huggingface.co/facebook/wav2vec2-conformer-large-rel-pos), `attention_mask` should **not** be
...
# Copied from transformers.models.wav2vec2.modeling_wav2vec2.Wav2Vec2ForXVector with Wav2Vec2->Wav2Vec2Conformer,wav2vec2->wav2vec2_conformer,WAV_2_VEC_2->WAV2VEC2_CONFORMER
