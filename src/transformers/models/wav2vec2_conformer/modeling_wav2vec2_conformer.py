...
class Wav2Vec2ConformerSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, key_padding_mask=None, pos_emb=None):
        ...
        if self.position_embeddings_type == "rotary":
            hidden_states = self.apply_rotary_embedding(hidden_states)

        query, key, value = self.forward_qkv(hidden_states, hidden_states, value)
...
#
#        n_batch = query.size(0)
#        query = self.linear_q(query).view(n_batch, -1, self.h, self.d_k)
#        key = self.linear_k(key).view(n_batch, -1, self.h, self.d_k)
#        value = self.linear_v(value).view(n_batch, -1, self.h, self.d_k)
#        query = query.transpose(1, 2)  # (batch, head, time1, d_k)
#        key = key.transpose(1, 2)  # (batch, head, time2, d_k)
#        value = value.transpose(1, 2)  # (batch, head, time2, d_k)
        ...
        scores = self.forward_attention(value, scores, key_padding_mask)
        scores = scores.transpose(0, 1)
        return scores, None
    ...
    def apply_rotary_embedding(self, hidden_states, offset=0):
        ...
        query = (hidden_states * cos) + (self.rotate_half(hidden_states) * sin)
        query = query.view(T, B, self.h * self.d_k)

        return query

#        key = (hidden_states * cos) + (self.rotate_half(hidden_states) * sin)
#        key = key.view(T, B, self.h * self.d_k)
#        return query, key

    def n_apply_rotary_embedding(self, query, key, value, offset=0):
        T, B, C = value.size()
        query = query.view(T, B, self.h, self.d_k)
        key = key.view(T, B, self.h, self.d_k)
        value = value.view(T, B, self.h, self.d_k)
        ...
        cos, sin = (
            cos[offset : query.shape[0] + offset, ...],
            sin[offset : query.shape[0] + offset, ...],
        )
        query, key = (query * cos) + (self.rotate_half(query) * sin), (key * cos) + (self.rotate_half(key) * sin)
