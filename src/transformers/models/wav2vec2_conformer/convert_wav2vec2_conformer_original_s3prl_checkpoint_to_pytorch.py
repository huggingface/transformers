...
def convert_classification(base_model_name, hf_config, downstream_dict):
    model = Wav2Vec2ConformerForSequenceClassification.from_pretrained(base_model_name, config=hf_config)
...
def convert_diarization(base_model_name, hf_config, downstream_dict):
    model = Wav2Vec2ConformerForAudioFrameClassification.from_pretrained(base_model_name, config=hf_config)
...
def convert_xvector(base_model_name, hf_config, downstream_dict):
    model = Wav2Vec2ConformerForXVector.from_pretrained(base_model_name, config=hf_config)
...
def convert_s3prl_checkpoint(base_model_name, config_path, checkpoint_path, model_dump_path):
    ...
    hf_config = Wav2Vec2ConformerConfig.from_pretrained(config_path)
