...
def extract_nemo_archive(nemo_file_path: str, extract_dir: str) -> Dict[str, str]:
    ...
    for root, dirs, files in os.walk(extract_dir):
        for file in files:
            ...
            if (file.endswith('.pt') or file.endswith('.pth') or 
                file.endswith('.ckpt') or file.endswith('.bin') or
                'model' in file_lower and ('weight' in file_lower or 'state' in file_lower) or
                file_lower == 'model.pt' or file_lower == 'pytorch_model.bin' or
                file_lower == 'model_weights.ckpt'):
                model_files['model_weights'] = file_path
                logger.info(f"Found model weights: {file}")
                
            # Look for config files
            elif (file == 'model_config.yaml' or file == 'config.yaml' or
                  (file.endswith('.yaml') and 'config' in file_lower)):
...
def create_hf_config_from_nemo(model_info: Dict[str, Any], state_dict: Dict[str, torch.Tensor]) -> Union[FastConformerConfig, ParakeetCTCConfig]:
    ...
    if layer_keys:
        layer_nums = [int(re.search(r'encoder\.layers\.(\d+)\.', k).group(1)) for k in layer_keys if re.search(r'encoder\.layers\.(\d+)\.', k)]
        if layer_nums:
    ...
    ff_keys = [k for k in state_dict.keys() if 'feed_forward_1.linear1.weight' in k or 'feed_forward1.linear1.weight' in k]
    if ff_keys:
    ...
    config_params = {
        ...
        "intermediate_size": encoder_cfg.get('ff_expansion_factor', 4) * encoder_cfg.get('d_model', actual_hidden_size) if 'ff_expansion_factor' in encoder_cfg else actual_ffn_dim,
        "hidden_act": "silu",
...
def create_hf_model(hf_config: Union[FastConformerConfig, ParakeetCTCConfig], hf_state_dict: Dict[str, torch.Tensor], model_info: Dict[str, Any]) -> Union[FastConformerModel, ParakeetCTC]:
...
def convert_nemo_to_hf(input_path: str, output_dir: str) -> Dict[str, Any]:
    ...
    if input_path.endswith('.nemo'):
        ...
        weight_files = (list(Path(input_path).glob("*.pt")) + 
...
def main():
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True, 
        help="Path to .nemo file or extracted NeMo model directory"
    )
    parser.add_argument(
        "--output_dir", 
        type=str, 
    ...
    parser.add_argument(
        "--verify", 
        action="store_true", 
        help="Verify conversion by testing model loading and forward pass"
    )
    parser.add_argument(
        "--force", 
        action="store_true", 
