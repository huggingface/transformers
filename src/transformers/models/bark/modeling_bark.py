...
class BarkFineModel(BarkPreTrainedModel):
    ...
    def _resize_token_embeddings(self, new_num_tokens, pad_to_multiple_of = None):
        ...
        new_embeddings_list = nn.ModuleList(
            [self._get_resized_embeddings(old_embeddings, new_num_tokens, pad_to_multiple_of) for old_embeddings in old_embeddings_list]
        )
        ...
        if self.get_output_embeddings() is not None and not self.config.tie_word_embeddings:
            ...
            new_lm_head_list = nn.ModuleList(
                [self._get_resized_lm_head(old_lm_head, new_num_token) for old_lm_head, new_num_token in zip(old_lm_head_list, new_num_tokens)]
            )
