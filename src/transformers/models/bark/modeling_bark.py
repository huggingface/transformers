...
from ...generation.logits_process import (AlternatingCodebooksLogitsProcessor,
...
from .configuration_bark import (BarkCoarseConfig, BarkConfig, BarkFineConfig,
                                 BarkSemanticConfig, BarkSubModelConfig)
from .generation_configuration_bark import (BarkCoarseGenerationConfig,
    ...
    def generate(
        ...
        **kwargs,
        ...
        semantic_output = super().generate(
            torch.ones((batch_size, max_input_semantic_length + 1), dtype=torch.int).to(
                self.device
            ),
            input_embeds=input_embeds,
            logits_processor=[
...
class BarkCoarseModel(BarkCausalModel):
    ...
    def preprocess_histories(
        ...
        if history_prompt is not None:
            ...
            # shape: (batch_size, 0)
            x_semantic_history = torch.tensor([[]] * batch_size, dtype=torch.int).to(
                self.device
            )
            x_coarse_history = torch.tensor([[]] * batch_size, dtype=torch.int).to(
                self.device
            )
