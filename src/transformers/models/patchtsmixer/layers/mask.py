...
class PatchMasking(nn.Module):
    def __init__(
        ...
        Args:
            ...
            cv_channel_indices (list, optional):
            ...
            channel_consistent_masking (bool, optional):
                When true, masking will be same across all channels of a timeseries. Otherwise, masking positions will
                vary across channels. Defaults to True.
    ...
    def forward(self, x: Tensor):
        ...
        Args:
            x: patched input
                4D: [bs x n_vars x num_patch x patch_len] 6D: [bs x tsg1 x tsg2 x n_vars x num_patch x patch_len]

        Returns:
            x_mask: Masked patched input
                4D: [bs x n_vars x num_patch x patch_len] 6D: [bs x tsg1 x tsg2 x n_vars x num_patch x patch_len]
            mask: bool tensor indicating True on masked points
                4D: [bs x n_vars x num_patch] 6D: [bs x tsg1 x tsg2 x n_vars x num_patch]
        """
        ...
        if self.mask_type == "random":
            ...
            x_mask, mask = multi_forecast_masking(
                xb=x,
