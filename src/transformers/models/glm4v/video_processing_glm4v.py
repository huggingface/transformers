...
class Glm4vVideoProcessor(BaseVideoProcessor):
    ...
    def sample_frames(
        ...
        **kwargs,
        ...
        video_fps = metadata.fps or 2.0
        max_frame_idx = total_frames - 1
        duration = metadata.duration or round(max_frame_idx / video_fps) + 1

        if duration <= self.max_duration:
            n = int(math.floor(duration * requested_fps))
            frame_indices = [min(max_frame_idx, int(math.ceil(i * video_fps / requested_fps))) for i in range(n)]
        else:
