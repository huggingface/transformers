...
_CONFIG_FOR_DOC = "Eve2Config"


class Eve2Config(Qwen2Config):
    model_type = "eve2"
...
class Eve2ImageProcessor(CLIPImageProcessor):
    pass
...
class Eve2DivideAndConquerModule(nn.Module):
    ...
    def forward(self, hidden_states, visual_token_mask):
        visual_projection = self.visual_projection(hidden_states)
        textual_projection = self.textual_projection(hidden_states)
        visual_projection = visual_projection * visual_token_mask
        textual_projection = textual_projection * (1 - visual_token_mask)
        return visual_projection + textual_projection


class Eve2VisionEmbeddings(nn.Module):
    def __init__(self, config: Eve2Config):
        super().__init__()
    ...
    def forward(self, pixel_values):
        patch_embeds = []
        target_dtype = self.patch_embedding[0].weight.dtype
        for i in range(len(pixel_values)):
            pixel_value = pixel_values[i].to(dtype=target_dtype)
            ...
            patch_embed = torch.cat([patch_embed, split_embed.to(dtype=target_dtype)], dim=-1)
            class_embed = self.class_embedding[None, :].to(dtype=target_dtype)
            patch_embeds.append(torch.cat([class_embed, patch_embed.flatten(1).transpose(0, 1)], dim=0))

        return patch_embeds


class Eve2Attention(Qwen2Attention):
    def __init__(self, config: Eve2Config, layer_idx: int):
        super(Qwen2Attention, self).__init__(config, layer_idx)
        self.q_proj = Eve2DivideAndConquerModule(
            nn.Linear,
            in_features=config.hidden_size,
            out_features=config.num_key_value_heads * self.head_dim,
            bias=True,
        )
        self.k_proj = Eve2DivideAndConquerModule(
            nn.Linear,
            in_features=config.hidden_size,
            out_features=config.num_key_value_heads * self.head_dim,
            bias=True,
        )
        self.v_proj = Eve2DivideAndConquerModule(
            nn.Linear,
            in_features=config.hidden_size,
            out_features=config.num_key_value_heads * self.head_dim,
            bias=True,
        )
        self.o_proj = Eve2DivideAndConquerModule(
            nn.Linear,
...
class Eve2DecoderLayer(Qwen2DecoderLayer):
    def __init__(self, config: Eve2Config, layer_idx: int):
        super(Qwen2DecoderLayer, self).__init__(config)
        self.mlp = Eve2DivideAndConquerModule(Eve2MLP, config=config)
        self.input_layernorm = Eve2DivideAndConquerModule(
            Eve2RMSNorm, hidden_size=config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = Eve2DivideAndConquerModule(
            Eve2RMSNorm, hidden_size=config.hidden_size, eps=config.rms_norm_eps
        )


class Eve2Model(Qwen2Model):
    config_class = Eve2Config

    def __init__(self, config: Eve2Config):
        super().__init__(config)

        self.vision_embeddings = Eve2VisionEmbeddings(config)


class Eve2ForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: Eve2Config):
        super().__init__(config)
        self.model = Eve2Model(config)
    ...
    def get_transfer_tensor(self, tmp_tensor):
        return tmp_tensor.to(dtype=self.model.dtype, device=self.model.device)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                cur_visual_mask.append(self.get_transfer_tensor(torch.zeros_like(cur_labels_noim[i]).unsqueeze(-1)))
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    ...
                    cur_visual_mask.append(self.get_transfer_tensor(torch.ones(cur_image_features.shape[0], 1)))
