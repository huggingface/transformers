...
class EnCodecFeatureExtractor(SequenceFeatureExtractor):
    r"""
    Constructs a EnCodec feature extractor.

    This class can pre-process a raw speech signal by (optionally) normalizing to zero-mean unit-variance, for use by
    the EnCodec speech encoder prenet.

    This class can also extract log-mel filter bank features from raw speech, for use by the EnCodec speech decoder
    prenet.
