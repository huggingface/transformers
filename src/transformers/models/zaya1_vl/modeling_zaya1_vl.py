...
class Zaya1VLQKNorm(nn.Module):
    """
    L2-normalizes q/k states to sqrt(head_dim) and applies ZAYA1_VL's learned per-KV-head key scale.
    """
...
class Zaya1VLAttention(nn.Module):
    ...
    def __init__(self, config: Zaya1VLTextConfig, layer_idx: int):
        ...
        self.qkv_proj = Zaya1VLCCAProjection(
            config=self.config,
            layer_idx=layer_idx,
        )
        ...
        self.hidden_size = config.hidden_size
        self.num_attention_heads = config.num_attention_heads
        self.qk_norm = Zaya1VLQKNorm(config)
        if config.vision_lora:
...
class Zaya1VLSparseMoeBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        self.gate = Zaya1VLRouter(config, layer_idx)
        self.experts = Zaya1VLExperts(config)
...
class Zaya1VLPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, Zaya1VLModel):
