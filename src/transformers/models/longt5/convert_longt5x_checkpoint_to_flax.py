...
def convert_t5x_checkpoint_to_flax(t5x_checkpoint_path, config_name, flax_dump_folder_path):
    ...
    for layer_index in range(config.num_layers):
        ...
        # Assigning
        flax_model.params["encoder"]["block"][str(layer_index)]["layer"]["0"][encoder_attn_name]["k"][
            "kernel"
        ] = t5x_attention_key
        flax_model.params["encoder"]["block"][str(layer_index)]["layer"]["0"][encoder_attn_name]["o"][
            "kernel"
        ] = t5x_attention_out
        flax_model.params["encoder"]["block"][str(layer_index)]["layer"]["0"][encoder_attn_name]["q"][
            "kernel"
        ] = t5x_attention_query
        flax_model.params["encoder"]["block"][str(layer_index)]["layer"]["0"][encoder_attn_name]["v"][
            "kernel"
        ] = t5x_attention_value
