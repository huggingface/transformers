# coding=utf-8
# Copyright 2020 LongT5 Authors and The HuggingFace Inc. team.
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5DenseReluDense with T5->LongT5
class TFLongT5DenseReluDense(tf.keras.layers.Layer):
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5GatedGeluDense with T5->LongT5
class TFLongT5GatedGeluDense(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        wi_initializer = tf.keras.initializers.RandomNormal(
            mean=0, stddev=config.initializer_factor * (config.d_model**-0.5)
...
class TFLongT5LayerFF(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        if config.feed_forward_proj == "relu":
            self.DenseReluDense = TFLongT5DenseReluDense(config, name="DenseReluDense")
        elif config.feed_forward_proj == "gated-gelu":
            self.DenseReluDense = TFLongT5GatedGeluDense(config, name="DenseReluDense")
        else:
            raise ValueError(
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5Block with T5->LongT5
...
####################################################
@keras_serializable
# Copied from transformers.models.t5.modeling_tf_t5.TFT5MainLayer with T5->LongT5
class TFLongT5MainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # LongT5 has a mask that can compare sequence ids, we can simulate this here with this transposition
...
####################################################
# Copied from transformers.models.t5.modeling_tf_t5.TFT5PreTrainedModel with T5->LongT5
class TFLongT5PreTrainedModel(TFPreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        ), "self.model.config.decoder_start_token_id has to be defined. In TF LongT5 it is usually set to the pad_token_id. See LongT5 docs for more information"
...
class TFLongT5Model(TFLongT5PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = TFLongT5MainLayer(encoder_config, embed_tokens, name="encoder")

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = TFLongT5MainLayer(decoder_config, embed_tokens, name="decoder")
    ...
    def call(
        ...
        ```python
        >>> from transformers import T5Tokenizer, TFLongT5Model
...
class TFLongT5ForConditionalGeneration(TFLongT5PreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def call(
        ...
        ```python
        >>> from transformers import T5Tokenizer, TFLongT5ForConditionalGeneration
        ...
        # LongT5v1.1 does not tie output word embeddings and thus does not require downscaling
        if self.config.tie_word_embeddings:
...
class TFLongT5EncoderModel(TFLongT5PreTrainedModel):
    ...
    def call(
        ...
        ```python
        >>> from transformers import T5Tokenizer, TFLongT5EncoderModel

        >>> tokenizer = T5Tokenizer.from_pretrained("")
        >>> model = TFLongT5EncoderModel.from_pretrained("")
