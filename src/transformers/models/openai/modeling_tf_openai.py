...
class TFBlock(tf.keras.layers.Layer):
    def __init__(self, n_positions, config, scale=False, **kwargs):
        super().__init__(**kwargs)
        nx = config.n_embd
        self.attn = TFAttention(nx, n_positions, config, scale, name="attn")
...
class TFOpenAIGPTMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.h = [TFBlock(config.n_positions, config, scale=True, name=f"h_._{i}") for i in range(config.n_layer)]
