...
from .configuration_llama_infinite import LlamaInfiniteConfig
...
_CONFIG_FOR_DOC = "LlamaInfiniteConfig"
...
class LlamaInfiniteMLP(nn.Module):
...
class LlamaInfinitePreTrainedModel(PreTrainedModel):
    config_class = LlamaInfiniteConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LlamaInfiniteDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LlamaInfiniteModel):
...
class LlamaInfiniteModel(LlamaInfinitePreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaInfiniteDecoderLayer`]
...
class LlamaInfiniteForCausalLM(LlamaInfinitePreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaInfiniteModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, LlamaInfiniteForCausalLM

        >>> model = LlamaInfiniteForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
class LlamaInfiniteForSequenceClassification(LlamaInfinitePreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
