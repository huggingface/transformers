...
MIMI_ATTENTION_CLASSES = {
    "flash_attention_2": None,  # Not implemented yet
...
class MimiAttention(nn.Module):
    ...
    def __init__(self, config: MimiConfig, layer_idx: Optional[int] = None):
        ...
        self.attn_impl = config._attn_implementation if hasattr(config, "_attn_implementation") else "sdpa"
...
class MimiSdpaAttention(MimiAttention):
    ...
    def forward(
        ...
        attn_output, attn_weights = MIMI_ATTENTION_FUNCTION[attention_type](
            self, hidden_states, attention_mask=attention_mask, output_attentions=output_attentions
        )
...
class MimiTransformerLayer(nn.Module):
    def __init__(self, config: MimiConfig, layer_idx: int):
        ...
        attention_class = MIMI_ATTENTION_CLASSES.get(attn_implementation, MimiAttention)
        if attention_class is None:
            raise ValueError(f"Attention implementation {attn_implementation} is not supported")
        self.self_attn = attention_class(config=config)
