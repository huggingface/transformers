...
class YosoSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return context_layer, context_layer
...
class YosoAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None) -> tuple[torch.Tensor, torch.Tensor]:
        context_layer, attention_weights = self.self(hidden_states, attention_mask)
...
class YosoLayer(GradientCheckpointingLayer):
    ...
    def forward(self, hidden_states, attention_mask=None) -> torch.Tensor:
...
class YosoPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": YosoLayer,
        "attentions": YosoAttention,
    }
