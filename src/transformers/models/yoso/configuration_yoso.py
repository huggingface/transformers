...
YOSO_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    "yoso-base": "https://huggingface.co/yoso-base/resolve/main/config.json",
...
class YosoConfig(PretrainedConfig):
    ...
    the YOSO `yoso-base <https://huggingface.co/yoso-base>`__ architecture.
    ...
    Args:
        ...
        use_expectation (`bool`, *optional*, defaults to `False`):
            Whether or not to use YOSO_E.
        hash_code_len (:obj:`int`, `optional`, defaults to 9):
            The length of hashes generated by the hash functions.
        num_hash (:obj:`int`, `optional`, defaults to 64):
            Number of hash functions used in :class:`~transformers.YosoSelfAttention`.
        conv_window (:obj:`int`, `optional`, defaults to None):
            Kernel size of depth-wise convolution in :class:`~transformers.YosoSelfAttention`.
        use_fast_hash (`bool`, *optional*, defaults to `False`):
            Whether or not to use fast hash.
        Example::

        >>> from transformers import YosoModel, YosoConfig

        >>> # Initializing a YOSO yoso-base style configuration
        >>> configuration = YosoConfig()

        >>> # Initializing a model from the yoso-base style configuration
