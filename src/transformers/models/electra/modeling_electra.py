...
# Modification of the original ElectraEmbedding called CellEmbedding for our modified architecture       
class CellEmbeddings(ElectraEmbeddings):
    """Construct the embeddings from word, position and token_type embeddings."""
    ...
    def forward(
        ...
        embeddings = inputs_embeds + token_type_embeddings
        # Removed position_embedding_type check
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class ElectraLayer(nn.Module):
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = CellSelfAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = CellSelfAttention(config)
