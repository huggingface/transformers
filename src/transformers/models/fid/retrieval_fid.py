...
"""FID Retriever model implementation."""
...
# from .tokenization_fid import RagTokenizer
...
LEGACY_INDEX_PATH = "https://stofide.googleapis.com/huggingface-nlp/datasets/wiki_dpr/"
...
class Index:
    """
    A base class for the Indices encapsulated by the [`FiDRetriever`].
    """
    ...
    def init_index(self):
        """
        A function responsible for loading the index into memory. Should be called only once per training run of a FID
...
class LegacyIndex(Index):
    ...
    Args:
        ...
        index_path (`str`):
            A path to a *directory* containing index files compatible with [`~models.fid.retrieval_fid.LegacyIndex`]
    """
...
class FiDRetriever:
    ...
    contents, and it formats them to be used with a FiDModel.

    Args:
        config ([`FiDConfig`]):
            The configuration of the FID model this Retriever is used with. Contains parameters indicating which
        ...
        generator_tokenizer ([`PreTrainedTokenizer`]):
            The tokenizer used for the generator part of the FiDModel.
        index ([`~models.fid.retrieval_fid.Index`], optional, defaults to the one defined by the configuration):
    ...
    >>> from transformers import FiDRetriever

    >>> retriever = FiDRetriever.from_pretrained(
    ...     "facebook/dpr-ctx_encoder-single-nq-base", dataset="wiki_dpr", index_name="compressed"
    ... )

    >>> # To load your own indexed dataset built with the datasets library. More info on how to build the indexed dataset in examples/fid/use_own_knowledge_dataset.py
    >>> from transformers import FiDRetriever

    >>> dataset = (
    ...     ...
    >>> )  # dataset must be a datasets.Datasets object with columns "title", "text" and "embeddings", and it must have a faiss index
    >>> retriever = FiDRetriever.from_pretrained("facebook/dpr-ctx_encoder-single-nq-base", indexed_dataset=dataset)

    >>> # To load your own indexed dataset built with the datasets library that was saved on disk. More info in examples/fid/use_own_knowledge_dataset.py
    >>> from transformers import FiDRetriever
    ...
    >>> # To load the legacy index built originally for FiD's paper
    >>> from transformers import FiDRetriever

    >>> retriever = FiDRetriever.from_pretrained("facebook/dpr-ctx_encoder-single-nq-base", index_name="legacy")
    ```"""
    ...
    def from_pretrained(cls, retriever_name_or_path, indexed_dataset=None, **kwargs):
        ...
        config = kwargs.pop("config", None) or FiDConfig.from_pretrained(retriever_name_or_path, **kwargs)
        fid_tokenizer = RagTokenizer.from_pretrained(retriever_name_or_path, config=config)
        question_encoder_tokenizer = fid_tokenizer.question_encoder
        generator_tokenizer = fid_tokenizer.generator
    ...
    def postprocess_docs(self, docs, input_strings, prefix, n_docs, return_tensors=None):
        ...
        def cat_input_and_doc(doc_title, doc_text, input_string, prefix):
            # TODO(Patrick): if we train more FID models, I want to put the input first to take advantage of effortless truncation
