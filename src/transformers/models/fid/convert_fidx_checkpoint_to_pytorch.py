...
"""
Convert FiDX checkpoint to PyTorch

Steps:
- Install gsutil according to https://cloud.google.com/storage/docs/gsutil_install
- Get a FiDX checkpoint at https://github.com/google-research/fidx/blob/main/docs/models.md#fid-11-checkpoints Example:
    `gsutil -m cp -r gs://fid-data/pretrained_models/fidx/fid_1_1_small $HOME/`
- Create or download a corresponding config for the downloaded model. E.g. for FiD v1.1 small, you can use
    https://huggingface.co/google/fid-v1_1-small/blob/main/config.json
- Convert:
    ```
    python3 convert_fidx_checkpoint_to_pytorch.py --fidx_checkpoint_path=$HOME/fid_1_1_small --config_file=config.json\
      --pytorch_dump_path=$HOME/fid_1_1_small_pt
    ```
"""
...
from transformers import FiDConfig, FiDEncoderModel, FiDForConditionalGeneration
...
def fidx_attention_lookup(params, i, prefix, layer_name="attention"):
...
def fidx_mlp_lookup(params, i, prefix, split_mlp_wi=False):
...
def fidx_layer_norm_lookup(params, i, prefix, layer_name):
    """Returns the layer norm param of a layer."""
    return params[f"{prefix}/layers_{i}/{layer_name}/scale"]


def convert_fidx_to_pytorch(variables: dict, *, num_layers: int, num_decoder_layers: int, is_encoder_only: bool):
    """Converts the parameters from FiDX-Flax to Transformers-PyTorch."""
    ...
    for i in range(num_layers):
        ...
        layer_norm = fidx_layer_norm_lookup(old, i, "encoder", "pre_attention_layer_norm")
        k, o, q, v = fidx_attention_lookup(old, i, "encoder", "attention")
        ...
        # Block i, layer 1 (MLP).
        layer_norm = fidx_layer_norm_lookup(old, i, "encoder", "pre_mlp_layer_norm")
        wi, wo = fidx_mlp_lookup(old, i, "encoder", split_mlp_wi)
    ...
    if not is_encoder_only:
        ...
        for i in range(num_decoder_layers):
            # Block i, layer 0 (Self Attention).
            layer_norm = fidx_layer_norm_lookup(old, i, "decoder", "pre_self_attention_layer_norm")
            k, o, q, v = fidx_attention_lookup(old, i, "decoder", "self_attention")
            ...
            layer_norm = fidx_layer_norm_lookup(old, i, "decoder", "pre_cross_attention_layer_norm")
            k, o, q, v = fidx_attention_lookup(old, i, "decoder", "encoder_decoder_attention")
            ...
            # Block i, layer 2 (MLP).
            layer_norm = fidx_layer_norm_lookup(old, i, "decoder", "pre_mlp_layer_norm")
            wi, wo = fidx_mlp_lookup(old, i, "decoder", split_mlp_wi)
...
def load_fidx_weights_in_fid(model, config, fidx_checkpoint_path, is_encoder_only):
    """Replaces the params in model witht the FiDX converted params."""
    variables = checkpoints.load_fidx_checkpoint(fidx_checkpoint_path)
    converted = convert_fidx_to_pytorch(
        variables,
...
def convert_fidx_checkpoint_to_pytorch(
    fidx_checkpoint_path, config_file, pytorch_dump_path, is_encoder_only: bool = False
):
    """Loads the config and model, converts the FiDX checkpoint, and saves a PyTorch checkpoint."""
    # Initialise PyTorch model
    config = FiDConfig.from_json_file(config_file)
    print(f"Building PyTorch model from configuration: {config}")
    # Non-v1.1 checkpoints could also use FiDModel, but this works for all.
    # The v1.0 checkpoints will simply have an LM head that is the word embeddings.
    if is_encoder_only:
        model = FiDEncoderModel(config)
    else:
        model = FiDForConditionalGeneration(config)

    # Load weights from tf checkpoint
    load_fidx_weights_in_fid(model, config, fidx_checkpoint_path, is_encoder_only)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Converts a native FiDX checkpoint into a PyTorch checkpoint.")
    # Required parameters
    parser.add_argument(
        "--fidx_checkpoint_path", default=None, type=str, required=True, help="Path to the FiDX checkpoint."
    )
    parser.add_argument(
        "--config_file",
        default=None,
        type=str,
        required=True,
        help="The config json file corresponding to the pre-trained FiD model.\nThis specifies the model architecture.",
    )
    parser.add_argument(
    ...
    convert_fidx_checkpoint_to_pytorch(
        args.fidx_checkpoint_path, args.config_file, args.pytorch_dump_path, args.is_encoder_only
    )
