...
class BLTModel(BLTPreTrainedModel):
    """
    The ByteLatentTransformer (BLT) model that processes byte sequences by dynamically segmenting them into patches.
    It uses a combination of local encoders, global transformers, and local decoders to efficiently encode and decode
    byte sequences.
    """
    ...
    def __init__(self, config: BLTConfig):
        ...
        # Special tokens
        self.boe_id = config.boe_token_id
    ...
    def _get_patch_lengths(self, input_ids: torch.Tensor) -> torch.Tensor:
        """Compute patch lengths based on the patching mode and threshold."""
        batch_size, seq_len = input_ids.shape
        
        if self.patching_mode == "fixed":
            # Fixed-size patches
            patch_lengths = torch.full((batch_size, seq_len // self.patch_size), self.patch_size)
            if seq_len % self.patch_size != 0:
                last_patch = torch.full((batch_size, 1), seq_len % self.patch_size)
                patch_lengths = torch.cat([patch_lengths, last_patch], dim=1)
        elif self.patching_mode == "entropy":
            # Dynamic patching based on entropy
            # TODO: Implement entropy-based patching
            raise NotImplementedError("Entropy-based patching not implemented yet")
        else:
            raise ValueError(f"Unknown patching mode: {self.patching_mode}")
    ...
    def _process_patches(
        ...
        """Process input tokens into patches for encoder, global, and decoder."""
        batch_size, seq_len = input_ids.shape
        
        # Add BOE tokens if needed
        nb_boe = int(0 if self.patching_mode != "" else self.patch_size - 1)
        if nb_boe > 0:
            boe_tokens = torch.full((batch_size, nb_boe), self.boe_id, device=input_ids.device)
            input_ids = torch.cat([boe_tokens, input_ids], dim=1)
            
        # Create encoder tokens
        encoder_tokens = input_ids
        
        # Create global tokens
        global_tokens = torch.full((batch_size, patch_lengths.shape[1]), self.boe_id, device=input_ids.device)
        rows, cols = torch.where(input_ids == self.eos_id)
        eos_patch_ids = torch.div(cols, self.patch_size, rounding_mode='floor')
        global_tokens[rows, eos_patch_ids] = self.eos_id
        
        # Create decoder tokens
        decoder_tokens = input_ids[:, nb_boe:nb_boe + seq_len]
        
        return encoder_tokens, global_tokens, decoder_tokens
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Process input into patches
        patch_lengths = self._get_patch_lengths(input_ids)
        encoder_tokens, global_tokens, decoder_tokens = self._process_patches(input_ids, patch_lengths)
        
        # Local encoder
        encoder_outputs = self.local_encoder(
        ...
        # Global transformer
        global_outputs = self.global_transformer(
        ...
        # Local decoder
        decoder_outputs = self.local_decoder(
            input_ids=decoder_tokens,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
            inputs_embeds=None,  # Use decoder tokens directly
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            cross_attention_kv=global_outputs.last_hidden_state,
        )
...
class BLTCrossAttention(nn.Module):
    """Cross attention module for BLT that handles attention between different components."""
    ...
    def __init__(self, config: BLTConfig):
        ...
        self.norm_q = BLTRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.norm_kv = BLTRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        k = self.k_proj(kv_norm).view(batch_size, kv_len, self.num_key_value_heads, self.head_dim)
        v = self.v_proj(kv_norm).view(batch_size, kv_len, self.num_key_value_heads, self.head_dim)
...
class BLTLocalEncoder(nn.Module):
    """Local encoder component of BLT that processes byte sequences with local attention."""
    ...
    def __init__(self, config: BLTConfig):
        ...
        # Transformer layers
        self.layers = nn.ModuleList([BLTDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        
        # Cross attention layers if enabled
        self.cross_attn = None
        if config.use_cross_attention:
            self.cross_attn = BLTCrossAttention(config)
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        cross_attention_kv: Optional[torch.Tensor] = None,
    ) -> BaseModelOutputWithPast:
        ...
        if self.cross_attn is not None and cross_attention_kv is not None:
            hidden_states = self.cross_attn(hidden_states, cross_attention_kv)
...
class BLTGlobalTransformer(nn.Module):
    """Global transformer component of BLT that handles global context."""
...
class BLTLocalDecoder(nn.Module):
    """Local decoder component of BLT that generates output with local attention."""
