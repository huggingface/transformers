...
class NeoMMEConfig(PreTrainedConfig):
    ...
    layer_types (`list[str]`, *optional*):
        ...
        contain one value per layer. Set `global_attn_every_n_layers=None` unless the list matches the attention
        pattern generated by that setting.
    global_attn_every_n_layers (`int`, *optional*, defaults to 6):
        Number of layers between full-attention layers. The final layer always uses full attention. This setting
        generates `layer_types` when `layer_types` is not set.
    ...
    def generate_layer_types(self, global_attn_every_n_layers: int) -> list[str]:
        """Attention pattern from stride: every Nth layer is global, including the last."""
        return [
            "full_attention"
