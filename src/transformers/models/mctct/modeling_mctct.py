...
class MCTCTForAudioFrameClassification(MCTCTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def freeze_feature_encoder(self):
        ...
        not be updated during training.
        """
        self.mctct.feature_extractor._freeze_parameters()
...
@add_start_docstrings(
    """
    MCTCT Model with a sequence classification head on top (a linear layer over the pooled output) for tasks like
    Language Identification (LID). It creates a single prediction for the full sequence.
    """,
    MCTCT_START_DOCSTRING,
)
class MCTCTForSequenceClassification(MCTCTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
