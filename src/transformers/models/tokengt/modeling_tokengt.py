...
from .performer import FastAttention, ProjectionUpdater, gaussian_orthogonal_random_matrix_batched
...
class TokenGTForGraphClassification(TokenGTPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.decoder.num_classes == 1:  # regression
                loss_fct = MSELoss()
                loss = loss_fct(logits[mask].squeeze(), labels[mask].squeeze().float())
            elif self.decoder.num_classes > 1 and len(labels.shape) == 1:  # One task classification
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits[mask].view(-1, self.decoder.num_classes), labels[mask].view(-1))
