...
_CHECKPOINT_FOR_DOC = "bros-base-uncased"
...
def load_tf_weights_in_bros(model, config, tf_checkpoint_path):
...
class BrosAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = BrosSelfAttention(config, position_embedding_type=position_embedding_type)
...
class BrosLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = BrosAttention(config, position_embedding_type="absolute")
...
class BrosPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_bros
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, BrosEncoder):
...
BROS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            ...
            Indices can be obtained using [`BrosTokenizer`].
...
@add_start_docstrings("""Bros Model with a `language modeling` head on top. """, BROS_START_DOCSTRING)
class BrosForMaskedLM(BrosPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `BrosForMaskedLM` make sure `config.is_decoder=False` for "
...
@add_start_docstrings(
    """Bros Model with a `language modeling` head on top for CLM fine-tuning. """, BROS_START_DOCSTRING
)
class BrosForCausalLM(BrosPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `BrosForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        ```python
        >>> from transformers import BrosTokenizer, BrosForCausalLM, BrosConfig
        >>> import torch

        >>> tokenizer = BrosTokenizer.from_pretrained('bros-base-uncased')
        >>> config = BrosConfig.from_pretrained("bros-base-uncased")
        >>> config.is_decoder = True
        >>> model = BrosForCausalLM.from_pretrained('bros-base-uncased', config=config)
...
class BrosClassificationHead(nn.Module):
...
@add_start_docstrings(
    """Bros Model transformer with a sequence classification/regression head on top (a linear layer on top of
...
class BrosForSequenceClassification(BrosPreTrainedModel):
    def __init__(self, config):
        ...
        self.bros = BrosModel(config)
        self.classifier = BrosClassificationHead(config)
...
@add_start_docstrings(
    """Bros Model with a multiple choice classification head on top (a linear layer on top of
    the pooled output and a softmax) e.g. for RocStories/SWAG tasks. """,
    BROS_START_DOCSTRING,
)
class BrosForMultipleChoice(BrosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_start_docstrings_to_model_forward(BROS_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """Bros Model with a token classification head on top (a linear layer on top of
...
@add_start_docstrings(
    """Bros Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layers on top of the hidden-states output to compute `span start logits` and `span end logits`). """,
    BROS_START_DOCSTRING,
)
class BrosForQuestionAnswering(BrosPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
