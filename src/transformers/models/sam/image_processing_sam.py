...
class SamImageProcessor(BaseImageProcessor):
    ...
    def post_process_masks(
        ...
        Args:
            masks (`Unionlist[torch.Tensor],list[np.ndarray],list[tf.Tensor]]`):
                Batched masks from the mask_decoder in (batch_size, num_channels, height, width) format.
            original_sizes (`Union[torch.Tensor, tf.Tensor, Listtuple[int,int]]]`):
                The original sizes of each image before it was resized to the model's expected input shape, in (height,
                width) format.
            reshaped_input_sizes (`Union[torch.Tensor, tf.Tensor, Listtuple[int,int]]]`):
    ...
    def _post_process_masks_pt(
        ...
        Args:
            masks (`Unionlist[torch.Tensor],list[np.ndarray]]`):
                Batched masks from the mask_decoder in (batch_size, num_channels, height, width) format.
            original_sizes (`Union[torch.Tensor, Listtuple[int,int]]]`):
                The original sizes of each image before it was resized to the model's expected input shape, in (height,
                width) format.
            reshaped_input_sizes (`Union[torch.Tensor, Listtuple[int,int]]]`):
                The size of each image as it is fed to the model, in (height, width) format. Used to remove padding.
            mask_threshold (`float`, *optional*, defaults to 0.0):
                The threshold to use for binarizing the masks.
            binarize (`bool`, *optional*, defaults to `True`):
    ...
    def post_process_for_mask_generation(
        ...
        Args:
            all_masks (`Unionlist[torch.Tensor],list[tf.Tensor]]`):
                List of all predicted segmentation masks
            all_scores (`Unionlist[torch.Tensor],list[tf.Tensor]]`):
                List of all predicted iou scores
            all_boxes (`Unionlist[torch.Tensor],list[tf.Tensor]]`):
