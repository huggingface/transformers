...
class SamImageProcessor(BaseImageProcessor):
    ...
    def post_process_masks(
        self, masks, original_sizes, reshaped_input_sizes, mask_threshold=0.0, binarize=True, pad_size=None, return_tensors="pt"
    ):
        """
        ...
        Returns:
            (`Union[torch.Tensor, tf.Tensor]`): Batched masks in batch_size, num_channels, height, width) format, where (height, width)
    ...
    def post_process_for_mask_generation(self, all_masks, all_scores, all_boxes, crops_nms_thresh, return_tensors="pt"):
        ...
        """
        if return_tensors == "pt":
            return self._post_process_for_mask_generation_pt(all_masks, all_scores, all_boxes, crops_nms_thresh)
        elif return_tensors == "tf":
            return self._post_process_for_mask_generation_tf(all_masks, all_scores, all_boxes, crops_nms_thresh)

    def _post_process_for_mask_generation_pt(self, all_masks, all_scores, all_boxes, crops_nms_thresh):
        """
        Post processes mask that are generated by calling the Non Maximum Suppression algorithm on the predicted masks.
    ...
    def _post_process_for_mask_generation_tf(self, all_masks, all_scores, all_boxes, crops_nms_thresh):
        """
        Post processes mask that are generated by calling the Non Maximum Suppression algorithm on the predicted masks.

        Args:
            all_masks (`List[tf.Tensor]`):
                List of all predicted segmentation masks
