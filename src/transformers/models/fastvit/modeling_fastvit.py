...
class FastViTPatchEmbeddings(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.inference_mode:
            ...
            activation = self.activation(inference_conv) 
            return activation
...
class FastViTmbeddings(nn.Module):
    """
    ...
    def forward(
        ...
        dropout = self.dropout(third_conv)
        return dropout
...
class FastViTAttention(nn.Module):
    def __init__(self, config: FastViTConfig) -> None:
        super().__init__()
        self.attention = FastViTSelfAttention(config)
        self.output = FastViTSelfOutput(config)
...
class FastViTLayer(nn.Module):
    """This corresponds to the Stage implementation."""
    ...
    def __init__(self, config: FastViTConfig, stage: int) -> None:
        ...
        if pos_embs[stage] is not None:
            self.position_emb = FastViTCPE(config.embed_dims[stage], config.embed_dims[stage], inference_mode = False)

        self.stage = FastViTStage(config)
...
FASTVIT_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            Pixel values. Pixel values can be obtained using [`AutoImageProcessor`]. See [`FastViTImageProcessor.__call__`]
            for details.
...
class FastViTModel(FastViTPreTrainedModel):
    def __init__(self, config: FastViTConfig, add_pooling_layer: bool = True, use_mask_token: bool = False):
        ...
        self.embeddings = FastViTmbeddings(config) 
...
class FastViTForMaskedImageModeling(FastViTPreTrainedModel):
    def __init__(self, config: FastViTConfig) -> None:
        ...
        self.vit = FastViTModel(config, add_pooling_layer=False, use_mask_token=True)
    ...
    def forward(
        ...
        >>> model = FastViTForMaskedImageModeling.from_pretrained("google/vit-base-patch16-224-in21k")
...
class FastViTForImageClassification(FastViTPreTrainedModel):
    def __init__(self, config: FastViTConfig) -> None:
        ...
        self.vit = FastViTModel(config, add_pooling_layer=False)
