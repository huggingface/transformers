...
class FastViTConvLayer(nn.Module):
    """
    Build of Convolution Layer following structure proposed in <https://arxiv.org/pdf/2303.14189.pdf> This block has a
    multi-branched architecture at train-time and plain-CNN style architecture at inference time
    """
...
class FastViTRepMixer(nn.Module):
    """
    Part of Metaformer block with RepMixer as token mixer, uses structural reparameterization to lower the memory
    access cost by removing skip-connections in the network. For more info: `MetaFormer Is Actually What You Need for
...
class FastViTForImageClassification(FastViTPreTrainedModel):
    def __init__(self, config: FastViTConfig, inference_mode: bool = False) -> None:
        ...
        self.classifier = (
            nn.Linear(int(hidden_size * 2), config.num_labels) if config.num_labels > 0 else nn.Identity()
        )
