...
@dataclass
# Copied from transformers.models.idefics.modeling_idefics.IdeficsCausalLMOutputWithPast with Idefics->Mllama
class MllamaCausalLMOutputWithPast(ModelOutput):
    """
    Base class for Mllama causal language model (or autoregressive) outputs.
...
# Copied from transformers.models.llava.modeling_llava.LlavaMultiModalProjector with Llava->Mllama
...
class TilePositionEmbedding(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Parameter(
            torch.randn(num_tiles, num_tiles, 1, width) / width**0.5  # changed from / math.sqrt(width)
        )
        self.gated = gated
        if gated:
...
class ImageFeedForward(torch.nn.Module):
    ...
    def forward(self, x):
        hidden = torch.nn.functional.linear(x, self.c_fc.weight, self.c_fc.bias)
        hidden = self.non_linearity(hidden)
        hidden = torch.nn.functional.linear(hidden, self.c_proj.weight)
...
class ImageAttention(nn.Module):
    def __init__(
        ...
        # removing parallelism from core
    ...
    def forward(
        ...
        out = torch.nn.functional.linear(attn_output, self.wo.weight)
...
class MllamaVisionEncoder(PreTrainedModel):
    def __init__(
        self,
        max_num_tiles: int,
        image_size: int = 224,
        ...
        self.image_size = to_2tuple(image_size)  # converts to (image_size, image_size)
    ...
    # TODO do we need this load hook that checks and resizes global/local pos embeds?
    # won't the embeddings always exist from the state dict?
    '''
    def load_hook(
        self,
...
# TODO move that to a configuration json + object
...
class MllamaVisionModel(torch.nn.Module):
    def __init__(self, args: ModelArgs) -> None:
        ...
        if return_intermediate is not None:
            return_intermediate = [int(intermediate_value) for intermediate_value in return_intermediate.split(",")]
            self.vision_input_dim = (
                len(return_intermediate) + 1
            ) * self.vision_input_dim
        # TODO move all of these values to config
        self.patch_size = 14
        self.vision_encoder = MllamaVisionEncoder(
            max_num_tiles=4,
    ...
    def forward(
        ...
        vision_tokens = torch.nn.functional.linear(vision_tokens, self.vision_projection.weight)
...
### Modified Llama text model with cross-attention layers
...
class MllamaLanguageModelRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        MllamaLanguageModelRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class MllamaLanguageModelRotaryEmbedding(nn.Module):
...
class MllamaLanguageModelLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
...
class MllamaLanguageModelDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
...
class MllamaLanguageModelMLP(nn.Module):
...
class MllamaLanguageModelAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: MllamaLanguageModelConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = MllamaLanguageModelRotaryEmbedding(
                self.head_dim,
            ...
            if scaling_type == "linear":
                self.rotary_emb = MllamaLanguageModelLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = MllamaLanguageModelDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class MllamaLanguageModelFlashAttention2(LlamaAttention):
    """
    MllamaLanguageModel flash attention module. This module inherits from `LlamaAttention` as the weights of the module stays
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in MllamaLanguageModelFlashAttention2 __init__.
...
class MllamaLanguageModelSdpaAttention(LlamaAttention):
    """
    MllamaLanguageModel attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `LlamaAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from MllamaLanguageModelAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "LlamaModel is using MllamaLanguageModelSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
LLAMA_ATTENTION_CLASSES = {
    "eager": MllamaLanguageModelAttention,
    "flash_attention_2": MllamaLanguageModelFlashAttention2,
    "sdpa": MllamaLanguageModelSdpaAttention,
}


class MllamaLanguageModelDecoderLayer(nn.Module):
    def __init__(self, config: MllamaLanguageModelConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = MllamaLanguageModel_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = MllamaLanguageModelMLP(config)
        self.input_layernorm = MllamaLanguageModelRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MllamaLanguageModelRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@add_start_docstrings(
    "The bare MllamaLanguageModel Model outputting raw hidden-states without any specific head on top.",
    MllamaLanguageModel_START_DOCSTRING,
)
class MllamaLanguageModelPreTrainedModel(PreTrainedModel):
    config_class = MllamaLanguageModelConfig
...
class MllamaLanguageModelModel(LlamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaDecoderLayer`]

    Args:
        config: MllamaLanguageModelConfig
    """

    def __init__(self, config: MllamaLanguageModelConfig):
        ...
        self.norm = MllamaLanguageModelRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            # TODO this is the biggest modification to llama - hidden states 
            # will be passed here through cross attention and self attention layers, iteratively
            # and the xattn mask should be passed here as well.
            # There is an init schedule of attention layers that ensures that xattn layers 
            # are spread evenly across self attention layers. 
            # However, for us, we probably always init these layers at the same places. 
            # forward pass in original code looks like
            """
            def forward(
                self,
                ...
                return output.float() # this will btw move to float32 always
...
# FIXME we just need the base Llama model (without head) and to add xattn to it
class MllamaLanguageModelForCausalLM(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MllamaLanguageModelModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, MllamaLanguageModelForCausalLM

        >>> model = MllamaLanguageModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf")
...
@add_start_docstrings(
    """
    The MllamaLanguageModel Model transformer with a sequence classification head on top (linear layer).
...
MLLAMA_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`MllamaConfig`] or [`MllamaVisionConfig`]):
            Model configuration class with all the parameters of the model. Initializing with a config file does not
            load the weights associated with the model, only the configuration. Check out the
            [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""
...
# Copied from transformers.models.llava.modeling_llava.LlavaPreTrainedModel with Llava->Mllama,llava->mllama
class MllamaPreTrainedModel(PreTrainedModel):
    config_class = MllamaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MllamaVisionAttention"]
    _skip_keys_device_placement = "past_key_values"
    _supports_flash_attn_2 = True

    def _init_weights(self, module):
        # important: this ported version of Mllama isn't meant for training from scratch - only
        # inference and fine-tuning - so the proper init weights code has been removed - the original codebase
        # https://github.com/haotian-liu/LLaVA/tree/main/mllama should serve for that purpose
        std = (
...
# Copied from transformers.models.llava.modeling_llava.LlavaForConditionalGeneration with LLAVA->MLLAMA,Llava->Mllama,llava->mllama
class MllamaForConditionalGeneration(MllamaPreTrainedModel):
    def __init__(self, config: MllamaConfig):
        super().__init__(config)
        self.vision_model = AutoModel.from_config(config.vision_config) # vit large, 32 layers
        self.vision_global_transformer = AutoModel.from_config(config.global_vision_config) # global transformer, 8 layers
        self.vision_projection = nn.Linear(in_features=config.vision_config.vision_input_dim, out_features=config.vision_config.hidden_size, bias=False)

        self.language_model = AutoModelForCausalLM.from_config(
            config.text_config, attn_implementation=config._attn_implementation
        )  # should match llama3-70b
