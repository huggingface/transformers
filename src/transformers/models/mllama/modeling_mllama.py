...
def _prepare_cross_attention_mask(
    ...
    # Use expand + reshape instead of repeat_interleave for better torch.compile compatibility
    cross_attention_mask = cross_attention_mask.unsqueeze(-1).expand(-1, -1, -1, -1, num_vision_tokens)
    cross_attention_mask = cross_attention_mask.reshape(batch_size, text_total_length, -1)
    cross_attention_mask = cross_attention_mask.unsqueeze(1)

    # invert the mask: 1 (attend) -> 0.0, 0 (not attend) -> min_dtype
    # Use multiplication instead of masked_fill for better torch.compile compatibility
    cross_attention_mask = (1 - cross_attention_mask).to(dtype) * torch.finfo(dtype).min
