...
VIDEO_LLAVA_INPUTS_DOCSTRING = r"""
    Args:
        ...
        pixel_values_images (`torch.Tensor` of shape `(batch_size, num_channels, image_size, image_size)):
...
class VideoLlavaForConditionalGeneration(VideoLlavaPreTrainedModel, GenerationMixin):
    ...
    def get_image_features(
        self, pixel_values_images: torch.Tensor, vision_feature_layer: int, vision_feature_select_strategy: str
    ):
        """
        Obtains image last hidden states from the vision tower and apply multimodal projection.

        Args:
            pixel_values_images (`torch.Tensor]` of shape `(batch_size, channels, height, width)`)
    ...
    def get_video_features(self, pixel_values_videos: torch.Tensor, vision_feature_layer: int):
        """
