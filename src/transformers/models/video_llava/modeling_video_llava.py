...
class VideoLlavaVisionEmbeddings(nn.Module):
    def __init__(self, config: VideoLlavaConfig):
        super().__init__()
        self.config = config
...
class VideoLlavaVisionEncoderLayer(nn.Module):
    def __init__(self, config: VideoLlavaConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = VideLlavaVisionAttention(config)
...
class VideoLlavaVisionEncoder(nn.Module):
    ...
    def __init__(self, config: VideoLlavaConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([VideoLlavaVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class VideoLlavaVisionTransformer(nn.Module):
    def __init__(self, config: VideoLlavaConfig):
        ...
        self.embeddings = VideoLlavaVisionEmbeddings(config)
        self.pre_layrnorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        self.encoder = VideoLlavaVisionEncoder(config)
        self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)

    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=VideoLlavaConfig)
    def forward(
        self,
...
class VideoLlavaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # https://github.com/PKU-YuanGroup/Video-LLaVA/tree/main should serve for that purpose
        std = (
...
# Copied from transformers.models.llava.modeling_llava.LlavaForConditionalGeneration with LLAVA->VideoLlava,Llava->VideoLlava,llava-hf/llava-1.5-7b-hf->LanguageBind/Video-LLaVA-7B
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                if past_key_values is not None and pixel_values is not None and input_ids.shape[1] == 1:
                    ...
                    # if one uses VideoLlava + Fused modules where the cache on the
