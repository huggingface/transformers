...
class GPTSanJapanesePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.LayerNorm):
            ...
            nn.init.normal_(module.embed_tokens.weight, mean=0.0, std=factor * 1.0)
            nn.init.normal_(module.position_embeddings.weight, mean=0.0, std=factor * 1.0)
            if hasattr(module, "extra_position_embeddings") and module.extra_position_embeddings is not None:
                nn.init.normal_(module.extra_position_embeddings.weight, mean=0.0, std=factor * 1.0)
            ...
            nn.init.normal_(module.k_proj.weight, mean=0.0, std=factor * ((d_model * key_value_proj_dim) ** -0.5))
            nn.init.normal_(module.v_proj.weight, mean=0.0, std=factor * ((d_model * key_value_proj_dim) ** -0.5))
            nn.init.normal_(module.q_proj.weight, mean=0.0, std=factor * ((d_model * key_value_proj_dim) ** -0.5))
            nn.init.normal_(module.out_proj.weight, mean=0.0, std=factor * ((n_heads * key_value_proj_dim) ** -0.5))
