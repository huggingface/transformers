...
@add_start_docstrings("""CANINE Model with a `language modeling` head on top. """, CANINE_START_DOCSTRING)
...
@add_start_docstrings(
    """CANINE Model with a `language modeling` head on top for CLM fine-tuning. """, CANINE_START_DOCSTRING
)
class CanineForCausalLM(CaninePreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `CanineForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import CanineTokenizer, CanineForCausalLM, CanineConfig
            >>> import torch

            >>> tokenizer = CanineTokenizer.from_pretrained('google/canine-s')
            >>> config = CanineConfig.from_pretrained("google/canine-s")
            >>> config.is_decoder = True
            >>> model = CanineForCausalLM.from_pretrained('google/canine-s', config=config)
