...
def apply_rotary_pos_emb(tensor: tf.Tensor, sin: tf.Tensor, cos: tf.Tensor) -> tf.Tensor:
    sin = tf.repeat(sin, 2, 3)
    cos = tf.repeat(cos, 2, 3)
...
class TFGPTJAttention(keras.layers.Layer):
    ...
    def call(
        ...
        sin, cos = tf.split(sincos, 2, axis=-1)
        sin = sin[:, None, :, :]
        cos = cos[:, None, :, :]
