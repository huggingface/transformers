...
class FalconYaRNScalingRotaryEmbedding(FalconRotaryEmbedding):
    def __init__(
        self,
        dim,
        ...
        self._cos_cached = (emb.cos() * self.mscale)[:, :].to(torch.get_default_dtype())
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            self._cos_cached = (emb.cos() * self.mscale)[:, :].to(x.dtype)
            self._sin_cached = (emb.sin() * self.mscale)[:, :].to(x.dtype)
        return (
...
class FalconDynamicYaRNScalingRotaryEmbedding(FalconYaRNScalingRotaryEmbedding):
    def __init__(
        self,
        dim,
        max_position_embeddings=2048,
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                self.rotary_emb = FalconYaRNScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
                ...
                self.rotary_emb = FalconDynamicYaRNScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
