...
class FalconRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    @deprecate_kwarg("device", version="5.18")
    def compute_default_rope_parameters(config: FalconConfig, device=None, **kwargs) -> tuple[torch.Tensor, float]:
        """
        ...
        if alibi is not None and causal_mask is not None and causal_mask.ndim == 4:
            ...
            if causal_mask.dtype == torch.bool:
                causal_mask = torch.where(
                    causal_mask, torch.full((), 0.0, device=causal_mask.device, dtype=inputs_embeds.dtype), min_dtype
                )
