...
class FlexOlmoAttention(nn.Module):
    ...
    def __init__(self, config: FlexOlmoConfig, layer_idx: Optional[int] = None):
        ...
        self.q_norm = FlexOlmoRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class FlexOlmoFlashAttention2(FlexOlmoAttention):
    """
    FlexOlmo flash attention module. This module inherits from `FlexOlmoAttention` as the weights of the module stays
    ...
    def forward(
        ...
        **kwargs,
        ...
        # in fp32. (FlexOlmoRMSNorm handles it correctly)
...
class FlexOlmoSdpaAttention(FlexOlmoAttention):
    """
    FlexOlmo attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `FlexOlmoAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from FlexOlmoAttention.forward
    @deprecate_kwarg("past_key_value", new_name="past_key_values", version="4.58")
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "FlexOlmoModel is using FlexOlmoSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
FLEX_OLMO_ATTENTION_CLASSES = {
    "eager": FlexOlmoAttention,
    "flash_attention_2": FlexOlmoFlashAttention2,
    "sdpa": FlexOlmoSdpaAttention,
}
...
class FlexOlmoDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: FlexOlmoConfig, layer_idx: int):
        ...
        self.self_attn = FLEX_OLMO_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = FlexOlmoSparseMoeBlock(config)
        self.input_layernorm = FlexOlmoRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class FlexOlmoPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, FlexOlmoRMSNorm):
