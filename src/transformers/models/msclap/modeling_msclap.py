...
# Copied from transformers.models.roberta.modeling_roberta.RobertaEmbeddings with Roberta->MSClapText, persistent=False->persistent=True
class MSClapTextEmbeddings(nn.Module):
    """
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->MSClapText
class MSClapTextSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in MSClapTextModel forward() function)
...
class MSClapTextSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
MSCLAP_TEXT_SELF_ATTENTION_CLASSES = {
    "eager": MSClapTextSelfAttention,
}


# Copied from transformers.models.bert.modeling_bert.BertAttention with Bert->MSClapText,BERT->MSCLAP_TEXT
class MSClapTextAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = MSCLAP_TEXT_SELF_ATTENTION_CLASSES[config._attn_implementation](
            config, position_embedding_type=position_embedding_type
        )
        self.output = MSClapTextSelfOutput(config)
        self.pruned_heads = set()
...
class MSClapTextIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MSClapTextOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->MSClapText
class MSClapTextLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = MSClapTextAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = MSClapTextAttention(config, position_embedding_type="absolute")
        self.intermediate = MSClapTextIntermediate(config)
        self.output = MSClapTextOutput(config)
...
# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->MSClapText
class MSClapTextEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([MSClapTextLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
class MSClapTextPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class MSClapTextModel(MSClapPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = MSClapTextEmbeddings(config)
        self.encoder = MSClapTextEncoder(config)

        self.pooler = MSClapTextPooler(config) if add_pooling_layer else None
