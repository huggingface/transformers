...
class AudiocraftSelfAttention(nn.Module):
...
class AudiocraftBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden_size = config.hidden_size
        inner_dim = config.intermediate_size if config.intermediate_size is not None else 4 * hidden_size
        self.ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = AudiocraftSelfAttention(config)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.mlp = AudiocraftMLP(inner_dim, config)
...
class AudiocraftPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, AudiocraftModel):
...
class AudiocraftModel(AudiocraftPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([AudiocraftBlock(config, layer_id=i) for i in range(config.num_layers)])
    ...
    @add_start_docstrings_to_model_forward(AUDIOCRAFT_INPUTS_DOCSTRING)
