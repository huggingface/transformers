...
from ...modeling_outputs import BaseModelOutput, ModelOutput, SequenceClassifierOutput
...
class CvtSelfAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.conv_projection_query = CvtSelfAttentionProjection(
            embed_dim, kernel_size, padding_q,
            stride_q, projection_method='linear' if qkv_projection_method == 'avg' else qkv_projection_method
        )
        self.conv_projection_key = CvtSelfAttentionProjection(
            embed_dim, kernel_size, padding_kv,
            stride_kv,projection_method= qkv_projection_method
        )
    ...
    def rearrange_for_multi_head_attention(self, inputs):
        batch_size, hidden_size, _,  = inputs.shape
