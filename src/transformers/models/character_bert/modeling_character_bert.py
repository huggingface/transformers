...
_CHECKPOINT_FOR_SEQUENCE_CLASSIFICATION = "textattack/character_bert-base-uncased-yelp-polarity"
...
# Copied from transformers.models.bert.modeling_bert.BertSdpaSelfAttention with Bert->CharacterBert
class CharacterBertSdpaSelfAttention(CharacterBertSelfAttention):
    def __init__(self, config, position_embedding_type=None):
        super().__init__(config, position_embedding_type=position_embedding_type)
        self.dropout_prob = config.attention_probs_dropout_prob
        self.require_contiguous_qkv = version.parse(get_torch_version()) < version.parse("2.2.0")

    # Adapted from CharacterBertSelfAttention
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        if self.position_embedding_type != "absolute" or output_attentions or head_mask is not None:
            ...
            logger.warning_once(
                "CharacterBertSdpaSelfAttention is used but `torch.nn.functional.scaled_dot_product_attention` does not support "
...
CHARACTER_BERT_SELF_ATTENTION_CLASSES = {
    "eager": CharacterBertSelfAttention,
    "sdpa": CharacterBertSdpaSelfAttention,
}


# Copied from transformers.models.bert.modeling_bert.BertAttention with BERT->CHARACTER_BERT,Bert->CharacterBert
class CharacterBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = CHARACTER_BERT_SELF_ATTENTION_CLASSES[config._attn_implementation](
...
class CharacterBertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, CharacterBertLMPredictionHead):
            module.bias.data.zero_()
...
CHARACTER_BERT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`CharacterBertConfig`]): Model configuration class with all the parameters of the model.
...
# Copied from transformers.models.bert.modeling_bert.BertForPreTraining with BERT->CHARACTER_BERT,Bert->CharacterBert,bert->character_bert,google-bert/bert-base-uncased->helboukkouri/character-bert-base-uncased
class CharacterBertForPreTraining(CharacterBertPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], CharacterBertForPreTrainingOutput]:
        r"""
        ...
        ```python
        >>> from transformers import AutoTokenizer, CharacterBertForPreTraining
        >>> import torch

        >>> tokenizer = AutoTokenizer.from_pretrained("helboukkouri/character-bert-base-uncased")
        >>> model = CharacterBertForPreTraining.from_pretrained("helboukkouri/character-bert-base-uncased")
...
class CharacterBertLMHeadModel(CharacterBertPreTrainedModel, GenerationMixin):
...
class CharacterBertForMaskedLM(CharacterBertPreTrainedModel):
    ...
    def can_generate(cls) -> bool:
        """
        Legacy correction: CharacterBertForMaskedLM can't call `generate()` from `GenerationMixin`, even though it has a
...
# Copied from transformers.models.bert.modeling_bert.BertForNextSentencePrediction with BERT->CHARACTER_BERT,Bert->CharacterBert,bert->character_bert,google-bert/bert-base-uncased->helboukkouri/character-bert-base-uncased
...
@add_start_docstrings(
    """
    CharacterBert Model transformer with a sequence classification/regression head on top (a linear layer on top of the pooled
...
@add_start_docstrings(
    """
    CharacterBert Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
...
@add_start_docstrings(
    """
    CharacterBert Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
