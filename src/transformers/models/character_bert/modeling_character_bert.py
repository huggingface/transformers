...
CHARACTER_BERT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `{0}`):
            ...
            Indices can be obtained using [`CharacterBertTokenizer`]. See [`PreTrainedTokenizer.encode`] and
...
class CharacterBertForPreTraining(CharacterBertPreTrainedModel):
    ...
    def forward(
        ...
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            config.mlm_vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked),
            the loss is only computed for the tokens with labels in `[0, ..., config.mlm_vocab_size]`
