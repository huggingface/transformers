...
class CharacterBertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = CharacterBertSelfAttention(config)
...
class CharacterBertLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = CharacterBertAttention(config)
...
class CharacterBertForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (`optional`, returned when ``labels`` is provided, ``torch.FloatTensor`` of shape :obj:`(1,)`):
            ...
            (classification) loss.
        prediction_logits (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, config.mlm_vocab_size)`):
...
class CharacterBertForPreTraining(CharacterBertPreTrainedModel):
    ...
    def forward(
        ...
        labels (:obj:`torch.LongTensor` of shape ``(batch_size, sequence_length)``, `optional`):
            ...
            config.mlm_vocab_size]`` (see ``input_ids`` docstring) Tokens with indices set to ``-100`` are ignored
