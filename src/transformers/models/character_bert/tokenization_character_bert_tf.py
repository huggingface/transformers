...
from tensorflow_text import CharacterBertTokenizer as CharacterBertTokenizerLayer
from tensorflow_text import FastCharacterBertTokenizer, ShrinkLongestTrimmer, case_fold_utf8, combine_segments, pad_model_inputs
...
class TFCharacterBertTokenizer(keras.layers.Layer):
    """
    This is an in-graph tokenizer for CHARACTER_BERT. It should be initialized similarly to other tokenizers, using the
    ...
    Args:
        ...
        use_fast_character_bert_tokenizer (`bool`, *optional*, defaults to `True`):
            If True, will use the FastCharacterBertTokenizer class from Tensorflow Text. If False, will use the CharacterBertTokenizer
            class instead. CharacterBertTokenizer supports some additional options, but is slower and cannot be exported to
            TFLite.
    """
    ...
    def __init__(
        ...
        use_fast_character_bert_tokenizer: bool = True,
        **tokenizer_kwargs,
    ):
        super().__init__()
        if use_fast_character_bert_tokenizer:
            self.tf_tokenizer = FastCharacterBertTokenizer(
            ...
            self.tf_tokenizer = CharacterBertTokenizerLayer(
    ...
    def from_tokenizer(cls, tokenizer: "PreTrainedTokenizerBase", **kwargs):  # noqa: F821
        """
        Initialize a `TFCharacterBertTokenizer` from an existing `Tokenizer`.

        Args:
            tokenizer (`PreTrainedTokenizerBase`):
                The tokenizer to use to initialize the `TFCharacterBertTokenizer`.

        Examples:

        ```python
        from transformers import AutoTokenizer, TFCharacterBertTokenizer

        tokenizer = AutoTokenizer.from_pretrained("helboukkouri/character-bert-base-uncased")
        tf_tokenizer = TFCharacterBertTokenizer.from_tokenizer(tokenizer)
        ```
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], *init_inputs, **kwargs):
        """
        Instantiate a `TFCharacterBertTokenizer` from a pre-trained tokenizer.
        ...
        ```python
        from transformers import TFCharacterBertTokenizer

        tf_tokenizer = TFCharacterBertTokenizer.from_pretrained("helboukkouri/character-bert-base-uncased")
        ```
        """
        try:
            tokenizer = CharacterBertTokenizer.from_pretrained(pretrained_model_name_or_path, *init_inputs, **kwargs)
        except:  # noqa: E722
            from .tokenization_character_bert_fast import CharacterBertTokenizerFast

            tokenizer = CharacterBertTokenizerFast.from_pretrained(pretrained_model_name_or_path, *init_inputs, **kwargs)
...
__all__ = ["TFCharacterBertTokenizer"]
