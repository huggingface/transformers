...
# Copied from transformers.models.llama.modeling_llama.rotate_half with Llama->Mistral
def rotate_half(x):
    """Rotates half the hidden dims of the input."""
    logger.warning_once(
        "Using the global `rotate_half` function is deprecated. Please use `MistralRotaryEmbedding.rotate_half` instead. "
...
# Copied from transformers.models.llama.modeling_llama.apply_rotary_pos_emb with Llama->Mistral
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    logger.warning_once(
        "Using the global `apply_rotary_pos_emb` function is deprecated. Please use `MistralRotaryEmbedding.apply_rotary_pos_emb` instead. "
        "This is deprecated to improve the export to ONNX by applying the rotary embeddings during the forward call in "
        "the rotary embedding class. "
    )
    cos = cos[position_ids].unsqueeze(1)  # [seq_len, dim] -> [batch_size, 1, seq_len, head_dim]
