...
class StyleTextToSpeech2Tokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            fallback = espeak.EspeakFallback(british=language_code=='b')
        except Exception as e:
            logger.warning("EspeakFallback not Enabled: OOD words will be skipped")
            logger.warning({str(e)})
            fallback = None

        self.g2p = en.G2P(trf=transformer_based_g2p, british=language_code=='b', fallback=fallback, unk='')
    ...
    def _tokenize(self, text):
        ...
        tokens = list(''.join(t.phonemes + (' ' if t.whitespace else '') for t in m_tokens).strip())
