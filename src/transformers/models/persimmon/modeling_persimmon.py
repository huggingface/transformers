...
class PersimmonRotaryEmbedding(nn.Module):
    ...
    def sin_cached(self):
        logger.warning_once(
            ...
            "the forward method of RoPE from now on instead. It is not used in the `PersimmonAttention` class"
        )
...
class PersimmonSdpaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class PersimmonFlashAttention2(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class PersimmonModel(PersimmonPreTrainedModel):
    ...
    def __init__(self, config: PersimmonConfig):
        ...
        self.norm = PersimmonRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class PersimmonForCausalLM(PersimmonPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = PersimmonForCausalLM.from_pretrained("meta-llama/Persimmon-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-llama/Persimmon-2-7b-hf")
