...
# Copied from transformers.models.falcon.modeling_falcon.FalconYaRNScalingRotaryEmbedding with Falcon->Persimmon
class PersimmonYaRNScalingRotaryEmbedding(PersimmonRotaryEmbedding):
    def __init__(
        self,
        dim,
...
# Copied from transformers.models.falcon.modeling_falcon.FalconDynamicYaRNScalingRotaryEmbedding with Falcon->Persimmon
class PersimmonDynamicYaRNScalingRotaryEmbedding(PersimmonYaRNScalingRotaryEmbedding):
    def __init__(
        self,
        dim,
        max_position_embeddings=2048,
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                self.rotary_emb = PersimmonYaRNScalingRotaryEmbedding(
                    int(self.partial_rotary_factor * self.head_dim),
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
                ...
                self.rotary_emb = PersimmonDynamicYaRNScalingRotaryEmbedding(
                    int(self.partial_rotary_factor * self.head_dim),
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
