...
from transformers import BeitFeatureExtractor, Data2VecVisionConfig, Data2VecVisionForImageClassification
...
def get_args():
    ...
    parser.add_argument(
        "--attn_drop_rate", type=float, default=0.0, metavar="PCT", help="Attention dropout rate (default: 0.)"
    )
    ...
    parser.add_argument(
        ...
        weight decay. We use a cosine schedule for WD and using a larger decay by the end of training improves
    ...
    parser.add_argument(
        "--learn_layer_weights", action="store_true", dest="learn_layer_weights"
    )  # supersede `target_layer`
    ...
    parser.add_argument("--linear_classifier", action="store_true", help="linear classifier")
...
def load_model(args):
    ...
    if args.linear_classifier:
        frozen_params = set(n for n, _ in model.named_parameters()) & set(checkpoint_model.keys())
