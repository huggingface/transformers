...
"""Convert Data2VecVision checkpoints from the unilm repository."""
...
@torch.no_grad()
def convert_data2vec_vision_checkpoint(checkpoint_url, pytorch_dump_folder_path):
    """
    Copy/paste/tweak model's weights to our Data2VecVision structure.
    """

    # define default Data2VecVision configuration
...
#        raise ValueError("Should either find 'base' or 'large' in checkpoint URL")
    ...
    if checkpoint_url[-9:-4] == "pt22k":
        model = Data2VecVisionForMaskedImageModeling(config)
    elif "ade20k" in checkpoint_url:
    ...
    if checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_224_pt22k"):
        expected_shape = torch.Size([1, 196, 8192])
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_224_pt22k"):
        expected_shape = torch.Size([1, 196, 8192])
    elif checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_224_pt22k_ft22k"):
        expected_shape = torch.Size([1, 21841])
        expected_logits = torch.tensor([2.2288, 2.4671, 0.7395])
        expected_class_idx = 2397
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_224_pt22k_ft22k"):
        expected_shape = torch.Size([1, 21841])
        expected_logits = torch.tensor([1.6881, -0.2787, 0.5901])
        expected_class_idx = 2396
    elif checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_224_pt22k_ft1k"):
        expected_logits = torch.tensor([0.1241, 0.0798, -0.6569])
        expected_class_idx = 285
    elif checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_224_pt22k_ft22kto1k"):
        expected_logits = torch.tensor([-1.2385, -1.0987, -1.0108])
        expected_class_idx = 281
    elif checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_384_pt22k_ft22kto1k"):
        expected_logits = torch.tensor([-1.5303, -0.9484, -0.3147])
        expected_class_idx = 761
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_224_pt22k_ft1k"):
        expected_logits = torch.tensor([0.4610, -0.0928, 0.2086])
        expected_class_idx = 761
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_224_pt22k_ft22kto1k"):
        expected_logits = torch.tensor([-0.4804, 0.6257, -0.1837])
        expected_class_idx = 761
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_384_pt22k_ft22kto1k"):
        expected_logits = torch.tensor([[-0.5122, 0.5117, -0.2113]])
        expected_class_idx = 761
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_512_pt22k_ft22kto1k"):
        expected_logits = torch.tensor([-0.3062, 0.7261, 0.4852])
        expected_class_idx = 761
    elif checkpoint_url[:-4].endswith("data2vec_vision_base_patch16_640_pt22k_ft22ktoade20k"):
    ...
    elif checkpoint_url[:-4].endswith("data2vec_vision_large_patch16_640_pt22k_ft22ktoade20k"):
        expected_shape = (1, 150, 160, 160)
        expected_logits = torch.tensor(
            [
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_url",
        default="https://unilm.blob.core.windows.net/data2vec_vision/data2vec_vision_base_patch16_224_pt22k_ft22kto1k.pth",
        type=str,
    ...
    convert_data2vec_vision_checkpoint(args.checkpoint_url, args.pytorch_dump_folder_path)
