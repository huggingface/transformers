...
class Pop2PianoTokenizer(PreTrainedTokenizer):
    ...
    def __call__(
        ...
        if (
            ...
            or input_features["beatsteps"].shape[0] != input_features["ext_beatstep"].shape[0]
        ):
            raise ValueError(
                "length mistamtch between relative_tokens, beatsteps and extrapolated_beatstep. All lengths must be same! Found"
                f"relative_tokens length - {len(relative_tokens)}, beatsteps shape - {input_features['beatsteps'].shape[0]}, extrapolated_beatsteps shape - {input_features['ext_beatstep'].shape[0]}"
            )
        ...
        if (
            ...
            and (not hasattr(input_features, "attention_mask_ext_beatstep"))
        ):
        ...
        for index in range(len(relative_tokens)):
            ...
            ext_beatstep = input_features["ext_beatstep"][index]
            ...
            if hasattr(input_features, "attention_mask_beatsteps") and hasattr(
                input_features, "attention_mask_ext_beatstep"
            ):
                attention_mask_beatsteps = input_features["attention_mask_beatsteps"][index]
                attention_mask_ext_beatstep = input_features["attention_mask_ext_beatstep"][index]

                beatsteps = beatsteps[: torch.max(torch.where(attention_mask_beatsteps == 1)[0]) + 1]
                ext_beatstep = ext_beatstep[: torch.max(torch.where(attention_mask_ext_beatstep == 1)[0]) + 1]

            relative_token = to_numpy(relative_token)
            beatsteps = to_numpy(beatsteps)
            ext_beatstep = to_numpy(ext_beatstep)
        ...
        #
        # relative_tokens = to_numpy(relative_tokens)
        # beatsteps = to_numpy(beatsteps)
        # ext_beatstep = to_numpy(ext_beatstep)
        #
        # pm, notes = self.relative_batch_tokens_to_midi(
