...
class HunYuanVLImageProcessorPil(PilBackend):
    ...
    def from_dict(cls, image_processor_dict, **kwargs):
        # ``backend`` is a read-only ``@property`` on ``PilBackend``; the parent
        # ``from_dict`` blindly ``setattr``s any kwarg that exists on the instance and
        # isn't in ``valid_kwargs``. This would raise ``AttributeError: property
        # 'backend' of '...' object has no setter`` when callers (e.g. vLLM) pass
        # ``--mm-processor-kwargs '{"backend":"..."}'``. Drop these kwargs here; the
        # backend selection has already been resolved by ``AutoImageProcessor`` before
        # this point. Also drop ``device`` because the PIL backend is CPU-only.
        for k in ("backend", "use_fast", "device"):
            kwargs.pop(k, None)
            image_processor_dict.pop(k, None)
    ...
    def preprocess(
        ...
        **kwargs: Unpack[HunYuanVLImageProcessorKwargs],
        ...
        # PIL backend doesn't support GPU device, silently drop the kwarg if user passes it.
    ...
    def get_number_of_image_patches(self, height: int, width: int, images_kwargs=None):
        """
        A utility that returns number of image patches (grid_h, grid_w) for a given image size.

        Note: returns a tuple `(grid_h, grid_w)` to be consistent with the original
        ``HunYuanVL`` implementation, which is consumed by the processor for token
        accounting.
        """
