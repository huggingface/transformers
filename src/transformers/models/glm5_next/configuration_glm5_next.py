...
class Glm5NextVisionConfig(PreTrainedConfig):
    ...
    out_hidden_size (`int`, *optional*, defaults to 4096):
        The output hidden size of the vision model. Matches the text model's
        `hidden_size` so vision features project straight into the LLM.
    projection_intermediate_size (`int`, *optional*, defaults to 10240):
        Intermediate size of the vision patch merger's SwiGLU projection. Unlike
        GLM-OCR (which derives this as `out_hidden_size * in_channels`), GLM-5-Next
        stores it explicitly so the multimodal projection head can be sized
        independently of the vision tower's output width.
    ...
    >>> # Initializing a GLM-5-Next vision (GLM-OCR tower) configuration
    >>> configuration = Glm5NextVisionConfig()
    ```"""
