...
@auto_docstring
@dataclass
class NeuCodecOutput(ModelOutput):
    r"""
...
class NeuCodecAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: NeuCodecConfig, layer_idx: int):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # NeuCodec uses position_ids of shape (1, num_attention_heads) so cos/sin have shape
...
class NeuCodecRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        NeuCodecRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class NeuCodecDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: NeuCodecConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = NeuCodecAttention(config=config, layer_idx=layer_idx)

        self.mlp = NeuCodecMLP(config)
        self.input_layernorm = NeuCodecRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = NeuCodecRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class NeuCodecSnakeBeta(nn.Module):
    """
...
class NeuCodecDownSample1d(nn.Module):
...
class NeuCodecUpSample1d(nn.Module):
    def __init__(self, ratio=2, kernel_size=None):
        super().__init__()
        self.ratio = ratio
...
class NeuCodecFiniteScalarQuantization(nn.Module):
    """
    ...
    Original modeling uses `ResidualFSQ` with a single quantizer: https://huggingface.co/HKUSTAudio/neucodec/blob/main/vq/codec_decoder_vocos.py#L389
    But we can directly use FSQ since a main feature of NeuCodec is that it uses a single codebook.
    """
...
class NeuCodecISTFTHead(nn.Module):
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Clamp as expected by original: https://huggingface.co/HKUSTAudio/neucodec/blob/main/vq/codec_decoder_vocos.py#L82
...
@auto_docstring
class NeuCodecPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": NeuCodecDecoderLayer,
        "attentions": NeuCodecDecoderLayer,
    }

    def _init_weights(self, module):
        super()._init_weights(module)
        if isinstance(module, NeuCodecSnakeBeta):
            init.zeros_(module.alpha)
            init.zeros_(module.beta)
        elif isinstance(module, NeuCodecISTFTHead):
            window = torch.hann_window(module.n_fft)
            init.copy_(module.window, window)
        elif isinstance(module, NeuCodecFiniteScalarQuantization):
        ...
        elif isinstance(module, NeuCodecUpSample1d):
        ...
        elif isinstance(module, NeuCodecDownSample1d):
...
class NeuCodecAntiAliasedActivation1d(nn.Module):
    def __init__(
        self,
        activation,
        up_ratio: int = 2,
        ...
        self.act = activation
        self.upsample = NeuCodecUpSample1d(up_ratio, up_kernel_size)
        self.downsample = NeuCodecDownSample1d(down_ratio, down_kernel_size)
...
class NeuCodecResidualUnit(nn.Module):
    """
    ...
    def __init__(self, dimension, dilation):
        super().__init__()
        pad = ((7 - 1) * dilation) // 2
        self.snake1 = NeuCodecAntiAliasedActivation1d(activation=NeuCodecSnakeBeta(dimension))
        self.conv1 = nn.Conv1d(dimension, dimension, kernel_size=7, dilation=dilation, padding=pad)
        self.snake2 = NeuCodecAntiAliasedActivation1d(activation=NeuCodecSnakeBeta(dimension))
...
class NeuCodecEncoderBlock(nn.Module):
    """Encoder block used in NEUCODEC encoder."""

    def __init__(self, config: NeuCodecConfig, stride: int = 1, stride_index: int = 1):
        super().__init__()
        dimension = config.encoder_hidden_size * 2**stride_index
        self.res_unit1 = NeuCodecResidualUnit(dimension // 2, dilation=1)
        self.res_unit2 = NeuCodecResidualUnit(dimension // 2, dilation=3)
        self.res_unit3 = NeuCodecResidualUnit(dimension // 2, dilation=9)
        self.snake1 = NeuCodecAntiAliasedActivation1d(activation=NeuCodecSnakeBeta(dimension // 2))
...
class NeuCodecEncoder(nn.Module):
    """NEUCODEC Encoder"""
    ...
    def __init__(self, config: NeuCodecConfig):
        ...
        for stride_index, stride in enumerate(config.downsampling_ratios):
            ...
            self.block += [NeuCodecEncoderBlock(config, stride=stride, stride_index=stride_index)]

        self.block = nn.ModuleList(self.block)
        d_model = config.encoder_hidden_size * 2 ** len(config.downsampling_ratios)
        self.snake1 = NeuCodecAntiAliasedActivation1d(activation=NeuCodecSnakeBeta(d_model))
...
class NeuCodecResNetBlock(nn.Module):
    def __init__(self, config: NeuCodecConfig):
        super().__init__()
...
class NeuCodecQuantizer(nn.Module):
    def __init__(self, config: NeuCodecConfig):
        super().__init__()
        self.quantizer = NeuCodecFiniteScalarQuantization(config)
...
class NeuCodecDecoder(nn.Module):
    ...
    def __init__(self, config: NeuCodecConfig):
        ...
        self.prior_net = nn.ModuleList([NeuCodecResNetBlock(config), NeuCodecResNetBlock(config)])
        self.num_attention_heads = config.num_attention_heads
        self.rotary_emb = NeuCodecRotaryEmbedding(config=config)
        self.layers = nn.ModuleList(
            [NeuCodecDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_net = nn.ModuleList([NeuCodecResNetBlock(config), NeuCodecResNetBlock(config)])
        self.norm = nn.LayerNorm(config.hidden_size, eps=1e-6)
        self.head = NeuCodecISTFTHead(config)
...
class NeuCodecSemanticAdapter(nn.Module):
    def __init__(self, config: NeuCodecConfig):
        super().__init__()
        self.conv1 = nn.Conv1d(
...
class NeuCodecModel(NeuCodecPreTrainedModel):
    ...
    def __init__(self, config: NeuCodecConfig):
        ...
        self.semantic_adapter = NeuCodecSemanticAdapter(config)
        self.acoustic_encoder = NeuCodecEncoder(config)
...
__all__ = ["NeuCodecModel", "NeuCodecPreTrainedModel"]
