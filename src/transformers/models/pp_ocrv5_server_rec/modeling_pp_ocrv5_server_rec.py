...
class PPOCRV5ServerRecModel(PPOCRV5ServerRecPreTrainedModel):
    ...
    def forward(self, pixel_values, **kwargs):
        ...
        x = backbone_output.feature_maps[-1]
        if self.config.text_rec:
            if self.training:
                x = F.adaptive_avg_pool2d(x, (1, 40))
            else:
                target_w = x.shape[-1] * 4
                x = F.interpolate(x, size=(x.shape[-2], target_w), mode="bilinear", align_corners=False)
                x = F.adaptive_avg_pool2d(x, (1, target_w))
