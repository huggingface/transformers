...
class Data2VecTextAttention(nn.Module):
    ...
    DATA2VEC_TEXT historically split this over `Data2VecTextSelfAttention` (q/k/v) and `Data2VecTextSelfOutput` (the output
    projection plus the residual LayerNorm). The projection lives here as `o_proj`; the LayerNorm
    and residual belong to `Data2VecTextLayer`, which is what makes the layer's topology visible.
    """
...
class Data2VecTextCrossAttention(nn.Module):
    """Cross-attention query/key/value. Its output projection is `Data2VecTextSelfOutput`, see above."""
...
class Data2VecTextSelfOutput(nn.Module):
    ...
    The self-attention path no longer uses this: its projection is `Data2VecTextAttention.o_proj` and its
    LayerNorm belongs to `Data2VecTextLayer`. Cross-attention keeps the original layout deliberately, so
...
class Data2VecTextCrossAttentionBlock(nn.Module):
    """Cross-attention plus its output projection, in DATA2VEC_TEXT's original module layout."""

    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.self = Data2VecTextCrossAttention(config, is_causal=False, layer_idx=layer_idx)
...
class Data2VecTextMLP(nn.Module):
    """Ungated feed-forward network.

    Replaces `Data2VecTextIntermediate` (up projection plus activation) and `Data2VecTextOutput` (down projection
    plus the residual LayerNorm); the LayerNorm and residual now belong to `Data2VecTextLayer`.
    """
...
class Data2VecTextLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.self_attn = Data2VecTextAttention(config, is_causal=config.is_decoder, layer_idx=layer_idx)
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = Data2VecTextCrossAttentionBlock(config, layer_idx=layer_idx)
        self.mlp = Data2VecTextMLP(config)
