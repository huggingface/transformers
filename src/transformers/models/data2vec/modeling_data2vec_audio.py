...
class Data2VecAudioForSequenceClassification(Data2VecAudioPreTrainedModel):
    ...
    def forward(
        ...
        input_values (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
            ...
            into an array of type `list[float]`, a `numpy.ndarray` or a `torch.Tensor`, *e.g.* via the torchcodec
            library (`pip install torchcodec`) or the soundfile library (`pip install soundfile`).
...
class Data2VecAudioForAudioFrameClassification(Data2VecAudioPreTrainedModel):
    ...
    def forward(
        ...
        input_values (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
            Float values of input raw speech waveform. Values can be obtained by loading a `.flac` or `.wav` audio file
            into an array of type `list[float]`a `numpy.ndarray`  or a `torch.Tensor`, *e.g.* via the torchcodec library (`pip install
