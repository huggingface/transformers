...
class AudioVisualFlamingoForConditionalGeneration(AudioVisualFlamingoPretrainedModel, GenerationMixin):
    ...
    def encode_video(self, inp, block_sizes: tuple[int, ...] | None = None, mm_info: dict | None = None, num_frames: list[int] | None = None):
        ...
        if len(images) > 0:
            ...
            image_features = [self.split_chessboard(x, block_size[0], block_size[1]) for x, block_size in zip(image_features, new_block_sizes)]
        ...
        image_features = list(image_features.split([block_size[0] * block_size[1] for block_size in new_block_sizes], dim=0))
        image_features = [self.merge_chessboard(x, block_size[0], block_size[1]) for x, block_size in zip(image_features, new_block_sizes)]
