...
# Copied from transformers.models.big_bird.modeling_big_bird.BigBirdSelfAttention with BigBirdSelf->BigBirdPegasus
...
# Copied from transformers.models.big_bird.modeling_big_bird.BigBirdBlockSparseAttention with BigBirdSelf->BigBirdPegasus
...
# Copied from transformers.models.big_bird.modeling_big_bird.BigBirdSelfOutput with BigBird->BigBirdPegasus
class BigBirdPegasusSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.big_bird.modeling_big_bird.BigBirdAttention with BigBird->BigBirdPegasus
class BigBirdPegasusAttention(nn.Module):
    def __init__(self, config, seed=None):
        super().__init__()
        ...
        self.output = BigBirdPegasusSelfOutput(config)
...
class BigBirdPegasusEncoderLayer(nn.Module):
    def __init__(self, config: BigBirdPegasusConfig):
        ...
        self.self_attn = BigBirdPegasusAttention(
...
class BigBirdPegasusDecoderLayer(nn.Module):
    def __init__(self, config: BigBirdPegasusConfig):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = BigBirdPegasusAttention(
            self.embed_dim,
...
BIGBIRD_PEGASUS_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_attention_mask (:obj:`torch.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
            ...
            If you want to change padding behavior, you should read :func:`modeling_bigbird_pegasus._prepare_decoder_inputs` and
...
class BigBirdPegasusEncoder(BigBirdPegasusPreTrainedModel):
    ...
    def __init__(self, config: BigBirdPegasusConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BigBirdPegasusEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class BigBirdPegasusDecoder(BigBirdPegasusPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`BigBirdPegasusDecoderLayer`
...
@add_start_docstrings(
    """
    BigBirdPegasus model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    BIGBIRD_PEGASUS_START_DOCSTRING,
)
...
@add_start_docstrings(
    """
    BigBirdPegasus Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
