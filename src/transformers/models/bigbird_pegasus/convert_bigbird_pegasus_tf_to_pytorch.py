...
def convert_bigbird_pegasus(tf_weights: dict, config_update: dict) -> BigBirdPegasusForConditionalGeneration:
    ...
    for k, v in tqdm(decoder_weights.items(), "tf -> hf conversion"):
        ...
        if v.shape != state_dict[new_k].shape:
            raise ValueError(f"{new_k}, {k}, {v.shape}, {state_dict[new_k].shape}")
    ...
    if unexpected_missing != []:
        raise ValueError(f"no matches found for the following torch keys {unexpected_missing}")
    if extra != []:
        raise ValueError(f"no matches found for the following tf keys {extra}")
    return torch_model
