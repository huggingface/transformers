...
def write_model(model_path, input_base_path, config, tokenizer_path=None, safe_serialization=True):
    ...
    if config.get("n_kv_heads", None) is not None:
    ...
    model = GoldenGateForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float32, low_cpu_mem_usage=True)
