...
class StableLmRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        StableLmRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class StableLmSdpaAttention(LlamaAttention):
    def __init__(self, config: LlamaConfig, layer_idx: int | None = None):
        super().__init__(config, layer_idx) # here call to super means
                                            # we should copy super
        self.qk_layernorm = config.qk_layernorm
...
class StableLmFlashAttention2(LlamaAttention):
    """
...
class StableLmPreTrainedModel(PreTrainedModel):
    config_class = StableConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["StableDecoderLayer"]
...
class StableLmdModel(StablePreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`StableDecoderLayer`]
    ...
    def __init__(self, config: StableConfig):
        ...
        self.layers = nn.ModuleList(
            [StableDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = StableRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class StableLmForCausalLM(StablePreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = StableModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        >>> model = StableLmForCausalLM.from_pretrained("meta-llama/Stable-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-llama/Stable-2-7b-hf")
...
@add_start_docstrings(
    ...
    [`StableLmForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class StableLmForSequenceClassification(StablePreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
