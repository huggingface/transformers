...
def megatron_to_hf_config(megatron_cfg):
    ...
    hf_model_config["use_bias"] = all([megatron_cfg.encoder.bias_activation_fusion, megatron_cfg.encoder.bias_dropout_add_fusion, megatron_cfg.encoder.bias])
...
def convert_megatron_checkpoint(args, input_state_dict, config):
    ...
    print(f"attention_dim:{attention_dim}, config.d_kv:{config.d_kv}, config.num_heads:{config.num_heads}, hidden_size_per_head:{hidden_size_per_head}, config.d_model:{config.d_model}")
        ...
        for i in range(0, config.num_layers):       
            ...
            for weight_bias in ['weight','bias']:
                if nn_type == "decoder":
                    ...
                    input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.inter_attention.query.{}".format(nn_type, i, weight_bias)
                    ...
                    input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.inter_attention.key_value.{}".format(nn_type, i, weight_bias)
                    attention_qkv_weight = input_state_dict[input_key]
                    ...
                    input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.inter_attention.dense.{}".format(nn_type, i, weight_bias)
                ...
                input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.self_attention.query_key_value.{}".format(nn_type, i, weight_bias)
                ...
                input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.self_attention.dense.{}".format(nn_type, i, weight_bias)
                ...
                if nn_type == "encoder":
                    input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.mlp.dense_h_to_4h.{}".format(nn_type, i, weight_bias)
                    output_key = "{}.block.{}.layer.1.DenseReluDense.wi_0.{}".format(nn_type, i, weight_bias)
                    output_state_dict[output_key] = input_state_dict[input_key]
                    
                    input_key  = "enc_dec_model.enc_dec_model.{}.model.layers.{}.mlp.dense_h_to_4h_2.{}".format(nn_type, i, weight_bias)
                    output_key = "{}.block.{}.layer.1.DenseReluDense.wi_1.{}".format(nn_type, i, weight_bias)
                    output_state_dict[output_key] = input_state_dict[input_key]

                    input_key  = "enc_dec_model.enc_dec_model.{}.model.layers.{}.mlp.dense_4h_to_h.{}".format(nn_type, i, weight_bias)
                ...
                # convert ln 
                input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.input_layernorm.{}".format(nn_type, i, weight_bias)
                output_key = "{}.block.{}.layer.0.layer_norm.{}".format(nn_type, i, weight_bias)
                output_state_dict[output_key] = input_state_dict[input_key]

                if nn_type == "decoder":

                    input_key = "enc_dec_model.enc_dec_model.{}.model.layers.{}.post_attention_layernorm.{}".format(nn_type, i, weight_bias)
                    output_key = "{}.block.{}.layer.1.layer_norm.{}".format(nn_type, i, weight_bias)
                    output_state_dict[output_key] = input_state_dict[input_key]

                    input_key = "enc_dec_model.enc_dec_model.decoder.model.layers.{}.post_inter_attention_layernorm.{}".format(i, weight_bias)
