...
PARALLELIZE_DOCSTRING = r"""
    ...
    model = MegatronT5ForConditionalGeneration.from_pretrained("t5-3b")
    device_map = {
        0: [0, 1, 2],
        1: [3, 4, 5, 6, 7, 8, 9],
...
def get_layernorm_layer(normalization):
    try:
        if normalization == "rmsnorm":
            ...
            if len(ALL_LAYERNORM_LAYERS) == ALL_LAYERNORM_LAYERS_len:
                ALL_LAYERNORM_LAYERS.append(_T5LayerNorm)
            return _T5LayerNorm
...
class T5DenseActDense(nn.Module):
    def __init__(self, config: MegatronT5Config):
        super().__init__()
        self.wi = nn.Linear(config.d_model, config.d_ff, bias=config.use_bias)
        self.wo = nn.Linear(config.d_ff, config.d_model, bias=config.use_bias)
...
class T5DenseGatedActDense(nn.Module):
    def __init__(self, config: MegatronT5Config):
        super().__init__()
        self.wi_0 = nn.Linear(config.d_model, config.d_ff, bias=config.use_bias)
        self.wi_1 = nn.Linear(config.d_model, config.d_ff, bias=config.use_bias)
        self.wo = nn.Linear(config.d_ff, config.d_model, bias=config.use_bias)
        self.dropout = nn.Dropout(config.dropout_rate)
        self.act = ACT2FN[config.dense_act_fn]
...
class T5Attention(nn.Module):
    def __init__(self, config: MegatronT5Config, layer_number, has_relative_attention_bias=False):
        super().__init__()
    ...
    def forward(
        ...
        if key_value_states is None:
            # self-attn
            mixed_x_layer = torch.cat([self.q(hidden_states), self.k(hidden_states), self.v(hidden_states)], dim=-1)
            ...
            # cross-attn
            mixed_kv_layer = torch.cat([self.k(key_value_states), self.v(key_value_states)], dim=-1)
            # [b, sq, (np * 2 * hn)] --> [b, sq, np, 2 * hn]
        ...
        if past_key_value is not None and past_key_value[0] is not None:
            if key_value_states is None:
                ...
                if key_states is None:
                    ...
                    _hidden_states = key_states
                key_states = torch.cat([past_key_value[0], _hidden_states], dim=2)
            else:
                # cross-attn
        ...
        if past_key_value is not None and past_key_value[1] is not None:
            if key_value_states is None:
                ...
                if value_states is None:
                    ...
                    _hidden_states = value_states
                value_states = torch.cat([past_key_value[1], _hidden_states], dim=2)
            else:
                # cross-attn
        ...
        coeff = None
        self.norm_factor = math.sqrt(self.key_value_proj_dim)
        ...
        # scaled softmax in ParallelAttention (Megatron)
        # https://github.com/NVIDIA/apex/blob/190a44692e1e6bc66a15f9457d7ee2c671d2849f/apex/transformer/functional/fused_softmax.py#L257
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, layer_number):
        super().__init__()
        self.layer_number = max(1, layer_number)
        self.EncDecAttention = T5Attention(config, layer_number=self.layer_number, has_relative_attention_bias=False)
        T5LayerNorm = get_layernorm_layer(config.normalization)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Stack(MegatronT5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, position_embeddings=None):
    ...
    def build_position_ids(self, token_ids):
        # Create position ids
...
T5_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`MegatronT5Config`]): Model configuration class with all the parameters of the model.
...
class MegatronT5Model(MegatronT5PreTrainedModel):
    ...
    def __init__(self, config: MegatronT5Config):
        ...
        self.encoder = T5Stack(encoder_config, self.shared, self.position_embeddings)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared, self.position_embeddings)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import T5Tokenizer, MegatronT5Model

        >>> tokenizer = T5Tokenizer.from_pretrained("t5-small")
        >>> model = MegatronT5Model.from_pretrained("t5-small")
...
class MegatronT5ForConditionalGeneration(MegatronT5PreTrainedModel):
    ...
    def __init__(self, config: MegatronT5Config):
        ...
        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=config.use_bias)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import T5Tokenizer, MegatronT5ForConditionalGeneration

        >>> tokenizer = T5Tokenizer.from_pretrained("t5-small")
        >>> model = MegatronT5ForConditionalGeneration.from_pretrained("t5-small")
        ...
        if self.tie_word_embeddings and self.use_rescale_tie_word_embeddings:
            # Rescale output before projecting on vocab
            # Megatron does not rescale output
...
class MegatronT5EncoderModel(MegatronT5PreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import T5Tokenizer, MegatronT5EncoderModel

        >>> tokenizer = T5Tokenizer.from_pretrained("t5-small")
        >>> model = MegatronT5EncoderModel.from_pretrained("t5-small")
        >>> input_ids = tokenizer(
        ...     "Studies have been shown that owning a dog is good for you", return_tensors="pt"
        ... ).input_ids  # Batch size 1
