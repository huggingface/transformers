...
class FlaxWhisperEncoderLayerCollection(nn.Module):
    ...
    def setup(self):
        if self.gradient_checkpointing:
            FlaxWhisperEncoderCheckpointLayer = remat(
                FlaxWhisperEncoderLayer, static_argnums=(2, 3)
            )
            self.layers = [
...
class FlaxWhisperDecoderLayerCollection(nn.Module):
    ...
    def setup(self):
        if self.gradient_checkpointing:
            FlaxWhisperDecoderCheckpointLayer = remat(
                FlaxWhisperDecoderLayer, static_argnums=(4, 5, 6)
            )
            self.layers = [
...
class FlaxWhisperDecoder(nn.Module):
    ...
    def setup(self) -> None:
        self.embed_tokens = nn.Embed(
            self.config.vocab_size, self.config.d_model, dtype=self.dtype
        )
        self.embed_positions = nn.Embed(
...
append_call_sample_docstring(
    FlaxWhisperModel, _CHECKPOINT_FOR_DOC, FlaxSeq2SeqModelOutput, _CONFIG_FOR_DOC
)
...
overwrite_call_docstring(
    ...
    WHISPER_INPUTS_DOCSTRING + FLAX_WHISPER_AUDIO_CLASSIFICATION_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxWhisperForAudioClassification,
