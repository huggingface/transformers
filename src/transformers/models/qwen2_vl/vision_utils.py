...
"""Pure vision utility functions for Qwen2-VL and Qwen2.5-VL.
...
def get_cu_seqlens(grid_thw: torch.Tensor) -> torch.Tensor:
    """Compute cumulative sequence lengths from vision grid info.

    Works for all Qwen VL models. Uses ``repeat_interleave`` which produces
    a data-dependent output size — not traceable by ``torch.export``.
...
def get_window_index(
    ...
    """Compute window attention indices for Qwen2.5-VL vision encoder.
