...
class OmDetTurboEncoder(OmDetTurboPreTrainedModel):
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            self.lateral_convs.append(OmDetBaseConv(self.hidden_dim, self.hidden_dim, 1, 1, act=activation))
        ...
        for idx in range(len(self.in_channels) - 1):
            self.downsample_convs.append(OmDetBaseConv(self.hidden_dim, self.hidden_dim, 3, stride=2, act=activation))
...
class OmDetTurboResidualMLP(nn.Module):
    def __init__(self, d_m, dropout, d_hidden=1024, activation="relu"):
        super().__init__()
        self.mlp = OmDetTurboMLP(d_m, d_m, d_hidden, dropout, activation)
...
class OmDetTurboDecoder(OmDetTurboPreTrainedModel):
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        self.num_head = config.decoder_num_head
        self.num_levels = len(config.channels)  # num level
        self.nc = 80
        self.num_queries = config.num_queries
        ...
        if self.fuse_type is not None:
            self.task_encoder = OmDetTurboMLP(self.label_dim, dropout=dropout)
...
_CHECKPOINT_FOR_DOC = "omlab/omdet-turbo-tiny"


@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderOutput with GroundingDino->OmDetTurbo
class OmDetTurboDecoderOutput(ModelOutput):
    """
    Base class for outputs of the OmDetTurboDecoder. This class adds two attributes to
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderOutput with GroundingDino->OmDetTurbo
class OmDetTurboEncoderOutput(ModelOutput):
    """
    Base class for outputs of the OmDetTurboEncoder. This class extends BaseModelOutput, due to:
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoModelOutput with GroundingDino->OmDetTurbo,Grounding DINO->OmDet-Turbo
class OmDetTurboModelOutput(ModelOutput):
    """
    Base class for outputs of the OmDet-Turbo encoder-decoder model.
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoObjectDetectionOutput with GroundingDino->OmDetTurbo
class OmDetTurboObjectDetectionOutput(ModelOutput):
    """
    Output type of [`OmDetTurboForObjectDetection`].
    ...
    Args:
        ...
        pred_boxes (`torch.FloatTensor` of shape `(batch_size, num_queries, 4)`):
            ...
            possible padding). You can use [`~OmDetTurboProcessor.post_process_object_detection`] to retrieve the
...
# Copied from transformers.models.detr.modeling_detr.DetrFrozenBatchNorm2d with Detr->OmDetTurbo
class OmDetTurboFrozenBatchNorm2d(nn.Module):
    """
...
# Copied from transformers.models.detr.modeling_detr.replace_batch_norm with Detr->OmDetTurbo
def replace_batch_norm(model):
    r"""
    Recursively replace all `torch.nn.BatchNorm2d` with `OmDetTurboFrozenBatchNorm2d`.
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            new_module = OmDetTurboFrozenBatchNorm2d(module.num_features)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoConvEncoder with GroundingDino->OmDetTurbo
class OmDetTurboConvEncoder(nn.Module):
    """
    Convolutional backbone, using either the AutoBackbone API or one from the timm library.

    nn.BatchNorm2d layers are replaced by OmDetTurboFrozenBatchNorm2d as defined above.
...
# Copied from transformers.models.detr.modeling_detr.DetrConvModel with Detr->OmDetTurbo
class OmDetTurboConvModel(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoSinePositionEmbedding with GroundingDino->OmDetTurbo
class OmDetTurboSinePositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoLearnedPositionEmbedding with GroundingDino->OmDetTurbo
class OmDetTurboLearnedPositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.build_position_encoding with GroundingDino->OmDetTurbo
def build_position_encoding(config):
    if config.position_embedding_type == "sine":
        position_embedding = OmDetTurboSinePositionEmbedding(config)
    elif config.position_embedding_type == "learned":
        position_embedding = OmDetTurboLearnedPositionEmbedding(config)
    else:
...
# Not useful for now
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoTextEnhancerLayer with GroundingDino->OmDetTurbo
class OmDetTurboTextEnhancerLayer(nn.Module):
    """Vanilla Transformer with text embeddings as input"""

    def __init__(self, config):
        super().__init__()
        self.self_attn = OmDetTurboMultiheadAttention(config, num_attention_heads=config.encoder_attention_heads // 2)
    ...
    def forward(
        ...
        the text encoder (AutoModel based on text_config) within OmDetTurboEncoderLayer
...
# No tuseful for now
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoBiMultiHeadAttention with GroundingDino->OmDetTurbo
class OmDetTurboBiMultiHeadAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Not useful for now?
# Copied from transformers.models.beit.modeling_beit.BeitDropPath with Beit->OmDetTurbo
class OmDetTurboDropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
# Not useful for now?
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoFusionLayer with GroundingDino->OmDetTurbo
class OmDetTurboFusionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.attn = OmDetTurboBiMultiHeadAttention(config)

        # add layer scale for training stability
        self.drop_path = OmDetTurboDropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        init_values = 1e-4
...
# Adapt?
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDeformableLayer with GroundingDino->OmDetTurbo
class OmDetTurboDeformableLayer(nn.Module):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = OmDetTurboMultiscaleDeformableAttention(
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderLayer with GroundingDino->OmDetTurbo
class OmDetTurboEncoderLayer(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.text_enhancer_layer = OmDetTurboTextEnhancerLayer(config)
        self.fusion_layer = OmDetTurboFusionLayer(config)
        self.deformable_layer = OmDetTurboDeformableLayer(config)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderLayer with GroundingDino->OmDetTurbo
class OmDetTurboDecoderLayer(nn.Module):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__()
        self.embed_dim = config.d_model

        # self-attention
        self.self_attn = OmDetTurboMultiheadAttention(config, num_attention_heads=config.decoder_attention_heads)
        ...
        self.encoder_attn_text = OmDetTurboMultiheadAttention(
        ...
        # cross-attention
        self.encoder_attn = OmDetTurboMultiscaleDeformableAttention(
            config,
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoContrastiveEmbedding with GroundingDino->OmDetTurbo
class OmDetTurboContrastiveEmbedding(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoPreTrainedModel with GroundingDino->OmDetTurbo
class OmDetTurboPreTrainedModel(PreTrainedModel):
    config_class = OmDetTurboConfig
    base_model_prefix = "model"
    main_input_name = "pixel_values"
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, OmDetTurboLearnedPositionEmbedding):
            nn.init.uniform_(module.row_embeddings.weight)
            nn.init.uniform_(module.column_embeddings.weight)
        elif isinstance(module, OmDetTurboMultiscaleDeformableAttention):
            module._reset_parameters()
        elif isinstance(module, OmDetTurboBiMultiHeadAttention):
        ...
        elif isinstance(module, (OmDetTurboEncoderLayer, OmDetTurboDecoderLayer)):
        ...
        elif isinstance(module, OmDetTurboMLPPredictionHead):
...
GROUNDING_DINO_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            ...
            Pixel values can be obtained using [`AutoImageProcessor`]. See [`OmDetTurboImageProcessor.__call__`] for
            details.
        ...
        input_ids (`torch.LongTensor` of shape `(batch_size, text_sequence_length)`):
            ...
            Indices can be obtained using [`AutoTokenizer`]. See [`OmDetTurboTokenizer.__call__`] for details.
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoder with GroundingDino->OmDetTurbo
class OmDetTurboEncoder(OmDetTurboPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* deformable attention layers. Each layer is a
    [`OmDetTurboEncoderLayer`].
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoder with GroundingDino->OmDetTurbo,Grounding DINO->OmDet-Turbo
class OmDetTurboDecoder(OmDetTurboPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`OmDetTurboDecoderLayer`].
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        self.layers = nn.ModuleList([OmDetTurboDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.reference_points_head = OmDetTurboMLPPredictionHead(
...
@add_start_docstrings(
    """
    The bare OmDet-Turbo Model (consisting of a backbone and encoder-decoder Transformer) outputting raw
    hidden-states without any specific head on top.
    """,
    GROUNDING_DINO_START_DOCSTRING,
)
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoModel with GroundingDino->OmDetTurbo,IDEA-Research/grounding-dino-tiny->omlab/omdet-turbo-tiny
class OmDetTurboModel(OmDetTurboPreTrainedModel):
    def __init__(self, config: OmDetTurboConfig):
        super().__init__(config)

        # Create backbone + positional encoding
        backbone = OmDetTurboConvEncoder(config)
        position_embeddings = build_position_encoding(config)
        self.backbone = OmDetTurboConvModel(backbone, position_embeddings)
        ...
        if config.two_stage:
            ...
            if (
                ...
                self.encoder_output_bbox_embed = OmDetTurboMLPPredictionHead(
                    input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
                )

            self.encoder_output_class_embed = OmDetTurboContrastiveEmbedding(config)
        else:
    ...
    def forward(
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a OmDetTurboEncoderOutput when return_dict=True
        elif return_dict and not isinstance(encoder_outputs, OmDetTurboEncoderOutput):
            encoder_outputs = OmDetTurboEncoderOutput(
...
class OmDetTurboMLPPredictionHead(nn.Module):
    """
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrHungarianMatcher with DeformableDetr->OmDetTurbo
class OmDetTurboHungarianMatcher(nn.Module):
    """
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrLoss with DeformableDetr->OmDetTurbo
class OmDetTurboLoss(nn.Module):
    """
    This class computes the losses for `OmDetTurboForObjectDetection`. The process happens in two steps: 1) we
    compute hungarian assignment between ground truth boxes and the outputs of the model 2) we supervise each pair of
    matched ground-truth / prediction (supervise class and box).

    Args:
        matcher (`OmDetTurboHungarianMatcher`):
...
@add_start_docstrings(
    """
    OmDet-Turbo Model (consisting of a backbone and encoder-decoder Transformer) with object detection heads on top,
    for tasks such as COCO detection.
    """,
    GROUNDING_DINO_START_DOCSTRING,
)
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoForObjectDetection with GroundingDino->OmDetTurbo,Grounding DINO->OmDet-Turbo,IDEA-Research/grounding-dino-tiny->omlab/omdet-turbo-tiny
class OmDetTurboForObjectDetection(OmDetTurboPreTrainedModel):
    ...
    def __init__(self, config: OmDetTurboConfig):
        ...
        _class_embed = OmDetTurboContrastiveEmbedding(config)

        if config.decoder_bbox_embed_share:
            _bbox_embed = OmDetTurboMLPPredictionHead(
                input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
            )
    ...
    def forward(
        ...
        encoder_outputs: Optional[Union[OmDetTurboEncoderOutput, Tuple]] = None,
        ...
        # First, sent images through OmDet-Turbo base model to obtain encoder + decoder outputs
        outputs = self.model(
        ...
        dict_outputs = OmDetTurboObjectDetectionOutput(
            loss=loss,
            loss_dict=loss_dict,
            logits=logits,
