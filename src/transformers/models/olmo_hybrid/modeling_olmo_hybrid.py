...
def torch_chunk_gated_delta_rule(
    ...
    **kwargs,
    ...
    """Pure-torch gated delta fallback used when FLA kernels are unavailable.

    The previous chunked implementation zero-padded incomplete final chunks to
    ``chunk_size``. Those pad positions updated the recurrent state, so a length-T
    full forward and a length-(T-1) prefill produced different states for the shared
    prefix (different pad counts). Decode then used the recurrent step with that
    inconsistent state, causing large logit drift vs full-sequence forward (#46190).

    Delegate to the step-wise recurrent rule so prefill+decode matches full forward.
