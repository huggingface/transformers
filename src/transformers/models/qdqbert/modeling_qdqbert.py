...
@dataclass
class QDQBertForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~transformers.QDQBertForPreTraining`.
...
@add_start_docstrings(
    """
    QDQ Bert Model with two heads on top as done during the pretraining: a `masked language modeling` head and a `next
    sentence prediction (classification)` head.
    """,
    QDQBERT_START_DOCSTRING,
)
class QDQBertForPreTraining(QDQBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = QDQBertModel(config)
        self.cls = QDQBertPreTrainingHeads(config)
    ...
    @add_start_docstrings_to_model_forward(QDQBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=QDQBertForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        Example::
            ...
            >>> from transformers import QDQBertTokenizer, QDQBertForPreTraining
            >>> import torch

            >>> tokenizer = QDQBertTokenizer.from_pretrained('bert-base-uncased')
            >>> model = QDQBertForPreTraining.from_pretrained('bert-base-uncased')
        ...
        return QDQBertForPreTrainingOutput(
            loss=total_loss,
