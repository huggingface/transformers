...
class TFConvNextV2GRN(tf.keras.layers.Layer):
    """This corresponds to the `GRN` (Global Response Normalization) class in the original implementation.
    ...
    def call(self, hidden_states, training=False):
        input = hidden_states
        x = tf.norm(hidden_states, ord=2, axis=(1, 2), keepdims=True)
        x = x / (tf.math.reduce_mean(x, axis=-1, keepdims=True) + 1e-6)
        x = self.gamma * (input * x) + self.beta + x
        return x
...
CONVNEXTV2_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`np.ndarray`, `tf.Tensor`, `List[tf.Tensor]` ``Dict[str, tf.Tensor]` or `Dict[str, np.ndarray]` and each example must have the shape `(batch_size, num_channels, height, width)`):
            ...
            [`ConvNextV2ImageProcessor.__call__`] for details.
