...
from .configuration_glm_image import GlmImageConfig
...
class GlmImagePreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["GlmImageDecoderLayer"]
...
class GlmImageRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: GlmImageConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: Optional[GlmImageConfig] = None,
...
class GlmImageModel(GlmImagePreTrainedModel):
    def __init__(self, config: GlmImageConfig):
        ...
        self.layers = nn.ModuleList(
            [GlmImageDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
@auto_docstring
class GlmImageForCausalLM(GlmImagePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, GlmImageForCausalLM

        >>> model = GlmImageForCausalLM.from_pretrained("THUDM/GLM-4-9B-0414")
...
__all__ = ["GlmImagePreTrainedModel", "GlmImageModel", "GlmImageForCausalLM"]
