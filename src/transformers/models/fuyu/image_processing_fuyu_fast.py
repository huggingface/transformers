...
class FuyuImageProcessorFast(BaseImageProcessorFast):
    ...
    def _get_tensor_image_size(self, image: "torch.Tensor") -> Tuple[int, int]:
        ...
        if image.ndim == 3:  # C, H, W
            ...
            raise ValueError(
                f"Unsupported tensor dimension: {image.ndim}. Expected 3 or 4."
            )
    ...
    def resize(
        ...
        scaled_image = F.resize(
            image, [new_height, new_width], interpolation=interpolation, antialias=True
        )
        return scaled_image
    ...
    def pad_image(
        ...
        if padding_bottom < 0 or padding_right < 0:
            raise ValueError(
                "Image dimensions after resize are larger than target padding size."
            )
        ...
        if mode != "constant":
            logger.warning_once(
                f"Padding mode '{mode}' not directly supported, using 'constant'."
            )
            mode = "constant"
        ...
        padded_image = F.pad(
            image, padding=padding_ltrb, fill=fill_value, padding_mode=mode
        )  # Use fill_value
        return padded_image
    ...
    def _convert_and_prepare_images(
        ...
        do_convert_rgb: Optional[
            bool
        ] = None,  # Fuyu ignores this (do_convert_rgb=False)
        ...
        for image_list in images:
            ...
            if tensor.ndim == 2:  # H, W -> 1, H, W -> 3, H, W
                ...
                if tensor.shape[0] == 1:
                    ...
                    raise ValueError(
                        f"Input tensor has unexpected channel dimension: {tensor.shape[0]}"
                    )
            elif tensor.ndim == 4:
                ...
                if tensor.shape[1] == 1:
                    ...
                    raise ValueError(
                        f"Input tensor has unexpected channel dimension: {tensor.shape[1]}"
                    )
    ...
    def preprocess(self, images: ImageInput, **kwargs) -> BatchFeature:
        ...
        )  # Uses InterpolationMode if available
    ...
    def _preprocess(
        ...
        images: List[
            List["torch.Tensor"]
        ],  # Input List[List[Tensor]] from custom _convert
        do_resize: bool,
        size: Dict[str, int],
        ...
        **kwargs,
        ...
        if original_sizes is None or len(original_sizes) != len(images):
            ...
            original_sizes = [
                self._get_tensor_image_size(img_list[0])
        ...
        for idx, image_list in enumerate(images):
            ...
            original_height, original_width = (
                ...
                else self._get_tensor_image_size(image)
            )
            if do_resize:
            ...
            scale_factor = (
                (resized_height / original_height) if original_height > 0 else 1.0
            )
            ...
            if do_pad:
                final_image = self.pad_image(
                    resized_image, size=size, mode=padding_mode, value=padding_value
                )
            else:
            ...
            if do_pad and (
                final_shape[-2] != target_height or final_shape[-1] != target_width
            ):
                raise RuntimeError(
        ...
        if not processed_images_list:
            ...
            grouped_images, grouped_images_index_dict = group_images_by_shape(
                processed_images_list
            )
            ...
            if not all(
                isinstance(key, (torch.Size, tuple)) and len(key) == 2
            ...
            for (
                ...
                final_processed_batches[shape_key_hw] = (
                    processed_batch  # Store with (H, W) key
                )
            ...
            if grouped_images_index_dict:
                ...
                for original_idx, idx_info in grouped_images_index_dict.items():
                    ...
                    if (
                        not isinstance(shape_key_hw, (torch.Size, tuple))
                        or len(shape_key_hw) != 2
                    ):
                        raise ValueError(
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            f"Error retrieving processed image for original index {original_idx}: {e}"
                        )
                ...
                logger.warning(
                    "grouped_images_index_dict was empty unexpectedly for batch size > 1."
                )
                final_processed_images_list = []
        ...
        if len(final_processed_images_list) > 0:
            try:
                ...
                logger.error(
                    f"Stacking failed. Images likely have different sizes. Error: {e}"
                )
            ...
            if final_target_h == 0 or final_target_w == 0:
                logger.warning(
                    "Target size not available for empty tensor creation, using 0x0."
                )
            final_images_tensor = torch.empty(
                (0, target_c, final_target_h, final_target_w), dtype=torch.float32
            )
    ...
    def patchify_image(
        ...
        if image.ndim == 3:
            ...
            raise ValueError(
                f"Input image must have 3 or 4 dimensions, got {image.ndim}"
            )
