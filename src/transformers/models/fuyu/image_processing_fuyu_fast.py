...
class FuyuImageProcessorFast(BaseImageProcessorFast):
    # Docstring - Good, explains scope and args
    """ ... (Full docstring as provided previously) ... """

    # Class Attributes - Correct defaults, matches slow processor
    do_resize: bool = True
            ...
            try:
                ...
                if np_image.ndim > 2:
                    try:
                        ...
                        input_df = None
                        logger.warning_once(f"Could not infer channel dimension format for input shape {np_image.shape}. Assuming channels first.")
    ...
    def _preprocess(
        ...
        **kwargs
        ...
        if original_sizes is None or len(original_sizes) != len(images):
             logger.warning("Original sizes not found or mismatched, calculating scale factor based on input tensor size.")
        ...
        for idx, image_list in enumerate(images):
            ...
            if len(image_list) != 1: raise ValueError("Multiple images per sample not yet supported.")
            if not image_list: logger.warning(f"Skipping empty image list at index {idx}"); continue
            image = image_list[0]
            original_height, original_width = original_sizes[idx] if idx < len(original_sizes) else self._get_tensor_image_size(image)
            if do_resize: resized_image = self.resize(image, size=size, interpolation=interpolation)
            ...
            if do_pad: final_image = self.pad_image(resized_image, size=size, mode=padding_mode, value=padding_value)
            else: final_image = resized_image
            # Optional shape check after padding
            final_shape = final_image.shape
            expected_num_dims = 3 # Expect C, H, W
            if len(final_shape) != expected_num_dims: raise ValueError(f"Unexpected final_image dimension {len(final_shape)}")
            if do_pad and (final_shape[-2] != target_height or final_shape[-1] != target_width):
                 raise RuntimeError(f"Padding failed for image index {idx}. Shape was {final_shape}, expected H={target_height}, W={target_width}")
        ...
        if not processed_images_list:
            ...
            if single_image.ndim != 3: raise ValueError(f"Expected 3D tensor for single image, got {single_image.ndim}D shape {single_image.shape}")
            image_batch = single_image.unsqueeze(0)
            processed_batch = self.rescale_and_normalize(
                image_batch, do_rescale=False, rescale_factor=rescale_factor, do_normalize=do_normalize, image_mean=image_mean, image_std=image_std
            )
            ...
            if not all(len(s) == 3 and s[0] == 3 for s in input_shapes):
                 raise ValueError(f"Incorrect shapes entering group_images_by_shape: {input_shapes}. Expected (3, H, W).")
            ...
            if not all(isinstance(key, (torch.Size, tuple)) and len(key) == 2 for key in grouped_images.keys()):
                 logger.warning(f"Unexpected keys from group_images_by_shape: {list(grouped_images.keys())}. Expected (H, W).")

            final_processed_batches = {}
            for shape_key_hw, image_batch in grouped_images.items(): # shape_key_hw is (H, W)
                 processed_batch = self.rescale_and_normalize(
                     image_batch, do_rescale=False, rescale_factor=rescale_factor, do_normalize=do_normalize, image_mean=image_mean, image_std=image_std
                 )
            ...
            if grouped_images_index_dict:
                 if not isinstance(grouped_images_index_dict, dict):
                      ...
                      raise TypeError(f"grouped_images_index was expected to be a dict based on error, but got {type(grouped_images_index_dict)}")
                 ...
                 for original_idx, idx_info in grouped_images_index_dict.items():
                     if not isinstance(idx_info, (list, tuple)) or len(idx_info) != 2:
                         raise ValueError(f"grouped_images_index_dict item {original_idx} has unexpected format: {idx_info}")
                     ...
                     if not isinstance(shape_key_hw, (torch.Size, tuple)) or len(shape_key_hw) != 2:
                          raise ValueError(f"grouped_images_index_dict item {original_idx} has invalid shape key: {shape_key_hw}")
                     if shape_key_hw not in final_processed_batches:
                          raise KeyError(f"Shape key {shape_key_hw} from index dict not found in processed batches. Available keys: {list(final_processed_batches.keys())}")
                     ...
                     try:
                         ...
                         processed_count += 1
                          ...
                          raise IndexError(f"Batch index {batch_idx} out of range for shape {shape_key_hw} in final_processed_batches.")
                     except Exception as e:
                 ...
                 if processed_count != num_images or None in final_processed_images_list:
                      raise RuntimeError(f"Manual reordering failed. Processed {processed_count}/{num_images}. Result list: {final_processed_images_list}")
        ...
        if len(final_processed_images_list) > 0:
            ...
            if final_target_h == 0 or final_target_w == 0: logger.warning("Target size not available for empty tensor creation, using 0x0.")
        ...
        heights_tensor = torch.tensor(image_unpadded_heights_list, **tensor_kwargs_int) if image_unpadded_heights_list else torch.empty((0, 1), **tensor_kwargs_int)
        widths_tensor = torch.tensor(image_unpadded_widths_list, **tensor_kwargs_int) if image_unpadded_widths_list else torch.empty((0, 1), **tensor_kwargs_int)
        scale_factors_tensor = torch.tensor(image_scale_factors_list, **tensor_kwargs_float) if image_scale_factors_list else torch.empty((0, 1), **tensor_kwargs_float)
    ...
    def patchify_image(self, image: "torch.Tensor", patch_size: Optional[Dict[str, int]] = None) -> "torch.Tensor":
        ...
        if height % patch_height != 0 or width % patch_width != 0:
            raise ValueError(f"Image size ({height}, {width}) must be divisible by patch size ({patch_height}, {patch_width}).")
