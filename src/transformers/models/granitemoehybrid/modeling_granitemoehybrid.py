...
class GraniteMultiHeadLatentAttention(nn.Module):
    def __init__(self, config: GraniteMoeHybridConfig, layer_idx: int):
        ...
        self.c_attn_down_projection = nn.Linear(
            self.hidden_size, self.query_compression_size + 2 * self.key_value_compression_size, bias=self.add_bias
        )
        self.query_up_projection = nn.Linear(self.query_compression_size, self.hidden_size, bias=self.add_bias)
        self.key_up_projection = nn.Linear(self.key_value_compression_size, self.hidden_size, bias=self.add_bias)
        self.value_up_projection = nn.Linear(self.key_value_compression_size, self.hidden_size, bias=self.add_bias)
