...
class GraniteMultiHeadLatentAttention(nn.Module):
    def __init__(self, config: GraniteMoeHybridConfig, layer_idx: int):
        ...
        self.softmax_dropout = (
            nn.Identity() if config.mla_softmax_dropout == 0 else nn.Dropout(config.mla_softmax_dropout)
        )
...
class GraniteMoeHybridAttention(nn.Module):
    ...
    def __init__(self, config: GraniteMoeHybridConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
...
# NO LONGER EXIST Copied from transformers.models.granite.modeling_granite.GraniteFlashAttention2 with Granite->GraniteMoeHybrid
# TODO cyril: modular
class GraniteMoeHybridFlashAttention2(GraniteMoeHybridAttention):
    """
    GraniteMoeHybrid flash attention module. This module inherits from `GraniteMoeHybridAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # in fp32. (GraniteMoeHybridRMSNorm handles it correctly)
...
# NO LONGER EXIST Copied from transformers.models.granite.modeling_granite.GraniteSdpaAttention with Granite->GraniteMoeHybrid
# TODO cyril: modular
class GraniteMoeHybridSdpaAttention(GraniteMoeHybridAttention):
    """
    GraniteMoeHybrid attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `GraniteMoeHybridAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from GraniteMoeHybridAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "GraniteMoeHybridModel is using GraniteMoeHybridSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
