        ...
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    ...
                    "Please make sure to instantiate class with `H3Attention(..., is_cross_attention=True)`."
                )
...
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2MLP with GPT2->H3
...
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2Block with GPT2->H3
class H3Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = H3Attention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = H3Attention(config, is_cross_attention=True, layer_idx=layer_idx)
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        self.mlp = H3MLP(inner_dim, config)
...
@add_start_docstrings(
    "The bare H3 Model transformer outputting raw hidden-states without any specific head on top.",
    H3_START_DOCSTRING,
)
...
@add_start_docstrings(
    """
    The H3 Model transformer with a language modeling head on top (linear layer with weights tied to the input
    embeddings).
    """,
    H3_START_DOCSTRING,
)
