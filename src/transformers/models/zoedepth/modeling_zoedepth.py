...
@add_start_docstrings(
    """
    ZOEDEPTH Model with a semantic segmentation head on top e.g. for ADE20k, CityScapes.
    """,
    ZOEDEPTH_START_DOCSTRING,
)
class ZoeDepthForSemanticSegmentation(ZoeDepthPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        # Segmentation head(s)
        self.head = ZoeDepthSemanticSegmentationHead(config)
        self.auxiliary_head = ZoeDepthAuxiliaryHead(config) if config.use_auxiliary_head else None
    ...
    def forward(
        ...
        Examples:
        ```python
        >>> from transformers import AutoImageProcessor, ZoeDepthForSemanticSegmentation
        >>> from PIL import Image
        >>> import requests
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("Intel/zoedepth-base-ade")
        >>> model = ZoeDepthForSemanticSegmentation.from_pretrained("Intel/zoedepth-base-ade")
