...
# Conditionally add tokenizer to the structure based on SentencePiece availability
try:
    if not is_sentencepiece_available():
        raise OptionalDependencyNotAvailable()
except OptionalDependencyNotAvailable:
    # Don't add tokenizer if sentencepiece is not available
    pass
else:
    _import_structure["tokenization_hindi_causal_lm"] = ["HindiCausalLMTokenizer"]

# Conditionally add modeling classes to the structure based on PyTorch availability
try:
