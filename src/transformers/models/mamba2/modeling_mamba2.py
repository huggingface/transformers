...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->Mamba2
class Mamba2RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Mamba2RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralAttention with Mistral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaAttention with Jamba->Mamba2
class Mamba2Attention(nn.Module):
    ...
    def __init__(self, config: Mamba2Config, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralFlashAttention2 with Mistral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaFlashAttention2 with Jamba->Mamba2
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralSdpaAttention with Mistral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaSdpaAttention with Jamba->Mamba2
class Mamba2SdpaAttention(Mamba2Attention):
    ...
    `Mamba2Attention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from Mamba2Attention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Mamba2Model is using Mamba2SdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
# Copied from transformers.models.jamba.modeling_jamba.JambaMambaMixer with Jamba->Mamba2
class Mamba2MambaMixer(nn.Module):
    """
    ...
    def __init__(self, config: Mamba2Config, layer_idx):
        ...
        self.dt_layernorm = Mamba2RMSNorm(self.time_step_rank, eps=config.rms_norm_eps)
        self.b_layernorm = Mamba2RMSNorm(self.ssm_state_size, eps=config.rms_norm_eps)
        self.c_layernorm = Mamba2RMSNorm(self.ssm_state_size, eps=config.rms_norm_eps)
...
# Copied from transformers.models.mistral.modeling_mistral.MistralMLP with Mistral->Mamba2
...
# Adapted from transformers.models.mixtral.modeling_mixtral.MixtralSparseMoeBlock with Mistral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaSparseMoeBlock with Jamba->Mamba2
class Mamba2SparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config: Mamba2Config):
        ...
        self.experts = nn.ModuleList([Mamba2MLP(config) for _ in range(self.num_experts)])
...
# Copied from transformers.models.jamba.modeling_jamba.JambaAttentionDecoderLayer with JAMBA->MAMBA2,Jamba->Mamba2
class Mamba2AttentionDecoderLayer(nn.Module):
    def __init__(self, config: Mamba2Config, layer_idx: int):
        super().__init__()
        num_experts = config.layers_num_experts[layer_idx]
        self.self_attn = MAMBA2_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        ffn_layer_class = Mamba2SparseMoeBlock if num_experts > 1 else Mamba2MLP
        self.feed_forward = ffn_layer_class(config)
        self.input_layernorm = Mamba2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.pre_ff_layernorm = Mamba2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
# Copied from transformers.models.jamba.modeling_jamba.JambaMambaDecoderLayer with Jamba->Mamba2
class Mamba2MambaDecoderLayer(nn.Module):
    def __init__(self, config: Mamba2Config, layer_idx: int):
        super().__init__()
        num_experts = config.layers_num_experts[layer_idx]
        self.mamba = Mamba2MambaMixer(config=config, layer_idx=layer_idx)
...
# Copied from transformers.models.jamba.modeling_jamba.JambaPreTrainedModel with Jamba->Mamba2
class Mamba2PreTrainedModel(PreTrainedModel):
    config_class = Mamba2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Mamba2AttentionDecoderLayer", "Mamba2MambaDecoderLayer"]
...
ALL_DECODER_LAYER_TYPES = {"attention": Mamba2AttentionDecoderLayer, "mamba": Mamba2MambaDecoderLayer}
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralModel with MISTRAL->MAMBA2, Mistral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaModel with JAMBA->MAMBA2,Jamba->Mamba2
class Mamba2Model(Mamba2PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Mamba2DecoderLayer`]
    ...
    def __init__(self, config: Mamba2Config):
        ...
        self.final_layernorm = Mamba2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        if use_cache and past_key_values is None:
            logger.warning_once(
                "Mamba2 requires an initialized `HybridMambaAttentionDynamicCache` to return a cache. None was "
...
# Adapted from transformers.models.mixtral.modeling_mixtral.MixtralForCausalLM with MIXTRAL->MAMBA2, Mixtral->Mamba2
# Copied from transformers.models.jamba.modeling_jamba.JambaForCausalLM with JAMBA->MAMBA2,Jamba->Mamba2
class Mamba2ForCausalLM(Mamba2PreTrainedModel):
    ...
    def forward(
        ...
        >>> model = Mamba2ForCausalLM.from_pretrained("ai21labs/Mamba2-v0.1")
        >>> tokenizer = AutoTokenizer.from_pretrained("ai21labs/Mamba2-v0.1")
...
@add_start_docstrings(
    """
    The Mamba2 Model with a sequence classification head on top (linear layer).
...
# Copied from transformers.models.mixtral.modeling_mixtral.MixtralForSequenceClassification with Mixtral->Mamba2, MIXTRAL->MAMBA2
