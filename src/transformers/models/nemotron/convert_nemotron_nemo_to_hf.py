...
def get_args():
    ...
    parser.add_argument(
        ...
        "e.g. a folder containing https://huggingface.co/nvidia/Minitron-8B-Base",
    )
    parser.add_argument(
        "--hf_output_path",
        type=str,
...
def convert_hf_config(
    ...
    dtype,
    hf_output_path,
    hf_url="nvidia/Minitron-8B-Base",
):
    """
    ...
    hf_config = {
        ...
        "num_key_value_heads": nemo_config.get(
            "num_query_groups", nemo_config.num_attention_heads
        ),
...
def convert(input_nemo_file, output_hf_file, precision=None, cpu_only=False) -> None:
    ...
    for l in range(int(num_layers)):
        ...
        if (
            model.state_dict().get(
                f"model.decoder.layers.{l}.mlp.linear_fc1.layer_norm_bias", None
            )
            is not None
        ):
...
def extract_nemotron_tokenizer(nemo_file, model_config, output_hf_path, nemo_tokenizer):
    ...
    if tokenizer_cfg.library == "sentencepiece":
        ...
        raise ValueError(
            f"Unsupported tokenizer type: library: {tokenizer_cfg.library}, type: {tokenizer_cfg.type}"
        )
...
if __name__ == "__main__":
    ...
    if not args.hf_output_path:
        ...
        ), "Need to provide either output_path or hf_output_path"
    else:
    ...
    if args.hf_input_path and args.hf_output_path:
        ...
        extract_nemotron_tokenizer(
            args.input_name_or_path, nemo_config, args.hf_output_path, nemo_tokenizer
        )
    else:
        logging.info(
