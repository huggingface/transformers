...
_CHECKPOINT_FOR_DOC = "flexibert-mini"
_CONFIG_FOR_DOC = "FlexiBERTConfig"
_TOKENIZER_FOR_DOC = "FlexiBERTTokenizer"
...
def load_tf_weights_in_flexibert(model, config, tf_checkpoint_path):
...
class FlexiBERTEmbeddings(nn.Module):
...
class FlexiBERTSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.W = nn.Linear(self.attention_head_size, self.attention_head_size, bias=False)
...
class FlexiBERTHeteroAttention(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
    ...
    def forward(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if self.position_embedding_type == "relative_key":
                relative_position_scores = torch.einsum("bhld,lrd->bhlr", query_layer, positional_embedding)
                print(attention_scores.size(), relative_position_scores.size())
                attention_scores = attention_scores + relative_position_scores
            elif self.position_embedding_type == "relative_key_query":
                relative_position_scores_query = torch.einsum("bhld,lrd->bhlr", query_layer, positional_embedding)
                relative_position_scores_key = torch.einsum("bhrd,lrd->bhlr", key_layer, positional_embedding)
...
class FlexiBERTSelfOutput(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
...
class FlexiBERTAttention(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
...
class FlexiBERTIntermediate(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        ...
        modules = []
        modules.append(nn.Linear(config.hidden_dim_list[layer_id], config.ff_dim_list[layer_id][0]))
...
class FlexiBERTOutput(nn.Module):
...
class FlexiBERTEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        layer_list = []
...
class FlexiBERTPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FlexiBERTPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_dim_list[-1], config.hidden_dim_list[-1])
        if isinstance(config.hidden_act, str):
...
class FlexiBERTLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = FlexiBERTPredictionHeadTransform(config)
...
class FlexiBERTOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = FlexiBERTLMPredictionHead(config)
...
class FlexiBERTOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FlexiBERTPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = FlexiBERTLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_dim_list[-1], 2)
...
class FlexiBERTPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = FlexiBERTConfig
    base_model_prefix = "flexibert"
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, FlexiBERTEncoder):
...
@add_start_docstrings(
    "The bare FlexiBERT Model transformer outputting raw hidden-states without any specific head on top.",
    FLEXIBERT_START_DOCSTRING,
)
class FlexiBERTModel(FlexiBERTPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True, transfer_mode='OD'):
        ...
        self.embeddings = FlexiBERTEmbeddings(config)
...
class FlexiBERTForMaskedLM(FlexiBERTPreTrainedModel):
    ...
    def __init__(self, config, transfer_mode='OD'):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `FlexiBERTForMaskedLM` make sure `config.is_decoder=False` for "
        ...
        self.cls = FlexiBERTOnlyMLMHead(config)
...
class FlexiBERTForSequenceClassification(FlexiBERTPreTrainedModel):
    def __init__(self, config):
        ...
        self.flexibert = FlexiBERTModel(config)
        self.classifier = FlexiBERTClassificationHead(config)
