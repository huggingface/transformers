...
class FlexiBERTConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`~FlexiBERTModel`].
    It is used to instantiate an FlexiBERT model according to the specified arguments, defining the model
    architecture. Instantiating a configuration with the defaults will yield a similar configuration to that of
    the FlexiBERT [flexibert-mini](https://huggingface.co/flexibert-mini) architecture.
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 30522):
            Vocabulary size of the FlexiBERT model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`~FlexiBERTModel`] or
            [`~TFFlexiBERTModel`].
        ...
        type_vocab_size (`int`, *optional*, defaults to 2):
            The vocabulary size of the `token_type_ids` passed when calling [`~FlexiBERTModel`] or
            [`~TFFlexiBERTModel`].
