...
class RTDetrV2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (RTDetrV2ForObjectDetection, RTDetrV2Decoder)):
            if module.decoder.class_embed is not None:
                for layer in module.decoder.class_embed:
                    prior_prob = self.config.initializer_bias_prior_prob or 1 / (self.config.num_labels + 1)
                    bias = float(-math.log((1 - prior_prob) / prior_prob))
                    nn.init.xavier_uniform_(layer.weight)
                    nn.init.constant_(layer.bias, bias)

            if module.decoder.bbox_embed is not None:
                for layer in module.decoder.bbox_embed:
