...
class DeepseekV4DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs: Unpack[TransformersKwargs]) -> torch.Tensor:
        ...
        pre, post, comb = _hyper_connection_weights(
            hidden_states, self.hc_attn_fn, self.hc_attn_base, self.hc_attn_scale,
        ...
        # --- MLP site ---
        pre, post, comb = _hyper_connection_weights(
            hidden_states, self.hc_ffn_fn, self.hc_ffn_base, self.hc_ffn_scale,
            self.hc_mult, self.hc_sinkhorn_iters, self.norm_eps, self.hc_eps,
        )
        collapsed = _hyper_connection_collapse(hidden_states, pre)
...
class DeepseekV4PreTrainedModel(MixtralPreTrainedModel):
    ...
    _keep_in_fp32_modules_strict = [
        ...
        "hc_head.hc_fn", "hc_head.hc_base", "hc_head.hc_scale",
    ]
