...
def convert_model(vq_model_id, llm_model_id, output_dir, hub_model_id=None, test_inference=False):
    ...
    image_processor = ImageProcessor.from_pretrained(vq_model_id)
    ...
    config = CosmosConfig(text_config=text_config, vocabulary_map=vocabulary_map)

    with init_empty_weights():
        model = ForConditionalGeneration(config=config)
    ...
    if test_inference and llm_model_id.endswith("Chat"):
        ...
        print("Loading the checkpoint in a  model...")
        print("*" * 100)
        model = ForConditionalGeneration.from_pretrained(output_dir, torch_dtype=torch.bfloat16, device_map="auto")
...
def main():
    ...
    parser.add_argument(
        "--vq_model_id",
        help="Model ID of  VQ-VAE on the hub",
        default="BAAI/-VisionTokenizer",
    )
    parser.add_argument(
        "--llm_model_id",
        help="Model ID of  bacbone LLM on the hub",
        default="BAAI/-Chat",
    )
    parser.add_argument(
        "--output_dir",
