...
class VoxtralEncoderConfig(PretrainedConfig):
    ...
    Args:
        ...
        num_key_value_heads (`int`, *optional*, defaults to 20):
            Number of key and value heads for each attention layer.
...
class VoxtralConfig(PretrainedConfig):
    ...
    ```python
    >>> from transformers import VoxtralForConditionalGeneration, VoxtralConfig, VoxtralEncoderConfig, Qwen2Config

    >>> # Initializing a VoxtralEncoder config
    ...
    >>> configuration = VoxtralConfig(audio_config, text_config)
