...
@auto_docstring(
    custom_intro="""
    Base class for Voxtral causal language model (or autoregressive) outputs.
    """
)
class VoxtralCausalLMOutputWithPast(ModelOutput):
    r"""
...
class VoxtralForConditionalGeneration(VoxtralPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | VoxtralCausalLMOutputWithPast:
        r"""
        Example:
        ...
        return VoxtralCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
