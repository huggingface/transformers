...
def repeat_kv(hidden_states: torch.Tensor, num_groups: int) -> torch.Tensor:
    ...
    hidden_states = hidden_states[:, :, None, :, :].expand(batch, num_key_value_heads, num_groups, slen, head_dim)
    return hidden_states.reshape(batch, num_key_value_heads * num_groups, slen, head_dim)
...
class BlueberryAttention(nn.Module):
    def __init__(self, config: BlueberryConfig, layer_idx: int, attention_type: str):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.attention_type = attention_type
        self.head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
        self.num_key_value_groups = 1  # MHA default (no GQA/MQA in spec)
        self.scaling = self.head_dim**-0.5
        self.attention_dropout = config.attention_dropout
        self.is_causal = True

        self.q_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=config.attention_bias)
        self.k_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=config.attention_bias)
        self.v_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=config.attention_bias)
        self.o_proj = nn.Linear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=config.attention_bias)

        # sliding window only for layers declared as sliding_attention
...
class BlueberryMLP(nn.Module):
    def __init__(self, config: BlueberryConfig):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, intermediate_size, bias=True)
        self.up_proj = nn.Linear(self.hidden_size, intermediate_size, bias=True)
        self.down_proj = nn.Linear(intermediate_size, self.hidden_size, bias=True)
...
class BlueberryDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: BlueberryConfig, layer_idx: int):
        ...
        if attention_type == "full_attention":
            # NoPE full attention layer
            self.self_attn = BlueberryNoPEAttention(config=config, layer_idx=layer_idx, attention_type=attention_type)
        else:
            # RoPE sliding window attention layer
            self.self_attn = BlueberryAttention(config=config, layer_idx=layer_idx, attention_type=attention_type)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if isinstance(self.self_attn, BlueberryAttention):
...
class BlueberryPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        ...
        "attentions": (BlueberryAttention, BlueberryNoPEAttention),
    }
