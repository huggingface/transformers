...
# Copied from transformers.models.timm_backbone.modeling_timm_backbone.TimmBackbone with TimmBackbone->TimmWrapper,timm_backbone->timm_wrapper
class TimmWrapper(PreTrainedModel, BackboneMixin):
    """
    ...
    def __init__(self, config, **kwargs):
        ...
        if hasattr(config, "out_features") and config.out_features is not None:
            raise ValueError("out_features is not supported by TimmWrapper. Please use out_indices instead.")
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        config = kwargs.pop("config", TimmWrapperConfig())

        use_timm = kwargs.pop("use_timm_wrapper", True)
        if not use_timm:
            raise ValueError("use_timm_wrapper must be True for timm backbones")
