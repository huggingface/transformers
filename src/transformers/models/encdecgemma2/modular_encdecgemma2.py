...
from ...cache_utils import Cache, HybridCache, StaticCache, EncoderDecoderCache
...
class EncdecGemma2StackConfig(Gemma2Config):
...
class EncdecGemma2MLP(Gemma2MLP):
...
class EncdecGemma2RotaryEmbedding(Gemma2RotaryEmbedding):
...
class EncdecGemma2Attention(nn.Module):
    def __init__(
        ...
        self.cross_attention_hidden_size = config.cross_attention_hidden_size if is_cross_attention else config.hidden_size
        ...
        if self.is_cross_attention:
            ...
            if self.cross_attention_hidden_size != config.hidden_size:
                raise ValueError(
                    f"{config.hidden_size} != {self.cross_attention_hidden_size} in self-attention."
                )
...
class EncdecGemma2Block(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        # self attention
        self.self_attn = EncdecGemma2Attention(
            config=config,
            layer_idx=layer_idx,
...
class EncdecGemma2PreTrainedModel(Gemma2PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.is_decoder and encoder_hidden_states is None:
            raise ValueError(f"`encoder_hidden_states` must be given in decoder")
...
__all__ = [
    ...
    "EncdecGemma2Model",
    "EncdecGemma2PreTrainedModel",  # noqa: F822
