...
class HeliumRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        HeliumRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class HeliumRotaryEmbedding(nn.Module):
    def __init__(
        ...
        if config is None:
            logger.warning_once(
                "`HeliumRotaryEmbedding` can now be fully parameterized by passing the model config through the "
...
class HeliumFlashAttention2(HeliumAttention):
    """
    Helium flash attention module. This module inherits from `HeliumAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # in fp32. (HeliumRMSNorm handles it correctly)
...
class HeliumSdpaAttention(HeliumAttention):
    """
    Helium attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `HeliumAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from HeliumAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "HeliumModel is using HeliumSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class HeliumDecoderLayer(nn.Module):
    def __init__(self, config: HeliumConfig, layer_idx: Optional[int] = None):
        ...
        self.self_attn = HELIUM_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
