...
class ChatGlmRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        ChatGlmRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
ALL_LAYERNORM_LAYERS.append(ChatGlmRMSNorm)


# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->ChatGlm
class ChatGlmRotaryEmbedding(nn.Module):
...
# Copied from transformers.models.llama.modeling_llama.LlamaLinearScalingRotaryEmbedding with Llama->ChatGlm
class ChatGlmLinearScalingRotaryEmbedding(ChatGlmRotaryEmbedding):
    """ChatGlmRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
# Copied from transformers.models.llama.modeling_llama.LlamaDynamicNTKScalingRotaryEmbedding with Llama->ChatGlm
class ChatGlmDynamicNTKScalingRotaryEmbedding(ChatGlmRotaryEmbedding):
    """ChatGlmRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
...
# Copied from transformers.models.gpt_neox.modeling_gpt_neox.GPTNeoXMLP with GPTNeoX->ChatGlm
...
class ChatGlmAttention(nn.Module):
    ...
    # Copied from transformers.models.llama.modeling_llama.LlamaAttention._shape with Llama->ChatGlm
...
class ChatGlmFlashAttention2(ChatGlmAttention):
    """
    ChatGlm flash attention module. This module inherits from `ChatGlmAttention` as the weights of the module stays
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ChatGlmFlashAttention2 attention does not support output_attentions
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be removed in v4.37. Please make sure use `attention_mask` instead.`"
            )
        ...
        # in fp32. (ChatGlmRMSNorm handles it correctly)
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in ChatGlmFlashAttention2 __init__.
...
# Copied from transformers.models.llama.modeling_llama.LlamaPreTrainedModel with Llama->ChatGlm
class ChatGlmPreTrainedModel(PreTrainedModel):
    config_class = ChatGlmConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["ChatGlmDecoderLayer"]
...
# Copied from transformers.models.llama.modeling_llama.LlamaModel with LLAMA->CHATGLM,Llama->ChatGlm
...
# Copied from transformers.models.llama.modeling_llama.LlamaForCausalLM with LLAMA->CHATGLM,Llama->ChatGlm
class ChatGlmForCausalLM(ChatGlmPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = ChatGlmForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
@add_start_docstrings(
    ...
    [`ChatGlmForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with LLAMA->CHATGLM,Llama->ChatGlm
