...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->Blip2QFormer
class Blip2QFormerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in Blip2QFormerModel forward() function)
...
# Copied from transformers.models.bert.modeling_bert.BertAttention with Bert->Blip2QFormer
class Blip2QFormerAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = Blip2QFormerSelfAttention(config, position_embedding_type=position_embedding_type)
...
# Copied from transformers.models.bert.modeling_bert.BertEmbeddings with Bert->Blip2QFormer
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->Blip2QFormer
class Blip2QFormerLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = Blip2QFormerAttention(config, position_embedding_type="absolute")
...
# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->Blip2QFormer
class Blip2QFormerEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([Blip2QFormerLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
@add_start_docstrings(
    """
    BLIP-2 Model for image captioning. The model consists of a vision encoder, Querying Transformer (Q-Former) and a
    language model.
