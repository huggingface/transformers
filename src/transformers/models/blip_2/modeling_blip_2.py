...
@dataclass
# Copied from transformers.models.blip.modeling_blip.BlipOutput with Blip->Blip2
class Blip2Output(ModelOutput):
    ...
    Args:
        ...
        text_embeds(`torch.FloatTensor` of shape `(batch_size, output_dim`):
            The text embeddings obtained by applying the projection layer to the pooled output of [`Blip2TextModel`].
        image_embeds(`torch.FloatTensor` of shape `(batch_size, output_dim`):
            The image embeddings obtained by applying the projection layer to the pooled output of
            [`Blip2VisionModel`].
        text_model_output(`BaseModelOutputWithPooling`):
            The output of the [`Blip2TextModel`].
...
@dataclass
# Copied from transformers.models.blip.modeling_blip.BlipImageTextMatchingModelOutput with Blip->Blip2
...
# Copied from transformers.models.blip.modeling_blip_text.BlipTextPredictionHeadTransform with Blip->Blip2
class Blip2TextPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
# Copied from transformers.models.blip.modeling_blip_text.BlipTextLMPredictionHead with Blip->Blip2
class Blip2TextLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = Blip2TextPredictionHeadTransform(config)
...
# Copied from transformers.models.blip.modeling_blip_text.BlipTextOnlyMLMHead with Blip->Blip2
class Blip2TextOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = Blip2TextLMPredictionHead(config)
...
class Blip2ModelWithProjection(Blip2PreTrainedModel):
    config_class = Blip2Config
    main_input_name = "pixel_values"
    ...
    def get_text_features(
        ...
        text_features = self.text_proj(embeds)
    ...
    def get_image_features(
        ...
        embeds = query_outputs.last_hidden_state
        image_features = normalize(self.vision_proj(embeds), dim=-1)
    ...
    def forward(
        ...
        ```"""
        # Use BLIP2 model's config for some fields (if specified) instead of those of vision & text components.
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        question_outputs = self.qformer(
            input_ids=input_ids,
            attention_mask=attention_mask,
            return_dict=return_dict,
        )
        question_embeds = question_outputs[0] if not return_dict else question_outputs.last_hidden_state
        question_embeds = self.text_proj(question_embeds[:, 0, :])

        image_feat = normalize(image_embeds, dim=-1)
        text_feat = normalize(question_embeds, dim=-1)

        # text-query similarity
        sim_t2q = torch.matmul(text_feat.unsqueeze(1).unsqueeze(1), image_feat.permute(0, 2, 1)).squeeze(2)

        # text-image similarity: aggregate across all query tokens
        logits_per_text, _ = sim_t2q.max(-1)
        logits_per_text = logits_per_text / self.temp
        logits_per_image = logits_per_text.t()

        loss = None
        if return_loss:
            loss = blip2_loss(logits_per_text)

        if not return_dict:
            output = (logits_per_image, logits_per_text, question_embeds, image_embeds, query_outputs, vision_outputs)
            return ((loss,) + output) if loss is not None else output
        ...
        return Blip2Output(
            ...
            text_model_output=query_outputs,
...
class Blip2VisionModelWithProjection(Blip2PreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(BLIP_2_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Blip2VisionModelOutput, config_class=Blip2VisionConfig)
    def forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
...
class Blip2ForImageTextRetrieval(Blip2PreTrainedModel):
    ...
    def __init__(self, config: Blip2Config):
        ...
        self.qformer = Blip2QFormerModel(config.qformer_config)
        self.cls = Blip2TextOnlyMLMHead(config.qformer_config)
    ...
    @add_start_docstrings_to_model_forward(BLIP_2_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Blip2ImageTextMatchingModelOutput, config_class=Blip2VisionConfig)
    def forward(
        self,
        pixel_values: torch.FloatTensor,
        ...
        if use_itm_head:
            ...
            outputs = torch.bmm(image_feat, text_feat.unsqueeze(-1))
            output, _ = torch.max(outputs, dim=1)
