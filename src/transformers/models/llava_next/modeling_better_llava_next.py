...
class BetterLlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    def __init__(self, config: LlavaNextConfig):
...
class BetterAlternativeLlavaNextForConditionalGeneration(BetterLlavaNextForConditionalGeneration):
    def pack_image_features(self, image_features, image_sizes, image_newline=None):
        """
        This pack_image_features function:
        - pre-calculate the sizes and indices of each path on the final image_features
        - create a single tensor for final_image_features
        - copy each image feature at the right position to the final image features
        ...
        """
        feature_lens = []
        # ! pre-calculating sizes and patch_index
        #   this should be done by cpu and done at processor
        patch_indices = []
        img_dim = None
        image_features_shapes = [x.shape for x in image_features]
        with torch.no_grad():
            for image_idx, image_feature in enumerate(image_features):
                if image_feature.shape[0] > 1:
                    ...
                    patch_index = torch.arange(num_patches * patch_len, dtype=torch.int32, device=image_feature.device)
                    assert num_patches == (num_patch_width * num_patch_height), (
                        f'{image_idx=} | {num_patches=} {num_patch_width=} * {num_patch_height=} | {img_size=} | '
                        f'{self.config.vision_config.image_size=} | {self.config.image_grid_pinpoints=} |'
                        f'{image_feature.size()=} '
                    )
                    patch_index = patch_index.view(num_patch_height, num_patch_width, height, width)
                    patch_index = patch_index.permute(0, 2, 1, 3).flatten(0, 1).flatten(1, 2).unsqueeze_(0)
                    patch_index = unpad_image(patch_index, image_sizes[image_idx])
                    if image_newline is not None:
                        patch_index = torch.cat(
                            (
                                patch_index,
                                torch.tensor([-1e5], dtype=patch_index.dtype, device=patch_index.device)[:, None, None].expand(*patch_index.shape[:-1], 1),
                            ),
                            dim=-1,
                        )
                    patch_index = patch_index.flatten(1, 2).transpose(0, 1)
                    patch_index = torch.cat((
                        torch.arange(patch_len, dtype=torch.int32, device=image_feature.device).unsqueeze_(-1),
                        patch_index + patch_len,
                    ), dim=0)
                else:
                    patch_index = torch.arange(patch_len, dtype=torch.int32, device=image_feature.device).unsqueeze_(-1)
                    if image_newline is not None:
                        patch_index = torch.cat(
                            (
                                patch_index,
                ...
                feature_lens.append(patch_index.size(0))
        all_image_features = image_features[0].new(
            sum(feature_lens), img_dim,
        ).fill_(-1000)
        begin_index = 0
        if image_newline is not None:
            image_newline = image_newline.to(all_image_features)
        for image_idx, (image_feature, patch_index, feat_len) in enumerate(zip(image_features, patch_indices, feature_lens)):
            if image_feature.shape[0] > 1:
                flat_image_feature = image_feature.contiguous().view(-1, img_dim)
                with torch.no_grad():
                    image_positions = patch_index >= 0
                    image_index = patch_index[image_positions]
                    patch_arange = torch.arange(feat_len, dtype=torch.long, device=image_feature.device)
                    img_patch_positions = patch_arange[image_positions.squeeze()]
                all_image_features[begin_index:begin_index + feat_len][img_patch_positions] = flat_image_feature[image_index]
                if image_newline is not None:
                    newline_positions = patch_index < 0
                    nll_patch_positions = patch_arange[newline_positions.squeeze()]
                    all_image_features[begin_index:begin_index + feat_len][nll_patch_positions] = image_newline[None, :].expand(len(nll_patch_positions), img_dim)
                begin_index += feat_len
            else:
                flat_image_feature = image_feature.view(-1, img_dim)
                newline_offset = (1 if image_newline is not None else 0)
                assert flat_image_feature.size(0) == feat_len - newline_offset, f'wrong leng? {flat_image_feature.size()=} !=- {feat_len=}'
                all_image_features[begin_index:begin_index + feat_len - newline_offset] = flat_image_feature
                if image_newline is not None:
                    all_image_features[begin_index + feat_len - newline_offset] = image_newline
                begin_index += feat_len
        feature_lens = torch.tensor(feature_lens, dtype=torch.long, device=all_image_features.device)
