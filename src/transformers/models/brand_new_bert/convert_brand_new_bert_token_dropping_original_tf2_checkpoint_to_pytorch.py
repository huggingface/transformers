...
"""
This script converts a lm-head checkpoint from the "Token Dropping" implementation into a PyTorch-compatible BRAND_NEW_BERT
...
from transformers import BrandNewBERTConfig, BrandNewBERTForMaskedLM
from transformers.models.brand_new_bert.modeling_brand_new_bert import (
    BrandNewBERTIntermediate,
    BrandNewBERTLayer,
    BrandNewBERTOutput,
    BrandNewBERTPooler,
    BrandNewBERTSelfAttention,
    BrandNewBERTSelfOutput,
)
...
def convert_checkpoint_to_pytorch(tf_checkpoint_path: str, config_path: str, pytorch_dump_path: str):
    ...
    config = BrandNewBERTConfig.from_json_file(config_path)
    model = BrandNewBERTForMaskedLM(config)

    # Layers
    for layer_index in range(0, config.num_hidden_layers):
        layer: BrandNewBERTLayer = model.brand_new_bert.encoder.layer[layer_index]

        # Self-attention
        self_attn: BrandNewBERTSelfAttention = layer.attention.self
        ...
        self_output: BrandNewBERTSelfOutput = layer.attention.output
        ...
        # Intermediate
        intermediate: BrandNewBERTIntermediate = layer.intermediate
        ...
        # Output
        brand_new_bert_output: BrandNewBERTOutput = layer.output

        brand_new_bert_output.dense.weight.data = get_encoder_layer_array(layer_index, "_output_dense/kernel")
        brand_new_bert_output.dense.bias.data = get_encoder_layer_array(layer_index, "_output_dense/bias")

        brand_new_bert_output.LayerNorm.weight.data = get_encoder_layer_array(layer_index, "_output_layer_norm/gamma")
        brand_new_bert_output.LayerNorm.bias.data = get_encoder_layer_array(layer_index, "_output_layer_norm/beta")

    # Embeddings
    model.brand_new_bert.embeddings.position_embeddings.weight.data = get_encoder_array("_position_embedding_layer/embeddings")
    model.brand_new_bert.embeddings.token_type_embeddings.weight.data = get_encoder_array("_type_embedding_layer/embeddings")
    model.brand_new_bert.embeddings.LayerNorm.weight.data = get_encoder_array("_embedding_norm_layer/gamma")
    model.brand_new_bert.embeddings.LayerNorm.bias.data = get_encoder_array("_embedding_norm_layer/beta")
    ...
    model.brand_new_bert.embeddings.word_embeddings.weight.data = get_masked_lm_array("embedding_table")

    # Pooling
    model.brand_new_bert.pooler = BrandNewBERTPooler(config=config)
    model.brand_new_bert.pooler.dense.weight.data: BrandNewBERTPooler = get_encoder_array("_pooler_layer/kernel")
    model.brand_new_bert.pooler.dense.bias.data: BrandNewBERTPooler = get_encoder_array("_pooler_layer/bias")

    # Export final model
    model.save_pretrained(pytorch_dump_path)

    # Integration test - should load without any errors ;)
    new_model = BrandNewBERTForMaskedLM.from_pretrained(pytorch_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the BRAND_NEW_BERT model. This specifies the model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path",
        type=str,
    ...
    convert_checkpoint_to_pytorch(args.tf_checkpoint_path, args.brand_new_bert_config_file, args.pytorch_dump_path)
