...
def convert_weight_and_push(name: str, config: ResNetConfig, save_directory: Path, push_to_hub: bool = True):
    ...
    if push_to_hub:
        our_model.push_to_hub(repo_id=checkpoint_name, commit_message="Add model", use_temp_dir=True)

        # we can use the convnext one
        image_processor = AutoImageProcessor.from_pretrained("facebook/convnext-base-224-22k-1k")
        image_processor.push_to_hub(repo_id=checkpoint_name, commit_message="Add image processor", use_temp_dir=True)
