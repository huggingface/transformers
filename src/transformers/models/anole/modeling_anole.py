...
class AnoleVQVAEAttnBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                down.downsample = AnoleVQVAEEncoderConvDownsample(block_in)
                curr_res = curr_res // 2
...
class AnolePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, AnoleVQVAE):
...
class AnoleImageVocabularyMapping:
    """
...
class AnoleRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        AnoleRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
# copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->Anole
# TODO(joao): add me back asap :)
class AnoleRotaryEmbedding(nn.Module):
...
class AnoleLinearScalingRotaryEmbedding(AnoleRotaryEmbedding):
    """AnoleRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class AnoleDynamicNTKScalingRotaryEmbedding(AnoleRotaryEmbedding):
    """AnoleRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
...
class AnoleLayerNorm(nn.LayerNorm):
    """
    LayerNorm but computes stats only over the last dim because Anole applies gamma and beta
...
class AnoleAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: AnoleConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.q_norm = AnoleLayerNorm((self.num_heads, self.head_dim))
        self.k_norm = AnoleLayerNorm((self.num_key_value_heads, self.head_dim))
        self._init_rope()

    # copied from transformers.models.llama.modeling_llama.LlamaAttention._init_rope with Llama->Anole
    # TODO(joao): add me back asap :)
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = AnoleRotaryEmbedding(
                self.head_dim,
            ...
            if scaling_type == "linear":
                self.rotary_emb = AnoleLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = AnoleDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
# NO LONGER EXIST copied from transformers.models.llama.modeling_llama.LlamaFlashAttention2 with Llama->Anole
# TODO(joao): add me back asap :)
class AnoleFlashAttention2(AnoleAttention):
    """
    Anole flash attention module. This module inherits from `AnoleAttention` as the weights of the module stays
    ...
    def forward(
        ...
        **kwargs,
        ...
        # in fp32. (AnoleRMSNorm handles it correctly)
...
class AnoleSdpaAttention(AnoleAttention):
    """
    Anole attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `AnoleAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from AnoleAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "AnoleModel is using AnoleSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
ANOLE_ATTENTION_CLASSES = {
    "eager": AnoleAttention,
    "flash_attention_2": AnoleFlashAttention2,
    "sdpa": AnoleSdpaAttention,
}


# copied from transformers.models.llama.modeling_llama.LlamaDecoderLayer with Llama->Anole, LLAMA->ANOLE
# TODO(joao): add me back asap :)
class AnoleDecoderLayer(nn.Module):
    def __init__(self, config: AnoleConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = ANOLE_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = AnoleMLP(config)
        self.input_layernorm = AnoleRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = AnoleRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class AnoleSwinDecoderLayer(nn.Module):
    def __init__(self, config: AnoleConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
ANOLE_INPUTS_DOCSTRING = r"""
    Args:
        ...
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, image_size, image_size)):
            ...
            [`AutoImageProcessor`]. See [`AnoleImageProcessor.__call__`] for details.
...
class AnoleModel(AnolePreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`AnoleDecoderLayer`]
