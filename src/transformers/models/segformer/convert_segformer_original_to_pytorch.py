...
def read_in_k_v(state_dict, config):
    ...
    for i in range(config.num_encoder_blocks):
        for j in range(config.depths[i]):
            ...
            state_dict[f"segformer.encoder.block.{i}.{j}.attention.self.key.weight"] = (
                kv_weight[: config.hidden_sizes[i], :]
            )
            state_dict[f"segformer.encoder.block.{i}.{j}.attention.self.key.bias"] = (
                kv_bias[: config.hidden_sizes[i]]
            )
            state_dict[
                f"segformer.encoder.block.{i}.{j}.attention.self.value.weight"
            ] = kv_weight[config.hidden_sizes[i] :, :]
            state_dict[f"segformer.encoder.block.{i}.{j}.attention.self.value.bias"] = (
                kv_bias[config.hidden_sizes[i] :]
            )
