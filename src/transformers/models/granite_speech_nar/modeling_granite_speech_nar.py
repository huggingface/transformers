...
"""Model classes for Granite Speech NAR (Non-Autoregressive ASR)."""
...
class GraniteSpeechNarBidirectionalGraniteModel(GraniteModel):
    ...
    Replaces create_causal_mask() with create_bidirectional_mask() so all
...
class GraniteSpeechNarForASR(GraniteSpeechNarPreTrainedModel):
    def __init__(self, config: GraniteSpeechNarConfig):
        ...
        self.language_model = GraniteSpeechNarLanguageModel(text_config)
        ...
        *,
        ...
        if labels is not None:
            ...
            if self.config.ce_loss_lambda > 0.0:
                ce_targets_list = [self._add_insertion_slots(ids) for ids in ctc_token_ids]
                ce_targets = torch.cat(ce_targets_list)
