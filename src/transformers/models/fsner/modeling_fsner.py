# coding=utf-8
# Copyright 2021 sayef The HuggingFace Inc. team. All rights reserved.
#
...
from .configuration_fsner import FSNERConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "sayef/fsner-bert-base-uncased"
_CONFIG_FOR_DOC = "FSNERConfig"
_TOKENIZER_FOR_DOC = "FSNERTokenizer"

FSNER_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "sayef/fsner-bert-base-uncased",
    # See all FSNER models at https://huggingface.co/models?filter=fsner
]


def load_tf_weights_in_fsner(model, config, tf_checkpoint_path):
...
class FSNEREmbeddings(nn.Module):
...
class FSNERSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in FSNERModel forward() function)
...
class FSNERSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FSNERAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = FSNERSelfAttention(config)
        self.output = FSNERSelfOutput(config)
...
class FSNERIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FSNERLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = FSNERAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = FSNERAttention(config)
        self.intermediate = FSNERIntermediate(config)
        self.output = FSNEROutput(config)
...
class FSNEREncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([FSNERLayer(config) for _ in range(config.num_hidden_layers)])
...
class FSNERPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class FSNERLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = FSNERPredictionHeadTransform(config)
...
class FSNEROnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = FSNERLMPredictionHead(config)
...
class FSNERPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = FSNERConfig
    load_tf_weights = load_tf_weights_in_fsner
    base_model_prefix = "fsner"
...
FSNER_START_DOCSTRING = r"""
    This model is a PyTorch `torch.nn.Module <https://pytorch.org/docs/stable/nn.html#torch.nn.Module>`_ sub-class. Use
    it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage and
    behavior.

    Parameters:
        config (:class:`~transformers.FSNERConfig`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""

FSNER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using :class:`transformers.FSNERTokenizer`. See
...
@add_start_docstrings(
    "The bare FSNER Model transformer outputting raw hidden-states without any specific head on top.",
    FSNER_START_DOCSTRING,
)
class FSNERModel(FSNERPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = FSNEREmbeddings(config)
        self.encoder = FSNEREncoder(config)
    ...
    @add_start_docstrings_to_model_forward(FSNER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
    ...
    def get_start_end_token_scores(self, W_query, W_supports):
        ...
        Args:
            ...
            W_supports (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
                Indices of support sequence tokens in the vocabulary.


        Returns:
            p_start (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length)`): Scores of each token as
            being start token of an entity

            p_end (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length)`): Scores of each token as
        ...
        Example::
            ...
            >>> output = tokenizer.decode_srart_end_token_scores(query, W_query, start_scores, end_scores)
            >>> print(output)
