...
class DeepseekOcr2VisionRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def compute_default_rope_parameters(config: DeepseekOcr2VisionConfig) -> tuple[torch.Tensor, float]:
        """
...
class DeepseekOcr2TextRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def compute_default_rope_parameters(config: DeepseekOcr2TextConfig) -> tuple[torch.Tensor, float]:
        """
        Computes the inverse frequencies according to the original RoPE implementation
        Args:
            config ([`~transformers.PreTrainedConfig`]):
