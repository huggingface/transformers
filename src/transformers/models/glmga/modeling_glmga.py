...
from .configuration_glmga import GlmgaConfig


@auto_docstring
class GlmgaPreTrainedModel(PreTrainedModel):
    config: GlmgaConfig
...
@dataclass
class GlmgaModelOutputWithPast(ModelOutput):
    r"""
...
@auto_docstring
class GlmgaModel(GlmgaPreTrainedModel):
    base_model_prefix = "model"
    ...
    def get_rope_index(
        ...
        **kwargs,
        ...
        - GLMGA uses timestamps to separate each video frame, so the video_grid_thw should also be split too.
    ...
    def forward(
        ...
        mm_token_type_ids: torch.IntTensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | GlmgaModelOutputWithPast:
        r"""
        image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
            The temporal, height and width of feature shape of each image in LLM.
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
        ...
        return GlmgaModelOutputWithPast(
            **outputs,
...
@auto_docstring(
    custom_intro="""
    Base class for Glmga causal language model (or autoregressive) outputs.
    """
)
@dataclass
class GlmgaCausalLMOutputWithPast(ModelOutput):
    r"""
...
class GlmgaForConditionalGeneration(GlmgaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | GlmgaCausalLMOutputWithPast:
        r"""
        ...
        >>> from transformers import AutoProcessor, GlmgaForConditionalGeneration

        >>> model = GlmgaForConditionalGeneration.from_pretrained("zai-org/GLM-4.1V-9B-Thinking")
        ...
        return GlmgaCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
__all__ = ["GlmgaModel", "GlmgaPreTrainedModel", "GlmgaForConditionalGeneration"]
