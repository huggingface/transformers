""" TransformerM model configuration"""
...
TRANSFORMERM_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    # TODO add config url
    "transformerm-base": "https://huggingface.co/tbd",
}


class TransformerMConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`~TransformerMModel`]. It is used to instantiate an
    TransformerM model according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the Transformer-M architecture.
    ...
    Args:
        ...
        Example:
            ```python
            >>> from transformers import TransformerMForGraphClassification, TransformerMConfig

            >>> # Initializing a TransformerM style configuration
            >>> configuration = TransformerMConfig()

            >>> # Initializing a model from the TransformerM style configuration
            >>> model = TransformerMForGraphClassification(configuration)

            >>> # Accessing the model configuration
            >>> configuration = model.config
            ```
    """
    model_type = "transformerm"
