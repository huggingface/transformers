...
class Florence2FlashAttention2(Florence2Attention):
    ...
    def forward(
        ...
        if output_attentions:
            raise ValueError(
                "Florence2SdpaAttention2 attention does not support `output_attentions`. "
...
class Florence2SdpaAttention(Florence2Attention):
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Florence2Model is using Florence2SdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` . Falling back to the manual attention"
...
class Florence2EncoderLayer(nn.Module):
    def __init__(self, config: Florence2Config, layer_idx: Optional[int] = None):
        super().__init__()
...
class Florence2ClassificationHead(nn.Module):
...
class PretrainedFlorence2Model(Florence2PreTrainedModel):
    def __init_subclass__(self):
        warnings.warn(
            "The class `PretrainedFlorence2Model` has been depreciated, please use `Florence2PreTrainedModel` instead.",
            FutureWarning,
        )


class Florence2PretrainedModel(Florence2PreTrainedModel):
    def __init_subclass__(self):
        warnings.warn(
            "The class `PretrainedFlorence2Model` has been depreciated, please use `Florence2PreTrainedModel` instead.",
            FutureWarning,
...
class Florence2Model(Florence2PreTrainedModel):
    ...
    def __init__(self, config: Florence2Config):
        ...
        embed_scale = math.sqrt(config.d_model) if config.scale_embedding else 1.0
        self.shared = Florence2ScaledWordEmbedding(vocab_size, config.d_model, padding_idx, embed_scale=embed_scale)
    ...
    def _tie_weights(self):
        if self.config.tie_word_embeddings:
            # Some model checkpoints like "facebook/florence2-large-cnn"'s embedding weight is in decoder.embed_tokens, need check here, see issue #36247
    ...
    def forward(
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            Florence2 uses the `eos_token_id` as the starting token for `decoder_input_ids` generation. If `past_key_values`
        ...
        decoder_attention_mask (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            If you want to change padding behavior, you should read [`modeling_florence2._prepare_decoder_attention_mask`]
...
@auto_docstring(
    custom_intro="""
    The FLORENCE2 Model with a language modeling head. Can be used for summarization.
    """
)
class Florence2ForConditionalGeneration(Florence2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, Florence2ForConditionalGeneration

        >>> model = Florence2ForConditionalGeneration.from_pretrained("facebook/florence2-large-cnn")
        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/florence2-large-cnn")
        ...
        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/florence2-base")
        >>> model = Florence2ForConditionalGeneration.from_pretrained("facebook/florence2-base")
...
@auto_docstring(
    custom_intro="""
    Florence2 model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """
)
class Florence2ForSequenceClassification(Florence2PreTrainedModel):
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]

    def __init__(self, config: Florence2Config, **kwargs):
        super().__init__(config, **kwargs)
        self.model = Florence2Model(config)
        self.classification_head = Florence2ClassificationHead(
            config.d_model,
            config.d_model,
            config.num_labels,
...
@auto_docstring
class Florence2ForQuestionAnswering(Florence2PreTrainedModel):
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
...
class Florence2DecoderWrapper(Florence2PreTrainedModel):
    """
...
@auto_docstring(
    custom_intro="""
    FLORENCE2 decoder with a language modeling head on top (linear layer with weights tied to the input embeddings).
    """
)
class Florence2ForCausalLM(Florence2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        super().__init__(config)
        self.model = Florence2DecoderWrapper(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, Florence2ForCausalLM

        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/florence2-base")
        >>> model = Florence2ForCausalLM.from_pretrained("facebook/florence2-base", add_cross_attention=False)
