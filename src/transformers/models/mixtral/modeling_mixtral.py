...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, specified_num_experts=0):
        ...
        # gating
        self.gate = nn.Linear(self.hidden_dim, self.real_num_experts, bias=False)

        self.experts = nn.ModuleList([MixtralBlockSparseTop2MLP(config) for _ in range(self.real_num_experts)])
