...
def write_model(model_path, input_base_path, model_size, tokenizer_path=None, safe_serialization=True):
    ...
    model = MixtralForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
...
def main():
    ...
    parser.add_argument(
        ...
        help="'f' models correspond to the finetuned versions, and are specific to the Mixtral2 official release. For more details on Mixtral2, checkout the original repo: https://huggingface.co/meta-mixtral",
    )
    parser.add_argument(
        "--output_dir",
