...
class LEDEncoderSelfAttention(nn.Module):
    ...
    def _chunk(hidden_states, window_overlap, onnx_export=False):
        ...
        if not onnx_export:
            ...
            if hidden_states.size(1) == window_overlap * 2:
                ...
                res = torch.empty(chunk_size)
                for chunk in range(chunk_size[1]):
                    res[:, chunk, :, :] = hidden_states[
    ...
    def _mask_invalid_locations(input_tensor, affected_seq_len) -> torch.Tensor:
        ...
        beginning_input = torch.full_like(beginning_input, -float("inf")).where(beginning_mask.bool(), beginning_input)
        ending_input = input_tensor[:, -affected_seq_len:, :, -(affected_seq_len + 1) :]
        ending_mask = ending_mask.expand(ending_input.size())
        ending_input = torch.full_like(ending_input, -float("inf")).where(ending_mask.bool(), ending_input)
    ...
    def _sliding_chunks_query_key_matmul(self, query: torch.Tensor, key: torch.Tensor, window_overlap: int):
        ...
        query = self._chunk(query, window_overlap, self.config.onnx_export)
        key = self._chunk(key, window_overlap, self.config.onnx_export)
