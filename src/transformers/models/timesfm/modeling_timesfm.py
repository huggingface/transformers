...
####################################################
# Copied from transformers.models.t5.modeling_t5.load_tf_weights_in_t5 with t5->timesfm
def load_tf_weights_in_timesfm(model, config, tf_checkpoint_path):
...
try:
    ...
    logger.warning("discovered apex but it failed to load, falling back to TimesFMLayerNorm")
    pass
...
# Copied from transformers.models.t5.modeling_t5.T5LayerSelfAttention with T5->TimesFM
class TimesFMLayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = TimesFMAttention(config, has_relative_attention_bias=has_relative_attention_bias)
...
# Copied from transformers.models.t5.modeling_t5.T5LayerCrossAttention with T5->TimesFM
class TimesFMLayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = TimesFMAttention(config, has_relative_attention_bias=False)
        self.layer_norm = TimesFMLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
# Copied from transformers.models.t5.modeling_t5.T5Block with T5->TimesFM
class TimesFMBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.layer = nn.ModuleList()
        self.layer.append(TimesFMLayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
            self.layer.append(TimesFMLayerCrossAttention(config))

        self.layer.append(TimesFMLayerFF(config))
...
# Copied from transformers.models.t5.modeling_t5.T5ClassificationHead with T5->TimesFM
class TimesFMClassificationHead(nn.Module):
...
# Copied from transformers.models.t5.modeling_t5.T5PreTrainedModel with T5->TimesFM,t5->timesfm
class TimesFMPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_timesfm
    base_model_prefix = "transformer"
    is_parallelizable = True
    supports_gradient_checkpointing = True
    _no_split_modules = ["TimesFMBlock"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, TimesFMLayerNorm):
            module.weight.data.fill_(factor * 1.0)
        elif isinstance(
            module,
            (TimesFMModel, TimesFMForConditionalGeneration, TimesFMEncoderModel, TimesFMForQuestionAnswering),
        ):
        ...
        elif isinstance(module, TimesFMForTokenClassification):
            if hasattr(module, "classifier"):
                module.classifier.weight.data.normal_(mean=0.0, std=factor * 1.0)
                module.classifier.bias.data.zero_()
        elif isinstance(module, TimesFMClassificationHead):
        ...
        elif isinstance(module, TimesFMDenseActDense):
    ...
    def _shift_right(self, input_ids):
        ...
        if decoder_start_token_id is None:
            raise ValueError(
                "self.model.config.decoder_start_token_id has to be defined. In TimesFM it is usually set to the pad_token_id. "
                "See TimesFM docs for more information."
            )
...
TIMESFM_ENCODER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. TIMESFM is a model with relative position embeddings so you
            should be able to pad the inputs on both the right and the left.
...
@add_start_docstrings(
    "The bare TIMESFM Model transformer outputting raw hidden-states without any specific head on top.",
...
class TimesFMModel(TimesFMPreTrainedModel):
    ...
    def __init__(self, config: TimesFMConfig):
        ...
        self.encoder = TimesFMStack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = TimesFMStack(decoder_config, self.shared)
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            "`TimesFMModel.parallelize` is deprecated and will be removed in v5 of Transformers, you should load your model"
            " with `device_map='balanced'` in the call to `from_pretrained`. You can also provide your own"
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TimesFMModel

        >>> tokenizer = AutoTokenizer.from_pretrained("google/timesfm-1.0-200m")
        >>> model = TimesFMModel.from_pretrained("google/timesfm-1.0-200m")
        ...
        >>> # preprocess: Prepend decoder_input_ids with start token which is pad token for TimesFMModel.
        >>> # This is not needed for torch's TimesFMForConditionalGeneration as it does this internally using labels arg.
...
@add_start_docstrings("""TIMESFM Model with a `language modeling` head on top.""", TIMESFM_START_DOCSTRING)
class TimesFMForConditionalGeneration(TimesFMPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [
        "decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight",
    ]
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            "`TimesFMForConditionalGeneration.parallelize` is deprecated and will be removed in v5 of Transformers, you"
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TimesFMForConditionalGeneration

        >>> tokenizer = AutoTokenizer.from_pretrained("google/timesfm-1.0-200m")
        >>> model = TimesFMForConditionalGeneration.from_pretrained("google/timesfm-1.0-200m")
...
@add_start_docstrings(
    "The bare TIMESFM Model transformer outputting encoder's raw hidden-states without any specific head on top.",
    TIMESFM_START_DOCSTRING,
)
class TimesFMEncoderModel(TimesFMPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, TimesFMEncoderModel

        >>> tokenizer = AutoTokenizer.from_pretrained("google/timesfm-1.0-200m")
        >>> model = TimesFMEncoderModel.from_pretrained("google/timesfm-1.0-200m")
        >>> input_ids = tokenizer(
        ...     "Studies have been shown that owning a dog is good for you", return_tensors="pt"
        ... ).input_ids  # Batch size 1
...
@add_start_docstrings(
    """
    TIMESFM model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    TIMESFM_START_DOCSTRING,
)
class TimesFMForSequenceClassification(TimesFMPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = ["decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight"]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]

    def __init__(self, config: TimesFMConfig):
        super().__init__(config)
        self.transformer = TimesFMModel(config)
        self.classification_head = TimesFMClassificationHead(config)
...
@add_start_docstrings(
    """
    TIMESFM Encoder Model with a token classification head on top (a linear layer on top of the hidden-states output)
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    TIMESFM_START_DOCSTRING,
)
class TimesFMForTokenClassification(TimesFMPreTrainedModel):
    ...
    def __init__(self, config: TimesFMConfig):
        ...
        self.transformer = TimesFMEncoderModel(config)
...
@add_start_docstrings(
    """
    TIMESFM Model with a span classification head on top for extractive question-answering tasks like SQuAD (linear layers
    on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    TIMESFM_START_DOCSTRING,
)
class TimesFMForQuestionAnswering(TimesFMPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = ["decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight"]
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
    ...
    def forward(
        ...
        #   different to other models, TIMESFM automatically creates decoder_input_ids from
