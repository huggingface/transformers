...
"""
Convert TimesFMX checkpoint to PyTorch

Steps:
- Install gsutil according to https://cloud.google.com/storage/docs/gsutil_install
- Get a TimesFMX checkpoint at https://github.com/google-research/timesfmx/blob/main/docs/models.md#timesfm-11-checkpoints Example:
    `gsutil -m cp -r gs://timesfm-data/pretrained_models/timesfmx/timesfm_1_1_small $HOME/`
- Create or download a corresponding config for the downloaded model. E.g. for TimesFM v1.1 small, you can use
    https://huggingface.co/google/timesfm-v1_1-small/blob/main/config.json
- Convert:
    ```
    python3 convert_timesfmx_checkpoint_to_pytorch.py --timesfmx_checkpoint_path=$HOME/timesfm_1_1_small --config_file=config.json\
      --pytorch_dump_path=$HOME/timesfm_1_1_small_pt
    ```
"""
...
def timesfmx_attention_lookup(params, i, prefix, layer_name="attention"):
...
def timesfmx_mlp_lookup(params, i, prefix, split_mlp_wi=False):
...
def timesfmx_layer_norm_lookup(params, i, prefix, layer_name):
...
def convert_timesfmx_to_pytorch(variables: dict, *, num_layers: int, num_decoder_layers: int, is_encoder_only: bool):
    ...
    for i in range(num_layers):
        ...
        # Block i, layer 1 (MLP).
        layer_norm = timesfmx_layer_norm_lookup(old, i, "encoder", "pre_mlp_layer_norm")
        wi, wo = timesfmx_mlp_lookup(old, i, "encoder", split_mlp_wi)
...
def convert_timesfmx_checkpoint_to_pytorch(
    ...
    # Non-v1.1 checkpoints could also use TimesFMModel, but this works for all.
    # The v1.0 checkpoints will simply have an LM head that is the word embeddings.
    if is_encoder_only:
        model = TimesFMEncoderModel(config)
    else:
