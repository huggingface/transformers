...
def format_output(data):
    keypoints0 = data["mkpts0_f"]
    keypoints1 = data["mkpts1_f"]
    scores = data["mconf"]
    batch_idx = data["m_bids"]
...
class CoarseMatching(nn.Module):
    ...
    def get_coarse_match(self, conf_matrix, data):
        ...
        if self.training:
            ...
            assert self.train_pad_num_gt_min < num_matches_train, (
...
class FinePreprocess(nn.Module):
    ...
    def forward(self, feat_c0, feat_c1, data):
        ...
        if data["hw0_i"] == data["hw1_i"]:
            feat_c = rearrange(
                torch.cat([feat_c0, feat_c1], 0), "b (h w) c -> b c h w", h=data["hw0_c"][0]
            )  # 1/8 feat
