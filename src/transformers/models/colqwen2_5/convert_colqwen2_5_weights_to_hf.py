...
"""
Convert ColQwen2.5 weights from the original repository to the HF model format.

Don't forget to manually upload the processor-related files to the HF model repository
after running this script.

Original repository: https://github.com/illuin-tech/colqwen2_5.

NOTE: This script was originally run using `torch==2.5.1` and with:

```bash
python src/transformers/models/colqwen2_5/convert_colqwen2_5_weights_to_hf.py \
    --model_id vidore/colqwen2_5-v1.0-merged \
    --revision eeccbae1d44bdcb0c83b1788127a2b2cad7d718e \
    --original_vlm_name_or_path Qwen/Qwen2_5-VL-2B-Instruct \
    --output_dir vidore/colqwen2_5-v1.0-hf-internal \
    --push_to_hub
```
"""
...
from transformers.models.colqwen2_5 import ColQwen2_5ForRetrieval
from transformers.models.colqwen2_5.configuration_colqwen2_5 import ColQwen2_5Config
...
def rename_state_dict_keys(state_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in state_dict.items():
        if key.startswith("custom_text_proj"):
            ...
            # The original ColQwen2.5 inherits from Qwen2_5VL, so we simply need to add the `vlm.` prefix
...
@torch.no_grad()
def convert_colqwen2_5_weights_to_hf(
    model_id: str,
    output_dir: str,
    push_to_hub: bool,
    ...
    config.model_type = "colqwen2_5"
    config.is_composition = False

    # Load the untrained model
    model = ColQwen2_5ForRetrieval(config=config).to("cpu").eval()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""
        This script converts the original ColQwen2.5 model to the HF model format.
    ...
    convert_colqwen2_5_weights_to_hf(
