...
class XLMRobertaSdpaSelfAttention(XLMRobertaSelfAttention):
    ...
    def forward(
        ...
        if self.position_embedding_type != "absolute" or output_attentions or head_mask is not None:
            ...
            logger.warning_once(
                "XLMXLMRobertaSdpaSelfAttention is used but `torch.nn.functional.scaled_dot_product_attention` does not support "
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            value_layer = self.transpose_for_scores(self.k_proj(current_states))
            value_layer = self.transpose_for_scores(self.v_proj(current_states))
