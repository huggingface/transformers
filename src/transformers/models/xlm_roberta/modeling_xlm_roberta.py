...
class XLMRobertaAttention(nn.Module):
    ...
    XLM_ROBERTA historically split this over `XLMRobertaSelfAttention` (q/k/v) and `XLMRobertaSelfOutput` (the output
    projection plus the residual LayerNorm). The projection lives here as `o_proj`; the LayerNorm
    and residual belong to `XLMRobertaLayer`, which is what makes the layer's topology visible.
    """
...
class XLMRobertaCrossAttention(nn.Module):
    """Cross-attention query/key/value. Its output projection is `XLMRobertaSelfOutput`, see above."""
...
class XLMRobertaSelfOutput(nn.Module):
    ...
    The self-attention path no longer uses this: its projection is `XLMRobertaAttention.o_proj` and its
    LayerNorm belongs to `XLMRobertaLayer`. Cross-attention keeps the original layout deliberately, so
...
class XLMRobertaCrossAttentionBlock(nn.Module):
    """Cross-attention plus its output projection, in XLM_ROBERTA's original module layout."""

    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.self = XLMRobertaCrossAttention(config, is_causal=False, layer_idx=layer_idx)
...
class XLMRobertaMLP(nn.Module):
    """Ungated feed-forward network.

    Replaces `XLMRobertaIntermediate` (up projection plus activation) and `XLMRobertaOutput` (down projection
    plus the residual LayerNorm); the LayerNorm and residual now belong to `XLMRobertaLayer`.
    """
...
class XLMRobertaLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.self_attn = XLMRobertaAttention(config, is_causal=config.is_decoder, layer_idx=layer_idx)
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = XLMRobertaCrossAttentionBlock(config, layer_idx=layer_idx)
        self.mlp = XLMRobertaMLP(config)
...
class XLMRobertaPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        ...
        "attentions": XLMRobertaAttention,
