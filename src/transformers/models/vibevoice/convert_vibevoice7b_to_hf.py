...
def update_state_dict_for_hf_model(state_dict):
    ...
    for key, value in state_dict.items():
        ...
        # Handle acoustic tokenizer encoder transformations
        ...
        # Handle acoustic tokenizer decoder transformations
...
def convert_checkpoint(checkpoint, output_dir, config_path, push_to_hub, bfloat16, processor_config=None, push_tokenizers=False):
    ...
    if len(unexpected) != 0:
        raise ValueError(f"Unexpected {len(unexpected)} keys: {unexpected}")
    if len(missing) != 0:
        raise ValueError(f"missing {len(missing)} keys found: {missing}")
    # -- push to hub
    ...
    # Default to 7B model: https://huggingface.co/vibevoice/VibeVoice-7B/blob/main/preprocessor_config.json
...
{%- endfor %}
 Speech output:{{ "\n" }}{{ speech_start_token }}"""
    ...
    # -- load into HF model
    # Handle lm_head weights - check if they exist separately in the original checkpoint
    lm_head_key = "lm_head.weight"
    model_lm_head_key = "model.lm_head.weight"  # 7B model might have "model." prefix
    
    if lm_head_key in original_state_dict:
        updated_state_dict["lm_head.weight"] = original_state_dict[lm_head_key]
        print("Found separate lm_head weights in checkpoint")
    elif model_lm_head_key in original_state_dict:
        updated_state_dict["lm_head.weight"] = original_state_dict[model_lm_head_key] 
        print("Found separate lm_head weights with model. prefix in checkpoint")
    else:
        # Fallback: copy from embed_tokens (for tied embeddings)
        updated_state_dict["lm_head.weight"] = updated_state_dict["language_model.embed_tokens.weight"]
        print("No separate lm_head found, copying from embed_tokens")
...
# -- run conversion
python src/transformers/models/vibevoice/convert_vibevoice7b_to_hf.py \
...
- bezzam/VibeVoice-7B
- bezzam/VibeVoice-7B-AcousticTokenizer
- bezzam/VibeVoice-7B-SemanticTokenizer
