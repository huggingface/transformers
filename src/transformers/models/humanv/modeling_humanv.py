...
class HumanVRotaryEmbedding(nn.Module):
    def __init__(self, config: HumanVConfig, device=None):
        ...
        inv_freq = 1.0 / (
            base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).to(device=device, dtype=torch.float32) / self.dim)
        )
...
class HumanVAttention(nn.Module):
    def __init__(self, config: HumanVConfig, layer_idx: int, layer_type: str):
        ...
        self.k_proj = nn.Linear(
            config.hidden_size, self.num_kv_heads * self.head_dim, bias=getattr(config, "attention_bias", False)
        )
    ...
    def _get_intra_causal(self, block_size: int, device: torch.device) -> torch.Tensor:
        ...
        m = torch.triu(torch.ones((block_size, block_size), device=device, dtype=torch.bool), diagonal=1)
    ...
    def _select_key_blocks(self, q_block_idx: int, n_blocks: int) -> torch.Tensor:
        ...
        stride = self.sparse_global_block_stride

        start = max(0, q_block_idx - local_nb + 1)
        local = list(range(start, q_block_idx + 1))

        global_candidates = list(range(0, q_block_idx + 1, max(1, stride)))
        if len(global_candidates) > 0 and g_nb is not None and g_nb > 0 and len(global_candidates) > g_nb:
            global_candidates = global_candidates[-g_nb:]

        global_list = [0] + global_candidates if 0 not in global_candidates else global_candidates

        idx = sorted(set(local + global_list))
    ...
    def _local_global_block_sparse_prefill(
        ...
        intra_causal = self._get_intra_causal(b, q.device)
        ...
        for chunk_start in range(0, n_blocks, chunk):
            ...
            for bi in range(chunk_start, chunk_end):
                ...
                same_block_pos = (idx == bi).nonzero(as_tuple=False)
                if same_block_pos.numel() > 0:
                    p = int(same_block_pos[0].item())
                    start = p * b
                    end = start + b
                    scores[:, :, :, start:end] = scores[:, :, :, start:end].masked_fill(
                        intra_causal[None, None, :, :], -1e9
                    )

                if self.sparse_attention_window > 0:
                    max_ctx = self.sparse_attention_window
                    q_tok_base = bi * b
                    key_positions = torch.arange(idx.numel() * b, device=q.device, dtype=torch.long)
                    abs_key_pos = idx.repeat_interleave(b) * b + (key_positions % b)
                    abs_q_pos = q_tok_base + torch.arange(b, device=q.device, dtype=torch.long)
                    min_allowed = (abs_q_pos[:, None] - (max_ctx - 1)).clamp(min=0)
                    window_mask = abs_key_pos[None, :] < min_allowed
                    scores = scores.masked_fill(window_mask[None, None, :, :], -1e9)

                probs = torch.nn.functional.softmax(scores, dim=-1)
                probs = torch.nn.functional.dropout(probs, p=self.attention_dropout, training=self.training)
                o = torch.matmul(probs.to(vb_flat.dtype), vb_flat)
                o_chunk[:, :, bi - chunk_start] = o.to(dtype=q.dtype)

            out_blocks.append(o_chunk)

        out = torch.cat(out_blocks, dim=2).reshape(bsz, n_heads, n_blocks * b, d)
        out = out[:, :, :seqlen, :]
        return out
    ...
    def _local_global_block_sparse_decode_one(
        ...
        idx = self._select_key_blocks(q_block_idx, n_blocks).to(device=q.device)
        ...
        same_block_pos = (idx == q_block_idx).nonzero(as_tuple=False)
        if same_block_pos.numel() > 0:
            p = int(same_block_pos[0].item())
            start = p * b
            end = start + b
            if q_intra < b - 1:
                mask = torch.arange(b, device=q.device, dtype=torch.long) > q_intra
                scores[:, :, 0, start:end] = scores[:, :, 0, start:end].masked_fill(mask[None, None, :], -1e9)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if use_sparse:
            if q_len == k_len:
                attn_output = self._local_global_block_sparse_prefill(
                    query_states, key_states, value_states, attention_mask_2d
                )
            elif q_len == 1:
                attn_output = self._local_global_block_sparse_decode_one(
                    query_states, key_states, value_states, attention_mask_2d
                )
            else:
                attn_output = self._dense_attention(query_states, key_states, value_states, attention_mask_4d)
        else:
            attn_output = self._dense_attention(query_states, key_states, value_states, attention_mask_4d)
...
class HumanVDecoderLayer(nn.Module):
    def __init__(self, config: HumanVConfig, layer_idx: int):
        ...
        if layer_types is None:
            use_sliding = bool(getattr(config, "use_sliding_window", False))
            layer_type = "sliding_attention" if use_sliding else "full_attention"
        else:
        ...
        **kwargs,
        ...
        if attention_mask is None:
            attention_mask_2d = torch.ones(
                (bsz, past_length + seq_length), device=inputs_embeds.device, dtype=torch.float32
            )
        else:
        ...
        position_ids = torch.arange(past_length, past_length + seq_length, device=inputs_embeds.device, dtype=torch.long)
