...
class Speech2TextDecoderLayerStableLayerNorm(nn.Module):
    def __init__(self, config: Speech2TextConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class Speech2TextDecoderStableLayerNorm(Speech2TextPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`Speech2TextDecoderLayer`
    ...
    def __init__(self, config: Speech2TextConfig):
        ...
        self.layers = nn.ModuleList(
            [Speech2TextDecoderLayerStableLayerNorm(config) for _ in range(config.decoder_layers)]
        )
...
class Speech2TextModel(Speech2TextPreTrainedModel):
    def __init__(self, config: Speech2TextConfig):
        ...
        if self.config.do_stable_layer_norm:
            self.decoder = Speech2TextDecoderStableLayerNorm(config)
        else:
...
class Speech2TextDecoderWrapper(Speech2TextPreTrainedModel):
    """
...
class Speech2TextForCausalLM(Speech2TextPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        self.model = Speech2TextDecoderWrapper(config)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import Speech2TextTokenizer, Speech2TextForCausalLM

            >>> tokenizer = Speech2TextTokenizer.from_pretrained('facebook/bart-large')
            >>> model = Speech2TextForCausalLM.from_pretrained('facebook/bart-large', add_cross_attention=False)
