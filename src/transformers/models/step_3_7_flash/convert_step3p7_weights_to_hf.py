...
Weight conversion is handled automatically via the ``step3p7`` entry in
``src/transformers/conversion_mapping.py``.  The transformations applied are:
...
def resolve_input_dir(input_dir: str, cache_dir: str | None = None) -> str:
    """If input_dir is a Hub repo id, download the snapshot and return its local path."""
...
def _build_config_from_dict(cfg_dict: dict) -> Step3p7Config:
    ...
    # Trim per-layer lists extended by num_nextn_predict_layers (3 extra prediction heads).
...
def load_config(input_dir: str, config_path: str | None = None) -> Step3p7Config:
    """Load Step3p7Config from input_dir/config.json or an explicit config_path."""
    path = config_path or os.path.join(input_dir, "config.json")
    if not os.path.isfile(path):
        raise FileNotFoundError(f"No config.json found at {path!r}. Pass --config /path/to/config.json explicitly.")
...
def convert_checkpoint(
    ...
    print(f"Loading and converting weights from {input_dir} ...")
    model = Step3p7ForConditionalGeneration.from_pretrained(input_dir, config=config)
...
def convert_processor(
    ...
    try:
        ...
        print("Step3p7Processor not found, falling back to AutoTokenizer.")
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        default="./mini-step-3-7-flash-hf",
        help="Directory to write the converted HuggingFace checkpoint. Defaults to ./mini-step-3-7-flash-hf.",
    )
    parser.add_argument(
        "--push_to_hub",
        action="store_true",
