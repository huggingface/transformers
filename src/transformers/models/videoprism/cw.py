...
from transformers import VideoPrismModel, VideoPrismClip
...
def  transform_state(state, checkpoint_info):
    ...
    if checkpoint_info['model_type'] == 'backbone':
        ...
        #? attention LN
        new_state[f'{pooler_layer}.layernorm.weight'] = state[f'{pooler_prefix}/pooling_attention_layer_norm/scale']  #? [768]
        new_state[f'{pooler_layer}.layernorm.bias'] = state[f'{pooler_prefix}/pooling_attention_layer_norm/bias']  #? [768]
        #? attention
        new_state[f'{pooler_layer}.pooling_attention_query'] = state[f'{pooler_prefix}/pooling_attention_query'].reshape(1,1,-1)
        new_state[f'{pooler_layer}.per_dim_scale.per_dim_scale'] = state[f'{pooler_prefix}/pooling_attention/per_dim_scale/per_dim_scale']
        new_state[f'{pooler_layer}.query.weight'] = state[f'{pooler_prefix}/pooling_attention/query/w'].reshape(hidden_size, -1).T #? [768, 12, 64] -> [768, 768]
        new_state[f'{pooler_layer}.query.bias'] = state[f'{pooler_prefix}/pooling_attention/query/b'].reshape(-1)
        new_state[f'{pooler_layer}.key.weight'] = state[f'{pooler_prefix}/pooling_attention/key/w'].reshape(hidden_size, -1).T #? [768, 12, 64] -> [768, 768]
        new_state[f'{pooler_layer}.key.bias'] = state[f'{pooler_prefix}/pooling_attention/key/b'].reshape(-1)        
        new_state[f'{pooler_layer}.value.weight'] = state[f'{pooler_prefix}/pooling_attention/value/w'].reshape(hidden_size, -1).T #? [768, 12, 64] -> [768, 768]
        new_state[f'{pooler_layer}.value.bias'] = state[f'{pooler_prefix}/pooling_attention/value/b'].reshape(-1)       
        new_state[f'{pooler_layer}.projection.weight'] = state[f'{pooler_prefix}/pooling_attention/post/w'].reshape(hidden_size, -1) #? [768, 12, 64] -> [768, 768]
        ...
        new_state[f"{unimodal_layer}.token_embeddings.weight"] = state[f"{unimodal_prefix}/token_emb/emb_var"] #? (32000, 768)
...
if __name__ == "__main__":
    convert(model_type='lvt', model_size='base', convert=False, upload=False, load_model=True, load_video=True, inference=True)
