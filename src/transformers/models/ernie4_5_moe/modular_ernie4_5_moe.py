...
class Ernie4_5_MoeSparseMoeBlock(DeepseekV3MoE):
    ...
    def forward(
        ...
        with torch.autocast(device_type=device_type, enabled=False):  # Force float32
            ...
            # Get routing weights from logits
            routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)

            # Get top-k expert indices from the corrected weights.
            # Note: The correction bias only influences *selection*, not the final weights.
            _, topk_indices = torch.topk(self.moe_statics(routing_weights), self.top_k, dim=-1)

            # Gather the original (uncorrected) routing weights for the selected experts
