...
from .configuration_sarvam import SarvamMLAConfig
...
class SarvamMLAYarnRotaryEmbedding(SarvamMLARotaryEmbedding):
    ...
    def _yarn_linear_ramp_mask(self, min_val, max_val, dim):
        if min_val == max_val:
            max_val += 0.001
...
class SarvamMLAAttention(nn.Module):
    ...
    def __init__(self, config: SarvamMLAConfig, layer_idx: int | None = None):
        super().__init__()
        self.config = config
        ...
        if self.config.rope_scaling is not None:
            ...
            if mscale_all_dim:
                mscale = self._yarn_get_mscale(scaling_factor, mscale_all_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            kv_seq_len += self._get_usable_past_kv_length(past_key_value, kv_seq_len, self.layer_idx)
...
class SarvamMLAModel(SarvamMLAPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache:
            ...
            past_key_values_length = self._get_usable_past_kv_length(past_key_values, seq_length)
...
class SarvamMLAForCausalLM(SarvamMLAPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        >>> model = SarvamMLAForCausalLM.from_pretrained("meta-sarvam_m_l_a/SarvamMLA-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-sarvam_m_l_a/SarvamMLA-2-7b-hf")
...
__all__ = ["SarvamMLAModel", "SarvamMLAForCausalLM", "SarvamMLAPreTrainedModel"]
