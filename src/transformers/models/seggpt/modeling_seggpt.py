...
class SegGptModel(SegGptPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=SegGptEncoderOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        pixel_values: torch.Tensor,
        ...
        if bool_masked_pos is None:
            ...
            bool_masked_pos = torch.zeros(num_patches, dtype=torch.bool).to(
                pixel_values.device
            )
...
class SegGptForImageSegmentation(SegGptPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(SEGGPT_INPUTS_DOCSTRING)
    @replace_return_docstrings(
        output_type=SegGptImageSegmentationOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        pixel_values: torch.Tensor,
