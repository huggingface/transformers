...
class ChineseCLIPTextEmbeddings(nn.Module):
    def __init__(self, config: ChineseCLIPTextConfig):
        super().__init__()
...
class ChineseCLIPAttention(nn.Module):
...
class ChineseCLIPMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class ChineseCLIPEncoderLayer(nn.Module):
    def __init__(self, config: ChineseCLIPConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = ChineseCLIPAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim)
        self.mlp = ChineseCLIPMLP(config)
...
class ChineseCLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ChineseCLIPTextEmbeddings):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ChineseCLIPEncoder):
...
class ChineseCLIPEncoder(nn.Module):
    """
    Transformer encoder consisting of `config.num_hidden_layers` self attention layers. Each layer is a
    [`ChineseCLIPEncoderLayer`].
    ...
    def __init__(self, config: ChineseCLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([ChineseCLIPEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class ChineseCLIPTextTransformer(nn.Module):
    def __init__(self, config: ChineseCLIPTextConfig):
        ...
        self.encoder = ChineseCLIPEncoder(config)
    ...
    def forward(
        ...
        # CHINESE_CLIP's text model uses causal mask, prepare it here.
        # https://github.com/openai/CHINESE_CLIP/blob/cfcffb90e69f37bf2ff1e988237a0fbe41f33c04/chinese_clip/model.py#L324
...
class ChineseCLIPTextModel(ChineseCLIPPreTrainedModel):
    ...
    def __init__(self, config: ChineseCLIPTextConfig):
        ...
        self.text_model = ChineseCLIPTextTransformer(config)
    ...
    def forward(
        ...
        >>> tokenizer = CLIPTokenizer.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16")
...
class ChineseCLIPModel(ChineseCLIPPreTrainedModel):
    ...
    def __init__(self, config: ChineseCLIPConfig):
        ...
        self.text_model = ChineseCLIPTextTransformer(text_config)
    ...
    def get_text_features(
        ...
        ```python
        >>> from transformers import CLIPTokenizer, ChineseCLIPModel
    ...
    def get_image_features(
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import CLIPProcessor, ChineseCLIPModel
