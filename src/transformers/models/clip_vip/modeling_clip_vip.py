...
class CLIPViPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, CLIPViPVisionModelWithProjection):
            nn.init.normal_(
                module.visual_projection.weight,
                std=self.config.hidden_size**-0.5 * self.config.initializer_factor,
            )
        elif isinstance(module, CLIPViPTextModelWithProjection):
            nn.init.normal_(
                module.text_projection.weight,
                std=self.config.hidden_size**-0.5 * self.config.initializer_factor,
            )
...
@add_start_docstrings(
    """
    CLIP_VIP Text Model with a projection layer on top (a linear layer on top of the pooled output).
    """,
    CLIP_VIP_START_DOCSTRING,
)
class CLIPViPTextModelWithProjection(CLIPViPPreTrainedModel):
    config_class = CLIPViPTextConfig
    ...
    @add_start_docstrings_to_model_forward(CLIP_VIP_TEXT_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=CLIPViPTextModelOutput, config_class=CLIPViPTextConfig)
    def forward(
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, CLIPViPTextModelOutput]:
        r"""
        Returns:
        ...
        ```python
        >>> from transformers import AutoTokenizer, CLIPViPTextModelWithProjection

        >>> model = CLIPViPTextModelWithProjection.from_pretrained("tensorpro/clip_vip")
        >>> tokenizer = AutoTokenizer.from_pretrained("tensorpro/clip_vip")
...
@add_start_docstrings(
    """
    CLIP_VIP Vision Model with a projection layer on top (a linear layer on top of the pooled output).
    """,
    CLIP_VIP_START_DOCSTRING,
)
class CLIPViPVisionModelWithProjection(CLIPViPPreTrainedModel):
    config_class = CLIPViPVisionConfig
    main_input_name = "pixel_values"
    ...
    @add_start_docstrings_to_model_forward(CLIP_VIP_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=CLIPViPVisionModelOutput, config_class=CLIPViPVisionConfig)
    def forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, CLIPViPVisionModelOutput]:
        r"""
        Returns:
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import AutoProcessor, CLIPViPVisionModelWithProjection

        >>> model = CLIPViPVisionModelWithProjection.from_pretrained("tensorpro/clip_vip")
        >>> processor = AutoProcessor.from_pretrained("tensorpro/clip_vip")
        ...
        return CLIPViPVisionModelOutput(
            image_embeds=image_embeds,
