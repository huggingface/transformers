...
@dataclass
class TmlTextModelOutputWithPast(BaseModelOutputWithPast):
    """
...
@auto_docstring
@dataclass
class TmlAudioModelOutput(BaseModelOutputWithPooling):
    r"""
...
class TmlClippableLinear(nn.Module):
    def __init__(
        self,
        config: TmlVisionConfig | TmlAudioConfig,
        in_features: int,
        out_features: int,
    ) -> None:
        super().__init__()
...
class TmlAudioRelPositionalEncoding(nn.Module):
    ...
    concatenated [sin..., cos...] layout matching the original Tml convention.
    """
...
class TmlAudioAttention(nn.Module):
    """Chunked local attention with relative position bias"""

    def __init__(self, config: TmlAudioConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_proj = TmlClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.k_proj = TmlClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.v_proj = TmlClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.post = TmlClippableLinear(config, config.hidden_size, config.hidden_size)
...
class TmlAudioSubSampleConvProjectionLayer(nn.Module):
...
class TmlAudioSubSampleConvProjection(nn.Module):
    def __init__(self, config: TmlAudioConfig):
        super().__init__()
        self.layer0 = TmlAudioSubSampleConvProjectionLayer(
            in_channels=1,
            out_channels=config.subsampling_conv_channels[0],
            norm_eps=config.rms_norm_eps,
        )
        self.layer1 = TmlAudioSubSampleConvProjectionLayer(
...
class TmlAudioFeedForward(nn.Module):
    def __init__(self, config: TmlAudioConfig):
        super().__init__()
        self.config = config

        self.ffw_layer_1 = TmlClippableLinear(config, config.hidden_size, config.hidden_size * 4)
        self.ffw_layer_2 = TmlClippableLinear(config, config.hidden_size * 4, config.hidden_size)

        self.pre_layer_norm = TmlRMSNorm(config.hidden_size)
        self.post_layer_norm = TmlRMSNorm(config.hidden_size)
...
class TmlAudioCausalConv1d(nn.Conv1d):
    # def __init__(
    #     self,
    #     in_channels: int,
    #     out_channels: int,
...
class TmlAudioLightConv1d(nn.Module):
    def __init__(self, config: TmlAudioConfig):
        super().__init__()
        self.config = config

        self.linear_start = TmlClippableLinear(config, config.hidden_size, config.hidden_size * 2)
        self.linear_end = TmlClippableLinear(config, config.hidden_size, config.hidden_size)
        self.depthwise_conv1d = TmlAudioCausalConv1d(
        ...
        self.pre_layer_norm = TmlRMSNorm(config.hidden_size, eps=config.rms_norm_eps, with_scale=True)
        self.conv_norm = TmlRMSNorm(config.hidden_size, eps=config.rms_norm_eps, with_scale=True)
        self.act_fn = ACT2FN[config.hidden_act]
...
class TmlAudioLayer(nn.Module):
    def __init__(self, config: TmlAudioConfig, layer_idx: int):
        super().__init__()
        self.config = config

        self.feed_forward1 = TmlAudioFeedForward(config)
        self.feed_forward2 = TmlAudioFeedForward(config)
        self.self_attn = TmlAudioAttention(config, layer_idx)
        self.lconv1d = TmlAudioLightConv1d(config)

        self.norm_pre_attn = TmlRMSNorm(config.hidden_size)
        self.norm_post_attn = TmlRMSNorm(config.hidden_size)
        self.norm_out = TmlRMSNorm(config.hidden_size)
...
class TmlVisionPatchEmbedder(nn.Module):
    ...
    def forward(
        ...
        # Tml applies no normalization and instead scales in model code
...
class TmlVisionPooler(nn.Module):
    ...
    (``TmlVisionModel.forward``) standardizes them and casts back to the working dtype.
    """
...
class TmlVisionMLP(nn.Module):
    def __init__(self, config: TmlVisionConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.gate_proj = TmlClippableLinear(config, self.hidden_size, self.intermediate_size)
        self.up_proj = TmlClippableLinear(config, self.hidden_size, self.intermediate_size)
        self.down_proj = TmlClippableLinear(config, self.intermediate_size, self.hidden_size)
...
class TmlVisionRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: TmlVisionConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: TmlVisionConfig | None = None,
...
class TmlVisionAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: TmlVisionConfig, layer_idx: int):
        super().__init__()
        ...
        self.q_proj = TmlClippableLinear(config, config.hidden_size, config.num_attention_heads * self.head_dim)
        self.k_proj = TmlClippableLinear(config, config.hidden_size, config.num_key_value_heads * self.head_dim)
        self.v_proj = TmlClippableLinear(config, config.hidden_size, config.num_key_value_heads * self.head_dim)
        self.o_proj = TmlClippableLinear(config, config.num_attention_heads * self.head_dim, config.hidden_size)

        self.q_norm = TmlRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.k_norm = TmlRMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.v_norm = TmlRMSNorm(self.head_dim, eps=config.rms_norm_eps, with_scale=False)
...
class TmlVisionEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: TmlVisionConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        self.self_attn = TmlVisionAttention(config=config, layer_idx=layer_idx)
        self.mlp = TmlVisionMLP(config)
        self.input_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class TmlVisionEncoder(nn.Module):
    def __init__(self, config: TmlVisionConfig):
        super().__init__()
        self.config = config
        self.num_layers = config.num_hidden_layers
        self.rotary_emb = TmlVisionRotaryEmbedding(config)
        self.layers = nn.ModuleList(
            [TmlVisionEncoderLayer(config=config, layer_idx=i) for i in range(self.num_layers)]
        )
...
class TmlTextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: TmlTextConfig, device=None, layer_type=None):
        super().__init__()
        self.max_seq_len_cached = config.max_position_embeddings
        self.original_max_seq_len = config.max_position_embeddings
    ...
    @staticmethod
    def compute_default_rope_parameters(
        config: TmlTextConfig | None = None,
...
class TmlTextAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: TmlTextConfig, layer_idx: int):
        ...
        self.q_norm = TmlRMSNorm(dim=self.head_dim, eps=config.rms_norm_eps)

        # Layers sharing kv states don't need any weight matrices
        if not self.is_kv_shared_layer:
            self.k_norm = TmlRMSNorm(dim=self.head_dim, eps=config.rms_norm_eps)
            self.v_norm = TmlRMSNorm(self.head_dim, eps=config.rms_norm_eps, with_scale=False)
...
class TmlTextExperts(nn.Module):
...
class TmlTextRouter(nn.Module):
    def __init__(self, config: TmlTextConfig):
        ...
        self.norm = TmlRMSNorm(self.hidden_size, eps=self.eps, with_scale=False)
...
class TmlTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: TmlTextConfig | TmlVisionConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        self.self_attn = TmlTextAttention(config=config, layer_idx=layer_idx)
        self.mlp = TmlTextMLP(config, layer_idx)
        self.input_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        ...
        if self.hidden_size_per_layer_input:
            ...
            self.post_per_layer_input_norm = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)

        self.enable_moe_block = config.enable_moe_block
        if self.enable_moe_block:
            self.router = TmlTextRouter(config)
            self.experts = TmlTextExperts(config)
            self.post_feedforward_layernorm_1 = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
            self.post_feedforward_layernorm_2 = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
            self.pre_feedforward_layernorm_2 = TmlRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class TmlTextScaledWordEmbedding(nn.Embedding):
    """
...
class TmlPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, TmlVisionPatchEmbedder):
            init.ones_(module.position_embedding_table)
        elif isinstance(module, TmlAudioRelPositionalEncoding):
            min_timescale = 1.0
            max_timescale = 10000.0
        ...
        elif isinstance(module, TmlVisionRotaryEmbedding):
            rope_fn = (
        ...
        elif isinstance(module, TmlTextScaledWordEmbedding):
            init.constant_(module.embed_scale, module.scalar_embed_scale)
        elif isinstance(module, TmlTextRouter):
            init.ones_(module.scale)
            init.ones_(module.per_expert_scale)
        elif isinstance(module, TmlTextExperts):
            std = self.config.initializer_range
            init.normal_(module.gate_up_proj, mean=0.0, std=std)
            init.normal_(module.down_proj, mean=0.0, std=std)
        elif isinstance(module, TmlTextDecoderLayer):
            init.ones_(module.layer_scalar)
        elif isinstance(module, TmlClippableLinear) and module.use_clipped_linears:
        ...
        elif isinstance(module, TmlVisionModel) and module.config.standardize:
...
class TmlTextModel(TmlPreTrainedModel):
    ...
    _can_record_outputs = {
        "router_logits": OutputRecorder(TmlTextRouter, index=0),
        "hidden_states": TmlTextDecoderLayer,
        "attentions": TmlTextAttention,
    }
    ...
    def __init__(self, config: TmlTextConfig):
        ...
        # Tml downcasts the below to bfloat16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        self.embed_tokens = TmlTextScaledWordEmbedding(
            config.vocab_size, config.hidden_size, self.padding_idx, embed_scale=self.config.hidden_size**0.5
        )
        self.layers = nn.ModuleList(
            [TmlTextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = TmlRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = TmlTextRotaryEmbedding(config)
        ...
        self.hidden_size_per_layer_input = config.hidden_size_per_layer_input
        if self.hidden_size_per_layer_input:
            self.embed_tokens_per_layer = TmlTextScaledWordEmbedding(
            ...
            self.per_layer_projection_norm = TmlRMSNorm(config.hidden_size_per_layer_input, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return TmlTextModelOutputWithPast(
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        "the `inputs_embeds` you provided do not exactly match the embedding weights. Since Tml needs to reverse "
...
class TmlForCausalLM(TmlPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: TmlTextConfig):
        super().__init__(config)
        self.model = TmlTextModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, TmlForCausalLM

        >>> model = TmlForCausalLM.from_pretrained("google/gemma-2-9b")
        ...
        outputs: TmlTextModelOutputWithPast = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
...
class TmlAudioModel(TmlPreTrainedModel):
    ...
    base_model_prefix = "model.audio_tower"  # prefix for TmlForConditionalGeneration saved checkpoints, required for TmlAudioModel.from_pretrained()
    _can_record_outputs = {
        "hidden_states": TmlAudioLayer,
        "attentions": TmlAudioAttention,
    }
    ...
    def __init__(self, config: TmlAudioConfig):
        ...
        self.subsample_conv_projection = TmlAudioSubSampleConvProjection(config)
        self.rel_pos_enc = TmlAudioRelPositionalEncoding(config)
        self.layers = nn.ModuleList(
            [TmlAudioLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return TmlAudioModelOutput(last_hidden_state=hidden_states, attention_mask=output_mask)


class TmlVisionModel(TmlPreTrainedModel):
    """The Gemma 4 Vision Encoder."""

    config = TmlVisionConfig
    _can_record_outputs = {
        "hidden_states": TmlVisionEncoderLayer,
        "attentions": TmlVisionAttention,
    }

    def __init__(self, config: TmlVisionConfig):
        super().__init__(config)
        self.patch_embedder = TmlVisionPatchEmbedder(config)
        self.encoder = TmlVisionEncoder(config)
        self.pooler = TmlVisionPooler(config)
...
class TmlMultimodalEmbedder(nn.Module):
    """Embeds token ids or soft tokens for multimodal content into language model space."""

    def __init__(
        self,
        multimodal_config: TmlAudioConfig | TmlVisionConfig,
        ...
        self.embedding_pre_projection_norm = TmlRMSNorm(self.multimodal_hidden_size, eps=self.eps, with_scale=False)
...
class TmlModel(TmlPreTrainedModel):
    ...
    def __init__(self, config: TmlConfig):
        ...
        self.embed_vision = (
            TmlMultimodalEmbedder(config.vision_config, config.text_config)
            if config.vision_config is not None
            else None
        )
        self.embed_audio = (
            TmlMultimodalEmbedder(config.audio_config, config.text_config) if config.audio_config is not None else None
        )
        self.post_init()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if input_features is not None and input_features_mask is not None:
            ...
            # This mirrors the vision encoder's padding stripping (see TmlVisionEncoder.forward).
