...
class TmlTextConfig(PreTrainedConfig):
    ...
    hidden_size_per_layer_input (`int`, defaults to 256):
        ...
        because all layers are packed into a single table. See the [Tml](https://huggingface.co/docs/transformers/main/en/model_doc/tml#per-layer-embeddings-ple) docs
...
class TmlConfig(PreTrainedConfig):
    ...
    >>> configuration = TmlConfig(text_config, vision_config, audio_config)

    >>> # Initializing a model from the google/gemma-4-e2b-it configuration
    >>> model = TmlForConditionalGeneration(configuration)
    ...
    text_config: TmlTextConfig | dict[str, Any] | None = None
    vision_config: TmlVisionConfig | dict[str, Any] | None = None
    audio_config: TmlAudioConfig | dict[str, Any] | None = None
    ...
    def __post_init__(self, **kwargs):
        if self.text_config is None:
            self.text_config = TmlTextConfig()
            logger.info("text_config is None. Using default TmlTextConfig.")
        elif isinstance(self.text_config, dict):
            self.text_config = TmlTextConfig(**self.text_config)

        if self.vision_config is None:
            logger.info("vision_config is None. TmlModel.vision_tower will not be initialized.")
        if isinstance(self.vision_config, dict):
            self.vision_config = TmlVisionConfig(**self.vision_config)

        if self.audio_config is None:
            logger.info("audio_config is None. TmlModel.audio_tower will not be initialized.")
        if isinstance(self.audio_config, dict):
            self.audio_config = TmlAudioConfig(**self.audio_config)

        super().__post_init__(**kwargs)


__all__ = ["TmlAudioConfig", "TmlConfig", "TmlTextConfig", "TmlVisionConfig"]
