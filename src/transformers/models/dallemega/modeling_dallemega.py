...
class DalleMegaForConditionalGeneration(DalleMegaPretrainedModel):
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, inputs_tensor: torch.Tensor, model_kwargs, superconditioning_scale: float, model_input_name: Optional[str] = None
    ) -> Dict[str, Any]:
        # 1. get encoder
        ...
        encoder_outputs: BaseModelOutput = encoder(**encoder_kwargs)
        ...
        if superconditioning_scale > 1:
            ...
            encoder_outputs_unconditional = encoder(
                input_ids_uncond, attention_mask=attention_mask_uncond, return_dict=True
            )

            # concatenate encoder outputs
            encoder_outputs = torch.cat([encoder_outputs, encoder_outputs_unconditional], dim=0)
            # concatenate attention_mask if if it exists
            if model_kwargs.get("attention_mask", None) is not None:
                attention_mask = torch.cat([model_kwargs["attention_mask"], attention_mask_uncond], dim=0)
    ...
    def generate(
        ...
        superconditioning_scale: Optional[float] = None,
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                inputs_tensor, model_kwargs, model_input_name, superconditioning_scale
            )
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, superconditioning_scale=superconditioning_scale, **model_kwargs)
            ...
            if superconditioning_scale > 1:
                next_token_logits, next_token_logits_uncond = next_token_logits.chunk(2)
                next_token_logits = next_token_logits + superconditioning_scale * (next_token_logits - next_token_logits_uncond)
