...
class Gemma4UnifiedAudioProcessorMixin:
    ...
    def compute_features(self, audio, *, audio_samples_per_token, **kwargs):
        return [self._chunk_waveform(waveform, audio_samples_per_token=audio_samples_per_token) for waveform in audio]
...
class Gemma4UnifiedAudioProcessor(Gemma4UnifiedAudioProcessorMixin, TorchAudioBackend):
    def _chunk_waveform(self, waveform, *, audio_samples_per_token):
        ...
        return waveform.reshape(num_tokens, audio_samples_per_token).to(torch.float32)
