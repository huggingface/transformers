# coding=utf-8
# Copyright 2025 the Cohere team. All rights reserved.
#
...
_CONFIG_FOR_DOC = "Cohere2VisionConfig"
...
class Cohere2VisionProjector(nn.Module):
    ...
    def mul_and_silu(self, x):
        x = x[..., : self.intermediate_size // 2] * self.act(x[..., self.intermediate_size // 2 :])
        return x

    def forward(self, image_features):
        image_features = self.pixel_shuffle(image_features)
        hidden_states = self.linear_1(image_features)
        hidden_states = self.mul_and_silu(hidden_states)
...
COHERE2_VISION_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`Cohere2VisionConfig`] or [`Cohere2VisionVisionConfig`]):
...
@add_start_docstrings(
    ...
    COHERE2_VISION_START_DOCSTRING,
)
class Cohere2VisionPreTrainedModel(PreTrainedModel):
    config_class = Cohere2VisionConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Cohere2VisionAttention"]
    ...
    def _init_weights(self, module):
        # important: this ported version of Cohere2Vision isn't meant for training from scratch - only
...
COHERE2_VISION_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    """The Cohere2 Vision model which consists of a vision backbone and a language model.""",
    COHERE2_VISION_START_DOCSTRING,
)
class Cohere2VisionForConditionalGeneration(Cohere2VisionPreTrainedModel, GenerationMixin):
    def __init__(self, config: Cohere2VisionConfig):
        ...
        self.multi_modal_projector = Cohere2VisionProjector(config)
    ...
    @add_start_docstrings_to_model_forward(COHERE2_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Cohere2VisionCausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        ...
        last_cache_position: Optional[torch.LongTensor] = None,
        ...
        if pixel_values is not None and pixel_values.size(0) > 0:
            assert image_num_patches is not None, "image_num_patches must be provided if pixel_values are provided"
...
__all__ = ["Cohere2VisionForConditionalGeneration", "Cohere2VisionPreTrainedModel"]
