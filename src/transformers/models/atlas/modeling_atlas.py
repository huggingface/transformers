...
@dataclass
# Copied from transformers.models.rag.modeling_rag.RetrievAugLMMarginOutput with rag->atlas
class RetrievAugLMMarginOutput(ModelOutput):
    ...
    Args:
        ...
        question_enc_attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            aveatlase in the self-attention heads.
        ...
        generator_cross_attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            weighted aveatlase in the cross-attention heads.
    """
...
@dataclass
# Copied from transformers.models.rag.modeling_rag.RetrievAugLMOutput with rag->atlas
...
# Copied from transformers.models.rag.modeling_rag.RagPreTrainedModel with RAG->ATLAS,Rag->Atlas,rag->atlas
class AtlasPreTrainedModel(PreTrainedModel):
    r"""
    ATLAS models were released with the paper [Retrieval-Augmented Generation for Knowledge-Intensive NLP
    Tasks](https://arxiv.org/abs/2005.11401) by Patrick Lewis, Ethan Perez, Aleksandra Piktus et al.

    ATLAS is a retriever augmented model and encapsulate three components: a question encoder, a dataset retriever and a
    ...
    def from_pretrained_question_encoder_generator(
        ...
        **kwargs
        ...
        Params:
            ...
            retriever ([`AtlasRetriever`], *optional*):
        ...
        >>> # initialize a ATLAS from two pretrained models.
        >>> model = AtlasModel.from_pretrained_question_encoder_generator(
        ...     "facebook/dpr-question_encoder-single-nq-base", "t5-small"
        ... )
        >>> # saving model after fine-tuning
        >>> model.save_pretrained("./atlas")
        >>> # load fine-tuned model
        >>> model = AtlasModel.from_pretrained("./atlas")
        ```"""
        ...
        if config is None:
            config = AtlasConfig.from_question_encoder_generator_configs(
...
ATLAS_START_DOCSTRING = r"""

    ATLAS is a seq2seq model which encapsulates two core components: a question encoder and a generator. During a forward
    ...
    The model can be initialized with a [`AtlasRetriever`] for end-to-end generation or used in combination with the
    ...
    *autoencoding* model as the `question_encoder` and any *seq2seq* model with language model head as the `generator`.
    ...
    Args:
        ...
        generator ([`PreTrainedModel`]):
            A seq2seq model used as the generator in the ATLAS architecture.
        retriever ([`AtlasRetriever`]):
            A retriever class encapsulating a faiss index queried to obtain context documents for current inputs.
"""


ATLAS_FORWARD_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. [`AtlasConfig`], used to initialize the model, specifies
        ...
        encoder_outputs (`tuple(tuple(torch.FloatTensor)`, *optional*)
            ...
            *optional*: `generator_enc_attentions`). `generator_enc_last_hidden_state` of shape `(batch_size, n_docs *
            ...
            Used by the ([`AtlasModel`]) model during decoding.
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            Provide for generation tasks. `None` by default, construct as per instructions for the generator model
            you're using with your ATLAS instance.
        decoder_attention_mask (`torch.BoolTensor` of shape `(batch_size,  target_sequence_length)`, *optional*):
            Default behavior: generate a tensor that ignores pad tokens in `decoder_input_ids`. Causal mask will also
            be used by default.
        past_key_values (`tuple(tuple(torch.FloatTensor))`):
            Tuple consists of two elements: `encoder_outputs` of the ATLAS model (see `encoder_outputs`) and
            `past_key_values` of the underlying generator. Can be used to speed up decoding. `past_key_values` are used
            in the ([`AtlasTokenForGeneration`]) model during decoding.
        doc_scores (`torch.FloatTensor` of shape `(batch_size, config.n_docs)`):
            Score between each retrieved document embeddings (see `retrieved_doc_embeds`) and
        ...
        context_input_ids (`torch.LongTensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):
            ...
            forward pass. `context_input_ids` are returned by [`~AtlasRetriever.__call__`]. context_attention_mask
            ...
            forward pass. `context_attention_mask` are returned by [`~AtlasRetriever.__call__`].
...
class AtlasModel(AtlasPreTrainedModel):
    def __init__(
        ...
        retriever: Optional[AtlasRetriever] = None,  # or maybe just use a `set_retriever(...)` method
        **kwargs,
    ):
        ...
        if self.retriever is not None:
            assert isinstance(
                retriever, AtlasRetriever
            ), f"`self.retriever` is of type {type(self.retriever)}, but should be of type `AtlasRetriever`"
            self.retriever = retriever
    ...
    @add_start_docstrings_to_model_forward(ATLAS_FORWARD_INPUTS_DOCSTRING)
    ...
    def forward(
        ...
        # encoder_outputs are pre-computed during ATLAS-token generation
...
@add_start_docstrings_to_model_forward(
    """
    A ATLAS-sequence model implementation. It performs ATLAS-sequence specific marginalization in the forward pass.
    """,
    ATLAS_START_DOCSTRING,
)
# Copied from transformers.models.rag.modeling_rag.RagSequenceForGeneration with RAG->ATLAS,Rag->Atlas,rag->atlas
class AtlasSequenceForGeneration(AtlasPreTrainedModel):
    def __init__(
        self,
        config: Optional[PretrainedConfig] = None,
        question_encoder: Optional[PreTrainedModel] = None,
        generator: Optional[PreTrainedModel] = None,
        retriever: Optional[AtlasRetriever] = None,
        **kwargs,
    ):
        assert config is not None or (
            question_encoder is not None and generator is not None
        ...
        # instantiate model
        self.atlas = AtlasModel(config=config, question_encoder=question_encoder, generator=generator, retriever=retriever)

    def set_retriever(self, retriever: AtlasRetriever):
        self.atlas.retriever = retriever

    def set_context_encoder_for_training(self, ctx_encoder: PreTrainedModel):
        self.atlas.context_encoder_training = True
        self.atlas.ctx_encoder = ctx_encoder
    ...
    def forward(
        ...
        **kwargs  # needs kwargs for generation
        ...
        ... )
        >>> # initialize with AtlasRetriever to do everything in one forward call
        >>> model = AtlasSequenceForGeneration.from_pretrained("facebook/atlas-token-nq", retriever=retriever)
        ...
        >>> model = AtlasSequenceForGeneration.from_pretrained("facebook/atlas-sequence-nq", use_dummy_dataset=True)
        >>> # 1. Encode
    ...
    @property
    def retriever(self):
        return self.atlas.retriever

    @property
    def generator(self):
        return self.atlas.generator

    @property
    def question_encoder(self):
        return self.atlas.question_encoder
    ...
    def generate(
        ...
        **model_kwargs
        ...
        """
        Implements ATLAS sequence "thorough" decoding. Read the [`~generation.GenerationMixin.generate`]` documentation
        ...
        Args:
            ...
            context_attention_mask (`torch.LongTensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):
                ...
                [`~AtlasRetriever.__call__`].
            doc_scores (`torch.FloatTensor` of shape `(batch_size, config.n_docs)`):
                Score between each retrieved document embeddings (see `retrieved_doc_embeds`) and
                `question_encoder_last_hidden_state`.

                If the model is not initialized with a `retriever` or `input_ids` is not given, `doc_scores` has to be
                provided to the forward pass. `doc_scores` are returned by [`~AtlasRetriever.__call__`].
    ...
    def get_nll(
        ...
        # ATLAS-sequence marginalization
        first_token_scores = seq_logprobs[:, :, :1, :]
        second_token_scores = seq_logprobs[:, :, 1:2, :]
        remainder = seq_logprobs[:, :, 2:, :]
        atlas_logprobs = torch.cat([first_token_scores, second_token_scores + doc_logprobs, remainder], dim=2)

        # calculate loss
        target = target.unsqueeze(1).unsqueeze(-1).repeat(1, n_docs, 1, 1)
        assert target.dim() == atlas_logprobs.dim()

        ll = atlas_logprobs.gather(dim=-1, index=target)
        smooth_obj = atlas_logprobs.sum(dim=-1, keepdim=True)  # total sum of all (normalised) logits
        ...
        eps_i = epsilon / atlas_logprobs.size(-1)
...
@add_start_docstrings_to_model_forward(
    """
    A ATLAS-token model implementation. It performs ATLAS-token specific marginalization in the forward pass.
    """,
    ATLAS_START_DOCSTRING,
)
# Copied from transformers.models.rag.modeling_rag.RagTokenForGeneration with RAG->ATLAS,Rag->Atlas,rag->atlas
class AtlasTokenForGeneration(AtlasPreTrainedModel):
    def __init__(
        self,
        config: Optional[PretrainedConfig] = None,
        question_encoder: Optional[PreTrainedModel] = None,
    ...
    def forward(
        ...
        **kwargs  # needs kwargs for generation
        ...
        ... )
        >>> # initialize with AtlasRetriever to do everything in one forward call
        >>> model = AtlasTokenForGeneration.from_pretrained("facebook/atlas-token-nq", retriever=retriever)
        ...
        >>> # or use retriever separately
        >>> model = AtlasTokenForGeneration.from_pretrained("facebook/atlas-token-nq", use_dummy_dataset=True)
        >>> # 1. Encode
        >>> question_hidden_states = model.question_encoder(input_ids)[0]
        >>> # 2. Retrieve
        >>> docs_dict = retriever(input_ids.numpy(), question_hidden_states.detach().numpy(), return_tensors="pt")
    ...
    def generate(
        ...
        **model_kwargs
        ...
        """
        Implements ATLAS token decoding.
        ...
        Args:
            ...
            context_input_ids (`torch.LongTensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):
                ...
                If the model has is not initialized with a `retriever`, `context_input_ids` has to be provided to the
                forward pass. `context_input_ids` are returned by [`~AtlasRetriever.__call__`].
            context_attention_mask (`torch.LongTensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):
                Attention mask post-processed from the retrieved documents and the question encoder `input_ids` by the
                retriever.
            ...
            length_penalty (`float`, *optional*, defaults to 1.0):
                ...
                while `length_penalty` < 0.0 encouatlases shorter sequences.
        ...
        encoder = self.atlas.generator.get_encoder()
    ...
    def get_input_embeddings(self):
        return self.atlas.generator.get_input_embeddings()

    def get_output_embeddings(self):
        return self.atlas.generator.get_output_embeddings()

    def set_output_embeddings(self, new_embeddings):
        return self.atlas.generator.set_output_embeddings(new_embeddings)
    ...
    def get_nll(self, seq_logits, doc_scores, target, reduce_loss=False, epsilon=0.0, n_docs=None):
        ...
        atlas_logprobs = self.marginalize(seq_logits, doc_scores, n_docs)
