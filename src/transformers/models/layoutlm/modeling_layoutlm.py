...
class RelationExtractionOutput(ModelOutput):
    """
    Class for outputs of [`LayoutLMForRelationExtraction`].
    Args:
...
class LayoutLMRelationExtractionDecoder(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, hidden_states, entities, relations):
        ...
        for b in range(batch_size):
            ...
            # get the mean embedding for head entity
            head_emb = list()
            for s, e in zip(head_start_index, head_end_index):
                head_emb.append(hidden_states[b][s:e].mean(0))
            tail_emb = list()
            for s, e in zip(tail_start_index, tail_end_index):
                tail_emb.append(hidden_states[b][s:e].mean(0))
            head_emb = torch.stack(head_emb)
            tail_emb = torch.stack(tail_emb)

            head_repr = torch.cat(
                (head_emb, head_label_repr),
                dim=-1,
            )
            tail_repr = torch.cat(
                (tail_emb, tail_label_repr),
                dim=-1,
            )
...
@add_start_docstrings(
    """
    LayoutLM Model with a relation extraction head on top for key-value extraction tasks such as
    [XFUND](https://github.com/doc-analysis/XFUND) (a bi-affine attention layer on top).
    """,
    LAYOUTLM_START_DOCSTRING,
)
class LayoutLMForRelationExtraction(LayoutLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.layoutlm = LayoutLMModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.extractor = LayoutLMRelationExtractionDecoder(config)
    ...
    def forward(
        ...
        Example:
        ```python
        >>> from transformers import LayoutLMProcessor, LayoutLMForRelationExtraction
