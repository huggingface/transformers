...
class BertAttention(nn.Module):
    ...
    BERT historically split this over `BertSelfAttention` (q/k/v) and `BertSelfOutput` (the output
    projection plus the residual LayerNorm). The projection lives here as `o_proj`; the LayerNorm
    and residual belong to `BertLayer`, which is what makes the layer's topology visible.
    """
...
# self-attention path no longer uses this: its projection is `BertAttention.o_proj` and its
# LayerNorm belongs to `BertLayer`. Cross-attention keeps the original layout deliberately, so that
...
class BertCrossAttention(nn.Module):
    """Cross-attention query/key/value. Its output projection is `BertSelfOutput`, see above."""
...
class BertCrossAttentionBlock(nn.Module):
    """Cross-attention plus its output projection, in BERT's original module layout."""

    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.self = BertCrossAttention(config, is_causal=False, layer_idx=layer_idx)
...
class BertMLP(nn.Module):
    ...
    Replaces `BertIntermediate` (up projection plus activation) and `BertOutput` (down projection
    plus the residual LayerNorm); the LayerNorm and residual now belong to `BertLayer`.
    """
...
class BertLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.self_attn = BertAttention(config, is_causal=config.is_decoder, layer_idx=layer_idx)
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = BertCrossAttentionBlock(config, layer_idx=layer_idx)
