...
from transformers import GenerationConfig, BLTConfig, BLTForCausalLM, BLTTokenizer, PreTrainedTokenizerFast
...
try:
    ...
    BLTTokenizerFast = None

"""
Sample usage:

```
python src/transformers/models/blt/convert_blt_weights_to_hf.py \
    --input_dir /path/to/downloaded/blt/weights --model_size 1B --blt_version 3.2 --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import BLTForCausalLM, BLTTokenizer

model = BLTForCausalLM.from_pretrained("/output/path")
tokenizer = BLTTokenizer.from_pretrained("/output/path")
```
...
def write_model(
    ...
    if base > 10000.0 and not is_blt_3(blt_version):
        max_position_embeddings = 16384
    else:
        max_position_embeddings = CONTEXT_LENGTH_FOR_VERSION[blt_version]
    ...
    with tempfile.TemporaryDirectory() as tmp_model_path:
        ...
        if num_shards == 1:
            ...
            concat_dim = 0 if is_blt_3(blt_version) else 1
            state_dict = {
        ...
        if blt_version in ["3.1", "3.2", "Guard-3"]:
            rope_scaling = {
                "factor": 32.0 if blt_version == "3.2" else 8.0,
                "low_freq_factor": 1.0,
        ...
        config = BLTConfig(
            ...
            tie_word_embeddings=True if blt_version in ["3.2"] else False,
        )
        ...
        print("Loading the checkpoint in a BLT model.")
        model = BLTForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
...
class BLT3Converter(TikTokenConverter):
    def __init__(self, vocab_file, special_tokens=None, instruct=False, blt_version="3.2", **kwargs):
        ...
        templates_for_version = {
            "2": ("meta-blt/BLT-2-7b-chat-hf", "f5db02db724555f92da89c216ac04704f23d4590"),
            "3": ("meta-blt/Meta-BLT-3-8B-Instruct", "5f0b02c75b57c5855da9ae460ce51323ea669d8a"),
            "3.1": ("meta-blt/BLT-3.1-8B-Instruct", "0e9e39f249a16976918f6564b8830bc894c89659"),
            "3.2": ("meta-blt/BLT-3.2-1B-Instruct", "e9f8effbab1cbdc515c11ee6e098e3d5a9f51e14"),
            "Guard-3": ("meta-blt/BLT-Guard-3-1B", "acf7aafa60f0410f8f42b1fa35e077d705892029"),
        }
        ...
        additional_kwargs = {}
        if instruct or blt_version in ["Guard-3"]:
            model_id, revision = templates_for_version.get(blt_version, (None, None))
        ...
        self.converted_tokenizer = PreTrainedTokenizerFast(
            ...
            model_max_length=CONTEXT_LENGTH_FOR_VERSION[blt_version],
...
def write_tokenizer(
    tokenizer_path, input_tokenizer_path, blt_version="2", special_tokens=None, instruct=False, push_to_hub=False
):
    print("Converting the tokenizer.")
    tokenizer_class = BLTTokenizer if BLTTokenizerFast is None else BLTTokenizerFast
    if is_blt_3(blt_version):
        tokenizer = BLT3Converter(
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        help="Location of BLT weights, which contains tokenizer.model and model folders",
    )
    parser.add_argument(
        "--model_size",
        default=None,
        help="'f' Deprecated in favor of `num_shards`: models correspond to the finetuned versions, and are specific to the BLT2 official release. For more details on BLT2, checkout the original repo: https://huggingface.co/meta-blt",
    )
    parser.add_argument(
        "--output_dir",
    ...
    # Different BLT versions used different default values for max_position_embeddings, hence the need to be able to specify which version is being used.
    parser.add_argument(
        "--blt_version",
        choices=["1", "2", "3", "3.1", "3.2", "Guard-3"],
        default="1",
        type=str,
        help="Version of the BLT model to convert. Currently supports BLT1 and BLT2. Controls the context size",
    )
    parser.add_argument(
        "--num_shards",
        default=None,
    ...
    if args.special_tokens is None:
        ...
        args.special_tokens = DEFAULT_BLT_SPECIAL_TOKENS.get(str(args.blt_version), [])
