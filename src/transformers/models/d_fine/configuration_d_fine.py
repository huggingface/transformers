...
class DFineConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`DFineModel`]. It is used to instantiate a
    ...
    Args:
        ...
        backbone_config (`Dict`, *optional*, defaults to `DFineResNetConfig()`):
    ...
    ```python
    >>> from transformers import DFineConfig, DFineModel

    >>> # Initializing a RT-DETR configuration
    >>> configuration = DFineConfig()

    >>> # Initializing a model (with random weights) from the configuration
    >>> model = DFineModel(configuration)
