...
def prepare_coco_detection_annotation(
    ...
    """
    Convert the target in COCO format into the format expected by LwDetr.
    """
...
def prepare_coco_panoptic_annotation(
    ...
    """
    Prepare a coco panoptic annotation for LwDetr.
    """
    image_height, image_width = get_image_size(image, channel_dim=input_data_format)
...
class LwDetrImageProcessor(BaseImageProcessor):
    ...
    def from_dict(cls, image_processor_dict: dict[str, Any], **kwargs):
        ...
        created using from_dict and kwargs e.g. `LwDetrImageProcessor.from_pretrained(checkpoint, size=600,
        max_size=800)`
        """
    ...
    def prepare_annotation(
        ...
        """
        Prepare an annotation for feeding into LwDetr model.
        """
    ...
    def post_process(self, outputs, target_sizes):
        """
        Converts the raw output of [`LwDetrForObjectDetection`] into final bounding boxes in (top_left_x,
        top_left_y, bottom_right_x, bottom_right_y) format. Only supports PyTorch.

        Args:
            outputs ([`LwDetrObjectDetectionOutput`]):
