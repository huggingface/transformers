...
class PEAudioVideoEncoder(PEAudioVideoPretrainedModel):
    ...
    def _align_video_hidden_state(
        ...
        video_lengths = (
            ...
            else video_hidden_state.shape[1]
            * video_hidden_state.new_ones(video_hidden_state.shape[0], dtype=torch.long)
        )
        audio_lengths = (
            padding_mask.sum(dim=-1)
            if padding_mask is not None
            else audio_hidden_state.shape[1]
            * audio_hidden_state.new_ones(audio_hidden_state.shape[0], dtype=torch.long)
        )
...
class PEAudioVideoModel(PEAudioVideoPretrainedModel):
    def __init__(self, config: PEAudioVideoConfig):
        ...
        self.text_audio_head = PEAudioVideoContrastiveHead(config.text_config.hidden_size, config.projection_dim)
        ...
        self.text_video_head = PEAudioVideoContrastiveHead(config.text_config.hidden_size, config.projection_dim)
        ...
        self.text_audio_video_head = PEAudioVideoContrastiveHead(config.text_config.hidden_size, config.projection_dim)
