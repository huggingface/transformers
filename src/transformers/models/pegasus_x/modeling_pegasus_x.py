# coding=utf-8
# Copyright 2022 Google The HuggingFace Inc. team. All rights reserved.
#
...
class PegasusXLearnedPositionalEmbedding(nn.Embedding):
    """
...
class PegasusXClassificationHead(nn.Module):
...
PEGASUS_X_GENERATION_EXAMPLE = r"""
    ...
    >>> model = PegasusXForConditionalGeneration.from_pretrained('pegasus-x-base')
...
PEGASUS_X_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_attention_mask (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            If you want to change padding behavior, you should read [`modeling_pegasus_x._prepare_decoder_attention_mask`] and
...
PEGASUS_X_STANDALONE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
class PegasusXEncoder(PegasusXPreTrainedModel):
    ...
    def __init__(self, config: PegasusXConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = PegasusXLearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        self.layers = nn.ModuleList([PegasusXEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class PegasusXDecoder(PegasusXPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`PegasusXDecoderLayer`]
...
class PegasusXForSequenceClassification(PegasusXPreTrainedModel):
    def __init__(self, config: PegasusXConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.model = PegasusXModel(config)
        self.classification_head = PegasusXClassificationHead(
            config.d_model,
            config.d_model,
            config.num_labels,
...
class PegasusXForQuestionAnswering(PegasusXPreTrainedModel):
...
class PegasusXForCausalLM(PegasusXPreTrainedModel):
    def __init__(self, config):
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        super().__init__(config)
        self.model = PegasusXDecoderWrapper(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import PegasusXTokenizer, PegasusXForCausalLM

        >>> tokenizer = PegasusXTokenizer.from_pretrained('facebook/bart-large')
        >>> model = PegasusXForCausalLM.from_pretrained('facebook/bart-large', add_cross_attention=False)
