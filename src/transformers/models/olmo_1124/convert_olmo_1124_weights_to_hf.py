...
def _write_tokenizer(
    output_path: Path, config: Olmo1124Config, input_tokenizer_path: Path, fix_eos_token_id: bool = True
) -> None:
    ...
    if fix_eos_token_id and eos_token_id == 0:
        # Fixing a bug in OLMo November 2024 where eos token id was incorrectly set
...
def main():
    ...
    parser.add_argument(
        "--tokenizer_json_path",
        default=None,
        help="Location of OLMo November 2024 tokenizer json file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
