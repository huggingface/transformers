# coding=utf-8
# Copyright 2022 zyy334 and The HuggingFace Inc. team. All rights reserved.
#
...
_TOKENIZER_FOR_DOC = "CodeGeeXTokenizer"
...
class TFCodeGeeXLearnedPositionalEmbedding(tf.keras.layers.Embedding):
    """
...
class TFCodeGeeXAttention(tf.keras.layers.Layer):
...
class TFCodeGeeXEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: CodeGeeXConfig, **kwargs):
        super().__init__(**kwargs)
        self.embed_dim = config.d_model
        self.self_attn = TFCodeGeeXAttention(
...
class TFCodeGeeXDecoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: CodeGeeXConfig, **kwargs):
        super().__init__(**kwargs)
        self.embed_dim = config.d_model
        self.self_attn = TFCodeGeeXAttention(
        ...
        self.self_attn_layer_norm = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="self_attn_layer_norm")
        self.encoder_attn = TFCodeGeeXAttention(
            self.embed_dim,
...
class TFCodeGeeXPreTrainedModel(TFPreTrainedModel):
...
CODEGEEX_START_DOCSTRING = r"""
    ...
    Args:
        config ([`~CodeGeeXConfig`]): Model configuration class with all the parameters of the model.
...
CODEGEEX_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (`tf.Tensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            CodeGeeX uses the `eos_token_id` as the starting token for
...
@keras_serializable
class TFCodeGeeXEncoder(tf.keras.layers.Layer):
    config_class = CodeGeeXConfig
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`TFCodeGeeXEncoderLayer`].

    Args:
        config: CodeGeeXConfig
    """

    def __init__(self, config: CodeGeeXConfig, embed_tokens: Optional[tf.keras.layers.Embedding] = None, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.dropout = tf.keras.layers.Dropout(config.dropout)
        ...
        self.embed_positions = TFCodeGeeXLearnedPositionalEmbedding(
            config.max_position_embeddings,
            config.d_model,
            name="embed_positions",
        )
        self.layers = [TFCodeGeeXEncoderLayer(config, name=f"layers.{i}") for i in range(config.encoder_layers)]
...
@keras_serializable
class TFCodeGeeXDecoder(tf.keras.layers.Layer):
    config_class = CodeGeeXConfig
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`TFCodeGeeXDecoderLayer`]
    ...
    def __init__(self, config: CodeGeeXConfig, embed_tokens: Optional[tf.keras.layers.Embedding] = None, **kwargs):
        ...
        self.embed_scale = tf.math.sqrt(float(config.d_model)) if config.scale_embedding else 1.0
        self.layers = [TFCodeGeeXDecoderLayer(config, name=f"layers.{i}") for i in range(config.decoder_layers)]
        self.layernorm_embedding = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="layernorm_embedding")
...
@keras_serializable
class TFCodeGeeXMainLayer(tf.keras.layers.Layer):
    config_class = CodeGeeXConfig
    ...
    def __init__(self, config: CodeGeeXConfig, **kwargs):
        ...
        self.encoder = TFCodeGeeXEncoder(config, self.shared, name="encoder")
        self.decoder = TFCodeGeeXDecoder(config, self.shared, name="decoder")
...
class TFCodeGeeXModel(TFCodeGeeXPreTrainedModel):
    def __init__(self, config: CodeGeeXConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        self.model = TFCodeGeeXMainLayer(config, name="model")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(CODEGEEX_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
@add_start_docstrings(
    "The CODEGEEX Model with a language modeling head. Can be used for summarization.",
    CODEGEEX_START_DOCSTRING,
)
class TFCodeGeeXForConditionalGeneration(TFCodeGeeXPreTrainedModel):
    ...
    def call(
        ...
        ```python
        >>> from transformers import CodeGeeXTokenizer, TFCodeGeeXForConditionalGeneration
        >>> import tensorflow as tf
        >>> mname = 'codegeex-base-cased'
        >>> tokenizer = CodeGeeXTokenizer.from_pretrained(mname)
        >>> TXT = "My friends are <mask> but they eat too many carbs."
        >>> model = TFCodeGeeXForConditionalGeneration.from_pretrained(mname)
