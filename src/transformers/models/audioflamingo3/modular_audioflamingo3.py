...
class AudioFlamingo3ForConditionalGeneration(AudioFlamingo3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if input_features is not None and input_ids is not None and input_ids.shape[1] != 1:
            ...
            with torch.no_grad():
                per_sample_counts = (input_ids == self.config.audio_token_id).sum(dim=1).tolist()
            ...
            for b in range(bsz):
                pos_b = (input_ids[b] == self.config.audio_token_id).nonzero(as_tuple=False).squeeze(-1)  # (T_b,)
                if pos_b.numel() == 0:
                    continue
