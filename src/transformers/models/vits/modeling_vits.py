...
class HifiGanDiscriminatorResidualBlock(nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, leaky_relu_slope=0.1):
        ...
        convs = []
        for in_channels, out_channels in [(1,32), (32,128), (128,512), (5012,1024), (1024,1024)]:
            convs.append(nn.Conv2d(in_channels, out_channels, (kernel_size, 1), (stride, 1), padding = (self.get_padding(kernel_size, 1),0)))
...
class VitsDiscriminator(nn.Module):
    def __init__(self, config):
        ...
        self.discriminators = nn.ModuleList([HifiGanDiscriminatorResidualBlock(period, config.discriminator_kernel_size, config.discriminator_stride, config.leaky_relu_slope)] for period in config.discriminator_periods)
    ...
    def forward(self, hidden_states):
        ...
        for discriminator in self.discriminators:
            hidden_states, fmap = discriminator(hidden_states)
            fmaps.append(fmap)
            
        return hidden_states, fmaps
