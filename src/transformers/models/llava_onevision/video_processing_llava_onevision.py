...
class LlavaOnevisionVideoProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        for video in videos:
            if do_convert_rgb:
                video = [convert_to_rgb(frame) for frame in video]

            # All transformations expect numpy arrays.
            video = [to_numpy_array(frame) for frame in video]
            ...
            if input_data_format is None:
                ...
                input_data_format = infer_channel_dimension_format(video[0])

            new_video = []
            video_sizes = [get_image_size(frame, channel_dim=input_data_format) for frame in video]
            for frame in video:
                ...
                new_video.append(np.array(pixel_values))

            if do_pad:
                processed_video = self._pad_for_batching(new_video)

            pixel_values_batch.append(processed_video)
            video_sizes_batch.append(video_sizes)

        pixel_values_batch = np.array(pixel_values_batch)

        return BatchFeature(
            data={"pixel_values_videos": pixel_values_batch, "image_sizes_videos": video_sizes_batch},
