...
class ALIGNEncoderLayer(nn.Module):
    def __init__(self, config: ALIGNConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = ALIGNAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.mlp = ALIGNMLP(config)
...
class ALIGNPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, ALIGNVisionModelWithProjection):
            nn.init.normal_(
                module.visual_projection.weight,
                std=self.config.hidden_size**-0.5 * self.config.initializer_factor,
            )
        elif isinstance(module, ALIGNTextModelWithProjection):
            nn.init.normal_(
                module.text_projection.weight,
                std=self.config.hidden_size**-0.5 * self.config.initializer_factor,
            )
...
class ALIGNEncoder(nn.Module):
    ...
    def __init__(self, config: ALIGNConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([ALIGNEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class ALIGNTextTransformer(nn.Module):
    def __init__(self, config: ALIGNTextConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
    ...
    def forward(
        ...
        # ALIGN's text model uses causal mask, prepare it here.
        # https://github.com/openai/ALIGN/blob/cfcffb90e69f37bf2ff1e988237a0fbe41f33c04/align/model.py#L324
...
class ALIGNVisionModel(ALIGNPreTrainedModel):
    ...
    def __init__(self, config: ALIGNVisionConfig):
        super().__init__(config)
        self.vision_model = ALIGNVisionTransformer(config)
        # Initialize weights and apply final processing
        self.post_init()
...
@add_start_docstrings(
    """
    ALIGN Text Model with a projection layer on top (a linear layer on top of the pooled output).
    """,
    ALIGN_START_DOCSTRING,
)
class ALIGNTextModelWithProjection(ALIGNPreTrainedModel):
    config_class = ALIGNTextConfig
    ...
    @add_start_docstrings_to_model_forward(ALIGN_TEXT_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=ALIGNTextModelOutput, config_class=ALIGNTextConfig)
    def forward(
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, ALIGNTextModelOutput]:
        r"""
        Returns:
        ...
        ```python
        >>> from transformers import AutoTokenizer, ALIGNTextModelWithProjection

        >>> model = ALIGNTextModelWithProjection.from_pretrained("kakaobrain/align-base")
        >>> tokenizer = AutoTokenizer.from_pretrained("kakaobrain/align-base")
...
@add_start_docstrings(
    """
    ALIGN Vision Model with a projection layer on top (a linear layer on top of the pooled output).
    """,
    ALIGN_START_DOCSTRING,
)
class ALIGNVisionModelWithProjection(ALIGNPreTrainedModel):
    config_class = ALIGNVisionConfig
    main_input_name = "pixel_values"
    ...
    @add_start_docstrings_to_model_forward(ALIGN_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=ALIGNVisionModelOutput, config_class=ALIGNVisionConfig)
    def forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, ALIGNVisionModelOutput]:
        r"""
        Returns:
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import AutoProcessor, ALIGNVisionModelWithProjection

        >>> model = ALIGNVisionModelWithProjection.from_pretrained("kakaobrain/align-base")
        >>> processor = AutoProcessor.from_pretrained("kakaobrain/align-base")
