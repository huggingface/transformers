...
class SplinterForQuestionAnswering(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            start_logits = start_logits + (1 - attention_mask) * torch.finfo(self.dtype).min
            end_logits = end_logits + (1 - attention_mask) * torch.finfo(self.dtype).min
...
class SplinterForPreTraining(SplinterPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            start_logits = start_logits + (1 - attention_mask_for_each_question) * torch.finfo(self.dtype).min
            end_logits = end_logits + (1 - attention_mask_for_each_question) * torch.finfo(self.dtype).min
