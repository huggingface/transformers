...
class Beit3Embedder(nn.Module):
    ...
    def forward(self, hidden_states, text_end_position, multiway_split_position=-1):
        if text_end_position is None:
            positions = torch.arange(2, hidden_states.size(1) + 2,
                                     device=hidden_states.device).long().unsqueeze(0)
        else:
        ...
        y1, y2 = self.first(text_positions), self.second(image_positions)
        return torch.cat([y1, y2], dim=1)
