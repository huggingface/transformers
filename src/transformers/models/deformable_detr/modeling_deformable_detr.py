    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ref_y, ref_x = meshgrid(
                torch.linspace(0.5, height - 0.5, height, dtype=spatial_shapes.dtype, device=device),
                torch.linspace(0.5, width - 0.5, width, dtype=spatial_shapes.dtype, device=device),
                indexing="ij",
            )
