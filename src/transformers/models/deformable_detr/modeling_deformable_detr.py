...
class DeformableDetrSelfAttention(nn.Module):
    ...
    In Deformable DETR, position embeddings are added to both queries and keys (but not values) in self-attention.
    """
...
class DeformableDetrMLP(nn.Module):
    """
    MLP module used in Deformable DETR encoder and decoder layers.
    """
...
class DeformableDetrEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        self.mlp = DeformableDetrMLP(config, self.embed_dim, config.encoder_ffn_dim)
...
class DeformableDetrDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.mlp = DeformableDetrMLP(config, self.embed_dim, config.decoder_ffn_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
