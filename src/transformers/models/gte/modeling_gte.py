...
class GteRotaryEmbedding(nn.Module):
    @deprecate_kwarg("device", version="5.18")
    def __init__(self, config: GteConfig, device=None):
        super().__init__()
    ...
    @staticmethod
    @deprecate_kwarg("device", version="5.18")
    def compute_default_rope_parameters(config: GteConfig, device=None, **kwargs) -> tuple[torch.Tensor, float]:
        """
...
class GteLayer(GradientCheckpointingLayer):
    def __init__(self, config: GteConfig):
        ...
        self.mlp = GteMLP(config)
        self.self_attn = GteAttention(config=config)
...
class GteModel(GtePreTrainedModel):
    _no_split_modules = ["GteEmbeddings", "GteLayer"]
    ...
    def __init__(self, config: GteConfig, add_pooling_layer=False):
        ...
        self.rotary_emb = GteRotaryEmbedding(config)
        self.layers = nn.ModuleList([GteLayer(config) for _ in range(config.num_hidden_layers)])
...
class GteLMHead(nn.Module):
    """Gte Head for masked language modeling."""
...
@auto_docstring
class GteForMaskedLM(GtePreTrainedModel):
...
@auto_docstring
class GteForTokenClassification(GtePreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
@auto_docstring
class GteForQuestionAnswering(GtePreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
@auto_docstring
class GteForMultipleChoice(GtePreTrainedModel):
    def __init__(self, config: GteConfig):
        super().__init__(config)
        self.gte = GteModel(config, add_pooling_layer=True)
        classifier_dropout = (
