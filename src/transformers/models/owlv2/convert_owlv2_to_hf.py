...
from transformers import (CLIPTokenizer, Owlv2Config, Owlv2ForObjectDetection,
                          Owlv2ImageProcessor, Owlv2Processor, Owlv2TextConfig,
                          Owlv2VisionConfig)
...
@torch.no_grad()
def convert_owlv2_checkpoint(
    model_name, checkpoint_path, pytorch_dump_folder_path, push_to_hub, verify_logits
):
    """
    ...
    variables = (
        variables["params"] if "v2" in model_name else variables["optimizer"]["target"]
    )
    ...
    filepath = hf_hub_download(
        ...
        filename="owlvit_pixel_values_960.pt",
        repo_type="dataset",
    )
    original_pixel_values = torch.load(filepath).permute(0, 3, 1, 2)

    filepath = hf_hub_download(
        repo_id="nielsr/test-image", filename="owlv2_input_ids.pt", repo_type="dataset"
    )
    original_input_ids = torch.load(filepath).squeeze()

    filepath = hf_hub_download(
        repo_id="adirik/OWL-ViT", repo_type="space", filename="assets/astronaut.png"
    )
    ...
    if "large" not in model_name:
        assert torch.allclose(
            inputs.pixel_values, original_pixel_values.float(), atol=1e-6
        )
    ...
    if verify_logits:
        if model_name == "owlv2-base-patch16":
            ...
            expected_logits = torch.tensor(
                [
                    [-9.2391, -9.2313, -8.0295],
                    [-14.5498, -16.8450, -14.7166],
                    [-15.1278, -17.3060, -15.7169],
                ],
            )
            expected_boxes = torch.tensor(
                [
                    [0.0103, 0.0094, 0.0207],
                    [0.0483, 0.0729, 0.1013],
                    [0.0629, 0.1396, 0.1313],
                ]
            )
            expected_objectness_logits = torch.tensor(
            ...
            expected_logits = torch.tensor(
                [
                    [-8.6353, -9.5409, -6.6154],
                    [-7.9442, -9.6151, -6.7117],
                    [-12.4593, -15.3332, -12.1048],
                ]
            )
            expected_boxes = torch.tensor(
                [
                    [0.0126, 0.0090, 0.0238],
                    [0.0387, 0.0227, 0.0754],
                    [0.0582, 0.1058, 0.1139],
                ]
            )
            expected_objectness_logits = torch.tensor(
            ...
            expected_logits = torch.tensor(
                [
                    [-12.6662, -11.8384, -12.1880],
                    [-16.0599, -16.5835, -16.9364],
                    [-21.4957, -26.7038, -25.1313],
                ],
            )
            expected_boxes = torch.tensor(
                [
                    [0.0136, 0.0161, 0.0256],
                    [0.0126, 0.0135, 0.0202],
                    [0.0498, 0.0948, 0.0915],
                ],
            )
            expected_objectness_logits = torch.tensor(
            ...
            expected_logits = torch.tensor(
                [
                    [-9.5413, -9.7130, -7.9762],
                    [-9.5731, -9.7277, -8.2252],
                    [-15.4434, -19.3084, -16.5490],
                ],
            )
            expected_boxes = torch.tensor(
                [
                    [0.0089, 0.0080, 0.0175],
                    [0.0112, 0.0098, 0.0179],
                    [0.0375, 0.0821, 0.0528],
                ],
            )
            expected_objectness_logits = torch.tensor(
            ...
            expected_logits = torch.tensor(
                [
                    [-12.2037, -12.2070, -11.5371],
                    [-13.4875, -13.8235, -13.1586],
                    [-18.2007, -22.9834, -20.6816],
                ],
            )
            expected_boxes = torch.tensor(
                [
                    [0.0126, 0.0127, 0.0222],
                    [0.0107, 0.0113, 0.0164],
                    [0.0482, 0.1162, 0.0885],
                ],
            )
            expected_objectness_logits = torch.tensor(
        ...
        assert torch.allclose(
            objectness_logits[:3, :3], expected_objectness_logits, atol=1e-3
        )
        print("Looks ok!")
    else:
