...
"""Convert OWLv2 checkpoints from the original repository. URL:
...
@torch.no_grad()
def convert_owlv2_checkpoint(pt_backbone, flax_params, attn_params, pytorch_dump_folder_path, config_path=None):
    """
    ...
    if config_path is not None:
        config = Owlv2Config.from_pretrained(config_path)
    else:
        config = Owlv2Config()

    hf_backbone = Owlv2Model(config).eval()
    hf_model = Owlv2ForObjectDetection(config).eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="OWLv2 model name [clip_b16, clip_b32, clip_l14].",
    )
    parser.add_argument(
        "--owlv2_checkpoint", default=None, type=str, required=True, help="Path to flax model checkpoint."
    )
    ...
    variables = checkpoints.restore_checkpoint(args.owlv2_checkpoint, target=None)["optimizer"]["target"]
    ...
    convert_owlv2_checkpoint(clip_pt, flax_params, attn_params, args.pytorch_dump_folder_path, args.hf_config)
