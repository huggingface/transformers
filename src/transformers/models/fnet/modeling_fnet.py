# coding=utf-8
# Copyright 2021 James Lee-Thorp, Joshua Ainslie, Ilya Eckstein, and Santiago Ontanon The HuggingFace Inc. team. All rights reserved.
#
...
def load_tf_weights_in_fnet(model, config, tf_checkpoint_path):
...
class FNetSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in FNetModel forward() function)
...
class FNetForMaskedLM(FNetPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `FNetForMaskedLM` make sure `config.is_decoder=False` for "
...
@add_start_docstrings(
    """FNet Model with a `language modeling` head on top for CLM fine-tuning. """, FNET_START_DOCSTRING
)
class FNetForCausalLM(FNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `FNetForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import FNetTokenizer, FNetForCausalLM, FNetConfig
            >>> import torch

            >>> tokenizer = FNetTokenizer.from_pretrained('google/fnet')
            >>> config = FNetConfig.from_pretrained("google/fnet")
            >>> config.is_decoder = True
            >>> model = FNetForCausalLM.from_pretrained('google/fnet', config=config)
...
class FNetClassificationHead(nn.Module):
...
class FNetForSequenceClassification(FNetPreTrainedModel):
    def __init__(self, config):
        ...
        self.fnet = FNetModel(config)
        self.classifier = FNetClassificationHead(config)
