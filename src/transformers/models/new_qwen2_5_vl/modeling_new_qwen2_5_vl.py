#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/models/new_qwen2_5_vl/modular_new_qwen2_5_vl.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_new_qwen2_5_vl.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
from .configuration_new_qwen2_5_vl import NewQwen25VlConfig, NewQwen25VlTextConfig, NewQwen25VlVisionConfig
...
class NewQwen25VlRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        NewQwen25VlRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class NewQwen25VlMLP(nn.Module):
...
class NewQwen25VlPatchMerger(nn.Module):
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int = 2) -> None:
        super().__init__()
        self.hidden_size = context_dim * (spatial_merge_size**2)
        self.ln_q = NewQwen25VlRMSNorm(context_dim, eps=1e-6)
...
class NewQwen25VlVisionAttention(nn.Module):
    def __init__(self, config: NewQwen25VlVisionConfig) -> None:
        super().__init__()
...
class NewQwen25VlVisionBlock(GradientCheckpointingLayer):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__()
        self.norm1 = NewQwen25VlRMSNorm(config.hidden_size, eps=1e-6)
        self.norm2 = NewQwen25VlRMSNorm(config.hidden_size, eps=1e-6)
        self.attn = NewQwen25VlVisionAttention(config=config)
        self.mlp = NewQwen25VlMLP(config, bias=True)
...
@auto_docstring
class NewQwen25VlPreTrainedModel(PreTrainedModel):
    config: NewQwen25VlConfig
    base_model_prefix = "model"
    input_modalities = ("image", "video", "text")
    supports_gradient_checkpointing = True
    _no_split_modules = ["NewQwen25VlDecoderLayer", "NewQwen25VlVisionBlock"]
...
class Qwen2_5_VisionTransformerPretrainedModel(NewQwen25VlPreTrainedModel):
    config: NewQwen25VlVisionConfig
    _no_split_modules = ["NewQwen25VlVisionBlock"]
    _input_embed_layer = "patch_embed"
    _can_record_outputs = {
        "hidden_states": NewQwen25VlVisionBlock,
        "attentions": NewQwen25VlVisionAttention,
    }
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.ModuleList([NewQwen25VlVisionBlock(config) for _ in range(config.depth)])
        self.merger = NewQwen25VlPatchMerger(
...
class NewQwen25VlModelOutputWithPast(ModelOutput):
    r"""
...
class NewQwen25VlRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: NewQwen25VlConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: NewQwen25VlConfig | None = None,
    ...
    def forward(self, x, position_ids):
        # In contrast to other models, NewQwen25Vl has different position ids for the grids
...
class NewQwen25VlAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: NewQwen25VlTextConfig, layer_idx: int | None = None):
        super().__init__()
        self.config = config
...
class NewQwen25VlDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: NewQwen25VlTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = NewQwen25VlAttention(config, layer_idx)

        self.mlp = Qwen2MLP(config)
        self.input_layernorm = NewQwen25VlRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = NewQwen25VlRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class NewQwen25VlTextModel(NewQwen25VlPreTrainedModel):
    config: NewQwen25VlTextConfig
    input_modalities = ("text",)
    _can_record_outputs = {
        "hidden_states": NewQwen25VlDecoderLayer,
        "attentions": NewQwen25VlAttention,
    }

    def __init__(self, config: NewQwen25VlTextConfig):
        ...
        self.layers = nn.ModuleList(
            [NewQwen25VlDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = NewQwen25VlRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.has_sliding_layers = "sliding_attention" in self.config.layer_types
        self.rotary_emb = NewQwen25VlRotaryEmbedding(config=config)
...
@auto_docstring
class NewQwen25VlModel(NewQwen25VlPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config):
        ...
        self.language_model = NewQwen25VlTextModel._from_config(config.text_config)
    ...
    def forward(
        ...
        second_per_grid_ts: torch.Tensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | NewQwen25VlModelOutputWithPast:
        r"""
        image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
            The temporal, height and width of feature shape of each image in LLM.
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
        ...
        return NewQwen25VlModelOutputWithPast(
...
@auto_docstring(
    custom_intro="""
    Base class for NewQwen25Vl causal language model (or autoregressive) outputs.
    """
)
class NewQwen25VlCausalLMOutputWithPast(ModelOutput):
    r"""
...
class NewQwen25VlForConditionalGeneration(NewQwen25VlPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = NewQwen25VlModel(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | NewQwen25VlCausalLMOutputWithPast:
        r"""
        ...
        ```python
        >>> from transformers import AutoProcessor, NewQwen25VlForConditionalGeneration

        >>> model = NewQwen25VlForConditionalGeneration.from_pretrained("Qwen/Qwen2.5-VL-7B-Instruct")
        ...
        return NewQwen25VlCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
