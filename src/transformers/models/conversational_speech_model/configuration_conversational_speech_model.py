...
"""ConversationalSpeechModel model configuration"""
...
class ConversationalSpeechModelDepthConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`ConversationalSpeechModelDepthDecoder`]. It is used to instantiate a
    ConversationalSpeechModel depth decoder model according to the specified arguments, defining the ConversationalSpeechModel depth decoder config.
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 32000):
            Vocabulary size of the ConversationalSpeechModelDepthDecoder model. Defines the number of different tokens that can be
            represented by the `inputs_ids` passed when calling [`ConversationalSpeechModelDepthDecoder`].
        ...
        audio_vocab_size (`int`, *optional*, defaults to 2048):
            ...
            represented by the `audio_codes` passed when calling the ConversationalSpeechModel models.
    ...
    >>> configuration = ConversationalSpeechModelDepthConfig()

    >>> # Initializing a ConversationalSpeechModelDepthDecoder (with random weights) from the eustlb/csm style configuration
    >>> model = ConversationalSpeechModelDepthDecoder(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config
    ```"""

    model_type = "conversational_speech_model_depth"
...
class ConversationalSpeechModelConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`ConversationalSpeechModelModel`]. It is used to instantiate a
    ConversationalSpeechModel model according to the specified arguments, defining the audio encoder, ConversationalSpeechModel depth decoder and ConversationalSpeechModel decoder
    configs. Instantiating a configuration with the defaults will yield a similar configuration to that of the ConversationalSpeechModelko model,
    e.g. [eustlb/csm](https://huggingface.co/eustlb/csm)
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 32000):
            Vocabulary size of the ConversationalSpeechModelDecoder model. Defines the number of different tokens that can be
            represented by the `inputs_ids` passed when calling [`ConversationalSpeechModelDecoder`].
    ...
    >>> configuration = ConversationalSpeechModelConfig()

    >>> # Initializing a ConversationalSpeechModelForConditionalGeneration (with random weights) from the eustlb/csm style configuration
    >>> model = ConversationalSpeechModelForConditionalGeneration(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config

    >>> # Saving the model, including its configuration
    >>> model.save_pretrained("eustlb/csm")

    >>> # loading model and config from pretrained folder
    >>> conversational_speech_model_config = ConversationalSpeechModelConfig.from_pretrained("eustlb/csm")
    >>> model = ConversationalSpeechModelForConditionalGeneration.from_pretrained("eustlb/csm", config=conversational_speech_model_config)
    ```"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.depth_decoder_config = ConversationalSpeechModelDepthConfig(**depth_decoder_config)
    ...
    def from_audio_encoder_config(
        ...
        **kwargs,
        ...
        r"""
        Instantiate a [`ConversationalSpeechModelConfig`] (or a derived class) from an audio encoder configuration.

        Returns:
            [`ConversationalSpeechModelConfig`]: An instance of a configuration object
        """
...
__all__ = ["ConversationalSpeechModelConfig", "ConversationalSpeechModelDepthConfig"]
