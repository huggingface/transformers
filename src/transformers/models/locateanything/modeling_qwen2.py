...
from .configuration_locateanything import Qwen2Config
...
class Qwen2MagiAttention(Qwen2Attention):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: dict | None = None,  # magi_plan dict
        position_ids: torch.LongTensor | None = None,
        past_key_value: Cache | None = None,
        output_attentions: bool = False,
        use_cache: bool = False,
        ...
        bsz, q_len, _ = hidden_states.size()
        if bsz != 1:
            raise ValueError("MagiAttention only supports batch_size=1 (use packing instead).")
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        if config._attn_implementation == "magi" and not _MAGI_AVAILABLE:
            if is_flash_attn_2_available():
                logger.warning_once("magi_attention not available, falling back to flash_attention_2")
                config._attn_implementation = "flash_attention_2"
            else:
                logger.warning_once("magi_attention not available, falling back to sdpa")
                config._attn_implementation = "sdpa"
        if config._attn_implementation == "flash_attention_2" and not is_flash_attn_2_available():
            logger.warning_once("flash_attn is not available, falling back to sdpa")
            config._attn_implementation = "sdpa"
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def image_processing(self, input_ids, visual_features, image_token_index):
        if visual_features is not None:
            ...
            if selected.sum() == 0:
                raise ValueError("No image token found in `input_ids` for the provided visual features.")
    ...
    def forward(
        ...
        if self._attn_implementation == "magi":
            ...
            attention_mask = (
                _prepare_block_mask_for_training()
                if self.training
                else _prepare_block_mask_for_inference(attention_mask)
            )
        ...
        if labels is not None:
            ...
            for ix in range(max_n_future_tokens):
                seg_loss = F.cross_entropy(
                    shift_logits[shift_pos_masks == ix], shift_labels[shift_pos_masks == ix], reduction="mean"
                )
