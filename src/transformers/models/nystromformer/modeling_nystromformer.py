# coding=utf-8
# Copyright 2021 novice03 The HuggingFace Inc. team. All rights reserved.
#
...
class NystromformerLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = NystromformerAttention(config, position_embedding_type="absolute")
...
class NystromformerPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_nystromformer
...
NYSTROMFORMER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            ...
            Indices can be obtained using :class:`transformers.NystromformerTokenizer`.
...
@add_start_docstrings(
    "The bare Nystromformer Model transformer outputting raw hidden-states without any specific head on top.",
...
@add_start_docstrings("""Nystromformer Model with a `language modeling` head on top. """, NYSTROMFORMER_START_DOCSTRING)
...
@add_start_docstrings(
    """Nystromformer Model with a `language modeling` head on top for CLM fine-tuning. """, NYSTROMFORMER_START_DOCSTRING
)
class NystromformerForCausalLM(NystromformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `NystromformerForCausalLM` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import NystromformerTokenizer, NystromformerForCausalLM, NystromformerConfig
            >>> import torch

            >>> tokenizer = NystromformerTokenizer.from_pretrained('nystromformer-roberta')
            >>> config = NystromformerConfig.from_pretrained("nystromformer-roberta")
            >>> config.is_decoder = True
            >>> model = NystromformerForCausalLM.from_pretrained('nystromformer-roberta', config=config)
...
@add_start_docstrings(
    """Nystromformer Model transformer with a sequence classification/regression head on top (a linear layer on top of
...
@add_start_docstrings(
    """Nystromformer Model with a multiple choice classification head on top (a linear layer on top of
...
@add_start_docstrings(
    """Nystromformer Model with a token classification head on top (a linear layer on top of
...
@add_start_docstrings(
    """Nystromformer Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
