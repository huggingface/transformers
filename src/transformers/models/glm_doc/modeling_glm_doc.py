#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/models/glm_doc/modular_glm_doc.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_glm_doc.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
class GlmDocTextAttention(nn.Module):
    """
...
class GlmDocTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: GlmDocTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = GlmDocTextAttention(config, layer_idx)
        self.mlp = GlmDocTextMLP(config)
        self.input_layernorm = GlmDocRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = GlmDocRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_self_attn_layernorm = GlmDocRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_mlp_layernorm = GlmDocRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class GlmDocPreTrainedModel(PreTrainedModel):
    config: GlmDocConfig
    base_model_prefix = "model"
    input_modalities = ("image", "video", "text")
    supports_gradient_checkpointing = True
    _no_split_modules = ["GlmDocTextDecoderLayer", "GlmDocVisionBlock"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GlmDocVisionRotaryEmbedding):
...
class GlmDocModelOutputWithPast(ModelOutput):
    r"""
...
class GlmDocVisionAttention(nn.Module):
...
class GlmDocVisionBlock(GradientCheckpointingLayer):
...
class GlmDocTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: GlmDocTextConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: GlmDocTextConfig | None = None,
...
@auto_docstring
class GlmDocTextModel(GlmDocPreTrainedModel):
    config: GlmDocTextConfig
    input_modalities = ("text",)

    def __init__(self, config: GlmDocTextConfig):
        super().__init__(config)
        ...
        self.layers = nn.ModuleList(
            [GlmDocTextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = GlmDocRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = GlmDocTextRotaryEmbedding(config=config)
...
@auto_docstring
class GlmDocModel(GlmDocPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = GlmDocVisionModel._from_config(config.vision_config)
        self.language_model = GlmDocTextModel._from_config(config.text_config)
    ...
    def forward(
        ...
        cache_position: torch.LongTensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | GlmDocModelOutputWithPast:
        r"""
        image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
            The temporal, height and width of feature shape of each image in LLM.
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
        ...
        return GlmDocModelOutputWithPast(
...
@auto_docstring(
    custom_intro="""
    Base class for GlmDoc causal language model (or autoregressive) outputs.
    """
)
class GlmDocCausalLMOutputWithPast(ModelOutput):
    r"""
...
class GlmDocForConditionalGeneration(GlmDocPreTrainedModel, GenerationMixin):
    _checkpoint_conversion_mapping = {}
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | GlmDocCausalLMOutputWithPast:
        r"""
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, GlmDocForConditionalGeneration

        >>> model = GlmDocForConditionalGeneration.from_pretrained("THUDM/GLM-4.1V-9B-Thinking")
        ...
        return GlmDocCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
...
__all__ = ["GlmDocModel", "GlmDocPreTrainedModel", "GlmDocForConditionalGeneration"]
