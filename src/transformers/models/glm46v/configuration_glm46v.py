...
class Glm46VTextConfig(PreTrainedConfig):
    r"""
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 151552):
            Vocabulary size of the Glm46V model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`Glm46VModel`]
    ...
    ```python
    >>> from transformers import Glm46VTextModel, Glm46VConfig
    ...
    >>> model = Glm46VTextModel(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config
    ```"""

    model_type = "glm46v_text"
    base_config_key = "text_config"
    keys_to_ignore_at_inference = ["past_key_values"]
    # Default tensor parallel plan for base model `Glm46V`
    base_model_tp_plan = {
...
class Glm46VVisionConfig(PreTrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`Glm46VVisionModel`]. It is used to instantiate an Glm46VVisionModel
    ...
    ```python
    >>> from transformers import Glm46VVisionConfig, Glm46VVisionModel

    >>> # Initializing a Glm46VVisionConfig GLM-4.1V-9B style configuration
    >>> configuration = Glm46VVisionConfig()

    >>> # Initializing a model (with random weights) from the GLM-4.1V-9B configuration
    >>> model = Glm46VVisionModel(configuration)
...
class Glm46VConfig(PreTrainedConfig):
    ...
    model_type = "glm46v"
    sub_configs = {"vision_config": Glm46VVisionConfig, "text_config": Glm46VTextConfig}
    keys_to_ignore_at_inference = ["past_key_values"]
...
__all__ = ["Glm46VConfig", "Glm46VTextConfig"]
