...
from .configuration_glm46v import Glm46VConfig, Glm46VTextConfig, Glm46VVisionConfig
...
class Glm46VVisionPatchEmbed(nn.Module):
    def __init__(self, config: Glm46VVisionConfig) -> None:
        super().__init__()
...
class Glm46VVisionRotaryEmbedding(nn.Module):
...
class Glm46VVisionPatchMerger(nn.Module):
...
class Glm46VVisionEmbeddings(nn.Module):
    def __init__(self, config: Glm46VVisionConfig):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
class Glm46VVisionAttention(nn.Module):
    def __init__(self, config: Glm46VVisionConfig) -> None:
        super().__init__()
...
class Glm46VisionMlp(nn.Module):
    def __init__(self, config, bias: bool = False):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.out_hidden_size
...
class Glm46VVisionBlock(GradientCheckpointingLayer):
    def __init__(self, config) -> None:
        super().__init__()
        self.norm1 = Glm46VRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.norm2 = Glm46VRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.attn = Glm46VVisionAttention(config)
        self.mlp = Glm46VisionMlp(config, bias=False)
...
class Glm46VTextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: Glm46VTextConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: Optional[Glm46VTextConfig] = None,
    ...
    def forward(self, x, position_ids):
        # In contrast to other models, GLM46V different position ids for the grids
...
class Glm46VVisionModel(Glm46VPreTrainedModel):
    config: Glm46VVisionConfig
    input_modalities = ["image", "video"]
    _no_split_modules = ["Glm46VVisionBlock"]
    ...
    def __init__(self, config) -> None:
        ...
        self.embeddings = Glm46VVisionEmbeddings(config)
        self.patch_embed = Glm46VVisionPatchEmbed(config)

        head_dim = config.hidden_size // config.num_heads
        self.rotary_pos_emb = Glm46VVisionRotaryEmbedding(head_dim // 2)

        self.blocks = nn.ModuleList([Glm46VVisionBlock(config) for _ in range(config.depth)])
        self.merger = Glm46VVisionPatchMerger(
            dim=config.out_hidden_size, context_dim=config.intermediate_size, hidden_act=config.hidden_act
        )

        self.post_conv_layernorm = Glm46VRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class Glm46VTextModel(Glm46VPreTrainedModel):
    config: Glm46VTextConfig
    input_modalities = "text"

    def __init__(self, config: Glm46VTextConfig):
        super().__init__(config)
        ...
        self.layers = nn.ModuleList(
            [Glm46VTextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Glm46VRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Glm46VTextRotaryEmbedding(config=config)
...
class Glm46VModel(Glm46VPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = Glm46VVisionModel._from_config(config.vision_config)
        self.language_model = Glm46VTextModel._from_config(config.text_config)
...
__all__ = ["Glm46VForConditionalGeneration", "Glm46VModel", "Glm46VPreTrainedModel", "Glm46VTextModel"]
