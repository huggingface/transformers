...
class Gemma3nModelOutputWithPast(BaseModelOutputWithPast):
    """
    Base class for Gemma 3n outputs, with hidden states and attentions.
...
class Gemma3nCausalLMOutputWithPast(ModelOutput):
    """
    Base class for Gemma 3n causal language model (or autoregressive) outputs.
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        logits_to_keep (`int` or `torch.Tensor`, *optional*):
            ...
            `input_ids` (special case). Only last token logits are needed for generation, and calculating them only for
            that token can save memory, which becomes pretty significant for long sequences or large vocabulary size.
