...
@auto_docstring(
    custom_intro="""
    Outputs of [`Lfm2AudioForConditionalConditionalGeneration.generate`].
    """
)
class Lfm2AudioConditionalGenerationGenerateOutput(ModelOutput):
    r"""
...
@auto_docstring(
    custom_intro="""
    `Lfm2AudioForCausalLM` outputs.
    """
)
class Lfm2AudioCausalLMOutputWithPast(ModelOutput):
    r"""
...
@auto_docstring(
    custom_intro="""
    `Lfm2AudioForConditionalGeneration` outputs.
    """
)
class Lfm2AudioConditionalGenerationOutputWithPast(ModelOutput):
    r"""
...
@dataclass
@auto_docstring
class Lfm2AudioUnconditionalInput(ModelOutput):
    r"""
    ...
    lfm2_audio_audio_codes (`torch.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
        The audio codes used as audio Lfm2Audio prompt for the generation. Has priority over `lfm2_audio_input_values` and represents the audio "tokens" of `lfm2_audio_input_values` once passed through the audio encoder.
    ...
    lfm2_audio_audio_codes: torch.Tensor | None = None
...
class Lfm2AudioFlexibleLinear(nn.Module):
    ...
    def forward(self, x, layer_idx=None):
        """
        `Lfm2AudioFlexibleLinear` creates one linear layer per codebook. There's multiple ways to use it.
...
class Lfm2AudioLinear(nn.Module):
    def __init__(self, input_dim, output_dim, num_codebooks, use_flexible_linear=False):
        ...
        if not use_flexible_linear:
            ...
            self.linear = Lfm2AudioFlexibleLinear(input_dim, output_dim, num_layers=num_codebooks)
...
class Lfm2AudioRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: Lfm2AudioConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: Lfm2AudioConfig | None = None,
...
class Lfm2AudioGatingMLP(nn.Module):
    def __init__(self, config, use_flexible_linear=False):
        ...
        if num_layers == 1:
            ...
            self.fc1 = Lfm2AudioFlexibleLinear(hidden_size, ffn_dim, num_layers)
            self.fc2 = Lfm2AudioFlexibleLinear(ffn_dim // 2, hidden_size, num_layers)
...
class Lfm2AudioAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(
        self, config: Lfm2AudioConfig, layer_idx: int | None = None, use_flexible_linear=False, use_rope=True
    ):
        super().__init__()
        self.config = config
        ...
        if use_rope:
            self.rotary_emb = Lfm2AudioRotaryEmbedding(config)
...
# NO LONGER EXIST Copied from transformers.models.gemma.modeling_gemma.GemmaFlashAttention2 with Gemma->Lfm2Audio
# TODO cyril: modular
class Lfm2AudioFlashAttention2(Lfm2AudioAttention):
    """
    Lfm2Audio flash attention module. This module inherits from `Lfm2AudioAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # in fp32. (Lfm2AudioRMSNorm handles it correctly)
...
class Lfm2AudioSdpaAttention(Lfm2AudioAttention):
    """
    Lfm2Audio attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `Lfm2AudioAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from Lfm2AudioAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: torch.Tensor | None = None,
...
LFM2_AUDIO_ATTENTION_CLASSES = {
    "eager": Lfm2AudioAttention,
    "flash_attention_2": Lfm2AudioFlashAttention2,
    "sdpa": Lfm2AudioSdpaAttention,
}


class Lfm2AudioDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Lfm2AudioConfig, layer_idx: int, use_flexible_linear: bool, use_rope=True):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.use_flexible_linear = use_flexible_linear

        self.self_attn = LFM2_AUDIO_ATTENTION_CLASSES[config._attn_implementation](
            config=config, layer_idx=layer_idx, use_flexible_linear=use_flexible_linear, use_rope=use_rope
        )

        self.mlp = Lfm2AudioGatingMLP(config, use_flexible_linear)
        self.input_layernorm = Lfm2AudioRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Lfm2AudioRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class Lfm2AudioPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Lfm2AudioDecoderLayer", "MimiTransformerLayer"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Lfm2AudioFlexibleLinear):
            init.normal_(module.weight)


class Lfm2AudioDepthDecoder(Lfm2AudioPreTrainedModel, GenerationMixin):
    """
    Transformer depth decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Lfm2AudioTransformerLayer`]

    Args:
        config: Lfm2AudioConfig
    """

    config: Lfm2AudioDepthConfig
    ...
    def __init__(self, config: Lfm2AudioDepthConfig):
        ...
        self.input_projections = Lfm2AudioFlexibleLinear(config.input_size, config.hidden_size, config.num_codebooks)

        self.layers = nn.ModuleList(
            [
                Lfm2AudioDecoderLayer(config, layer_idx, use_flexible_linear=True, use_rope=False)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )

        self.lm_heads = Lfm2AudioFlexibleLinear(config.hidden_size, config.audio_vocab_size, config.num_codebooks)
        self._attn_implementation = config._attn_implementation
...
class Lfm2AudioModel(Lfm2AudioPreTrainedModel):
    def __init__(self, config: Lfm2AudioConfig):
        ...
        self.layers = nn.ModuleList(
            [
                Lfm2AudioDecoderLayer(config, layer_idx, use_flexible_linear=False)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = Lfm2AudioRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.gradient_checkpointing = False
...
@auto_docstring(
    custom_intro="""
    The Lfm2Audio decoder model with a text language modelling head on top. Only usable for text.
    """
)
class Lfm2AudioForCausalLM(Lfm2AudioPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
    ) -> tuple | Lfm2AudioCausalLMOutputWithPast:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
            config.vocab_size]` or -100 (see `input_ids` docstring). Tokens with indices set to `-100` are ignored
        ...
        ```python
        >>> from transformers import AutoTokenizer, Lfm2AudioForCausalLM

        >>> model = Lfm2AudioForCausalLM.from_pretrained("kmhf/hf-lfm2_audioko")
        >>> tokenizer = AutoTokenizer.from_pretrained("kmhf/hf-lfm2_audioko")
        ...
        return Lfm2AudioCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
@auto_docstring(
    custom_intro="""
    The original Lfm2Audio model with an audio encoder, a Lfm2Audio depth decoder and a Lfm2Audio decoder, for speech-to-speech.
    """
)
class Lfm2AudioForConditionalGeneration(Lfm2AudioPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Lfm2AudioConfig):
        ...
        self.decoder = Lfm2AudioForCausalLM(config)

        self.depth_decoder = Lfm2AudioDepthDecoder._from_config(config.depth_decoder_config)
    ...
    def forward(
        ...
        user_audio_codes: torch.Tensor | None = None,
        lfm2_audio_input_values: torch.FloatTensor | None = None,
        lfm2_audio_audio_codes: torch.Tensor | None = None,
        past_key_values: Cache | None = None,
        inputs_embeds: torch.FloatTensor | None = None,
        ...
        **kwargs,
        ...
        user_audio_codes (`torch.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio user prompt for the generation. Has priority over `user_input_values` and represents the audio "tokens" of `user_input_values` once passed through the audio encoder.
        lfm2_audio_input_values (`torch.Tensor `of shape `(batch_size, 1, audio_sequence_length), *optional*):
            The audio waveforms used as audio Lfm2Audio prompt for the generation.
        lfm2_audio_audio_codes (`torch.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio Lfm2Audio prompt for the generation. Has priority over `lfm2_audio_input_values` and represents the audio "tokens" of `lfm2_audio_input_values` once passed through the audio encoder.
        inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        ...
        >>> model = Lfm2AudioForConditionalGeneration.from_pretrained("kmhf/hf-lfm2_audioko")
        >>> inputs = lfm2_audio.get_unconditional_inputs()
        ...
        if inputs_embeds is None:
            ...
            if lfm2_audio_input_values is not None and lfm2_audio_audio_codes is None:
                lfm2_audio_audio_codes = self.audio_encoder.encode(
                    lfm2_audio_input_values, num_quantizers=self.num_codebooks, **kwargs_audio_encoder
                )[0]

            audio_codes = torch.cat([lfm2_audio_audio_codes, user_audio_codes], dim=1)
        ...
        return Lfm2AudioConditionalGenerationOutputWithPast(
    ...
    def _prepare_inputs_embeds_for_generation(
        ...
        lfm2_audio_delay_pattern_mask = None
        ...
        if (
            ...
            and lfm2_audio_input_values is None
            and lfm2_audio_audio_codes is None
        ):
            raise ValueError(
                "You must provide at least one of `input_ids`, `user_input_values`, `lfm2_audio_input_values`, `user_audio_codes`, `lfm2_audio_audio_codes` or `inputs_embeds`."
            )
        ...
        if inputs_embeds is None or apply_delay_pattern_mask:
            ...
            if lfm2_audio_input_values is not None and lfm2_audio_audio_codes is None:
                lfm2_audio_audio_codes = self.audio_encoder.encode(
                    lfm2_audio_input_values, num_quantizers=self.num_codebooks
                )[0]
        ...
        if inputs_embeds is None and concat_unconditional_inputs:
            ...
            lfm2_audio_audio_codes = torch.cat(
                [unconditional_inputs.lfm2_audio_audio_codes, lfm2_audio_audio_codes], dim=2
            )
        ...
        if inputs_embeds is None or apply_delay_pattern_mask:
            ...
            if apply_delay_pattern_mask and lfm2_audio_audio_codes is not None:
                lfm2_audio_audio_codes, lfm2_audio_delay_pattern_mask = self.build_delay_pattern_mask(
                    lfm2_audio_audio_codes,
                    bos_token_id=self.config.audio_vocab_size,
                    pad_token_id=self.config.audio_vocab_size,
                    max_length=generation_config.max_length,
                )
        ...
        if inputs_embeds is None:
            ...
            if user_audio_codes is not None and lfm2_audio_audio_codes is not None:
                audio_codes = torch.cat([lfm2_audio_audio_codes, user_audio_codes], dim=1)
                audio_inputs_embeds = sum(
                    self.embed_tokens[codebook](audio_codes[:, codebook]) for codebook in range(audio_codes.shape[1])
                )
            elif lfm2_audio_audio_codes is not None:
                audio_codes = lfm2_audio_audio_codes
                audio_inputs_embeds = sum(
                    self.embed_tokens[codebook](audio_codes[:, codebook]) for codebook in range(audio_codes.shape[1])
                )
    ...
    def generate(
        ...
        **kwargs,
        ...
        Return:
            [`Lfm2AudioConditionalGenerationGenerateOutput`]
        """
        ...
        input_ids, user_audio_codes, lfm2_audio_audio_codes, concat_unconditional_inputs = (
            self._check_and_maybe_initialize_inputs(
                input_ids=input_ids,
                user_input_values=user_input_values,
                user_audio_codes=user_audio_codes,
                lfm2_audio_input_values=lfm2_audio_input_values,
                lfm2_audio_audio_codes=lfm2_audio_audio_codes,
                inputs_embeds=inputs_embeds,
        ...
        # create blank user inputs - lfm2_audio needs a constant stream of user inputs
        ...
        kwargs["lfm2_audio_delay_pattern_mask"] = (
            lfm2_audio_delay_pattern_mask
            if lfm2_audio_delay_pattern_mask is not None
            else kwargs.get("lfm2_audio_delay_pattern_mask")
        )

        self.generated_audio_codes = torch.repeat_interleave(
            lfm2_audio_audio_codes, max(generation_config.num_beams, generation_config.num_return_sequences), dim=0
        )
        ...
        if generation_config.num_return_sequences > 1:
            lfm2_audio_delay_pattern_mask = torch.repeat_interleave(
                lfm2_audio_delay_pattern_mask, generation_config.num_return_sequences, dim=0
            )
        ...
        if generation_config.num_beams > 1:
            ...
            beam_indices = outputs.beam_indices[:, : -lfm2_audio_audio_codes.shape[-1]]

            generated_audio_codes = self.generated_audio_codes[:, :, lfm2_audio_audio_codes.shape[-1] :]
            ...
            lfm2_audio_audio_codes = torch.repeat_interleave(
                lfm2_audio_audio_codes, generation_config.num_return_sequences, dim=0
            )
            self.generated_audio_codes = torch.cat((lfm2_audio_audio_codes, generated_audio_codes), dim=2)
        ...
        output_audio_codes = self.apply_delay_pattern_mask(self.generated_audio_codes, lfm2_audio_delay_pattern_mask)

        # revert the pattern delay mask by filtering the pad token id and bos token ids
        mask = lfm2_audio_delay_pattern_mask != self.config.audio_vocab_size
        ...
        if generation_config.return_dict_in_generate:
            return Lfm2AudioConditionalGenerationGenerateOutput(
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Overwritten -- Lfm2Audio has custom post-processing on the prepared inputs.
        ...
        model_inputs = super().prepare_inputs_for_generation(
            ...
            lfm2_audio_delay_pattern_mask=lfm2_audio_delay_pattern_mask,
            kwargs_depth_decoder=kwargs_depth_decoder,
        ...
        if model_inputs["input_ids"] is not None:
            ...
            self.generated_audio_codes = self.apply_delay_pattern_mask(
                torch.cat([self.generated_audio_codes, generated_audio_codes], dim=2), lfm2_audio_delay_pattern_mask
            )

            inputs_embeds, _, _, _, _, _, _ = self._prepare_inputs_embeds_for_generation(
                input_ids,
                lfm2_audio_audio_codes=self.generated_audio_codes[:, :, -1:],
                user_audio_codes=user_audio_codes,
            )
    ...
    def get_unconditional_inputs(self, num_samples=1):
        ...
        >>> model = Lfm2AudioForConditionalGeneration.from_pretrained("kmhf/hf-lfm2_audioko-pytorch-bf16")
        ...
        return Lfm2AudioUnconditionalInput(
            input_ids=input_ids,
            user_audio_codes=user_audio_codes,
            lfm2_audio_audio_codes=lfm2_audio_audio_codes,
            attention_mask=attention_mask,
    ...
    def _check_and_maybe_initialize_inputs(
        ...
        lfm2_audio_input_values=None,
        lfm2_audio_audio_codes=None,
        inputs_embeds=None,
        ...
        lfm2_audio_input = lfm2_audio_audio_codes if lfm2_audio_input_values is None else lfm2_audio_input_values

        one_input_has_been_passed = (user_input is not None) or (lfm2_audio_input is not None) or (inputs is not None)
        ...
        if one_input_has_been_passed and (user_input is None):
            raise ValueError(
                "No user audio inputs have been passed alongside the other inputs. Make sure either `user_input_values` or `user_audio_codes` is passed or use `Lfm2AudioForConditionalGeneration.get_unconditional_inputs`. Check the `Lfm2AudioForConditionalGeneration` docstrings for more information."
            )
        elif one_input_has_been_passed and (lfm2_audio_input is None):
            raise ValueError(
                "No Lfm2Audio audio inputs have been passed alongside the other inputs. Make sure either `lfm2_audio_input_values` or `lfm2_audio_audio_codes` is passed or use `Lfm2AudioForConditionalGeneration.get_unconditional_inputs`. Check the `Lfm2AudioForConditionalGeneration` docstrings for more information."
            )
        elif one_input_has_been_passed and (inputs is None):
            raise ValueError(
                "No `input_ids` or `inputs_embeds` have been passed alongside the other inputs. Make sure `input_ids` is passed or use `Lfm2AudioForConditionalGeneration.get_unconditional_inputs`. Check the `Lfm2AudioForConditionalGeneration` docstrings for more information."
            )
            ...
            lfm2_audio_audio_codes = unconditional_inputs.lfm2_audio_audio_codes
            ...
            lfm2_audio_seq_length = lfm2_audio_input.shape[-1]
            tokens_seq_length = inputs.shape[1]

            ratio = self.config.audio_encoder_config.frame_rate / self.config.sampling_rate
            lfm2_audio_seq_length = (
                math.ceil(lfm2_audio_seq_length * ratio) if lfm2_audio_audio_codes is None else lfm2_audio_seq_length
            )
            user_seq_length = math.ceil(user_seq_length * ratio) if user_audio_codes is None else user_seq_length

            if tokens_seq_length != lfm2_audio_seq_length or tokens_seq_length != user_seq_length:
                raise ValueError(
                    "At least one of the 3 inputs of `Lfm2AudioForConditionalGeneration` doesn't have the same sequence length as the others."
                    "Make sure that they all have the same sequence length. Check the `Lfm2AudioForConditionalGeneration` docstrings for more information."
                )

        return input_ids, user_audio_codes, lfm2_audio_audio_codes, concat_unconditional_inputs


__all__ = ["Lfm2AudioForCausalLM", "Lfm2AudioForConditionalGeneration", "Lfm2AudioModel", "Lfm2AudioPreTrainedModel"]
