...
# Copied from transformers.models.gpt2.modeling_gpt2.load_tf_weights_in_gpt2 with gpt2->bigscience176b
def load_tf_weights_in_bigscience176b(model, config, bigscience176b_checkpoint_path):
    ...
    try:
        ...
        raise
    tf_path = os.path.abspath(bigscience176b_checkpoint_path)
...
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2Attention with GPT2->BigScience176B
        ...
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    ...
                    "Please make sure to instantiate class with `BigScience176BAttention(..., is_cross_attention=True)`."
                )
...
class BigScience176BBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = BigScience176BAttention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = BigScience176BAttention(config, is_cross_attention=True, layer_idx=layer_idx)
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        self.mlp = BigScience176BMLP(inner_dim, config)
...
class BigScience176BPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_bigscience176b
...
@dataclass
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2DoubleHeadsModelOutput with GPT2->BigScience176B
class BigScience176BDoubleHeadsModelOutput(ModelOutput):
    """
    ...
    Args:
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            BigScience176BAttentions weights after the attention softmax, used to compute the weighted average in the
...
class BigScience176BModel(BigScience176BPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([BigScience176BBlock(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
@add_start_docstrings(
    """
The BigScience176B Model transformer with a language modeling and a multiple-choice classification head on top e.g. for
...
class BigScience176BDoubleHeadsModel(BigScience176BPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
    ...
    @add_start_docstrings_to_model_forward(BIGSCIENCE176B_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=BigScience176BDoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        past_key_values=None,
        ...
        **kwargs,
        ...
        ```python
        >>> import torch
        >>> from transformers import BigScience176BTokenizer, BigScience176BDoubleHeadsModel

        >>> tokenizer = BigScience176BTokenizer.from_pretrained("bigscience176b")
        >>> model = BigScience176BDoubleHeadsModel.from_pretrained("bigscience176b")
        ...
        return BigScience176BDoubleHeadsModelOutput(
            loss=lm_loss,
            mc_loss=mc_loss,
            logits=lm_logits,
            mc_logits=mc_logits,
...
@add_start_docstrings(
    """
    The BIGSCIENCE176B Model transformer with a sequence classification head on top (linear layer).

    [`BigScience176BForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-1) do.
...
class BigScience176BForSequenceClassification(BigScience176BPreTrainedModel):
...
@add_start_docstrings(
    """
    BIGSCIENCE176B Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """,
    BIGSCIENCE176B_START_DOCSTRING,
)
class BigScience176BForTokenClassification(BigScience176BPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
