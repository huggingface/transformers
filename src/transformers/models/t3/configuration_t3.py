...
class T3Config(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`T3Model`]. It is used to instantiate a T3
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the T3
    [ResembleAI/t3_cfg](https://huggingface.co/ResembleAI/t3_cfg) architecture.
    ...
    ```python
    >>> from transformers import T3Config, T3Model

    >>> # Initializing a T3 English-only configuration
    >>> configuration = T3Config.english_only()

    >>> # Initializing a model (with random weights) from the configuration
    >>> model = T3Model(configuration)
