...
class PixoEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        # add the [CLS] tokens to the embedded patch tokens
...
# Copied from transformers.models.vit.modeling_vit.ViTSelfAttention with ViT->Pixo
class PixoSelfAttention(nn.Module):
...
# Copied from transformers.models.vit.modeling_vit.ViTSelfOutput with ViT->Pixo
class PixoSelfOutput(nn.Module):
    """
    The residual connection is defined in PixoLayer instead of here (as is the case with other models), due to the
...
# Copied from transformers.models.vit.modeling_vit.ViTAttention with ViT->Pixo
class PixoAttention(nn.Module):
    def __init__(self, config: PixoConfig):
        super().__init__()
        self.attention = PixoSelfAttention(config)
        self.output = PixoSelfOutput(config)
...
class PixoLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        # in Pixo, layernorm is also applied after self-attention
...
@auto_docstring
class PixoPreTrainedModel(PreTrainedModel):
    config: PixoConfig
    base_model_prefix = "pixo"
...
class PixoBackbone(PixoPreTrainedModel, BackboneMixin):
    def __init__(self, config):
        super().__init__(config)
