#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/models/moshi_asr/modular_moshi_asr.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_moshi_asr.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# coding=utf-8
...
_CONFIG_FOR_DOC = "MoshiAsrConfig"
...
class MoshiAsrRMSNorm(nn.Module):
...
class MoshiAsrFlexibleLinear(nn.Module):
    ...
    def forward(self, x, layer_idx=None):
        """
        `MoshiAsrFlexibleLinear` creates one linear layer per codebook. There's multiple ways to use it.
...
class MoshiAsrLinear(nn.Module):
    def __init__(self, input_dim, output_dim, num_codebooks, use_flexible_linear=False):
        ...
        if not use_flexible_linear:
            ...
            self.linear = MoshiAsrFlexibleLinear(input_dim, output_dim, num_layers=num_codebooks)
...
class MoshiAsrRotaryEmbedding(nn.Module):
    def __init__(self, config: MoshiAsrConfig, device=None):
        super().__init__()
...
class MoshiAsrGatingMLP(nn.Module):
    def __init__(self, config, use_flexible_linear=False):
        ...
        if num_layers == 1:
            ...
            self.fc1 = MoshiAsrFlexibleLinear(hidden_size, ffn_dim, num_layers)
            self.fc2 = MoshiAsrFlexibleLinear(ffn_dim // 2, hidden_size, num_layers)
...
class MoshiAsrAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(
        self, config: MoshiAsrConfig, layer_idx: Optional[int] = None, use_flexible_linear=False, use_rope=True
    ):
        super().__init__()
        self.config = config
        ...
        if use_rope:
            ...
            self.rotary_emb = MoshiAsrRotaryEmbedding(config)
...
# NO LONGER EXIST Copied from transformers.models.gemma.modeling_gemma.GemmaFlashAttention2 with Gemma->MoshiAsr
# TODO cyril: modular
class MoshiAsrFlashAttention2(MoshiAsrAttention):
    """
    MoshiAsr flash attention module. This module inherits from `MoshiAsrAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # in fp32. (MoshiAsrRMSNorm handles it correctly)
...
# NO LONGER EXIST Copied from transformers.models.gemma.modeling_gemma.GemmaSdpaAttention with Gemma->MoshiAsr
# TODO cyril: modular
class MoshiAsrSdpaAttention(MoshiAsrAttention):
    """
    MoshiAsr attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `MoshiAsrAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from MoshiAsrAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "MoshiAsrModel is using MoshiAsrSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
MOSHI_ASR_ATTENTION_CLASSES = {
    "eager": MoshiAsrAttention,
    "flash_attention_2": MoshiAsrFlashAttention2,
    "sdpa": MoshiAsrSdpaAttention,
}


class MoshiAsrDecoderLayer(nn.Module):
    def __init__(self, config: MoshiAsrConfig, layer_idx: int, use_flexible_linear: bool, use_rope=True):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.use_flexible_linear = use_flexible_linear

        self.self_attn = MOSHI_ASR_ATTENTION_CLASSES[config._attn_implementation](
            config=config, layer_idx=layer_idx, use_flexible_linear=use_flexible_linear, use_rope=use_rope
        )

        self.mlp = MoshiAsrGatingMLP(config, use_flexible_linear)
        self.input_layernorm = MoshiAsrRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MoshiAsrRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class MoshiAsrPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = MoshiAsrConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MoshiAsrDecoderLayer", "MimiTransformerLayer"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, MoshiAsrFlexibleLinear):
            module.weight.data.normal_()
        elif isinstance(module, nn.Embedding):
            module.weight.data.normal_(mean=0.0, std=std)
            if module.padding_idx is not None:
                module.weight.data[module.padding_idx].zero_()
        elif isinstance(module, MoshiAsrRMSNorm):
            module.weight.data.fill_(1.0)


MOSHI_ASR_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`MoshiAsrConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

MOSHI_ASR_DECODER_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    "The bare MoshiAsr Model outputting raw hidden-states without any specific head on top.",
    MOSHI_ASR_START_DOCSTRING,
)
class MoshiAsrModel(MoshiAsrPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MoshiAsrDecoderLayer`]
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                MoshiAsrDecoderLayer(config, layer_idx, use_flexible_linear=False)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = MoshiAsrRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    @add_start_docstrings_to_model_forward(MOSHI_ASR_DECODER_INPUTS_DOCSTRING)
    def forward(
        self,
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of MoshiAsr. Make sure to "
...
class MoshiAsrForConditionalGeneration(MoshiAsrPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = MoshiAsrModel(config)
        self.vocab_size = config.vocab_size
    ...
    @can_return_tuple
    @add_start_docstrings_to_model_forward(MOSHI_ASR_INPUTS_DOCSTRING)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        ```python
        >>> from transformers import AutoTokenizer, MoshiAsrForConditionalGeneration

        >>> model = MoshiAsrForConditionalGeneration.from_pretrained("meta-moshi_asr/MoshiAsr-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-moshi_asr/MoshiAsr-2-7b-hf")
