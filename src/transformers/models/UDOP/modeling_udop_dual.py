...
class UDOPDualDenseActDense(nn.Module):
...
class UDOPDualDenseGatedActDense(nn.Module):
    def __init__(self, config: UdopConfig):
        super().__init__()
...
class UDOPDualLayerFF(nn.Module):
    def __init__(self, config: UdopConfig):
        ...
        if config.is_gated_act:
            self.DenseReluDense = UDOPDualDenseGatedActDense(config)
        else:
...
class UDOPDualLayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = UDOPDualAttention(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = UDOPLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class UDOPDualLayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = UDOPDualAttention(config, has_relative_attention_bias=False)
        self.layer_norm = UDOPLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class UDOPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, UDOPLayerNorm):
        ...
        elif isinstance(module, (UDOPDualModel, UdopDualForConditionalGeneration, UDOPEncoderModel)):
            # Mesh TensorFlow embeddings initialization
            # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L1624
            module.shared.weight.data.normal_(mean=0.0, std=factor * 1.0)
            if hasattr(module, "lm_head") and not self.config.tie_word_embeddings:
                module.lm_head.weight.data.normal_(mean=0.0, std=factor * 1.0)
        elif isinstance(module, UDOPDualDenseActDense):
        ...
        elif isinstance(module, UDOPDualDenseGatedActDense):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, UDOPDualAttention):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (UDOPDualAttention, UDOPDualStack)):
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            "self.model.config.decoder_start_token_id has to be defined. In UDOPDual it is usually set to the pad_token_id."
            " See UDOPDual docs for more information"
        )
...
class UDOPDualModel(UDOPPreTrainedModel):
...
class UdopDualForConditionalGeneration(UDOPPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = UDOPDualStack(self.encoder.config, self.shared)
        self.decoder = UDOPDualStack(self.decoder.config, self.shared)
