    ...
    def forward(
        ...
        Args:
            input_values (`torch.FloatTensor` of shape `(batch_size, sequence_length)`):
                ...
                install soundfile*). To prepare the array into `input_values`, the [`SpeechT5ProcessorForCTC`] should
            ...
            attention_mask (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                `attention_mask` should only be passed if the corresponding processor has `config.return_attention_mask
                == True`. For all models whose processor has `config.return_attention_mask == False`, `attention_mask`
                should **not** be passed to avoid degraded performance when doing batched inference. For such models
                `input_values` should simply be padded with 0 and passed without `attention_mask`. Be aware that these
                models also yield slightly different results depending on whether `input_values` is padded or not.
            ...
            labels (`torch.LongTensor` of shape `(batch_size, target_length)`, *optional*):
                ...
                to the sequence length of the output logits. Indices are selected in `[-100, 0, ..., config.vocab_size
...
class SpeechT5ForSpeechToSpeech(SpeechT5PreTrainedModel):
    ...
    def generate_speech(
        ...
        if isinstance(self.speecht5.encoder, SpeechT5EncoderWithSpeechPrenet):
            encoder_attention_mask = self.speecht5.encoder.prenet._get_feature_vector_attention_mask(
