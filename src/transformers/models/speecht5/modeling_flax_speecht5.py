...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2BaseModelOutput with Wav2Vec2->Speecht5
class FlaxSpeecht5BaseModelOutput(ModelOutput):
    """
    Output type of [`FlaxSpeecht5BaseModelOutput`], with potential hidden states and attentions.
...
@flax.struct.dataclass
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2ForPreTrainingOutput with Wav2Vec2->Speecht5
class FlaxSpeecht5ForPreTrainingOutput(ModelOutput):
    """
    Output type of [`FlaxSpeecht5ForPreTrainingOutput`], with potential hidden states and attentions.
...
SPEECHT5_START_DOCSTRING = r"""
    Speecht5 was proposed in [wav2vec 2.0: A Framework for Self-Supervised Learning of Speech
    ...
    Parameters:
        config ([`Speecht5Config`]): Model configuration class with all the parameters of the model.
...
SPEECHT5_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`jnp.ndarray` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            has `config.return_attention_mask == False`, such as [speecht5-base](https://huggingface.co/),
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2LayerNormConvLayer with Wav2Vec2->Speecht5
class FlaxSpeecht5LayerNormConvLayer(nn.Module):
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxConvWithWeightNorm with Wav2Vec2Config->Speecht5Config
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2PositionalConvEmbedding with Wav2Vec2->Speecht5
class FlaxSpeecht5PositionalConvEmbedding(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxConvLayersCollection with Wav2Vec2->Speecht5
class FlaxConvLayersCollection(nn.Module):
    ...
    def setup(self):
        if self.config.feat_extract_norm == "layer":
            self.layers = [
                FlaxSpeecht5LayerNormConvLayer(self.config, layer_id=i, name=str(i), dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2FeatureEncoder with Wav2Vec2->Speecht5
class FlaxSpeecht5FeatureEncoder(nn.Module):
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2FeatureProjection with Wav2Vec2->Speecht5
class FlaxSpeecht5FeatureProjection(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2Attention with Wav2Vec2->Speecht5
class FlaxSpeecht5Attention(nn.Module):
    config: Speecht5Config
    embed_dim: int
    num_heads: int
    dropout: float = 0.0
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2FeedForward with Wav2Vec2->Speecht5
class FlaxSpeecht5FeedForward(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2EncoderLayerStableLayerNorm with Wav2Vec2->Speecht5
class FlaxSpeecht5EncoderLayerStableLayerNorm(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.attention = FlaxSpeecht5Attention(
            config=self.config,
        ...
        self.layer_norm = nn.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype)
        self.feed_forward = FlaxSpeecht5FeedForward(self.config, dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2EncoderLayerStableLayerNormCollection with Wav2Vec2->Speecht5
class FlaxSpeecht5EncoderLayerStableLayerNormCollection(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.layers = [
            FlaxSpeecht5EncoderLayerStableLayerNorm(self.config, name=str(i), dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2StableLayerNormEncoder with Wav2Vec2->Speecht5
class FlaxSpeecht5StableLayerNormEncoder(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.pos_conv_embed = FlaxSpeecht5PositionalConvEmbedding(self.config, dtype=self.dtype)
        self.layer_norm = nn.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype)
        self.dropout = nn.Dropout(rate=self.config.hidden_dropout)
        self.layers = FlaxSpeecht5EncoderLayerStableLayerNormCollection(self.config, dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2GumbelVectorQuantizer with Wav2Vec2->Speecht5
class FlaxSpeecht5GumbelVectorQuantizer(nn.Module):
    """
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2Adapter with Wav2Vec2->Speecht5
class FlaxSpeecht5Adapter(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.layers = FlaxSpeecht5AdapterLayersCollection(self.config, dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2AdapterLayer with Wav2Vec2->Speecht5
class FlaxSpeecht5AdapterLayer(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2AdapterLayersCollection with Wav2Vec2->Speecht5
class FlaxSpeecht5AdapterLayersCollection(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.layers = [
            FlaxSpeecht5AdapterLayer(self.config, name=str(i), dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2PreTrainedModel with Wav2Vec2->Speecht5,wav2vec2->speecht5,WAV_2_VEC_2->SPEECHT5
class FlaxSpeecht5PreTrainedModel(FlaxPreTrainedModel):
    """
    ...
    base_model_prefix: str = "speecht5"
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2Module with Wav2Vec2->Speecht5
class FlaxSpeecht5Module(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.feature_extractor = FlaxSpeecht5FeatureEncoder(self.config, dtype=self.dtype)
        self.feature_projection = FlaxSpeecht5FeatureProjection(self.config, dtype=self.dtype)
        self.masked_spec_embed = self.param(
            "masked_spec_embed", jax.nn.initializers.uniform(), (self.config.hidden_size,)
        )

        if self.config.do_stable_layer_norm:
            self.encoder = FlaxSpeecht5StableLayerNormEncoder(self.config, dtype=self.dtype)
        else:
            raise NotImplementedError("``config.do_stable_layer_norm is False`` is currently not supported.")

        self.adapter = FlaxSpeecht5Adapter(self.config, dtype=self.dtype) if self.config.add_adapter else None
    ...
    def __call__(
        ...
        return FlaxSpeecht5BaseModelOutput(
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2Model with Wav2Vec2->Speecht5
class FlaxSpeecht5Model(FlaxSpeecht5PreTrainedModel):
    module_class = FlaxSpeecht5Module
...
FLAX_WAV2VEC2_MODEL_DOCSTRING = """
    ...
    ```python
    >>> from transformers import Wav2Vec2Processor, FlaxSpeecht5Model
    >>> from datasets import load_dataset
    >>> import soundfile as sf

    >>> processor = Wav2Vec2Processor.from_pretrained("facebook/speecht5-large-lv60")
    >>> model = FlaxSpeecht5Model.from_pretrained("facebook/speecht5-large-lv60")
...
overwrite_call_docstring(
    FlaxSpeecht5Model,
    SPEECHT5_INPUTS_DOCSTRING + FLAX_WAV2VEC2_MODEL_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxSpeecht5Model, output_type=FlaxSpeecht5BaseModelOutput, config_class=Speecht5Config
)


# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2ForCTCModule with Wav2Vec2->Speecht5,wav2vec2->speecht5
class FlaxSpeecht5ForCTCModule(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32

    def setup(self):
        self.speecht5 = FlaxSpeecht5Module(self.config, dtype=self.dtype)
...
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2ForCTC with Wav2Vec2->Speecht5
class FlaxSpeecht5ForCTC(FlaxSpeecht5PreTrainedModel):
    module_class = FlaxSpeecht5ForCTCModule
...
FLAX_WAV2VEC2_FOR_CTC_DOCSTRING = """
    ...
    ```python
    >>> import jax.numpy as jnp
    >>> from transformers import Wav2Vec2Processor, FlaxSpeecht5ForCTC
    >>> from datasets import load_dataset
    >>> import soundfile as sf

    >>> processor = Wav2Vec2Processor.from_pretrained("facebook/speecht5-large-960h-lv60")
    >>> model = FlaxSpeecht5ForCTC.from_pretrained("facebook/speecht5-large-960h-lv60")
...
overwrite_call_docstring(
    FlaxSpeecht5ForCTC,
    SPEECHT5_INPUTS_DOCSTRING + FLAX_WAV2VEC2_FOR_CTC_DOCSTRING,
)
append_replace_return_docstrings(FlaxSpeecht5ForCTC, output_type=FlaxCausalLMOutput, config_class=Speecht5Config)


# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2ForPreTrainingModule with Wav2Vec2->Speecht5,wav2vec2->speecht5
class FlaxSpeecht5ForPreTrainingModule(nn.Module):
    config: Speecht5Config
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.quantizer = FlaxSpeecht5GumbelVectorQuantizer(self.config, dtype=self.dtype)
    ...
    def __call__(
        ...
        return FlaxSpeecht5ForPreTrainingOutput(
...
@add_start_docstrings("""Speecht5 Model with a quantizer and `VQ` head on top.""", SPEECHT5_START_DOCSTRING)
# Copied from transformers.models.wav2vec2.modeling_flax_wav2vec2.FlaxWav2Vec2ForPreTraining with Wav2Vec2->Speecht5,WAV_2_VEC_2->SPEECHT5
class FlaxSpeecht5ForPreTraining(FlaxSpeecht5PreTrainedModel):
    module_class = FlaxSpeecht5ForPreTrainingModule
...
FLAX_WAV2VEC2_FOR_PRETRAINING_DOCSTRING = """
    ...
    >>> import numpy as np
    >>> import jax.numpy as jnp
    >>> from transformers import Wav2Vec2FeatureExtractor, FlaxSpeecht5ForPreTraining
    >>> from transformers.models.speecht5.modeling_flax_speecht5 import _compute_mask_indices
    >>> from datasets import load_dataset
    >>> import soundfile as sf

    >>> feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/speecht5-large-lv60")
    >>> model = FlaxSpeecht5ForPreTraining.from_pretrained("facebook/speecht5-large-lv60")
...
overwrite_call_docstring(
    ...
    SPEECHT5_INPUTS_DOCSTRING + FLAX_WAV2VEC2_FOR_PRETRAINING_DOCSTRING,
)
append_replace_return_docstrings(
    FlaxSpeecht5ForPreTraining, output_type=FlaxSpeecht5ForPreTrainingOutput, config_class=Speecht5Config
)
