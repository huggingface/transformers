...
@dataclass
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2BaseModelOutput with Wav2Vec2->Speecht5
class TFSpeecht5BaseModelOutput(ModelOutput):
    """
    Output type of [`TFSpeecht5BaseModelOutput`], with potential hidden states and attentions.
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2GroupNorm with Wav2Vec2->Speecht5
class TFSpeecht5GroupNorm(tf.keras.layers.Layer):
    """
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2WeightNormConv1D with Wav2Vec2->Speecht5
class TFSpeecht5WeightNormConv1D(tf.keras.layers.Conv1D):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2NoLayerNormConvLayer with Wav2Vec2->Speecht5
class TFSpeecht5NoLayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2LayerNormConvLayer with Wav2Vec2->Speecht5
class TFSpeecht5LayerNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2GroupNormConvLayer with Wav2Vec2->Speecht5
class TFSpeecht5GroupNormConvLayer(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, layer_id: int = 0, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.in_conv_dim = config.conv_dim[layer_id] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
        ...
        self.activation = get_tf_activation(config.feat_extract_activation)
        self.layer_norm = TFSpeecht5GroupNorm(
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2PositionalConvEmbedding with Wav2Vec2->Speecht5
class TFSpeecht5PositionalConvEmbedding(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.conv = TFSpeecht5WeightNormConv1D(
        ...
        self.padding = TFSpeecht5SamePadLayer(config.num_conv_pos_embeddings)
        self.activation = get_tf_activation(config.feat_extract_activation)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2SamePadLayer with Wav2Vec2->Speecht5
class TFSpeecht5SamePadLayer(tf.keras.layers.Layer):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureEncoder with Wav2Vec2->Speecht5
class TFSpeecht5FeatureEncoder(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs: Any) -> None:
        super().__init__(**kwargs)

        if config.feat_extract_norm == "group":
            conv_layers = [TFSpeecht5GroupNormConvLayer(config, layer_id=0, name=f"conv_layers.{0}")] + [
                TFSpeecht5NoLayerNormConvLayer(config, layer_id=i + 1, name=f"conv_layers.{i+1}")
                for i in range(config.num_feat_extract_layers - 1)
            ]
        elif config.feat_extract_norm == "layer":
            conv_layers = [
                TFSpeecht5LayerNormConvLayer(config, layer_id=i, name=f"conv_layers.{i}")
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureExtractor with Wav2Vec2->Speecht5,Wav2Vec2FeatureExtractor->Wav2Vec2FeatureExtractor
class TFWav2Vec2FeatureExtractor(TFSpeecht5FeatureEncoder):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeatureProjection with Wav2Vec2->Speecht5
class TFSpeecht5FeatureProjection(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.bart.modeling_tf_bart.TFBartAttention with TFBart->TFSpeecht5
class TFSpeecht5Attention(tf.keras.layers.Layer):
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2FeedForward with Wav2Vec2->Speecht5
class TFSpeecht5FeedForward(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderLayer with Wav2Vec2->Speecht5
class TFSpeecht5EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFSpeecht5Attention(
        ...
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.feed_forward = TFSpeecht5FeedForward(config, name="feed_forward")
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderLayerStableLayerNorm with Wav2Vec2->Speecht5
class TFSpeecht5EncoderLayerStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
        self.attention = TFSpeecht5Attention(
            embed_dim=config.hidden_size,
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2Encoder with Wav2Vec2->Speecht5
class TFSpeecht5Encoder(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFSpeecht5PositionalConvEmbedding(config, name="pos_conv_embed")
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name="layer_norm")
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [TFSpeecht5EncoderLayer(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)]
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2EncoderStableLayerNorm with Wav2Vec2->Speecht5
class TFSpeecht5EncoderStableLayerNorm(tf.keras.layers.Layer):
    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.pos_conv_embed = TFSpeecht5PositionalConvEmbedding(config, name="pos_conv_embed")
        ...
        self.dropout = tf.keras.layers.Dropout(config.hidden_dropout)
        self.layer = [
            TFSpeecht5EncoderLayerStableLayerNorm(config, name=f"layers.{i}") for i in range(config.num_hidden_layers)
        ]
...
@keras_serializable
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2MainLayer with Wav2Vec2->Speecht5
class TFSpeecht5MainLayer(tf.keras.layers.Layer):
    config_class = Speecht5Config

    def __init__(self, config: Speecht5Config, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.feature_extractor = TFSpeecht5FeatureEncoder(config, name="feature_extractor")
        self.feature_projection = TFSpeecht5FeatureProjection(config, name="feature_projection")

        if config.do_stable_layer_norm:
            self.encoder = TFSpeecht5EncoderStableLayerNorm(config, name="encoder")
        else:
            self.encoder = TFSpeecht5Encoder(config, name="encoder")
    ...
    def call(
        ...
        **kwargs: Any,
        ...
        return TFSpeecht5BaseModelOutput(
            last_hidden_state=hidden_states,
...
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2PreTrainedModel with Wav2Vec2->Speecht5,wav2vec2->speecht5
class TFSpeecht5PreTrainedModel(TFPreTrainedModel):
    """
...
@add_start_docstrings(
    "The bare TFSpeecht5 Model transformer outputing raw hidden-states without any specific head on top.",
    SPEECHT5_START_DOCSTRING,
)
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2Model with Wav2Vec2->Speecht5,wav2vec2->speecht5,WAV_2_VEC_2->SPEECHT5,facebook/wav2vec2-base-960h->
class TFSpeecht5Model(TFSpeecht5PreTrainedModel):
    def __init__(self, config: Speecht5Config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.config = config
        self.speecht5 = TFSpeecht5MainLayer(config, name="speecht5")
    ...
    def call(
        ...
        ```python
        >>> from transformers import Speecht5Processor, TFSpeecht5Model
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Speecht5Processor.from_pretrained("facebook/speecht5-base-960h")
        >>> model = TFSpeecht5Model.from_pretrained("facebook/speecht5-base-960h")
...
@add_start_docstrings(
    """TFSpeecht5 Model with a `language modeling` head on top for Connectionist Temporal Classification (CTC).""",
    SPEECHT5_START_DOCSTRING,
)
# Copied from transformers.models.wav2vec2.modeling_tf_wav2vec2.TFWav2Vec2ForCTC with Wav2Vec2->Speecht5,wav2vec2->speecht5,WAV_2_VEC_2->SPEECHT5,facebook/wav2vec2-base-960h->
class TFSpeecht5ForCTC(TFSpeecht5PreTrainedModel):
    def __init__(self, config: Speecht5Config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
    ...
    def freeze_feature_encoder(self):
        ...
        not be updated during training.
        """
        self.speecht5.feature_extractor.trainable = False
    ...
    def call(
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import Speecht5Processor, TFSpeecht5ForCTC
        >>> from datasets import load_dataset
        >>> import soundfile as sf

        >>> processor = Speecht5Processor.from_pretrained("facebook/speecht5-base-960h")
        >>> model = TFSpeecht5ForCTC.from_pretrained("facebook/speecht5-base-960h")
        ...
        if labels is not None:
            ...
            input_lengths = self.speecht5._get_feat_extract_output_lengths(tf.reduce_sum(attention_mask, axis=-1))
