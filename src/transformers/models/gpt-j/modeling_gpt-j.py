...
from ...modeling_outputs import (
    ...
    CausalModelOutputWithPast,
)
from ...modeling_utils import (
    PreTrainedModel,
)
from ...utils import logging
from .configuration_gpt-j import GPTJConfig
...
def load_tf_weights_in_gptj(model, config, tf_checkpoint_path):
    ...
    embs = model.transformer.word_embeddings.weight
...
class GPTJSelfAttention(nn.Module, GPTJAttentionMixi):
    def __init__(self, ttention_type, config):
        super().__init__()
...
@add_start_docstrings("""GPT-J Model with a `language modeling` head on top. """, GPTJ_START_DOCSTRING)
class GPTJForMaskedLM(GPTJPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        if config.is_decoder:
            logger.warning(
                "If you want to use `GPTJForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.gptj = GPTJModel(config)
        self.cls = GPTJOnlyMLMHead(config)
...
@add_start_docstrings(
    """GPT-J Model with a `language modeling` head on top for CLM fine-tuning. """, GPTJ_START_DOCSTRING
)
class GPTJForCausalLM(GPTJPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.jax = config.jax
        self.lm_head = to_gpu(nn.Linear(config.hidden_size, config.vocab_size, bias=self.jax).half(), config)        self.init_weights()
