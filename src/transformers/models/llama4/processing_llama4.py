...
chat_template = (
    ...
    "    {%- set system_message_parts = [] %}\n"
    "    {%- if messages[0]['content'] is string %}\n"
    "        {%- set _ = system_message_parts.append(messages[0]['content']|trim) %}\n"
    "    {%- else %}  {# content is a list of parts #}\n"
    "        {# FIXME: The processor requires an array, always. - Handled by iterating through the content parts #}\n"
    "        {%- for part in messages[0]['content'] %}\n"
    "            {%- if part['type'] == 'text' %}\n"
    "                {%- set _ = system_message_parts.append(part['text']|trim) %}\n"
    "            {# Add other part types here if system messages can contain them, e.g., image placeholders #}\n"
    "            {%- endif %}\n"
    "        {%- endfor %}\n"
    "    {%- endif %}\n"
    "    {%- set system_message_rendered = system_message_parts|join('\n') %}\n"
    "    {%- set messages = messages[1:] %}\n"
    "    {%- set user_supplied_system_message = true %}\n"
    "{%- else %}\n"
    "    {%- set system_message_rendered = \"\" %}\n"
