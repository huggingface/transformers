...
class CamembertAttention(nn.Module):
    ...
    CAMEMBERT historically split this over `CamembertSelfAttention` (q/k/v) and `CamembertSelfOutput` (the output
    projection plus the residual LayerNorm). The projection lives here as `o_proj`; the LayerNorm
    and residual belong to `CamembertLayer`, which is what makes the layer's topology visible.
    """
...
class CamembertCrossAttention(nn.Module):
    """Cross-attention query/key/value. Its output projection is `CamembertSelfOutput`, see above."""
...
class CamembertSelfOutput(nn.Module):
    ...
    The self-attention path no longer uses this: its projection is `CamembertAttention.o_proj` and its
    LayerNorm belongs to `CamembertLayer`. Cross-attention keeps the original layout deliberately, so
...
class CamembertCrossAttentionBlock(nn.Module):
    """Cross-attention plus its output projection, in CAMEMBERT's original module layout."""

    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.self = CamembertCrossAttention(config, is_causal=False, layer_idx=layer_idx)
...
class CamembertMLP(nn.Module):
    ...
    Replaces `CamembertIntermediate` (up projection plus activation) and `CamembertOutput` (down projection
    plus the residual LayerNorm); the LayerNorm and residual now belong to `CamembertLayer`.
    """
...
class CamembertLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.self_attn = CamembertAttention(config, is_causal=config.is_decoder, layer_idx=layer_idx)
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = CamembertCrossAttentionBlock(config, layer_idx=layer_idx)
        self.mlp = CamembertMLP(config)
