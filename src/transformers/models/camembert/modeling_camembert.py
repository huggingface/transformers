...
class CamembertSdpaSelfAttention(CamembertSelfAttention):
    ...
    def forward(
        ...
        if self.position_embedding_type != "absolute" or output_attentions or head_mask is not None:
            ...
            logger.warning_once(
                "XLMCamembertSdpaSelfAttention is used but `torch.nn.functional.scaled_dot_product_attention` does not support "
...
class CamembertLayer(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.attention = CamembertAttention(config, layer_idx)
        self.is_decoder = config.is_decoder
