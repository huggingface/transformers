...
class MiniCPM_V_4RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        MiniCPM_V_4RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class MiniCPM_V_4MLP(nn.Module):
...
class MiniCPM_V_4Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: MiniCPM_V_4Config, layer_idx: int):
        super().__init__()
        self.config = config
...
class MiniCPM_V_4DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: MiniCPM_V_4Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = MiniCPM_V_4Attention(config=config, layer_idx=layer_idx)

        self.mlp = MiniCPM_V_4MLP(config)
        self.input_layernorm = MiniCPM_V_4RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MiniCPM_V_4RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class MiniCPM_V_4PreTrainedModel(PreTrainedModel):
    config_class = MiniCPM_V_4Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MiniCPM_V_4DecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": MiniCPM_V_4DecoderLayer,
        "attentions": MiniCPM_V_4Attention,
    }
