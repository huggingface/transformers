...
# from transformers.models.configuration_minicpm_v_4 import MiniCPM_V_4Config
import warnings
...
class MultiheadAttention(nn.Module):
    def __init__(
        ...
        dropout: float = 0.0,
        bias: bool = True,
        add_bias_kv: bool = False, # Note: add_bias_kv is not supported in this simplified implementation
        add_zero_attn: bool = False, # Note: add_zero_attn is not supported in this simplified implementation
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            if key_padding_mask.dim() != 2 or key_padding_mask.shape != (bsz, src_len):
                 raise ValueError(f"Expected key_padding_mask shape of {(bsz, src_len)}, but got {key_padding_mask.shape}")
...
__all__ = ["MiniCPM_V_4Config", "MiniCPM_V_4ForConditionalGeneration", "MiniCPM_V_4Model", "MiniCPM_V_4PreTrainedModel"]
