...
# Copied from transformers.models.llama.modeling_llama.LlamaMLP with Llama->OLMo
...
@add_start_docstrings(
    ...
    [`OLMoForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with LLAMA->OLMO,Llama->OLMo
...
@add_start_docstrings(
    """
The OLMo Model transformer with a span classification head on top for extractive question-answering tasks like
SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    OLMO_START_DOCSTRING,
)
# Copied from transformers.models.llama.modeling_llama.LlamaForQuestionAnswering with LLAMA->OLMO,Llama->OLMo
class OLMoForQuestionAnswering(OLMoPreTrainedModel):
    base_model_prefix = "transformer"

    def __init__(self, config):
        super().__init__(config)
        self.transformer = OLMoModel(config)
