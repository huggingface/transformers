    ...
    for k in state_dict.keys():
        ...
        if "generator.backbone.transformers" in k:
            if "c_attn" in k:
                ...
                c_attn_weight = state_dict.get_tensor(k)
                ...
                tensors[k.replace(".att.c_proj", ".self_attn.o_proj")] = state_dict.get_tensor(k)
            elif "att_norm" in k:
                tensors[k.replace("att_norm", "input_layernorm")] = state_dict.get_tensor(k)
            elif "ffn_norm" in k:
                tensors[k.replace("ffn_norm", "post_attention_layernorm")] = state_dict.get_tensor(k)
            else:
                new_k = k.replace("beta", "bias")
                tensors[new_k] = state_dict.get_tensor(k)
        else:
            new_k = k.replace("beta", "bias")
            tensors[new_k] = state_dict.get_tensor(k)
    state_dict = tensors
