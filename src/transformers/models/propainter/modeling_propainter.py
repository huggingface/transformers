...
_IMAGE_CLASS_CHECKPOINT = "google/propainter-base-patch16-224"
...
# Copied from transformers.models.vit.modeling_vit.ViTSdpaSelfAttention with ViT->ProPainter
class ProPainterSdpaSelfAttention(ProPainterSelfAttention):
    def __init__(self, config: ProPainterConfig) -> None:
...
# Copied from transformers.models.vit.modeling_vit.ViTSelfOutput with ViT->ProPainter
class ProPainterSelfOutput(nn.Module):
    """
    The residual connection is defined in ProPainterLayer instead of here (as is the case with other models), due to the
...
# Copied from transformers.models.vit.modeling_vit.ViTSdpaAttention with ViT->ProPainter
class ProPainterSdpaAttention(ProPainterAttention):
    def __init__(self, config: ProPainterConfig) -> None:
        super().__init__(config)
        self.attention = ProPainterSdpaSelfAttention(config)
...
PROPAINTER_ATTENTION_CLASSES = {
    "eager": ProPainterAttention,
    "sdpa": ProPainterSdpaAttention,
}
...
# Copied from transformers.models.vit.modeling_vit.ViTPooler with ViT->ProPainter
class ProPainterPooler(nn.Module):
...
# Copied from transformers.models.vit.modeling_vit.ViTForMaskedImageModeling with VIT->PROPAINTER,ViT->ProPainter,vit->propainter,google/vit-base-patch16-224-in21k->ruffy369/propainter
class ProPainterForMaskedImageModeling(ProPainterPreTrainedModel):
    def __init__(self, config: ProPainterConfig) -> None:
        super().__init__(config)
...
class ProPainterForImageClassification(ProPainterPreTrainedModel):
    def __init__(self, config: ProPainterConfig) -> None:
        super().__init__(config)
