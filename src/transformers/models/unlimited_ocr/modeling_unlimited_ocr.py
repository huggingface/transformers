...
@dataclass
class UnlimitedOcrModelOutputWithPast(BaseModelOutputWithPast):
    r"""
...
@auto_docstring(
    custom_intro="""
    Base class for UnlimitedOcr causal language model (or autoregressive) outputs.
    """
)
@dataclass
class UnlimitedOcrCausalLMOutputWithPast(ModelOutput):
    r"""
...
class UnlimitedOcrSamVisionSdpaAttention(UnlimitedOcrSamVisionAttention):
    """
    Multi-head Attention block with relative position embeddings.
...
UNLIMITED_OCR_SAM_VISION_ATTENTION_CLASSES = {
    "eager": UnlimitedOcrSamVisionAttention,
    "sdpa": UnlimitedOcrSamVisionSdpaAttention,
}
...
class UnlimitedOcrSamVisionLayer(GradientCheckpointingLayer):
    def __init__(self, config, window_size):
        ...
        self.attn = UNLIMITED_OCR_SAM_VISION_ATTENTION_CLASSES[config._attn_implementation](config, window_size)
        self.layer_norm2 = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.mlp = UnlimitedOcrSamMLPBlock(config)
...
class UnlimitedOcrSamLayerNorm(nn.LayerNorm):
...
class UnlimitedOcrSamVisionNeck(nn.Module):
    def __init__(self, config: UnlimitedOcrSamVisionConfig):
        ...
        self.layer_norm1 = UnlimitedOcrSamLayerNorm(config.output_channels, data_format="channels_first")
        self.conv2 = nn.Conv2d(config.output_channels, config.output_channels, kernel_size=3, padding=1, bias=False)
        self.layer_norm2 = UnlimitedOcrSamLayerNorm(config.output_channels, data_format="channels_first")
...
class UnlimitedOcrVisionRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        UnlimitedOcrVisionRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class UnlimitedOcrVisionRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: UnlimitedOcrVisionConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: UnlimitedOcrVisionConfig | None = None,
...
class UnlimitedOcrVisionAttention(nn.Module):
    ...
    def __init__(self, config: UnlimitedOcrVisionConfig, layer_idx: int):
        super().__init__()
...
class UnlimitedOcrVisionEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: UnlimitedOcrVisionConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = UnlimitedOcrVisionAttention(config=config, layer_idx=layer_idx)

        self.mlp = UnlimitedOcrVisionMLP(config)
        self.input_layernorm = UnlimitedOcrVisionRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class UnlimitedOcrVisionEncoder(UnlimitedOcrPreTrainedModel):
    _can_record_outputs = {
        "hidden_states": UnlimitedOcrVisionEncoderLayer,
        "attentions": UnlimitedOcrVisionAttention,
    }
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [UnlimitedOcrVisionEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = UnlimitedOcrVisionRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = UnlimitedOcrVisionRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
...
class UnlimitedOcrTextRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def compute_default_rope_parameters(
        config: UnlimitedOcrTextConfig | None = None,
        device: Optional["torch.device"] = None,
        seq_len: int | None = None,
    ) -> tuple["torch.Tensor", float]:
        """
...
class UnlimitedOcrTextPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        super()._init_weights(module)
        if isinstance(module, UnlimitedOcrTextExperts):
