# coding=utf-8
# Copyright 2022 Haotian Liu, Chunyuan Li, Qingyang Wu, Yong Jae Lee The HuggingFace Inc. team. All rights reserved.
#
...
_CHECKPOINT_FOR_DOC = "liuhaotian/llava-v1.5-13b"
...
def load_tf_weights_in_llava(model, config, tf_checkpoint_path):
...
class LlaVaEmbeddings(nn.Module):
...
class LlaVaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in LlaVaModel forward() function)
...
class LlaVaSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LlaVaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = LlaVaSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = LlaVaSelfOutput(config)
...
class LlaVaIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LlaVaLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = LlaVaAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = LlaVaAttention(config, position_embedding_type="absolute")
        self.intermediate = LlaVaIntermediate(config)
        self.output = LlaVaOutput(config)
...
class LlaVaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([LlaVaLayer(config) for _ in range(config.num_hidden_layers)])
...
class LlaVaPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class LlaVaLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = LlaVaPredictionHeadTransform(config)
...
class LlaVaOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = LlaVaLMPredictionHead(config)
...
class LlaVaPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = load_tf_weights_in_llava
...
@add_start_docstrings(
    "The bare LLaVA Model transformer outputting raw hidden-states without any specific head on top.",
...
class LlaVaModel(LlaVaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = LlaVaEmbeddings(config)
        self.encoder = LlaVaEncoder(config)
...
class LlaVaForMaskedLM(LlaVaPreTrainedModel):
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `LlaVaForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.llava = LlaVaModel(config)
        self.cls = LlaVaOnlyMLMHead(config)
...
class LlaVaForCausalLM(LlaVaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `LlaVaForCausalLM` as a standalone, add `is_decoder=True.`")
...
class LlaVaForSequenceClassification(LlaVaPreTrainedModel):
    def __init__(self, config):
        ...
        self.llava = LlaVaModel(config)
        self.classifier = LlaVaClassificationHead(config)
...
class LlaVaForMultipleChoice(LlaVaPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LLAVA_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
