...
@auto_docstring
class LingbotVisionForImageClassification(LingbotVisionPreTrainedModel):
    def __init__(self, config: LingbotVisionConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.lingbot_vision = LingbotVisionModel(config)
        self.classifier = (
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs: BaseModelOutputWithPooling = self.lingbot_vision(pixel_values, **kwargs)
        cls_token = outputs.last_hidden_state[:, 0]
        patch_tokens = outputs.last_hidden_state[:, self.config.num_storage_tokens + 1 :]
        logits = self.classifier(torch.cat((cls_token, patch_tokens.mean(dim=1)), dim=1))
