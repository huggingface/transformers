...
@strict
class Quatfit1AudioConfig(PreTrainedConfig):
    r"""
...
@auto_docstring(checkpoint="google/gemma-4-e2b-it")
@strict
class Quatfit1TextConfig(PreTrainedConfig):
    r"""
    ...
    hidden_size_per_layer_input (`int`, defaults to 256):
        ...
        because all layers are packed into a single table. See the [Quatfit1](https://huggingface.co/docs/transformers/main/en/model_doc/quatfit1#per-layer-embeddings-ple) docs
...
@auto_docstring(checkpoint="google/gemma-4-e2b-it")
@strict
class Quatfit1VisionConfig(PreTrainedConfig):
    r"""
...
@auto_docstring(checkpoint="google/gemma-4-e2b-it")
@strict
class Quatfit1Config(PreTrainedConfig):
    r"""
    ...
    >>> configuration = Quatfit1Config(text_config, vision_config, audio_config)

    >>> # Initializing a model from the google/gemma-4-e2b-it configuration
    >>> model = Quatfit1ForConditionalGeneration(configuration)
    ...
    text_config: Quatfit1TextConfig | dict[str, Any] | None = None
    vision_config: Quatfit1VisionConfig | dict[str, Any] | None = None
    audio_config: Quatfit1AudioConfig | dict[str, Any] | None = None
    ...
    def __post_init__(self, **kwargs):
        if self.text_config is None:
            self.text_config = Quatfit1TextConfig()
            logger.info("text_config is None. Using default Quatfit1TextConfig.")
        elif isinstance(self.text_config, dict):
            self.text_config = Quatfit1TextConfig(**self.text_config)

        if self.vision_config is None:
            logger.info("vision_config is None. Quatfit1Model.vision_tower will not be initialized.")
        if isinstance(self.vision_config, dict):
            self.vision_config = Quatfit1VisionConfig(**self.vision_config)

        if self.audio_config is None:
            logger.info("audio_config is None. Quatfit1Model.audio_tower will not be initialized.")
...
__all__ = ["Quatfit1AudioConfig", "Quatfit1Config", "Quatfit1TextConfig", "Quatfit1VisionConfig"]
