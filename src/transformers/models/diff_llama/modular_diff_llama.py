...
class DiffLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: DiffLlamaConfig, layer_idx: int):
        ...
        self.self_attn = DIFFLLAMA_ATTENTION_CLASSES[config.attn_implementation](config=config, layer_idx=layer_idx)


class DiffLlamaModel(LlamaModel):
    def __init__(self, config: DiffLlamaConfig):
        super().__init__(config)
        self.layers = nn.ModuleList([DiffLlamaDecoderLayer(config, i) for i in range(config.num_layers)])
