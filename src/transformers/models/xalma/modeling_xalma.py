...
class XALMAAttention(nn.Module):
...
class XALMAFlashAttention2(XALMAAttention):
    """
    XALMA flash attention module. This module inherits from `XALMAAttention` as the weights of the module stays
...
class XALMASdpaAttention(XALMAAttention):
    """
    XALMA attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `XALMAAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from XALMAAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "XALMAModel is using XALMASdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
XALMA_ATTENTION_CLASSES = {
    "eager": XALMAAttention,
    "flash_attention_2": XALMAFlashAttention2,
    "sdpa": XALMASdpaAttention,
}


class XALMADecoderLayer(nn.Module):
    def __init__(self, config: XALMAConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = XALMA_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
...
@add_start_docstrings(
    "The bare XALMA Model outputting raw hidden-states without any specific head on top.",
    XALMA_START_DOCSTRING,
)
class XALMAPreTrainedModel(PreTrainedModel):
    config_class = XALMAConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["XALMADecoderLayer"]
...
class XALMAModel(XALMAPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`XALMADecoderLayer`]

    Args:
        config: XALMAConfig
    """

    def __init__(self, config: XALMAConfig):
        ...
        self.layers = nn.ModuleList(
            [XALMADecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    @add_start_docstrings_to_model_forward(XALMA_INPUTS_DOCSTRING)
    def forward(
        self,
...
class XALMAForCausalLM(XALMAPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = XALMAModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        >>> model = XALMAForCausalLM.from_pretrained("haoranxu/X-ALMA")
        >>> tokenizer = AutoTokenizer.from_pretrained("haoranxu/X-ALMA")
...
@add_start_docstrings(
    """
    The XALMA Model transformer with a sequence classification head on top (linear layer).
...
class XALMAForSequenceClassification(XALMAPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings(
    """
The XALMA Model transformer with a span classification head on top for extractive question-answering tasks like
SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    XALMA_START_DOCSTRING,
)
class XALMAForQuestionAnswering(XALMAPreTrainedModel):
    base_model_prefix = "transformer"

    def __init__(self, config):
        super().__init__(config)
        self.transformer = XALMAModel(config)
...
@add_start_docstrings(
    """
    The XALMA Model transformer with a token classification head on top (a linear layer on top of the hidden-states
    output) e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    XALMA_START_DOCSTRING,
)
class XALMAForTokenClassification(XALMAPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = XALMAModel(config)
