...
class PPDocLayoutV4MultiscaleDeformableAttention(nn.Module):
    """
    Multiscale deformable attention as proposed in Deformable DETR.
    """

    def __init__(self, config: PPDocLayoutV4Config, num_heads: int, n_points: int):
        super().__init__()
        ...
        if not ((dim_per_head & (dim_per_head - 1) == 0) and dim_per_head != 0):
            warnings.warn(
                "You'd better set embed_dim (d_model) in PPDocLayoutV4MultiscaleDeformableAttention to make the"
...
class PPDocLayoutV4MLP(nn.Module):
    def __init__(
        self, config: PPDocLayoutV4Config, hidden_size: int, intermediate_size: int, activation_function: str
    ):
        super().__init__()
...
class PPDocLayoutV4SelfAttention(nn.Module):
    """
    Multi-headed self-attention from 'Attention Is All You Need' paper.

    In PP_DOCLAYOUT_V4, position embeddings are added to both queries and keys (but not values) in self-attention.
    """
...
class PPDocLayoutV4ConvNormLayer(nn.Module):
...
class PPDocLayoutV4EncoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV4Config):
        super().__init__()
        ...
        # self-attention
        self.self_attn = PPDocLayoutV4SelfAttention(
            config=config,
...
class PPDocLayoutV4RepVggBlock(nn.Module):
    """
    ...
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.conv1 = PPDocLayoutV4ConvNormLayer(config, hidden_channels, hidden_channels, 3, 1, padding=1)
        self.conv2 = PPDocLayoutV4ConvNormLayer(config, hidden_channels, hidden_channels, 1, 1, padding=0)
        self.activation = nn.Identity() if activation is None else ACT2CLS[activation]()
...
class PPDocLayoutV4CSPRepLayer(nn.Module):
    """
    ...
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.conv1 = PPDocLayoutV4ConvNormLayer(config, in_channels, hidden_channels, 1, 1, activation=activation)
        self.conv2 = PPDocLayoutV4ConvNormLayer(config, in_channels, hidden_channels, 1, 1, activation=activation)
        self.bottlenecks = nn.Sequential(*[PPDocLayoutV4RepVggBlock(config) for _ in range(num_blocks)])
        if hidden_channels != out_channels:
            self.conv3 = PPDocLayoutV4ConvNormLayer(config, hidden_channels, out_channels, 1, 1, activation=activation)
        else:
...
class PPDocLayoutV4SinePositionEmbedding(nn.Module):
    """
...
class PPDocLayoutV4AIFILayer(nn.Module):
    """
    ...
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.position_embedding = PPDocLayoutV4SinePositionEmbedding(
            embed_dim=self.encoder_hidden_dim,
            temperature=config.positional_encoding_temperature,
        )
        self.layers = nn.ModuleList([PPDocLayoutV4EncoderLayer(config) for _ in range(config.encoder_layers)])
...
class PPDocLayoutV4HybridEncoder(PPDocLayoutV4PreTrainedModel):
    """
    ...
    _can_record_outputs = {
        "hidden_states": PPDocLayoutV4AIFILayer,
        "attentions": PPDocLayoutV4SelfAttention,
    }
    ...
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.aifi = nn.ModuleList([PPDocLayoutV4AIFILayer(config) for _ in range(len(self.encode_proj_layers))])

        # top-down FPN
        self.lateral_convs = nn.ModuleList()
        self.fpn_blocks = nn.ModuleList()
        for _ in range(self.num_fpn_stages):
            lateral_conv = PPDocLayoutV4ConvNormLayer(
                config,
            ...
            fpn_block = PPDocLayoutV4CSPRepLayer(config)
        ...
        for _ in range(self.num_pan_stages):
            downsample_conv = PPDocLayoutV4ConvNormLayer(
                config,
                in_channels=self.encoder_hidden_dim,
                out_channels=self.encoder_hidden_dim,
                kernel_size=3,
            ...
            pan_block = PPDocLayoutV4CSPRepLayer(config)
...
class PPDocLayoutV4DecoderLayer(nn.Module):
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.hidden_size, eps=config.layer_norm_eps)
        # cross-attention
        self.encoder_attn = PPDocLayoutV4MultiscaleDeformableAttention(
            config,
...
class PPDocLayoutV4Decoder(PPDocLayoutV4PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": PPDocLayoutV4DecoderLayer,
        "attentions": PPDocLayoutV4SelfAttention,
        "cross_attentions": PPDocLayoutV4MultiscaleDeformableAttention,
    }
    ...
    def __init__(self, config: PPDocLayoutV4Config):
        ...
        self.dropout = config.dropout
        self.layers = nn.ModuleList([PPDocLayoutV4DecoderLayer(config) for _ in range(config.decoder_layers)])
...
class PPDocLayoutV4Model(PPDocLayoutV4PreTrainedModel):
...
class PPDocLayoutV4ForObjectDetection(PPDocLayoutV4PreTrainedModel):
...
__all__ = ["PPDocLayoutV4ForObjectDetection", "PPDocLayoutV4Model", "PPDocLayoutV4PreTrainedModel"]
