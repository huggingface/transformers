...
class GLMFlashAttention2(GLMAttention):
    ...
    def forward(
        ...
        value_states = value_states.view(bsz, q_len, self.num_heads, self.hidden_size_per_attention_head).transpose(1, 2)
        ...
        sliding_window = None
        if self.config.use_sliding_window and getattr(self.config, "sliding_window", None) is not None and self.layer_number >= self.config.max_window_layers:
