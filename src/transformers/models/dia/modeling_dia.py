...
class DiaFlashAttention2(DiaAttention):
    """
    Dia flash attention module. This module inherits from `DiaAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # DiaFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("DiaFlashAttention2 attention does not support output_attentions")
...
class DiaSdpaAttention(DiaAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "DiaModel is using DiaSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
DIA_ATTENTION_CLASSES = {
    "eager": DiaAttention,
    "flash_attention_2": DiaFlashAttention2,
    "sdpa": DiaSdpaAttention,
}
...
class DiaEncoderLayer(nn.Module):
    def __init__(self, config: DiaConfig):
        ...
        self.self_attn = DIA_ATTENTION_CLASSES[config._attn_implementation](
...
class DiaDecoderLayer(nn.Module):
    def __init__(self, config: DiaConfig, layer_idx: Optional[int] = None):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = DIA_ATTENTION_CLASSES[config._attn_implementation](
            self.embed_dim,
...
DIA_INPUTS_DOCSTRING = r"""
    Args:
        input_features (`torch.FloatTensor` of shape `(batch_size, feature_size, sequence_length)`):
            ...
            tensor of type `torch.FloatTensor`. See [`~DiaFeatureExtractor.__call__`]
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            Indices can be obtained using [`DiaTokenizer`]. See [`PreTrainedTokenizer.encode`] and
            [`PreTrainedTokenizer.__call__`] for details.

            [What are decoder input IDs?](../glossary#decoder-input-ids)

            Dia uses the `decoder_start_token_id` as the starting token for `decoder_input_ids` generation. If
        ...
        decoder_attention_mask (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            [`modeling_dia._prepare_decoder_attention_mask`] and modify to your needs. See diagram 1 in [the BART
...
DIA_ENCODER_INPUTS_DOCSTRING = r"""
    Args:
        input_features (`torch.FloatTensor` of shape `(batch_size, feature_size, sequence_length)`):
            Float values mel features extracted from the raw speech waveform. Raw speech waveform can be obtained by
            loading a `.flac` or `.wav` audio file into an array of type `List[float]` or a `numpy.ndarray`, *e.g.* via
...
class DiaEncoder(DiaPreTrainedModel):
    ...
    def __init__(self, config: DiaConfig):
        ...
        self.layers = nn.ModuleList([DiaEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        Args:
            input_features (`torch.LongTensor` of shape `(batch_size, feature_size, sequence_length)`):
                ...
                and conversion into a tensor of type `torch.FloatTensor`. See [`~DiaFeatureExtractor.__call__`]
            attention_mask (`torch.Tensor`)`, *optional*):
                Dia does not support masking of the `input_features`, this argument is preserved for compatibility,
        ...
        if input_features.shape[-1] != expected_seq_length:
            raise ValueError(
                f"Dia expects the mel input features to be of length {expected_seq_length}, but found {input_features.shape[-1]}. Make sure to pad the input mel features to {expected_seq_length}."
            )
...
class DiaDecoder(DiaPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`DiaDecoderLayer`]
    ...
    def __init__(self, config: DiaConfig):
        ...
        self.embed_positions = DiaPositionalEmbedding(self.max_target_positions, config.d_model)

        self.layers = nn.ModuleList(
            [DiaDecoderLayer(config, layer_idx) for layer_idx in range(config.decoder_layers)]
        )
...
@add_start_docstrings(
    "The Dia Model with a language modeling head. Can be used for automatic speech recognition.",
    DIA_START_DOCSTRING,
)
...
class DiaDecoderWrapper(DiaPreTrainedModel):
    """
...
@add_start_docstrings(
    """
    Dia decoder with a language modeling head on top (linear layer with weights tied to the input embeddings).
    """,
    DIA_START_DOCSTRING,
)
class DiaForCausalLM(DiaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["proj_out.weight"]
    main_input_name = "input_ids"
    ...
    def __init__(self, config):
        super().__init__(config)
        config.is_encoder_decoder = False
        self.model = DiaDecoderWrapper(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import DiaForCausalLM, DiaForConditionalGeneration, DiaProcessor
        >>> import torch
        >>> from datasets import load_dataset
        ...
        >>> assistant_model = DiaForCausalLM.from_pretrained("distil-dia/distil-large-v2")
...
@add_start_docstrings(
    """
    Dia Encoder Model with a sequence classification head on top (a linear layer over the pooled output) for tasks
    like SUPERB Keyword Spotting.
    """,
    DIA_ENCODER_INPUTS_DOCSTRING,
)
class DiaForAudioClassification(DiaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_start_docstrings_to_model_forward(DIA_ENCODER_INPUTS_DOCSTRING)
    ...
    def forward(
        ...
        ```python
        >>> import torch
        >>> from transformers import AutoFeatureExtractor, DiaForAudioClassification
        >>> from datasets import load_dataset

        >>> feature_extractor = AutoFeatureExtractor.from_pretrained("sanchit-gandhi/dia-medium-fleurs-lang-id")
        >>> model = DiaForAudioClassification.from_pretrained("sanchit-gandhi/dia-medium-fleurs-lang-id")
