...
class DiaTokenizer(PreTrainedTokenizer):
    ...
    def set_prefix_tokens(
        ...
        >>> tokenizer = DiaTokenizer.from_pretrained("nari-labs/Dia-1.6B", language="spanish")
    ...
    def _convert_id_to_token(self, index):
        """
        Converts an index (integer) in a token (str) using the vocab. Dia's base tokenizer always decodes OOV
...
def _decode_asr(tokenizer, model_outputs, *, return_timestamps, return_language, time_precision):
    """
    Internal method meant to only be used by asr pipeline. Handles all the little quirks specific to dia to handle
        ...
        for i, token in enumerate(token_ids):
            ...
            if token in all_special_ids:
                ...
                if last_timestamp and token >= last_timestamp:
                    # Dia outputted a timestamp token, but it falls within
    ...
    if previous_tokens:
        if return_timestamps:
            logger.warning(
                "Dia did not predict an ending timestamp, which can happen if audio is cut off in the middle of a word. "
                "Also make sure DiaTimeStampLogitsProcessor was used during generation."
            )
    ...
    for seq_idx, right_sequence in enumerate(sequences[1:]):
        ...
        for i in range(1, left_length + right_length):
            ...
            if len(left) != len(right):
                raise RuntimeError(
                    "There is a bug within dia `decode_asr` function, please report it. Dropping to prevent bad inference."
                )
