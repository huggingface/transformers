...
class T5Gemma2MergedAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        effective_impl = (
            "sdpa" if self.config._attn_implementation == "flash_attention_2" else self.config._attn_implementation
        )
        key_states = torch.cat([key_states, cross_key_states], dim=2)
        value_states = torch.cat([value_states, cross_value_states], dim=2)

        attention_interface: Callable = ALL_ATTENTION_FUNCTIONS.get_interface(effective_impl, eager_attention_forward)
