...
from transformers import Gemma3Config, Gemma3ForCausalLM, Gemma3Tokenizer


try:
    from transformers import Gemma3TokenizerFast
except ImportError as e:
    warnings.warn(e)
    warnings.warn(
        "The converted tokenizer will be the `slow` tokenizer. To use the fast, update your `tokenizers` library and re-run the tokenizer conversion"
    )
    Gemma3TokenizerFast = None

"""
Sample usage:

```
python src/transformers/models/gemma3/convert_gemma3_weights_to_hf.py \
...
```py
from transformers import Gemma3ForCausalLM, Gemma3TokenizerFast

model = Gemma3ForCausalLM.from_pretrained("/output/path")
tokenizer = Gemma3TokenizerFast.from_pretrained("/output/path")
```
...
def write_model(save_path, input_base_path, config, safe_serialization=True, push_to_hub=False, dtype=torch.float32):
    ...
    print("Loading the checkpoint in a Gemma3 model.")
...
def write_tokenizer(input_tokenizer_path, save_path, push_to_hub=False):
    ...
    tokenizer_class = Gemma3Tokenizer if Gemma3TokenizerFast is None else Gemma3TokenizerFast
...
def main():
    ...
    parser.add_argument(
        "--input_checkpoint",
        help="Absolute path to the target Gemma3 weights.",
        required=True,
    )
    parser.add_argument(
        "--tokenizer_checkpoint",
        help="Location of Gemma3 tokenizer model",
    )
    parser.add_argument(
        "--model_size",
        default="9B",
        choices=["9B", "27B", "tokenizer_only"],
        help="'f' models correspond to the finetuned versions, and are specific to the Gemma32 official release. For more details on Gemma3, checkout the original repo: https://huggingface.co/google/gemma-7b",
    )
    parser.add_argument(
        "--output_dir",
