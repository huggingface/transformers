...
class XmodModel(XmodPreTrainedModel):
    ...
    def forward(
        ...
        encoder_attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        past_key_values (`tuple(tuple(torch.Tensor))` of length `config.n_layers` with each tuple having 4 tensors:
...
class XmodForCausalLM(XmodPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        Returns: `transformers.modeling_outputs.CausalLMOutputWithCrossAttentions` or `tuple(torch.Tensor)`
