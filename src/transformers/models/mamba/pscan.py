...
def pad_npo2(X):
    ...
    len_npo2 = npo2(X.size(2))
    pad_tuple = (0, 0, 0, len_npo2 - X.size(2), 0, 0)
...
class PScan(torch.autograd.Function):
    ...
    def forward(ctx, A_in, X_in):
        ...
        if L == npo2(L):
            ...
            A = pad_npo2(A_in) # (B, D, npo2(L), N)
            X = pad_npo2(X_in) # (B, D, npo2(L), N)
    ...
    def backward(ctx, grad_output_in):
        ...
        Args:
            ctx : A_in : (B, D, L, N), X : (B, D, L, N)
            grad_output_in : (B, D, L, N)

        Returns:
            gradA : (B, D, L, N), gradX : (B, D, L, N)
        """

        A_in, X = ctx.saved_tensors

        L = grad_output_in.size(2)
        ...
        if L == npo2(L):
            ...
            grad_output = pad_npo2(grad_output_in) # (B, D, npo2(L), N)
            A_in = pad_npo2(A_in) # (B, D, npo2(L), N)
