...
class T5SdpaAttention(T5Attention):
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            logger.warning_once(
                "T5Model is using T5SdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
