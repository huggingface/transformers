...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, positional_embedding_injected_in_attention=None, cross_attention:bool = False):
        ...
        # print(positional_embedding_injected_in_attention)
    ...
    def compute_bias_for_relative_position_embedding_name(self, position_embedding_name, position_embedding_type, position_ids, query_length, key_length, device=None):
        ...
        if position_embedding_type == POSITION_EMBEDDING_T5_DEFAULT_RELATIVE:
        ...
        elif position_embedding_type == POSITION_EMBEDDING_T5_RELATIVE:        
            ...
            raise Exception(f'compute_bias_for_relative_position_embedding_name() only supports {POSITION_EMBEDDING_T5_DEFAULT_RELATIVE} and {POSITION_EMBEDDING_T5_RELATIVE} got {position_embedding_type}')
    ...
    def forward(
        ...
        elif self.memory_efficient_attention in ["torch_scaled_dot_product_attention", "torch_scaled_dot_product_attention_force_flash", "torch_scaled_dot_product_attention_with_position_bias"]:
            scale = 1 / math.sqrt(K) if self.scale == None else self.scale
            is_causal = self.is_decoder and not self.cross_attention
            if self.memory_efficient_attention in ["torch_scaled_dot_product_attention_force_flash", "torch_scaled_dot_product_attention"]:
