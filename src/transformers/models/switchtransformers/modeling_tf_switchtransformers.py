# coding=utf-8
# Copyright 2022 SwitchTransformers Authors and The HuggingFace Inc. team.
...
""" TF 2.0 SwitchTransformers model."""
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerNorm with T5->SwitchTransformers
class TFSwitchTransformersLayerNorm(tf.keras.layers.Layer):
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5DenseActDense with T5->SwitchTransformers
class TFSwitchTransformersDenseActDense(tf.keras.layers.Layer):
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5DenseGatedActDense with T5->SwitchTransformers
class TFSwitchTransformersDenseGatedActDense(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        wi_initializer = tf.keras.initializers.RandomNormal(
            mean=0, stddev=config.initializer_factor * (config.d_model**-0.5)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerFF with T5->SwitchTransformers
class TFSwitchTransformersLayerFF(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        if config.is_gated_act:
            self.DenseReluDense = TFSwitchTransformersDenseGatedActDense(config, name="DenseReluDense")
        else:
            self.DenseReluDense = TFSwitchTransformersDenseActDense(config, name="DenseReluDense")

        self.layer_norm = TFSwitchTransformersLayerNorm(epsilon=config.layer_norm_epsilon, name="layer_norm")
        self.dropout = tf.keras.layers.Dropout(config.dropout_rate)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5Attention with T5->SwitchTransformers
class TFSwitchTransformersAttention(tf.keras.layers.Layer):
    NEW_ID = itertools.count()

    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.layer_id = next(TFSwitchTransformersAttention.NEW_ID)
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerSelfAttention with T5->SwitchTransformers
class TFSwitchTransformersLayerSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.SelfAttention = TFSwitchTransformersAttention(
            config,
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5LayerCrossAttention with T5->SwitchTransformers
class TFSwitchTransformersLayerCrossAttention(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super().__init__(**kwargs)
        self.EncDecAttention = TFSwitchTransformersAttention(
            config,
...
# Copied from transformers.models.t5.modeling_tf_t5.TFT5Block with T5->SwitchTransformers
class TFSwitchTransformersBlock(tf.keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        super().__init__(**kwargs)
        self.is_decoder = config.is_decoder
        self.layer = []
        self.layer.append(
            TFSwitchTransformersLayerSelfAttention(
                config,
                has_relative_attention_bias=has_relative_attention_bias,
                name="layer_._0",
            )
        ...
        if self.is_decoder:
            self.layer.append(
                TFSwitchTransformersLayerCrossAttention(
                    config,
                    name="layer_._1",
                )
            )

        self.layer.append(TFSwitchTransformersLayerFF(config, name=f"layer_._{len(self.layer)}"))
...
# provided as a tf.keras.layers.Layer usually called "TFSwitchTransformersMainLayer"
####################################################
@keras_serializable
# Copied from transformers.models.t5.modeling_tf_t5.TFT5MainLayer with T5->SwitchTransformers
class TFSwitchTransformersMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, embed_tokens=None, **kwargs):
        ...
        self.block = [
            TFSwitchTransformersBlock(config, has_relative_attention_bias=bool(i == 0), name=f"block_._{i}")
            for i in range(config.num_layers)
        ]
        self.final_layer_norm = TFSwitchTransformersLayerNorm(
    ...
    def call(
        ...
        # SwitchTransformers has a mask that can compare sequence ids, we can simulate this here with this transposition
...
####################################################
# TFSwitchTransformersPreTrainedModel is a sub-class of tf.keras.Model
# which take care of loading and saving pretrained weights
# and various common utilities.
# Here you just need to specify a few (self-explanatory)
# pointers for your model.
####################################################
# Copied from transformers.models.t5.modeling_tf_t5.TFT5PreTrainedModel with T5->SwitchTransformers
class TFSwitchTransformersPreTrainedModel(TFPreTrainedModel):
    """
    ...
    def _shift_right(self, input_ids):
        ...
        assert decoder_start_token_id is not None, (
            "self.model.config.decoder_start_token_id has to be defined. In TF SwitchTransformers it is usually set to"
...
SWITCHTRANSFORMERS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`tf.Tensor` of shape `(batch_size, sequence_length)`):
            ...
            To know more on how to prepare `inputs` for pretraining take a look at [SWITCHTRANSFORMERS
            Training](./switchtransformers#training).
        decoder_input_ids (`tf.Tensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            Provide for sequence to sequence training. SWITCHTRANSFORMERS uses the `pad_token_id` as the starting token
            for `decoder_input_ids` generation. If `past_key_values` is used, optionally only the last
...
SWITCHTRANSFORMERS_ENCODER_INPUTS_DOCSTRING = r"""
    Args:
        inputs (`tf.Tensor` of shape `(batch_size, sequence_length)`):
            ...
            Indices can be obtained using [`SwitchTransformersTokenizer`]. See [`PreTrainedTokenizer.__call__`] and
            [`PreTrainedTokenizer.encode`] for details.

            To know more on how to prepare `inputs` for pre-training take a look at [SWITCHTRANSFORMERS
            Training](./switchtransformers#training).
        attention_mask (`tf.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Mask to avoid performing attention on padding token indices. Mask values selected in `[0, 1]`:
...
class TFSwitchTransformersModel(TFSwitchTransformersPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = TFSwitchTransformersMainLayer(encoder_config, embed_tokens, name="encoder")

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = TFSwitchTransformersMainLayer(decoder_config, embed_tokens, name="decoder")
    ...
    def call(
        ...
        ```python
        >>> from transformers import SwitchTransformersTokenizer, TFSwitchTransformersModel

        >>> tokenizer = SwitchTransformersTokenizer.from_pretrained("ybelkada/switchtransformers-base")
        >>> model = TFSwitchTransformersModel.from_pretrained("ybelkada/switchtransformers-base")
...
class TFSwitchTransformersForConditionalGeneration(TFSwitchTransformersPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def call(
        ...
        ```python
        >>> from transformers import SwitchTransformersTokenizer, TFSwitchTransformersForConditionalGeneration

        >>> tokenizer = SwitchTransformersTokenizer.from_pretrained("ybelkada/switchtransformers-base")
        >>> model = TFSwitchTransformersForConditionalGeneration.from_pretrained("ybelkada/switchtransformers-base")
        ...
        # SwitchTransformersv1.1 does not tie output word embeddings and thus does not require downscaling
        if self.config.tie_word_embeddings:
...
class TFSwitchTransformersEncoderModel(TFSwitchTransformersPreTrainedModel):
    ...
    def call(
        ...
        ```python
        >>> from transformers import SwitchTransformersTokenizer, TFSwitchTransformersEncoderModel

        >>> tokenizer = SwitchTransformersTokenizer.from_pretrained("ybelkada/switchtransformers-base")
        >>> model = TFSwitchTransformersEncoderModel.from_pretrained("ybelkada/switchtransformers-base")
