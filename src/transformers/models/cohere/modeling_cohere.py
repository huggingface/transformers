    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            emb = torch.repeat_interleave(
                freqs, 2, dim=-1
            )  # This line differs from Llama's implementation
            cos = emb.cos()
            sin = emb.sin()
    ...
    def __init__(self, config: CohereConfig, layer_idx: Optional[int] = None):
        ...
        if self.use_qk_norm:
            ...
            self.q_norm = CohereLayerNorm(
                hidden_size=(self.num_heads, self.head_dim), eps=config.layer_norm_eps
            )
...
class CohereDecoderLayer(nn.Module):
    def __init__(self, config: CohereConfig, layer_idx: int):
        ...
        self.self_attn = COHERE_ATTENTION_CLASSES[config._attn_implementation](
