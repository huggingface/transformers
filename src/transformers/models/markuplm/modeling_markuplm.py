...
class MarkupLMLayer(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.seq_len_dim = 1
        self.attention = MarkupLMAttention(config, layer_idx=layer_idx)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = MarkupLMAttention(config, position_embedding_type="absolute", layer_idx=layer_idx)
...
class MarkupLMEncoder(nn.Module):
    def __init__(self, config, layer_idx=None):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([MarkupLMLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
