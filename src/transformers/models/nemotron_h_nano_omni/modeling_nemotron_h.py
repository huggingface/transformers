...
class NemotronHMamba2Mixer(nn.Module):
    ...
    def forward(
        ...
        logger.warning_once(
            "Falling back to torch_forward for the Mamba2 mixer: numerics differ substantially "
            "from the fused CUDA kernels and will degrade model accuracy. Install mamba-ssm and "
            "causal-conv1d (and run on CUDA) to enable the fast path."
        )
...
class NemotronHMoE(nn.Module):
    ...
    def __init__(self, config, layer_idx: int | None = None):
        ...
        self.experts = nn.ModuleList(
            [
                NemotronHMLP(expert_config, intermediate_size=config.moe_intermediate_size)
