...
class FlaxUMT5LayerNorm(nn.Module):
    hidden_size: int
...
class FlaxUMT5DenseActDense(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32
...
class FlaxUMT5DenseGatedActDense(nn.Module):
    config: UMT5Config
...
class FlaxUMT5LayerFF(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation

    def setup(self):
        if self.config.is_gated_act:
            self.DenseReluDense = FlaxUMT5DenseGatedActDense(self.config, dtype=self.dtype)
        else:
            self.DenseReluDense = FlaxUMT5DenseActDense(self.config, dtype=self.dtype)
...
class FlaxUMT5LayerCrossAttention(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
...
class FlaxUMT5Block(nn.Module):
    ...
    def setup(self):
        ...
        if self.causal:
            self.layer += (FlaxUMT5LayerCrossAttention(self.config, name=str(1), dtype=self.dtype),)
            feed_forward_index += 1

        self.layer += (FlaxUMT5LayerFF(self.config, name=str(feed_forward_index), dtype=self.dtype),)
...
class FlaxUMT5BlockCollection(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    ...
    def setup(self):
        ...
        if self.gradient_checkpointing:
            FlaxUMT5CheckpointLayer = remat(FlaxUMT5LayerCollection, static_argnums=(6, 7, 8))
            self.blocks = [
                FlaxUMT5CheckpointLayer(
                    self.config,
                    has_relative_attention_bias=True,
                    dtype=self.dtype,
                    name=str(i),
...
class FlaxUMT5Stack(nn.Module):
    ...
    def setup(self):
        ...
        self.block = FlaxUMT5BlockCollection(
            self.config, dtype=self.dtype, gradient_checkpointing=self.gradient_checkpointing
        )
        self.final_layer_norm = FlaxUMT5LayerNorm(
...
UMT5_ENCODE_INPUTS_DOCSTRING = r"""
    Args:
...
UMT5_DECODE_INPUTS_DOCSTRING = r"""
    Args:
...
class FlaxUMT5PreTrainedModel(FlaxPreTrainedModel):
    """
    ...
    @add_start_docstrings(UMT5_ENCODE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=FlaxBaseModelOutput, config_class=UMT5Config)
    def encode(
        self,
        input_ids: jnp.ndarray,
        attention_mask: Optional[jnp.ndarray] = None,
        ...
        ```python
        >>> from transformers import AutoTokenizer, FlaxUMT5ForConditionalGeneration

        >>> tokenizer = AutoTokenizer.from_pretrained("umt5-small")
        >>> model = FlaxUMT5ForConditionalGeneration.from_pretrained("umt5-small")
    ...
    @replace_return_docstrings(output_type=FlaxBaseModelOutputWithPastAndCrossAttentions, config_class=UMT5Config)
    def decode(
        self,
        decoder_input_ids,
        encoder_outputs,
        ...
        >>> model = FlaxUMT5ForConditionalGeneration.from_pretrained("t5-small")
...
@add_start_docstrings(
    "The bare UMT5 Model transformer outputting raw hidden-stateswithout any specific head on top.",
    UMT5_START_DOCSTRING,
)
class FlaxUMT5Module(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    gradient_checkpointing: bool = False
...
class FlaxUMT5EncoderModule(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    gradient_checkpointing: bool = False
...
class FlaxUMT5EncoderModel(FlaxUMT5PreTrainedModel):
    module_class = FlaxUMT5EncoderModule

    @add_start_docstrings_to_model_forward(UMT5_ENCODE_INPUTS_DOCSTRING)
    def __call__(
        self,
        input_ids: jnp.ndarray,
        attention_mask: Optional[jnp.ndarray] = None,
...
class FlaxUMT5ForConditionalGenerationModule(nn.Module):
    config: UMT5Config
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation
    gradient_checkpointing: bool = False
...
class FlaxUMT5ForConditionalGeneration(FlaxUMT5PreTrainedModel):
    ...
    @add_start_docstrings(UMT5_DECODE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=FlaxCausalLMOutputWithCrossAttentions, config_class=UMT5Config)
    def decode(
        self,
        decoder_input_ids,
        encoder_outputs,
...
FLAX_UMT5_CONDITIONAL_GENERATION_DOCSTRING = """
    Returns:
