...
_CHECKPOINT_FOR_DOC = "umt5-small"
...
PARALLELIZE_DOCSTRING = r"""
    ...
    model = UMT5ForConditionalGeneration.from_pretrained("mt5-xl")
    device_map = {
        0: [0, 1, 2],
        1: [3, 4, 5, 6, 7, 8, 9],
...
class UMT5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer.append(UMT5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
...
class UMT5Stack(UMT5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [UMT5Block(config, has_relative_attention_bias=True) for i in range(config.num_layers)]
        )
...
class UMT5ForConditionalGeneration(UMT5PreTrainedModel):
    ...
    >>> model = UMT5ForConditionalGeneration.from_pretrained("google/mt5-small")
