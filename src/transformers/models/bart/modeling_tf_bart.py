...
def _make_causal_mask(input_ids_shape: List[int], past_key_values_length: int) -> tf.Tensor:
    """
...
def _expand_mask(mask: tf.Tensor, tgt_len: Optional[int] = None) -> tf.Tensor:
    """
...
class TFBartLearnedPositionalEmbedding(TFSharedEmbeddings):
    ...
    def call(self, input_shape: List[int], past_key_values_length: Optional[int] = 0):
        """Input is expected to be of size [bsz x seqlen]."""
        seq_len = input_shape[1]

        positions = tf.range(
            start=past_key_values_length, limit=seq_len + past_key_values_length, delta=1, name="range"
        )
...
class TFBartPretrainedModel(TFPreTrainedModel):
    ...
    def serving(self, inputs: Dict[str, tf.Tensor]) -> ModelOutput:
...
BART_INPUTS_DOCSTRING = r"""
    Args:
        ...
        head_mask (:obj:`np.ndarray` or :obj:`tf.Tensor` of shape :obj:`(encoder_layers, encoder_attention_heads)`, `optional`):
            Mask to nullify selected heads of the attention modules in the encoder. Mask values selected in ``[0, 1]``:

            - 1 indicates the head is **not masked**,
            - 0 indicates the heas is **masked**.

        decoder_head_mask (:obj:`np.ndarray` or :obj:`tf.Tensor` of shape :obj:`(decoder_layers, decoder_attention_heads)`, `optional`):
            Mask to nullify selected heads of the attention modules in the decoder. Mask values selected in ``[0, 1]``:

            - 1 indicates the head is **not masked**,
            - 0 indicates the head is **masked**.

        encoder_outputs (:obj:`np.ndarray` or :obj:`tf.Tensor`, `optional`):
...
class TFBartEncoder(tf.keras.layers.Layer):
    ...
    def get_embed_tokens(self) -> tf.keras.layers.Layer:
        return self.embed_tokens

    def set_embed_tokens(self, embed_tokens: tf.keras.layers.Layer):
        self.embed_tokens = embed_tokens
    ...
    def call(
        ...
        **kwargs,
        ...
        """
        Args:
            input_ids (:obj:`np.ndarray`, :obj:`tf.Tensor`, :obj:`List[tf.Tensor]` :obj:`Dict[str, tf.Tensor]` or :obj:`Dict[str, np.ndarray]` of shape :obj:`(batch_size, sequence_length)`):
                ...
                `What are input IDs? <../glossary.html#input-ids>`__
            attention_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
                Mask to avoid performing attention on padding token indices. Mask values selected in ``[0, 1]``:
                ...
                `What are attention masks? <../glossary.html#attention-mask>`__
            head_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(encoder_layers, encoder_attention_heads)`, `optional):
                Mask to nullify selected heads of the attention modules. Mask values selected in ``[0, 1]``:

                - 1 indicates the head is **not masked**,
                - 0 indicates the heas is **masked**.

            inputs_embeds (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
        ...
        if inputs["inputs_embeds"] is None:
            inputs["inputs_embeds"] = self.embed_tokens(inputs=inputs["input_ids"]) * self.embed_scale

        embed_pos = self.embed_positions(input_shape)
        hidden_states = inputs["inputs_embeds"] + embed_pos
        hidden_states = self.layernorm_embedding(inputs=hidden_states)
        hidden_states = self.dropout(inputs=hidden_states, training=inputs["training"])

        # check attention mask and invert
        if inputs["attention_mask"] is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            inputs["attention_mask"] = _expand_mask(mask=inputs["attention_mask"])
...
class TFBartDecoder(tf.keras.layers.Layer):
    ...
    def call(
        ...
        encoder_attention_mask: Optional[tf.Tensor] = None,
        head_mask: Optional[Union[np.ndarray, tf.Tensor]] = None,
        encoder_head_mask: Optional[tf.Tensor] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            attention_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
                ...
                `What are attention masks? <../glossary.html#attention-mask>`__
            encoder_hidden_states (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, encoder_sequence_length, hidden_size)`, `optional`):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
                of the decoder.
            encoder_attention_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, encoder_sequence_length)`, `optional`):
                ...
                `What are attention masks? <../glossary.html#attention-mask>`__
            head_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(decoder_layers, decoder_attention_heads)`, `optional`):
                Mask to nullify selected heads of the attention modules. Mask values selected in ``[0, 1]``:

                - 1 indicates the head is **not masked**,
                - 0 indicates the heas is **masked**.

            encoder_head_mask (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(encoder_layers, encoder_attention_heads)`, `optional`):
        ...
        if inputs["encoder_hidden_states"] is not None and inputs["encoder_attention_mask"] is not None:
            ...
            inputs["encoder_attention_mask"] = _expand_mask(
                mask=inputs["encoder_attention_mask"], tgt_len=input_shape[-1]
            )

        hidden_states = self.layernorm_embedding(inputs=hidden_states + positions)
        hidden_states = self.dropout(inputs=hidden_states, training=inputs["training"])
...
class TFBartMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: BartConfig, **kwargs):
        ...
        self.shared = TFSharedEmbeddings(
            ...
            initializer_range=config.pad_token_id,
            name="model.shared",
        )
        ...
        embed_tokens = TFWrappedEmbeddings(layer=self.shared, abs_scope_name=shared_abs_scope_name)
    ...
    def set_input_embeddings(self, new_embeddings: tf.Variable):
    ...
    def call(
        ...
        encoder_outputs: Optional[Union[Tuple[tf.Tensor], TFBaseModelOutput]] = None,
        past_key_values: Optional[Tuple[Tuple[tf.Tensor]]] = None,
        inputs_embeds: Optional[Union[np.ndarray, tf.Tensor]] = None,
...
class TFBartModel(TFBartPretrainedModel):
    ...
    def get_encoder(self) -> tf.keras.layers.Layer:
        return self.model.encoder

    def get_decoder(self) -> tf.keras.layers.Layer:
    ...
    def serving_output(self, output: TFSeq2SeqModelOutput) -> TFSeq2SeqModelOutput:
...
class TFBartForConditionalGeneration(TFBartPretrainedModel, TFCausalLanguageModelingLoss):
    ...
    def call(
        ...
        **kwargs
        ...
        r"""
        labels (:obj:`np.ndarray`, :obj:`tf.Tensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        lm_logits = self.model.shared(inputs=outputs[0], mode="linear")
        lm_logits = lm_logits + self.final_logits_bias
        masked_lm_loss = (
            None if inputs["labels"] is None else self.compute_loss(labels=inputs["labels"], logits=lm_logits)
        )
    ...
    def serving_output(self, output: TFSeq2SeqLMOutput) -> TFSeq2SeqLMOutput:
        pkv = tf.tuple(output.past_key_values)[1] if self.config.use_cache else None
        dec_hs = tf.convert_to_tensor(output.decoder_hidden_states) if self.config.output_hidden_states else None
        dec_attns = tf.convert_to_tensor(output.decoder_attentions) if self.config.output_attentions else None
        enc_hs = tf.convert_to_tensor(output.encoder_hidden_states) if self.config.output_hidden_states else None
