...
from .configuration_conformer import ConformerCTCConfig, ConformerEncoderConfig
...
class ConformerEncoderModelOutput(BaseModelOutput):
    attention_mask: torch.Tensor | None = None


@dataclass
class ConformerGenerateOutput(ModelOutput):
    """
    Outputs of Conformer models.
...
class ConformerEncoderRelPositionalEncoding(nn.Module):
    """Relative positional encoding for Conformer."""

    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: ConformerEncoderConfig, device=None):
        super().__init__()
...
class ConformerEncoderFeedForward(nn.Module):
    def __init__(self, config: ConformerEncoderConfig):
        super().__init__()
...
class ConformerEncoderConvolutionModule(nn.Module):
    def __init__(self, config: ConformerEncoderConfig, module_config=None):
        """
        Args:
            config (ConformerEncoderConfig): Configuration for the model.
        ...
        if module_config is None:
            # e.g. using `ConformerEncoderEncoderConfig` in src/transformers/models/conformer_encoder/configuration_conformer_encoder.py
...
class ConformerEncoderBlock(GradientCheckpointingLayer):
    def __init__(self, config: ConformerEncoderConfig, layer_idx: int | None = None):
        super().__init__()
        self.gradient_checkpointing = False

        self.feed_forward1 = ConformerEncoderFeedForward(config)
        self.self_attn = ConformerEncoderAttention(config, layer_idx)
        self.conv = ConformerEncoderConvolutionModule(config)
        self.feed_forward2 = ConformerEncoderFeedForward(config)
...
class ConformerPreTrainedModel(PreTrainedModel):
    config: ConformerCTCConfig
    base_model_prefix = "model"
    main_input_name = "input_features"
    input_modalities = "audio"
    supports_gradient_checkpointing = True
    _no_split_modules = ["ConformerEncoderBlock"]
    ...
    _can_record_outputs = {
        "hidden_states": ConformerEncoderBlock,
        "attentions": ConformerEncoderAttention,
    }
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ConformerEncoderAttention):
        ...
        elif isinstance(module, ConformerEncoderRelPositionalEncoding):
            inv_freq = 1.0 / (
    ...
    def _get_subsampling_output_length(self, input_lengths: torch.Tensor):
        encoder_config = self.config.encoder_config if isinstance(self.config, ConformerCTCConfig) else self.config
...
class ConformerEncoder(ConformerPreTrainedModel):
    ...
    def __init__(self, config: ConformerEncoderConfig):
        ...
        self.subsampling = ConformerEncoderSubsamplingConv2D(config)
        self.encode_positions = ConformerEncoderRelPositionalEncoding(config)

        self.layers = nn.ModuleList(
            [ConformerEncoderBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return ConformerEncoderModelOutput(
...
class ConformerForCTC(ConformerPreTrainedModel):
    ...
    def __init__(self, config: ConformerCTCConfig):
        super().__init__(config)
        self.encoder = ConformerEncoder(config.encoder_config)
    ...
    def generate(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> ConformerGenerateOutput | torch.LongTensor:
        r"""
        Example:
        ...
        if return_dict_in_generate:
            return ConformerGenerateOutput(
                sequences=sequences,
...
__all__ = ["ConformerEncoder", "ConformerForCTC", "ConformerPreTrainedModel"]
