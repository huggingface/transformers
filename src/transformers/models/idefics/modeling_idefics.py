...
def expand_inputs_for_generation(
    ...
    **model_kwargs,
    ...
    if "token_type_ids" in model_kwargs:
        ...
        model_kwargs["token_type_ids"] = token_type_ids.repeat_interleave(expand_size, dim=0)

    if attention_mask is not None:
        model_kwargs["attention_mask"] = attention_mask.repeat_interleave(expand_size, dim=0)

    if model_kwargs["image_attention_mask"] is not None:
        model_kwargs["image_attention_mask"] = model_kwargs["image_attention_mask"].repeat_interleave(
            expand_size, dim=0
        )
    ...
    if model_kwargs["pixel_values"] is not None:
        ...
        model_kwargs["image_encoder_embeddings"] = model_kwargs["image_encoder_embeddings"].repeat_interleave(
            expand_size, dim=0
        )

    elif model_kwargs["perceiver_embeddings"] is not None:
        model_kwargs["perceiver_embeddings"] = model_kwargs["perceiver_embeddings"].repeat_interleave(
            expand_size, dim=0
        )
