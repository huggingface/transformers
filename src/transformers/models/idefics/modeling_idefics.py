...
class IdeficsBaseModelOutputWithPast(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(torch.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
        image_hidden_states (`tuple(torch.Tensor)`, *optional*):
            Tuple of `torch.Tensor` (one for the output of the image embeddings, `(batch_size, num_images,
...
class IdeficsGatedCrossAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor`, *optional*): attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            image_attention_mask (`torch.Tensor`, *optional*): image attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            cross_attention_gate (`torch.Tensor`, *optional*):
...
class IdeficsModel(IdeficsPreTrainedModel):
    ...
    def forward(
        ...
        image_encoder_embeddings: Optional[torch.Tensor] = None,
        perceiver_embeddings: Optional[torch.Tensor] = None,
        image_attention_mask: Optional[torch.Tensor] = None,
