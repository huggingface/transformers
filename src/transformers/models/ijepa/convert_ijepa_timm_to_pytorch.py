...
"""Convert I-JEPA and non-distilled DeiT checkpoints from the timm library."""
...
@torch.no_grad()
def convert_ijepa_checkpoint(ijepa_name, pytorch_dump_folder_path):
    """
    ...
    timm_model = timm.create_model(ijepa_name, pretrained=True)
    timm_model.eval()
    ...
    # Check outputs on an image, prepared by IJepaImageProcessor/DeiTImageProcessor
    if "deit" in ijepa_name:
        image_processor = DeiTImageProcessor(size=config.image_size)
    else:
        image_processor = IJepaImageProcessor(size=config.image_size)
    ...
    print(f"Saving model {ijepa_name} to {pytorch_dump_folder_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ijepa_name",
        default="ijepa_base_patch16_224",
        type=str,
        help="Name of the IJEPA timm model you'd like to convert.",
    )
    parser.add_argument(
    ...
    convert_ijepa_checkpoint(args.ijepa_name, args.pytorch_dump_folder_path)
