...
class ShieldGemma2ProcessorKwargs(Gemma3ProcessorKwargs, total=False):
    ...
    policies (`Sequence[str]`, *optional*):
        Names of the policies to check the images against. Defaults to every key of the processor's
        `policy_definitions`, extended with `custom_policies`.
    custom_policies (`Mapping[str, str]`, *optional*):
        Extra policy name to policy text entries, merged on top of the processor's `policy_definitions`.
    """
...
class ShieldGemma2Processor(Gemma3Processor):
    def __init__(
        ...
        r"""
        policy_definitions (`Mapping[str, str]`, *optional*):
            Mapping from policy name to the policy text inserted in the prompt. Defaults to the built-in
            ShieldGemma 2 policies.
        """
