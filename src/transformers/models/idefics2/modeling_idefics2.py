...
# Copied from transformers.models.siglip.modeling_siglip.SiglipEncoderLayer with Siglip->Idefics2Vision
class Idefics2VisionEncoderLayer(nn.Module):
    def __init__(self, config: Idefics2VisionConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
...
IDEFICS2_VISION_INPUTS_DOCSTRING = r"""
    Args:
...
# Copied from transformers.models.mistral.modeling_mistral.MistralAttention with MistralAttention->Idefics2DecoderAttention,Mistral->Idefics2
class Idefics2DecoderAttention(nn.Module):
    """
    ...
    def __init__(self, config: Idefics2Config, layer_idx: Optional[int] = None):
        ...
        self.rotary_emb = Idefics2RotaryEmbedding(
            self.head_dim,
...
# Copied from transformers.models.mistral.modeling_mistral.MistralDecoderLayer with Mistral->Idefics2, MISTRAL->IDEFICS2
class Idefics2DecoderLayer(nn.Module):
    def __init__(self, config: Idefics2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = IDEFICS2_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        self.mlp = Idefics2MLP(config)
        self.input_layernorm = Idefics2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Idefics2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@add_start_docstrings(
    """The IDEFICS2 model which consists of a vision backbone and a language model.""",
    IDEFICS2_START_DOCSTRING,
)
# Copied from transformers.models.llava.modeling_llava.LlavaForConditionalGeneration with LLAVA->IDEFICS2,Llava->Idefics2,llava-hf/llava-1.5-7b-hf->HuggingFaceM4
class Idefics2ForConditionalGeneration(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        >>> model = Idefics2ForConditionalGeneration.from_pretrained("HuggingFaceM4")
        >>> processor = AutoProcessor.from_pretrained("HuggingFaceM4")
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                if past_key_values is not None and pixel_values is not None and input_ids.shape[1] == 1:
                    ...
                    # if one uses Idefics2 + Fused modules where the cache on the
