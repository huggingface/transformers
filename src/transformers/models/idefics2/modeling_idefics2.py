...
class Idefics2PerceiverAttention(nn.Module):
    def __init__(self, config: Idefics2PerceiverConfig, layer_idx: Optional[int] = None):
...
class Idefics2PerceiverLayer(nn.Module):
    def __init__(self, config: Idefics2PerceiverConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        latents, attn_weights = self.self_attn(
            latents=latents,
            context=context,
            attention_mask=attention_mask,
        )
...
class Idefics2PerceiverResampler(Idefics2PreTrainedModel):
    ...
    def __init__(self, config: Idefics2PerceiverConfig) -> None:
        super().__init__(config)
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        for perceiver_layer in self.layers:
            compressed_context, _ = perceiver_layer(compressed_context, context, attention_mask=attention_mask)
...
class Idefics2ForConditionalGeneration(Idefics2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs: Idefics2BaseModelOutputWithPast = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
