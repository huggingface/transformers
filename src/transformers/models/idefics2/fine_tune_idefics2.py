...
from transformers import AutoProcessor, BitsAndBytesConfig, Idefics2ForConditionalGeneration, GenerationConfig
...
class MyDataCollator:
    ...
    def __call__(self, examples):
        ...
        batch = processor(text=texts, images=images, return_tensors="pt", truncation=True, padding="max_length", max_length=200)
...
class Idefics2Trainer(Seq2SeqTrainer):
    def prediction_step(
        ...
        **gen_kwargs,
        ...
        custom_inputs["input_ids"] = processor_inputs.input_ids.repeat(batch_size, 1).to(device) # repeat along batch dimension
        custom_inputs["attention_mask"] = processor_inputs.attention_mask.repeat(batch_size, 1).to(device) # repeat along batch dimension
