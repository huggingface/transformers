...
from .integrations import (PeftAdapterMixin, deepspeed_config,
...
from .utils import (ACCELERATE_MIN_VERSION, ADAPTER_SAFE_WEIGHTS_NAME,
                    ADAPTER_WEIGHTS_NAME, CONFIG_NAME, DUMMY_INPUTS,
                    FLAX_WEIGHTS_NAME, SAFE_WEIGHTS_INDEX_NAME,
                    ...
                    download_url, extract_commit_hash, has_file,
                    is_accelerate_available, is_bitsandbytes_available,
                    is_flash_attn_2_available, is_offline_mode,
                    is_optimum_available, is_peft_available, is_remote_url,
                    is_safetensors_available, is_torch_flex_attn_available,
                    is_torch_greater_or_equal, is_torch_npu_available,
                    is_torch_sdpa_available, is_torch_xla_available, logging,
                    replace_return_docstrings, strtobool)
from .utils.hub import create_and_tag_model_card, get_checkpoint_shard_files
from .utils.import_utils import (ENV_VARS_TRUE_VALUES, is_sagemaker_mp_enabled,
                                 is_torch_fx_proxy, is_torchdynamo_compiling)
...
def check_support_param_buffer_assignment(model_to_load, state_dict):
    ...
    if first_key in state_dict:
        return (
            state_dict[first_key].dtype == model_to_load.state_dict()[first_key].dtype
        )
...
class PreTrainedModel(
    ...
    def _autoset_attn_implementation(
        ...
        if (
            ...
            ) and config._attn_implementation not in ["eager"] + list(
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if device_map is not None:
            if low_cpu_mem_usage is None:
                ...
                raise ValueError(
                    "Passing along a `device_map` or a `tp_plan` requires `low_cpu_mem_usage=True`"
                )
        ...
        model_init_context = cls.get_init_context(
            _fast_init, is_quantized, _is_ds_init_called, low_cpu_mem_usage
        )
    ...
    def _load_pretrained_model(
        ...
        if low_cpu_mem_usage:
            model._move_missing_keys_from_meta_to_cpu(
                missing_keys, unexpected_keys, dtype, hf_quantizer
            )
        ...
        for shard_file in checkpoint_files:
            ...
            if low_cpu_mem_usage:
                ...
                assign_params = check_support_param_buffer_assignment(
                    model_to_load, state_dict
                )
                if is_deepspeed_zero3_enabled():
                    error_msgs += _load_state_dict_into_zero3_model(
                        model_to_load, state_dict, assign_params
                    )
                else:
                    model_to_load.load_state_dict(
                        state_dict, strict=False, assign=assign_params
                    )
...
def caching_allocator_warmup(model: PreTrainedModel, expanded_device_map: Dict):
    ...
    for device, byte_count in total_byte_count.items():
        ...
        # Allocate memory
        _ = torch.empty(
            byte_count // 2, dtype=torch.float16, device=device, requires_grad=False
        )
