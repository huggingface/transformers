...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _autoset_attn_implementation(
        ...
        if hasattr(config, "_attn_implementation_internal") and config._attn_implementation_internal is not None:
            ...
            if config._attn_implementation not in ["eager", "sdpa", "flash_attention_2", "simple"]:
        ...
        elif requested_attn_implementation in ["simple"]:
            # do nothing and return config._attn_implementation as it is
            logger.warning_once(
                f'{config._attn_implementation=} (simple attention in use)'
            )
        else:
