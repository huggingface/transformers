...
# NOTE: The only difference with create_causal_mask is that we select here sliding instead of full layers.
# TODO: Check if we really want to keep this. The only reason we need it is in case we have a hybrid cache
# with different layer types. In the default case where config.layer_types contains a single type of layer
# create_causal_mask is sufficient.
def create_reference_sliding_window_causal_mask(
    ...
    """
    Create the mask for the reference-sliding layers. This mirrors `create_sliding_window_causal_mask` (it reads the
    sizes from the sliding cache layer), but uses a plain causal mask function instead of a sliding window one: the
    reference-sliding cache already holds the window physically as a ring buffer (prefill + the last `sliding_window`
    generated tokens) and reports its physical length with offset 0. A sliding window mask function would wrongly
    evict the prefill; a plain causal mask instead stays correctly causal during prefill and degenerates to full
    visibility over the physical buffer during single-token decode (every cached key index <= the large query
    offset) — which is exactly what the order-agnostic ring buffer needs (RoPE is baked into the cached keys).
