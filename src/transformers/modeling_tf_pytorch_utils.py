...
from .utils import (ExplicitEnum, expand_dims, is_numpy_array,
                    is_safetensors_available, is_torch_tensor, logging,
                    reshape, squeeze, tensor_size)
...
def convert_tf_weight_name_to_pt_weight_name(
    ...
    if (
        ...
        and len(tf_weight_shape) == 4
    ):
        transpose = TransposeType.CONV2D
    elif (
        tf_name[-1] == "kernel"
        and tf_weight_shape is not None
        and len(tf_weight_shape) == 3
    ):
...
def _log_key_warnings(missing_keys, unexpected_keys, mismatched_keys, class_name):
    if len(unexpected_keys) > 0:
        ...
        logger.warning(
            f"All PyTorch model weights were used when initializing {class_name}.\n"
        )
...
def load_pytorch_state_dict_in_tf2_model(
    ...
    if not any(
        s.startswith(tf_model.base_model_prefix) for s in tf_keys_to_pt_keys.keys()
    ):
    ...
    for symbolic_weight in symbolic_weights:
        ...
        if tf_to_pt_weight_rename is not None:
            aliases = tf_to_pt_weight_rename(
                name
    ...
    if not skip_logger_warnings:
        _log_key_warnings(
            ...
            unexpected_keys,
            mismatched_keys,
            class_name=tf_model.__class__.__name__,
        )
...
def load_sharded_pytorch_safetensors_in_tf2_model(
    ...
    missing_keys = sorted(
        set.intersection(*[set(info["missing_keys"]) for info in all_loading_infos])
    )
...
def load_tf2_state_dict_in_pytorch_model(
    pt_model, tf_state_dict, allow_missing_keys=False, output_loading_info=False
):
    import torch
