...
class Trainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = self.get_optimizer_cls_and_kwargs(opt_model)
    ...
    def get_optimizer_cls_and_kwargs(self, model: Optional[PreTrainedModel] = None) -> Tuple[Any, Any]:
        """
        ...
        if self.args.optim == OptimizerNames.ADAFACTOR:
        ...
        elif self.args.optim == OptimizerNames.ADAMW_HF:
            from .optimization import AdamW

            optimizer_cls = AdamW
            optimizer_kwargs.update(adam_kwargs)
        elif self.args.optim in [OptimizerNames.ADAMW_TORCH, OptimizerNames.ADAMW_TORCH_FUSED]:
            from torch.optim import AdamW

            optimizer_cls = AdamW
            optimizer_kwargs.update(adam_kwargs)
            if self.args.optim == OptimizerNames.ADAMW_TORCH_FUSED:
                optimizer_kwargs.update({"fused": True})
        elif self.args.optim == OptimizerNames.ADAMW_TORCH_XLA:
            try:
        ...
        elif self.args.optim == OptimizerNames.ADAMW_TORCH_NPU_FUSED:
            try:
        ...
        elif self.args.optim == OptimizerNames.ADAMW_APEX_FUSED:
            try:
            ...
            try:
                ...
                if "paged" in self.args.optim:
                    is_paged = True
                if "8bit" in self.args.optim:
                    optim_bits = 8
                if "adam" in self.args.optim:
                    optimizer_cls = AdamW
                elif "lion" in self.args.optim:
                    optimizer_cls = Lion
                    additional_optim_kwargs = {"betas": (self.args.adam_beta1, self.args.adam_beta2)}
                elif "rmsprop" in self.args.optim:
                ...
                if "rmsprop" not in self.args.optim:
        ...
        elif self.args.optim == OptimizerNames.ADAMW_ANYPRECISION:
            try:
        ...
        elif self.args.optim == OptimizerNames.SGD:
            optimizer_cls = torch.optim.SGD
        elif self.args.optim == OptimizerNames.ADAGRAD:
            optimizer_cls = torch.optim.Adagrad
        elif self.args.optim == OptimizerNames.RMSPROP:
            ...
            is_layerwise = self.args.optim.lower().endswith("layerwise")
            if is_layerwise and self.args.parallel_mode == ParallelMode.DISTRIBUTED:
            ...
            optimizer_cls = optimizer_mapping[self.args.optim]

            if self.args.optim_target_modules is None:
                raise ValueError(
                    "You need to define a `optim_target_modules` in order to properly use GaLore optimizers"
                )

            if not isinstance(self.args.optim_target_modules, (list, str)):
                raise ValueError(
                    f"`optim_target_modules` has to be a list of strings, a string corresponding to a regex, or a specific module or 'all-linear', you passed {self.args.optim_target_modules}"
                )
            ...
            all_linear = (
                isinstance(self.args.optim_target_modules, str)
                and self.args.optim_target_modules.replace("_", "-") == "all-linear"
            )
            ...
            for module_name, module in model.named_modules():
                target_module_exists, is_regex = check_target_module_exists(
                    self.args.optim_target_modules, module_name, return_is_regex=True
                )
            ...
            if len(galore_params) == 0:
                raise ValueError(
                    f"None of the target modules were found! ({self.args.optim_target_modules}). Please make sure to pass a valid `target_modules`."
                )
