...
class Trainer:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if labels is not None:
                labels = self.accelerator.pad_across_processes(labels, pad_index=1)
            if inputs_decode is not None:
                inputs_decode = self.accelerator.pad_across_processes(inputs_decode, pad_index=1)
            ...
            if logits is not None:
                logits = self.accelerator.pad_across_processes(logits, pad_index=1)
