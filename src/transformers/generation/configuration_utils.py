...
class GenerationConfig(PushToHubMixin):
    ...
    Arg:
        ...
        offload_logits_to_cpu (`bool`, *optional*, defaults to `False`):
            Whether or not to transfer logits, scores, and attentions to CPU memory after each generation step to reduce GPU memory
            usage. Only effective when `output_scores=True`, `output_logits=True`, or `output_attentions=True`. This can significantly reduce
            GPU memory consumption during long generations at the cost of some CPU-GPU transfer overhead.
            NB: if `offload_logits_to_cpu=True`, the returned `scores`, `logits`, and `attentions` tensors in the generation output
