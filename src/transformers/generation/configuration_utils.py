...
class GenerationConfig(PushToHubMixin):
    ...
    Arg:
        ...
        max_assistant_tokens (`int`, *optional*, defaults to 5):
            Defines the number of _speculative tokens_ that shall be generated by the assistant model before
            being checked by the target model at each iteration. Higher values for `max_assistant_tokens` make 
            the generation more _speculative_ : If the assistant model is performant larger speed-ups can be reached, 
            if the assistant model requires lots of corrections, lower speed-ups are reached.

        max_assistant_tokens_schedule (`str`, *optional*, defaults to `"heuristic"`):
            Defines the schedule at which max assistant tokens shall be changed during inference.
            - `"_heuristic_`: When all _speculative_ tokens are correct, increase `max_assistant_tokens` by 2 else reduce by 1
            - `"constant"`: `max_assistant_tokens` stays unchanged during generation
    ...
    def __init__(self, **kwargs):
        ...
        self.max_assistant_tokens = kwargs.pop("max_assistant_tokens", 5)
        self.max_assistant_tokens_schedule  = kwargs.pop("max_assistant_tokens_schedule", "heuristic")
