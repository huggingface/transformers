...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        vocab_tensor = torch.arange(scores.shape[-1], device=scores.device)
        begin_suppress_tokens = self.begin_suppress_tokens[self.begin_suppress_tokens < scores.shape[-1]]
        suppress_token_mask = torch.isin(vocab_tensor, begin_suppress_tokens.to(scores.device))
        scores_processed = scores
...
class SuppressTokensLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        vocab_tensor = torch.arange(scores.shape[-1], device=scores.device)
        suppress_tokens = self.suppress_tokens[self.suppress_tokens < scores.shape[-1]]
        suppress_token_mask = torch.isin(vocab_tensor, suppress_tokens.to(scores.device))
