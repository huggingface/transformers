        ...
        if scores.dim() == 3:
            if self.logits_indices is not None and self.cu_seq_lens_q is not None:
                ...
                seq_indices = torch.repeat_interleave(
                    torch.arange(len(lengths), device=input_ids.device), lengths
                )
                ...
                penalty_scores = torch.where(
                    last_scores < 0,
                    last_scores * self.penalty,
                    last_scores / self.penalty,
                )
                scores[0, last_positions, :] = torch.where(
                    token_mask, penalty_scores, last_scores
                )
            else:
                batch_size, seq_len, vocab_size = scores.shape
            ...
            bad_words_ids = list(
                filter(
                    lambda bad_token_seq: all(
                        bad_token_seq != [i] for i in eos_token_id_list
                    ),
                    bad_words_ids,
                )
            )
...
class ConfigurableLogitsProcessorList(LogitsProcessorList):
    ...
    @classmethod
    def from_config(
        cls, config: Union[str, list[dict[str, Any]]]
        ...
        if not isinstance(config, list):
            raise ValueError(
                "logit_processors config must be a list of processor configurations"
            )
