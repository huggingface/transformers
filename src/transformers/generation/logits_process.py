...
class EpsilonLogitsWarper(LogitsWarper):
    ...
    >>> # Another example where the next tokens are different from the common case mentioned above
    >>> outputs = model.generate(**inputs, max_new_tokens=4, do_sample=True)
    >>> print(tokenizer.decode(outputs[0], skip_special_tokens=True))
    The full name of Donald is Donald McGaw. We

    >>> # Same example where the next tokens generated by using `epsilon_cutoff` are from the common case
    >>> outputs = model.generate(
    ...     **inputs, max_new_tokens=4, do_sample=True, epsilon_cutoff=6e-4
    ... )  # need to set do_sample=True for epsilon_cutoff to work
    >>> print(tokenizer.decode(outputs[0], skip_special_tokens=True))
