...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # In some very rare cases, the embedding vocab may be larger than the lm_head output - try to recover in those cases by padding
        # the logits (the tokens with id > output_vocab cannot be generated by the model, so there's no need to apply a penalty to them)
        except RuntimeError:
            vocab_size = scores.shape[-1]
            scores = torch.nn.functional.pad(scores, (0, input_ids.max() + 1 - vocab_size))
            score = torch.gather(scores, 1, input_ids)
            score = torch.where(score < 0, score * self.penalty, score / self.penalty)
            scores_processed = scores.scatter(1, input_ids, score)
            # Drop whatever we padded, since thoses indices are outside the output vocab they cannot be generated so no need to apply penalty
