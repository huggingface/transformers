...
from .beam_search import BeamScorer, BeamSearchScorer, CandidateBeamSearchScorer, ConstrainedBeamSearchScorer
...
class GenerationMode(ExplicitEnum):
    ...
    CANDIDATE_BEAM_SEARCH = "candidate_beam_search"
...
class GenerationMixin:
    ...
    def _get_generation_mode(
        ...
        elif generation_config.candidate_words_ids is not None:
            generation_mode = GenerationMode.CANDIDATE_BEAM_SEARCH
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
        ...
        elif generation_mode == GenerationMode.CANDIDATE_BEAM_SEARCH:
            # 11. prepare candiate beam search scorer
            beam_scorer = CandidateBeamSearchScorer(
                batch_size=batch_size,
                num_beams=generation_config.num_beams,
                candidate_words_ids=generation_config.candidate_words_ids,
                eos_token_id=generation_config.eos_token_id,
                device=inputs_tensor.device,
                length_penalty=generation_config.length_penalty,
                do_early_stopping=generation_config.early_stopping,
            ...
            # 13. run candidate beam search
            return self.candidate_beam_search(
                input_ids,
                beam_scorer,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
    ...
    def candidate_beam_search(
        ...
        **model_kwargs,
        ...
        Parameters:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
                The sequence used as a prompt for the generation.
            candidate_beam_scorer (`CandidateBeamSearchScorer`):
                An derived instance of [`BeamScorer`] that defines how beam hypotheses are constructed, stored and
                sorted during generation, while satisfying a list of candidates. For more information, the documentation of [`CandidateBeamSearchScorer`] should be read.
            logits_processor (`LogitsProcessorList`, *optional*):
                An instance of [`LogitsProcessorList`]. List of instances of class derived from [`LogitsProcessor`]
                used to modify the prediction scores of the language modeling head applied at each generation step.
            stopping_criteria (`StoppingCriteriaList`, *optional*):
        ...
        >>> # Encode candidates
        >>> candidate_list = ['Wie alt ist Großvater?', 'Wie alt ist dein Bruder?']
        >>> candidate_words_ids = [tokenizer.encode(candidate) for candidate in candidate_list]

        >>> # instantiate candidate beam scorer
        >>> beam_scorer = CandidateBeamSearchScorer(
        ...     batch_size=1,
        ...     num_beams=num_beams,
        ...     candidate_words_ids=candidate_words_ids,
        ...
        >>> outputs = model.candidate_beam_search(input_ids, beam_scorer, logits_processor=logits_processor, **model_kwargs)
        ...
        while True:
            ...
            # Masking the entries that are not possible in next_token_scores
            valid_token_list = [
                list(nested_candidate_dict.keys()) for nested_candidate_dict in nested_candidate_dict_list
            ]
            mask = torch.ones_like(next_token_scores_processed)
            mask = mask * float("-inf")
            for i, sublist in enumerate(valid_token_list):
                if not sublist:
                    continue
                sublist_device = torch.tensor(sublist, device=input_ids.device)
                inf_idx = next_token_scores_processed[i, sublist_device] == float("-inf")
                min_score = torch.topk(next_token_scores_processed[i, :], len(sublist))
                next_token_scores_processed[i, sublist_device[inf_idx].tolist()] = min_score.values[-1].item()
                mask[i, sublist] = 0.0
            next_token_scores_processed = torch.add(next_token_scores_processed, mask)
