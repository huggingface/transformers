    ...
    def generate(
        ...
        **kwargs,
        ...
        if not kwargs_has_attention_mask and not self.config.is_encoder_decoder and accepts_attention_mask:
            ...
            if self.config._attn_implementation == "flash_attention_torch" and (model_kwargs["attention_mask"] == 1).all():
                raise ValueError("Flash Attention torch cannot handle generations without valid `attention_mask` with padding.")
