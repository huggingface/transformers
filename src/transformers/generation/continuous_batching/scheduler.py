...
class Scheduler(ABC):
    ...
    def add_waiting_request(self, state: RequestState):
        ...
        if self.retain_cache_on_finish and state.request_id in self.active_requests:
            ...
            state.scheduled_tokens = state.scheduled_tokens[len(old_state.initial_tokens) :]  # XXX: check for indexing error?
    ...
    def _allocate_blocks_if_needed(self, state: RequestState) -> bool:
        ...
        len_next_tokens = len(state.scheduled_tokens)
    ...
    def _prepare_request_for_processing(
        ...
        if self.cache.use_prefix_sharing and state.status == RequestStatus.PENDING:
            prefill_length = self.cache.search_prefix_match(state.request_id, state.scheduled_tokens)
            if prefill_length > 0:
                ...
                prefill_length = min(prefill_length, len(state.scheduled_tokens) - 1)
                state.remaining_prefill_tokens = state.scheduled_tokens[prefill_length:]
                state.scheduled_tokens = state.scheduled_tokens[prefill_length:]
        ...
        # Otherwise, the tokens to process are the prompt ids, which are the full prompt or the last predicted tokens
            ...
            request_tokens = state.scheduled_tokens
        ...
        if len(request_tokens) < token_budget:
            ...
            if state.status == RequestStatus.PENDING:
                ...
                state.status = RequestStatus.PREFILLING
                state.scheduled_tokens = state.remaining_prefill_tokens
            ...
            state.scheduled_tokens = request_tokens[:token_budget]
...
class FIFOScheduler(Scheduler):
    ...
    def schedule_batch(self, token_budget: int) -> list[RequestState]:
        ...
        for state in candidates:
            ...
            request_len = len(state.scheduled_tokens)
