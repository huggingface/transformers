...
class MtpLayer(nn.Module):
    ...
    def __init__(self, config: PreTrainedConfig, decoder_layer: nn.Module):
        super().__init__()
...
class MtpLayerStack(PreTrainedModel):
    ...
    def __init__(self, main_model: PreTrainedModel, num_mtp_layers: int):
        ...
        self.layers = nn.ModuleList(
            [MtpLayer(self.config, layer_cls(self.config, self.config.num_hidden_layers + k)) for k in range(num_mtp_layers)]
        )
