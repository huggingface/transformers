...
def accuracy_precision_recall_f1(confusion_matrix: tf.Tensor, num_classes: int, positive_indices: Optional[list] = None):
    """
    ...
    accuracy_score = tf.math.divide_no_nan(tf.reduce_sum(tf.linalg.diag_part(confusion_matrix)), tf.reduce_sum(confusion_matrix))
...
def metrics_from_confusion_matrix(confusion_matrix: tf.Tensor, num_classes: int, positive_indices: Optional[list] = None, average: str = 'micro'):
    """
    Compute the Precision, Recall and F1 scores from the confusion matrix
    Args:
        confusion_matrix (:obj:`tf.Tensor`):
            The confusion matrix.
        num_classes (:obj:`int`):
        ...
        average (:obj:`str`, `optional`, defaults to micro):
            micro: True positivies, false positives and
                false negatives are computed globally for the
                classes in `pos_indices`.
            'macro': True positivies, false positives and
                false negatives are computed for each class in
    ...
    if average == 'micro':
        ...
        for idx in positive_indices:
            accuracy_score, precision_score, recall_score, f1_score = accuracy_precision_recall_f1(confusion_matrix, num_classes, [idx])
...
class F1AndAccuracyMeanScore(tf.keras.metrics.Metric):
    """Computes the mean between the F-1 and the accuracy scores.
    F-1 is the weighted harmonic mean of precision
    and recall. Works for both multi-class and multi-label
    classification.
    F-1 = 2 * ((prec * recall) / (prec + recall))
    ...
    def __init__(self,
        ...
        self.confusion_matrices = self.add_weight("confusion_matrices", shape=(num_classes, num_classes), initializer="zeros", dtype=tf.float32)
    ...
    def result(self):
        accuracy_score, _, _, f1_score = metrics_from_confusion_matrix(self.confusion_matrices, self.num_classes, self.positive_indices, self.average)
...
class F1Score(tf.keras.metrics.Metric):
    ...
    def result(self):
        _, _, _, f1_score = metrics_from_confusion_matrix(self.confusion_matrices, self.num_classes, self.positive_indices, self.average)
