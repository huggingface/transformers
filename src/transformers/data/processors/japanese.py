...
from ...tokenization_bert import convert_to_unicode
...
def japanese_convert_examples_to_features(examples, tokenizer,
                                      max_length=512,
                                      task=None,
                                      label_list=None,
                                      output_mode=None,
    ...
    if task is not None:
        processor = japanese_processors[task]()
        if label_list is None:
            label_list = processor.get_labels()
            logger.info("Using label list %s for task %s" % (label_list, task))
        if output_mode is None:
            output_mode = japanese_output_modes[task]
