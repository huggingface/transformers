...
@dataclass
class DataCollatorForNSP(DataCollatorForLanguageModeling):
    """
    Data collator used for next sentence classification task.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for both masked language modeling and next sentence prediction
    """
    ...
    def __call__(self, examples: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        nsp_label_list = [example["next_sentence_label"] for example in examples]
        next_sentence_label = torch.stack(nsp_label_list)
