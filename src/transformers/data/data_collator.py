...
class DataCollatorForSeqParallel:
    """
    Data collator for sequence parallel training. It shards the input tensors for the current sequence parallel rank.
    """

    def __init__(
        self,
        data_collator,
        sequenece_parallel_rank=0,
        sequence_parallel_size=1,
    ):
        self.initialized = False
        self.data_collator = data_collator
        self.sequence_parallel_rank = sequenece_parallel_rank
        self.sequence_parallel_size = sequence_parallel_size

    def __call__(self, features):
        batch = self.data_collator(features)

        # print("HERE COLLATING YOUR BATCHES")

        if self.sequence_parallel_size == 1:
            return batch

        for prefix in ["", "prompt_", "chosen_", "rejected_"]:
            input_ids_key = f"{prefix}input_ids"
            if input_ids_key in batch:
                seq_length = batch[input_ids_key].size(1)
                sub_seq_length = seq_length // self.sequence_parallel_size

                position_ids_key = f"{prefix}position_ids"
                if position_ids_key not in batch:
                    position_ids = np.arange(0, seq_length)
                    batch[position_ids_key] = position_ids.unsqueeze(0).view(-1, seq_length)

                sp_rank = self.sequence_parallel_rank

                # Get the shard of the input tensors corresponding to the current sequence parallel rank
                for k in [input_ids_key, f"{prefix}attention_mask", position_ids_key, f"{prefix}input_embeds"]:
                    if k in batch and batch[k] is not None:
                        # print(f"{k}_before", batch[k].shape)
                        batch[k] = batch[k][:, sp_rank * sub_seq_length : (sp_rank + 1) * sub_seq_length]
                        # print(f"{k}_after", batch[k].shape)
