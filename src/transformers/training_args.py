...
class TrainingArguments:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.fsdp_config, str):
            if len(self.fsdp) == 0:
                logging.warning("`--fsdp_config` is useful only when `--fsdp` is specified.")
        ...
        if self.fsdp_min_num_params > 0:
            logging.warning("using `--fsdp_min_num_params` is deprecated. Use fsdp_config instead ", FutureWarning)
        ...
        if len(self.fsdp) == 0 and self.fsdp_config["min_num_params"] > 0:
            logging.warning("`min_num_params` is useful only when `--fsdp` is specified.")

        if len(self.fsdp) == 0 and self.fsdp_config.get("transformer_layer_cls_to_wrap", None) is not None:
            logging.warning("`transformer_layer_cls_to_wrap` is useful only when `--fsdp` is specified.")
        ...
        if self.fsdp_config["xla"]:
            if len(self.fsdp) > 0:
                ...
                logging.warning("XLA FSDP can be used only when `--fsdp` is specified.")
        else:
            if self.fsdp_config["xla_fsdp_grad_ckpt"]:
                logging.warning("`--xla_fsdp_grad_ckpt` is useful only when `--xla` is set to true.")
