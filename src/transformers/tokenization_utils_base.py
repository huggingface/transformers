...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    @classmethod
    def get_tokenizer_config_dict(
        cls, pretrained_model_name_or_path: Union[str, os.PathLike], **kwargs
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
        From a `pretrained_model_name_or_path`, resolve to a dictionary of parameters, to be used for instantiating a
        tokenizer of type [`~tokenization_utils_base.PreTrainedTokenizerBase`] using
        `from_dict`.
        ...
        Returns:
            `Tuple[Dict, Dict]`: The dictionary(ies) that will be used to instantiate the tokenizer
            object.
        """
        cache_dir = kwargs.pop("cache_dir", None)
        force_download = kwargs.pop("force_download", False)
        ...
        try:
            ...
            with open(resolved_tokenizer_config_file, "r", encoding="utf-8") as reader:
                text = reader.read()
            tokenizer_config_dict = json.loads(text)
            ...
            msg = (
                ...
                f"- '{pretrained_model_name_or_path}' is a correct model identifier listed on 'https://huggingface.co/models'\n"
                f"  (make sure '{pretrained_model_name_or_path}' is not a path to a local directory with something else, in that case)\n\n"
                f"- or '{pretrained_model_name_or_path}' is the correct path to a directory containing a {TOKENIZER_CONFIG_FILE} file\n\n"
            )
            raise EnvironmentError(msg)

        except json.JSONDecodeError:
            msg = (
                f"Couldn't reach server at '{tokenizer_config_file}' to download tokenizer configuration file or "
                "tokenizer configuration file is not a valid JSON file. "
                f"Please check network or file content here: {resolved_tokenizer_config_file}."
            )
            raise EnvironmentError(msg)

        if resolved_tokenizer_config_file == tokenizer_config_file:
            logger.info(f"loading tokenizer configuration file {tokenizer_config_file}")
        else:
            logger.info(
                f"loading tokenizer configuration file {tokenizer_config_file} from cache at {resolved_tokenizer_config_file}"
            )

        return tokenizer_config_dict, kwargs
