...
INIT_TOKENIZER_DOCSTRING = r"""
    ...
    Args:
        ...
        extra_special_tokens (`dict[str, Union[str, AddedToken]]` or `list[Union[str, AddedToken]]`, *optional*):
            Dict maps token names to values (e.g., `{"image_token": "<image>"}`) creating named attributes.
            List format (legacy) adds tokens without named attributes.
...
class PreTrainedTokenizerBase(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        # Storage for all special tokens (named + model-specific)
        ...
        for key in list(kwargs.keys()):
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                ...
                if isinstance(value, dict):
                    explicit_model_specific_tokens.update(value)
                else:
                    # Legacy list format: store for backward compatibility
                    if not isinstance(value, (list, tuple)) or not all(
                        isinstance(t, (str, AddedToken)) for t in value
                    ):
                        raise TypeError(
                            "extra_special_tokens must be a dict mapping names to tokens, or a list/tuple of str/AddedToken (legacy)"
                        )
                    self.init_kwargs["extra_special_tokens"] = list(value)
            elif (
                key.endswith("_token")
    ...
    def add_special_tokens(
        ...
        for key, value in special_tokens_dict.items():
            if key == "extra_special_tokens":
                if isinstance(value, dict):
                    model_specific_tokens.update(value)
                    continue
                # Legacy list format
                if not isinstance(value, (list, tuple)) or not all(isinstance(t, (str, AddedToken)) for t in value):
                    raise ValueError(f"extra_special_tokens must be a dict or list/tuple of str/AddedToken")
                new_tokens = [
        ...
        if model_specific_tokens:
            self._set_model_specific_special_tokens(special_tokens=model_specific_tokens)
            tokens_to_add.extend(model_specific_tokens.values())
    ...
    def special_tokens_map(self) -> dict[str, str]:
        """
        `dict[str, str]`: Dictionary mapping named special token attributes to their string values.
        
        Includes standard named tokens (bos_token, eos_token, etc.) and model-specific tokens.
        """
        return {
    ...
    @property
    def extra_special_tokens(self) -> list[str]:
        """`list[str]`: Legacy property for backward compatibility. Returns legacy list if present, else empty list."""
        if "extra_special_tokens" in self.init_kwargs and isinstance(self.init_kwargs["extra_special_tokens"], (list, tuple)):
            return [str(tok) for tok in self.init_kwargs["extra_special_tokens"]]
        return []

    @property
    def all_special_tokens(self) -> list[str]:
        """`list[str]`: All unique special tokens (named + model-specific) as strings."""
        seen = set()
        all_toks = []

        for attr, value in self._special_tokens_map.items():
            if value is not None:
                token_str = str(value)
    ...
    def _set_model_specific_special_tokens(self, special_tokens: dict[str, Union[str, AddedToken]]):
        """Adds model-specific special tokens (e.g., image_token, audio_token) as named attributes."""
    ...
    def _from_pretrained(
        ...
        **kwargs,
        ...
        # Extract model-specific tokens (e.g., image_token, audio_token) from config
        ...
        # Handle extra_special_tokens: dict -> model_specific_special_tokens, list -> legacy format
        if "extra_special_tokens" in init_kwargs:
            extra_tokens = init_kwargs["extra_special_tokens"]
            if isinstance(extra_tokens, dict):
                model_specific_tokens.update(extra_tokens)
        ...
        if "added_tokens_decoder" in init_kwargs:
                ...
                with open(special_tokens_map_file, encoding="utf-8") as special_tokens_map_handle:
                    ...
                    for key, value in special_tokens_map.items():
                        if key in kwargs and kwargs[key]:
                            ...
                            continue
                        # Convert dict-format tokens to AddedToken
                        if isinstance(value, dict):
                            value["special"] = True
                            value = AddedToken(**value)
                        elif key == "extra_special_tokens":
                            if isinstance(value, dict):
                                if "model_specific_special_tokens" not in init_kwargs:
                                    init_kwargs["model_specific_special_tokens"] = {}
                                init_kwargs["model_specific_special_tokens"].update(value)
            ...
            if added_tokens_file is not None:
                ...
                if "model_specific_special_tokens" in init_kwargs:
                    special_tokens += [str(token) for token in init_kwargs["model_specific_special_tokens"].values()]
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        # Save all special tokens (named + model-specific) from special_tokens_map
