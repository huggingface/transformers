    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        Args:
            ...
            escape_special_tokens (`bool`, defaults to `True`):
                Whether to prevent special tokens contained in the user-supplied `conversation`, `tools` or
                `documents` from being interpreted as control tokens. When `tokenize=True`, the escaped content is
                encoded with `split_special_tokens=True` so that it becomes ordinary text. When `tokenize=False`, a
                zero-width space is inserted inside each injected special token so that it is visually identical but
                can no longer be re-tokenized as a control token by downstream calls. Set to `False` to restore the
                previous (unsafe) behaviour where user content could inject control tokens.

                <Tip warning={true}>

                When user content actually contains special tokens, tokenization is performed segment by segment,
                which is incompatible with `padding` and `truncation`. Pass `tokenize=False` and encode the result
                yourself if you need those options.
        ...
        escaped_conversations, escaped_tools, escaped_documents = conversations, tools, documents

        if escape_special_tokens:
            # `dict.fromkeys` de-duplicates while keeping a deterministic order, unlike `set`
            special_tokens_to_escape = list(
                dict.fromkeys(
                    getattr(self, "all_special_tokens", []) + list(getattr(self, "additional_special_tokens", []))
                )
            )
            nonce = uuid.uuid4().hex

            escaped_conversations, conv_map = _escape_special_tokens(
                conversations, special_tokens_to_escape, nonce=nonce
            )
            placeholder_to_token.update(conv_map)

            if tools is not None:
                escaped_tools, tools_map = _escape_special_tokens(tools, special_tokens_to_escape, nonce=nonce)
                placeholder_to_token.update(tools_map)

            if documents is not None:
                escaped_documents, docs_map = _escape_special_tokens(documents, special_tokens_to_escape, nonce=nonce)
                placeholder_to_token.update(docs_map)

        rendered_chat, generation_indices = render_jinja_template(
            conversations=escaped_conversations,
            tools=escaped_tools,
            documents=escaped_documents,
        ...
        if tokenize:
            if placeholder_to_token:
                # User content contains special tokens: they must be encoded separately with
                # `split_special_tokens=True` so that they cannot be turned into control tokens. This is
                # incompatible with padding/truncation, which operate on a single already-encoded sequence.
                if padding not in (False, None, PaddingStrategy.DO_NOT_PAD):
                    raise ValueError(
                        "`padding` is not supported when the conversation contains special tokens, because the "
                        "escaped content has to be encoded segment by segment. Call `apply_chat_template` with "
                        "`tokenize=False` and encode the resulting string yourself, or pass "
                        "`escape_special_tokens=False` if you trust the input."
                    )
                if truncation not in (False, None, TruncationStrategy.DO_NOT_TRUNCATE):
                    raise ValueError(
                        "`truncation` is not supported when the conversation contains special tokens, because the "
                        "escaped content has to be encoded segment by segment. Call `apply_chat_template` with "
                        "`tokenize=False` and encode the resulting string yourself, or pass "
                        "`escape_special_tokens=False` if you trust the input."
                    )
                out = self._encode_escaped_chat(
                    rendered_chat if is_batched else [rendered_chat],
                    placeholder_to_token=placeholder_to_token,
                    generation_indices=generation_indices if return_assistant_tokens_mask else None,
                    tokenizer_kwargs=tokenizer_kwargs,
                    keep_batch_dim=is_batched or bool(return_tensors),
                )
                if return_tensors:
            ...
            # Compute assistant_masks on initial un-replaced output using char_to_token if requested
            if return_dict and return_assistant_tokens_mask:
                ...
                if not is_batched and not return_tensors:
                    out["assistant_masks"] = assistant_masks[0]
                else:
            ...
            if placeholder_to_token:
                # A zero-width space is inserted inside each injected token so that the output stays visually
                # identical but can no longer be re-tokenized into control tokens by a downstream call.
                rendered_chat = neutralize_special_tokens(rendered_chat, placeholder_to_token)
            return rendered_chat

    def _encode_escaped_chat(
        self,
        rendered_chat: list[str],
        placeholder_to_token: dict[str, str],
        generation_indices: list[list[tuple[int, int]]] | None,
        tokenizer_kwargs: dict[str, Any],
        keep_batch_dim: bool,
    ) -> BatchEncoding:
        """
        Encodes rendered chats that contain escaped special tokens, segment by segment.

        Segments coming from user input are encoded with `split_special_tokens=True`, so that a special token
        written by the user is broken into ordinary pieces instead of becoming a control token. Segments coming
        from the template itself are encoded normally, so the template's own control tokens are preserved.

        Args:
            rendered_chat (`list[str]`): Rendered chats, always batched.
            placeholder_to_token (`dict[str, str]`): Mapping from placeholder strings to original special tokens.
            generation_indices (`list[list[tuple[int, int]]]`, *optional*):
                Character spans of the assistant messages inside each rendered chat. When provided, an
                `assistant_masks` entry is added to the output.
            tokenizer_kwargs (`dict[str, Any]`): Extra kwargs forwarded to the tokenizer.
            keep_batch_dim (`bool`): Whether the returned values should keep the batch dimension.

        Returns:
            [`~tokenization_utils_base.BatchEncoding`]: The encoded chats, without offset mappings (they cannot be
            expressed against a single input string once user content has been re-encoded).
        """
        tokenizer_kwargs = {key: value for key, value in tokenizer_kwargs.items() if key != "return_offsets_mapping"}
        all_input_ids, all_assistant_masks = [], []

        for i, text in enumerate(rendered_chat):
            # Cutting the text on the assistant boundaries too guarantees that each boundary falls exactly
            # between two tokens, which is what lets us rebuild the assistant mask without offset mappings.
            assistant_cuts = set()
            if generation_indices is not None:
                for assistant_start_char, assistant_end_char in generation_indices[i]:
                    assistant_cuts.update((assistant_start_char, assistant_end_char))

            input_ids = []
            token_index_at_char = {}
            for segment_text, is_user_content, char_start, char_end in split_on_placeholders(
                text, placeholder_to_token
            ):
                if is_user_content:
                    # The substituted token is atomic, it cannot be cut in the middle
                    pieces = [(segment_text, char_start)]
                else:
                    cuts = sorted({char_start, char_end}.union(c for c in assistant_cuts if char_start < c < char_end))
                    pieces = [(text[start:end], start) for start, end in zip(cuts, cuts[1:])]
                for piece_text, piece_start in pieces:
                    token_index_at_char[piece_start] = len(input_ids)
                    input_ids.extend(
                        self.encode(
                            piece_text,
                            add_special_tokens=False,
                            split_special_tokens=is_user_content,
                            **tokenizer_kwargs,
                        )
                    )
            token_index_at_char[len(text)] = len(input_ids)
            all_input_ids.append(input_ids)

            if generation_indices is not None:
                boundaries = sorted(token_index_at_char)
                mask = [0] * len(input_ids)
                for assistant_start_char, assistant_end_char in generation_indices[i]:
                    start_token = self._token_index_at_char(token_index_at_char, boundaries, assistant_start_char)
                    end_token = self._token_index_at_char(token_index_at_char, boundaries, assistant_end_char)
                    for token_id in range(start_token, end_token):
                        mask[token_id] = 1
                all_assistant_masks.append(mask)
        ...
        if generation_indices is not None:
            data["assistant_masks"] = all_assistant_masks
        if not keep_batch_dim:
            data = {key: value[0] for key, value in data.items()}
        return BatchEncoding(data)

    @staticmethod
    def _token_index_at_char(token_index_at_char: dict[int, int], boundaries: list[int], char_index: int) -> int:
        """
        Maps a character offset in a rendered chat to a token index.

        Args:
            token_index_at_char (`dict[int, int]`): Mapping from segment boundary offsets to token indices.
            boundaries (`list[int]`): Sorted keys of `token_index_at_char`.
            char_index (`int`): Character offset to convert.

        Returns:
            `int`: The token index of the boundary at, or just before, `char_index`.
        """
        if char_index in token_index_at_char:
            return token_index_at_char[char_index]
        # Should not happen, as every assistant boundary is used as a cut point, but stay safe on odd templates
        return token_index_at_char[boundaries[max(bisect.bisect_right(boundaries, char_index) - 1, 0)]]
