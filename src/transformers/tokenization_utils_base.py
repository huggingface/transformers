...
class PreTrainedTokenizerBase(PushToHubMixin):
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if getattr(self, "response_format", None) is not None:
            tokenizer_config["response_format"] = self.response_format
    ...
    def parse_response(
        ...
        This method is intended for use with chat models, and will read the tokenizer's `response_format` attribute
        ...
        if schema is None:
            if getattr(self, "response_format", None) is not None:
                ...
                raise AttributeError(
                    "This tokenizer does not have a `response_format` (or legacy `response_schema`) "
