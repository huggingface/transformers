...
class PreTrainedTokenizerBase(PushToHubMixin):
    ...
    def _decode_input(
        self,
        x: str | list[str] | list[int] | list[list[int]] | np.ndarray | torch.Tensor,
    ) -> tuple[str | list[str], bool]:
        """Normalize a text-or-tokens input to (str, False) for a single sequence or
        (list[str], True) for a batch. Accepts strings, lists of strings, token-id
        lists (1D or 2D), and numpy / torch arrays (ndim 1 or 2)."""
        if isinstance(x, str):
            return x, False
        if isinstance(x, list):
            ...
            if isinstance(x[0], (list, tuple)):
                return self.batch_decode(x), True
            return self.decode(x), False
        ndim = getattr(x, "ndim", None)
        if ndim == 1:
            return self.decode(x), False
        if ndim == 2:
            return self.batch_decode(x), True
        raise TypeError(f"Unsupported input: {type(x).__name__} with ndim={ndim}")

    def parse_response(
        self,
        response: str | list[str] | list[int] | list[list[int]] | np.ndarray | torch.Tensor,
        schema: list | dict | None = None,
        *,
        prefix: str | list[str] | list[int] | list[list[int]] | np.ndarray | torch.Tensor | None = None,
    ):
        """
        ...
        Args:
            response (`str`, `list[str]`, token ids or tensor):
                The output generated by the model. Either decoded text (a single string or list of strings) or
                token ids (a list / 1D array for a single sequence, list of lists / 2D array for a batch).
            ...
            prefix (`str`, `list[str]`, token ids or tensor, *optional*):
                ...
                generated token is classified into the correct field. Its shape must match `response`: both must
                be single sequences or both must be batches of the same length. Only supported with new-style
        ...
        text, batched = self._decode_input(response)
        if prefix is None:
            prefix_text: str | list[str | None] | None = [None] * len(text) if batched else None
        else:
            prefix_text, prefix_batched = self._decode_input(prefix)
            if prefix_batched != batched:
                raise ValueError("`prefix` and `response` must both be single or both be batched.")
            if batched and len(prefix_text) != len(text):
                raise ValueError(
                    f"Batched `prefix` length {len(prefix_text)} does not match `response` length {len(text)}."
                )

        if use_new_template:
            parse_one = lambda t, p: _template_parse_response(t, schema, prefix=p)  # noqa: E731
        else:
            parse_one = lambda t, p: recursive_parse(t, schema)  # noqa: E731

        if batched:
            return [parse_one(t, p) for t, p in zip(text, prefix_text)]
        return parse_one(text, prefix_text)
    ...
    def get_response_parser(
        ...
        *,
        ...
        if prefix is not None:
            prefix_str, prefix_batched = self._decode_input(prefix)
            if prefix_batched:
                raise ValueError(
