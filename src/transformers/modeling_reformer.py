...
def load_tf_weights_in_reformer(model, config, tf_checkpoint_path):
    return NotImplementedError('check code below to implement')
...
class ReformerIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ReformerOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ReformerPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ReformerPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class ReformerLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = ReformerPredictionHeadTransform(config)
...
class ReformerOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ReformerPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = ReformerLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler = ReformerPooler(config)
    ...
    Examples::
        ...
        tokenizer = ReformerTokenizer.from_pretrained('reformer-base-uncased')
        model = ReformerModel.from_pretrained('reformer-base-uncased')
...
@add_start_docstrings(
    """Reformer Model with two heads on top as done during the pre-training: a `masked language modeling` head and
...
class ReformerForPreTraining(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.cls = ReformerPreTrainingHeads(config)
    ...
    Examples::
        ...
        from transformers import ReformerTokenizer, ReformerForPreTraining
        import torch

        tokenizer = ReformerTokenizer.from_pretrained('reformer-base-uncased')
        model = ReformerForPreTraining.from_pretrained('reformer-base-uncased')
...
@add_start_docstrings(
    """Reformer Model with a `next sentence prediction (classification)` head on top. """, REFORMER_START_DOCSTRING,
)
class ReformerForNextSentencePrediction(ReformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.reformer = ReformerModel(config)
        self.cls = ReformerOnlyNSPHead(config)
    ...
    Examples::
        ...
        from transformers import ReformerTokenizer, ReformerForNextSentencePrediction
        import torch

        tokenizer = ReformerTokenizer.from_pretrained('reformer-base-uncased')
        model = ReformerForNextSentencePrediction.from_pretrained('reformer-base-uncased')
