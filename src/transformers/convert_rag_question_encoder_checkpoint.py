...
def load_states_from_checkpoint(model_file: str) -> CheckpointState:
    ...
    for k, v in state_dict["model"].items():
        if k.startswith("question_encoder"):
            print(k)
...
class DPRQuestionEncoderState(DPRState):
    def load_dpr_model(self):
        ...
        prev_state_dict = model.question_encoder.state_dict()
        print("HF STATE KEYS")
        for k, v in prev_state_dict.items():
            print(k)
        print("Loading DPR biencoder from {}".format(self.src_file))
        saved_state = load_states_from_checkpoint(self.src_file)
        encoder = model.question_encoder
        prefix = "question_encoder."
        state_dict = {}
...
if __name__ == "__main__":
    """ Logic mostly copied over from ``convert_dpr_original_checkpoint_to_pytorch.py``. WIP.
    """
