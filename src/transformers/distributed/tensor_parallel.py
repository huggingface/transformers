...
def _matching_weight_plan_key(generic_key: str, weight_plan: dict[str, str]) -> str | None:
    """Return the weight_plan entry that covers a wildcarded parameter name, or None."""
    if generic_key in weight_plan:
        return generic_key
    # To avoid stripping .gate_up_proj for MoE
    if generic_key.endswith(".weight"):
        param_name = generic_key.removesuffix(".weight")
    elif generic_key.endswith(".bias"):
        param_name = generic_key.removesuffix(".bias")
    else:
        param_name = generic_key
    if param_name != generic_key and param_name in weight_plan:
        return param_name
    if "." in param_name and (parent := param_name.rsplit(".", 1)[0]) in weight_plan:
        return parent
    return None
...
def verify_tp_plan(expected_keys: list[str], tp_plan: dict[str, str] | None) -> None:
    """
    Verify the TP plan of the model, log a warning if the layers that were not sharded and the rules
    that were not applied.

    Only weight-sharding rules (colwise, rowwise, …) that override shard_param are checked. Comm-only entries
    (activation, ep_router, moe_experts_allreduce, …) are excluded.
    """
    if tp_plan is None:
        return
    ...
    for key in generic_keys:
        if (matched_key := _matching_weight_plan_key(key, weight_plan)) is not None:
...
class ColwiseParallel(TensorParallelLayer):
    ...
    def transform_output_post_forward(self, module, output, mesh):
        if not isinstance(output, DTensor):
            return output
        # Inference: skip DTensor redistribute. A colwise output stays sharded on the last dim and
        # feeds straight into the following rowwise_allreduce; colwise_allgather (Replicate output)
        # rebuilds the full width with a plain all-gather.
        if not module.training and self.use_local_output:
            if self.output_layouts == Replicate():
                return _all_gather_last_dim(output.to_local(), _mesh_tp_group(mesh))
...
class RowwiseParallel(TensorParallelLayer):
    ...
    def transform_output_post_forward(self, module, output, mesh):
        if not isinstance(output, DTensor):
            return output
        # Inference: unwrap the Partial output and run a raw all-reduce (sum) instead of DTensor's
        # redistribute machinery. Only the allreduce boundary (Replicate output) has a fast path;
        # reduce-scatter (Shard(1), sequence parallel) keeps the DTensor path.
        if not module.training and self.output_layouts == Replicate():
            local = output.to_local()
            dist.all_reduce(local, op=dist.ReduceOp.SUM, group=_mesh_tp_group(mesh))
            return local
        if output.placements != (self.output_layouts,):
            output = output.redistribute(placements=[self.output_layouts], async_op=True)
        return output.to_local() if self.use_local_output else output
...
class MoEExpertsParallel(TensorParallelLayer):
    ...
    def transform_output_post_forward(self, module, output, mesh):
        ...
        # Inference: unwrap and run a raw all-reduce (sum) over the partial per-rank expert outputs
        # instead of DTensor redistribute, matching the rowwise inference fast path.
        if not module.training and self.output_layouts == Replicate():
...
# (and staying continuous-batching friendly). Parameters remain DTensors in both modes — only the
# boundary collective differs. Selected purely by `model.train()` / `model.eval()`, no extra plan.


def _mesh_tp_group(mesh):
    """Process group for the tensor-parallel mesh dim (1-D tp mesh, or the 'tp' dim of an n-D mesh)."""
    return mesh.get_group() if mesh.ndim == 1 else mesh.get_group("tp")


def _all_gather_last_dim(local: torch.Tensor, group) -> torch.Tensor:
    """All-gather equal-sized last-dim shards and concatenate (colwise_allgather inference path)."""
