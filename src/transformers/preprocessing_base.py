...
class PreprocessingMixin(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        # This key was saved while we still used `XXXFeatureExtractor` for image/audio processing. Now we use
        # `XXXImageProcessor`/`XXXAudioProcessor`, this attribute and its value are misleading.
        kwargs.pop("feature_extractor_type", None)
        # Pop "processor_class", should not be saved with image/audio processing config anymore
    ...
    def _init_kwargs_from_valid_kwargs(self, kwargs: dict):
        ...
        Membership, not truthiness, decides whether a value was supplied: an explicit `None`
        is a deliberate override of a non-None class default and must be preserved.
        """
    ...
    def _set_attributes(self, **kwargs):
        """
        Resolve and standardize instance attributes for all valid kwargs.
        Called from backend subclasses' `__init__` for backward compatibility with remote code
        """
    ...
    def _standardize_kwargs(self, **kwargs) -> dict:
        ...
        `SizeDict`). Overridden by modality base classes; default is a no-op.
        """
        return kwargs
    ...
    def from_dict(cls, config_dict: dict[str, Any], **kwargs):
        ...
        # Use valid_kwargs pattern when available (image processors)
    ...
    def get_config_dict(
        ...
        """
        Hook: resolve the config dict for `from_pretrained`. Modality mixins redirect this to their
        public, historically overridable method (e.g. `get_image_processor_dict`) so that subclasses
        overriding it keep control over resolution.
        """
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # excluded keys from the serialized config
        for key in ("_valid_kwargs_names", "_valid_processor_keys", "mel_filters", "window"):
            output.pop(key, None)
    ...
    def _keep_in_dict(self, key, value) -> bool:
        """
        Keep non-None values; keep an explicit None only when the declared default is itself
        ...
        if class_default == "NOT_FOUND" and "FeatureExtractor" in type(self).__name__:
            # Legacy feature extractors declare defaults in `__init__`, not as class attributes, so
            # the probe above misses: an explicit None would be dropped, then refilled on load.
            param = inspect.signature(type(self).__init__).parameters.get(key)
            if param is not None and param.default is not inspect.Parameter.empty:
                class_default = param.default
