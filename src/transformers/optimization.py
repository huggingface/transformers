...
class RecAdam(Optimizer):
    """
    Implementation of RecAdam optimizer, a variant of Adam optimizer. By recalling the knowledge from the pretraining
    tasks and gradually focusing the learning on the downstream tasks, it can facilitate fine-tuning deep pretrained
    language models with less forgetting. Paper: https://www.aclweb.org/anthology/2020.emnlp-main.634 Original
    implementation: https://github.com/Sanyuan-Chen/RecAdam
    ...
    Parameters:
        ...
        correct_bias (:obj:`bool`, `optional`, defaults to `True`):
            Whether or not to correct bias in Adam (for instance, in Bert TF repository they use :obj:`False`).
        anneal_fun (:obj:`str`, `optional`, defaults to `sigmoid`):
            A hyperparam for the annealing function, decide the function of the curve.
        anneal_k (:obj:`float`, `optional`, defaults to 0.1):
            A hyperparam for the annealing function, decide the slop of the curve.
        anneal_t0 (:obj:`int`, `optional`, defaults to 1000):
            A hyperparam for the annealing function, decide the middle point of the curve.
        anneal_w (:obj:`float`, `optional`, defaults to 1.0):
            A hyperparam for the annealing function, decide the scale of the curve.
        pretrain_cof (:obj:`float`, `optional`, defaults to 5000.0):
            The coefficient of the quadratic penalty.
        pretrain_params (:obj:`Iterable[nn.parameter.Parameter]`, `optional`, defaults to None):
            The corresponding group of params in the pretrained model.

    Usage::

        # replace AdamW with RecAdam
        optimizer = RecAdam(
            model.parameters(),
            lr=1e-3,
            betas=(0.9, 0.999),
            ...
            anneal_w=1.0,
            pretrain_cof=5000.0,
            pretrain_params=pretrained_model.parameters(),
        )
    """
    ...
    def __init__(
        ...
        pretrain_params: Iterable[nn.parameter.Parameter] = None,
    ):
        require_version("torch>=1.5.0")  # add_ with alpha
        if lr < 0.0:
            raise ValueError(f"Invalid learning rate: {lr} - should be >= 0.0")
    ...
    def anneal_function(function, step, k, t0, weight):
        if function == "sigmoid":
            ...
            ValueError(f"Invalid annealing function: {function} - should be in ['sigmoid', 'linear', 'constant']")
        ...
        for group in self.param_groups:
            for p, pp in zip(group["params"], group["pretrain_params"]):
                ...
                if group["anneal_w"] > 0.0:
                    # The lambda(t) is calculated as the annealing function
                    anneal_lambda = self.anneal_function(
