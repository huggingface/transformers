...
class Adafactor(Optimizer):
    ...
    for group in self.param_groups:
        for p in group["params"]:
            ...
            # Minimize casting by keeping gradients in their original precision
            if grad.is_sparse:
            ...
            if len(state) == 0:
                ...
                if use_first_moment:
                    state["exp_avg"] = state["exp_avg"].to(grad.dtype)
                if factored:
                    state["exp_avg_sq_row"] = state["exp_avg_sq_row"].to(grad.dtype)
                    state["exp_avg_sq_col"] = state["exp_avg_sq_col"].to(grad.dtype)
                else:
                    state["exp_avg_sq"] = state["exp_avg_sq"].to(grad.dtype)
            ...
            update = (grad**2).add_(group["eps"][0])  
            ...
            if factored:
                ...
                # Approximate exponential moving average of the square of the gradient
