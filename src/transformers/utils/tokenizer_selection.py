...
class CorpusAnalyzer:
    ...
    def _detect_language_hint(char_counter: Counter) -> Optional[str]:
        ...
        latin_chars = sum(count for char, count in char_counter.items() 
                         if ord(char) < 256)
        asian_chars = sum(count for char, count in char_counter.items() 
                         if ord(char) > 4352)  # CJK range approximation
    ...
    def suggest_and_train_tokenizer(
        ...
        **trainer_kwargs
        ...
        trained_tokenizer = base_tok.train_new_from_iterator(
            text_iterator=iter(text_batches),
            vocab_size=vocab_size,
            **trainer_config
        )
