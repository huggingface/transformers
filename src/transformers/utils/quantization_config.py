        ...
        if self.modules_to_not_convert:
            ...
            if is_auto_awq_available():
                try:
                    ...
                    awq_version_supports_non_conversion = version.parse(current_version) >= version.parse(MIN_AWQ_VERSION)
                    ...
                    try:
                        ...
                        if module_version:
                            awq_version_supports_non_conversion = version.parse(module_version) >= version.parse(MIN_AWQ_VERSION)
                        else:
                            # If no version found but module imports successfully, assume recent enough
                            awq_version_supports_non_conversion = True
                            logger.warning("AutoAWQ version could not be determined, assuming it supports module quantization skipping")
                    except ImportError:
