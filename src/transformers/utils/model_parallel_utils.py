...
def assert_device_map(device_map, num_blocks):
    ...
    assert len(missing_blocks) == 0, 'There are attention blocks for this model that are not specified in the device_map. Add these attention_blocks to a device on the device_map:' + str(missing_blocks)
...
def get_device_map(n_layers, n_devices):
    '''Returns a dictionary of layers distributed evenly across all devices.'''
    device_map = {i: list(range(int(i * n_layers / n_devices), int((i+1) * n_layers / n_devices))) for i in range(n_devices)}
    return device_map
