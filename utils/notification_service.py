...
class Message:
    ...
    def model_failures(self) -> List[Dict]:
        ...
        file_path = os.path.join(os.getcwd(), f"{ci_results_dir}/model_failures_report.txt")
        ...
        file_path = os.path.join(os.getcwd(), f"{ci_results_dir}/module_failures_report.txt")
        with open(file_path, "w", encoding="UTF-8") as fp:
            fp.write(module_failures_report)

        if self.prev_ci_artifacts is not None:
            # if the last run produces artifact named `f"{ci_results_dir}"`
            if (
                f"{ci_results_dir}" in self.prev_ci_artifacts
                and "model_failures_report.txt" in self.prev_ci_artifacts[f"{ci_results_dir}"]
            ):
                # Compute the difference of the previous/current (model failure) table
                prev_model_failures = self.prev_ci_artifacts[f"{ci_results_dir}"]["model_failures_report.txt"]
                ...
                if len(entries_changed) > 0:
                    ...
                    file_path = os.path.join(os.getcwd(), f"{ci_results_dir}/changed_model_failures_report.txt")
                    with open(file_path, "w", encoding="UTF-8") as fp:
    ...
    def get_new_model_failure_blocks(self, with_header=True):
        ...
        if (
            f"{ci_results_dir}" in self.prev_ci_artifacts
            and "model_results.json" in self.prev_ci_artifacts[f"{ci_results_dir}"]
        ):
            prev_model_results = json.loads(self.prev_ci_artifacts[f"{ci_results_dir}"]["model_results.json"])
...
if __name__ == "__main__":
    ...
    ci_results_dir = f"ci_results_{job_name}"
    ...
    if not os.path.isdir(os.path.join(os.getcwd(), f"{ci_results_dir}")):
        os.makedirs(os.path.join(os.getcwd(), f"{ci_results_dir}"))

    # Only the model testing job is concerned: this condition is to avoid other jobs to upload the empty list as
    # results.
    if job_name == "run_models_gpu":
        with open(f"{ci_results_dir}/model_results.json", "w", encoding="UTF-8") as fp:
    ...
    for job, job_result in additional_results.items():
        with open(f"{ci_results_dir}/{test_to_result_name[job]}_results.json", "w", encoding="UTF-8") as fp:
    ...
    if os.environ.get("CI_WORKFLOW_REF") == target_workflow:
        ...
        artifact_names = [f"{ci_results_dir}"]
