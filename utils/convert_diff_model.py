...
def replace_super_calls_in_method(method_body, parent_method_body, method_name):
    ...
    indented_parent_method_body = "\n".join(
        [indent + line if line.strip() else line for line in parent_method_body.split("\n")]
    )
...
def get_source_code_from_register(converter, line):
...
def extract_code_elements(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        for node in cst.walk(tree):
            if isinstance(node, (cst.FunctionDef, cst.Import, cst.ImportFrom)):
                print(f"Found:\n```\n{cst.unparse(node)}\n```")
                code_elements.append(cst.unparse(node))
...
def create_single_model_file(converter):
    if hasattr(converter, "diff_file"):
        ...
        for module in modules | {re.sub(r".*src/(.*)\.py", r"\1", converter.diff_file).replace("/", ".")}:
            ...
            extract_code_elements(modeling_.__file__)
            globals().update({k: getattr(modeling_, k) for k in modeling_.__dict__.keys()})

        with open(converter.diff_file, "r") as file, open(
            f"{model_identifier[0]}modeling_{model_identifier[1]}", "w+"
        ) as modeling:
            modeling.write(APACHE_LICENCE)
            function_set = {}
            for line in file:
                if "Converter.register" in line:  # TODO use map() to map lines to this
                    source_code = get_source_code_from_register(converter, line)
                    modeling.write(source_code)
                    modeling.write("\n")

                elif match := re.match(r"class (\w+)\((\w+)\):", line):
                    function_name_patter = r"(?=    def ([\S]*)\()"
                    # inspect.getsource(getattr(CLASS, "forward"))
                    ...
                    for match in matches:
                        ...
                        if "def" in full_function:
                            ...
                            if (
                                "super()." in full_function or "return super()." in full_function
                            ) and parent_identifier_camel != child_identifier_camel:
                                replaced_function = replace_super_calls_in_method(
                                    full_function, parent_function_set.get(function_name, ""), function_name
                                )
                    ...
                    modeling.write(
                        "\n".join(child_function_set.values())
                    )  # TODO we wrote the code, next lines shall be ignored
                    modeling.write("\n")
