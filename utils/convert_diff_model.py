from transformers.models.cohere.diff_cohere import CohereConverter
import regex as re
import inspect
...
with open(CohereConverter.diff_file, 'r') as file, open("result.py", "w+") as modeling:
    ...
    for line in file: 
            if "Converter.register" in line: # TODO use map() to map lines to this 
                ...
                source_code = inspect.getsource(CohereConverter.registered_classes[class_to_use])
                source_code = source_code.replace(old_class, class_to_use)
