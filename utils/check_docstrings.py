...
def find_matching_model_files(check_all: bool = False):
    ...
    image_processing_glob_pattern = os.path.join(PATH_TO_TRANSFORMERS, "models/**/video_processing_*.py")
    potential_files += glob.glob(image_processing_glob_pattern)
    image_processing_glob_pattern = os.path.join(PATH_TO_TRANSFORMERS, "models/**/image_processing_pil_*.py")
    potential_files += glob.glob(image_processing_glob_pattern)
