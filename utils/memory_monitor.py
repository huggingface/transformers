...
def print_both(message, flush=True):
    """Print to both stdout (log file) and terminal."""
    print(message, flush=flush)
    try:
        with open('/dev/tty', 'w') as tty:
            tty.write(message + '\n')
    except:
        pass  # If /dev/tty not available, just use stdout
...
def monitor_memory(threshold=90, interval=5, dry_run=False, verbose=False):
    ...
    print(
        f"Starting memory monitor (kill when available < {min_available_percent:.1f}%, interval: {interval}s, dry_run: {dry_run})",
        flush=True)
    ...
    try:
        while True:
            ...
            if verbose or mem_info['percent_available'] < (100 - threshold) * 1.2:  # Show when approaching threshold
                ...
                print(
                    f"  System Memory: {mem_info['percent_used']:.1f}% used (apps), {mem_info['percent_available']:.1f}% available, {mem_info['percent_non_reclaimable_cache']:.1f}% non-reclaimable cache")
                print(
                    f"  Total: {format_size(mem_info['total'])}, Used: {format_size(mem_info['used'])}, Available: {format_size(mem_info['available'])}, Non-reclaimable Cache: {format_size(mem_info['non_reclaimable_cache'])}")
                print(
                    f"  Python processes: {len(python_procs)} running, using {total_python_mem_mb:.2f} MB ({total_python_mem_percent:.1f}% of total)")
            ...
            if mem_info['percent_available'] < min_available_percent:
                ...
                print_both(f"\n{'=' * 80}")
                print_both(
                    f"⚠️  MEMORY THRESHOLD EXCEEDED: {mem_info['percent_available']:.1f}% available < {min_available_percent:.1f}% minimum")
                print_both(f"⚠️  (System memory used: {mem_info['percent_used']:.1f}%)")
                if use_sigkill:
                    print_both(f"⚠️  CRITICAL: Using SIGKILL immediately (available < 5%)")
                print_both(f"{'=' * 80}")
                print_both(
                    f"Available memory: {format_size(mem_info['available'])} ({mem_info['percent_available']:.1f}%)")
                print_both(
                    f"Non-reclaimable Buffers/Cache: {format_size(mem_info['non_reclaimable_cache'])} ({mem_info['percent_non_reclaimable_cache']:.1f}%)")
