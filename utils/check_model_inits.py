...
"""
Utility that regenerates `src/transformers/models/__init__.py` from the model directories.
...
```bash
python utils/check_model_inits.py
```

Regenerate it if needed (used in `make fix-repo`):

```bash
python utils/check_model_inits.py --fix_and_overwrite
```
"""
...
CHECKER_CONFIG = {
    ...
    "cache_globs": ["src/transformers/models/*/__init__.py", "src/transformers/models/__init__.py"],
    "check_args": [],
...
def get_model_names() -> list[str]:
    """Return the model directories that should be exposed by `models/__init__.py`, sorted.

    A directory qualifies as soon as it holds an `__init__.py`. This skips leftovers from branch
    switches, which are empty apart from a stale `__pycache__`.
    """
    model_names = (path.name for path in MODELS_PATH.iterdir() if (path / "__init__.py").is_file())
...
def main(overwrite: bool):
    ...
    raise Exception(
        f"`{relative_path}` is not consistent with the model directories on disk.\n"
        "Run `make fix-repo` or `python utils/check_model_inits.py --fix_and_overwrite` to fix it.\n\n"
