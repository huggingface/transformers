...
## 2026-03-09 - Apples-to-apples verification after class-score padding fix

### Goal
- Re-run the exact early-stopping configuration that previously gave the best HF Trainer result, now on the branch with the additional encoder class-score padding fix (`pad class scores as well`), to verify that fine-tuning behavior remains unchanged.

### 1) Exact setup used

Job:
- [69aed302424ad135b8440d29](https://huggingface.co/jobs/nielsr/69aed302424ad135b8440d29)

Hardware:
- `a10g-small`

Command differences vs previous best run:
- None intended on training hyperparameters.
- Explicitly set:
  - `--num-train-epochs 300`
  - `--metric-for-best-model eval_map_50`
  - `--early-stopping-patience 3`
  - `--seed 42`
- Kept:
  - no augmentation
  - original checkpoint initialization (`xbsu/LW-DETR/...LWDETR_tiny_60e_coco.pth`)
  - batch size `4`
  - default TF32 setting

Note:
- I had to update `train_lw_detr_tray_cart_hf_trainer_early_stop.py` locally to match the current `convert_lw_detr_to_hf.py` API on this branch (the old helper imports used in the script are no longer exported). This was a script-compatibility fix, not a model/training change.

### 2) Result

The run completed successfully on GPU and trained normally.

Final summary:
- `best_metric (eval_map_50)`: `0.58998`
- Final validation: `map=0.30197`, `map_50=0.58998`
- Final test: `map=0.40886`, `map_50=0.78305`
- Stopped at: `epoch=16`, `global_step=400`
...
### 3) Comparison to previous best Trainer run in this file

Previous best no-augmentation early-stopping run:
- [69a88713de6eb6ba6d52cd3b](https://huggingface.co/jobs/nielsr/69a88713de6eb6ba6d52cd3b)
- `best_metric (eval_map_50)`: `0.5638`
- Final validation: `map=0.2832`, `map_50=0.5638`
- Final test: `map=0.3961`, `map_50=0.7540`
- Stopped at: `epoch=16`, `global_step=400`

New vs previous-best deltas:
- Validation `map`: `+0.0188`
- Validation `map_50`: `+0.0262`
- Test `map`: `+0.0128`
- Test `map_50`: `+0.0290`
- Same stopping point: `epoch=16`, `global_step=400`

### 4) Comparison to prior full HF vs original side-by-side run

Prior full HF run:
- [69a87a7773aa33c77fb22c1f](https://huggingface.co/jobs/nielsr/69a87a7773aa33c77fb22c1f)
- Validation HF: `map=0.2614`, `map_50=0.5382`
- Test HF: `map=0.4071`, `map_50=0.7583`

New apples-to-apples run vs that prior full HF run:
- Validation `map`: `+0.0406`
- Validation `map_50`: `+0.0518`
- Test `map`: `+0.0018`
- Test `map_50`: `+0.0247`

### 5) Conclusion

- The additional class-score padding fix does **not** break fine-tuning.
- Under the same effective early-stopping schedule as the prior best Trainer run, the new run is at least as good and in this run clearly better on both validation and test metrics.
- This is strong evidence that the change is safe for fine-tuning and likely beneficial rather than regressive.
