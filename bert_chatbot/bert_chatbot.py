...
from bert_chatbot_config import BERT_CONFIG
from tf_idf_ko import ContextFinder
...
def prepare_optimizer(model, args, num_train_steps = None):
    # Prepare optimizer
    if args.fp16:
...
def bert_train():
    args, device, n_gpu = chatbot_prepare()
    ...
    optimizer = prepare_optimizer(model, args, num_train_steps)
    ...
    if args.do_train:
        ...
        for cur_epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            print('cur_epoch', cur_epoch, '/', int(args.num_train_epochs))
...
def bert_predict_once(args, model, device, n_gpu, qas_id, context_text, question_text):
    tokenizer = BertTokenizer.from_pretrained(args.bert_model)
...
def bert_chat_test():
    ...
    while(True):
        ...
        if question_text == 'quit': break
            
        answer = bert_predict_once(args, model, device, n_gpu, qas_id, context_text, question_text)
...
def bert_chat():
    ...
    while(True):
        ...
        print(context_text)
        answer = bert_predict_once(args, model, device, n_gpu, qas_id, context_text, question_text)
        print('', 'question:', question_text, c.check_pos(question_text))
        print('', 'answer:', answer, c.check_pos(answer))
        print('')
