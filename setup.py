...
class DepsTableUpdateCommand(Command):
    ...
    def run(self):
        if SUPPORTED_PYTHON_VERSIONS[0] > PYTHON_MINOR_VERSION:
            print(
                f"Skipping DepsTableUpdateCommand: Python {sys.version_info.major}.{PYTHON_MINOR_VERSION} is not "
                f"supported. Min supported version: 3.{SUPPORTED_PYTHON_VERSIONS[0]}"
            )
            return
        entries = []
        for dep in _deps:
            ...
            if not match:
                raise ValueError(f"Can't parse dependency {dep}")
            entries.append(f'    "{match[0]}": "{dep}",')
        content = []
        with open("src/transformers/dependency_versions_table.py", "r") as f:
            for line in f:
                if line.strip().startswith('#  THIS FILE HAS BEEN AUTOGENERATED'):
                    continue
                content.append(line)
        with open("src/transformers/dependency_versions_table.py", "w") as f:
            f.write("# THIS FILE HAS BEEN AUTOGENERATED. To update:\n")
            f.write("# 1. modify the `_deps` dict in setup.py\n")
            f.write("# 2. run `make fix-repo``\n")
            f.write("deps = {\n")
            f.write("\n".join(entries))
            f.write("\n}\n")
...
setup(
    name="transformers",
    version="5.16.0.dev0",  # expected format is one of x.y.z.dev0, or x.y.z.rc1 or x.y.z (no extra)
    author="The Hugging Face team (past and future) with the help of all our contributors (https://github.com/huggingface/transformers/graphs/contributors)",
    author_email="transformers@huggingface.co",
    description="State-of-the-art Machine Learning for JAX, PyTorch and TensorFlow",
    long_description=open("README.md", "r", encoding="utf-8").read(),
    long_description_content_type="text/markdown",
    keywords="NLP vision speech deep learning transformer pytorch tensorflow jax BERT GPT-2",
