...
dep_versions_short = { k:v for k,v in (re.split(r"(?=\b[\!=<>].+)", d, 2) for d in _dep_versions) }
...
with open("src/transformers/dependency_versions_table.py", "w") as f:
    entries = "\n".join([f'    "{k}": "{v}",' for k,v in dep_versions.items()])
...
if os.name == "nt":  # windows
    ...
    extras["flax"] = dep_versions_list("jax", "jaxlib", "flax")

extras["tokenizers"] = dep_versions_list("tokenizers")
