...
RUN pip install uv && uv venv && uv pip install --no-cache-dir -U pip setuptools GitPython ruff==0.4.4
uUN uv pip install --no-cache-dir --upgrade 'torch' --index-url https://download.pytorch.org/whl/cpu
