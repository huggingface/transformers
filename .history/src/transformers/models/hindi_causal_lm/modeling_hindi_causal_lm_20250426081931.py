    ...
    def forward(
        ...
        if self.config.positional_encoding_type in ["absolute", "learned"]:
            ...
            if inputs_embeds.shape[0] != position_embeds.shape[0] and position_embeds.shape[0] != 1:
                raise RuntimeError(
                    f"Batch dimension mismatch between input embeds ({inputs_embeds.shape[0]}) and position embeds ({position_embeds.shape[0]})"
                )
            if inputs_embeds.shape[1] != position_embeds.shape[1]:
                raise RuntimeError(
                    f"Sequence length mismatch between input embeds ({inputs_embeds.shape[1]}) and position embeds ({position_embeds.shape[1]})"
                )
