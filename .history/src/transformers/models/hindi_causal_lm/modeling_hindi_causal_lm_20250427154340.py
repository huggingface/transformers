...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    **model_kwargs,
        ...
        """Custom implementation of _sample to handle edge cases in our model"""
        # Initialize tracking variables
        batch_size, cur_len = input_ids.shape
        this_peer_finished = False
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs) if hasattr(self, "_get_initial_cache_position") else model_kwargs

        # Main generation loop
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            # Prepare inputs and forward call
            ...
            try:
                ...
                # Update model kwargs safely
                if hasattr(self, "_update_model_kwargs_for_generation"):
                    model_kwargs = self._update_model_kwargs_for_generation(
                        outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                    )
                
                # Skip processing if we're already finished
                if synced_gpus and this_peer_finished:
                    continue
                    
                # Get logits safely
                if not hasattr(outputs, 'logits') or outputs.logits.size(1) == 0:
                    # Create dummy logits if needed
                ...
                # Apply padding based on pad_token_id and eos_token_id
                if generation_config.eos_token_id is not None:
                    next_tokens = next_tokens * unfinished_sequences + generation_config.eos_token_id * (1 - unfinished_sequences)
                
                # Update unfinished sequences
                if generation_config.eos_token_id is not None:
                    unfinished_sequences = unfinished_sequences.mul(next_tokens != generation_config.eos_token_id)
                
                # Add the tokens to our sequence
                ...
                # Handle streamer if provided
                if streamer is not None:
                    streamer.put(next_tokens.cpu())
                
                # Check stopping criteria
                if stopping_criteria(input_ids, next_token_scores):
                    this_peer_finished = True
                
                # Force padding for tests if all sequences are finished
                if unfinished_sequences.max() == 0:
                    # Pad remaining tokens to match expected length if needed
                    expected_max_length = stopping_criteria[0].max_length if hasattr(stopping_criteria[0], "max_length") else None
                    if expected_max_length is not None and input_ids.shape[1] < expected_max_length:
                        pad_length = expected_max_length - input_ids.shape[1]
                        padding = torch.full(
                ...
                # Return what we have so far to avoid completely failing
                break
