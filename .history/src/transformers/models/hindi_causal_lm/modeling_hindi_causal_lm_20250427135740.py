...
class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Reshape 2D attention_mask to 4D for compatibility with the attention mechanisms
        if attention_mask is not None:
            # Convert 2D [batch_size, seq_length] mask to 4D [batch_size, 1, q_len, kv_len] mask
            if attention_mask.dim() == 2:
                extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
                # Create proper mask values (1.0 for tokens to attend to, 0.0 for masked tokens)
                extended_attention_mask = extended_attention_mask.to(dtype=inputs_embeds.dtype)
                # Convert 0s to -10000 and 1s to 0s for proper softmax attention
                extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
                _attention_mask = extended_attention_mask
            else:
