...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.weight"]; _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config=None): requires_backends(self, ["torch"])
...
if is_torch_available():
    ...
    # RMSNorm definition remains the same
    ...
    class HindiCausalLMRotaryEmbedding(nn.Module):
        # Removed device argument from init
        def __init__(self, dim, max_position_embeddings=2048, base=10000):
            super().__init__()
            self.dim = dim
            self.max_position_embeddings = max_position_embeddings
            self.base = base
            # Create inv_freq directly without device, will be moved later
            inv_freq = 1.0 / (self.base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).float() / self.dim))
            self.register_buffer("inv_freq", inv_freq, persistent=False)
            # Initialize cache with default dtype, device will be set later
            self._set_cos_sin_cache(
                seq_len=max_position_embeddings, device=None, dtype=torch.get_default_dtype()
            )

        def _set_cos_sin_cache(self, seq_len, device, dtype):
            self.max_seq_len_cached = seq_len
            # If device is None, create on default device (usually CPU)
            t_device = device if device is not None else torch.device("cpu")
            t = torch.arange(self.max_seq_len_cached, device=t_device, dtype=torch.int64).type_as(self.inv_freq)

            freqs = torch.outer(t, self.inv_freq.to(t_device)) # Ensure inv_freq is on same device
            emb = torch.cat((freqs, freqs), dim=-1)
            self.register_buffer("cos_cached", emb.cos().to(dtype), persistent=False)
            self.register_buffer("sin_cached", emb.sin().to(dtype), persistent=False)

        def forward(self, x, seq_len=None):
            # Ensure cache uses same device and dtype as input tensor 'x'
            if seq_len > self.max_seq_len_cached or self.cos_cached.device != x.device or self.cos_cached.dtype != x.dtype:
                 self._set_cos_sin_cache(seq_len=seq_len, device=x.device, dtype=x.dtype)

            return (
                 self.cos_cached[:seq_len], # Already on correct device/dtype due to _set_cos_sin_cache call
                 self.sin_cached[:seq_len],
             )

    # rotate_half and apply_rotary_pos_emb remain the same
    def rotate_half(x):
    ...
    # _prepare_4d_* mask helpers remain the same
    def _prepare_4d_causal_attention_mask(...): # Code omitted for brevity
        # ... (same implementation as before) ...
        pass
    def _prepare_4d_causal_attention_mask_for_sdpa(...): # Code omitted for brevity
        # ... (same implementation as before) ...
        pass

    # HindiCausalLMAttention remains the same
    class HindiCausalLMAttention(nn.Module):
        def __init__(self, config: HindiCausalLMConfig, layer_idx: Optional[int] = None):
            ...
            self.q_proj=nn.Linear(self.hidden_size, self.num_heads*self.head_dim, bias=False); self.k_proj=nn.Linear(self.hidden_size, self.num_heads*self.head_dim, bias=False)
            self.v_proj=nn.Linear(self.hidden_size, self.num_heads*self.head_dim, bias=False); self.o_proj=nn.Linear(self.num_heads*self.head_dim, self.hidden_size, bias=False)
    ...
    class HindiCausalLMLayer(nn.Module):
        ...
        def forward(
            ...
            **kwargs,
            ...
            # Note: cache_position is accepted but not used in this specific implementation
            # If using static cache or specific optimizations later, it would be used here.
    ...
    class HindiCausalLMPreTrainedModel(PreTrainedModel):
        ...
        def _init_weights(self, module):
            ...
            if isinstance(module, nn.Linear): module.weight.data.normal_(mean=0.0, std=std); # noqa
    ...
    class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
        ...
        def forward(
            ...
            if (input_ids is None) == (inputs_embeds is None): raise ValueError("...")
            ...
            else: past_key_values = None; use_legacy_cache = False

            if inputs_embeds is None: inputs_embeds = self.embed_tokens(input_ids)
            batch_size, seq_length = inputs_embeds.shape[:2]

            # <<< cache_position handling (simplified for now) >>>
            if cache_position is None:
                if position_ids is None:
                     device = inputs_embeds.device
                     position_ids = torch.arange(past_key_values_length, seq_length + past_key_values_length, dtype=torch.long, device=device)
                     position_ids = position_ids.unsqueeze(0)
                # If cache_position is None but position_ids is provided, use position_ids
            else:
                 # If cache_position is provided, derive position_ids from it
                 # This assumes cache_position represents the absolute positions
            ...
            if self.config._attn_implementation == "flash_attention_2":
                attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
                 ...
                 if is_sdpa:
                     ...
                     attention_mask = _prepare_4d_causal_attention_mask(attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length, is_causal=is_causal)
            ...
            for decoder_layer in self.layers:
                ...
                if self.gradient_checkpointing and self.training:
                    # Gradient checkpointing function definition
                    def create_custom_forward(module):
                         def custom_forward(*inputs):
                             return module(*inputs, past_key_value=None, output_attentions=output_attentions, use_cache=False, cache_position=cache_position) # Pass cache_position if needed by GC
            ...
            if output_hidden_states: all_hidden_states += (hidden_states,)
            next_cache = next_decoder_cache if use_cache else None # Use the updated Cache object directly
            if use_cache and use_legacy_cache: next_cache = next_cache.to_legacy_cache()
