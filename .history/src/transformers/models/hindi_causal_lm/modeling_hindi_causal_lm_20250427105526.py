...
if is_torch_available():
        ...
        def forward(
            ...
            **kwargs, # Catch potential extra kwargs like head_mask
            ...
            if self.rotary_emb is not None:
                ...
                cos = cos[position_ids].unsqueeze(1) # Shape: [bs, 1, seq_len, dim]
                sin = sin[position_ids].unsqueeze(1) # Shape: [bs, 1, seq_len, dim]
    ...
    class HindiCausalLMEncoder(nn.Module):
        """
        Transformer encoder consisting of `config.num_hidden_layers` layers. Adapted from LlamaModel.
        """
        ...
        def forward(
            ...
            # hidden_states = hidden_states.to(self.layers[0].self_attn.q_proj.weight.device) # Handled by main model
    ...
    class HindiCausalLMPreTrainedModel(PreTrainedModel):
        ...
        def _set_gradient_checkpointing(self, module, value=False):
             # Enable/disable gradient checkpointing in the encoder
    ...
    class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
        ...
        def __init__(self, config: HindiCausalLMConfig):
            ...
            self.dropout = nn.Dropout(config.hidden_dropout_prob) # Embedding dropout moved here

            # Initialize encoder (Transformer layers)
            self.layers = HindiCausalLMEncoder(config) # Renamed from encoder to layers for consistency
        ...
        def _prune_heads(self, heads_to_prune):
            ...
            for layer, heads in heads_to_prune.items():
                self.layers.layers[layer].self_attn.prune_heads(heads) # Access self_attn via layer list
        ...
        def forward(
            ...
            if position_ids is None:
                ...
                position_ids = position_ids.unsqueeze(0) # No need to expand, broadcasting handles it later
            ...
            # Standard causal mask is created inside the attention module if attention_mask is None
            # If a mask is provided, it needs proper expansion for attention calculation
            if attention_mask is not None:
                # Expand attention_mask: [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
                # This is the format expected by the scaled dot product attention functions
                 if attention_mask.dim() == 2:
                     # Assuming the provided mask is for padding, create the combined causal + padding mask
                     # Causal mask part
                     ...
                     causal_mask = causal_mask.triu(diagonal=1 + past_key_values_length) # Upper triangle is masked
                     causal_mask = causal_mask[None, None, :, :].expand(batch_size, 1, -1, -1) # Expand bs and head dim

                     # Padding mask part
                     padding_mask = (attention_mask[:, None, None, :] == 0) # 0 means pad
                     expanded_padding_mask = padding_mask.expand(batch_size, 1, seq_length, seq_length + past_key_values_length)
                     # Combine masks: where either is masked, the result should be masked (-inf)
                     # Start with causal mask, then apply padding mask where needed
                     attention_mask = causal_mask.masked_fill(expanded_padding_mask, torch.finfo(hidden_states.dtype).min)

                 elif attention_mask.dim() == 4:
                     # Assume it's already in the correct format [bs, 1, q_len, k_len]
                     pass
                 else:
                     raise ValueError(f"Invalid attention mask shape: {attention_mask.shape}. Expected 2D or 4D.")
    ...
    class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
        ...
        def __init__(self, config: HindiCausalLMConfig):
            super().__init__(config)
            self.model = HindiCausalLMModel(config) # Changed attribute name from hindi_causal_lm to model
            self.vocab_size = config.vocab_size # Store vocab_size
        ...
        def get_input_embeddings(self):
            return self.model.embed_tokens # Access embeddings via self.model

        def set_input_embeddings(self, value):
            self.model.embed_tokens = value # Set embeddings via self.model
        ...
        def tie_weights(self):
            # Tie or untie weights if specified
            if self.config.tie_word_embeddings:
                output_embeddings = self.get_output_embeddings()
                input_embeddings = self.get_input_embeddings()
                if output_embeddings is not None and input_embeddings is not None:
                    output_embeddings.weight = input_embeddings.weight
            # Call super only after potential tying
            super().tie_weights()
            ...
            if self.config.tie_word_embeddings:
                ...
                if output_embeddings is not None and input_embeddings is not None:
                    if output_embeddings.weight is input_embeddings.weight:
                         # Clone weight only if currently tied
                         output_embeddings.weight = nn.Parameter(output_embeddings.weight.clone())
            # No need to call super()._untie_weights() as it does nothing by default

        # Override save_pretrained for safetensors weight tying handling (Copied from base, seems okay)
        def save_pretrained(
            self,
            save_directory,
        ...
        def prepare_inputs_for_generation(
            ...
            # Handle KV cache - if past is used, only need the last token's input_ids
            if past_key_values is not None:
                past_length = past_key_values[0][0].shape[2] # Get length from KV cache shape

                # Keep only the unprocessed tokens:
                # 1. If the length of input_ids is greater than that of the past_key_values, treat the input_ids as if the cache is reused.
                if input_ids.shape[1] > past_length:
                    input_ids = input_ids[:, past_length:]
                # 2. Otherwise, take the last token that has not been processed
                else:
            ...
            if attention_mask is not None and position_ids is None:
                ...
                if past_key_values:
                    position_ids = position_ids[:, -input_ids.shape[1]:] # Adjust position IDs for cached tokens
        ...
        # Override get_generation_config if specific defaults are needed (Copied from base, seems okay)
        ...
        def forward(
            ...
            # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
            outputs = self.model( # Call self.model now
                input_ids=input_ids,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,
