...
class HindiTransformerBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.normalization_layer == "rmsnorm":
            self.attention_layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps, elementwise_affine=False)
...
class HindiCausalLMModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([
            HindiTransformerBlock(config) for _ in range(config.num_hidden_layers)
        ])
        
        # Final layer norm
        if config.normalization_layer == "rmsnorm":
            self.final_layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps, elementwise_affine=False)
        else:
