...
if is_torch_available():
    ...
    from ...modeling_utils import PreTrainedModel # Import only PreTrainedModel now
    ...
    # RotaryEmbedding definition remains the same
    ...
    # rotate_half definition remains the same
    def rotate_half(x):
        x1 = x[..., : x.shape[-1] // 2]; x2 = x[..., x.shape[-1] // 2 :]
        return torch.cat((-x2, x1), dim=-1)

    # apply_rotary_pos_emb definition remains the same
    ...
    def _prepare_4d_causal_attention_mask(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if not torch.is_floating_point(attention_mask) and attention_mask.dtype != torch.bool: raise ValueError(...)
        ...
        if is_causal:
             seq_ids = torch.arange(tgt_len, device=inputs_embeds.device)
             # diagonal=1 means that position i cannot attend to position i+1 or more
             causal_mask = torch.ones((tgt_len, tgt_len), dtype=torch.bool, device=inputs_embeds.device).tril(diagonal=0)
             mask.masked_fill_(causal_mask, 0.0)

        if past_key_values_length > 0 and is_causal: # is_causal check might be redundant here if mask is already causal
        ...
        if attention_mask is not None: # 2D padding mask [bsz, src_len]
            if attention_mask.dim() != 2: raise ValueError(...)
    ...
    class HindiCausalLMAttention(nn.Module):
        # ... (rest of the class definition remains the same as previous correct version) ...
    ...
    class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
        ...
        def forward(
            ...
            # token_type_ids removed from signature
            ...
            if use_cache:
                ...
                past_key_values_length = past_key_values.get_seq_length() # Corrected: Get length from Cache object
            else:
            ...
            if self.config._attn_implementation == "flash_attention_2":
                attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
                 ...
                 # SDPA expects boolean mask, others expect additive float mask
            ...
            # If attention_mask is None and not Flash Attn 2, layers should handle causal masking
            ...
            for decoder_layer in self.layers:
                ...
                if use_cache:
                    next_decoder_cache = past_key_values # Cache object updated inplace
                if output_attentions:
    ...
    class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
        ...
        # get/set embeddings, tie/untie weights, save_pretrained, prepare_inputs, get_generation_config remain the same
        ...
        def forward(
            ...
            outputs = self.model(
                ...
                # Pass token_type_ids=None here, or just omit it if base model doesn't accept it
                use_cache=use_cache,
                output_attentions=output_attentions,
