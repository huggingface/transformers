    ...
    class HindiCausalLMAttention(nn.Module):
        ...
        def __init__(self, config):
            ...
            if self.positional_encoding_type == "rope":
                ...
                if self.rotary_dim % 2 != 0:
                    self.rotary_dim = max(2, self.rotary_dim - 1)
                # Cache frequency
                self.register_buffer(
                    "inv_freq", 
                    1.0 / (10000 ** (torch.arange(0, self.rotary_dim, 2).float() / self.rotary_dim))
                )
        ...
        def forward(
            ...
            if self.positional_encoding_type == "rope":
                ...
                positions = torch.arange(seq_len, device=hidden_states.device, dtype=torch.float)
                
                # Compute angles
                half_dim = self.rotary_dim // 2
                freqs = torch.outer(positions, self.inv_freq)
                emb = torch.cat((freqs, freqs), dim=-1).reshape(seq_len, self.rotary_dim)
                cos = emb.cos().to(query_layer.dtype)  # [seq_len, rotary_dim]
                sin = emb.sin().to(query_layer.dtype)  # [seq_len, rotary_dim]
                
                # Apply RoPE to query and key, but only for the first rotary_dim dimensions
                # First, extract the parts that will get rotation
                q_rot = query_layer[..., :self.rotary_dim]  # [batch, heads, seq_len, rotary_dim]
                k_rot = key_layer[..., :self.rotary_dim]    # [batch, heads, seq_len, rotary_dim]
                
                # Reshape cos and sin for broadcasting: [1, 1, seq_len, rotary_dim]
                cos = cos.unsqueeze(0).unsqueeze(0)
                sin = sin.unsqueeze(0).unsqueeze(0)
                
                # Split the rotation dimension
                q_rot1, q_rot2 = torch.chunk(q_rot, 2, dim=-1)  # Each is [batch, heads, seq_len, rotary_dim/2]
                k_rot1, k_rot2 = torch.chunk(k_rot, 2, dim=-1)  # Each is [batch, heads, seq_len, rotary_dim/2]
                
                # Get the rotated versions
                cos1, sin1 = torch.chunk(cos, 2, dim=-1)  # Each is [1, 1, seq_len, rotary_dim/2]
                
                # Rotary embeddings
                q_rot_result = torch.cat(
                    [q_rot1 * cos1 - q_rot2 * sin1, q_rot2 * cos1 + q_rot1 * sin1], 
                    dim=-1
                )
                k_rot_result = torch.cat(
                    [k_rot1 * cos1 - k_rot2 * sin1, k_rot2 * cos1 + k_rot1 * sin1], 
                    dim=-1
                )
                
                # If we're rotating all dimensions, replace the tensors completely
                if self.rotary_dim == self.attention_head_size:
                    query_layer = q_rot_result
                    key_layer = k_rot_result
                else:
                    # Otherwise, replace just the rotated parts
                    query_layer = torch.cat([q_rot_result, query_layer[..., self.rotary_dim:]], dim=-1)
                    key_layer = torch.cat([k_rot_result, key_layer[..., self.rotary_dim:]], dim=-1)
