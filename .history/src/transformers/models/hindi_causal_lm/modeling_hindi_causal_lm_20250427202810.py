...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    if position_ids is None or position_ids.numel() == 0:
        logger.warning_once(
            "position_ids provided to apply_rotary_pos_emb is None or empty. Skipping RoPE application."
        )
        return q, k
    max_cached_len = cos.shape[0]
    max_pos_id = position_ids.max()
    if max_pos_id >= max_cached_len:
        # This should ideally not happen if prepare_inputs correctly calculates position_ids and cache length is sufficient
        logger.error(
            f"RoPE cache length {max_cached_len} is insufficient for maximum position ID {max_pos_id}. "
            f"Original position_ids shape: {position_ids.shape}. Clamping position IDs."
        )
        # Clamp position_ids instead of raising error to try to proceed
        position_ids = torch.clamp(position_ids, 0, max_cached_len - 1)

    clamped_position_ids = torch.clamp(position_ids, 0, max_cached_len - 1)  # Clamp again just in case
    try:
        cos_gathered = cos[clamped_position_ids]
        sin_gathered = sin[clamped_position_ids]
    except IndexError as e:
        logger.error(
            f"Error indexing RoPE cache (size {max_cached_len}) with clamped_position_ids (shape {clamped_position_ids.shape}, min {clamped_position_ids.min()}, max {clamped_position_ids.max()}). Original position_ids shape: {position_ids.shape}"
        )
    ...
    if attention_mask is not None:
        ...
        if attention_mask.dim() == 2:
            ...
            if expanded_padding_mask.shape[-1] != kv_seq_len:
                logger.warning(
                    f"Provided 4D mask shape[-1] {expanded_padding_mask.shape[-1]} != kv_seq_len {kv_seq_len}"
                )
                if expanded_padding_mask.shape[-1] < kv_seq_len:
                    pad_len = kv_seq_len - expanded_padding_mask.shape[-1]
                    expanded_padding_mask = nn.functional.pad(
                        expanded_padding_mask, (0, pad_len), value=torch.finfo(dtype).min
                    )
                else:
        ...
        if combined_mask.shape != (bsz, 1, tgt_len, kv_seq_len):
            raise ValueError(
                f"Final combined mask shape {combined_mask.shape} incorrect. Expected {(bsz, 1, tgt_len, kv_seq_len)}"
            )
...
class HindiCausalLMAttention(nn.Module):
    def __init__(self, config: HindiCausalLMConfig, layer_idx: Optional[int] = None):
        ...
        if layer_idx is None:
            logger.warning_once("Layer index not specified for HindiCausalLMAttention initialization")
...
@add_start_docstrings("The base class for Hindi causal language models.", """Initialize the weights.""")
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                if len(layer_outputs) > 1 and layer_outputs[1] is not None:
                    all_self_attns += (layer_outputs[1],)  # Collect potentially sliced attentions
...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # This handles the case where input_ids might be full length initially,
        # or just the last token(s) during generation.
        if inputs_embeds is not None and input_ids is None:
            current_length = inputs_embeds.shape[1]
            # If past_key_values exist, inputs_embeds should represent the new tokens.
            # GenerationMixin typically passes only the next token's embedding.
            # No slicing needed here if generate calls this correctly.
        elif input_ids is not None:
        ...
        if position_ids is None:
            ...
            # **Fix for IndexError:** Ensure current_length > 0
            if current_length <= 0:
                # This might happen in continue_from_inputs_embeds if slicing was wrong
                # or if an empty prompt is generated from.
                logger.warning(
                    f"prepare_inputs_for_generation received current_length={current_length}. Generating empty position_ids."
                )
                # Create empty tensor with correct shape [batch_size, 0]
                batch_size = input_ids.shape[0] if input_ids is not None else inputs_embeds.shape[0]
                position_ids = torch.empty((batch_size, 0), dtype=torch.long, device=device)
            else:
                position_ids = torch.arange(
                    past_length, past_length + current_length, dtype=torch.long, device=device
                ).unsqueeze(0)  # Shape [1, current_length]
        else:
            # Ensure provided position_ids match the length of new tokens
        ...
        if inputs_embeds is not None and input_ids is None:
            ...
            # Ensure input_ids passed to model only contain new tokens if cache is used
            if past_key_values is not None:
                model_inputs = {"input_ids": input_ids[:, -current_length:]}
            else:
                model_inputs = {"input_ids": input_ids}
        else:
            raise ValueError("Error in prepare_inputs: No valid input_ids or inputs_embeds.")

        # Prepare attention mask for the *forward* pass
        # GenerationMixin handles the growing mask passed here.
        if attention_mask is not None:
            # Ensure the mask covers the full expected key/value sequence length
            ...
            if attention_mask.shape[1] < expected_kv_len:
                ...
                pad = torch.ones(
                    (attention_mask.shape[0], padding_length), dtype=attention_mask.dtype, device=attention_mask.device
                )
    ...
    def generate(self, *args, **kwargs):
        # Keep previous corrected version
