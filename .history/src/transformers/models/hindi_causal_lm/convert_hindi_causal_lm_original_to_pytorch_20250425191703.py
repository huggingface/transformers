...
def convert_hindi_causal_lm_checkpoint(checkpoint_path, pytorch_dump_folder_path, config_path=None):
    ...
    for i in range(config.num_hidden_layers):
        ...
        if f"layers.{i}.attention.query.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention.query.weight"] = state_dict[
                f"layers.{i}.attention.query.weight"
            ]
        if f"layers.{i}.attention.key.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention.key.weight"] = state_dict[
                f"layers.{i}.attention.key.weight"
            ]
        if f"layers.{i}.attention.value.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention.value.weight"] = state_dict[
                f"layers.{i}.attention.value.weight"
            ]
        if f"layers.{i}.attention.output.0.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention.output.0.weight"] = state_dict[
                f"layers.{i}.attention.output.0.weight"
            ]

        # Layer norms
        if f"layers.{i}.attention_layernorm.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention_layernorm.weight"] = state_dict[
                f"layers.{i}.attention_layernorm.weight"
            ]
        if f"layers.{i}.attention_layernorm.bias" in state_dict:
            new_state_dict[f"transformer.layers.{i}.attention_layernorm.bias"] = state_dict[
                f"layers.{i}.attention_layernorm.bias"
            ]
        if f"layers.{i}.ffn_layernorm.weight" in state_dict:
            new_state_dict[f"transformer.layers.{i}.ffn_layernorm.weight"] = state_dict[
                f"layers.{i}.ffn_layernorm.weight"
            ]
