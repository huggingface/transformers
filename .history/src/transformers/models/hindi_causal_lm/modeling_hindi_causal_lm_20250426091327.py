...
class CausalSelfAttention(_MODEL_BASE_CLASS):
    ...
    def forward( self, hidden_states: "torch.Tensor", attention_mask: Optional["torch.Tensor"]=None, output_attentions: bool=False ) -> Union[Tuple["torch.Tensor"], Tuple["torch.Tensor", "torch.Tensor"]]:
        ...
        if attention_mask is not None:
            ...
            attention_mask = (1.0 - attention_mask.to(attention_scores.dtype)) * torch.finfo(attention_scores.dtype).min;
            attention_mask = attention_mask.to(attention_scores.device, dtype=attention_scores.dtype); attention_scores = attention_scores + attention_mask;
...
class HindiCausalLMHeadModel(PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.config.positional_encoding_type in ["absolute", "learned"]:
            ...
            position_ids_to_use = torch.clamp(current_position_ids, 0, self.config.max_position_embeddings - 1).to(torch.long)
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None: input_ids = input_ids[:, -1:] # Handle KV cache case if implemented later
