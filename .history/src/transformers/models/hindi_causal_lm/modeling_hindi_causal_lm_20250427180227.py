            ...
            if attention_mask.size() != expected_mask_shape:
                 if not (attention_mask.shape[0] == bsz and attention_mask.shape[1] == 1 and attention_mask.shape[2] == q_len and attention_mask.shape[3] == final_kv_seq_len):
                     ...
                     # ** Indentation Fix Start **
                         ...
                         # Slice the mask if it's longer than needed (can happen with left padding/generation)
...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if not return_dict:
            # Ensure past_key_values is included if use_cache=True
        ...
        **kwargs
        ...
        if past_key_values is not None:
            ...
            if inputs_embeds is not None:
                 # If using embeds for continuation, slice to the new part
                 # Assuming inputs_embeds might contain the full sequence up to now
                 if inputs_embeds.shape[1] > current_length: # Check if slicing is needed
                      inputs_embeds = inputs_embeds[:, -current_length:, :]
                 input_ids = None # Must unset input_ids if using embeds
            elif input_ids is not None:
                 # If using ids, keep only the new tokens
        ...
        if position_ids is None:
            ...
            # Use only the position IDs corresponding to the new tokens
        ...
        # GenerationMixin handles the growing mask, so we just need to ensure
        # the mask passed to forward covers the full key/value length
        if attention_mask is not None:
            expected_mask_len = past_length + current_length
            if attention_mask.shape[1] < expected_mask_len:
                 # Typically, generate provides the mask for the *current* step's inputs + past.
                 # We might need to pad it if it doesn't cover the full expected KV length.
                 ...
                 # Pad on the right side, assuming the provided mask covers the beginning
                 attention_mask = torch.cat([attention_mask, pad], dim=1)

            # Ensure mask is not longer than needed (can happen with left padding)
            if attention_mask.shape[1] > expected_mask_len:
    ...
    def _reorder_cache(past_key_values, beam_idx):
        ...
        for layer_past in past_key_values:
            ...
            # Ruff Fix E701: Moved append to next line
