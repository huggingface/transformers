...
class HindiCausalLMRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len is None:
            # Try to infer from input shape if not provided
            try:
                seq_len = x.shape[-2] # Assuming shape is [..., seq_len, dim]
            except IndexError:
                raise ValueError("Could not infer sequence length from input tensor shape.")

        # Update cache if necessary based on calculated or provided seq_len
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # cos: [bsz, 1, seq_len, dim] or [seq_len, dim]
    # sin: [bsz, 1, seq_len, dim] or [seq_len, dim]
    # q: [bsz, num_heads, seq_len, head_dim]
    # k: [bsz, num_heads, seq_len, head_dim]

    # Ensure cos/sin have the correct dimensions for broadcasting with q/k
    # Expected shape for broadcast: [bsz or 1, 1 or num_heads, seq_len, dim]
    # Typically derived from [seq_len, dim] -> indexed by position_ids -> [bsz, seq_len, dim]
    # Needs to become [bsz, 1, seq_len, dim] for broadcasting across heads
    cos = cos.unsqueeze(unsqueeze_dim) # [bsz, 1, seq_len, dim]
    sin = sin.unsqueeze(unsqueeze_dim) # [bsz, 1, seq_len, dim]
...
def _prepare_4d_causal_attention_mask(
    ...
    if tgt_len > 0:
        ...
        if sliding_window is not None:
            ...
            causal_mask_bool = causal_mask_bool | window_mask_bool # Combine: mask if outside causal or window
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            # k/v cache should have shape [bsz, num_heads, seq_len_past, head_dim]
        ...
        if self.rotary_emb is not None:
            ...
            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len) # Get cache slice
        ...
        if attention_mask is not None:
            ...
            if attention_mask.size() != expected_mask_shape:
                # If q_len is 0, allow mask shape [bsz, 1, 0, final_kv_seq_len]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if token_type_ids is not None:
            logger.info_once("token_type_ids provided but not used.")
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                 ...
                 if attn_idx > 0 and layer_outputs[attn_idx] is not None:
                     all_self_attns += (layer_outputs[attn_idx],)
...
# Causal LM Head Model - ruff fixed
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    # prepare_inputs_for_generation - ruff fixed
    def prepare_inputs_for_generation(
        ...
        if position_ids is None:
            # Calculate position_ids dynamically based on attention mask and past length
        ...
        for layer_past in past_key_values:
            ...
            for past_state in layer_past:
                if past_state is not None:
                     ...
                     reordered_layer_past.append(None)
            while len(reordered_layer_past) < 2:
                reordered_layer_past.append(None)
