        ...
        def forward(
            ...
            if attention_mask is not None and attention_mask.dim() == 2 and self.config._attn_implementation != "flash_attention_2":
                 if self.config._attn_implementation == "sdpa" and not output_attentions:
                     attention_mask = _prepare_4d_causal_attention_mask_for_sdpa(...) # Args omitted
                 else:
                     attention_mask = _prepare_4d_causal_attention_mask(...) # Args omitted
            ...
            for decoder_layer in self.layers:
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(...) # Args omitted
                else:
