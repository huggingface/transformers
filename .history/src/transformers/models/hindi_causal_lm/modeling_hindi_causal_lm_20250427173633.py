...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    if position_ids is None or position_ids.numel() == 0:
        # If position_ids are invalid or empty, return original q, k
        # This might happen in edge cases, prevent the runtime error
        logger.warning_once("Attempting to apply RoPE with invalid position_ids. Skipping rotation.")
        return q, k
    max_cached_len = cos.shape[0]
    # Ensure position_ids are not out of bounds
    ...
    # Check if gathered tensors are empty before unsqueezing
    if cos_gathered.numel() == 0 or sin_gathered.numel() == 0:
        logger.warning_once("RoPE gathered cos/sin tensors are empty. Skipping rotation.")
        return q, k
...
# Function to repeat key/value heads for GQA/MQA (needed for code structure)
...
def _prepare_4d_causal_attention_mask(
    ...
    if tgt_len > 0:
        ...
        # Ensure mask matches float type for addition
    ...
    if attention_mask is not None:
        # Ensure attention_mask is broadcastable to [bsz, 1, tgt_len, kv_seq_len]
        input_pad_mask_len = attention_mask.shape[-1]
        if attention_mask.dim() == 2:
            # Standard case: [bsz, seq_len] -> expand to 4D
            ...
            # Adjust mask length to match kv_seq_len for keys/values
        ...
        # Merge masks: ensure shapes match before maximum
        if causal_4d_mask.shape != expanded_padding_mask.shape:
            raise ValueError(
                f"Shape mismatch before combining masks: causal {causal_4d_mask.shape}, padding {expanded_padding_mask.shape}"
            )
...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Calculate logits for the *entire sequence* if needed for loss, otherwise maybe optimize
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        # Calculate cache_position if not provided (needed for some cache types, not tuple cache directly)
        ...
        # Update attention_mask if needed
        if attention_mask is not None:
