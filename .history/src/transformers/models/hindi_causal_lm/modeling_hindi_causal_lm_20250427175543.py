...
class HindiCausalLMRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        # Initialize cache later on correct device/dtype in forward
        self.cos_cached = None
        self.sin_cached = None
        self.max_seq_len_cached = -1 # Initialize max length cached


    def _set_cos_sin_cache(self, seq_len, device, dtype):
        self.max_seq_len_cached = seq_len
        t = torch.arange(self.max_seq_len_cached, device=device, dtype=self.inv_freq.dtype)
        freqs = torch.outer(t, self.inv_freq) # Use outer product
        emb = torch.cat((freqs, freqs), dim=-1)
        # Re-register buffer is safe, replaces existing ones if needed
    ...
    def forward(self, x, seq_len=None):
        """
        Calculates the rotary embeddings for the given sequence length.

        Args:
            x (torch.Tensor): Input tensor, used to determine device and dtype.
            seq_len (int, optional): The sequence length to compute embeddings for.
                                     If None, attempts to infer from `x`.
        ...
        if seq_len is None:
            try:
                # Infer sequence length from the second to last dimension (common for Q/K/V)
                seq_len = x.shape[-2]
            except IndexError:
        ...
        # Check if cache needs update based on length, device, or dtype
        # Also handles the initial cache creation
        ...
        # Return the cached values sliced to the requested sequence length
        return (
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # Unsqueeze gathered cos/sin: Shape becomes [bsz, 1, seq_len, dim] for broadcasting
    cos_final = cos_gathered.unsqueeze(unsqueeze_dim)
    sin_final = sin_gathered.unsqueeze(unsqueeze_dim)

    # q/k shapes: [bsz, num_heads, seq_len, head_dim]
    # Check sequence length dimension (dim=2 for Q/K, dim=2 for cos/sin after unsqueeze)
...
def _prepare_4d_causal_attention_mask(
    ...
    if attention_mask is not None:
        ...
        if attention_mask.dim() == 2:
            ...
            expanded_padding_mask = attention_mask # Assume pre-computed 4D mask
            # Ensure kv_seq_len dimension matches
            if expanded_padding_mask.shape[-1] != kv_seq_len:
                ...
                if expanded_padding_mask.shape[-1] < kv_seq_len:
                     ...
                     expanded_padding_mask = nn.functional.pad(expanded_padding_mask, (0, pad_len), value=torch.finfo(dtype).min) # Pad with mask value
                else:
        ...
        if causal_4d_mask.shape != expanded_padding_mask.shape:
             # Try broadcasting if shapes are compatible except for batch or head dim (dim 0 or 1)
             try:
...
class HindiCausalLMAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attn_weights.size() != expected_attn_shape: raise ValueError(...)

        if attention_mask is not None:
            expected_mask_shape = (bsz, 1, q_len, final_kv_seq_len)
            if attention_mask.size() != expected_mask_shape:
                 # Allow broadcasting if mask shape is compatible (e.g., kv_len dimension)
                 if not (attention_mask.shape[0] == bsz and attention_mask.shape[1] == 1 and attention_mask.shape[2] == q_len and attention_mask.shape[3] == final_kv_seq_len):
                     if not (q_len == 0 and attention_mask.size() == (bsz, 1, 0, final_kv_seq_len)):
                         # Check if the kv_len dimension is at least as long as needed
                         if attention_mask.shape[3] < final_kv_seq_len:
...
class HindiCausalLMLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = attn_outputs[1] # Full attention matrix (potentially)
        ...
        # Handle attention output shape for generation tests
        if output_attentions:
            # If caching is enabled (generation), only the last token's attention is usually expected by tests.
            # Otherwise (training/full pass), return the full attention matrix.
            # The GenerationMixin handles the collection and stacking, expecting [bsz, heads, 1, kv_len].
            # Let's return the full matrix here and let generate handle it, or skip the problematic tests.
            # Returning the full matrix aligns with standard forward pass.
            outputs += (attn_weights,) # Return full matrix
    ...
    def forward(
        ...
        if self.rotary_emb:
             ...
             if self.rotary_emb.cos_cached is None or self.rotary_emb.cos_cached.device != hidden_states.device:
                 self.rotary_emb.to(hidden_states.device) # Ensure RoPE module is on correct device
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        # Ensure all required args are passed correctly
            ...
            if output_attentions:
                # Append the full attention matrix returned by the layer
...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # Return full attentions from the base model; GenerationMixin handles collection/slicing
        if not return_dict:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        # Determine current_length based on the input (ids or embeds)
        if inputs_embeds is not None and input_ids is None:
            current_length = inputs_embeds.shape[1]
        elif input_ids is not None:
            current_length = input_ids.shape[1]
        else: raise ValueError("Must provide either input_ids or inputs_embeds")
        ...
        if past_key_values is not None:
            ...
            if inputs_embeds is not None:
                ...
                inputs_embeds = inputs_embeds[:, past_length:, :] # Correct slicing if embeds contains full sequence
                current_length = inputs_embeds.shape[1] # Update current length after slicing
                input_ids = None # Unset input_ids
            elif input_ids is not None:
                input_ids = input_ids[:, -current_length:] # Keep only the new tokens
        ...
        # Prepare attention mask for the *forward* pass (might need full length)
        # `generate` utility usually handles the growing mask. If passed explicitly,
        # ensure it covers the full expected sequence length for the keys/values.
        if attention_mask is not None:
            expected_mask_len = past_length + current_length
            if attention_mask.shape[1] < expected_mask_len:
                # If mask is shorter (e.g., only for current tokens), pad it
                num_new_tokens = current_length
                # Assume previous tokens were attended to if using cache
                past_mask_len = past_length
                # Determine correct shape based on past_key_values existence
                if past_key_values is not None:
                     # Assume the passed mask covers the *new* tokens only
                     # Need to prepend a mask for the past tokens
                     past_mask = torch.ones((attention_mask.shape[0], past_mask_len), dtype=attention_mask.dtype, device=attention_mask.device)
                     attention_mask = torch.cat([past_mask, attention_mask], dim=1)
                # Else (no cache), the mask should already cover the input length
    ...
    # generate override - Keep previous corrected version
    ...
    # _reorder_cache - Keep previous corrected version
    @staticmethod
    def _reorder_cache(past_key_values, beam_idx):
        ...
        for layer_past in past_key_values:
            ...
            while len(reordered_layer_past_states) < 2: reordered_layer_past_states.append(None)
