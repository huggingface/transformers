...
class HindiCausalLMRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # `x.shape[-2]` should be the sequence length from the input tensor to this forward pass
        seq_len = x.shape[-2] # Get seq_len from input
        if seq_len > self.max_seq_len_cached:
             self._set_cos_sin_cache(seq_len=seq_len, device=x.device, dtype=x.dtype)
             cos = self.cos_cached.to(dtype=x.dtype)
             sin = self.sin_cached.to(dtype=x.dtype)

        # Use position_ids to gather the correct embeddings
...
class HindiCausalLMAttention(nn.Module):
    ...
    def __init__(self, config: HindiCausalLMConfig, layer_idx: Optional[int] = None):
        ...
        # Instantiated in the main model: self.rotary_emb
    ...
    def forward(
        ...
        # Need to pass position embeddings from the main model forward pass
        position_embeddings: Tuple[torch.Tensor, torch.Tensor],
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None, # Keep position_ids for RoPE application
        ...
        cos, sin = position_embeddings # Passed from model.forward
        ...
        kv_seq_len = key_states.shape[-2] # Get the sequence length from the key states (which includes past)
            ...
            if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
                 ...
                 if attention_mask.size()[-1] >= kv_seq_len:
                     attention_mask = attention_mask[:, :, :, :kv_seq_len]
                      ...
                      raise ValueError(
                         f"Attention mask should be of size {(bsz, 1, q_len, kv_seq_len)} or broadcastable, but is {attention_mask.size()}"
                      )
...
class HindiCausalLMDecoderLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,
        # Add position_embeddings tuple (cos, sin) as input
        position_embeddings: Tuple[torch.Tensor, torch.Tensor],
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,  # Accept arbitrary kwargs for flash usage
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            position_embeddings=position_embeddings, # Pass precomputed RoPE embeddings
...
class HindiCausalLMPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # Initialize RMSNorm weights like Llama/Gemma
        elif isinstance(module, HindiCausalLMRMSNorm):
             # Initialize weights to 1, not 0 like Gemma2's initial config
             module.weight.data.fill_(1.0)

    # Keep _setup_cache if using static cache option
...
class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache:
            ...
            if past_key_values is None:
                 past_key_values = cache_cls(
                     self.config, batch_size, self.config.max_position_embeddings, device=self.device, dtype=self.dtype
                 )
            past_key_values_length = past_key_values.get_seq_length()
        elif past_key_values is not None:
             # If `use_cache=False` but `past_key_values` are provided, it indicates pre-computation for specific scenarios
             # (like beam search with attention cache, though not typically done this way anymore)
             # We still need the length for mask/position calculations if applicable.
        ...
        is_causal = past_key_values is None # Check if it's the prefill phase
        ...
        # FA2 implementation requires 2D mask for kv-cache handling OR no mask if causal
            ...
            # If no padding and `past_key_values` is None, we are in the prefill phase and the input is already
            # causal. Set `attention_mask=None` for FA2.
            elif is_causal:
                 attention_mask = None
        else:
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    hidden_states,
                    position_embeddings, # Pass position embeddings
                    attention_mask,
                    position_ids,
                    None,  # Pass None for past_key_value to GC func
                    output_attentions,
                    use_cache=False,  # Cache must be disabled for GC
                    cache_position=cache_position,
                    use_reentrant=False,  # Recommended for PyTorch >= 1.11
                )
                # We cannot obtain the cache from the checkpointed function, handle accordingly
                if use_cache:
                     logger.warning_once("Gradient checkpointing is enabled with use_cache=True. The cache cannot be recovered.")
        ...
        if use_cache and next_decoder_cache is None and not (self.gradient_checkpointing and self.training):
             # If cache was intended but not returned (shouldn't happen without GC), use original cache
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                # Static Cache requires position = past_length + 1, Dynamic cache requires position = past_length
                # max_cache_length = ( # Removed as unused
                ...
                # max_cache_length = None # Removed as unused
        ...
        if attention_mask is not None and position_ids is None:
            ...
            if past_key_values is not None and not use_static_cache: # Only slice if not static
        ...
        # Retrieve use_cache from kwargs or default to True
        use_cache_arg = kwargs.get("use_cache", True)
        ...
        model_inputs.update(
            {
                ...
                "cache_position": cache_position,
                "past_key_values": past_key_values,
                "use_cache": use_cache_arg, # Use the retrieved value
                "attention_mask": attention_mask,
            }
        )
        return model_inputs
...
class HindiCausalLMForSequenceClassification(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            ...
            output = (pooled_logits,) + outputs[2:] # Index 2 to skip past_key_values
            return ((loss,) + output) if loss is not None else output
