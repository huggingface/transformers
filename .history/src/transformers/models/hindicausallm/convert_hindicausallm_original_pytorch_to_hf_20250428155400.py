    ...
    if not os.path.isfile(config_path):
        ...
        if os.path.isfile(params_path):
             ...
             config_dict = {
                 ...
                 "intermediate_size": params.get("multiple_of", 256) * params.get("dim") * 4 // 3 // params.get("multiple_of", 256), # Approximate SwiGLU size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", type=str, required=True, help="Path to the original PyTorch checkpoint file (.bin, .pth, .safetensors)")
    parser.add_argument("--config_path", type=str, default=None, help="Path to the model config.json or params.json file (optional, defaults to checkpoint directory)")
    parser.add_argument("--pytorch_dump_folder_path", type=str, required=True, help="Path to the output directory for the converted Hugging Face model")
