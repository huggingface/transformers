...
class HindiCausalLMAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs, # Accept arbitrary kwargs for future compatibility
        ...
        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training) # Use attention_dropout from config
...
class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(0) #.view(-1, seq_length) # No need to view(-1, seq_length) if bsz=1 assumed for generation
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            ...
            if attention_mask is not None and attention_mask.shape[1] > input_ids.shape[1]:
                ...
                input_ids = input_ids[:, past_key_values.get_seq_length(self.layer_idx) :] # Removed -1 as DynamicCache seq len should be correct
...
class HindiCausalLMForSequenceClassification(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        pooled_logits = logits[torch.arange(batch_size, device=logits.device), torch.max(sequence_lengths, torch.zeros_like(sequence_lengths))]
