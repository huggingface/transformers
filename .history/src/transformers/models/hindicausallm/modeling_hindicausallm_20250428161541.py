...
from ...cache_utils import Cache, DynamicCache  # Use DynamicCache for better kv-cache handling
...
class HindiCausalLMLayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-5):  # Changed default eps to match config
        ...
        # Removed bias to align with common RMSNorm implementations like Llama, can be added back if needed
...
class HindiCausalLMRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        # Return cos/sin cache sliced to the seq_len requested, ensuring correct dtype
        return (
...
class HindiCausalLMAttention(nn.Module):
    ...
    def __init__(self, config: HindiCausalLMConfig, layer_idx: Optional[int] = None):
        ...
        self.is_causal = True  # Causal attention is standard for this model type
    ...
    def forward(
        ...
        **kwargs,  # Accept arbitrary kwargs for future compatibility
        ...
        if past_key_value is not None:
            # Account for potential initial prefill state length if using DynamicCache
            if hasattr(past_key_value, "get_seq_length"):
                kv_seq_len += past_key_value.get_seq_length(self.layer_idx)
            else:  # Fallback for basic Cache tuple
        ...
        # Check attn_weights shape: [bsz, num_heads, q_len, kv_seq_len]
        ...
        )  # Use attention_dropout from config
        attn_output = torch.matmul(attn_weights, value_states)

        # Check attn_output shape: [bsz, num_heads, q_len, head_dim]
...
class HindiCausalLMDecoderLayer(nn.Module):
    def __init__(self, config: HindiCausalLMConfig, layer_idx: int):
        ...
        self.input_layernorm = HindiCausalLMLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.post_attention_layernorm = HindiCausalLMLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class HindiCausalLMPreTrainedModel(PreTrainedModel):
    ...
    _skip_keys_device_placement = "past_key_values"  # Changed to string
    _supports_flash_attn_2 = False  # Add Flash Attention support if applicable later
    _supports_sdpa = True  # Assuming SDPA support is intended
    ...
    # Removed _setup_cache as DynamicCache handles it internally
...
HINDICAUSALLM_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            See the documentation of the chosen attention mechanism for more information regarding the expected
            padding strategy and corresponding attributes (`padding_side`, `padding_strategy`). For more information on
            how `attention_mask` interacts with the chosen attention mechanism, please refer to the documentation of the
        ...
        past_key_values (`Cache` or `tuple(tuple(torch.FloatTensor))`, *optional*):
            ...
            sequential decoding. This typically consists of `past_key_values.key` and `past_key_values.value` tensors,
            both shape `(batch_size, num_heads, sequence_length, embed_size_per_head)`.

            If `past_key_values` are used, the user can optionally input only the last `input_ids` (those that don't
            have their past key value states given to this model) of shape `(batch_size, 1)` instead of all `input_ids`
            of shape `(batch_size, sequence_length)`.

            If `past_key_values` are passed, the attention mask needs to be adjusted accordingly needs to be adjusted
            accordingly.

            If the attention mechanism is compatible with `attn_implementation="flash_attention_2"` attention is
            performed using the Flash Attention kernel.
...
class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
    ...
    def __init__(self, config: HindiCausalLMConfig):
        ...
        self.norm = HindiCausalLMLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(
        ...
        if use_cache:
            if past_key_values is None:  # Initialize cache if needed
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(
                0
            )  # .view(-1, seq_length) # No need to view(-1, seq_length) if bsz=1 assumed for generation

        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)

        # Prepare 4D attention mask if necessary
        # Attention mask is only for storing dump info in change log for PRs
        # `_prepare_4d_causal_attention_mask` handles causal mask creation internally
        ...
        next_decoder_cache = None  # Will be assigned Cache object if use_cache=True
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    attention_mask,
                    position_ids,
                    past_key_values,  # Pass Cache object
                    output_attentions,
                    use_cache,
                )
            else:
            ...
            if use_cache:
                # layer_outputs[2] should be the updated Cache object
                # No need to append, the same Cache object is modified in-place
                # If layer_outputs[2] is None (e.g. GC disabled cache), handle it
                next_decoder_cache = layer_outputs[2] if len(layer_outputs) > 2 else None
        ...
        # Assign the final cache object if use_cache is True
...
class HindiCausalLMForCausalLM(HindiCausalLMPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            ...
            # 1. If provided, use the length of the attention mask
            # 2. If not provided, check the cache length directly
            if attention_mask is not None and attention_mask.shape[1] > input_ids.shape[1]:
                input_ids = input_ids[:, attention_mask.shape[1] - 1 :]
            elif past_key_values.get_seq_length(self.layer_idx) > input_ids.shape[1]:
                input_ids = input_ids[
                    :, past_key_values.get_seq_length(self.layer_idx) :
                ]  # Removed -1 as DynamicCache seq len should be correct
            # Cut CausalLM specific attention mask if needed
            # Deduce position_ids based on cache length + current input length
            # Keep only the last token of input_ids if past_key_values is used
...
class HindiCausalLMForSequenceClassification(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        # Ensure sequence_lengths are within bounds and handle cases where all tokens are padded
        pooled_logits = logits[
            torch.arange(batch_size, device=logits.device),
            torch.max(sequence_lengths, torch.zeros_like(sequence_lengths)),
        ]
