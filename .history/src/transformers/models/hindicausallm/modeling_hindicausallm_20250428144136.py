...
class HindiCausalLMAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.config.max_position_embeddings)
...
class HindiCausalLMDecoderLayer(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.self_attn = HindiCausalLMAttention(config)
...
class HindiCausalLMModel(HindiCausalLMPreTrainedModel):
    ...
    def forward(
        ...
        for idx, (past_key_value, decoder_layer) in enumerate(zip(past_key_values, self.layers)):
...
@add_start_docstrings(
    """
    The HindiCausalLM Model transformer with a language modeling head on top.
    """,
    HINDICAUSALLM_START_DOCSTRING,
)
