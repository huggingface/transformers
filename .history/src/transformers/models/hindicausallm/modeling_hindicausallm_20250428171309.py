    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            layer_outputs = decoder_layer(
                ...
                use_cache=use_cache,
                cache_position=cache_position,
                # **kwargs, # Removed position_embeddings
            )
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                # `ne(pad_token_id)` gives True for non-pad tokens
                # `cumsum` along seq dim gives cumulative count of non-pad tokens
        ...
        if not return_dict:
            ...
            output = (pooled_logits,) + outputs[2:]  # Index 2 to skip past_key_values in BaseModelOutputWithPast
            return ((loss,) + output) if loss is not None else output
