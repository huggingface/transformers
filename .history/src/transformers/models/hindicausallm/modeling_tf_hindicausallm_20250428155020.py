...
from ...utils import add_start_docstrings, add_start_docstrings_to_model_forward, logging, replace_return_docstrings # Added replace_return_docstrings
...
class TFHindiCausalLMAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape: tf.TensorShape):
        ...
        if getattr(self, "layer_idx", None) is None:
             raise ValueError(
                f"The layer index should be specified for {self.__class__.__name__}"
            )
    ...
    def call(
        ...
        value_states = tf.transpose(value_states, (0, 2, 1, 3)) # (batch_size, num_key_value_heads, seq_length, head_dim)
...
class TFHindiCausalLMMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: HindiCausalLMConfig, **kwargs):
        ...
        self.layers = [
            TFHindiCausalLMDecoderLayer(config, layer_idx=i, name=f"layers.{i}") for i in range(config.num_hidden_layers)
        ]
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if seq_length > 1:
            mask_values = tf.cast(tf.fill((batch_size, seq_length, seq_length), tf.float32.min), dtype=inputs_embeds.dtype)
            mask_cond = tf.range(seq_length) < tf.reshape(tf.range(seq_length), (-1, 1))
            causal_mask = tf.where(mask_cond, 0.0, mask_values)
            if past_key_values_length > 0:
                causal_mask = tf.concat([tf.zeros((batch_size, seq_length, past_key_values_length), dtype=causal_mask.dtype), causal_mask], axis=-1)
        ...
        if attention_mask is not None:
            ...
            expanded_attn_mask = tf.where(expanded_attn_mask > 0, 0.0, mask_value) # Invert mask: 0 for keep, min_float for mask

            if combined_attention_mask is not None:
                combined_attention_mask = expanded_attn_mask + combined_attention_mask # Add masks: 0 keeps, min_float masks
            else:
    ...
    def call(
        ...
        if use_cache and past_key_values is not None and cache_position is None:
            ...
            past_key_values_length = shape_list(past_key_values[0][0])[2] # K tensor shape: [batch, heads, seq_len, dim]
