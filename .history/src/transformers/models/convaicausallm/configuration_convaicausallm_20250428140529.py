...
class ConvaiCausalLMConfig(PretrainedConfig):
    ...
    convaiinnovations/hindi-causal-lm model.
    ...
    Args:
        ...
        head_dim (`int`, *optional*):
            The attention head dimension. If not set, will be calculated as `hidden_size // num_attention_heads`.
        hidden_act (`str` or `function`, *optional*, defaults to `"silu"`):
            The non-linear activation function (function or string) in the decoder MLP.
    ...
    # We follow Gemma's plans, adjust if needed based on model structure/size
    base_model_tp_plan = {
        "layers.*.self_attn.q_proj": "colwise",
        "layers.*.self_attn.k_proj": "colwise",
        "layers.*.self_attn.v_proj": "colwise",
        "layers.*.self_attn.o_proj": "rowwise",
        "layers.*.mlp.gate_proj": "colwise",  # Assuming similar MLP structure
        "layers.*.mlp.up_proj": "colwise",    # Assuming similar MLP structure
        "layers.*.mlp.down_proj": "rowwise",  # Assuming similar MLP structure
    }
    base_model_pp_plan = {
    ...
    def __init__(
        ...
        head_dim=None, # Calculate if None
        hidden_act="silu",
        ...
        eos_token_id=2,
        bos_token_id=1,
        tie_word_embeddings=False, # Defaulting to False as per common practice unless specified
        rope_theta=10000.0,
        ...
        **kwargs,
        ...
        if (self.head_dim * self.num_attention_heads) != self.hidden_size:
             raise ValueError(
                ...
                f" Ensure `hidden_size` = `num_attention_heads` * `head_dim` ( {self.num_attention_heads} * {self.head_dim} = {self.num_attention_heads * self.head_dim})."
            )

        # support conversion when num_key_value_heads == num_attention_heads
