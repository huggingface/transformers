    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        if attention_mask is not None:
            ...
            inputs_dict["position_ids"] = (
                torch.arange(self.seq_length, device=torch_device).unsqueeze(0).expand(self.batch_size, -1)
            )
    ...
    def test_left_padding_compatibility(self):
        ...
        with torch.no_grad():
            outputs_standard = model(
                input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids_standard
            )
        ...
        padded_input_ids = torch.cat([torch.full((1, 2), pad_token_id, device=torch_device), input_ids], dim=1)
        padded_attention_mask = torch.cat([torch.zeros(1, 2, device=torch_device), attention_mask], dim=1)
        ...
        with torch.no_grad():
            outputs_padded = model(
                input_ids=padded_input_ids, attention_mask=padded_attention_mask, position_ids=position_ids_padded
            )
            next_token_logits_padded = outputs_padded.logits[:, -1, :]

        # Compare with higher tolerance for numerical stability
        torch.testing.assert_close(next_token_logits_standard, next_token_logits_padded, rtol=1e-3, atol=1e-3)
