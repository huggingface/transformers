...
1. **[LlamaInfinite](https://huggingface.co/docs/transformers/model_doc/llama_infinite)** は、Chi Han によって提案された [LM-Infinite](https://browse.arxiv.org/abs/2308.16137) を適用します。 Qifan Wang、Wenhan Xiong、Yu Chen、Heng Ji、Sinong Wang がラマファミリー全員に加わりました。 この手法を使用すると、パラメータを更新せずに、一般的な品質を維持したままテキストを無限に生成できます。 LlamaInfinite は、最小限のコーディング作業で既存のすべての Llama チェックポイントと互換性があります。
