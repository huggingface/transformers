...
Two things about `modular` files that are easy to get wrong:

- **A `modular` file is inherited by other models.** Editing one changes every generated file derived from it, not just the model you are looking at — `modular_deepseek_vl.py` also generates `modeling_deepseek_vl_hybrid.py`. Run `make fix-repo` and check what it rewrote; hand-editing the one generated file you had in mind leaves the others stale and `Check repository consistency` red.
- **`some_attribute = AttributeError()` in a `modular` file deletes an inherited attribute**, it is not a placeholder or a bug. The converter drops the attribute from the generated class so it falls back to the parent's default; `raise AttributeError("No need to inherit!")` in a method body does the same for a method. Replacing either with a "real" value silently changes behaviour (for example `_no_split_modules = AttributeError()` → `[]` changes how the model may be sharded across devices). See [Removing attributes](../docs/source/en/modular_transformers.md#removing-attributes) and [Deleting unused methods](../docs/source/en/modular_transformers.md#deleting-unused-methods).
