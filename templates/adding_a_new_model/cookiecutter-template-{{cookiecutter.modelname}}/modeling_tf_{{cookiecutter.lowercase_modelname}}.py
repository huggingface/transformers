...
def shift_tokens_right(input_ids: tf.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    shifted_input_ids = tf.where(
        ...
        tf.fill(dims=shape_list(shifted_input_ids), value=pad_token_id),
        shifted_input_ids
    )
...
class TF{{cookiecutter.camelcase_modelname}}LearnedPositionalEmbedding(TFSharedEmbeddings):
    ...
    def call(self, input_shape: List[int], past_key_values_length: Optional[int] = 0):
        ...
        positions = tf.range(start=past_key_values_length, limit=seq_len + past_key_values_length, delta=1, name="range")
...
class TF{{cookiecutter.camelcase_modelname}}Attention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        query_states = tf.reshape(tensor=self.transpose_for_scores(query_states, tgt_len, batch_size), shape=proj_shape)
        ...
        if attention_mask is not None:
            ...
            attn_weights = tf.reshape(tensor=attn_weights, shape=(batch_size, self.num_heads, tgt_len, src_len)) + attention_mask
        ...
        attn_output = tf.transpose(
            tf.reshape(tensor=attn_output, shape=(batch_size, self.num_heads, tgt_len, self.head_dim)), perm=(0, 2, 1, 3)
        )
...
# Copied from transformers.models.bart.modeling_tf_bart.TFBartEncoderLayer with Bart->{{cookiecutter.camelcase_modelname}}
...
# Copied from transformers.models.bart.modeling_tf_bart.TFBartDecoderLayer with Bart->{{cookiecutter.camelcase_modelname}}
class TF{{cookiecutter.camelcase_modelname}}DecoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: {{cookiecutter.camelcase_modelname}}Config, **kwargs):
        ...
        self.self_attn_layer_norm = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="self_attn_layer_norm")
        self.encoder_attn = TFB{{cookiecutter.camelcase_modelname}}Attention(
            embed_dim=self.embed_dim,
            num_heads=config.decoder_attention_heads,
            dropout=config.attention_dropout,
            name="encoder_attn",
...
{{cookiecutter.uppercase_modelname}}_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`np.ndarray`, :obj:`tf.Tensor`, :obj:`List[tf.Tensor]` :obj:`Dict[str, tf.Tensor]` or :obj:`Dict[str, np.ndarray]` of shape :obj:`({0})`):
            Indices of input sequence tokens in the vocabulary.
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["inputs_embeds"] is None:
            inputs_embeds = self.embed_tokens(inputs=inputs["input_ids"]) * self.embed_scale
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["encoder_hidden_states"] is not None and inputs["encoder_attention_mask"] is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            inputs["encoder_attention_mask"] = _expand_mask(mask=inputs["encoder_attention_mask"], tgt_len=input_shape[-1])
...
class TF{{cookiecutter.camelcase_modelname}}ForConditionalGeneration(TF{{cookiecutter.camelcase_modelname}}PreTrainedModel, TFCausalLanguageModelingLoss):
