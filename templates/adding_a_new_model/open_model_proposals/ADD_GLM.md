...
The following sections explain in detail how to add GLM 
to Transformers. You will work closely with Suraj to
integrate GLM into Transformers. By doing so, you will both gain a 
theoretical and deep practical understanding of GLM. 
...
theoretical understanding of `GLM`. However, if you prefer to
understand the theoretical aspects of the model *on-the-job*, then it is
totally fine to directly dive into the `GLM`'s code-base. This
...
`GLM`'s paper, or if you just enjoy programming much more than
reading scientific papers.

### 1. (Optional) Theoretical aspects of GLM

You should take some time to read *GLM's* paper, if such
...
-   What type of model is *GLM*? BERT-like encoder-only
...
-   What are the applications of *GLM*? Text
...
of the model, you might want to write to Suraj with any
...
#### Make sure you've understood the fundamental aspects of GLM

Alright, now you should be ready to take a closer look into the actual code of GLM.
You should have understood the following aspects of GLM by now:

- The architecture is very similer to GPT2 (Implemented using MegatronLM GPT2). 
- GLM uses a combination of bi-directional and uni-directional attention mask. It splits the input sequence
  in two parts A and B. The tokens in part A can attened to one another (bi-directional) and tokens in part B
  can attend to tokens in part A and previous tokens in part B (uni-directional or causal mask).
- GLM uses two position embeddings to represent inter- and intra- span positions. 
- GLM treats every task as a generations task eliminating the need of task specific heads.

If any of the mentioned aspects above are **not** clear to you, now is a great time to talk to Suraj.
...
4.  We recommend adding the PyTorch version of *GLM* to
...
5.  To port *GLM*, you will also need access to its
    original repository:
...
Now you have set up a development environment to port *GLM*
to 🤗 Transformers.
...
At first, you will work on the original *GLM* repository.
...
*GLM*. At Hugging Face, one of our main goals is to *make
...
You should start thereby by diving into the [original repository](https://github.com/THUDM/GLM).
...
```python
model = GLMModel.load_pretrained_checkpoint("/path/to/checkpoint/")
...
6.  Retrieve the output of the whole GLM Model
...
*GLM* in which case an **efficient** debugging environment
...
#### More details on how to create a debugging environment for GLM 

- GLM has multiple pretrained checkpoints that should eventually all be ported to
Transformers. The checkpoints can be found [here](https://mailstsinghuaeducn-my.sharepoint.com/:f:/g/personal/duzx16_mails_tsinghua_edu_cn/Eg8MZe62MlVFs_mK2tHaH-sBC-UC01jpGPZop08pID7sOw?e=MsevNR)
- Setting up the environment for GLM is straightforward and is the instructions can be found [here](https://github.com/THUDM/GLM#installation)
- The most important part if the GLM code-base is the [model dir](https://github.com/THUDM/GLM/tree/main/model) which includes all the logic necessary to implement GLM.


### Port GLM to 🤗 Transformers
...
Add *GLM*", in 🤗 Transformers so that you and the Hugging
...
5.  Once you are satisfied, go to the webpage of your fork on GitHub.
    Click on "Pull request". Make sure to add the GitHub handle of 
		Suraj as a reviewer, so that the Hugging
...
PR. This way, Suraj will always be notified when you are
committing new code or if you have a question. It is often very helpful
to point Suraj to your added code so that the Hugging
...
In the same way, Suraj will open comments when reviewing
...
feel free to ping Suraj by Slack or email.

**5. Adapt the generated models code for GLM**
...
`src/transformers/models/glm/modeling_glm.py` and
`src/transformers/models/glm/configuration_glm.py`.

Now you can finally start coding :). The generated code in
`src/transformers/models/glm/modeling_glm.py` will
...
implementation of *GLM*, *i.e.* the following command
should work:
...
parameters as defined in `GLMConfig()` with random weights,
...
checkpoint you used to debug *GLM* in the original
repository to a checkpoint compatible with your just created 🤗
Transformers implementation of *GLM*. It is not advised to
...
framework as *GLM*. Usually, it is enough to copy an
already existing conversion script and slightly adapt it for your use
case. Don't hesitate to ask Suraj to point you to a
...
config parameters in `GLMConfig()` that do not exactly match
...
either you used incorrect parameters in `GLMConfig()`, have a
...
[TODO FILL: Here the model name might have to be adapted, *e.g.*, maybe GLMForConditionalGeneration instead of GLMModel]

```python
model = GLMModel.from_pretrained("/path/to/converted/checkpoint/folder")
input_ids = [0, 4, 4, 3, 2, 4, 1, 7, 19]
...
instead of `torch.float32`. Don't hesitate to ask Suraj
...
under the same `tests/test_modeling_glm.py`. Run this test
file to verify that all common tests pass:

```python
pytest tests/test_modeling_glm.py
```
...
`GLMModelIntegrationTests` and only has to be filled out by
you. To ensure that those tests are passing, run

```python
RUN_SLOW=1 pytest -sv tests/test_modeling_glm.py::GLMModelIntegrationTests
```

**Note:** In case you are using Windows, you should replace `RUN_SLOW=1` with `SET RUN_SLOW=1`

Second, all features that are special to *GLM* should be
tested additionally in a separate test under
`GLMModelTester`/`GLMModelTest`. This part is often
...
Next, we should add the tokenizer of *GLM*. Usually, the
...
[TODO FILL: Here the mentor should add a comment whether a new tokenizer is required or if this is not the case which existing tokenizer closest resembles 
 GLM's tokenizer and how the tokenizer should be implemented]
 [...]
 [...]
...
For GLM, the tokenizer files can be found here:
...
tokenizer = GLMTokenizer.from_pretrained("/path/to/tokenizer/folder/")
...
Analogous to the modeling test files of *GLM*, the
tokenization test files of *GLM* should contain a couple of
...
`tests/test_modeling_glm.py` in 🤗 Transformers. Such a test
...
Now, all the necessary functionality for *GLM* is added -
...
`docs/source/model_doc/glm.rst` that you should fill out.
...
model should be used. Don't hesitate to ping Suraj
regarding the docstrings.

Next, make sure that the docstring added to
`src/transformers/models/glm/modeling_glm.py` is
...
Great, now you have added all the necessary code for *GLM*.
...
docstring or some incorrect naming. Suraj will surely
...
should work alongside Suraj here to decide on a fitting
...
*GLM* can be used for inference and/or fine-tuned on a
...
getting your PR merged into master. Usually, Suraj
