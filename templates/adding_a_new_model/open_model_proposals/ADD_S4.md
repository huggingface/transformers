How can I add S4 to 🤗 Transformers?
=====================================

Mentor: [Matt](https://github.com/rocketknight1)

Begin: ASAP!

Estimated End: Ideally by the end of 2021, but this isn't an easy task and it could run over!
...
The following sections explain in detail how to add S4 
to Transformers. You will work closely with [Matt](https://github.com/rocketknight1) to
integrate S4 into Transformers. By doing so, you will both gain a 
theoretical and deep practical understanding of S4. 
...
explanatory purposes, we will call the PyTorch model to be added to 🤗 Transformers
`BrandNewBert`.
...
theoretical understanding of `S4`. However, if you prefer to
understand the theoretical aspects of the model *on-the-job*, then it is
totally fine to directly dive into the `S4`'s code-base. This
...
`S4`'s paper, or if you just enjoy programming much more than
reading scientific papers.

### 1. (Optional) Theoretical aspects of S4

You should take some time to read *S4's* paper. There might be large sections of the paper that
...
-   What type of model is *S4*? BERT-like encoder-only
...
-   What are the applications of *S4*? Text
...
of the model, you might want to write to [Matt](https://github.com/rocketknight1) with any
...
#### Make sure you've understood the fundamental aspects of S4

Alright, now you should be ready to take a closer look into the actual code of S4.
You should have understood the following aspects of S4 by now:

- The core of S4 is a drop-in replacement for attention that has extremely good scaling properties as sequence length increases.
- S4 is able to handle long-range tasks (like Pathfinder-X) that all previous known architectures fail on.
- Unlike many other models added to the Transformers library, S4 is interesting for its architecture and less so for its pretrained weights. There isn't a single "S4 model" that everyone is going to fine-tune from - yet!

If any of the mentioned aspects above are **not** clear to you, now is a great time to talk to [Matt](https://github.com/rocketknight1).
...
4.  We recommend adding the PyTorch version of *S4* to
...
5.  To port *S4*, you will also need access to its
    original repository:
...
Now you have set up a development environment to port *S4*
to 🤗 Transformers.
...
At first, you will work on the original *S4* repository.
...
*S4*. At Hugging Face, one of our main goals is to *make
...
You should therefore start by diving into the [original repository](https://github.com/HazyResearch/state-spaces). You should pay particular attention to the [standalone example](https://github.com/HazyResearch/state-spaces/blob/main/example.py), which depends only on a [single file](https://github.com/HazyResearch/state-spaces/blob/main/src/models/sequence/ss/standalone/s4.py) containing an implementation of the core layers of the model. Focusing on these will avoid having to deal with all the complexity of the library, which will hopefully make your task a lot easier!
...
```python
model = S4Model.load_pretrained_checkpoint("/path/to/checkpoint/")
...
6.  Retrieve the output of the whole S4 Model
...
*S4* in which case an **efficient** debugging environment
...
#### More details on how to create a debugging environment for S4 

One critical thing to note about S4 is that it depends on custom CUDA ops, which are provided either by the included .cu file, or by the `pykeops` library. For simplicity, we advise using the `pykeops` implementation for the port, as we can then make the model work for users simply by adding a `pykeops` dependency. `pykeops` is `pip` installable, and this is much more accessible to most users than requiring them to compile CUDA locally.

### Port S4 to 🤗 Transformers
...
Add *S4*", in 🤗 Transformers so that you and the Hugging
...
5.  Once you are satisfied, go to the webpage of your fork on GitHub.
    Click on "Pull request". Make sure to add the GitHub handle of 
		[Matt](https://github.com/rocketknight1) as a reviewer, so that the Hugging
...
PR. This way, [Matt](https://github.com/rocketknight1) will always be notified when you are
committing new code or if you have a question. It is often very helpful
to point [Matt](https://github.com/rocketknight1) to your added code so that the Hugging
...
In the same way, [Matt](https://github.com/rocketknight1) will open comments when reviewing
...
feel free to ping [Matt](https://github.com/rocketknight1) by Slack or email.

**5. Adapt the generated models code for S4**
...
`src/transformers/models/s4/modeling_s4.py` and
`src/transformers/models/s4/configuration_s4.py`.

Now you can finally start coding :). The generated code in
`src/transformers/models/s4/modeling_s4.py` will
...
`src/transformers/models/s4/modeling_s4.py`
...
implementation of *S4*, *i.e.* the following command
should work:

```python
from transformers import S4Model, S4Config
model = S4Model(S4Config())
```

The above command will create a model according to the default
parameters as defined in `S4Config()` with random weights,
thus making sure that the `init()` methods of all components works.

The most difficult part of implementing S4 will likely be the mechanics of the S4 layer itself, as it is very exotic compared to most normal Transformer architectures. Hopefully, a lot of the code can be ported as-is without necessarily needing a deep understanding of the underlying mathematics. We'll be available to help with any issues you encounter!

**6. Write a conversion script**

Next, you should write a conversion script that lets you convert the
checkpoint you used to debug *S4* in the original
repository to a checkpoint compatible with your just created 🤗
Transformers implementation of *S4*. It is not advised to
...
framework as *S4*. Usually, it is enough to copy an
already existing conversion script and slightly adapt it for your use
case. Don't hesitate to ask [Matt](https://github.com/rocketknight1) to point you to a
...
config parameters in `S4Config()` that do not exactly match
...
either you used incorrect parameters in `S4Config()`, have a
...
We're working on getting some exciting checkpoints here, but any checkpoint, even one that hasn't been trained for very long, will work for testing conversions.
...
```python
model = S4Model.from_pretrained("/path/to/converted/checkpoint/folder")
input_ids = [0, 4, 4, 3, 2, 4, 1, 7, 19]
...
instead of `torch.float32`. Don't hesitate to ask [Matt](https://github.com/rocketknight1)
...
under the same `tests/test_modeling_s4.py`. Run this test
file to verify that all common tests pass:

```python
pytest tests/test_modeling_s4.py
```
...
`S4ModelIntegrationTests` and only has to be filled out by
you. To ensure that those tests are passing, run

```python
RUN_SLOW=1 pytest -sv tests/test_modeling_s4.py::S4ModelIntegrationTests
```
...
Because S4 is focused on long-range inputs, no pre-trained tokenizer exists.
...
`tests/test_modeling_s4.py` in 🤗 Transformers. Such a test
...
Now, all the necessary functionality for *S4* is added -
...
`docs/source/model_doc/s4.rst` that you should fill out.
...
model should be used. Don't hesitate to ping [Matt](https://github.com/rocketknight1)
regarding the docstrings.

Next, make sure that the docstring added to
`src/transformers/models/s4/modeling_s4.py` is
...
Great, now you have added all the necessary code for *S4*.
...
docstring or some incorrect naming. [Matt](https://github.com/rocketknight1) will surely
...
should work alongside [Matt](https://github.com/rocketknight1) here to decide on a fitting
...
*S4* can be used for inference and/or fine-tuned on a
...
getting your PR merged into master. Usually, [Matt](https://github.com/rocketknight1)
