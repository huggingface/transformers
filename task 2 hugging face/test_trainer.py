...
if is_torch_available():
    ...
    def get_regression_trainer(
        ...
        **kwargs,
        ...
        compute_loss_func = kwargs.pop("compute_loss_func", None)
        signature_columns = kwargs.pop("signature_columns", None)
...
class TrainerIntegrationTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_evaluate_without_labels_calls_compute_metrics(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            called = {"value": False}

            def compute_metrics(eval_pred):
                called["value"] = True
                self.assertIsNone(eval_pred.label_ids)
                self.assertIsNotNone(eval_pred.predictions)
                preds = eval_pred.predictions
                if isinstance(preds, tuple):
                    preds = preds[0]
                return {"pred_mean": float(np.mean(preds))}
            ...
            self.assertIn("eval_pred_mean", results)

    def test_evaluate_without_labels_calls_compute_loss_func(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            called = {"value": False}
            ...
            results = trainer.evaluate()
            self.assertTrue(called["value"])
            self.assertIn("eval_loss", results)

    def test_floating_point_ops_fallback_input_key(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            trainer = get_regression_trainer(pretrained=True, output_dir=tmp_dir)
            flos = trainer.floating_point_ops({"input_x": torch.zeros(2, 3)})
            self.assertGreater(flos, 0)

    def test_remove_unused_columns_with_signature_columns(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            dataset = datasets.Dataset.from_dict(
                {"prompt": ["a", "b"], "labels": [0, 1], "extra": ["x", "y"]}
            )
            trainer = get_regression_trainer(
                output_dir=tmp_dir,
                label_names=["labels"],
                signature_columns=["prompt", "labels"],
            )
            filtered = trainer._remove_unused_columns(dataset, description="train")
            self.assertIn("prompt", filtered.column_names)
            self.assertIn("labels", filtered.column_names)
            self.assertNotIn("extra", filtered.column_names)
