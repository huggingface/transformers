...
class Trainer:
    ...
    Args:
        ...
        compute_metrics (`Callable[[EvalPrediction], Dict]`, *optional*):
            ...
            a dictionary string to metric values. Depending on the training setup, `EvalPrediction.predictions` or
            `EvalPrediction.label_ids` can be `None` if they were not computed or not provided. *Note* When passing
            TrainingArgs with `batch_eval_metrics` set to
        ...
        signature_columns (`list[str]`, *optional*):
            Column names to keep when `remove_unused_columns=True`. If provided, this overrides the model signature
            inspection and is useful for custom trainers that transform inputs before calling the model.
    ...
    def __init__(
        ...
        if signature_columns is not None:
            ...
            signature_columns += list(set(["label", "label_ids"] + self.label_names))
            self._signature_columns = list(dict.fromkeys(signature_columns))
    ...
    def evaluation_loop(
            ...
            if self.args.batch_eval_metrics:
                if self.compute_metrics is not None:
                    ...
                    has_metrics_inputs = (
                        ...
                        or batch_kwargs["losses"] is not None
                        or batch_kwargs["inputs"] is not None
                    )
                    if has_metrics_inputs:
        ...
        if self.compute_metrics is not None and not self.args.batch_eval_metrics:
            ...
            has_metrics_inputs = (
                all_preds is not None
                or all_labels is not None
                or eval_set_kwargs["losses"] is not None
                or eval_set_kwargs["inputs"] is not None
            )
            if has_metrics_inputs:
                metrics = self.compute_metrics(
    ...
    def prediction_step(
        ...
        compute_loss_is_overridden = getattr(self.compute_loss, "__func__", None) is not Trainer.compute_loss
        should_compute_loss = has_labels or loss_without_labels or self.compute_loss_func is not None or compute_loss_is_overridden
    ...
    def floating_point_ops(self, inputs: dict[str, torch.Tensor | Any]):
        ...
        input_tensor = inputs.get(main_input)
        if input_tensor is None:
            label_keys = set(self.label_names)
            label_keys.update({"labels", "label", "label_ids"})
        ...
        return 6 * input_tensor.numel() * self.model.num_parameters(exclude_embeddings=True)
