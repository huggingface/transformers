...
def _make_workload(max_len, quiet=False):
    ...
    tok_cache = [tokenizer(t, truncation=True, max_length=max_len, return_attention_mask=True) for t in texts]
    ...
    if not quiet:
        print(
            f"  max_len={max_len}: n_unique={n_unique}  std_len={std_len:.1f}  "
...
def main():
    ...
    for max_len in MAX_LENS:
        ...
        winner = "freeze" if (m_freeze and m_clear and m_freeze["mean_ms"] < m_clear["mean_ms"]) else "clear"

        print(
            f"  {max_len:>7}  {n_unique:>6}  {freeze_at:>6}  "
