...
class GPT2Benchmark(ModelBenchmark):
    ...
    def get_scenario_configs(self) -> list[dict[str, Any]]:
        """
        Get GPT-2-specific scenario configurations.
    ...
    def _is_kernelization_available(self) -> bool:
        """Check if kernelization is available for GPT-2."""
        try:
    ...
    def get_default_generation_config(self) -> dict[str, Any]:
        """Get GPT-2-specific generation configuration."""
        return {
            "do_sample": False,
            "top_p": 1.0,
            "temperature": 1.0,
...
def run_gpt2(logger, output_dir, **kwargs):
    ...
    model_id = kwargs.get("model_id", "gpt2")
    ...
    logger.info(f"Starting GPT-2 benchmark for model: {model_id}")
    logger.info(
