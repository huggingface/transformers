...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29500 \
  ...
  --start_step 0 --stop_step 10 --save_dir ./checkpoints_tp \
  --distributed_checkpoint > "${LOG_FSDP_TP}.phase1" 2>&1 &
PID1=$!

CUDA_VISIBLE_DEVICES=4,5 torchrun --nproc_per_node=2 --master_port=29501 \
  $SCRIPT $COMMON_ARGS --fsdp_size 2 \
  --start_step 0 --stop_step 10 --save_dir ./checkpoints_fsdp > "${LOG_FSDP_ONLY}.phase1" 2>&1 &
PID2=$!
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29500 \
  $SCRIPT $COMMON_ARGS --fsdp_size 2 --tp_size 2 --enable_sp \
  --start_step 10 --stop_step 20 \
  --resume_dir ./checkpoints_tp --save_dir ./checkpoints_tp_resumed \
  --distributed_checkpoint > "${LOG_FSDP_TP}.phase2" 2>&1 &
PID1=$!
