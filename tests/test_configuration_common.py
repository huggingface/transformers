...
class ConfigTester:
    ...
    def create_and_test_config_from_pretrained_custom_kwargs(self):
        ...
        config = self.config_class(**self.inputs_dict)
        if config.get_sub_config(modality="text") is config or not hasattr(self.parent.model_tester, "get_config"):
            return
        ...
        original_text_config = config.get_sub_config(modality="text")
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            text_config_reloaded_dict = config_reloaded.get_sub_config(modality="text").to_dict()
