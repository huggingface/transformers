...
class TokenizerTesterMixin:
        ...
        for tokenizer in tokenizers:
            with self.subTest(f"{tokenizer.__class__.__name__}"):
                ...
                try:
                    ...
                    self.skipTest("Encountered NotImplementedError calling prepare_seq2seq_batch")
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            ...
            with self.subTest(f"{tokenizer.__class__.__name__} ({pretrained_name}, {tokenizer.__class__.__name__})"):
                if is_torch_available():
                    ...
                    self.skipTest("No expected framework from PT, TF or JAX found")

                if not tokenizer.pad_token or tokenizer.pad_token_id < 0:
                    self.skipTest("This tokenizer has no padding token set, or pad_token_id < 0")
    ...
    def test_save_slow_from_fast_and_reload_fast(self):
        if not self.test_slow_tokenizer or not self.test_rust_tokenizer:
            ...
            self.skipTest("test_rust_tokenizer or test_slow_tokenizer is set to False")
