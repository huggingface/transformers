...
class EmformerModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (EmformerForCTC, EmformerModel, EmformerForMaskedLM, EmformerForSequenceClassification, EmformerForPreTraining)
        if is_torch_available()
        else ()
    )
    test_pruning = False
...
@require_torch
class EmformerRobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            EmformerForCTC,
            EmformerModel,
    ...
    def setUp(self):
        self.model_tester = EmformerModelTester(
    ...
    def test_model_for_pretraining(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        model = EmformerForPreTraining(config).to(torch_device)
    ...
    def test_mask_time_feature_prob_ctc_single_batch(self):
        model = EmformerForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-emformer",
            mask_time_prob=0.2,
...
class EmformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_normal(self):
        model = EmformerForCTC.from_pretrained("anton-l/emformer-base-librispeech")
        model.to(torch_device)
    ...
    def test_inference_ctc_robust_batched(self):
        model = EmformerForCTC.from_pretrained("facebook/emformer-large-960h-lv60-self").to(torch_device)
        processor = Wav2Vec2Processor.from_pretrained("facebook/emformer-large-960h-lv60-self", do_lower_case=True)
    ...
    def test_inference_integration(self):
        model = EmformerForPreTraining.from_pretrained("facebook/emformer-base")
        model.to(torch_device)
        feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/emformer-base")
        input_speech = self._load_datasamples(2)
    ...
    def test_inference_pretrained(self):
        ...
        feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(
            "facebook/emformer-base", return_attention_mask=True
        )
        input_speech = self._load_datasamples(2)
        ...
        config = EmformerConfig.from_pretrained("facebook/emformer-base")
        model_rand = EmformerForPreTraining(config).to(torch_device).eval()
        ...
        # a pretrained emformer model has learned to predict the quantized latent states
        # => the cosine similarity between quantized states and predicted states > 0.5
        # a random emformer model has not learned to predict the quantized latent states
        # => the cosine similarity between quantized states and predicted states is very likely < 0.1
        self.assertTrue(cosine_sim_masked.mean().item() - 5 * cosine_sim_masked_rand.mean().item() > 0)

    @unittest.skipIf(torch_device != "cpu", "cannot make deterministic on GPU")
    def test_loss_pretraining(self):
        model = EmformerForPreTraining.from_pretrained(
            "facebook/emformer-base",
    ...
    def test_inference_keyword_spotting(self):
        model = EmformerForSequenceClassification.from_pretrained("superb/emformer-base-superb-ks").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("superb/emformer-base-superb-ks")
    ...
    def test_inference_intent_classification(self):
        model = EmformerForSequenceClassification.from_pretrained("superb/emformer-base-superb-ic").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("superb/emformer-base-superb-ic")
    ...
    def test_inference_speaker_identification(self):
        model = EmformerForSequenceClassification.from_pretrained("superb/emformer-base-superb-sid").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("superb/emformer-base-superb-sid")
    ...
    def test_inference_emotion_recognition(self):
        model = EmformerForSequenceClassification.from_pretrained("superb/emformer-base-superb-er").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("superb/emformer-base-superb-er")
    ...
    def test_phoneme_recognition(self):
        model = EmformerForCTC.from_pretrained("facebook/emformer-lv-60-espeak-cv-ft").to(torch_device)
        processor = Wav2Vec2Processor.from_pretrained("facebook/emformer-lv-60-espeak-cv-ft")
    ...
    @require_pyctcdecode
    @require_torchaudio
    def test_emformer_with_lm(self):
        ...
        model = EmformerForCTC.from_pretrained("patrickvonplaten/emformer-large-xlsr-53-spanish-with-lm").to(
            torch_device
        )
        processor = Wav2Vec2ProcessorWithLM.from_pretrained("patrickvonplaten/emformer-large-xlsr-53-spanish-with-lm")
    ...
    def test_inference_diarization(self):
        model = EmformerForAudioFrameClassification.from_pretrained("anton-l/emformer-base-superb-sd").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("anton-l/emformer-base-superb-sd")
    ...
    def test_inference_speaker_verification(self):
        model = EmformerForXVector.from_pretrained("anton-l/emformer-base-superb-sv").to(torch_device)
        processor = Wav2Vec2FeatureExtractor.from_pretrained("anton-l/emformer-base-superb-sv")
        input_data = self._load_superb("si", 4)
