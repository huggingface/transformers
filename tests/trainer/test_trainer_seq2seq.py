...
from transformers import (AutoModelForSeq2SeqLM, BertTokenizer,
                          DataCollatorForSeq2Seq, EncoderDecoderModel,
                          GenerationConfig, Seq2SeqTrainer,
                          Seq2SeqTrainingArguments, T5Tokenizer)
from transformers.testing_utils import (TestCasePlus, require_sentencepiece,
                                        require_torch, slow)
...
class Seq2seqTrainerTester(TestCasePlus):
    ...
    def test_finetune_bert2bert(self):
        ...
        train_dataset = datasets.load_dataset(
            "abisee/cnn_dailymail", "3.0.0", split="train[:1%]"
        )
        val_dataset = datasets.load_dataset(
            "abisee/cnn_dailymail", "3.0.0", split="validation[:1%]"
        )
    ...
    def test_return_sequences(self):
        ...
        data_collator = DataCollatorForSeq2Seq(
            tokenizer, model=model, return_tensors="pt", padding="longest"
        )
        ...
        prepared_dataset = dataset.map(
            prepare_data, batched=True, remove_columns=[INPUT_COLUMN, TARGET_COLUMN]
        )
        ...
        for num_return_sequences in range(3, 0, -1):
            ...
            metrics = trainer.evaluate(
                eval_dataset=prepared_dataset, generation_config=gen_config
            )
            assert (
    ...
    def test_bad_generation_config_fail_early(self):
        ...
        self.assertIn(
            "The loaded generation config instance is invalid", str(exc.exception)
        )
