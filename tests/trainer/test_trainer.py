...
class TrainerIntegrationPrerunTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_gradient_accumulation_loss_alignment(self):
        ...
        dataset = datasets.load_dataset(
            dataset_name, dataset_config, split="train[:500]"
        )
        ...
        for diff in diff_broken:
            self.assertGreater(
                abs(diff), 0.1, f"Difference {diff} is not greater than 0.1"
            )
...
if is_torch_available():
    ...
    if is_bitsandbytes_available():
        ...
        if version.parse(importlib.metadata.version("bitsandbytes")) >= version.parse(
            ...
            optim_test_params.append(
                (
                    TrainingArguments(
                        optim=OptimizerNames.ADEMAMIX_8BIT, output_dir="None"
                    ),
                    bnb.optim.AdEMAMix,
                    default_ademamix_kwargs,
                )
            ...
            optim_test_params.append(
                (
                    TrainingArguments(
                        optim=OptimizerNames.PAGED_ADEMAMIX_8BIT, output_dir="None"
                    ),
                    bnb.optim.AdEMAMix,
                    default_ademamix_kwargs,
                )
            ...
            optim_test_params.append(
                (
                    TrainingArguments(
                        optim=OptimizerNames.PAGED_ADEMAMIX, output_dir="None"
                    ),
                    bnb.optim.AdEMAMix,
                    default_ademamix_kwargs,
                )
    ...
    if is_torchao_available():
        ...
        optim_test_params.append(
            (
                TrainingArguments(
                    optim=OptimizerNames.ADAMW_TORCH_4BIT, output_dir="None"
                ),
