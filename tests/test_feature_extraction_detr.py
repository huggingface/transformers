...
class DetrFeatureExtractionTest(FeatureExtractionSavingTestMixin, unittest.TestCase):
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        if not torch.allclose(encoding["pixel_values"][0, 0, 0, :3], expected_slice, atol=1e-4):
            raise ValueError("`pixel_values` in `encoding` doesn't match with `expected_slice`")

        # verify area
        expected_area = torch.tensor([5887.9600, 11250.2061, 489353.8438, 837122.7500, 147967.5156, 165732.3438])
        if not torch.allclose(encoding["labels"][0]["area"], expected_area):
            raise ValueError("`area` in `encoding` doesn't match with `expected_area`")
        # verify boxes
        expected_boxes_shape = torch.Size([6, 4])
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = torch.tensor([0.5503, 0.2765, 0.0604, 0.2215])
        if not torch.allclose(encoding["labels"][0]["boxes"][0], expected_boxes_slice, atol=1e-3):
            raise ValueError("`boxes` in `encoding` doesn't match with `expected_boxes_slice`")
        # verify image_id
        expected_image_id = torch.tensor([39769])
        if not torch.allclose(encoding["labels"][0]["image_id"], expected_image_id):
            raise ValueError("`image_id` in `encoding` doesn't match with `expected_image_id`")
        # verify is_crowd
        expected_is_crowd = torch.tensor([0, 0, 0, 0, 0, 0])
        if not torch.allclose(encoding["labels"][0]["iscrowd"], expected_is_crowd):
            raise ValueError("`iscrowd` in `encoding` doesn't match with `expected_is_crowd`")
        # verify class_labels
        expected_class_labels = torch.tensor([75, 75, 63, 65, 17, 17])
        if not torch.allclose(encoding["labels"][0]["class_labels"], expected_class_labels):
            raise ValueError("`class_labels` in `encoding` doesn't match with `expected_class_labels`")
        # verify orig_size
        expected_orig_size = torch.tensor([480, 640])
        if not torch.allclose(encoding["labels"][0]["orig_size"], expected_orig_size):
            raise ValueError("`orig_size` in `encoding` doesn't match with `expected_orig_size`")
        # verify size
        expected_size = torch.tensor([800, 1066])
        if not torch.allclose(encoding["labels"][0]["size"], expected_size):
            raise ValueError("`size` in `encoding` doesn't match with `expected_size`")
