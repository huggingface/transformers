...
class Qwen3IntegrationTest(unittest.TestCase):
    ...
    def test_model_600m_logits(self):
        ...
        EXPECTED_MEAN = torch.tensor([[-1.4607, 1.3215, 3.8012, 3.4572, 2.8690, 1.8552, 2.1274, 2.1316]])
        torch.testing.assert_close(out.mean(-1), EXPECTED_MEAN, rtol=1e-1, atol=1e-1)
        # slicing logits[0, 0, 0:30]
        EXPECTED_SLICE = torch.tensor([5.8750, 6.0938, 5.5625, 3.8594, 2.6094, 1.9531, 4.3125, 4.9375, 3.9062, 3.1250, 3.6875, 5.1875, 6.9062, 5.7500, 5.4062, 7.0625, 8.7500, 8.7500, 8.1250, 7.9375, 8.0625, 7.5312, 7.3750, 7.2188, 7.2500, 5.8750, 2.8750, 4.3438, 2.3438, 2.2500])  # fmt: skip
