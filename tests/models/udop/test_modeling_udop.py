...
class UdopModelTester:
    ...
    def get_large_model_config(self):
        return UdopConfig.from_pretrained("udop-base")
    ...
    def get_pipeline_config(self):
        return UdopConfig(
            vocab_size=166,  # udop forces 100 extra tokens
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [UdopModel, UdopForConditionalGeneration]:
            torch.manual_seed(0)
...
class UdopModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (UdopModel, UdopForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (UdopForConditionalGeneration,) if is_torch_available() else ()
    pipeline_model_mapping = (
        {
            "conversational": UdopForConditionalGeneration,
            "feature-extraction": UdopModel,
            "summarization": UdopForConditionalGeneration,
            "text2text-generation": UdopForConditionalGeneration,
        }
        if is_torch_available()
        else {}
    )
    all_parallelizable_model_classes = (UdopModel, UdopForConditionalGeneration) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def test_model_from_pretrained(self):
        for model_name in UDOP_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = UdopModel.from_pretrained(model_name)
...
class UdopModelFp16Tests(unittest.TestCase):
    def test_fp16_fp32_conversion(self):
        ...
        model = UdopForConditionalGeneration.from_pretrained("microsoft/udop-large", torch_dtype=torch.float16)
        ...
        model = UdopForConditionalGeneration.from_pretrained("microsoft/udop-large", torch_dtype=torch.bfloat16)
        ...
        model = UdopForConditionalGeneration.from_pretrained(
            "microsoft/udop-large", torch_dtype=torch.bfloat16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)
        ...
        # Load without using `accelerate`
        model = UdopForConditionalGeneration.from_pretrained(
            "microsoft/udop-large", torch_dtype=torch.float16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.float32)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.float16)
...
class UdopModelIntegrationTests(unittest.TestCase):
    ...
    def test_torch_quant(self):
        r"""
        Test that a simple `torch.quantization.quantize_dynamic` call works on a Udop model.
        """
        model_name = "google/flan-microsoft/udop-large"
    ...
    def test_small_integration_test(self):
        """
        For comparision run:
        >>> import udop  # pip install udop==0.7.1
        >>> from udop.data.sentencepiece_vocabulary import SentencePieceVocabulary

        >>> path_to_mtf_small_udop_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> udop_model = udop.models.MtfModel(model_dir=path_to_mtf_small_udop_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = udop_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_udop_v1_1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> udop_model = udop.models.MtfModel(model_dir=path_to_mtf_small_udop_v1_1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = udop_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = UdopForConditionalGeneration.from_pretrained("google/udop-v1_1-small").to(torch_device)
        tokenizer = T5Tokenizer.from_pretrained("google/udop-v1_1-small")
    ...
    @slow
    def test_small_byudop_integration_test(self):
        """
        For comparision run:
        >>> import udop  # pip install udop==0.9.1

        >>> path_to_byudop_small_checkpoint = '<fill_in>'
        >>> udop_model = udop.models.MtfModel(model_dir=path_to_tf_checkpoint, batch_size=1, tpu=None)
        >>> vocab = udop.data.ByteVocabulary()
        >>> score = udop_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = UdopForConditionalGeneration.from_pretrained("google/bymicrosoft/udop-large").to(torch_device)
        tokenizer = ByT5Tokenizer.from_pretrained("google/bymicrosoft/udop-large")
    ...
    @slow
    def test_contrastive_search_udop(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        udop_tokenizer = AutoTokenizer.from_pretrained("flax-community/udop-base-cnn-dm")
        udop_model = UdopForConditionalGeneration.from_pretrained("flax-community/udop-base-cnn-dm").to(torch_device)
        input_ids = udop_tokenizer(
            article, add_special_tokens=False, truncation=True, max_length=512, return_tensors="pt"
        ).input_ids.to(torch_device)

        outputs = udop_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = udop_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
@require_torch
class TestAsymmetricUdop(unittest.TestCase):
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to UdopConfig as num_layers
