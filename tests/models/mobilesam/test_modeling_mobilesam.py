...
""" Testing suite for the PyTorch MOBILESAM model. """
...
from transformers import MobileSamConfig, MobileSamMaskDecoderConfig, MobileSamPromptEncoderConfig, MobileSamVisionConfig, pipeline
...
if is_torch_available():
    ...
    from transformers import MobileSamModel, SamProcessor
    from transformers.models.mobilesam.modeling_mobilesam import MOBILESAM_PRETRAINED_MODEL_ARCHIVE_LIST
...
class MobileSamPromptEncoderTester:
    def __init__(
        self,
        hidden_size=32,
        input_image_size=24,
    ...
    def get_config(self):
        return MobileSamPromptEncoderConfig(
...
class MobileSamMaskDecoderTester:
    def __init__(
        self,
        hidden_size=32,
        hidden_act="relu",
    ...
    def get_config(self):
        return MobileSamMaskDecoderConfig(
            hidden_size=self.hidden_size,
            hidden_act=self.hidden_act,
            mlp_dim=self.mlp_dim,
            num_hidden_layers=self.num_hidden_layers,
            num_attention_heads=self.num_attention_heads,
            attention_downmobilesample_rate=self.attention_downmobilesample_rate,
...
class MobileSamModelTester:
    def __init__(
        ...
        self.prompt_encoder_tester = MobileSamPromptEncoderTester()
        self.mask_decoder_tester = MobileSamMaskDecoderTester()
    ...
    def create_and_check_model(self, config, pixel_values):
        model = MobileSamModel(config=config)
...
@require_torch
class MobileSamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as MOBILESAM's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (MobileSamModel,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    @unittest.skip(reason="MOBILESAM's vision encoder does not use inputs_embeds")
    ...
    @unittest.skip(reason="MobileSamModel does not support training")
    ...
    @unittest.skip(reason="MobileSamModel has no base class and is not available in MODEL_MAPPING")
    ...
    def test_model_from_pretrained(self):
        for model_name in MOBILESAM_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = MobileSamModel.from_pretrained(model_name)
...
def prepare_dog_img():
    img_url = "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/transformers/model_doc/dog-mobilesam.png"
    raw_image = Image.open(requests.get(img_url, stream=True).raw).convert("RGB")
    return raw_image


@slow
class MobileSamModelIntegrationTest(unittest.TestCase):
    def tearDown(self):
        super().tearDown()
    ...
    def test_inference_mask_generation_no_point(self):
        model = MobileSamModel.from_pretrained("facebook/mobilesam-vit-base")
        processor = SamProcessor.from_pretrained("facebook/mobilesam-vit-base")
    ...
    def test_dummy_pipeline_generation(self):
        generator = pipeline("mask-generation", model="facebook/mobilesam-vit-base", device=torch_device)
        raw_image = prepare_image()
