...
# Short single-speaker sample (~45 s of Obama's farewell address), used to exercise chunked streaming. The
# NeMo streaming reproducer (`reproducer_streaming_rnnt.py`) runs the full ~16 min `obama.mp3` as a reference,
# but the integration test only needs a short clip to validate the streaming pipeline.
OBAMA_45S_AUDIO_URL = (
...
class Nemotron3_5AsrForRNNTIntegrationTest(unittest.TestCase):
    ...
    Expected values are loaded from the `fixtures/nemotron3_5_asr/expected_results_*.json` fixtures and
    regenerated by the reproducers in the gist linked from each test below. Inference runs in float32 to
    track the NeMo reference as closely as possible.

    - Offline single/batched `generate` uses the model's default attention context `[56, 3]` (the first
      entry of `att_context_size`, the 320 ms setting), conditioned on `language="en-US"`. The expected
      transcriptions are the NeMo reference outputs of `nvidia/nemotron-3.5-asr-streaming-0.6b`, which the HF
      re-implementation matches exactly.
    - Streaming feeds per-chunk mel features through a cache-aware `generate` at `num_lookahead_tokens=6`
      (att_context `[56, 6]`, the 560 ms setting), the transcript consumed via a `TextIteratorStreamer`. As
      with NemotronAsr, cache-aware streaming accumulates ~1e-3 encoder drift, so the expected value is the
      HF model's own streamed transcript, sanity-checked against the NeMo reference.
    """
    ...
    def test_rnnt_model_integration_streaming(self):
        ...
        audio = load_audio(OBAMA_45S_AUDIO_URL, sampling_rate=sampling_rate)
