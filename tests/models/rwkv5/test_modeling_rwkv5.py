...
class Rwkv5ModelTester:
    def __init__(
        ...
        self.scope = scope
        self.test_resize_embeddings = test_resize_embeddings
        self.bos_token_id = 0
        self.eos_token_id = 0
        self.pad_token_id = 0

    def get_large_model_config(self):
        return Rwkv5Config.from_pretrained("RWKV/rwkv-5-world-1b5")
...
@require_torch
class Rwkv5ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Rwkv5Model, Rwkv5ForCausalLM) if is_torch_available() else ()
    pipeline_model_mapping = (
        {"feature-extraction": Rwkv5Model, "text-generation": Rwkv5ForCausalLM} if is_torch_available() else {}
    )
    # all_generative_model_classes = (Rwkv5ForCausalLM,) if is_torch_available() else ()
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = Rwkv5ModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=Rwkv5Config, n_embd=37, common_properties=["hidden_size", "num_hidden_layers"]
        )
            ...
            for name, param in model.named_parameters():
                if "time_decay" in name:
                    if param.requires_grad:
                        self.assertTrue(param.data.max().item() == -1.0)
                        self.assertTrue(param.data.min().item() == -6.0)
...
class RWKVIntegrationTests(unittest.TestCase):
    ...
    def test_logits_3b(self):
        ...
        torch.testing.assert_close(outputs.logits[0, 0, :40].cpu(), EXPECTED_LOGIT_SLICE)
        torch.testing.assert_close(outputs.state[0][0, -40: , 0].cpu(), EXPECTED_STATE_SLICE)
