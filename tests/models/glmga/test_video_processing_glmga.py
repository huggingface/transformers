...
if is_vision_available():
    if is_torchvision_available():
        from transformers import GlmgaVideoProcessor
        from transformers.models.glm46v.video_processing_glm46v import smart_resize


class GlmgaVideoProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=5,
    ...
    def prepare_video_metadata(self, videos):
        ...
        for video in videos:
            ...
            # Glmga samples frames at fps=1, so use a 1-fps, full-length metadata here to keep
...
@require_torch
@require_vision
class GlmgaVideoProcessingTest(VideoProcessingTestMixin, unittest.TestCase):
    fast_video_processing_class = GlmgaVideoProcessor if is_torchvision_available() else None
    input_name = "pixel_values_videos"

    def setUp(self):
        super().setUp()
        self.video_processor_tester = GlmgaVideoProcessingTester(self)
