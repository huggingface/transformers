...
"""Testing suite for the PyTorch GLM-GA model."""
...
class GlmgaVisionText2TextModelTester:
    def __init__(
        self,
        parent,
        batch_size=3,
...
@require_torch
class GlmgaModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (GlmgaModel, GlmgaForConditionalGeneration) if is_torch_available() else ()

    model_split_percents = [0.7, 0.9]  # model too big to split at 0.5
    _is_composite = True

    def setUp(self):
        self.model_tester = GlmgaVisionText2TextModelTester(self)
        self.config_tester = ConfigTester(self, config_class=GlmgaConfig, has_text_modality=False)
...
@require_torch
class GlmgaIntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_small_model_integration_test(self):
        model = GlmgaForConditionalGeneration.from_pretrained(
