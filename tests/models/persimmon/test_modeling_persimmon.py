...
class PersimmonIntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        ...
        model = PersimmonForCausalLM.from_pretrained("meta-persimmon/Persimmon-2-7b-hf", device_map="auto")
    ...
    def test_model_13b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = PersimmonForCausalLM.from_pretrained("adept/persimmon-8b", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_13bf_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = PersimmonForCausalLM.from_pretrained("meta-persimmon/Persimmon-2-13b-chat-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_70b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = PersimmonForCausalLM.from_pretrained("meta-persimmon/Persimmon-2-70b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
    ...
    def test_model_13b_greedy_generation(self):
        ...
        tokenizer = PersimmonTokenizer.from_pretrained("meta-persimmon/Persimmon-2-13b-chat-hf")
        input_ids = tokenizer.encode(prompt, return_tensors="pt")
        model = PersimmonForCausalLM.from_pretrained(
            "meta-persimmon/Persimmon-2-13b-chat-hf", device_map="sequential", use_safetensors=False
        )
...
@require_torch
class CodePersimmonIntegrationTest(unittest.TestCase):
    PROMPTS = [
...
<FILL_ME>
    ```
This downloads the Persimmon inference code and installs the repository as a local pip package.
""",
...
""",
    ...
    @slow
    def test_model_7b_logits(self):
        model = PersimmonForCausalLM.from_pretrained("codepersimmon/CodePersimmon-7b-hf").to(torch_device)
        tokenizer = CodePersimmonTokenizer.from_pretrained("codepersimmon/CodePersimmon-7b-hf")
