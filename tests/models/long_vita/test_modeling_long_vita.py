...
class Long_vitaVisionText2TextModelTester:
    def __init__(
        self,
        parent,
        batch_size=3,
    ...
    def create_and_check_long_vita_model_fp16_forward(
        self, config, input_ids, pixel_values, attention_mask, image_grid_thw
    ):
        model = Long_vitaForConditionalGeneration(config=config)
    ...
    def create_and_check_long_vita_model_fp16_autocast_forward(
        self, config, input_ids, pixel_values, attention_mask, image_grid_thw
    ):
...
@require_torch
class Long_vitaModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    """
    Model tester for `Long_vitaForConditionalGeneration`.
    """

    all_model_classes = (Long_vitaForConditionalGeneration,) if is_torch_available() else ()
    test_pruning = False
    test_head_masking = False

    def setUp(self):
        self.model_tester = Long_vitaVisionText2TextModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Long_vitaConfig, has_text_modality=False)
    ...
    @unittest.skip(reason="Compile not yet supported because in Long_vita models")
...
@require_torch
class Long_vitaIntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_small_model_integration_test(self):
        model = Long_vitaForConditionalGeneration.from_pretrained(
