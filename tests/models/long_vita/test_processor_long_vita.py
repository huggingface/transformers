...
from transformers import AutoProcessor, Long_vitaTokenizer
...
if is_vision_available():
    from transformers import Long_vitaProcessor, Long_vitaImageProcessor


@require_vision
@require_torch
class Long_vitaProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    processor_class = Long_vitaProcessor

    def setUp(self):
        self.tmpdirname = tempfile.mkdtemp()
        processor = Long_vitaProcessor.from_pretrained("Qwen/Qwen2-VL-7B-Instruct", patch_size=4)
    ...
    def test_save_load_pretrained_default(self):
        ...
        processor = Long_vitaProcessor(tokenizer=tokenizer, image_processor=image_processor)
        processor.save_pretrained(self.tmpdirname)
        processor = Long_vitaProcessor.from_pretrained(self.tmpdirname, use_fast=False)

        self.assertEqual(processor.tokenizer.get_vocab(), tokenizer.get_vocab())
        self.assertEqual(processor.image_processor.to_json_string(), image_processor.to_json_string())
        self.assertIsInstance(processor.tokenizer, Long_vitaTokenizer)
        self.assertIsInstance(processor.image_processor, Long_vitaImageProcessor)
