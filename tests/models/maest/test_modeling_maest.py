...
""" Testing suite for the PyTorch MAEST (MAEST) model. """
...
class MAESTModelTester:
    def __init__(
        ...
        # in MAEST, the seq length equals the number of patches + 2 (we add 2 for the [CLS] and distillation tokens)
...
@require_torch
@require_torchaudio
class MAESTModelIntegrationTest(unittest.TestCase):
    @cached_property
    def default_feature_extractor(self):
        return (
            MAESTFeatureExtractor.from_pretrained("mtg-upf/discogs-maest-30s-pw-129e")
            if is_torchaudio_available()
            else None
        )
    ...
    def test_inference_audio_classification(self):
        ...
        expected_slice = torch.tensor([-9.9254, -8.0450, -9.8166]).to(torch_device)
