...
class MusicgenMelodyIntegrationTests(unittest.TestCase):
    ...
    def test_logits_text_audio_prompt(self):
        ...
        self.assertTrue(
            torch.allclose(logits[1:3, -1, 32:40].cpu(), EXPECTED_LOGITS, atol=1e-4)
        )
...
class MusicgenMelodyStereoIntegrationTests(unittest.TestCase):
    ...
    def test_generate_unconditional_greedy(self):
        ...
        self.assertTrue(
            torch.allclose(
                output_values[0, 0, :16].cpu(), EXPECTED_VALUES_LEFT, atol=6e-4
            )
        )
        self.assertTrue(
            torch.allclose(
                output_values[0, 1, :16].cpu(), EXPECTED_VALUES_LEFT, atol=6e-4
            )
        )
