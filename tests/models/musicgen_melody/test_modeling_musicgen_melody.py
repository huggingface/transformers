...
class MusicgenMelodyIntegrationTests(unittest.TestCase):
    ...
    def test_generate_unconditional_greedy(self):
        ...
        EXPECTED_VALUES = torch.tensor(
            [
                -0.0684, -0.0732, -0.0672, -0.0689, -0.0725, -0.0720, -0.0722, -0.0752,
                -0.0641, -0.0565, -0.0514, -0.0558, -0.0753, -0.0917, -0.0877, -0.0628,
            ]
        )
        # fmt: on
    ...
    def test_generate_unconditional_sampling(self):
        ...
        EXPECTED_VALUES = torch.tensor(
            [
                0.0293,  0.0292,  0.0398,  0.0445,  0.0445,  0.0465,  0.0457,  0.0392,
                0.0431,  0.0381,  0.0274,  0.0037, -0.0370, -0.0772, -0.1020, -0.1047,
            ]
        )
        # fmt: on
    ...
    def test_generate_text_prompt_greedy(self):
        ...
        EXPECTED_VALUES = torch.tensor(
            [
                -0.0684, -0.0732, -0.0672, -0.0689, -0.0725,
                -0.0720, -0.0722, -0.0752, -0.0641, -0.0565
            ]
        )
        # fmt: on
        ...
        expectations = Expectations(
            {
                ...
                ("cuda", 8): [0.0293, 0.0292, 0.0398, 0.0445, 0.0445, 0.0465, 0.0457, 0.0392, 0.0431, 0.0381, 0.0274, 0.0037, -0.0370, -0.0772, -0.1020, -0.1047],
            }
        )
...
class MusicgenMelodyStereoIntegrationTests(unittest.TestCase):
    ...
    def test_generate_unconditional_greedy(self):
        ...
        EXPECTED_VALUES_LEFT = torch.tensor(
            [
                -0.0005, -0.0041,  0.0024,  0.0092,  0.0139,  0.0225,  0.0305,  0.0353,
                 0.0490,  0.0582,  0.0674,  0.0686,  0.0555,  0.0437,  0.0434,  0.0544
            ]
        )
        # fmt: on
