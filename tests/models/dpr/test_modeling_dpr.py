...
class DPRModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {"feature-extraction": DPRQuestionEncoder} if is_torch_available() else {}
    )
...
class DPRModelIntegrationTest(unittest.TestCase):
    ...
    def test_reader_inference(self):
        ...
        torch.testing.assert_close(
            outputs.start_logits[:, :10], expected_start_logits, rtol=1e-4, atol=1e-4
        )
        torch.testing.assert_close(
            outputs.end_logits[:, :10], expected_end_logits, rtol=1e-4, atol=1e-4
        )
