...
#         if return_labels:
#             if model_class.__name__ == "OmDetTurboForObjectDetection":
#                 labels = []
...
#     model_class = OmDetTurboForObjectDetection
#     config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
#     config.return_dict = True
...
#     # OmDetTurbo when sharing weights also uses the shared ones in OmDetTurboDecoder
#     # Therefore, differently from DeformableDetr, we expect the group lens to be 2
#     # one for self.bbox_embed in OmDetTurboForObejectDetection and another one
#     # in the decoder
...
class OmDetTurboModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_class_logits = torch.tensor([[[0.9624, -3.5492], [0.5973, -2.4723], [-3.0351, 1.1316]]]).to(
            torch_device
        )
        expected_coord_logits = torch.tensor(
            [[[0.7694, 0.4108, 0.4603, 0.7279], [0.2564, 0.5512, 0.4731, 0.8781], [0.1690, 0.1971, 0.2135, 0.0970]]]
        ).to(torch_device)

        self.assertTrue(torch.allclose(outputs.decoder_class_logits[:3, :3], expected_class_logits, atol=1e-3))
        self.assertTrue(torch.allclose(outputs.decoder_coord_logits[:3, :3], expected_coord_logits, atol=1e-4))

        # verify grounded postprocessing
        results = processor.post_process_grounded_object_detection(
            outputs, labels_names=[labels], target_sizes=[image.size]
        )[0]
