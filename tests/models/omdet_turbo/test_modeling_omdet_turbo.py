...
class OmDetTurboModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        encoding = processor(
            images=image, text=classes, task=task, return_tensors="pt"
        ).to(torch_device)
        ...
        self.assertTrue(
            torch.allclose(
                outputs.decoder_class_logits[:3, :3], expected_class_logits, atol=1e-1
            )
        )
        self.assertTrue(
            torch.allclose(
                outputs.decoder_coord_logits[:3, :3], expected_coord_logits, atol=1e-3
            )
        )
    ...
    def test_inference_object_detection_head_fp16(self):
        ...
        self.assertTrue(torch.allclose(results["scores"], expected_scores, atol=1e-2))
        self.assertTrue(
            torch.allclose(results["boxes"][0, :], expected_slice_boxes, atol=1e-1)
        )
    ...
    def test_inference_object_detection_head_no_task(self):
        ...
        encoding = processor(images=image, text=classes, return_tensors="pt").to(
            torch_device
        )
    ...
    def test_inference_object_detection_head_batched(self):
        ...
        self.assertListEqual(
            [result["classes"] for result in results], expected_classes
        )
    ...
    def test_inference_object_detection_head_equivalence_cpu_gpu(self):
        ...
        self.assertTrue(
            torch.allclose(results_cpu["scores"], result_gpu["scores"].cpu(), atol=1e-2)
        )
        self.assertTrue(
            torch.allclose(
                results_cpu["boxes"][0, :], result_gpu["boxes"][0, :].cpu(), atol=1e-2
            )
        )
