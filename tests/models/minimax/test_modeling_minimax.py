...
class MiniMaxModelTest(CausalLMModelTest, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        model = MiniMaxModel(config).to(torch_device).eval()
        ...
        for layer_type, attentions in zip(config.layer_types, outputs.attentions):
            if layer_type == "full_attention":
                self.assertEqual(attentions.shape[-3:], (config.num_attention_heads, seq_len, seq_len))
            else:
                self.assertEqual(attentions.shape[-3:], (config.num_attention_heads, head_dim, head_dim))
