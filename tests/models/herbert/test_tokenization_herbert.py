...
class HerbertTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    integration_expected_text_from_tokens = 'This is a test <unk>I was born in 92000 , and this is falsé . <unk><unk><unk><unk><unk>是 Hi Hello Hi Hello Hello <s>hi <s>there The following string should be properly encoded : Hello . But ird and <unk>ี ird ด Hey how are you doing'
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        from_pretrained_id =  "allegro/herbert-base-cased"

        tokenizer = HerbertTokenizer.from_pretrained(from_pretrained_id)
        
        cls.vocab = load_vocab(tokenizer.vocab_file)
        cls.merges = load_merges(tokenizer.merges_file)

        tokenizer.save_pretrained(cls.tmpdirname)
        tokenizer_from_vocab = HerbertTokenizer(vocab=cls.vocab, merges=cls.merges)
