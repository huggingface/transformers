...
class Xcodec2ModelTester:
    ...
    def prepare_config_and_inputs(self):
        audio_values = floats_tensor([self.batch_size, self.num_channels, self.num_samples], scale=1.0)
        input_features = floats_tensor(
            [self.batch_size, self.num_samples // self.mel_hop_length, self.num_mel_bins * self.stride], scale=1.0
        )
        config = self.get_config()
        inputs_dict = {"audio_values": audio_values, "input_features": input_features}
    ...
    def create_and_check_model_forward(self, config, inputs_dict):
        ...
        result = model(audio_values, input_features)
...
class Xcodec2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_forward_signature(self):
        ...
        for model_class in self.all_model_classes:
            ...
            expected_arg_names = ["audio_values", "input_features", "padding_mask"]
...
class Xcodec2IntegrationTest(unittest.TestCase):
    ...
    def test_integration(self):
        ...
        with torch.no_grad():
            audio_codes = model.encode(inputs["audio_values"], inputs["input_features"], return_dict=False)[0]
            ...
            # compare codec error
            codec_error = compute_rmse(inputs["audio_values"], dec).item()
            torch.testing.assert_close(codec_error, exp_codec_error, rtol=1e-5, atol=1e-5)

            # make sure forward and decode gives same result
            enc_dec = model(inputs["audio_values"], inputs["input_features"]).audio_values
    ...
    def test_batch_integration(self):
        ...
        for i in range(num_samples):
            ...
            codec_error = compute_rmse(inputs["audio_values"][i : i + 1], dec[i : i + 1]).item()
