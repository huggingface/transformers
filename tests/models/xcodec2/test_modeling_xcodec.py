...
from transformers import AutoProcessor, XCodecConfig
...
if is_torch_available():
    ...
    from transformers import EncodecFeatureExtractor, XCodecModel
...
@require_torch
class XCodecModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(reason="The XCodecModel is not transformers based, thus it does not have `inputs_embeds` logics")
    ...
    @unittest.skip(
        reason="The XCodecModel is not transformers based, thus it does not have the usual `attention` logic"
    )
    ...
    @unittest.skip(
        reason="The XCodecModel is not transformers based, thus it does not have the usual `hidden_states` logic"
    )
...
class XCodecIntegrationTest(unittest.TestCase):
    def test_integration_24kHz(self):
        ...
        model_id = "facebook/xcodec_24khz"

        model = XCodecModel.from_pretrained(model_id).to(torch_device)
    ...
    def test_integration_48kHz(self):
        ...
        librispeech_dummy = load_dataset("hf-internal-testing/librispeech_asr_dummy", "clean", split="validation")
        model_id = "facebook/xcodec_48khz"
