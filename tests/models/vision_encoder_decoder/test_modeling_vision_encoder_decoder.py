...
from transformers.testing_utils import (require_levenshtein, require_nltk,
                                        require_sentencepiece, require_torch,
                                        require_torch_sdpa, require_vision,
                                        slow, to_2tuple, torch_device)
...
if is_torch_available():
    ...
    from transformers import (AutoTokenizer, BartForCausalLM, BertLMHeadModel,
                              DeiTModel, DonutSwinModel, GPT2LMHeadModel,
                              LayoutLMv3Model, SwinModel, TrOCRForCausalLM,
...
class Swin2BartModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason="There are no published pretrained BART-causal checkpoints for now"
    )
    def test_real_model_save_load_from_pretrained(self):
        pass
...
class LayoutLMv32TrOCR(EncoderDecoderMixin, unittest.TestCase):
    ...
    def prepare_config_and_inputs(self):
        model_tester_encoder = LayoutLMv3ModelTester(
            self, batch_size=13, image_size=4, patch_size=2
        )
        model_tester_decoder = TrOCRStandaloneDecoderModelTester(
            self, batch_size=13, d_model=32, max_position_embeddings=512
        )
    ...
    def check_encoder_decoder_model_output_attentions(
        ...
        **kwargs,
        ...
        image_seq_length = (
            encoder_model.config.input_size // encoder_model.config.patch_size
        ) ** 2 + 1
...
class VIT2GPT2Test(EncoderDecoderMixin, unittest.TestCase):
    ...
    def prepare_config_and_inputs(self):
        model_tester_encoder = ViTModelTester(self, batch_size=13)
        model_tester_decoder = GPT2ModelTester(
            self, batch_size=13, hidden_size=32, max_position_embeddings=512
        )
        encoder_config_and_inputs = model_tester_encoder.prepare_config_and_inputs()
        decoder_config_and_inputs = model_tester_decoder.prepare_config_and_inputs()
    ...
    def check_encoder_decoder_model_output_attentions(
        ...
        **kwargs,
        ...
        seq_len = (
            encoder_model.config.image_size // encoder_model.config.patch_size
        ) ** 2 + 1
...
class TrOCRModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_handwritten(self):
        ...
        dataset = load_dataset(
            "hf-internal-testing/fixtures_ocr", split="test", trust_remote_code=True
        )
        ...
        torch.testing.assert_close(
            logits[0, 0, :10], expected_slice, rtol=1e-4, atol=1e-4
        )
...
class ViT2GPT2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_coco_en(self):
        ...
        pixel_values = image_processor(images=img, return_tensors="pt").pixel_values.to(
            torch_device
        )
...
class DonutModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_docvqa(self):
        ...
        torch.testing.assert_close(
            logits[0, 0, :3], expected_slice, rtol=1e-4, atol=1e-4
        )
...
class NougatModelIntegrationTest(unittest.TestCase):
    ...
    def default_model(self):
        return VisionEncoderDecoderModel.from_pretrained("facebook/nougat-base").to(
            torch_device
        )
    ...
    def test_generation(self):
        ...
        # verify generated sequence
        generated = processor.batch_decode(outputs.sequences, skip_special_tokens=True)[
            0
        ]
