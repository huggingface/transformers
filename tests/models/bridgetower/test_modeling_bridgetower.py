...
from transformers import (BridgeTowerConfig, BridgeTowerTextConfig,
                          BridgeTowerVisionConfig, is_torch_available,
                          is_vision_available)
...
class BridgeTowerModelTrainingTest(unittest.TestCase):
    ...
    def test_inference_interpolate_pos_encoding(self):
        ...
        image_processor = BridgeTowerProcessor.from_pretrained(
            model_name, size={"shortest_edge": 180}
        )
        ...
        expected_slice = torch.tensor(
            [
                [-0.6518, 0.4978, -0.4544],
                [-2.6672, -0.0843, -0.4210],
                [-2.4510, -0.1002, -0.3458],
            ]
        ).to(torch_device)

        torch.testing.assert_close(
            outputs.image_features[0, :3, :3], expected_slice, rtol=1e-4, atol=1e-4
        )
