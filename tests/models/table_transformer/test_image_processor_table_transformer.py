...
class TableTransformerImageProcessingTester(unittest.TestCase):
    def __init__(
        self,
        parent,
        batch_size=7,
    ...
    def get_expected_values(self, image_inputs, batched=False):
        """
        This function computes the expected height and width when providing images to TableTransformerImageProcessor,
...
@require_torch
@require_vision
class TableTransformerImageProcessingTest(AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase):
    image_processing_class = TableTransformerImageProcessor if is_vision_available() else None

    def setUp(self):
        self.image_processor_tester = TableTransformerImageProcessingTester(self)
    ...
    def test_valid_coco_detection_annotations(self):
        ...
        # encode them
        image_processing = TableTransformerImageProcessor.from_pretrained("microsoft/table-transformer-detection")
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        image_processing = TableTransformerImageProcessor()
    ...
    def test_batched_coco_panoptic_annotations(self):
        ...
        # encode them
        image_processing = TableTransformerImageProcessor(format="coco_panoptic")
        encoding = image_processing(
            images=images,
            annotations=annotations,
        ...
        # Check the pixel values have been padded
        postprocessed_height, postprocessed_width = 800, 800
        expected_shape = torch.Size([2, 3, postprocessed_height, postprocessed_width])
        self.assertEqual(encoding["pixel_values"].shape, expected_shape)
        ...
        expected_boxes_0 = torch.tensor(
            [
                [0.2625, 0.4078, 0.4688, 0.6469],
                [0.7719, 0.3078, 0.4531, 0.5344],
                [0.5000, 0.3695, 0.9969, 0.7391],
                [0.1688, 0.1500, 0.2062, 0.0688],
                [0.5492, 0.2070, 0.0578, 0.1641],
                [0.4992, 0.3742, 0.9984, 0.7484],
            ]
        )
        expected_boxes_1 = torch.tensor(
            [
                [0.2100, 0.3262, 0.3750, 0.5175],
                [0.6175, 0.2463, 0.3625, 0.4275],
                [0.4000, 0.2956, 0.7975, 0.5913],
                [0.1350, 0.1200, 0.1650, 0.0550],
                [0.4394, 0.1656, 0.0463, 0.1312],
                [0.3994, 0.2994, 0.7987, 0.5987],
            ]
        )
        ...
        # Check the masks have also been padded
        self.assertEqual(encoding["labels"][0]["masks"].shape, torch.Size([6, 800, 800]))
        self.assertEqual(encoding["labels"][1]["masks"].shape, torch.Size([6, 800, 800]))
