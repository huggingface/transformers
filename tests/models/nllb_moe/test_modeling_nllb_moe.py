...
class NllbMoeEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def get_large_model_config(self):
        return NllbMoeConfig.from_pretrained("switch_base_8")
    ...
    def create_and_check_model(self, config, input_ids, attention_mask):
        model = NllbMoeEncoderModel(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids=input_ids,
    ...
    def create_and_check_model_fp16_forward(self, config, input_ids, attention_mask):
        model = NllbMoeEncoderModel(config=config).to(torch_device).half().eval()
...
class NllbMoeEncoderOnlyModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (NllbMoeEncoderModel,) if is_torch_available() else ()
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = NllbMoeEncoderOnlyModelTester(self)
        self.config_tester = ConfigTester(self, config_class=NllbMoeConfig, d_model=37)
