...
class LocateAnythingIntegrationTest(unittest.TestCase):
    ...
    # Prompt built the way the model's documented worker does (py_apply_chat_template): a system prompt followed by
    # the `<image-1>` placeholder. Greedy `slow`/auto-regressive decoding is the path native `generate` reproduces.
    prompt = (
        "<|im_start|>system\nYou are a helpful assistant.\n<|im_end|>\n"
        f"<|im_start|>user\n<image-1>{question}<|im_end|>\n<|im_start|>assistant\n"
    )
    # Decoded grounding output of the original implementation (slow greedy), captured on the same image/prompt.
    expected_output = "<ref>cat</ref><box><0><115><494><988></box><|im_end|>"
    ...
    def test_detection_generation(self):
        model = LocateAnythingForConditionalGeneration.from_pretrained(self.model_id, dtype=torch.float32)
        ...
        decoded = self.processor.decode(generated[0, inputs["input_ids"].shape[-1] :], skip_special_tokens=False)
        self.assertEqual(decoded, self.expected_output)
