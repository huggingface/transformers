...
if is_torch_available():
    ...
    from transformers import SamForConditionalGeneration, SamModel, SamVisionModel
...
class SamVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_model_from_pretrained(self):
        for model_name in SAM_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = SamVisionModel.from_pretrained(model_name)
...
class SamForConditionalGenerationDecoderOnlyModelTester:
    def __init__(
    ...
    def create_and_check_for_conditional_generation(self, config, input_ids, attention_mask, pixel_values):
        model = SamForConditionalGeneration(config).to(torch_device).eval()
        with torch.no_grad():
...
@require_torch
class SamForConditionalGenerationDecoderOnlyTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (SamForConditionalGeneration,) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def setUp(self):
        self.model_tester = SamForConditionalGenerationDecoderOnlyModelTester(self)
    ...
    @unittest.skip(reason="SamModel does not have input/output embeddings")
    def test_model_common_attributes(self):
        pass

    @unittest.skip(reason="There's no base SamModel")
    def test_save_load_fast_init_from_base(self):
        pass
    ...
    def test_load_vision_qformer_text_config(self):
        ...
        # Save SamConfig and check if we can load SamVisionConfig from it
    ...
    @slow
    def test_model_from_pretrained(self):
        for model_name in SAM_PRETRAINED_MODEL_ARCHIVE_LIST:
            model = SamForConditionalGeneration.from_pretrained(model_name)
            self.assertIsNotNone(model)


# this class is based on `T5ModelTester` found in tests/models/t5/test_modeling_t5.py
class SamTextModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
...
class SamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (SamForConditionalGeneration, SamModel) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def test_get_text_features(self):
        ...
        model = SamModel(config).to(torch_device)
        model.eval()
...
class SamModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_t5(self):
        processor = SamProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl")
    ...
    def test_inference_opt_multi_gpu(self):
        processor = SamProcessor.from_pretrained("facebook/sam-vit-h")
        model = SamForConditionalGeneration.from_pretrained(
            "facebook/sam-vit-h", torch_dtype=torch.float16, device_map="balanced"
        )
