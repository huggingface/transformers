...
class BLIPVisionModelTester:
    ...
    def create_and_check_model_with_projection(self, config, pixel_values):
        model = BLIPVisionModelWithProjection(config=config)
        model.to(torch_device)
        model.eval()
        with torch.no_grad():
            result = model(pixel_values)
...
class BLIPVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (BLIPVisionModel, BLIPVisionModelWithProjection) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def test_model_with_projection_from_pretrained(self):
        for model_name in BLIP_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = BLIPVisionModelWithProjection.from_pretrained(model_name)
            self.assertIsNotNone(model)
...
class BLIPTextModelTester:
    ...
    def create_and_check_model_with_projection(self, config, input_ids, input_mask):
        model = BLIPTextModelWithProjection(config=config)
        model.to(torch_device)
        model.eval()
        with torch.no_grad():
            result = model(input_ids, attention_mask=input_mask)
...
class BLIPTextModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (BLIPTextModel, BLIPTextModelWithProjection) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    test_head_masking = False
    ...
    def test_model_with_projection_from_pretrained(self):
        for model_name in BLIP_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = BLIPTextModelWithProjection.from_pretrained(model_name)
            self.assertIsNotNone(model)
...
class BLIPModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    # overwrite from common since FlaxBLIPModel returns nested output
