...
if is_torch_available():
    ...
    from transformers import LevitForImageClassification, LevitForMaskedImageModeling, LevitModel
...
class LevitModelTester:
    def __init__(
        ...
        # in LeViT, the seq length equals the number of patches + 1 (we add 1 for the [CLS] token)
...
class LevitModelIntegrationTest(unittest.TestCase):
    ...
    def default_feature_extractor(self):
        return LevitFeatureExtractor.from_pretrained("anugunj/levit-128S") if is_vision_available() else None

    @slow
    def test_inference_image_classification_head(self):
        model = LevitForImageClassification.from_pretrained("anugunj/levit-128S").to(torch_device)
