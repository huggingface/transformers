...
if is_torch_available():
    ...
    from transformers import (MixtralForCausalLM, MixtralForQuestionAnswering,
                              MixtralForSequenceClassification,
                              MixtralForTokenClassification, MixtralModel)
...
class MixtralModelTest(
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=MixtralConfig, hidden_size=37
        )
    ...
    @unittest.skip(
        reason="Mixtral buffers include complex numbers, which breaks this test"
    )
    def test_save_load_fast_init_from_base(self):
        pass

    @unittest.skip(
        reason="Mixtral uses GQA on all models so the KV cache is a non standard format"
    )
...
class MixtralIntegrationTest(unittest.TestCase):
    ...
    def test_small_model_logits(self):
        ...
        EXPECTED_LOGITS = {
            7: torch.Tensor(
                [
                    [0.1670, 0.1620, 0.6094],
                    [-0.8906, -0.1588, -0.6060],
                    [0.1572, 0.1290, 0.7246],
                ]
            ).to(torch_device),
            8: torch.Tensor(
                [
                    [0.1631, 0.1621, 0.6094],
                    [-0.8906, -0.1621, -0.6094],
                    [0.1572, 0.1270, 0.7227],
                ]
            ).to(torch_device),
            9: torch.Tensor(
                [
                    [0.1641, 0.1621, 0.6094],
                    [-0.8906, -0.1631, -0.6094],
                    [0.1572, 0.1260, 0.7227],
                ]
            ).to(torch_device),
        }
        with torch.no_grad():
        ...
        torch.testing.assert_close(
            logits[0, :3, :3],
            EXPECTED_LOGITS[self.cuda_compute_capability_major_version],
            atol=1e-3,
            rtol=1e-3,
        )
        torch.testing.assert_close(
    ...
    def test_small_model_logits_batched(self):
        ...
        dummy_input = torch.LongTensor(
            [[0, 0, 0, 0, 0, 0, 1, 2, 3], [1, 1, 2, 3, 4, 5, 6, 7, 8]]
        ).to(torch_device)
        ...
        EXPECTED_LOGITS_LEFT = {
            ...
            8: torch.Tensor(
                [
                    [0.1914, 0.0508, 0.7188],
                    [0.1953, 0.0510, 0.7227],
                    [0.1973, 0.0562, 0.7148],
                ]
            ).to(torch_device),
            9: torch.Tensor(
                [
                    [0.1904, 0.0513, 0.7227],
                    [0.1943, 0.0518, 0.7227],
                    [0.1982, 0.0557, 0.7148],
                ]
            ).to(torch_device),
        }
        ...
        EXPECTED_LOGITS_LEFT_UNPADDED = {
            7: torch.Tensor(
                [
                    [0.2212, 0.5200, -0.3816],
                    [0.8213, -0.2313, 0.6069],
                    [0.2664, -0.7090, 0.2468],
                ],
            ).to(torch_device),
            8: torch.Tensor(
                [
                    [0.2217, 0.5195, -0.3828],
                    [0.8203, -0.2295, 0.6055],
                    [0.2676, -0.7109, 0.2461],
                ]
            ).to(torch_device),
            9: torch.Tensor(
                [
                    [0.2236, 0.5195, -0.3828],
                    [0.8203, -0.2285, 0.6055],
                    [0.2637, -0.7109, 0.2451],
                ]
            ).to(torch_device),
        }
        ...
        EXPECTED_LOGITS_RIGHT_UNPADDED = {
            7: torch.Tensor(
                [
                    [0.2205, 0.1232, -0.1611],
                    [-0.3484, 0.3030, -1.0312],
                    [0.0742, 0.7930, 0.7969],
                ]
            ).to(torch_device),
            8: torch.Tensor(
                [
                    [0.2178, 0.1260, -0.1621],
                    [-0.3496, 0.2988, -1.0312],
                    [0.0693, 0.7930, 0.8008],
                ]
            ).to(torch_device),
            9: torch.Tensor(
                [
                    [0.2197, 0.1250, -0.1611],
                    [-0.3516, 0.3008, -1.0312],
                    [0.0684, 0.7930, 0.8008],
                ]
            ).to(torch_device),
        }
        ...
        torch.testing.assert_close(
            logits[0, :3, :3],
            EXPECTED_LOGITS_LEFT[self.cuda_compute_capability_major_version],
            atol=1e-3,
            rtol=1e-3,
        )
        torch.testing.assert_close(
