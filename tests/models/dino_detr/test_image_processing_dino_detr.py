...
class DinoDetrImageProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=7,
    ...
    def get_expected_values(self, image_inputs, batched=False):
        """
        This function computes the expected height and width when providing images to DinoDetrImageProcessor,
...
@require_torch
@require_vision
class DinoDetrImageProcessingTest(
    AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase
):
    image_processing_class = DinoDetrImageProcessor if is_vision_available() else None

    def setUp(self):
        super().setUp()
        self.image_processor_tester = DinoDetrImageProcessingTester(self)
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            torch.testing.assert_close(
                encoding["labels"][0]["image_id"], expected_image_id
            )
            # verify is_crowd
            expected_is_crowd = torch.tensor([0, 0, 0, 0, 0, 0])
            torch.testing.assert_close(
                encoding["labels"][0]["iscrowd"], expected_is_crowd
            )
            # verify class_labels
            ...
            torch.testing.assert_close(
                encoding["labels"][0]["orig_size"], expected_orig_size
            )
            # verify size
    ...
    @slow
    # Copied from tests.models.detr.test_image_processing_detr.DetrImageProcessingTest.test_batched_coco_detection_annotations with Detr->DinoDetr
    def test_batched_coco_detection_annotations(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            expected_shape = torch.Size(
                [2, 3, postprocessed_height, postprocessed_width]
            )
            self.assertEqual(encoding["pixel_values"].shape, expected_shape)
            ...
            torch.testing.assert_close(
                encoding["labels"][0]["boxes"], expected_boxes_0, atol=1e-3, rtol=1e-3
            )
            torch.testing.assert_close(
                encoding["labels"][1]["boxes"], expected_boxes_1, atol=1e-3, rtol=1e-3
            )

            # Check the masks have also been padded
            self.assertEqual(
                encoding["labels"][0]["masks"].shape, torch.Size([6, 800, 1066])
            )
            self.assertEqual(
                encoding["labels"][1]["masks"].shape, torch.Size([6, 800, 1066])
            )
            ...
            torch.testing.assert_close(
                encoding["labels"][0]["boxes"], expected_boxes_0, atol=1, rtol=1
            )
            torch.testing.assert_close(
                encoding["labels"][1]["boxes"], expected_boxes_1, atol=1, rtol=1
            )

    # Copied from tests.models.detr.test_image_processing_detr.DetrImageProcessingTest.test_max_width_max_height_resizing_and_pad_strategy with Detr->DinoDetr
