...
class Fgclip2VisionModelTest(Fgclip2ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=Fgclip2VisionConfig, has_text_modality=False, hidden_size=37
        )
...
class Fgclip2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_en_inference(self):
        ...
        expected_logits = torch.tensor([[7.1313, -12.3647, -26.1456, -22.0568, -17.3236, -13.9281]], device=torch_device)
        ...
        expected_logits = torch.tensor([[1.5820, -14.4048, -23.1075, -16.6545, -16.3500, -21.9193]], device=torch_device)
        torch.testing.assert_close(logits_per_image, expected_logits, rtol=1e-3, atol=1e-3)
    ...
    def test_cn_inference(self):
        ...
        expected_logits = torch.tensor([[6.5309, -16.2254, -18.9228, -16.0596, -15.2771, -13.9292]], device=torch_device)
        torch.testing.assert_close(logits_per_image, expected_logits, rtol=1e-3, atol=1e-3)
        ...
        expected_logits = torch.tensor([[-0.3012, -15.8156, -21.3586, -13.2546, -15.2005, -23.6982]], device=torch_device)
        torch.testing.assert_close(logits_per_image, expected_logits, rtol=1e-3, atol=1e-3)
    ...
    def test_region_inference(self):
        ...
        bbox_features = self.model.get_image_region_features(**inputs, image_sizes=[[image_height,image_width]], region_infos=[[right_cat_bbox,left_remote_bbox]])
