...
@require_flax
class FlaxPegasusXModelTester:
    config_cls = PegasusXConfig
    config_updates = {}
    hidden_act = "gelu"
...
@require_flax
class FlaxPegasusXModelTest(FlaxModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            FlaxPegasusXForConditionalGeneration, 
            FlaxPegasusXForQuestionAnswering,
    ...
    all_generative_model_classes = (FlaxPegasusXForConditionalGeneration,) if is_flax_available() else ()
    ...
    def setUp(self):
        self.model_tester = FlaxPegasusXModelTester(self)
...
@require_tokenizers
@require_flax
class FlaxPegasusXModelIntegrationTest(unittest.TestCase):
    def test_inference_no_head(self):
        model = FlaxPegasusXModel.from_pretrained('pegasus-x-base')
    ...
    def test_inference_with_head(self):
        model = FlaxPegasusXForConditionalGeneration.from_pretrained('pegasus-x-base')
        # change to intended input here
        input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        decoder_input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        inputs_dict = prepare_pegasus_x_inputs_dict(model.config, input_ids, decoder_input_ids)
    ...
    def test_seq_to_seq_generation(self):
        hf = FlaxPegasusXForConditionalGeneration.from_pretrained('pegasus-x-base')
