...
@require_torch
class VibeVoiceSemanticTokenizerModelTester:
    def __init__(
        self,
        parent,
        batch_size=2,
    ...
    def get_config(self):
        return VibeVoiceSemanticTokenizerConfig(
...
@require_torch
class VibeVoiceSemanticTokenizerModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (VibeVoiceSemanticTokenizerModel,) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = VibeVoiceSemanticTokenizerModelTester(self)
    ...
    def test_forward_signature(self):
        ...
        for model_class in self.all_model_classes:
            ...
            expected_arg_names = ["audio", "padding_cache", "batch_mask", "use_cache"]
            self.assertListEqual(arg_names[: len(expected_arg_names)], expected_arg_names)

    @unittest.skip("VibeVoiceSemanticTokenizerModel does not have `inputs_embeds` logic")
    ...
    @unittest.skip("VibeVoiceSemanticTokenizerModel does not have the usual `attention` logic")
    ...
    @unittest.skip("VibeVoiceSemanticTokenizerModel does not have the usual `hidden_states` logic")
...
class VibeVoiceSemanticTokenizerIntegrationTest(unittest.TestCase):
    def setUp(self):
        self.model_checkpoint = "bezzam/VibeVoice-SemanticTokenizer"
    ...
    def test_batch_integration(self):
        """
        Reproducer which also generates expected outputs for acoustic tokenizer and main model:
        ...
        RESULTS_PATH = (
            Path(__file__).parent.parent.parent / "fixtures/vibevoice/expected_semantic_tokenizer_results.json"
        )
        ...
        with torch.no_grad():
            encoder_out = model.encode(processed_audio["input_features"]).latents
