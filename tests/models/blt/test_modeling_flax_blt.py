...
from transformers import BLTConfig, is_flax_available, is_tokenizers_available
...
@require_flax
class FlaxBLTModelTest(FlaxModelTesterMixin, unittest.TestCase):
    all_model_classes = (FlaxBLTModel, FlaxBLTForCausalLM) if is_flax_available() else ()

    def setUp(self):
        self.model_tester = FlaxBLTModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_class_name in self.all_model_classes:
            model = model_class_name.from_pretrained("openlm-research/open_blt_3b_v2", from_pt=True)
...
@slow
@require_flax
class FlaxBLTIntegrationTest(unittest.TestCase):
    def setUp(self):
        self.model_id = "openlm-research/open_blt_3b_v2"
        self.model = FlaxBLTForCausalLM.from_pretrained(self.model_id, from_pt=True)
    ...
    def test_generated_text(self):
        tokenizer = BLTTokenizerFast.from_pretrained(self.model_id)
