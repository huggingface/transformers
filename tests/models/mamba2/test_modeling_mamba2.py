...
class Mamba2ModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        # first forward pass
        # Attention: Mamba2 needs the cache to be initialized to return a cache!
...
class Mamba2ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skip("Mamba2 has its own special cache type")  # FIXME: @gante
    ...
    def test_flash_attn_2_fp32_ln(self):
        r"""
        Overriding the test_flash_attn_2_fp32_ln test as the Mamba2 model, like Mixtral, doesn't support
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                # NOTE: Mamba2 does not support right padding + use_cache with FA2.
    ...
    def test_flash_attn_2_generate_padding_right(self):
        r"""
        Overriding the test_flash_attn_2_generate_padding_right test as the Mamba2 model, like Mixtral, doesn't support
        right padding + use cache with FA2
        """
        import torch
    ...
    def test_flash_attn_2_generate_use_cache(self):
        r"""
        Overriding the test_flash_attn_2_generate_use_cache test as the Mamba2 model, like Mixtral, doesn't support
        right padding + use cache with FA2
        """
        import torch
    ...
    def test_flash_attn_2_inference_equivalence_right_padding(self):
        r"""
        Overriding the test_flash_attn_2_inference_padding_right test as the Mamba2 model, like Mixtral, doesn't support
        right padding + use cache with FA2
        """
        self.skipTest("Mamba2 flash attention does not support right padding")

    @unittest.skip("Mamba2 has its own special cache type")
...
@require_torch
class Mamba2ModelIntegrationTest(unittest.TestCase):
    model = None
    tokenizer = None

    @classmethod
    def setUpClass(cls):
        model_id = "ai21labs/Mamba2-tiny-random"
        cls.model = Mamba2ForCausalLM.from_pretrained(model_id, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
