                ...
                def recursive_check(tuple_object, dict_object):
                    if isinstance(tuple_object, DynamicCache):  # MODIFIED PART START
                        for idx in range(len(tuple_object)):
                            recursive_check(tuple_object.layers[idx].conv_states[0], dict_object.layers[idx].conv_states[0])
                            recursive_check(
                                tuple_object.layers[idx].recurrent_states[0], dict_object.layers[idx].recurrent_states[0]
                            )
