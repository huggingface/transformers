...
class ErnieMModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = ErnieMForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = ErnieMForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
...
class ErnieMModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (ErnieMForCausalLM,) if is_torch_available() else ()
...
class ErnieMModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = ErnieMForMaskedLM.from_pretrained("ernie-m")
