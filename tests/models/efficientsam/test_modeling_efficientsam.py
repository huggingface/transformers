...
""" Testing suite for the PyTorch EFFICIENTSAM model. """
...
from transformers import EfficientSamConfig, EfficientSamMaskDecoderConfig, EfficientSamPromptEncoderConfig, EfficientSamVisionConfig, pipeline
...
if is_torch_available():
    ...
    from transformers import EfficientSamModel, SamProcessor
    from transformers.models.efficientsam.modeling_efficientsam import EFFICIENTSAM_PRETRAINED_MODEL_ARCHIVE_LIST
...
class EfficientSamPromptEncoderTester:
    def __init__(
        self,
        hidden_size=32,
        input_image_size=24,
    ...
    def get_config(self):
        return EfficientSamPromptEncoderConfig(
...
class EfficientSamMaskDecoderTester:
    def __init__(
        self,
        hidden_size=32,
        hidden_act="relu",
    ...
    def get_config(self):
        return EfficientSamMaskDecoderConfig(
            hidden_size=self.hidden_size,
            hidden_act=self.hidden_act,
            mlp_dim=self.mlp_dim,
            num_hidden_layers=self.num_hidden_layers,
            num_attention_heads=self.num_attention_heads,
            attention_downefficientsample_rate=self.attention_downefficientsample_rate,
...
class EfficientSamModelTester:
    def __init__(
        ...
        self.prompt_encoder_tester = EfficientSamPromptEncoderTester()
        self.mask_decoder_tester = EfficientSamMaskDecoderTester()
    ...
    def create_and_check_model(self, config, pixel_values):
        model = EfficientSamModel(config=config)
...
@require_torch
class EfficientSamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as EFFICIENTSAM's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (EfficientSamModel,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    @unittest.skip(reason="EFFICIENTSAM's vision encoder does not use inputs_embeds")
    ...
    @unittest.skip(reason="EfficientSamModel does not support training")
    ...
    @unittest.skip(reason="EfficientSamModel has no base class and is not available in MODEL_MAPPING")
    ...
    def test_model_from_pretrained(self):
        for model_name in EFFICIENTSAM_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = EfficientSamModel.from_pretrained(model_name)
...
def prepare_dog_img():
    img_url = "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/transformers/model_doc/dog-efficientsam.png"
    raw_image = Image.open(requests.get(img_url, stream=True).raw).convert("RGB")
    return raw_image


@slow
class EfficientSamModelIntegrationTest(unittest.TestCase):
    def tearDown(self):
        super().tearDown()
    ...
    def test_inference_mask_generation_no_point(self):
        model = EfficientSamModel.from_pretrained("facebook/efficientsam-vit-base")
        processor = SamProcessor.from_pretrained("facebook/efficientsam-vit-base")
    ...
    def test_dummy_pipeline_generation(self):
        generator = pipeline("mask-generation", model="facebook/efficientsam-vit-base", device=torch_device)
        raw_image = prepare_image()
