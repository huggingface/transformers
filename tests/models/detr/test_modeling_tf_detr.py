...
from transformers import DetrConfig, is_tf_available, is_vision_available
from transformers.testing_utils import require_vision, slow
from transformers.utils import cached_property

from ...generation.test_generation_utils import GenerationTesterMixin
from ...test_configuration_common import ConfigTester
from ...test_modeling_tf_common import TFModelTesterMixin, _config_zero_init, floats_tensor
...
class DetrModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        pixel_mask = tf.ones([self.batch_size, self.min_size, self.max_size])
        ...
        if self.use_labels:
            ...
            for i in range(self.batch_size):
                target = {}
                target["class_labels"] = tf.random.uniform(
                    maxval=self.num_labels, shape=(self.n_targets,), dtype=tf.int32
                )
                target["boxes"] = tf.random.normal((self.n_targets, 4))
                target["masks"] = tf.random.normal((self.n_targets, self.min_size, self.max_size))
...
class DetrModelTest(TFModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            TFDetrModel,
        ...
        if return_labels:
            if model_class.__name__ in ["DetrForObjectDetection", "DetrForSegmentation"]:
                ...
                for i in range(self.model_tester.batch_size):
                    target = {}
                    target["class_labels"] = tf.ones(
                        size=(self.model_tester.n_targets,), dtype=tf.float64
                    )
                    target["boxes"] = tf.ones(
                        self.model_tester.n_targets, 4, dtype=tf.float32
                    )
                    target["masks"] = tf.ones(
...
class DetrModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = TFDetrModel.from_pretrained("facebook/detr-resnet-50")
        ...
        expected_shape = tf.TensorShape((1, 100, 256))
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_shape_logits = tf.TensorShape((1, model.config.num_queries, model.config.num_labels + 1))
        self.assertEqual(outputs.logits.shape, expected_shape_logits)
        expected_slice_logits = tf.constant(
            [[-19.1194, -0.0893, -11.0154], [-17.3640, -1.8035, -14.0219], [-20.0461, -0.5837, -11.1060]]
        )
        self.assertTrue(np.allclose(outputs.logits[0, :3, :3], expected_slice_logits, atol=1e-4))

        expected_shape_boxes = tf.TensorShape((1, model.config.num_queries, 4))
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        expected_slice_boxes = tf.constant(
            [[0.4433, 0.5302, 0.8853], [0.5494, 0.2517, 0.0529], [0.4998, 0.5360, 0.9956]]
        )
        self.assertTrue(np.allclose(outputs.pred_boxes[0, :3, :3], expected_slice_boxes, atol=1e-4))
    ...
    def test_inference_panoptic_segmentation_head(self):
        ...
        expected_shape_masks = tf.TensorShape((1, model.config.num_queries, 200, 267))
        self.assertEqual(outputs.pred_masks.shape, expected_shape_masks)
        expected_slice_masks = tf.constant(
            [[-7.7558, -10.8788, -11.9797], [-11.8881, -16.4329, -17.7451], [-14.7316, -19.7383, -20.3004]]
        )
        self.assertTrue(np.allclose(outputs.pred_masks[0, 0, :3, :3], expected_slice_masks, atol=1e-3))
