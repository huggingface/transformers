...
"""Testing suite for the PyTorch SAM_HQ model."""
...
if is_torch_available():
    ...
    from transformers import SAMHQModel, SamProcessor
...
class SAMHQMaskDecoderTester:
    ...
    def get_config(self):
        return SAMHQMaskDecoderConfig(
            ...
            attention_downsam_hqple_rate=self.attention_downsam_hqple_rate,
...
class SAMHQModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as SAM_HQ's vision encoder does not use input_ids, inputs_embeds,
    ...
    @unittest.skip(reason="SAM_HQ's vision encoder does not use inputs_embeds")
...
def prepare_dog_img():
    img_url = "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/transformers/model_doc/dog-sam_hq.png"
    raw_image = Image.open(requests.get(img_url, stream=True).raw).convert("RGB")
    return raw_image
...
class SAMHQModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_mask_generation_no_point(self):
        model = SAMHQModel.from_pretrained("facebook/sam_hq-vit-base")
        processor = SamProcessor.from_pretrained("facebook/sam_hq-vit-base")
    ...
    def test_dummy_pipeline_generation(self):
        generator = pipeline("mask-generation", model="facebook/sam_hq-vit-base", device=torch_device)
        raw_image = prepare_image()
