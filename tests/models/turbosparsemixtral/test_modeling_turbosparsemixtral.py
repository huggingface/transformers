...
class TurboSparseMixtralModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def get_config(self):
        return TurboSparseMixtralConfig(
    ...
    def create_and_check_model(
        ...
        model = TurboSparseMixtralModel(config=config)
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model_as_decoder with Llama->TurboSparseMixtral
    def create_and_check_model_as_decoder(
        ...
        model = TurboSparseMixtralModel(config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_for_causal_lm with Llama->TurboSparseMixtral
    def create_and_check_for_causal_lm(
        ...
        model = TurboSparseMixtralForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))

    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_decoder_model_past_large_inputs with Llama->TurboSparseMixtral
...
class TurboSparseMixtralModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (TurboSparseMixtralForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = (
        {
            "feature-extraction": TurboSparseMixtralModel,
            "text-classification": TurboSparseMixtralForSequenceClassification,
            "token-classification": TurboSparseMixtralForTokenClassification,
            "text-generation": TurboSparseMixtralForCausalLM,
            "zero-shot": TurboSparseMixtralForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = TurboSparseMixtralModelTester(self)
        self.config_tester = ConfigTester(self, config_class=TurboSparseMixtralConfig, hidden_size=37)
    ...
    def test_TurboSparseMixtral_sequence_classification_model(self):
        ...
        model = TurboSparseMixtralForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=attention_mask, labels=sequence_labels)
        self.assertEqual(result.logits.shape, (self.model_tester.batch_size, self.model_tester.num_labels))

    def test_TurboSparseMixtral_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_TurboSparseMixtral_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_token_classification_model with Llama->TurboSparseMixtral,llama->TurboSparseMixtral
    def test_TurboSparseMixtral_token_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
        token_labels = ids_tensor([self.model_tester.batch_size, self.model_tester.seq_length], config.num_labels)
        model = TurboSparseMixtralForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
    ...
    @unittest.skip("TurboSparseMixtral buffers include complex numbers, which breaks this test")
    def test_save_load_fast_init_from_base(self):
        pass

    @unittest.skip("TurboSparseMixtral uses GQA on all models so the KV cache is a non standard format")
    ...
    def test_flash_attn_2_generate_use_cache(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                # NOTE: TurboSparseMixtral apparently does not support right padding + use_cache with FA2.
