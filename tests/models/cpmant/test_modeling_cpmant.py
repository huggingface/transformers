...
from tests.test_modeling_common import ModelTesterMixin, floats_tensor, ids_tensor, random_attention_mask
from transformers import CPMAntConfig, CPMAntTokenizer, is_torch_available
...
if is_torch_available():
    ...
    from transformers.models.cpmant.modeling_cpmant import CPMANT_PRETRAINED_MODEL_ARCHIVE_LIST
...
class CPMAntModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = CPMAntForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = CPMAntForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = CPMAntForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = CPMAntForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = CPMAntForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = CPMAntForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class CPMAntModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (CPMAntForCausalLM,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = CPMAntModelTester(self)
        self.config_tester = ConfigTester(self, config_class=CPMAntConfig, hidden_size=37)
...
class CPMAntModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        texts = ["今天天气真好！"]
        model_path = "oliverpi/cpm-ant-10b"
