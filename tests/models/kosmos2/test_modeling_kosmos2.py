...
class Kosmos2ModelTest(
    ...
    all_generative_model_classes = (
        (Kosmos2ForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    def test_left_padding_compatibility(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            self.assertTrue(
                torch.allclose(
                    next_logits_wo_padding, next_logits_with_padding, atol=1e-3
                )
            )
...
class Kosmos2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_interpolate_pos_encoding(self):
        ...
        expected_slice = torch.tensor(
            [
                [1.0022, -1.1901, 3.2887],
                [2.6164, 0.0515, -0.8270],
                [1.8315, 0.1272, -0.8590],
            ]
        ).to(torch_device)
