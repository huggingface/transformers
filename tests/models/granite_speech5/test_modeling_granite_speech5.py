...
class GraniteSpeech5ForCTCIntegrationTest(unittest.TestCase):
    """Expected results were generated by running the original (remote-code) checkpoint
    `granite-speech-4.2-470m-turboctc` in float32:
    ...
    logits = model(inputs["input_features"], attention_mask=inputs["attention_mask"])
    transcriptions = processor.batch_decode(model.transcribe(**inputs).preds)
    ```

    with `token_ids = logits.argmax(-1)` masked with the CTC blank (id 0) past each sample's subsampled
    valid length, mirroring the padded-sequence contract of `GraniteSpeech5ForCTC.generate`.
    """
    ...
    @classmethod
    def setUp(cls):
        cls.checkpoint_name = "/raid/eustache/granite_speech5_converted"
    ...
    def test_model_integration_single(self):
        ...
        model = GraniteSpeech5ForCTC.from_pretrained(self.checkpoint_name, dtype=torch.float32, device_map="auto")
