...
class FuyuImageProcessingTester:
    def __init__(
        ...
        patch_size = (
            patch_size if patch_size is not None else {"height": 30, "width": 20}
        )  # Ensure patch divides size
        self.parent = parent
...
class FuyuImageProcessingTest(ImageProcessingTestMixin, unittest.TestCase):
    ...
    fast_image_processing_class = (
        FuyuImageProcessorFast if is_torchvision_available() else None
    )
    ...
    image_processor_list = [
        proc
        for proc in [image_processing_class, fast_image_processing_class]
        if proc is not None
    ]
    ...
    def test_save_load_fast_slow_auto(self):
        ...
        if (
            ...
            self.skipTest(
                "Skipping slow/fast save/load auto test as one processor is not defined"
            )

        image_processor_dict = (
            self.image_processor_tester.prepare_image_processor_dict()
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            image_processor_fast_0 = AutoImageProcessor.from_pretrained(
                tmpdirname, use_fast=True
            )
            self.assertIsInstance(
                image_processor_fast_0, self.fast_image_processing_class
            )

        image_processor_fast_1 = self.fast_image_processing_class(
            **image_processor_dict
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            image_processor_slow_1 = AutoImageProcessor.from_pretrained(
                tmpdirname, use_fast=False
            )
        ...
        difference_slow = {
            k: dict_slow_0.get(k, dict_slow_1.get(k)) for k in diff_keys_slow
        }
        ...
        unexpected_diffs_slow = {
            k: v
            for k, v in difference_slow.items()
            if k not in allowed_diff_keys_slow and v is not None
        }
    ...
    def test_save_load_fast_slow(self):
        ...
        if (
            ...
            self.skipTest(
                "Skipping slow/fast save/load test as one of the image processors is not defined"
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            image_processor_slow_0.save_pretrained(tmpdirname)
            image_processor_fast_0 = self.fast_image_processing_class.from_pretrained(
                tmpdirname
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            image_processor_fast_1.save_pretrained(tmpdirname)
            image_processor_slow_1 = self.image_processing_class.from_pretrained(
                tmpdirname
            )
        ...
        unexpected_diffs = {
            ...
            if k not in allowed_diff_keys and v is not None
        }
    ...
    def test_call_pil(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            if isinstance(
                processed_single.images, torch.Tensor
            ):  # Fast processor output
                self.assertEqual(
                ...
                self.assertEqual(
                    processed_single.images.shape[0], 1, "Batch dim should be 1"
                )
                ...
                self.fail(
                    f"Unexpected type for processed_single.images: {type(processed_single.images)}"
                )
            ...
            self.assertEqual(
                processed_tensor_single.ndim, 3, "Processed image should be 3D (C,H,W)"
            )
            expected_channels = 3
            ...
            # Handle different output types (Tensor vs List[List[Tensor]])
            if isinstance(
                processed_batch.images, torch.Tensor
            ):  # Fast processor output
                self.assertEqual(
                ...
                processed_tensor_batch_sample = processed_batch.images[
                    0
                ]  # Get first C, H, W
                ...
                processed_tensor_batch_sample = processed_batch.images[0][
                    0
                ]  # Get first C, H, W
            else:
                self.fail(
                    f"Unexpected type for processed_batch.images: {type(processed_batch.images)}"
                )
    ...
    def test_custom_resize_logic(self):
        ...
        small_image = (np.random.rand(target_h // 2, target_w // 2, 3) * 255).astype(
            np.uint8
        )
        large_image = (np.random.rand(target_h * 2, target_w * 2, 3) * 255).astype(
            np.uint8
        )
        tall_image = (np.random.rand(target_h * 3, target_w, 3) * 255).astype(
            np.uint8
        )  # Likely culprit
        ...
        for image_processing_class in self.image_processor_list:
            ...
            for img_np in test_images:
                ...
                if hasattr(image_processor, "resize") and callable(
                    getattr(image_processor, "resize")
                ):
                    if isinstance(image_processor, FuyuImageProcessorFast):
                        ...
                        )  # Ensure float for resize
                        resized_tensor = image_processor.resize(
                            img_tensor, size=image_processor.size
                        )
                        resized_h, resized_w = (
                    ...
                    print(
                        f"Original: ({orig_h}, {orig_w}), Aspect: {orig_h / orig_w:.4f}"
                    )
                    # Add check for division by zero before printing resized aspect
                    resized_aspect_str = (
                        f"{resized_h / resized_w:.4f}"
                        if resized_w > 0
                        else "N/A (width is zero)"
                    )
                    print(
                        f"Resized:  ({resized_h}, {resized_w}), Aspect: {resized_aspect_str}"
                    )
                    aspect_diff = (
                        abs((resized_h / resized_w) - (orig_h / orig_w))
                        if resized_w > 0
                        else float("inf")
                    )
                    ...
                    if orig_h <= target_h and orig_w <= target_w:
                        ...
                        if resized_w == 0:
                            self.fail(
                                f"Resized width became zero for {image_processing_class.__name__}"
                            )
    ...
    def test_padding_shape(self):
        ...
        image_np = (np.random.rand(target_h + 10, target_w // 2, 3) * 255).astype(
            np.uint8
        )

        for image_processing_class in self.image_processor_list:
            print(
                f"\nTesting class: {image_processing_class.__name__} in test_padding_shape"
            )  # Added print
            ...
            ):  # Fast processor output (B, C, H, W)
                self.assertEqual(
                    processed.images.shape[0], 1, "Test assumes batch size 1"
                )  # Sanity check
                processed_image = processed.images[0]  # Get the (C, H, W) tensor
            elif isinstance(
                processed.images, list
            ):  # Slow processor output (List[List[Tensor]])
                processed_image = processed.images[0][0]  # Get the (C, H, W) tensor
            else:
                self.fail(
                    f"Unexpected type for processed.images: {type(processed.images)}"
                )
            # --- END CORRECTION ---
    ...
    def test_metadata_output_presence_and_type(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            if isinstance(output["images"], list):  # Slow processor
                self.assertTrue(
                    len(output["images"]) > 0, "Output images list is empty (slow)"
                )
                self.assertIsInstance(
                    output["images"][0], list, "Inner element should be a list (slow)"
                )
                self.assertTrue(
                    len(output["images"][0]) > 0, "Inner images list is empty (slow)"
                )
                self.assertIsInstance(
                    output["images"][0][0],
                    np.ndarray,
                ...
                self.fail(
                    f"Unexpected type for output['images']: {type(output['images'])}"
                )
            ...
            for key in ["image_unpadded_heights", "image_unpadded_widths"]:
                ...
                if isinstance(metadata_list, np.ndarray):
                    self.assertTrue(
                        metadata_list.ndim >= 1, f"{key} ndarray should have >= 1 dim"
                    )
                    self.assertTrue(
                ...
                # If it's a list from slow proc (or potentially fast if batch=1 and squeezed?)
                    ...
                    if isinstance(metadata_list[0], list):
                        self.assertTrue(
                            len(metadata_list[0]) > 0, f"{key} inner list is empty"
                        )
            ...
            metadata_list = output[key]
            self.assertIsInstance(
                metadata_list, (list, np.ndarray), f"{key} should be list or ndarray"
            )
            ...
            if isinstance(output["images"], list):  # Slow
                ...
                actual_batch_size = (
                    output["images"].shape[0] if output["images"].ndim == 4 else 1
                )
    ...
    def test_patchify_output(self):
        ...
        if self.image_processing_class is not None:
            ...
            if (
                target_h % patch_size["height"] == 0
                and target_w % patch_size["width"] == 0
            ):
                patch_slow = image_processor_slow.patchify_image(
                    dummy_batch.clone(), patch_size=patch_size
                )
                ...
                self.skipTest(
                    "Tester config size not divisible by patch_size for slow processor."
                )
        ...
        if self.fast_image_processing_class is not None:
            ...
            if (
                ...
                patch_fast = image_processor_fast.patchify_image(
                    dummy_batch.clone(), patch_size=patch_size
                )
                ...
                self.skipTest(
                    "Tester config size not divisible by patch_size for fast processor."
                )
    ...
    def test_generic_slow_fast_equivalence(self):
        if (
            ...
            self.skipTest(
                reason="Skipping slow/fast equivalence test: requires both processors."
            )
        ...
        if isinstance(encoding_slow.images, list):
            processed_images_slow = torch.stack(
                [img[0] for img in encoding_slow.images], dim=0
            )
        elif isinstance(
            encoding_slow.images, torch.Tensor
        ):  # Should not happen for slow Fuyu, but safer
            processed_images_slow = encoding_slow.images
        else:
            self.fail(
                f"Unexpected type for encoding_slow.images: {type(encoding_slow.images)}"
            )
        ...
        try:
            torch.testing.assert_close(
                processed_images_slow, processed_images_fast, atol=atol, rtol=rtol
            )
        ...
        for key in ["image_unpadded_heights", "image_unpadded_widths"]:
            ...
            self.assertEqual(
                meta_slow.shape, meta_fast.shape, f"Shape mismatch for {key}"
            )
            # Use exact comparison for integer metadata
            torch.testing.assert_close(
                meta_slow.long(), meta_fast.long(), atol=0, rtol=0
            )
    ...
    @unittest.skip(
        reason="Fuyu processor uses 'images' key, not 'pixel_values', and may not support 4 channels."
    )
    ...
    @unittest.skip(
        reason="Fuyu processor uses 'images' key, not 'pixel_values'. Mixin test incompatible."
    )
    ...
    def test_slow_fast_equivalence(self):
        ...
        # Compare pixel values
        torch.testing.assert_close(
            processed_images_slow, processed_images_fast, atol=0.1, rtol=0.1
        )
        ...
        for key in ["image_unpadded_heights", "image_unpadded_widths"]:
            ...
            self.assertEqual(
                heights_slow.shape, heights_fast.shape, f"Shape mismatch for {key}"
            )
