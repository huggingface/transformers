...
class RTDetrModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        self.assertTrue(
            torch.allclose(outputs.pred_boxes[0, :3, :3], expected_boxes, atol=1e-4)
        )
        ...
        self.assertTrue(
            torch.allclose(results["scores"][:4], expected_scores, atol=1e-4)
        )
        self.assertSequenceEqual(results["labels"][:4].tolist(), expected_labels)
        self.assertTrue(
            torch.allclose(results["boxes"][:4], expected_slice_boxes, atol=1e-4)
        )
