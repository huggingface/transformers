...
class DiaTokenizerTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    def test_special_tokens_initialization(self):
        # Dia relies on specific additional special tokens, so we skip this
    ...
    def test_decode_asr_with_word_level_timestamps(self):
        ...
        tokenizer = DiaTokenizer.from_pretrained("onnx-community/dia-tiny.en_timestamped")
...
class SpeechToTextTokenizerMultilinguialTest(unittest.TestCase):
    checkpoint_name = "openai/dia-small.en"

    @classmethod
    def setUpClass(cls):
        cls.tokenizer: DiaTokenizer = DiaTokenizer.from_pretrained(cls.checkpoint_name)
        return cls

    def test_tokenizer_equivalence(self):
        text = "다람쥐 헌 쳇바퀴에 타고파"
        multilingual_tokenizer = DiaTokenizer.from_pretrained("nari-labs/Dia-1.6B", language="korean")
        monolingual_tokenizer = DiaTokenizer.from_pretrained("nari-labs/Dia-1.6B.en")
    ...
    def test_tokenizer_special(self):
        multilingual_tokenizer = DiaTokenizer.from_pretrained(
            "nari-labs/Dia-1.6B", language="english", task="transcribe"
        )
    ...
    def test_batch_encoding(self):
        multilingual_tokenizer = DiaTokenizer.from_pretrained(
            "nari-labs/Dia-1.6B", language="spanish", task="translate"
        )
    ...
    def test_batch_encoding_decoding(self):
        multilingual_tokenizer = DiaTokenizer.from_pretrained("nari-labs/Dia-1.6B", language="spanish")
    ...
    def test_offset_decoding(self):
        multilingual_tokenizer = DiaTokenizer.from_pretrained("nari-labs/Dia-1.6B")
        # fmt: off
        INPUT_TOKENS = [
    ...
    def test_convert_to_list_np(self):
        ...
        self.assertListEqual(DiaTokenizer._convert_to_list(test_list), test_list)
        self.assertListEqual(DiaTokenizerFast._convert_to_list(test_list), test_list)

        # Test with a numpy array
        np_array = np.array(test_list)
        self.assertListEqual(DiaTokenizer._convert_to_list(np_array), test_list)
        self.assertListEqual(DiaTokenizerFast._convert_to_list(np_array), test_list)
    ...
    def test_convert_to_list_tf(self):
        ...
        self.assertListEqual(DiaTokenizer._convert_to_list(tf_tensor), test_list)
        self.assertListEqual(DiaTokenizerFast._convert_to_list(tf_tensor), test_list)
    ...
    def test_convert_to_list_jax(self):
        ...
        self.assertListEqual(DiaTokenizer._convert_to_list(jax_array), test_list)
        self.assertListEqual(DiaTokenizerFast._convert_to_list(jax_array), test_list)
    ...
    def test_convert_to_list_pt(self):
        ...
        self.assertListEqual(DiaTokenizer._convert_to_list(torch_tensor), test_list)
        self.assertListEqual(DiaTokenizerFast._convert_to_list(torch_tensor), test_list)
