# coding=utf-8
# Copyright 2024 Google FiD Authors and HuggingFace Inc. team.
#
...
from transformers import FiDConfig, is_torch_available
...
class FiDModelTester:
    ...
    def get_large_model_config(self):
        return FiDConfig.from_pretrained("google-fid/fid-base")
    ...
    def get_pipeline_config(self):
        return FiDConfig(
            vocab_size=166,  # fid forces 100 extra tokens
    ...
    def create_and_check_with_lm_head(
        ...
        model = FiDForConditionalGeneration(config=config).to(torch_device).eval()
        outputs = model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
            decoder_attention_mask=decoder_attention_mask,
    ...
    def create_and_check_with_sequence_classification_head(
        ...
        model = FiDForSequenceClassification(config=config).to(torch_device).eval()
        outputs = model(
            input_ids=input_ids,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = FiDModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = FiDModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = FiDModel(config=config).to(torch_device).half().eval()
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [FiDModel, FiDForConditionalGeneration]:
            torch.manual_seed(0)
    ...
    def check_resize_embeddings_fid_v1_1(
        self,
        config,
    ):
...
@require_torch
class FiDModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (FiDModel, FiDForConditionalGeneration, FiDForSequenceClassification, FiDForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (FiDForConditionalGeneration,) if is_torch_available() else ()
    all_parallelizable_model_classes = (FiDModel, FiDForConditionalGeneration) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    test_resize_embeddings = True
    test_model_parallel = True
    is_encoder_decoder = True
    # The small FiD model needs higher percentages for CPU/MP tests
    model_split_percents = [0.8, 0.9]
    pipeline_model_mapping = (
        {
            "conversational": FiDForConditionalGeneration,
            "feature-extraction": FiDModel,
            "question-answering": FiDForQuestionAnswering,
            "table-question-answering": FiDForConditionalGeneration,
            "document-question-answering": FiDForConditionalGeneration,
            "summarization": FiDForConditionalGeneration,
            "text-classification": FiDForSequenceClassification,
            "text2text-generation": FiDForConditionalGeneration,
            "translation": FiDForConditionalGeneration,
            "zero-shot": FiDForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = FiDModelTester(self)
        self.config_tester = ConfigTester(self, config_class=FiDConfig, d_model=37)

    # FiDForSequenceClassification does not support inputs_embeds
    def test_inputs_embeds(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()

        for model_class in (FiDModel, FiDForConditionalGeneration, FiDForQuestionAnswering):
    ...
    def test_v1_1_resize_embeddings(self):
        ...
        self.model_tester.check_resize_embeddings_fid_v1_1(config)
    ...
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = FiDModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/fid_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = FiDForConditionalGeneration(config).eval()
        model.to(torch_device)
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from FiD model when head_mask specified
            if name == "head_mask":
...
class FiDEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_model(
        ...
        model = FiDEncoderModel(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids=input_ids,
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = FiDEncoderModel(config=config).to(torch_device).half().eval()
    ...
    def create_and_check_with_token_classification_head(
        ...
        model = FiDForTokenClassification(config=config).to(torch_device).eval()
        outputs = model(
            input_ids=input_ids,
            labels=labels,
            attention_mask=attention_mask,
...
class FiDEncoderOnlyModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (FiDEncoderModel, FiDForTokenClassification) if is_torch_available() else ()
    test_pruning = False
    test_resize_embeddings = False
    test_model_parallel = True
    pipeline_model_mapping = (
        {
            "token-classification": FiDForTokenClassification,
        }
        if is_torch_available()
        else {}
    )
    all_parallelizable_model_classes = (FiDEncoderModel,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = FiDEncoderOnlyModelTester(self)
        self.config_tester = ConfigTester(self, config_class=FiDConfig, d_model=37)
...
@require_tokenizers
@slow
class FiDModelFp16Tests(unittest.TestCase):
    def test_fp16_fp32_conversion(self):
        ...
        with unittest.mock.patch("builtins.__import__", side_effect=import_accelerate_mock):
            ...
            model = FiDForConditionalGeneration.from_pretrained("google-t5/t5-base", torch_dtype=torch.float16)
            ...
            model = FiDForConditionalGeneration.from_pretrained("google-t5/t5-base", torch_dtype=torch.bfloat16)
            self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
            self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)

        # Load using `accelerate` in bf16
        model = FiDForConditionalGeneration.from_pretrained(
...
class FiDModelIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return FiDForConditionalGeneration.from_pretrained("google-fid/fid-base").to(torch_device)

    @cached_property
    def tokenizer(self):
        return T5Tokenizer.from_pretrained("google-fid/fid-base")

    @slow
    def test_torch_quant(self):
        r"""
        Test that a simple `torch.quantization.quantize_dynamic` call works on a FiD model.
        """
        model_name = "google/flan-fid-small"
        tokenizer = T5Tokenizer.from_pretrained(model_name)
        model = FiDForConditionalGeneration.from_pretrained(model_name)
    ...
    def test_small_generation(self):
        model = FiDForConditionalGeneration.from_pretrained("google-t5/t5-base").to(torch_device)
    ...
    def test_small_integration_test(self):
        """
        For comparision run:
        >>> import fid  # pip install fid==0.7.1
        >>> from fid.data.sentencepiece_vocabulary import SentencePieceVocabulary

        >>> path_to_mtf_small_fid_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> fid_model = fid.models.MtfModel(model_dir=path_to_mtf_small_fid_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = fid_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_fid_v1_1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> fid_model = fid.models.MtfModel(model_dir=path_to_mtf_small_fid_v1_1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = fid_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = FiDForConditionalGeneration.from_pretrained("google/fid-v1_1-small").to(torch_device)
        tokenizer = T5Tokenizer.from_pretrained("google/fid-v1_1-small")
    ...
    @slow
    def test_small_byfid_integration_test(self):
        """
        For comparision run:
        >>> import fid  # pip install fid==0.9.1

        >>> path_to_byfid_small_checkpoint = '<fill_in>'
        >>> fid_model = fid.models.MtfModel(model_dir=path_to_tf_checkpoint, batch_size=1, tpu=None)
        >>> vocab = fid.data.ByteVocabulary()
        >>> score = fid_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = FiDForConditionalGeneration.from_pretrained("google/byfid-small").to(torch_device)
        tokenizer = ByT5Tokenizer.from_pretrained("google/byfid-small")
    ...
    def test_translation_en_to_fr(self):
        model = self.model  # google-fid/fid-base
        tok = self.tokenizer
    ...
    @slow
    def test_contrastive_search_fid(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        fid_tokenizer = AutoTokenizer.from_pretrained("flax-community/fid-base-cnn-dm")
        fid_model = FiDForConditionalGeneration.from_pretrained("flax-community/fid-base-cnn-dm").to(torch_device)
        input_ids = fid_tokenizer(
            article, add_special_tokens=False, truncation=True, max_length=512, return_tensors="pt"
        ).input_ids.to(torch_device)

        outputs = fid_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = fid_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
@require_torch
class TestAsymmetricFiD(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = FiDModelTester(self, **kwargs)
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to FiDConfig as num_layers
