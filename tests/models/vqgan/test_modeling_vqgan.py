...
class VQGANModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = VQGANForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = VQGANForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = VQGANForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = VQGANForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = VQGANForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = VQGANForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class VQGANModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (VQGANForCausalLM,) if is_torch_available() else ()
...
class VQGANModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = VQGANForMaskedLM.from_pretrained("vqgan-imagenet-f16-1024")
