...
class InternVLModelTester:
    ...
    def create_and_check_model(self, config, pixel_values):
        model = InternVLModel(config=config)
...
@require_torch
class InternVLModelModelTest(ModelTesterMixin, unittest.TestCase):
    """
    Model tester for `InternVLModel`.
    """

    all_model_classes = (InternVLModel,) if is_torch_available() else ()
    test_pruning = False
    test_head_masking = False
    test_torchscript = False

    def setUp(self):
        self.model_tester = InternVLModelTester(self)
    ...
    @unittest.skip(reason="Compile not yet supported because in InternVL models")
