...
LILT_TINY = "sshleifer/tiny-distilorganization/lilt-roberta-en-base"
...
class LiltModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = LiltForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        config.is_decoder = True
        config.add_cross_attention = True
        model = LiltForCausalLM(config=config).to(torch_device).eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = LiltForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = LiltForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class LiltModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (LiltForCausalLM,) if is_torch_available() else ()
    fx_compatible = False
    ...
    def test_create_position_ids_respects_padding_index(self):
        ...
        first available non-padding position index is LiltEmbeddings.padding_idx + 1
        """
        config = self.model_tester.prepare_config_and_inputs()[0]
        model = LiltEmbeddings(config=config)
    ...
    def test_create_position_ids_from_inputs_embeds(self):
        ...
        """
        config = self.model_tester.prepare_config_and_inputs()[0]
        embeddings = LiltEmbeddings(config=config)
...
class LiltModelIntegrationTest(TestCasePlus):
    ...
    def test_inference_masked_lm(self):
        model = LiltForMaskedLM.from_pretrained("organization/lilt-roberta-en-base")
        ...
        # lilt = torch.hub.load('pytorch/fairseq', 'lilt.base')
        # lilt.eval()
        # expected_slice = lilt.model.forward(input_ids)[0][:, :3, :3].detach()

        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=1e-4))

    @slow
    def test_inference_no_head(self):
        model = LiltModel.from_pretrained("organization/lilt-roberta-en-base")
        ...
        # lilt = torch.hub.load('pytorch/fairseq', 'lilt.base')
        # lilt.eval()
        # expected_slice = lilt.extract_features(input_ids)[:, :3, :3].detach()

        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=1e-4))

    @slow
    def test_inference_classification_head(self):
        model = LiltForSequenceClassification.from_pretrained("lilt-large-mnli")
        ...
        # lilt = torch.hub.load('pytorch/fairseq', 'lilt.large.mnli')
        # lilt.eval()
        # expected_tensor = lilt.predict("mnli", input_ids, return_logits=True).detach()
    ...
    def test_lm_head_ignore_keys(self):
        ...
        config = LiltConfig.from_pretrained(LILT_TINY)
        ...
        for cls in [LiltForMaskedLM, LiltForCausalLM]:
