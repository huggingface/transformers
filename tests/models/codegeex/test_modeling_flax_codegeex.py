...
@require_flax
class FlaxCodeGeeXModelTester:
    config_cls = CodeGeeXConfig
    config_updates = {}
    hidden_act = "gelu"
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        inputs_dict = prepare_codegeex_inputs_dict(config, input_ids, decoder_input_ids)
...
def prepare_codegeex_inputs_dict(
    config,
    input_ids,
    decoder_input_ids,
    attention_mask=None,
...
@require_flax
class FlaxCodeGeeXModelTest(FlaxModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            FlaxCodeGeeXForConditionalGeneration, 
            FlaxCodeGeeXForQuestionAnswering,
    ...
    all_generative_model_classes = (FlaxCodeGeeXForConditionalGeneration,) if is_flax_available() else ()
    ...
    def setUp(self):
        self.model_tester = FlaxCodeGeeXModelTester(self)
        self.config_tester = ConfigTester(self, config_class=CodeGeeXConfig)
...
@require_tokenizers
@require_flax
class FlaxCodeGeeXModelIntegrationTest(unittest.TestCase):
    def test_inference_no_head(self):
        model = FlaxCodeGeeXModel.from_pretrained('codegeex-base-cased')
        # change to intended input here
        input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        decoder_input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        inputs_dict = prepare_codegeex_inputs_dict(model.config, input_ids, decoder_input_ids)
    ...
    def test_inference_with_head(self):
        model = FlaxCodeGeeXForConditionalGeneration.from_pretrained('codegeex-base-cased')
        # change to intended input here
        input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        decoder_input_ids = _long_tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        inputs_dict = prepare_codegeex_inputs_dict(model.config, input_ids, decoder_input_ids)
    ...
    def test_seq_to_seq_generation(self):
        hf = FlaxCodeGeeXForConditionalGeneration.from_pretrained('codegeex-base-cased')
        tok = CodeGeeXTokenizer.from_pretrained('codegeex-base-cased')
