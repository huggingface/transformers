...
if is_torch_available():
    ...
    from transformers import (MODEL_FOR_PRETRAINING_MAPPING, FunnelBaseModel,
                              FunnelForMaskedLM, FunnelForMultipleChoice,
                              FunnelForPreTraining, FunnelForQuestionAnswering,
...
class FunnelModelIntegrationTest(unittest.TestCase):
    def test_inference_tiny_model(self):
        ...
        torch.testing.assert_close(
            output.sum(), expected_output_sum, rtol=1e-4, atol=1e-4
        )
        torch.testing.assert_close(
            output.mean(), expected_output_mean, rtol=1e-4, atol=1e-4
        )
