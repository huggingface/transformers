...
class MoTModelTester:
    ...
    def create_and_check_double_lm_head_model(
        ...
        model = MoTDoubleHeadsModel(config)
        model.to(torch_device)
        model.eval()
...
class MoTModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == "MoTDoubleHeadsModel":
    ...
    def test_mot_double_lm_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
...
class MoTModelLanguageGenerationTest(unittest.TestCase):
    ...
    def _test_lm_generate_mot_helper(
        self,
    ...
    @slow
    def test_lm_generate_mot(self):
        self._test_lm_generate_mot_helper()

    @slow
    def test_lm_generate_mot_with_gradient_checkpointing(self):
        self._test_lm_generate_mot_helper(gradient_checkpointing=True)

    @slow
    def test_lm_generate_mot_with_reorder_and_upcast_attn(self):
        self._test_lm_generate_mot_helper(reorder_and_upcast_attn=True)

    @slow
    def test_lm_generate_mot_with_scale_attn_by_inverse_layer_idx(self):
        self._test_lm_generate_mot_helper(scale_attn_by_inverse_layer_idx=True, verify_outputs=False)
    ...
    @slow
    def test_contrastive_search_mot(self):
        article = (
        ...
        mot_tokenizer = GPT2Tokenizer.from_pretrained("mot-large")
        mot_model = MoTLMHeadModel.from_pretrained("mot-large").to(torch_device)
        input_ids = mot_tokenizer(article, return_tensors="pt").input_ids.to(torch_device)

        outputs = mot_model.generate(input_ids, penalty_alpha=0.6, top_k=4, max_length=256)

        generated_text = mot_tokenizer.batch_decode(outputs, skip_special_tokens=True)
