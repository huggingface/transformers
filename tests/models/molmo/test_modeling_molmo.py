...
class MolmoForConditionalGenerationModelTest(
    ...
    @unittest.skip(reason="Molmo weights are not tied.")
...
class MolmoForConditionalGenerationIntegrationTest(unittest.TestCase):
    ...
    def test_7B_model_integration_test(self):
        prompt = "<|endoftext|><image> User: Describe this image. Assistant:"
        image_file = "https://picsum.photos/id/237/536/354"
        raw_image = Image.open(requests.get(image_file, stream=True).raw)
        inputs = (
            self.processor(images=raw_image, text=prompt, return_tensors="pt", add_bos_token=True)
            .to(torch.bfloat16)
        ...
        output = self.model.generate(**inputs, max_new_tokens=18)
