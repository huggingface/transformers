...
from transformers import AutoTokenizer, Phi3VConfig, is_torch_available, set_seed
...
class Phi3VModelTester:
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model with Llama->Phi3V
    def create_and_check_model(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        model = Phi3VModel(config=config)
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model_as_decoder with Llama->Phi3V
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_for_causal_lm with Llama->Phi3V
    def create_and_check_for_causal_lm(
        ...
        model = Phi3VForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))

    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_decoder_model_past_large_inputs with Llama->Phi3V
...
class Phi3VModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            "feature-extraction": Phi3VModel,
            "text-classification": Phi3VForSequenceClassification,
            "token-classification": Phi3VForTokenClassification,
            "text-generation": Phi3VForCausalLM,
            "zero-shot": Phi3VForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=Phi3VConfig, hidden_size=37)
    ...
    def test_Phi3V_sequence_classification_model(self):
        ...
        model = Phi3VForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=attention_mask, labels=sequence_labels)
        self.assertEqual(result.logits.shape, (self.model_tester.batch_size, self.model_tester.num_labels))

    def test_Phi3V_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_Phi3V_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_token_classification_model with Llama->Phi3V,llama->Phi3V
    def test_Phi3V_token_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
        token_labels = ids_tensor([self.model_tester.batch_size, self.model_tester.seq_length], config.num_labels)
        model = Phi3VForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
    ...
    @unittest.skip(reason="Phi3V buffers include complex numbers, which breaks this test")
    def test_save_load_fast_init_from_base(self):
        pass

    @unittest.skip(reason="Phi3V uses GQA on all models so the KV cache is a non standard format")
    ...
    def test_flash_attn_2_inference_equivalence_right_padding(self):
        self.skipTest(reason="Phi3V flash attention does not support right padding")
...
class Phi3VIntegrationTest(unittest.TestCase):
    ...
    def test_model_450m_logits(self):
        ...
        model = Phi3VForCausalLM.from_pretrained("Qwen/Phi3V-0.5B", device_map="auto")
    ...
    def test_model_450m_generation(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Phi3V-0.5B", use_fast=False)
        model = Phi3VForCausalLM.from_pretrained("Qwen/Phi3V-0.5B", device_map="auto")
    ...
    def test_model_450m_long_prompt_sdpa(self):
        ...
        # An input with 4097 tokens that is above the size of the sliding window
        input_ids = [1] + [306, 338] * 2048
        model = Phi3VForCausalLM.from_pretrained("Qwen/Phi3V-0.5B", device_map="auto", attn_implementation="sdpa")
        input_ids = torch.tensor([input_ids]).to(model.model.embed_tokens.weight.device)
        generated_ids = model.generate(input_ids, max_new_tokens=4, temperature=0)
        self.assertEqual(EXPECTED_OUTPUT_TOKEN_IDS, generated_ids[0][-2:].tolist())
    ...
    def test_speculative_generation(self):
        ...
        prompt = "My favourite condiment is "
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Phi3V-7B", use_fast=False)
        model = Phi3VForCausalLM.from_pretrained("Qwen/Phi3V-0.5B", device_map="auto", torch_dtype=torch.float16)
        assistant_model = Phi3VForCausalLM.from_pretrained(
            "Qwen/Phi3V-0.5B", device_map="auto", torch_dtype=torch.float16
        )
        input_ids = tokenizer.encode(prompt, return_tensors="pt").to(model.model.embed_tokens.weight.device)
