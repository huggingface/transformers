...
class BloomIntegrationTest(unittest.TestCase):
    ...
    def test_hidden_states_transformers(self):
        model = BloomModel.from_pretrained(self.path_bigscience_model, use_safetensors=False, use_cache=False, dtype="auto").to(torch_device)
        model.eval()
    ...
    def test_logits(self):
        model = BloomForCausalLM.from_pretrained(self.path_bigscience_model, use_safetensors=False, use_cache=False, dtype="auto").to(
            torch_device
        )  # load in bf16
        model.eval()
    ...
    def test_simple_generation(self):
        ...
        model = BloomForCausalLM.from_pretrained(path_560m, use_safetensors=False, use_cache=True, revision="gs555750").to(torch_device)
        model = model.eval()
