...
class BloomModelTester:
    ...
    def prepare_config_and_inputs(self, gradient_checkpointing=False, force_lm_head_in_fp32=True):
        ...
        config = self.get_config(
            gradient_checkpointing=gradient_checkpointing, force_lm_head_in_fp32=force_lm_head_in_fp32
        )

        return (config, input_ids, input_mask, sequence_labels)

    def get_config(self, gradient_checkpointing=False, force_lm_head_in_fp32=True, slow_but_exact=True):
        return BloomConfig(
...
class BloomModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_force_lm_head_in_fp32_is_close_to_fp16(self):
        ...
        # Test that we see at least one collapse in fp16 and none when `force_lm_head_in_fp32=True`,ie the `len(unique_values) < vocabulary_size`.
...
class BloomEmbeddingTest(unittest.TestCase):
    ...
    def test_hidden_states_transformers(self):
        ...
        model = BloomModel.from_pretrained(
            self.path_bigscience_model, use_cache=False, torch_dtype="auto", force_lm_head_in_fp32=False
        ).to(torch_device)
        model.eval()
