...
class Emu3ProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    ...
    def test_processor_for_generation(self):
        ...
        inputs = processor(
            text=input_str, return_for_image_generation=True, return_tensors="pt"
        )
        self.assertListEqual(
            list(inputs.keys()), ["input_ids", "attention_mask", "image_sizes"]
        )
    ...
    def test_processor_postprocess(self):
        ...
        inputs = processor(
            text=input_str, images=orig_image, do_resize=False, return_tensors="np"
        )
        normalized_image_input = inputs.pixel_values
        unnormalized_images = processor.postprocess(
            normalized_image_input, return_tensors="np"
        )["pixel_values"]
