...
from transformers import MantaConfig, is_torch_available
...
class MantaModelTester:
    ...
    def check_prepare_lm_labels_via_shift_left(
        ...
        model = MantaModel(config=config)
    ...
    def create_and_check_decoder_model_past(
        ...
        model = MantaModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = MantaModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = MantaModel(config=config).to(torch_device).half().eval()
...
@require_torch
class MantaModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):

    all_model_classes = (MantaModel, MantaForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (MantaForConditionalGeneration,) if is_torch_available() else ()
    all_parallelizable_model_classes = (MantaModel, MantaForConditionalGeneration) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    test_resize_embeddings = True
    test_model_parallel = True
    is_encoder_decoder = True
    # The small MANTA model needs higher percentages for CPU/MP tests
    model_split_percents = [0.8, 0.9]

    def setUp(self):
        self.model_tester = MantaModelTester(self)
        self.config_tester = ConfigTester(self, config_class=MantaConfig, d_model=37)
    ...
    def test_model_from_pretrained(self):
        for model_name in MANTA_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = MantaModel.from_pretrained(model_name)
            self.assertIsNotNone(model)

    @unittest.skip("Test has a segmentation fault on torch 1.8.0")
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = MantaModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/manta_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = MantaForConditionalGeneration(config).eval()
        model.to(torch_device)
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from MANTA model when head_mask specified
            if name == "head_mask":
...
class MantaEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_model(
        ...
        model = MantaEncoderModel(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids=input_ids,
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = MantaEncoderModel(config=config).to(torch_device).half().eval()
...
class MantaEncoderOnlyModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (MantaEncoderModel,) if is_torch_available() else ()
    test_pruning = False
    test_resize_embeddings = False
    test_model_parallel = True
    all_parallelizable_model_classes = (MantaEncoderModel,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = MantaEncoderOnlyModelTester(self)
        self.config_tester = ConfigTester(self, config_class=MantaConfig, d_model=37)
...
class MantaModelIntegrationTests(unittest.TestCase):
    ...
    def test_small_integration_test(self):
        ...
        >>> path_to_mtf_small_manta_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> manta_model = manta.models.MtfModel(model_dir=path_to_mtf_small_manta_checkpoint, batch_size=1, tpu=None)
...
@require_torch
class TestAsymmetricManta(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = MantaModelTester(self, **kwargs)
