...
"""Testing suite for the PyTorch Lfm2Audio model."""
...
@require_torch
class Lfm2AudioDecoderTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Lfm2AudioModel, Lfm2AudioForCausalLM) if is_torch_available() else ()

    test_resize_embeddings = True
    pipeline_model_mapping = (
        {
            "feature-extraction": Lfm2AudioModel,
            "text-generation": Lfm2AudioForCausalLM,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = Lfm2AudioDecoderTester(self)
    ...
    @unittest.skip(reason="The Lfm2AudioModel does not have support dynamic compile yet")
    ...
    @unittest.skip(reason="Test becomes too complex with Lfm2Audio requiring multiple input modalities.")
...
@require_torch
class Lfm2AudioTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Lfm2AudioForConditionalGeneration,) if is_torch_available() else ()
    # training is not supported yet for Lfm2Audio
    test_resize_embeddings = False

    def setUp(self):
        self.model_tester = Lfm2AudioTester(self)
    ...
    @unittest.skip(reason="The Lfm2Audio model does not have support dynamic compile yet")
    @pytest.mark.torch_compile_test
    def test_sdpa_can_compile_dynamic(self):
        pass
    ...
    def test_left_padding_compatibility(self):
        # Overwrite -- Lfm2Audio needs to prepare the audio codes, and they must be padded accordingly
    ...
    @unittest.skip(reason="Compile not yet supported because in Lfm2Audio models")
    ...
    @unittest.skip(reason="Test becomes too complex with Lfm2Audio requiring multiple modalities")
    def test_generate_continue_from_inputs_embeds(self):
        pass
    ...
    @pytest.mark.generate
    @unittest.skip(
        reason="Lfm2Audio requires setting `model.generated_audio_codes` in generate() before preparing inputs"
    )
    ...
    @unittest.skip(reason="Lfm2Audio has no separate base model without a head.")
...
@require_torch
class Lfm2AudioIntegrationTests(unittest.TestCase):
    @cached_property
