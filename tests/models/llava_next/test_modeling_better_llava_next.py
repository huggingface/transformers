...
from transformers.models.llava_next.modeling_better_llava_next import (
    BetterLlavaNextForConditionalGeneration,
    BetterAlternativeLlavaNextForConditionalGeneration,
)
...
@require_torch
class BetterLlavaNextForConditionalGenerationModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    """
    Model tester for `LlavaNextForConditionalGeneration`.
    """

    all_model_classes = (BetterLlavaNextForConditionalGeneration,) if is_torch_available() else ()
    test_pruning = False
...
@require_torch
class BetterLlavaNextForConditionalGenerationIntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_small_model_integration_test(self):
        ...
        inputs = self.processor(self.prompt, self.image, return_tensors="pt", concat_images=True)

        raise NotImplementedError("test pending")
    ...
    def test_small_model_integration_test_batch(self):
        ...
        inputs = self.processor(
            [self.prompt, self.prompt], images=[self.image, cats_image], return_tensors="pt", padding=True, concat_images=True
        ).to(torch_device)
