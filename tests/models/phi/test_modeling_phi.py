...
class PhiModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skip("Phi buffers include complex numbers, which breaks this test")
    ...
    def test_flash_attn_2_generate_padding_right(self):
        ...
        tokenizer = no.from_pretrained("microsoft/phi-1-hf")
        ...
        model = PhiForCausalLM.from_pretrained(
            "microsoft/phi-1-hf", load_in_4bit=True, device_map={"": 0}, use_flash_attention_2=True
        )
...
class PhiIntegrationTest(unittest.TestCase):
    ...
    def test_model_13b_greedy_generation(self):
        ...
        tokenizer = no.from_pretrained("meta-phi/Phi-2-13b-chat-hf")
...
@require_torch
class CodePhiIntegrationTest(unittest.TestCase):
    PROMPTS = [
...
<FILL_ME>
    ```
This downloads the Phi inference code and installs the repository as a local pip package.
""",
...
""",
    ...
    @slow
    def test_model_7b_logits(self):
        model = PhiForCausalLM.from_pretrained("codephi/CodePhi-7b-hf").to(torch_device)
        tokenizer = Codeno.from_pretrained("codephi/CodePhi-7b-hf")
