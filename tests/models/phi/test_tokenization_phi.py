...
@require_sentencepiece
@require_tokenizers
class PhiTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    tokenizer_class = no
    rust_tokenizer_class = noFast
    ...
    def setUp(self):
        ...
        tokenizer = no(SAMPLE_VOCAB, keep_accents=True)
    ...
    def test_save_pretrained(self):
        self.tokenizers_list += (self.rust_tokenizer_class, "hf-internal-testing/phi-tokenizer", {})
    ...
    def test_tokenizer_integration(self):
        ...
        self.tokenizer_integration_test_util(
            ...
            model_name="hf-internal-testing/phi-tokenizer",
    ...
    def test_picklable(self):
        with tempfile.NamedTemporaryFile() as f:
            ...
            tokenizer = no(f.name, keep_accents=True)
...
class PhiIntegrationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        checkpoint_name = "hf-internal-testing/phi-tokenizer-non-normalized"
        cls.tokenizer: no = no.from_pretrained(checkpoint_name)
        cls.rust_tokenizer = noFast.from_pretrained(checkpoint_name)
        return cls
    ...
    def test_fast_special_tokens(self):
        ...
        fast_tokenizer = noFast.from_pretrained(
            "hf-internal-testing/phi-tokenizer", add_eos_token=True, add_bos_token=False
        )
        fast = fast_tokenizer.encode("A sample test", add_special_tokens=True)
        assert fast == [319, 4559, 1243, 2]

        slow_tokenzier = no.from_pretrained(
            "hf-internal-testing/phi-tokenizer", add_eos_token=True, add_bos_token=False
        )
    ...
    def test_special_token_special_word(self):
        ...
        tokenizer = no.from_pretrained("huggyphi/phi-7b", legacy=False)
...
class CommonSpmIntegrationTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        tokenizer = no(SAMPLE_VOCAB, extra_ids=0, add_bos_token=False, legacy=False)
