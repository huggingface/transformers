...
@require_torch
@require_vision
class YolosImageProcessingTest(
    ...
    def test_resize_max_size_respected(self, image_size, longest_edge, shortest_edge):
        ...
        self.assertTrue(
            max_size <= 1333, f"Expected max_size <= 1333, got image shape {shape}"
        )
        self.assertTrue(
            min_size <= 800, f"Expected min_size <= 800, got image shape {shape}"
        )
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        expected_area = torch.tensor(
            [5832.7256, 11144.6689, 484763.2500, 829269.8125, 146579.4531, 164177.6250]
        )
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        # verify area
        expected_area = torch.tensor(
            [146591.5000, 163974.2500, 480092.2500, 11187.0000, 5824.5000, 7562.5000]
        )
        self.assertTrue(torch.allclose(encoding["labels"][0]["area"], expected_area))
        # verify boxes
        expected_boxes_shape = torch.Size([6, 4])
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        self.assertTrue(
            torch.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, atol=1e-3)
        )
        self.assertTrue(
            torch.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, atol=1e-3)
        )

        # Check the masks have also been padded
        self.assertEqual(
            encoding["labels"][0]["masks"].shape, torch.Size([6, 800, 1056])
        )
        self.assertEqual(
            encoding["labels"][1]["masks"].shape, torch.Size([6, 800, 1056])
        )
        ...
        self.assertTrue(
            torch.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, atol=1)
        )
        self.assertTrue(
            torch.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, atol=1)
        )
