...
class VaultGemmaModelTester(CausalLMModelTester):
    if is_torch_available():
        ...
        sequence_class = VaultGemmaForSequenceClassification
        token_class = VaultGemmaForTokenClassification
    pipeline_model_mapping = (
        {
            "feature-extraction": VaultGemmaModel,
            "text-classification": VaultGemmaForSequenceClassification,
            "token-classification": VaultGemmaForTokenClassification,
            "text-generation": VaultGemmaForCausalLM,
            "zero-shot": VaultGemmaForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
...
class VaultGemmaModelTest(CausalLMModelTest, unittest.TestCase):
    all_model_classes = (
        (VaultGemmaModel, VaultGemmaForCausalLM, VaultGemmaForSequenceClassification, VaultGemmaForTokenClassification)
        if is_torch_available()
        else ()
    )
    pipeline_model_mapping = (
...
class VaultGemmaIntegrationTest(unittest.TestCase):
    ...
    def test_model_9b_pipeline_bf16(self):
        # See https://github.com/huggingface/transformers/pull/31747 -- pipeline was broken for VaultGemma before this PR
        model_id = "google/gemma-2-9b"
