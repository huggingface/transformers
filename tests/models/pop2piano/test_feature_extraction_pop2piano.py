...
from transformers.testing_utils import (check_json_file_has_correct_format,
                                        require_essentia, require_librosa,
                                        require_scipy, require_tf,
                                        require_torch)
from transformers.utils.import_utils import (is_essentia_available,
...
@require_librosa
@require_scipy
class Pop2PianoFeatureExtractionTest(
    SequenceFeatureExtractionTestMixin, unittest.TestCase
):
    feature_extraction_class = (
        Pop2PianoFeatureExtractor if requirements_available else None
    )
    ...
    def test_call(self):
        ...
        input_features = feature_extractor(
            speech_input, sampling_rate=16_000, return_tensors="np"
        )
    ...
    def test_integration(self):
        ...
        feaure_extractor = Pop2PianoFeatureExtractor.from_pretrained(
        ...
        EXPECTED_INPUT_FEATURES = torch.tensor(
            [
                [-7.1493, -6.8701, -4.3214],
                [-5.9473, -5.7548, -3.8438],
                [-6.1324, -5.9018, -4.3778],
            ]
        )
        torch.testing.assert_close(
            input_features[0, :3, :3], EXPECTED_INPUT_FEATURES, rtol=1e-4, atol=1e-4
        )
    ...
    def test_attention_mask(self):
        ...
        speech_input2 = np.random.randint(low=0, high=10, size=500_000).astype(
            np.float32
        )
        input_features = feature_extractor(
        ...
        self.assertEqual(
            input_features["attention_mask_extrapolated_beatstep"].shape[0], 2
        )
        ...
        self.assertTrue(
            np.max(input_features["attention_mask_extrapolated_beatstep"]) == 1
        )

        self.assertTrue(np.min(input_features["attention_mask"]) == 0)
        self.assertTrue(np.min(input_features["attention_mask_beatsteps"]) == 0)
        self.assertTrue(
            np.min(input_features["attention_mask_extrapolated_beatstep"]) == 0
        )
    ...
    def test_batch_feature(self):
        ...
        speech_input3 = np.random.randint(low=0, high=10, size=500_000).astype(
            np.float32
        )
