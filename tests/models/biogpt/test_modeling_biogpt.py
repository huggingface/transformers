...
if is_torch_available():
    ...
    from transformers import (
        BioGptForCausalLM,
        BioGptForMaskedLM,
        BioGptForMultipleChoice,
        BioGptForQuestionAnswering,
...
class BioGptModelTester:
    ...
    def create_and_check_for_masked_lm(
        ...
        model = BioGptForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = BioGptForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = BioGptForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = BioGptForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class BioGptModelTest(ModelTesterMixin, unittest.TestCase):
...
class BioGptModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = BioGptForMaskedLM.from_pretrained("kamalkraj/biogpt")
