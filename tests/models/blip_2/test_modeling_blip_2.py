...
class Blip2ForConditionalGenerationDecoderOnlyTest(
    ...
    all_generative_model_classes = (
        (Blip2ForConditionalGeneration,) if is_torch_available() else ()
    )
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    ...
    def _check_outputs(
        ...
        internal_batch_size = (
            input_batch_size * num_beams
            if num_beams > 1
            else input_batch_size * num_return_sequences
        )
        ...
        # scores
        self._check_scores(
            internal_batch_size, output.scores, length=gen_len, config=config
        )
...
@require_torch
class Blip2ModelTest(
    ModelTesterMixin, PipelineTesterMixin, GenerationTesterMixin, unittest.TestCase
):
    all_model_classes = (
...
class Blip2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_itm(self):
        ...
        self.assertTrue(
            torch.allclose(out[0].cpu(), torch.Tensor([[0.4406]]), rtol=1e-3, atol=1e-3)
        )
