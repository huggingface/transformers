...
class Blip2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_t5(self):
        ...
        # prepare image
        image = prepare_img()
        inputs = processor(images=image, return_tensors="pt").to(torch_device, torch_dtype=torch.float16)
        ...
        # image and context
        prompt = "Question: which city is this? Answer:"
        inputs = processor(images=image, text=prompt, return_tensors="pt").to(torch_device, torch_dtype=torch.float16)
