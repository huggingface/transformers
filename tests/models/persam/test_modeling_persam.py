...
""" Testing suite for the PyTorch PERSAM model. """
...
from transformers import PerSamConfig, PerSamMaskDecoderConfig, PerSamPromptEncoderConfig, PerSamVisionConfig
...
if is_torch_available():
    ...
    from transformers import PerSamModel, SamProcessor
    from transformers.models.persam.modeling_persam import PERSAM_PRETRAINED_MODEL_ARCHIVE_LIST
...
class PerSamPromptEncoderTester:
    def __init__(
        self,
        hidden_size=32,
        input_image_size=24,
    ...
    def get_config(self):
        return PerSamPromptEncoderConfig(
...
class PerSamMaskDecoderTester:
    ...
    def get_config(self):
        return PerSamMaskDecoderConfig(
            hidden_size=self.hidden_size,
            hidden_act=self.hidden_act,
...
class PerSamModelTester:
    def __init__(
        ...
        self.prompt_encoder_tester = PerSamPromptEncoderTester()
        self.mask_decoder_tester = PerSamMaskDecoderTester()
    ...
    def create_and_check_model(self, config, pixel_values):
        model = PerSamModel(config=config)
...
@require_torch
class PerSamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as PERSAM's vision encoder does not use input_ids, inputs_embeds,
    ...
    @unittest.skip(reason="PerSamModel does not support training")
    ...
    @unittest.skip(reason="PerSamModel has no base class and is not available in MODEL_MAPPING")
    ...
    def test_model_from_pretrained(self):
        for model_name in PERSAM_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = PerSamModel.from_pretrained(model_name)
