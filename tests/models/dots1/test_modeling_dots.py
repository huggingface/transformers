...
if is_torch_available():
    ...
    from transformers import (
        Dots1ForCausalLM,
        Dots1ForQuestionAnswering,
        Dots1ForSequenceClassification,
        Dots1ForTokenClassification,
        Dots1Model,
    )
...
class Dots1ModelTester:
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model with Llama->Dots1
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model_as_decoder with Llama->Dots1
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_for_causal_lm with Llama->Dots1
    def create_and_check_for_causal_lm(
        ...
        model = Dots1ForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))

    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_decoder_model_past_large_inputs with Llama->Dots1
...
@require_torch
# Copied from tests.models.mistral.test_modeling_mistral.MistralModelTest with Mistral->Dots1
class Dots1ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            ...
            "text-classification": Dots1ForSequenceClassification,
            "token-classification": Dots1ForTokenClassification,
            "text-generation": Dots1ForCausalLM,
            "zero-shot": Dots1ForSequenceClassification,
            "question-answering": Dots1ForQuestionAnswering,
        }
        if is_torch_available()
        else {}
    )
    ...
    def test_Dots1_sequence_classification_model(self):
        ...
        model = Dots1ForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=attention_mask, labels=sequence_labels)
        self.assertEqual(result.logits.shape, (self.model_tester.batch_size, self.model_tester.num_labels))

    def test_Dots1_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_Dots1_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_token_classification_model with Llama->Dots1,llama->Dots1
    def test_Dots1_token_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
        token_labels = ids_tensor([self.model_tester.batch_size, self.model_tester.seq_length], config.num_labels)
        model = Dots1ForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
    ...
    @unittest.skip(reason="Dots1 buffers include complex numbers, which breaks this test")
    def test_save_load_fast_init_from_base(self):
        pass

    @unittest.skip(reason="Dots1 uses GQA on all models so the KV cache is a non standard format")
...
class Dots1IntegrationTest(unittest.TestCase):
    ...
    def test_model_15b_a2b_logits(self):
        ...
        model = Dots1ForCausalLM.from_pretrained("Qwen/Qwen3-15B-A2B-Base", device_map="auto")
    ...
    def test_speculative_generation(self):
        ...
        assistant_model = Dots1ForCausalLM.from_pretrained(
            "Qwen/Qwen3-15B-A2B-Base", device_map="auto", torch_dtype=torch.float16
        )
        input_ids = tokenizer.encode(prompt, return_tensors="pt").to(model.model.embed_tokens.weight.device)
