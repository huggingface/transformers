...
if is_torch_available():
    ...
    from transformers import (SEWDForCTC, SEWDForSequenceClassification,
                              SEWDModel, Wav2Vec2FeatureExtractor,
                              Wav2Vec2Processor)
...
class SEWDModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_pretrained_batched(self):
        ...
        torch.testing.assert_close(
            outputs[:, :4, :4], expected_outputs_first, rtol=1e-3, atol=1e-3
        )
        torch.testing.assert_close(
            outputs[:, -4:, -4:], expected_outputs_last, rtol=1e-3, atol=1e-3
        )
