...
from transformers import (OneFormerConfig, is_torch_available,
                          is_vision_available)
...
class OneFormerModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        task_inputs = (
            torch.randint(
                high=self.vocab_size, size=(self.batch_size, self.sequence_length)
            )
            .to(torch_device)
            .long()
        )
    ...
    def prepare_config_and_inputs_for_common(self):
        config, pixel_values, task_inputs, pixel_mask, _, _, _ = (
    ...
    def check_output_hidden_state(self, output, config):
        ...
        self.parent.assertTrue(
            len(transformer_decoder_hidden_states), config.decoder_layers - 1
        )
    ...
    def create_and_check_oneformer_model(
        ...
        with torch.no_grad():
            ...
            output = model(
                pixel_values, task_inputs=task_inputs, output_hidden_states=True
            )
...
class OneFormerModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {"feature-extraction": OneFormerModel} if is_torch_available() else {}
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=OneFormerConfig, has_text_modality=False
        )
    ...
    def test_oneformer_model(self):
        ...
        self.model_tester.create_and_check_oneformer_model(
            config, **inputs, output_hidden_states=False
        )

    def test_oneformer_universal_segmentation_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_oneformer_universal_segmentation_head_model(
            *config_and_inputs
        )
    ...
    def test_model_with_labels(self):
        ...
        inputs = {
            ...
            "task_inputs": torch.randint(
                high=self.model_tester.vocab_size, size=(2, 77), device=torch_device
            ).long(),
    ...
    def test_retain_grad_hidden_states_attentions(self):
        ...
        transformer_decoder_class_predictions = (
            outputs.transformer_decoder_class_predictions
        )
        transformer_decoder_class_predictions.retain_grad()

        transformer_decoder_mask_predictions = (
            outputs.transformer_decoder_mask_predictions
        )
...
class OneFormerModelIntegrationTest(unittest.TestCase):
    ...
    def default_processor(self):
        return (
            OneFormerProcessor.from_pretrained(self.model_checkpoints)
            if is_vision_available()
            else None
        )
    ...
    def test_inference_no_head(self):
        ...
        expected_slice_hidden_state = torch.tensor(
            [
                [0.2723, 0.8280, 0.6026],
                [1.2699, 1.1257, 1.1444],
                [1.1344, 0.6153, 0.4177],
            ]
        ).to(torch_device)
        self.assertTrue(
            torch.allclose(
        ...
        expected_slice_hidden_state = torch.tensor(
            [
                [1.0581, 1.2276, 1.2003],
                [1.1903, 1.2925, 1.2862],
                [1.158, 1.2559, 1.3216],
            ]
        ).to(torch_device)
        self.assertTrue(
    ...
    def test_inference_universal_segmentation_head(self):
        ...
        expected_slice = [
            [
                [3.1848, 4.2141, 4.1993],
                [2.9000, 3.5721, 3.6603],
                [2.5358, 3.0883, 3.6168],
            ]
        ]
        ...
        expected_slice = torch.tensor(
            [
                [3.0668, -1.1833, -5.1103],
                [3.3440, -3.3620, -5.1101],
                [2.6017, -4.3613, -4.1444],
            ]
        ).to(torch_device)
        torch.testing.assert_close(
            class_queries_logits[0, :3, :3],
            expected_slice,
            rtol=TOLERANCE,
            atol=TOLERANCE,
        )
    ...
    def test_inference_fp16(self):
        ...
        processor = self.default_processor
        image = prepare_img()
        inputs = processor(image, ["semantic"], return_tensors="pt").to(
    ...
    def test_with_segmentation_maps_and_loss(self):
        dummy_model = OneFormerForUniversalSegmentation.from_pretrained(
            self.model_checkpoints
        )
        processor = self.default_processor
        processor.image_processor.num_text = (
            dummy_model.config.num_queries - dummy_model.config.text_encoder_n_ctx
        )
        dummy_model.config.is_training = True
        model = (
            OneFormerForUniversalSegmentation(dummy_model.config)
            .to(torch_device)
            .eval()
        )
        del dummy_model
