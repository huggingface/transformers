...
from transformers import (AutoProcessor, AyaVisionConfig, is_torch_available,
                          is_vision_available)
...
class AyaVisionVisionText2TextModelTester:
    def __init__(
        ...
        self.image_seq_length = (
            image_size // (vision_config["patch_size"] * downsample_factor)
        ) ** 2
...
@require_torch
class AyaVisionModelTest(
    ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase
):
    all_model_classes = (
        (AyaVisionForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=AyaVisionConfig, has_text_modality=False
        )
...
class AyaVisionIntegrationTest(unittest.TestCase):
    ...
    def test_small_model_integration_forward(self):
        ...
        expected_logits = torch.tensor(
            [0.4109, 0.1532, 0.8018, 2.1328, 0.5483], dtype=torch.float16
        )
        self.assertTrue(
    ...
    def test_small_model_integration_batched_generate(self):
        ...
        # Check second output
        decoded_output = processor.decode(
            output[1, inputs["input_ids"].shape[1] :], skip_special_tokens=True
        )
        print("decoded_output", decoded_output)
