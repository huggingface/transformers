...
class Qwen2VLImageProcessingTest(ImageProcessingTestMixin, unittest.TestCase):
    ...
    fast_image_processing_class = (
        Qwen2VLImageProcessorFast if is_torchvision_available() else None
    )
    ...
    def test_video_inputs(self):
        for image_processing_class in self.image_processor_list:
            ...
            for num_frames, expected_dims in expected_dims_by_frames.items():
                image_processor_tester = Qwen2VLImageProcessingTester(
                    self, num_frames=num_frames
                )
                video_inputs = image_processor_tester.prepare_video_inputs(
                    equal_resolution=True
                )
                prcocess_out = image_processing(
                    None, videos=video_inputs, return_tensors="pt"
                )
                encoded_video = prcocess_out.pixel_values_videos
                expected_output_video_shape = (expected_dims, 1176)
                self.assertEqual(
                    tuple(encoded_video.shape), expected_output_video_shape
                )
