...
if is_vision_available():
    if is_torchvision_available():
        from transformers import VivitVideoProcessor


class VivitVideoProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=5,
...
@require_vision
@require_torchvision
class VivitVideoProcessingTest(VideoProcessingTestMixin, unittest.TestCase):
    fast_video_processing_class = VivitVideoProcessor if is_torchvision_available() else None
    input_name = "pixel_values"

    def setUp(self):
        super().setUp()
        self.video_processor_tester = VivitVideoProcessingTester(self)
    ...
    def test_offset_rescaling(self):
        video_processor = self.fast_video_processing_class(**self.video_processor_dict)
        frames = self.video_processor_tester.prepare_video_inputs(equal_resolution=True, return_tensors="np")
        pil_frames = [Image.fromarray(frame.astype("uint8")) for frame in frames[0]]
        out = video_processor(pil_frames, do_normalize=False, return_tensors="pt")

        pixel_values = out["pixel_values"]
        self.assertGreaterEqual(pixel_values.min().item(), -1.1)
        self.assertLessEqual(pixel_values.max().item(), 1.1)
        self.assertTrue(torch.any(pixel_values < 0))
