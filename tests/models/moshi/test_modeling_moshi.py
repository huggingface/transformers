...
class MoshiDecoderTest(
    ...
    def test_eager_matches_sdpa_inference(self, torch_dtype: str):
        self.skipTest(
            reason="Moshi has no strict equivalence between two modes, skipping this test."
        )
...
class MoshiTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (MoshiForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    @unittest.skip(
        "Adapting this test is costly. `test_eager_matches_sdpa_generate` tests this already."
    )
    @parameterized.expand([("float16",), ("bfloat16",), ("float32",)])
    @require_torch_sdpa
    @slow
    ...
    def test_eager_matches_sdpa_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                self.assertTrue(
                    torch.allclose(res_eager.audio_sequences, res_sdpa.audio_sequences)
                )
