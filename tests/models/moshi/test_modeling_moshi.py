...
def prepare_moshi_decoder_inputs_dict(
    config,
    input_ids,
    attention_mask=None,
    head_mask=None,
...
class MoshiDecoderTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_moshi_decoder_inputs_dict(
            config,
            input_ids,
            encoder_hidden_states=encoder_hidden_states,
        )
...
class MoshiDecoderTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def check_training_gradient_checkpointing(self, gradient_checkpointing_kwargs=None):
        ...
        inputs = self._prepare_for_class(inputs_dict, MoshiForCausalLM, return_labels=True)
        ...
        for k, v in model.named_parameters():
            if v.requires_grad:
                self.assertTrue(v.grad is not None, f"{k} in {MoshiForCausalLM.__name__} has no gradient!")
    ...
    @unittest.skip(reason="Moshi does not use inputs_embeds")
    def test_inputs_embeds_matches_input_ids(self):
        pass

    @unittest.skip(reason="Moshi does not support all arguments tested")
    def test_model_outputs_equivalence(self):
        pass

    @unittest.skip(reason="Moshi has multiple inputs embeds and lm heads that should not be tied")
...
class MoshiTester:
    ...
    def get_config(self):
        ...
        config = MoshiConfig.from_sub_models_config(text_encoder_config, audio_encoder_config, decoder_config)
        return config
...
@require_torch
class MoshiTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {"text-to-audio": MoshiForConditionalGeneration} if is_torch_available() else {}
    ...
    def check_moshi_model_output_attentions(
        self,
        model_class,
        config,
        input_ids,
    ...
    def check_moshi_model_output_attentions_from_config(
        self,
        model_class,
        config,
        input_ids,
        ...
        **kwargs,
        ...
        # Similar to `check_moshi_model_output_attentions`, but with `output_attentions` triggered from the
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            self.check_moshi_model_output_attentions(model_class, config, **inputs_dict)
            self.check_moshi_model_output_attentions_from_config(model_class, config, **inputs_dict)

    # override since we have a specific forward signature for moshi
    ...
    # override since the `input_ids` cannot be used as the `decoder_input_ids` for moshi (input / outputs are
    ...
    @unittest.skip(
        reason="MoshiModel is actually not the base of MoshiForCausalLM as the latter is a composit model"
    )
...
class MoshiIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return MoshiForConditionalGeneration.from_pretrained("kyutai/moshiko").to(torch_device)

    @cached_property
    def processor(self):
        return MoshiProcessor.from_pretrained("kyutai/moshiko")
...
@require_torch
class MoshiStereoIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return MoshiForConditionalGeneration.from_pretrained("facebook/moshi-stereo-small").to(torch_device)

    @cached_property
    def processor(self):
        return MoshiProcessor.from_pretrained("facebook/moshi-stereo-small")
