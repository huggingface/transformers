...
class GPTNeoModelTester:
    ...
    def create_and_check_gpt_neo_model(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        ...
        result = model(input_ids, token_type_ids=token_type_ids, head_mask=head_mask, training=False)
        result = model(input_ids, token_type_ids=token_type_ids, training=False)
    ...
    def create_and_check_gpt_neo_model_past(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        ...
        # first forward pass
        outputs = model(input_ids, token_type_ids=token_type_ids, use_cache=True, training=False)
        outputs_use_cache_conf = model(input_ids, token_type_ids=token_type_ids, training=False)
        outputs_no_past = model(input_ids, token_type_ids=token_type_ids, use_cache=False, training=False)
        ...
        self.parent.assertTrue(
            np.allclose(output_from_past_slice.numpy(), output_from_no_past_slice.numpy(), atol=1e-3)
        )
    ...
    def create_and_check_gpt_neo_model_past_large_inputs(
        ...
        # append to next input_ids and token_type_ids
        next_input_ids = tf.concat([input_ids, next_tokens], axis=-1)
        next_token_type_ids = tf.cohcat([token_type_ids, next_token_types], axis=-1)
...
@require_tf
class GPTNeoModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (TFGPTNeoModel, TFGPTNeoForCausalLM, TFGPTNeoForSequenceClassification) if is_tf_available() else ()
    )
    ...
    def test_local_attn_probs(self):
        model = TFGPTNeoModel.from_pretrained("valhalla/gpt-neo-random-tiny").eval()
...
class TFGPTNeoModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_batch_generation(self):
        ...
        num_paddings = tf.reduce_sum(inputs_non_padded.shape[-1] - inputs["attention_mask"][-1])
    ...
    def test_model_from_pretrained(self):
        for model_name in TF_GPT_NEO_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = TFGPTNeoModel.from_pretrained(model_name, from_pt=True)
