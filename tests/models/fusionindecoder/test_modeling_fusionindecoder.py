# coding=utf-8
# Copyright 2024 Google FusionInDecoder Authors and HuggingFace Inc. team.
#
...
class FusionInDecoderModelTester:
    ...
    def get_pipeline_config(self):
        return FusionInDecoderConfig(
            vocab_size=166,  # fusionindecoder forces 100 extra tokens
    ...
    def check_prepare_lm_labels_via_shift_left(
        ...
        model = FusionInDecoderModel(config=config)
    ...
    def create_and_check_decoder_model_past(
        ...
        model = FusionInDecoderModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = FusionInDecoderModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = FusionInDecoderModel(config=config).to(torch_device).half().eval()
    ...
    def check_resize_embeddings_fusionindecoder_v1_1(
        self,
        config,
    ):
...
class FusionInDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def test_v1_1_resize_embeddings(self):
        ...
        self.model_tester.check_resize_embeddings_fusionindecoder_v1_1(config)

    @slow
    def test_model_from_pretrained(self):
        for model_name in FUSIONINDECODER_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = FusionInDecoderModel.from_pretrained(model_name)
            self.assertIsNotNone(model)

    @unittest.skip("Test has a segmentation fault on torch 1.8.0")
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = FusionInDecoderModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/fusionindecoder_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = FusionInDecoderForConditionalGeneration(config).eval()
        model.to(torch_device)
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from FUSIONINDECODER model when head_mask specified
            if name == "head_mask":
...
@require_tokenizers
@slow
class FusionInDecoderModelFp16Tests(unittest.TestCase):
    def test_fp16_fp32_conversion(self):
        ...
        model = FusionInDecoderForConditionalGeneration.from_pretrained(
            "google-t5/t5-base", torch_dtype=torch.bfloat16, device_map="auto"
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)
        ...
        # Load using `accelerate` in bf16
        model = FusionInDecoderForConditionalGeneration.from_pretrained(
            "google-t5/t5-base", torch_dtype=torch.bfloat16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.bfloat16)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.bfloat16)
        ...
        # Load without using `accelerate`
        model = FusionInDecoderForConditionalGeneration.from_pretrained(
            "google-t5/t5-base", torch_dtype=torch.float16, low_cpu_mem_usage=True
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.float32)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.float16)
        ...
        model = FusionInDecoderForConditionalGeneration.from_pretrained(
            "google-t5/t5-base", torch_dtype=torch.float16, device_map="auto"
        )
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wo.weight.dtype == torch.float32)
        self.assertTrue(model.decoder.block[0].layer[2].DenseReluDense.wi.weight.dtype == torch.float16)
...
class FusionInDecoderModelIntegrationTests(unittest.TestCase):
    ...
    def test_translation_en_to_fr(self):
        model = self.model  # google-fusionindecoder/fusionindecoder-base
        tok = self.tokenizer
    ...
    @slow
    def test_contrastive_search_fusionindecoder(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        outputs = fusionindecoder_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = fusionindecoder_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
@require_torch
class TestAsymmetricFusionInDecoder(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = FusionInDecoderModelTester(self, **kwargs)
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to FusionInDecoderConfig as num_layers
