...
class StableLmModelTest(
    ...
    all_generative_model_classes = (
        (StableLmForCausalLM,) if is_torch_available() else ()
    )
...
class StableLmModelIntegrationTest(unittest.TestCase):
    ...
    def test_model_stablelm_3b_4e1t_logits(self):
        ...
        self.assertTrue(
            torch.allclose(output.mean(dim=-1), EXPECTED_MEAN, atol=1e-4, rtol=1e-4)
        )
        ...
        self.assertTrue(
            torch.allclose(output[0, 0, :30], EXPECTED_SLICE, atol=1e-4, rtol=1e-4)
        )
