...
if is_vision_available() and is_torchvision_available():
    from transformers import CohereCompassVideoProcessor
    from transformers.models.cohere_compass.video_processing_cohere_compass import smart_resize


class CohereCompassVideoProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=5,
...
@require_torch
@require_vision
class CohereCompassVideoProcessingTest(VideoProcessingTestMixin, unittest.TestCase):
    fast_video_processing_class = CohereCompassVideoProcessor if is_torchvision_available() else None
    input_name = "pixel_values_videos"

    def setUp(self):
        super().setUp()
        self.video_processor_tester = CohereCompassVideoProcessingTester(self)
    ...
    @unittest.skip("Skip for now, the test needs adjustment for CohereCompass")
