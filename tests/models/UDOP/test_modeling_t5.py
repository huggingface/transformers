...
class UDOPModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = UDOPModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = UDOPModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = UDOPModel(config=config).to(torch_device).half().eval()
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [UDOPModel, UDOPUnimodelForConditionalGeneration]:
            torch.manual_seed(0)
...
class T5ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = UDOPModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = UDOPUnimodelForConditionalGeneration(config).eval()
        model.to(torch_device)
...
class T5ModelIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tokenizer(self):
        return UdopTokenizer.from_pretrained("t5-base")
