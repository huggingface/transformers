...
class ClvpEncoderTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.encoder_config_tester = ConfigTester(
            self, config_class=ClvpEncoderConfig, hidden_size=32
        )
...
class ClvpDecoderTest(
    ...
    def setUp(self):
        ...
        self.decoder_config_tester = ConfigTester(
            self, config_class=ClvpDecoderConfig, hidden_size=32
        )
    ...
    def test_training(self):
        ...
        inputs = self._prepare_for_class(
            inputs_dict, ClvpForCausalLM, return_labels=True
        )
    ...
    @unittest.skip(
        reason="Clvp `prepare_inputs_for_generation` function doesn't have cache position."
    )
...
class ClvpModelForConditionalGenerationTester:
    def __init__(self, parent, is_training=False):
        ...
        self.is_training = is_training
        self.batch_size = (
            self.clvp_encoder_tester.batch_size
    ...
    def prepare_config_and_inputs(self):
        _, input_ids, attention_mask = (
            self.clvp_encoder_tester.prepare_config_and_inputs()
        )
        ...
        input_features = feature_extractor(
            raw_speech=audio, sampling_rate=sr, return_tensors="pt"
    ...
    def create_and_check_model(self, config, input_ids, attention_mask, input_features):
        ...
        self.parent.assertEqual(
            result.logits_per_text.shape, (self.clvp_encoder_tester.batch_size, 2)
        )
...
class ClvpModelForConditionalGenerationTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (ClvpModelForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    def test_hidden_states_output(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            expected_speech_encoder_num_layers = (
                config.text_config.num_hidden_layers + 1
            )
            self.assertEqual(
                len(text_encoder_hidden_states), expected_speech_encoder_num_layers
            )

            expected_text_encoder_num_layers = (
                config.speech_config.num_hidden_layers + 1
            )
            self.assertEqual(
            ...
            self.assertEqual(
                decoder_hidden_states[0].shape[-1], config.decoder_config.hidden_size
            )
            ...
            self.assertListEqual(
                ...
                [
                    self.model_tester.clvp_encoder_tester.seq_length,
            ...
            self.assertEqual(
                speech_encoder_hidden_states[0].shape[-1],
                config.speech_config.hidden_size,
            )
    ...
    @unittest.skip(
        reason="ClvpModelForConditionalGeneration does not have get_input_embeddings"
    )
    ...
    def test_load_speech_text_decoder_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            self.assertDictEqual(
                config.decoder_config.to_dict(), decoder_config.to_dict()
            )
...
class ClvpIntegrationTest(unittest.TestCase):
    def setUp(self):
        ...
        ds = ds.cast_column("audio", datasets.Audio(sampling_rate=22050))
        _, self.speech_samples, self.sr = (
            ds.sort("id").select(range(1))[:1]["audio"][0].values()
        )

        self.model = ClvpModelForConditionalGeneration.from_pretrained(
            "susnato/clvp_dev"
        ).to(torch_device)
        self.model.eval()
    ...
    def test_conditional_encoder(self):
        ...
        self.assertEqual(
            ...
            torch.Size(
                (
                    ...
                    18,
                    self.model.config.decoder_config.hidden_size,
                )
            ),
        )
        ...
        torch.testing.assert_close(
            conditioning_encoder_outputs[0, :3, :3],
            EXPECTED_OUTPUTS,
            rtol=1e-4,
            atol=1e-4,
        )
    ...
    def test_decoder_model_generate(self):
        autoregressive_model_output = self.model.speech_decoder_model.generate(
            input_ids=self.text_tokens
        ).cpu()

        EXPECTED_OUTPUTS = torch.tensor(
            [[147, 2, 54, 2, 43, 2, 169, 122, 29, 64, 2, 136, 37, 33, 9, 8193]]
        )
    ...
    def test_text_and_speech_encoder_models(self):
        ...
        text_embeds = self.model.text_encoder_model(
            input_ids=self.text_tokens, return_dict=True
        )[0].cpu()
        ...
        torch.testing.assert_close(
            text_embeds[0, :20], EXPECTED_TEXT_EMBEDS, rtol=1e-4, atol=1e-4
        )

        # check for speech embeds
        speech_embeds = self.model.speech_encoder_model(
            input_ids=self.text_tokens, return_dict=True
        )[0].cpu()
        ...
        torch.testing.assert_close(
            speech_embeds[0, :20], EXPECTED_SPEECH_EMBEDS, rtol=1e-4, atol=1e-4
        )
    ...
    def test_full_model_integration(self):
        ...
        EXPECTED_SPEECH_IDS = torch.tensor(
            [[1953, 1080, 612], [1953, 612, 493], [1953, 612, 716]]
        )
        EXPECTED_SIMILARITY_SCORES = torch.tensor(
            [[14.7660, 14.4569, 13.6472, 13.5683]]
        )

        torch.testing.assert_close(
            full_model_output.speech_ids.cpu()[-3:, -3:], EXPECTED_SPEECH_IDS
        )
        torch.testing.assert_close(
            full_model_output.logits_per_text.cpu(), EXPECTED_SIMILARITY_SCORES
        )
