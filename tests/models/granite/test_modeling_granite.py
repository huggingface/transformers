...
from transformers import GraniteConfig, StaticCache, is_torch_available, logging, set_seed
...
class GraniteModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (GraniteModel, GraniteForCausalLM, GraniteForSequenceClassification, GraniteForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    ...
    @unittest.skip("TODO @gante fix this for Granite")
...
class GraniteIntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        ...
        model = GraniteForCausalLM.from_pretrained("meta-granite/Granite-2-7b-hf", device_map="auto")
    ...
    def test_model_13b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = GraniteForCausalLM.from_pretrained("meta-granite/Granite-2-13b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_13bf_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = GraniteForCausalLM.from_pretrained("meta-granite/Granite-2-13b-chat-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_70b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = GraniteForCausalLM.from_pretrained("meta-granite/Granite-2-70b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
    ...
    def test_model_13b_greedy_generation(self):
        ...
        tokenizer = GraniteTokenizer.from_pretrained("meta-granite/Granite-2-13b-chat-hf")
        input_ids = tokenizer.encode(prompt, return_tensors="pt")
        model = GraniteForCausalLM.from_pretrained(
            "meta-granite/Granite-2-13b-chat-hf", device_map="sequential", use_safetensors=False
        )
...
@require_torch
class CodeGraniteIntegrationTest(unittest.TestCase):
    PROMPTS = [
...
<FILL_ME>
    ```
This downloads the Granite inference code and installs the repository as a local pip package.
""",
...
""",
    ...
    def test_model_7b_logits(self):
        model = GraniteForCausalLM.from_pretrained("codegranite/CodeGranite-7b-hf").to(torch_device)
        tokenizer = CodeGraniteTokenizer.from_pretrained("codegranite/CodeGranite-7b-hf")
