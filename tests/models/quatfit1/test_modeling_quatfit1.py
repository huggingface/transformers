...
"""Testing suite for the PyTorch Quatfit1 model."""
...
if is_torch_available():
    ...
    from transformers.models.quatfit1.modeling_quatfit1 import create_masks_for_vision_model


GEMMA4_RANDOM_MOE_FA2_SKIP_REASON = (
    "Randomly initialized Quatfit1 MoE routers are too sensitive to tiny eager/FA2 input differences"
)


class Quatfit1TextModelTester(CausalLMModelTester):
    if is_torch_available():
        config_class = Quatfit1TextConfig
        base_model_class = Quatfit1TextModel
        causal_lm_class = Quatfit1ForCausalLM
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.global_head_dim = self.head_dim  # quatfit1 use a different head_dim for full and sliding layers
...
@require_torch
class Quatfit1TextModelTest(CausalLMModelTest, unittest.TestCase):
    model_tester_class = Quatfit1TextModelTester
    # used in `test_torch_compile_for_training`
    _torch_compile_train_cls = Quatfit1ForCausalLM if is_torch_available() else None

    @unittest.skip("We need 4 layers to correctly test cache sharing.")
    def test_num_layers_is_small(self):
        pass

    @unittest.skip("Quatfit1 uses different rope per layer type, which is not compatible with this test")
    ...
    @unittest.skip(
        "Quatfit1 cannot use random inputs_embeds, as it needs to reverse them when input_ids is not provided"
    )
    ...
    def test_all_bidirectional_attention_uses_bidirectional_mask(self):
        ...
        model = Quatfit1TextModel(config).to(torch_device)
        model.eval()
...
class Quatfit1Audio2TextModelTester:
    def __init__(
        self,
        parent,
        image_token_id=4,
        ...
        self.llm_tester = Quatfit1TextModelTester(self.parent)
...
@require_torch
class Quatfit1Audio2TextModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Quatfit1Model, Quatfit1ForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (Quatfit1ForConditionalGeneration,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = Quatfit1Audio2TextModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Quatfit1Config, hidden_size=37)
    ...
    @unittest.skip("Quatfit1 needs correct embeddings for per-layer-input computation, random won't work!")
    def test_generate_from_random_inputs_embeds(self):
        pass
    ...
    def test_audio_rel_pos_encoding_uses_context_size_from_config(self):
        ...
        from transformers.models.quatfit1.configuration_quatfit1 import Quatfit1AudioConfig
        from transformers.models.quatfit1.modeling_quatfit1 import Quatfit1AudioRelPositionalEncoding

        config = Quatfit1AudioConfig(
            hidden_size=32,
        ...
        module = Quatfit1AudioRelPositionalEncoding(config)
...
class Quatfit1Vision2TextModelTester:
    def __init__(
        self,
        parent,
...
@require_torch
class Quatfit1Vision2TextModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Quatfit1Model, Quatfit1ForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (Quatfit1ForConditionalGeneration,) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = Quatfit1Vision2TextModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Quatfit1Config, hidden_size=37)
    ...
    def skip_flash_attn_inference_equivalence_tests(self):
        ...
        for test in skippable_tests:
            if self._testMethodName.startswith(test):
                self.skipTest(
                    reason="The base test does not pass image_position_ids and mm_token_type_ids required by Quatfit1"
                )
    ...
    def test_training(self):
        ...
        model = Quatfit1ForConditionalGeneration(config)
        model.to(torch_device)
        model.train()
        inputs = self._prepare_for_class(inputs_dict, Quatfit1ForConditionalGeneration, return_labels=True)
    ...
    def test_per_layer_inputs_are_correctly_forwarded(self):
        from transformers.models.quatfit1.modeling_quatfit1 import Quatfit1TextModel

        config, _ = self.model_tester.prepare_config_and_inputs_for_common()

        model = Quatfit1ForConditionalGeneration(config).to(torch_device)
        model.eval()
        ...
        def count_get_per_layer_inputs_calls():
            original = Quatfit1TextModel.get_per_layer_inputs
            ...
            Quatfit1TextModel.get_per_layer_inputs = count_calls
            try:
                yield counter
            finally:
                Quatfit1TextModel.get_per_layer_inputs = original
...
class Quatfit1IntegrationTest(unittest.TestCase):
    ...
    def test_model_with_image(self):
        model = Quatfit1ForConditionalGeneration.from_pretrained(self.model_name, device_map=torch_device)
    ...
    def test_generation_beyond_sliding_window(self, attn_implementation: str):
        ...
        model = Quatfit1ForConditionalGeneration.from_pretrained(
            self.model_name,
