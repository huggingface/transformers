...
if is_pyctcdecode_available():
    from transformers import SpeechT5ProcessorWithLM
...
class SpeechT5ModelTester:
    ...
    def create_and_check_model_with_adapter_for_ctc(self, config, input_values, attention_mask):
        ...
        model = SpeechT5ForCTC(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_values, attention_mask=attention_mask)
        self.parent.assertEqual(
    ...
    def check_seq_classifier_loss(self, config, input_values, *args):
        model = SpeechT5ForSequenceClassification(config=config)
        model.to(torch_device)
    ...
    def check_xvector_training(self, config, input_values, *args):
        config.ctc_zero_infinity = True
        model = SpeechT5ForXVector(config=config)
        model.to(torch_device)
        model.train()
...
class SpeechT5ModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (SpeechT5ForCTC, SpeechT5Model, SpeechT5ForSequenceClassification, SpeechT5ForPreTraining)
        if is_torch_available()
        else ()
    )
    test_pruning = False
    ...
    # SpeechT5 cannot resize token embeddings
    ...
    def test_mask_feature_prob_ctc(self):
        model = SpeechT5ForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-speecht5", mask_feature_prob=0.2, mask_feature_length=2
        )
        model.to(torch_device).train()
        processor = SpeechT5Processor.from_pretrained(
            "hf-internal-testing/tiny-random-speecht5", return_attention_mask=True
        )
    ...
    def test_mask_time_prob_ctc(self):
        model = SpeechT5ForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-speecht5", mask_time_prob=0.2, mask_time_length=2
        )
        model.to(torch_device).train()
        processor = SpeechT5Processor.from_pretrained(
            "hf-internal-testing/tiny-random-speecht5", return_attention_mask=True
    ...
    def test_model_from_pretrained(self):
        model = SpeechT5Model.from_pretrained("TODO")
        self.assertIsNotNone(model)


@require_torch
class SpeechT5RobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            SpeechT5ForCTC,
            SpeechT5Model,
    ...
    def setUp(self):
        self.model_tester = SpeechT5ModelTester(
    ...
    def test_model_for_pretraining(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        model = SpeechT5ForPreTraining(config).to(torch_device)
    ...
    def test_mask_time_feature_prob_ctc_single_batch(self):
        model = SpeechT5ForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-speecht5",
            mask_time_prob=0.2,
...
@require_torch
class SpeechT5UtilsTest(unittest.TestCase):
    ...
    def test_compute_perplexity(self):
        ...
        ppl = SpeechT5GumbelVectorQuantizer._compute_perplexity(probs)
        ...
        ppl = SpeechT5GumbelVectorQuantizer._compute_perplexity(probs, mask)
...
@require_soundfile
@slow
class SpeechT5ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_normal(self):
        ...
        model.to(torch_device)
        processor = SpeechT5Processor.from_pretrained("TODO", do_lower_case=True)
    ...
    def test_inference_ctc_robust_batched(self):
        model = SpeechT5ForCTC.from_pretrained("facebook/speecht5-large-960h-lv60-self").to(torch_device)
        processor = SpeechT5Processor.from_pretrained("facebook/speecht5-large-960h-lv60-self", do_lower_case=True)
    ...
    def test_inference_pretrained(self):
        ...
        config = SpeechT5Config.from_pretrained("facebook/speecht5-base")
        model_rand = SpeechT5ForPreTraining(config).to(torch_device).eval()
        ...
        # a pretrained speecht5 model has learned to predict the quantized latent states
        # => the cosine similarity between quantized states and predicted states > 0.5
        # a random speecht5 model has not learned to predict the quantized latent states
        # => the cosine similarity between quantized states and predicted states is very likely < 0.1
        self.assertTrue(cosine_sim_masked.mean().item() - 5 * cosine_sim_masked_rand.mean().item() > 0)

    @unittest.skipIf(torch_device != "cpu", "cannot make deterministic on GPU")
    def test_loss_pretraining(self):
        model = SpeechT5ForPreTraining.from_pretrained(
            "facebook/speecht5-base",
    ...
    def test_phoneme_recognition(self):
        model = SpeechT5ForCTC.from_pretrained("facebook/speecht5-lv-60-espeak-cv-ft").to(torch_device)
        processor = SpeechT5Processor.from_pretrained("facebook/speecht5-lv-60-espeak-cv-ft")
    ...
    @require_pyctcdecode
    @require_torchaudio
    def test_speecht5_with_lm(self):
        ...
        model = SpeechT5ForCTC.from_pretrained("patrickvonplaten/speecht5-large-xlsr-53-spanish-with-lm").to(
            torch_device
        )
        processor = SpeechT5ProcessorWithLM.from_pretrained("patrickvonplaten/speecht5-large-xlsr-53-spanish-with-lm")
