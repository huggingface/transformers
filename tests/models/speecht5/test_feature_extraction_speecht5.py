...
from transformers import SPEECHT5_PRETRAINED_MODEL_ARCHIVE_LIST, SpeechT5Config, SpeechT5FeatureExtractor
...
class SpeechT5FeatureExtractionTest(SequenceFeatureExtractionTestMixin, unittest.TestCase):
    ...
    def test_pretrained_checkpoints_are_set_correctly(self):
        ...
        # attention_mask
        for model_id in SPEECHT5_PRETRAINED_MODEL_ARCHIVE_LIST:
            config = SpeechT5Config.from_pretrained(model_id)
            feat_extract = SpeechT5FeatureExtractor.from_pretrained(model_id)
