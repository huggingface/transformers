...
from transformers import Speecht5Config, is_tf_available
...
if is_tf_available():
    ...
    from transformers import TFSpeecht5ForCTC, TFSpeecht5Model, Wav2Vec2Processor
    from transformers.models.speecht5.modeling_tf_speecht5 import _compute_mask_indices
...
class TFSpeecht5ModelTester:
    ...
    def check_ctc_loss(self, config, input_values, *args):
        model = TFSpeecht5ForCTC(config)

        input_values = input_values[:3]
        attention_mask = tf.ones_like(input_values)

        input_lengths = tf.constant([input_values.shape[-1] // i for i in [4, 2, 1]])
        max_length_labels = model.speecht5._get_feat_extract_output_lengths(input_lengths)
...
@require_tf
class TFSpeecht5ModelTest(TFModelTesterMixin, unittest.TestCase):

    all_model_classes = (TFSpeecht5Model, TFSpeecht5ForCTC) if is_tf_available() else ()
    test_resize_embeddings = False
    test_head_masking = False
    test_onnx = False

    def setUp(self):
        self.model_tester = TFSpeecht5ModelTester(self)
    ...
    def test_model_from_pretrained(self):
        model = TFSpeecht5Model.from_pretrained("")
        self.assertIsNotNone(model)


@require_tf
class TFSpeecht5RobustModelTest(TFModelTesterMixin, unittest.TestCase):
    all_model_classes = (TFSpeecht5Model, TFSpeecht5ForCTC) if is_tf_available() else ()
    test_resize_embeddings = False
    test_head_masking = False
    test_onnx = False
    ...
    def setUp(self):
        self.model_tester = TFSpeecht5ModelTester(
            self,
            conv_stride=(3, 3, 3),
...
@require_tf
class TFSpeecht5UtilsTest(unittest.TestCase):
...
@require_tf
@slow
class TFSpeecht5ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_normal(self):
        model = TFSpeecht5ForCTC.from_pretrained("")
    ...
    def test_inference_ctc_robust_batched(self):
        model = TFSpeecht5ForCTC.from_pretrained("facebook/speecht5-large-960h-lv60-self")
    ...
    def test_speecht5_with_lm(self):
        ...
        model = TFSpeecht5ForCTC.from_pretrained("patrickvonplaten/speecht5-large-xlsr-53-spanish-with-lm")
