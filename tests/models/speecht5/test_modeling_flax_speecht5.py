...
from transformers import Speecht5Config, is_flax_available
...
if is_flax_available():
    ...
    from transformers.models.speecht5.modeling_flax_speecht5 import (
        FlaxSpeecht5ForCTC,
        FlaxSpeecht5ForPreTraining,
        FlaxSpeecht5GumbelVectorQuantizer,
        FlaxSpeecht5Model,
...
@require_flax
class FlaxSpeecht5ModelTest(FlaxModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (FlaxSpeecht5Model, FlaxSpeecht5ForCTC, FlaxSpeecht5ForPreTraining) if is_flax_available() else ()
    )

    def setUp(self):
        self.model_tester = FlaxSpeecht5ModelTester(self)
    ...
    def test_train(self):
        ...
        model = FlaxSpeecht5ForPreTraining(config)
    ...
    def test_model_from_pretrained(self):
        for model_class_name in self.all_model_classes:
            model = model_class_name.from_pretrained("facebook/speecht5-large-960h-lv60-self", from_pt=True)
            outputs = model(np.ones((1, 1024), dtype="f4"))
            self.assertIsNotNone(outputs)


@require_flax
class FlaxSpeecht5UtilsTest(unittest.TestCase):
    ...
    def test_compute_perplexity(self):
        ...
        ppl = FlaxSpeecht5GumbelVectorQuantizer._compute_perplexity(probs)
        ...
        ppl = FlaxSpeecht5GumbelVectorQuantizer._compute_perplexity(probs, mask)
...
@require_soundfile
@slow
class FlaxSpeecht5ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_robust_batched(self):
        model = FlaxSpeecht5ForCTC.from_pretrained("facebook/speecht5-large-960h-lv60-self", from_pt=True)
    ...
    def test_inference_pretrained(self):
        model = FlaxSpeecht5ForPreTraining.from_pretrained("facebook/speecht5-large-lv60", from_pt=True)
        feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(
            "facebook/speecht5-large-lv60", return_attention_mask=True
        )
        ...
        config = Speecht5Config.from_pretrained("facebook/speecht5-large-lv60")
        model_rand = FlaxSpeecht5ForPreTraining(config)
    ...
    def test_speecht5_with_lm(self):
        ...
        model = FlaxSpeecht5ForCTC.from_pretrained("patrickvonplaten/speecht5-large-xlsr-53-spanish-with-lm")
