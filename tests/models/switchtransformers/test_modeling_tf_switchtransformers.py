...
from transformers import SwitchTransformersConfig, is_tf_available
...
class TFSwitchTransformersModelTester:
    ...
    def create_and_check_switchtransformers_model(self, config, input_ids, input_mask, token_labels):
        model = TFSwitchTransformersModel(config=config)
        inputs = {
            "input_ids": input_ids,
    ...
    def create_and_check_switchtransformers_with_lm_head(self, config, input_ids, input_mask, token_labels):
        model = TFSwitchTransformersForConditionalGeneration(config=config)
        inputs_dict = {
            "input_ids": input_ids,
            "decoder_input_ids": input_ids,
            "decoder_attention_mask": input_mask,
    ...
    def create_and_check_switchtransformers_decoder_model_attention_mask_past(
    ...
    def create_and_check_switchtransformers_decoder_model_past_large_inputs(
        self, config, input_ids, decoder_input_ids, attention_mask
    ):
        model = TFSwitchTransformersModel(config=config).get_decoder()
...
class TFSwitchTransformersModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        self.model_tester = TFSwitchTransformersModelTester(self)
        self.config_tester = ConfigTester(self, config_class=SwitchTransformersConfig, d_model=37)

    def test_config(self):
        self.config_tester.run_common_tests()

    def test_switchtransformers_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_switchtransformers_model(*config_and_inputs)

    def test_switchtransformers_model_v1_1(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        config = config_and_inputs[0]
        config.tie_word_embeddings = False
        config.feed_forward_proj = "gated-gelu"
        self.model_tester.create_and_check_switchtransformers_model(config, *config_and_inputs[1:])

    def test_with_lm_head(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_switchtransformers_with_lm_head(*config_and_inputs)

    def test_switchtransformers_decoder_model_past(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_switchtransformers_decoder_model_past(*config_and_inputs)

    def test_switchtransformers_decoder_model_past_with_attn_mask(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_switchtransformers_decoder_model_attention_mask_past(*config_and_inputs)

    def test_switchtransformers_decoder_model_past_large_inputs(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()

        # `create_and_check_switchtransformers_decoder_model_past_large_inputs` has special inputs:
        ...
        self.model_tester.create_and_check_switchtransformers_decoder_model_past_large_inputs(*config_and_inputs)
    ...
    def test_generate_with_headmasking(self):
        # TODO: Fix head-masking according to PyTorch SwitchTransformers model
        pass
    ...
    # This test is run in `TFSwitchTransformersEncoderOnlyModelTest`, where the main layer has the same inputs as the model
    @unittest.skip(reason="The inputs of the Main Layer are different.")
    def test_keras_save_load(self):
        pass


class TFSwitchTransformersEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_model(
        ...
        model = TFSwitchTransformersEncoderModel(config=config)
        result = model(
            input_ids=input_ids,
...
class TFSwitchTransformersEncoderOnlyModelTest(TFModelTesterMixin, unittest.TestCase):
    is_encoder_decoder = False
    all_model_classes = (TFSwitchTransformersEncoderModel,) if is_tf_available() else ()
    test_onnx = False

    def setUp(self):
        self.model_tester = TFSwitchTransformersEncoderOnlyModelTester(self)
        self.config_tester = ConfigTester(self, config_class=SwitchTransformersConfig, d_model=37)
...
@require_tokenizers
class TFSwitchTransformersGenerationIntegrationTests(unittest.TestCase):
    @slow
...
@require_sentencepiece
@require_tokenizers
class TFSwitchTransformersModelIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return TFSwitchTransformersForConditionalGeneration.from_pretrained("switchtransformers-base")
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_switchtransformers_v1.1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> switchtransformers_model = switchtransformers.models.MtfModel(model_dir=path_to_mtf_small_switchtransformers_v1.1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = switchtransformers_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = TFSwitchTransformersForConditionalGeneration.from_pretrained("google/switchtransformers-v1_1-small")
