...
def compute_nemo_reference(inputs):
    """Run NeMo's TDTLossPytorch (monkey-patched for CPU)."""
    import nemo.collections.asr.losses.rnnt_pytorch as rnnt_mod

    # NeMo hardcodes .cuda() — patch compute_forward_prob for CPU
    def patched_compute(self, acts, duration_acts, labels, act_lens, label_lens):
        B, T, U, _ = acts.shape
    ...
    rnnt_mod.TDTLossPytorch.compute_forward_prob = patched_compute
...
def main():
    ...
    fixture = {
        "_comment": "Generated by generate_tdt_loss_fixtures.py using NeMo's TDTLossPytorch (CPU-patched). "
