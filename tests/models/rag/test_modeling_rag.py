...
from transformers.testing_utils import (cleanup, get_tests_dir,
...
def require_retrieval(test_case):
    ...
    if not (is_torch_available() and is_datasets_available() and is_faiss_available()):
        test_case = unittest.skip(reason="test requires PyTorch, datasets and faiss")(
            test_case
        )
    return test_case
...
class RagDPRBartTest(RagTestMixin, unittest.TestCase):
    ...
    def config_and_inputs(self):
        ...
        (question_encoder_config, input_ids, _, input_mask, _, _, _) = (
...
class RagDPRT5Test(RagTestMixin, unittest.TestCase):
    ...
    def config_and_inputs(self):
        ...
        (generator_config, _, decoder_input_ids, _, decoder_attention_mask, _) = (
