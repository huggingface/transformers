...
@require_retrieval
@require_sentencepiece
class AtlasTestMixin:

    all_model_classes = (
        (AtlasModel, AtlasTokenForGeneration, AtlasSequenceForGeneration)
        if is_torch_available() and is_datasets_available() and is_faiss_available()
        else ()
    )
    ...
    def get_retriever(self, config):
        ...
        with patch("transformers.models.atlas.retrieval_atlas.load_dataset") as mock_load_dataset:
    ...
    def check_model_with_end2end_retriever(
        ...
        for model_class in [AtlasTokenForGeneration, AtlasSequenceForGeneration]:
...
@require_torch
@require_retrieval
class AtlasDPRBartTest(AtlasTestMixin, unittest.TestCase):
    @cached_property
...
@require_torch
@require_retrieval
class AtlasDPRT5Test(AtlasTestMixin, unittest.TestCase):
    @cached_property
    def config_and_inputs(self):
        question_encoder_tester = DPRModelTester(self)
        dpr_config_and_inputs = question_encoder_tester.prepare_config_and_inputs()
...
class AtlasModelIntegrationTests(unittest.TestCase):
    ...
    def sequence_model(self):
        return (
            AtlasSequenceForGeneration.from_pretrained_question_encoder_generator(
    ...
    def token_model(self):
        return (
            AtlasTokenForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base", "facebook/bart-large-cnn"
            )
            .to(torch_device)
            .eval()
    ...
    @slow
    def test_atlas_token_inference(self):
        atlas_config = self.get_atlas_config()
        atlas_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        atlas_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
    ...
    @slow
    def test_atlas_token_generate_beam(self):
        atlas_config = self.get_atlas_config()
        atlas_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        atlas_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        output_ids = atlas_token.generate(
            input_ids,
            decoder_start_token_id=atlas_token.generator.config.decoder_start_token_id,
            num_beams=2,
            num_return_sequences=2,
        )
    ...
    def test_data_questions(self):
        return [
            ...
            "how many episodes are there in datlason ball z",
        ]
    ...
    def test_atlas_sequence_generate_batch_from_context_input_ids(self):
        ...
        question_hidden_states = atlas_sequence.question_encoder(input_ids, attention_mask=attention_mask)[0]
        docs_dict = retriever(
...
@require_torch
@require_retrieval
class AtlasModelSaveLoadTests(unittest.TestCase):
    def tearDown(self):
        super().tearDown()
        # clean-up as much as possible GPU memory occupied by PyTorch
        gc.collect()
    ...
    @slow
    def test_atlas_sequence_from_pretrained(self):
        atlas_config = self.get_atlas_config()
        atlas_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        atlas_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            atlas_sequence = AtlasSequenceForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base",
            ...
            atlas_sequence.save_pretrained(tmp_dirname)
            atlas_sequence.from_pretrained(tmp_dirname, retriever=atlas_retriever)
            atlas_sequence.to(torch_device)
        ...
        atlas_sequence = AtlasSequenceForGeneration(
            config=atlas_config, question_encoder=question_encoder, generator=generator, retriever=atlas_retriever
        )
        atlas_sequence.to(torch_device)
    ...
    @slow
    def test_atlas_token_from_pretrained(self):
        atlas_config = self.get_atlas_config()
        atlas_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        atlas_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            atlas_token = AtlasTokenForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base",
                "facebook/bart-large-cnn",
                retriever=atlas_retriever,
                config=atlas_config,
            ...
            # check that the from pretrained methods work
            atlas_token.save_pretrained(tmp_dirname)
            atlas_token.from_pretrained(tmp_dirname, retriever=atlas_retriever)
            atlas_token.to(torch_device)

            self.assertTrue(atlas_token.question_encoder.config.max_length == 200)
            self.assertTrue(atlas_token.generator.config.max_length == 200)
        ...
        question_encoder = AutoModel.from_pretrained("facebook/dpr-question_encoder-single-nq-base")
        generator = AutoModelForSeq2SeqLM.from_pretrained("facebook/bart-large-cnn")
        atlas_token = AtlasTokenForGeneration(
            config=atlas_config, question_encoder=question_encoder, generator=generator, retriever=atlas_retriever
        )
        atlas_token.to(torch_device)
