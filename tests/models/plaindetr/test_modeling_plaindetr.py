...
"""Testing suite for the PyTorch PLAINDETR model."""

import inspect
import math
import unittest

from transformers import PlaindetrConfig, ResNetConfig, is_torch_available, is_vision_available
...
if is_torch_available():
    ...
    from transformers import PlaindetrForObjectDetection, PlaindetrForSegmentation, PlaindetrModel
...
class PlaindetrModelTester:
    ...
    def create_and_check_plaindetr_model(self, config, pixel_values, pixel_mask, labels):
        model = PlaindetrModel(config=config)
    ...
    def create_and_check_plaindetr_object_detection_head_model(self, config, pixel_values, pixel_mask, labels):
        model = PlaindetrForObjectDetection(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class PlaindetrModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            PlaindetrModel,
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ in ["PlaindetrForObjectDetection", "PlaindetrForSegmentation"]:
                labels = []
    ...
    def setUp(self):
        self.model_tester = PlaindetrModelTester(self)
        self.config_tester = ConfigTester(self, config_class=PlaindetrConfig, has_text_modality=False)

    def test_config(self):
        self.config_tester.run_common_tests()

    def test_plaindetr_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_plaindetr_model(*config_and_inputs)

    def test_plaindetr_object_detection_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_plaindetr_object_detection_head_model(*config_and_inputs)
    ...
    @unittest.skip(reason="PLAINDETR does not have a get_input_embeddings method")
    def test_model_get_set_embeddings(self):
        pass

    @unittest.skip(reason="PLAINDETR is not a generative model")
    def test_generate_without_input_ids(self):
        pass

    @unittest.skip(reason="PLAINDETR does not use token embeddings")
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if self.is_encoder_decoder:
                ...
                if model_class.__name__ == "PlaindetrForObjectDetection":
                    correct_outlen += 2
                # Panoptic Segmentation model returns pred_logits, pred_boxes, pred_masks
                if model_class.__name__ == "PlaindetrForSegmentation":
                    correct_outlen += 3
    ...
    def test_different_timm_backbone(self):
        ...
        for model_class in self.all_model_classes:
            ...
            elif model_class.__name__ == "PlaindetrForSegmentation":
                # Confirm out_indices was propogated to backbone
                self.assertEqual(len(model.plaindetr.model.backbone.conv_encoder.intermediate_channel_sizes), 3)
            else:
                # Confirm out_indices was propogated to backbone
...
@require_vision
@slow
class PlaindetrModelIntegrationTestsTimmBackbone(unittest.TestCase):
    @cached_property
    def default_image_processor(self):
        return DetrImageProcessor.from_pretrained("sushmanth/plaindetr") if is_vision_available() else None

    def test_inference_no_head(self):
        model = PlaindetrModel.from_pretrained("sushmanth/plaindetr").to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        model = PlaindetrForObjectDetection.from_pretrained("sushmanth/plaindetr").to(torch_device)
    ...
    def test_inference_panoptic_segmentation_head(self):
        model = PlaindetrForSegmentation.from_pretrained("sushmanth/plaindetr-panoptic").to(torch_device)
...
@require_torch
@slow
class PlaindetrModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_image_processor(self):
        return (
            DetrImageProcessor.from_pretrained("sushmanth/plaindetr", revision="no_timm")
            if is_vision_available()
            else None
        )

    def test_inference_no_head(self):
        model = PlaindetrModel.from_pretrained("sushmanth/plaindetr", revision="no_timm").to(torch_device)
