...
class FastViTModelTester:
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        model = FastViTModel(config=config)
...
@require_torch
class FastViTModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as FastViT does not use input_ids, inputs_embeds,
    ...
    @unittest.skip(reason="FastViT does not use inputs_embeds")
    ...
    def test_model_from_pretrained(self):
        for model_name in FASTVIT_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = FastViTModel.from_pretrained(model_name)
