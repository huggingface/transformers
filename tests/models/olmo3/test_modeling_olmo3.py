...
class Olmo3ModelTest(CausalLMModelTest, unittest.TestCase):
    ...
    def test_model_rope_scaling_from_config(self, scaling_type):
        ...
        config.rope_parameters = {"full_attention": config.rope_parameters["sliding_attention"]}
        ...
        set_seed(42)  # Fixed seed at init time so the two models get the same random weights
        rope_parameters = {"rope_type": scaling_type, "factor": 10.0, "rope_theta": 10_000.0}
        config.rope_parameters = {layer_type: rope_parameters.copy() for layer_type in config.layer_types}
    ...
    def test_model_rope_scaling_frequencies(self):
        ...
        config.rope_parameters = {k: rope_parameters.copy() for k in config.layer_types}
        ...
        rope_parameters = {"rope_type": "linear", "factor": scaling_factor, "rope_theta": 10_000.0}
        config.rope_parameters = {k: rope_parameters.copy() for k in config.layer_types}
        ...
        rope_parameters = {"rope_type": "dynamic", "factor": scaling_factor, "rope_theta": 10_000.0}
        config.rope_parameters = {k: rope_parameters.copy() for k in config.layer_types}
        ...
        rope_parameters = {"rope_type": "yarn", "factor": scaling_factor, "rope_theta": 10_000.0}
        config.rope_parameters = {k: rope_parameters.copy() for k in config.layer_types}
