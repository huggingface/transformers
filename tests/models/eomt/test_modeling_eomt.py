...
if is_torch_available():
    ...
    from transformers import EoMTBackbone, EoMTForImageClassification, EoMTModel
...
class EoMTModelTester:
    def __init__(
        ...
        # in EoMT, the seq length equals the number of patches + 1 (we add 1 for the [CLS] token)
    ...
    def create_and_check_backbone(self, config, pixel_values, labels):
        model = EoMTBackbone(config=config)
        model.to(torch_device)
        model.eval()
        result = model(pixel_values)
    ...
    def create_and_check_for_image_classification(self, config, pixel_values, labels):
        ...
        model = EoMTForImageClassification(config)
        model.to(torch_device)
        model.eval()
...
class EoMTModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as EoMT does not use input_ids, inputs_embeds,
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=EoMTConfig, has_text_modality=False, hidden_size=37)
    ...
    @unittest.skip(reason="EoMT does not support feedforward chunking yet")
    ...
    def test_model_from_pretrained(self):
        ...
        model = EoMTModel.from_pretrained(model_name)
...
@require_torch
@require_vision
class EoMTModelIntegrationTest(unittest.TestCase):
    @cached_property
    def default_image_processor(self):
        return (
            AutoImageProcessor.from_pretrained("tue-mps/coco_panoptic_eomt_large_640")
            if is_vision_available()
            else None
        )
    ...
    def test_inference_no_head(self):
        model = EoMTModel.from_pretrained("tue-mps/coco_panoptic_eomt_large_640").to(torch_device)
...
@require_torch
class EoMTBackboneTest(unittest.TestCase, BackboneTesterMixin):
    all_model_classes = (EoMTBackbone,) if is_torch_available() else ()
