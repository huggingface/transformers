...
class TimesFMModelTester:
    ...
    def get_large_model_config(self):
        return TimesFMConfig.from_pretrained("google-timesfm/timesfm-base")
    ...
    def get_pipeline_config(self):
        return TimesFMConfig(
            vocab_size=166,  # timesfm forces 100 extra tokens
    ...
    def check_prepare_lm_labels_via_shift_left(
        ...
        model = TimesFMModel(config=config)
    ...
    def create_and_check_decoder_model_past(
        ...
        model = TimesFMModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = TimesFMModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = TimesFMModel(config=config).to(torch_device).half().eval()
    ...
    def check_resize_embeddings_timesfm_v1_1(
        self,
        config,
    ):
...
class TimesFMModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    # The small TimesFM model needs higher percentages for CPU/MP tests
    ...
    # TimesFMForSequenceClassification does not support inputs_embeds
    ...
    def test_v1_1_resize_embeddings(self):
        ...
        self.model_tester.check_resize_embeddings_timesfm_v1_1(config)

    @slow
    def test_model_from_pretrained(self):
        model_name = "google/timesfm-1.0-200m"
        model = TimesFMModel.from_pretrained(model_name)
        self.assertIsNotNone(model)

    @unittest.skip(reason="Test has a segmentation fault on torch 1.8.0")
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = TimesFMModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/timesfm_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from TimesFM model when head_mask specified
            if name == "head_mask":
...
class TimesFMEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
...
@require_tokenizers
@slow
class TimesFMModelFp16Tests(unittest.TestCase):
...
class TimesFMModelIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tokenizer(self):
        return T5Tokenizer.from_pretrained("google-timesfm/timesfm-base")

    @slow
    def test_torch_quant(self):
        r"""
        Test that a simple `torch.quantization.quantize_dynamic` call works on a TimesFM model.
        """
    ...
    def test_small_integration_test(self):
        """
        For comparision run:
        >>> import timesfm  # pip install timesfm==0.7.1
        >>> from timesfm.data.sentencepiece_vocabulary import SentencePieceVocabulary

        >>> path_to_mtf_small_timesfm_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> timesfm_model = timesfm.models.MtfModel(model_dir=path_to_mtf_small_timesfm_checkpoint, batch_size=1, tpu=None)
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_timesfm_v1_1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> timesfm_model = timesfm.models.MtfModel(model_dir=path_to_mtf_small_timesfm_v1_1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = timesfm_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    @slow
    def test_small_bytimesfm_integration_test(self):
        """
        For comparision run:
        >>> import timesfm  # pip install timesfm==0.9.1

        >>> path_to_bytimesfm_small_checkpoint = '<fill_in>'
        >>> timesfm_model = timesfm.models.MtfModel(model_dir=path_to_tf_checkpoint, batch_size=1, tpu=None)
        >>> vocab = timesfm.data.ByteVocabulary()
        >>> score = timesfm_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_translation_en_to_fr(self):
        model = self.model  # google-timesfm/timesfm-base
        tok = self.tokenizer
    ...
    @slow
    def test_contrastive_search_timesfm(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        outputs = timesfm_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = timesfm_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
@require_torch
class TestAsymmetricTimesFM(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = TimesFMModelTester(self, **kwargs)
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to TimesFMConfig as num_layers
