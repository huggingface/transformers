...
class HieraModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        ...
        self.assertTrue(
            torch.allclose(
                inputs.pixel_values[0, :3, :3, :3], expected_pixel_values, atol=1e-4
            )
        )
        ...
        self.assertTrue(
            torch.allclose(outputs.logits[0, :5], expected_slice, atol=1e-4)
        )
    ...
    def test_inference_for_pretraining(self):
        ...
        self.assertTrue(
            torch.allclose(
                outputs.logits[0, :5, :5], expected_slice.to(torch_device), atol=1e-4
            )
        )
