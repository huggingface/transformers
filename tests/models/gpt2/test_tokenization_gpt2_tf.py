...
if is_tf_available():
    ...
    class ModelToSave(tf.Module):
        ...
        def serving(self, text):
            ...
            outputs = self.model(input_ids=input_ids_dense, attention_mask=input_mask)[
                "logits"
            ]
...
class GPTTokenizationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tokenizers = [
            GPT2Tokenizer.from_pretrained(checkpoint)
            for checkpoint in (TOKENIZER_CHECKPOINTS)
        ]
        self.tf_tokenizers = [
            TFGPT2Tokenizer.from_pretrained(checkpoint)
            for checkpoint in TOKENIZER_CHECKPOINTS
        ]
        ...
        for tokenizer, tf_tokenizer in zip(self.tokenizers, self.tf_tokenizers):
            for test_inputs in self.test_sentences:
                ...
                for key in python_outputs.keys():
                    ...
                    self.assertTrue(
                        tf.reduce_all(
                            python_outputs_values.shape == tf_outputs_values.shape
                        )
                    )
                    self.assertTrue(
                        tf.reduce_all(
                            tf.cast(python_outputs_values, tf.int64)
                            == tf_outputs_values
                        )
                    )
    ...
    def test_saved_model(self):
        for tf_tokenizer in self.tf_tokenizers:
            ...
            with TemporaryDirectory() as tempdir:
                ...
                tf.saved_model.save(
                    model, save_path, signatures={"serving_default": model.serving}
                )
                loaded_model = tf.saved_model.load(save_path)
            loaded_output = loaded_model.signatures["serving_default"](test_inputs)[
                "output_0"
            ]
