...
class NarrowBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_sequence_classification(self):
        model = NarrowBertForSequenceClassification.from_pretrained("lihaoxin2020/narrowbert-sparse_attn-mnli")
        input_ids = torch.tensor([[0, 1, 2, 3, 4, 5]])
        output = model(input_ids, output_hidden_states=True).logits
