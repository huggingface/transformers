...
from transformers import GPT2Tokenizer, OPTConfig, is_torch_available, pipeline
...
if is_torch_available():
    ...
    from transformers.models.opt.modeling_opt import OPTDecoder, OPTEncoder, shift_tokens_right
...
class OPTHeadTests(unittest.TestCase):
    ...
    def test_question_answering_forward(self):
        ...
        model = OPTForQuestionAnswering(config)
        model.to(torch_device)
        outputs = model(
            input_ids=input_ids,
    ...
    def test_lm_forward(self):
        ...
        lm_model = OPTForConditionalGeneration(config)
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = OPTForConditionalGeneration(config).to(torch_device)
...
class OPTModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (OPTModel, OPTForConditionalGeneration, OPTForSequenceClassification, OPTForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (OPTForConditionalGeneration,) if is_torch_available() else ()
    ...
    def test_inputs_embeds(self):
        ...
        for model_class in (OPTModel, OPTForConditionalGeneration, OPTForQuestionAnswering):
            model = model_class(config)
            model.to(torch_device)
            model.eval()
...
class FastIntegrationTests(unittest.TestCase):
    ...
    def xsum_1_1_model(self):
        return OPTForConditionalGeneration.from_pretrained("sshleifer/distilopt-xsum-1-1")
...
class OPTModelIntegrationTests(unittest.TestCase):
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = OPTForConditionalGeneration.from_pretrained("facebook/opt-large-xsum").to(torch_device)
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = OPTForConditionalGeneration.from_pretrained("facebook/opt-large-cnn").to(torch_device)
        tok = OPTTokenizer.from_pretrained("facebook/opt-large")
