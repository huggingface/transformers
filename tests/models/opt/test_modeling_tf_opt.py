...
if is_tf_available():
    ...
    from transformers import TFOPTForConditionalGeneration, TFOPTModel
...
class TFOPTModelTest(TFModelTesterMixin, TFCoreModelTesterMixin, unittest.TestCase):
    all_model_classes = (TFOPTForConditionalGeneration, TFOPTModel) if is_tf_available() else ()
    all_generative_model_classes = (TFOPTForConditionalGeneration,) if is_tf_available() else ()
...
class TFOPTHeadTests(unittest.TestCase):
    ...
    def test_lm_forward(self):
        ...
        lm_model = TFOPTForConditionalGeneration(config)
...
@slow
@require_tf
class TFOPTModelIntegrationTest(unittest.TestCase):
    def test_inference_no_head(self):
        model = TFOPTForConditionalGeneration.from_pretrained("facebook/opt-large").model
    ...
    def test_cnn_summarization_same_as_fairseq_hard(self):
        hf = TFOPTForConditionalGeneration.from_pretrained("facebook/opt-large-cnn")
        tok = self.tok
...
@slow
@require_tf
class FasterTFOPTModelIntegrationTests(unittest.TestCase):
    ...
    def xsum_1_1_model(self):
        return TFOPTForConditionalGeneration.from_pretrained("sshleifer/distilopt-xsum-1-1")
