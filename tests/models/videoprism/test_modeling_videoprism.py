...
if is_torch_available():
    ...
    from transformers import MODEL_FOR_VIDEO_CLASSIFICATION_MAPPING, VideoPrismForVideoClassification, VideoPrismModel


if is_vision_available():
    from transformers import VideoPrismImageProcessor


class VideoPrismModelTester:
    def __init__(
        self,
        parent,
        batch_size=2,
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        model = VideoPrismModel(config=config)
    ...
    def create_and_check_for_video_classification(self, config, pixel_values, labels):
        model = VideoPrismForVideoClassification(config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class VideoPrismModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as VideoPrism does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (VideoPrismModel, VideoPrismForVideoClassification) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = VideoPrismModelTester(self)
        self.config_tester = ConfigTester(self, config_class=VideoPrismConfig, has_text_modality=False, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        ...
        model = VideoPrismModel.from_pretrained(model_name)
...
class VideoPrismModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        return VideoPrismImageProcessor() if is_vision_available() else None

    @slow
    def test_inference_for_video_classification(self):
        model = VideoPrismForVideoClassification.from_pretrained("google/videoprism-base-f16r288").to(torch_device)
    ...
    def test_inference_interpolate_pos_encoding(self):
        # VideoPrism models have an `interpolate_pos_encoding` argument in their forward method,
        # allowing to interpolate the pre-trained position embeddings in order to use
        # the model on higher resolutions. The DINO model by Facebook AI leverages this
        # to visualize self-attention on higher resolution images.
        model = VideoPrismModel.from_pretrained("google/videoprism-base-f16r288").to(torch_device)

        image_processor = VideoPrismImageProcessor.from_pretrained("google/videoprism-base-f16r288")
        video = prepare_video()
