...
from transformers import NemotronHDenseConfig, NemotronHDenseForCausalLM, is_torch_available
...
if is_torch_available():
    ...
    from transformers import DynamicCache, NemotronHDenseForCausalLM, NemotronHDenseModel


class NemotronHDenseModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def create_and_check_model(self, config, input_ids, input_mask, _sequence_labels, _token_labels, _choice_labels):
        model = NemotronHDenseModel(config=config)
    ...
    def create_and_check_for_causal_lm(
        ...
        model = NemotronHDenseForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_mamba2_slow_vs_fast_forward(self, config, input_ids, *args):
        model = NemotronHDenseModel(config)
        model.eval()
...
@require_torch
class NemotronHDenseModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            NemotronHDenseModel,
    ...
    pipeline_model_mapping = (
        {
            "feature-extraction": NemotronHDenseModel,
            "text-generation": NemotronHDenseForCausalLM,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = NemotronHDenseModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=NemotronHDenseConfig, common_properties=["hidden_size", "num_attention_heads"]
        )
    ...
    @unittest.skip(reason="NemotronHDense needs at least 3 layers (mamba, mlp, attention)")
