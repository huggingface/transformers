...
from ...test_image_processing_common import (AnnotationFormatTestMixin,
...
class GroundingDinoImageProcessingTest(
    ...
    def test_post_process_object_detection(self):
        ...
        torch.testing.assert_close(
            results[0]["scores"], expected_scores, rtol=1e-4, atol=1e-4
        )

        expected_box_slice = torch.tensor([0.6908, 0.4354, 1.0737, 1.3947])
        torch.testing.assert_close(
            results[0]["boxes"][0], expected_box_slice, rtol=1e-4, atol=1e-4
        )
