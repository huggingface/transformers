...
@require_torch
@require_vision
class GroundingDinoImageProcessingTest(
    AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase
):
    image_processing_class = (
        GroundingDinoImageProcessor if is_vision_available() else None
    )
    ...
    def test_post_process_object_detection(self):
        ...
        self.assertEqual(
            results[0]["boxes"].shape, (self.image_processor_tester.num_queries, 4)
        )
        self.assertEqual(
            results[0]["scores"].shape, (self.image_processor_tester.num_queries,)
        )

        expected_scores = torch.tensor([0.7050, 0.7222, 0.7222, 0.6829, 0.7220])
        self.assertTrue(
            torch.allclose(results[0]["scores"], expected_scores, atol=1e-4)
        )

        expected_box_slice = torch.tensor([0.6908, 0.4354, 1.0737, 1.3947])
        self.assertTrue(
            torch.allclose(results[0]["boxes"][0], expected_box_slice, atol=1e-4)
        )
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            self.assertTrue(
                torch.allclose(encoding["labels"][0]["area"], expected_area)
            )
            # verify boxes
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            self.assertTrue(
                torch.allclose(
                    encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1e-3
                )
            )
            self.assertTrue(
                torch.allclose(
                    encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1e-3
                )
            )
            ...
            self.assertTrue(
                torch.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1)
            )
            self.assertTrue(
                torch.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1)
            )
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        for image_processing_class in self.image_processor_list:
            ...
            relative_error = (
                torch.abs(encoding["labels"][0]["masks"].sum() - expected_masks_sum)
                / expected_masks_sum
            )
