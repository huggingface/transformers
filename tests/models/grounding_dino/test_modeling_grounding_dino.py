...
class GroundingDinoModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        self.assertTrue(
            torch.allclose(outputs.logits[0, :3, :3], expected_logits, atol=1e-3)
        )
        ...
        self.assertTrue(
            torch.allclose(results["boxes"][0, :], expected_slice_boxes, atol=1e-2)
        )
    ...
    def test_inference_object_detection_head_equivalence_cpu_gpu(self):
        ...
        for key in cpu_outputs.keys():
            self.assertTrue(
                torch.allclose(cpu_outputs[key], gpu_outputs[key].cpu(), atol=1e-3)
            )
        ...
        self.assertTrue(
            torch.allclose(cpu_outputs.logits[0, :3, :3], expected_logits, atol=1e-3)
        )
        ...
        self.assertTrue(
            torch.allclose(results_cpu["scores"], result_gpu["scores"].cpu(), atol=1e-3)
        )
        self.assertTrue(
            torch.allclose(results_cpu["boxes"], result_gpu["boxes"].cpu(), atol=1e-3)
        )
    ...
    def test_cross_attention_mask(self):
        ...
        self.assertTrue(
            torch.allclose(outputs1.logits, outputs_batched.logits[:1], atol=1e-3)
        )
