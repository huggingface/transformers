...
class EvollaModelTester:
    ...
    def test_eager_matches_sdpa_inference(self, torch_dtype: str):
        self.skipTest(reason="Evolla has a hard requirement on SDPA, skipping this test")
...
class EvollaModel2Test(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        # XXX: EvollaForVisionText2TextTest has no MODEL_FOR group yet, but it should be the same
    ...
    @parameterized.expand([("float16",), ("bfloat16",), ("float32",)])
    @require_torch_sdpa
    @unittest.skip("Evolla requires both text and image inputs which is currently not done in this test.")
    ...
    def test_model_outputs_equivalence(self):
        try:
            ...
            # EvollaModel.forward doesn't have labels input arg - only EvollaForVisionText2Text does
            self.all_model_classes = (EvollaForVisionText2Text,) if is_torch_available() else ()
    ...
    def test_cross_attention_gates(self):
        ...
        model = EvollaModel(config=config).to(torch_device)
        model.eval()
        test_1_results = []
    ...
    def test_training(self):
        ...
        for model_class in self.all_model_classes:
            # EvollaModel does not support training, users should use
            # EvollaForVisionText2Text for this purpose
            if model_class == EvollaModel:
                self.skipTest(reason="EvollaModel does not support training")
    ...
    @unittest.skip(reason="""EVOLLA does not support retaining the gradients of the hidden states and attention""")
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with torch.no_grad():
                outputs = model(**self._prepare_for_class(inputs_dict, model_class))
            attentions = outputs.attentions
            # EVOLLA does not support outputting attention score becuase it uses SDPA under the hood
    ...
    def test_model_from_pretrained(self):
        model_name = "westlake-repl/Evolla-10B"
        model = EvollaModel.from_pretrained(model_name)
        self.assertIsNotNone(model)

    @unittest.skip("Evolla has a hard requirement on SDPA")
...
class EvollaForVisionText2TextTest(EvollaModelTest, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (EvollaForVisionText2Text,) if is_torch_available() else ()
    all_generative_model_classes = (EvollaForVisionText2Text,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = EvollaModelTester(
            self,
    ...
    def test_left_padding_compatibility(self):
        """Overwrite because EVOLLA needs image attention mask to be also padded"""
    ...
    def test_generate_continue_from_past_key_values(self):
        """Overwrite because EVOLLA needs image attention mask to be also processed"""
    ...
    def test_generate_without_input_ids(self):
        """Overwrite because EVOLLA needs image attention mask to be also processed and requires image at input always."""
    ...
    def _check_attentions_for_generate(
        ...
        """
        Overwrite from generation tests because Evolla has only SDPA layers.
    ...
    @unittest.skip(reason="EVOLLA cannot compile due to dynamic control flow when checking inputs")
