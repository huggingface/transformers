...
class Llama4ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=Llama4Config, hidden_size=37)
    ...
    def test_Llama4_sequence_classification_model(self):
    ...
    def test_Llama4_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_Llama4_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_Llama4_token_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
...
class Llama4IntegrationTest(unittest.TestCase):
    input_text = ["<|finetune_right_pad|>Roses are red,", "<|finetune_right_pad|>The capital of France is"]
    model_id = "meta-ai/Llama-4-17B"
    ...
    def test_model_9b_fp16(self):
        ...
        model = AutoModelForCausalLM.from_pretrained(
            self.model_id, low_cpu_mem_usage=True, deevice_map="auto", torch_dtype=torch.bfloat16
        )
