...
class MusicFlamingoForConditionalGenerationIntegrationTest(unittest.TestCase):
    ...
    def _generate_from_audio_paths(self, model, audio_paths: list[Path], prompt: str):
        conversations = [
            [
                {
                    "role": "user",
    ...
    def test_batched_generation_matches_single_generation_for_different_audio_lengths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_wav(short_audio_path, self._make_pulsed_tone(duration_s=105.0))
            self._write_wav(long_audio_path, self._make_stepped_melody(duration_s=268.0))
