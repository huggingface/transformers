...
""" Testing suite for the PyTorch BLIVA model. """
...
from transformers import CONFIG_MAPPING, BLIVAConfig, BLIVAQFormerConfig, BLIVAVisionConfig
...
if is_torch_available():
    ...
    from transformers import BLIVAForConditionalGeneration, BLIVAModel, BLIVAVisionModel
    from transformers.models.bliva.modeling_bliva import BLIVA_PRETRAINED_MODEL_ARCHIVE_LIST
...
class BLIVAVisionModelTester:
    ...
    def create_and_check_model(self, config, pixel_values):
        model = BLIVAVisionModel(config=config)
...
@require_torch
class BLIVAVisionModelTest(ModelTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as BLIVA's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (BLIVAVisionModel,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = BLIVAVisionModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=BLIVAVisionConfig, has_text_modality=False, hidden_size=37
        )

    def test_config(self):
        self.config_tester.run_common_tests()

    @unittest.skip(reason="BLIVA's vision encoder does not use inputs_embeds")
    ...
    @unittest.skip(reason="BLIVAVisionModel has no base class and is not available in MODEL_MAPPING")
    ...
    def test_model_from_pretrained(self):
        for model_name in BLIVA_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = BLIVAVisionModel.from_pretrained(model_name)
            self.assertIsNotNone(model)


class BLIVAQFormerModelTester:
    def __init__(
        self,
        parent,
        batch_size=12,
...
class BLIVATextModelDecoderOnlyTester:
    def __init__(
        self,
        parent,
        batch_size=12,
...
class BLIVAForConditionalGenerationDecoderOnlyModelTester:
    def __init__(
        ...
        self.parent = parent
        self.vision_model_tester = BLIVAVisionModelTester(parent, **vision_kwargs)
        self.qformer_model_tester = BLIVAQFormerModelTester(parent, **qformer_kwargs)
        self.text_model_tester = BLIVATextModelDecoderOnlyTester(parent, **text_kwargs)
        self.is_training = is_training
    ...
    def get_config(self):
        return BLIVAConfig.from_vision_qformer_text_configs(
    ...
    def create_and_check_for_conditional_generation(self, config, input_ids, attention_mask, pixel_values):
        model = BLIVAForConditionalGeneration(config).to(torch_device).eval()
        with torch.no_grad():
...
@require_torch
class BLIVAForConditionalGenerationDecoderOnlyTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (BLIVAForConditionalGeneration,) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def setUp(self):
        self.model_tester = BLIVAForConditionalGenerationDecoderOnlyModelTester(self)
    ...
    @unittest.skip(reason="BLIVAModel does not have input/output embeddings")
    def test_model_common_attributes(self):
        pass

    @unittest.skip(reason="There's no base BLIVAModel")
    def test_save_load_fast_init_from_base(self):
        pass
    ...
    def test_load_vision_qformer_text_config(self):
        ...
        # Save BLIVAConfig and check if we can load BLIVAVisionConfig from it
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            config.save_pretrained(tmp_dir_name)
            vision_config = BLIVAVisionConfig.from_pretrained(tmp_dir_name)
            self.assertDictEqual(config.vision_config.to_dict(), vision_config.to_dict())

        # Save BLIVAConfig and check if we can load BLIVAQFormerConfig from it
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            config.save_pretrained(tmp_dir_name)
            qformer_config = BLIVAQFormerConfig.from_pretrained(tmp_dir_name)
            self.assertDictEqual(config.qformer_config.to_dict(), qformer_config.to_dict())

    @slow
    def test_model_from_pretrained(self):
        for model_name in BLIVA_PRETRAINED_MODEL_ARCHIVE_LIST:
            model = BLIVAForConditionalGeneration.from_pretrained(model_name)
            self.assertIsNotNone(model)


# this class is based on `T5ModelTester` found in tests/models/t5/test_modeling_t5.py
class BLIVATextModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
...
class BLIVAModelTester:
    def __init__(
        ...
        self.vision_model_tester = BLIVAVisionModelTester(parent, **vision_kwargs)
        self.qformer_model_tester = BLIVAQFormerModelTester(parent, **qformer_kwargs)
        self.text_model_tester = BLIVATextModelTester(parent, **text_kwargs)
        self.is_training = is_training
        self.num_query_tokens = num_query_tokens
...
@require_torch
class BLIVAModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (BLIVAForConditionalGeneration, BLIVAModel) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def setUp(self):
        self.model_tester = BLIVAModelTester(self)
    ...
    def test_get_text_features(self):
        ...
        model = BLIVAModel(config).to(torch_device)
        model.eval()
...
@require_torch
@slow
class BLIVAModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_opt_multi_gpu(self):
        processor = Blip2Processor.from_pretrained("todo-bliva-checkpoint")
        model = BLIVAForConditionalGeneration.from_pretrained(
            "todo-bliva-checkpoint", torch_dtype=torch.float16, device_map="balanced"
        )
