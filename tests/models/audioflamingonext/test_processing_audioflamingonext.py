...
class AudioFlamingoNextProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    processor_class = AudioFlamingoNextProcessor

    @classmethod
    @require_torch
    @require_torchaudio
    def setUpClass(cls):
        cls.checkpoint = "nvidia/audio-flamingo-next-hf"
        cls.tmpdirname = tempfile.mkdtemp()

        processor = AudioFlamingoNextProcessor.from_pretrained(cls.checkpoint)
    ...
    def test_can_load_various_tokenizers(self):
        processor = AudioFlamingoNextProcessor.from_pretrained(self.checkpoint)
    ...
    def test_save_load_pretrained_default(self):
        ...
        processor = AudioFlamingoNextProcessor(tokenizer=tokenizer, feature_extractor=feature_extractor)

        with tempfile.TemporaryDirectory() as tmpdir:
            processor.save_pretrained(tmpdir)
            reloaded = AudioFlamingoNextProcessor.from_pretrained(tmpdir)
    ...
    def test_chat_template(self):
        ...
        expected_prompt = (
            ...
            "of whether the user calls it audio, speech, music, or sound.<|im_end|>\n"
    ...
    def test_apply_chat_template_audio(self, batch_size: int, return_tensors: str):
        if return_tensors == "np":
            self.skipTest("AudioFlamingoNext only supports PyTorch tensors")
