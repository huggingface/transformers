...
class DacIntegrationTest(unittest.TestCase):
    def test_integration_16khz(self):
        ...
        with torch.no_grad():
            ...
            self.assertTrue(
                torch.allclose(encoder_outputs_mean, expected_encoder_sums, atol=1e-3)
            )
            ...
            self.assertTrue(
                torch.allclose(input_values_dec, input_values_enc_dec, atol=1e-3)
            )
    ...
    def test_integration_24khz(self):
        ...
        with torch.no_grad():
            ...
            self.assertTrue(
                torch.allclose(input_values_from_codes, input_values_dec, atol=1e-5)
            )
