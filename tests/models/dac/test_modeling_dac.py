...
class DacIntegrationTest(unittest.TestCase):
    def test_integration_16khz(self):
        ...
        with torch.no_grad():
            ...
            expected_encoder_mean = torch.tensor(list(expected_encoder_mean_dict.values()), dtype=torch.float32)
            encoder_outputs_mean = torch.tensor([v.float().mean().cpu().item() for v in encoder_outputs.to_tuple()])

            # make sure audio encoded codes are correct
            self.assertTrue(torch.allclose(encoder_outputs_mean, expected_encoder_mean, atol=1e-3))
    ...
    def test_integration_24khz(self):
        ...
        expected_encoder_output_dict = {
            "audio_codes": torch.tensor([919, 919, 234, 777, 234, 234, 234, 690, 234, 876]),
        }
        librispeech_dummy = load_dataset("hf-internal-testing/librispeech_asr_dummy", "clean", split="validation")
        ...
        with torch.no_grad():
            ...
            expected_audio_codes = encoder_outputs["audio_codes"][0, 0, :10].cpu()

            # make sure we get the correct audio codes
            self.assertTrue(
