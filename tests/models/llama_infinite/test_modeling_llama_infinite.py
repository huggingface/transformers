...
""" Testing suite for the PyTorch LLaMA-Infinite model. """
...
from transformers import LlamaInfiniteConfig, is_torch_available, set_seed
...
class LlamaInfiniteModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def create_and_check_model(
        ...
        model = LlamaInfiniteModel(config=config)
    ...
    def create_and_check_model_as_decoder(
        ...
        model = LlamaInfiniteModel(config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_causal_lm(
        ...
        model = LlamaInfiniteForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
...
class LlamaInfiniteModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            "feature-extraction": LlamaInfiniteModel,
            "text-classification": LlamaInfiniteForSequenceClassification,
            "text-generation": LlamaInfiniteForCausalLM,
            "zero-shot": LlamaInfiniteForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = LlamaInfiniteModelTester(self)
        self.config_tester = ConfigTester(self, config_class=LlamaInfiniteConfig, hidden_size=37)
    ...
    def test_llama_infinite_sequence_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
        sequence_labels = ids_tensor([self.model_tester.batch_size], self.model_tester.type_sequence_label_size)
        model = LlamaInfiniteForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=attention_mask, labels=sequence_labels)
        self.assertEqual(result.logits.shape, (self.model_tester.batch_size, self.model_tester.num_labels))

    def test_llama_infinite_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_llama_infinite_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_model_rope_scaling(self, scaling_type):
        ...
        original_model = LlamaInfiniteModel(config)
        ...
        scaled_model = LlamaInfiniteModel(config)
...
class LlamaIntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        ...
        model = LlamaInfiniteForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", device_map="auto")
    ...
    def test_model_13b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = LlamaInfiniteForCausalLM.from_pretrained("meta-llama/Llama-2-13b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_13bf_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = LlamaInfiniteForCausalLM.from_pretrained("meta-llama/Llama-2-13b-chat-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_70b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = LlamaInfiniteForCausalLM.from_pretrained("meta-llama/Llama-2-70b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
...
""",
    ...
    @slow
    def test_model_7b_logits(self):
        model = LlamaInfiniteForCausalLM.from_pretrained("codellama/CodeLlama-7b-hf").to(torch_device)
