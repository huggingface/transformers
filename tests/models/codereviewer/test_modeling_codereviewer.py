...
class CodeReviewerModelTester:
    ...
    def get_large_model_config(self):
        return CodeReviewerConfig.from_pretrained("google-codereviewer/codereviewer-base")
...
class CodeReviewerModelIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tokenizer(self):
        return CodeReviewerTokenizer.from_pretrained("google-codereviewer/codereviewer-base")

    @slow
    def test_torch_quant(self):
        r"""
        Test that a simple `torch.quantization.quantize_dynamic` call works on a CodeReviewer model.
        """
        model_name = "google/flan-codereviewer-small"
        tokenizer = CodeReviewerTokenizer.from_pretrained(model_name)
        model = CodeReviewerForConditionalGeneration.from_pretrained(model_name)
    ...
    def test_small_generation(self):
        model = CodeReviewerForConditionalGeneration.from_pretrained("microsoft/codereviewer").to(torch_device)
        model.config.max_length = 8
        model.config.num_beams = 1
        model.config.do_sample = False
        tokenizer = CodeReviewerTokenizer.from_pretrained("microsoft/codereviewer")
    ...
    def test_small_integration_test(self):
        """
        For comparision run:
        >>> import codereviewer  # pip install codereviewer==0.7.1
        >>> from codereviewer.data.sentencepiece_vocabulary import SentencePieceVocabulary

        >>> path_to_mtf_small_codereviewer_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> codereviewer_model = codereviewer.models.MtfModel(model_dir=path_to_mtf_small_codereviewer_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = codereviewer_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """
    ...
    def test_small_v1_1_integration_test(self):
        ...
        >>> path_to_mtf_small_codereviewer_v1_1_checkpoint = '<fill_in>'
        >>> path_to_mtf_small_spm_model_path = '<fill_in>'
        >>> codereviewer_model = codereviewer.models.MtfModel(model_dir=path_to_mtf_small_codereviewer_v1_1_checkpoint, batch_size=1, tpu=None)
        >>> vocab = SentencePieceVocabulary(path_to_mtf_small_spm_model_path, extra_ids=100)
        >>> score = codereviewer_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = CodeReviewerForConditionalGeneration.from_pretrained("google/codereviewer-v1_1-small").to(torch_device)
        tokenizer = CodeReviewerTokenizer.from_pretrained("google/codereviewer-v1_1-small")
    ...
    @slow
    def test_small_bycodereviewer_integration_test(self):
        """
        For comparision run:
        >>> import codereviewer  # pip install codereviewer==0.9.1

        >>> path_to_bycodereviewer_small_checkpoint = '<fill_in>'
        >>> codereviewer_model = codereviewer.models.MtfModel(model_dir=path_to_tf_checkpoint, batch_size=1, tpu=None)
        >>> vocab = codereviewer.data.ByteVocabulary()
        >>> score = codereviewer_model.score(inputs=["Hello there"], targets=["Hi I am"], vocabulary=vocab)
        """

        model = CodeReviewerForConditionalGeneration.from_pretrained("google/bycodereviewer-small").to(torch_device)
        tokenizer = ByCodeReviewerTokenizer.from_pretrained("google/bycodereviewer-small")
    ...
    def test_translation_en_to_fr(self):
        model = self.model  # google-codereviewer/codereviewer-base
        tok = self.tokenizer
    ...
    @slow
    def test_contrastive_search_codereviewer(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        outputs = codereviewer_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64)
        generated_text = codereviewer_tokenizer.batch_decode(outputs, skip_special_tokens=True)
