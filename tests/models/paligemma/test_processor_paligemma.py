...
class PaliGemmaProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    ...
    def test_text_with_image_tokens(self):
        ...
        out_singlimage = processor(
            text=text_single_image, images=image, return_tensors="np"
        )
        ...
        out_noimage = processor(
            text=text_no_image, images=[[image, image]], return_tensors="np"
        )
        ...
        out_images = processor(
            text=text_batched_with_image, images=[image, image], return_tensors="np"
        )
        out_noimage_nested = processor(
            text=text_batched, images=[[image], [image]], return_tensors="np"
        )
        out_noimage = processor(
            text=text_batched, images=[image, image], return_tensors="np"
        )
        for k in out_noimage:
            self.assertTrue(
                out_noimage[k].tolist()
                == out_images[k].tolist()
                == out_noimage_nested[k].tolist()
            )
