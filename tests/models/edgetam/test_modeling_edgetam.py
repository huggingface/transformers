...
class EdgeTamVisionModelTester:
    def __init__(
        self,
        parent,
        hidden_size=12,
    ...
    def get_config(self):
        backbone_config = EdgeTamHieraDetConfig(
    ...
    def create_and_check_model(self, config, pixel_values):
        model = EdgeTamVisionModel(config=config)
...
@require_torch
class EdgeTamVisionModelTest(ModelTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as SAM's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (EdgeTamVisionModel,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = EdgeTamVisionModelTester(self)
        self.config_tester = ConfigTester(self, config_class=EdgeTamVisionConfig, has_text_modality=False)
...
class EdgeTamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    # Override as EdgeTamModel has different sub-modules
    @require_torch_sdpa
    ...
    @unittest.skip(reason="EdgeTamModel does not support training")
    ...
    def test_model_from_pretrained(self):
        model_name = "yonigozlan/edgetam.1_hiera_tiny_hf"
...
def prepare_image():
    img_url = "https://huggingface.co/datasets/hf-internal-testing/edgetam-fixtures/resolve/main/truck.jpg"
    raw_image = Image.open(requests.get(img_url, stream=True).raw).convert("RGB")
    return raw_image


def prepare_groceries_image():
    img_url = "https://huggingface.co/datasets/hf-internal-testing/edgetam-fixtures/resolve/main/groceries.jpg"
    raw_image = Image.open(requests.get(img_url, stream=True).raw).convert("RGB")
    return raw_image
...
def prepare_video():
    video_url = "https://huggingface.co/datasets/hf-internal-testing/edgetam-fixtures/resolve/main/bedroom.mp4"
...
class EdgeTamModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_mask_generation_video_one_point(self):
        ...
        frames = []
        for edgetam_video_output in self.video_model.propagate_in_video_iterator(
            inference_session=inference_session,
            max_frame_num_to_track=2,
        ):
            frames.append(edgetam_video_output.video_res_masks)
    ...
    def test_inference_propagate_video_from_mask_input(self):
        ...
        edgetam_video_output = self.video_model(
            inference_session=inference_session,
            frame_idx=ann_frame_idx,
        ...
        self.processor.add_inputs_to_inference_session(
            ...
            frame_idx=ann_frame_idx,
            obj_ids=ann_obj_id,
            input_masks=edgetam_video_output.video_res_masks,
        )
        edgetam_video_output = self.video_model(
            inference_session=inference_session,
            frame_idx=ann_frame_idx,
        ...
        low_res_masks = edgetam_video_output.consolidated_res_masks
        video_res_masks = edgetam_video_output.video_res_masks
        self.assertEqual(low_res_masks.shape, (1, 1, 256, 256))
        self.assertEqual(video_res_masks.shape, (1, 1, raw_video.shape[-3], raw_video.shape[-2]))
        torch.testing.assert_close(
            video_res_masks[0, 0, :3, :3],
    ...
    def test_inference_propagate_on_streamed_video(self):
        ...
        for frame_idx, frame in enumerate(raw_video):
            ...
            edgetam_video_output = self.video_model(inference_session=inference_session, frame=inputs.pixel_values[0])
            video_res_masks.append(edgetam_video_output.video_res_masks)
    ...
    def test_dummy_pipeline_generation(self):
        generator = pipeline("mask-generation", model="yonigozlan/edgetam.1_hiera_tiny_hf", device=torch_device)
        raw_image = prepare_image()
