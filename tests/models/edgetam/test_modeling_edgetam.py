...
class EdgeTamModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_batched_images_batched_boxes(self):
        ...
        torch.testing.assert_close(
            outputs.iou_scores,
            torch.tensor([[[0.9514], [0.9241], [0.9292], [0.9044]], [[0.6264], [0.9512], [0.9766], [0.8052]]]).to(
                torch_device
            ),
            atol=1e-4,
            rtol=1e-4,
        ...
        torch.testing.assert_close(
            ...
            torch.tensor(
                [
                    [
                        [[[-9.0350, -8.5963], [-8.5206, -9.7884]]],
                        [[[-15.1835, -17.5181], [-14.6591, -17.4362]]],
                        [[[-14.4556, -16.4878], [-13.8609, -17.3795]]],
                        [[[-20.7746, -23.7153], [-19.1292, -23.7991]]],
                    ],
                    [
                        [[[-11.8260, -11.3060], [-11.5297, -10.8281]]],
                        [[[-15.9894, -14.6909], [-14.8407, -14.4381]]],
                        [[[-15.0029, -13.5259], [-13.7243, -13.3990]]],
                        [[[-12.9556, -11.4367], [-12.2214, -11.6412]]],
                    ],
                ]
            ).to(torch_device),
            atol=1e-4,
