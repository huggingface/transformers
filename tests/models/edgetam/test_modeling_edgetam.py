...
class EdgeTamModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_mask_generation_batched_images_multi_points(self):
        ...
        torch.testing.assert_close(
            scores2, torch.tensor([0.7228, 0.6532, 0.6344]).to(torch_device), atol=1e-4, rtol=1e-4
        )
        torch.testing.assert_close(
            masks_logits2,
            torch.tensor([[-8.8497, -7.8133, -9.3914], [-8.2869, -8.7974, -9.1661], [-9.6242, -8.42, -9.082]]).to(
                torch_device
            ),
            atol=1e-4,
            rtol=1e-4,
