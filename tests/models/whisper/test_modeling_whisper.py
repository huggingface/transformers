...
class WhisperEncoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    # FlaxWhisper model have an extra input param 'input_shape'
    @is_pt_flax_cross_test
    def test_equivalence_flax_to_pt(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                # load Flax class
                input_shape = (1,) + inputs_dict["input_features"].shape[1:]
                fx_model = fx_model_class(config, input_shape=input_shape, dtype=jnp.float32)
    ...
    def test_equivalence_pt_to_flax(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    pt_model.save_pretrained(tmpdirname)
                    fx_model_loaded = fx_model_class.from_pretrained(tmpdirname, input_shape=input_shape, from_pt=True)
