...
class SeamlessM4Tv2ModelWithSpeechInputTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (SeamlessM4Tv2ForSpeechToText,) if is_torch_available() else ()
    )
...
@require_torch
class SeamlessM4Tv2ModelWithTextInputTest(
    ...
    all_generative_model_classes = (
        (SeamlessM4Tv2ForTextToText,) if is_torch_available() else ()
    )
...
class SeamlessM4Tv2GenerationTest(unittest.TestCase):
    ...
    def setUp(self):
        self.speech_model_tester = SeamlessM4Tv2ModelTester(
    ...
    def update_generation(self, model):
        ...
        generation_config.__setattr__(
            "text_decoder_lang_to_code_id", text_lang_code_to_id
        )
...
class SeamlessM4Tv2ModelIntegrationTest(unittest.TestCase):
    ...
    def factory_test_task(self, class1, class2, inputs, class1_kwargs, class2_kwargs):
        ...
        model1 = class1.from_pretrained(self.repo_id, torch_dtype=torch.float16).to(
            torch_device
        )
        model2 = class2.from_pretrained(self.repo_id, torch_dtype=torch.float16).to(
            torch_device
        )
    ...
    def test_to_eng_text(self):
        ...
        self.assertListEqual(
            expected_unit_tokens,
            (output.unit_sequences - model.config.vocoder_offset).squeeze().tolist(),
        )
        ...
        self.assertListAlmostEqual(
            [-2.349690e-04, 9.920777e-02],
            [output.waveform.mean().item(), output.waveform.std().item()],
        )
