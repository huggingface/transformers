...
from transformers import GeoLMConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.models.geolm.modeling_tf_geolm import (
        TFGeoLMForMaskedLM,
...
class TFGeoLMModelTester:
    ...
    def create_and_check_model(
        ...
        model = TFGeoLMModel(config=config)
    ...
    def create_and_check_causal_lm_model(
        ...
        model = TFGeoLMLMHeadModel(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_masked_lm(
        ...
        model = TFGeoLMForMaskedLM(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_next_sequence_prediction(
        ...
        model = TFGeoLMForNextSentencePrediction(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_pretraining(
        ...
        model = TFGeoLMForPreTraining(config=config)
        inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        result = model(inputs)
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = TFGeoLMForSequenceClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = TFGeoLMForMultipleChoice(config=config)
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = TFGeoLMForTokenClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_for_question_answering(
        ...
        model = TFGeoLMForQuestionAnswering(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
...
@require_tf
class TFGeoLMModelTest(TFModelTesterMixin, TFCoreModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            TFGeoLMModel,
            TFGeoLMForMaskedLM,
    ...
    def setUp(self):
        self.model_tester = TFGeoLMModelTester(self)
    ...
    def test_model_from_pretrained(self):
        model = TFGeoLMModel.from_pretrained("jplu/tiny-tf-geolm-random")
        self.assertIsNotNone(model)

    def test_custom_load_tf_weights(self):
        model, output_loading_info = TFGeoLMForTokenClassification.from_pretrained(
            "jplu/tiny-tf-geolm-random", output_loading_info=True
        )
...
@require_tf
class TFGeoLMModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_masked_lm(self):
        model = TFGeoLMForPreTraining.from_pretrained("lysandre/tiny-geolm-random")
