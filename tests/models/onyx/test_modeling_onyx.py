...
class OnyxModelTester(CausalLMModelTester):
    if is_torch_available():
        base_model_class = OnyxModel


@require_torch
class OnyxModelTest(CausalLMModelTest, unittest.TestCase):
    _is_stateful = True
    model_split_percents = [0.5, 0.6]
    model_tester_class = OnyxModelTester

    @unittest.skip("Onyx tanh soft-capping amplifies TP numerical noise beyond 80% match threshold")
...
class OnyxIntegrationTest(unittest.TestCase):
    ...
    def test_model_9b_pipeline_bf16(self):
        # See https://github.com/huggingface/transformers/pull/31747 -- pipeline was broken for Onyx before this PR
        model_id = "google/gemma-2-9b"
