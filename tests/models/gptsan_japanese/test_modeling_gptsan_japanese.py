...
if is_torch_available():
    ...
    from transformers.models.gptsan_japanese.modeling_gptsan_japanese import (
...
class GPTSANJapaneseModelTester:
    ...
    def get_pipeline_config(self):
        return GPTSANJapaneseConfig(
            vocab_size=166,  # gptsan_japanese forces 100 extra tokens
    ...
    def create_and_check_with_lm_head(
        ...
        model = GPTSANJapaneseForConditionalGeneration(config=config).to(torch_device).eval()
        outputs = model(
            input_ids=input_ids,
            decoder_input_ids=decoder_input_ids,
            decoder_attention_mask=decoder_attention_mask,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = GPTSANJapaneseModel(config=config).get_decoder().to(torch_device).eval()
        # first forward pass
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        model = GPTSANJapaneseModel(config=config).get_decoder()
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_generate_with_past_key_values(
        ...
        """
        model = (
            GPTSANJapaneseForConditionalGeneration.from_pretrained("Tanrei/GPTSAN-japanese").to(torch_device).eval()
        )
        torch.manual_seed(0)
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = GPTSANJapaneseModel(config=config).to(torch_device).half().eval()
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        for model_class in [GPTSANJapaneseModel, GPTSANJapaneseForConditionalGeneration]:
            torch.manual_seed(0)
    ...
    def check_resize_embeddings_gptsan_japanese_v1_1(
        self,
        config,
    ):
...
@require_torch
class GPTSANJapaneseModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):

    all_model_classes = (
        (GPTSANJapaneseModel, GPTSANJapaneseForConditionalGeneration) if is_torch_available() else ()
    )
    all_generative_model_classes = (GPTSANJapaneseForConditionalGeneration,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def test_beam_sample_generate_dict_output(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            model = model_class.from_pretrained("Tanrei/GPTSAN-japanese").to(torch_device).eval()
    ...
    def test_v1_1_resize_embeddings(self):
        ...
        self.model_tester.check_resize_embeddings_gptsan_japanese_v1_1(config)

    @slow
    def test_model_from_pretrained(self):
        for model_name in GPTSAN_JAPANESE_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = GPTSANJapaneseModel.from_pretrained(model_name)
            self.assertIsNotNone(model)

    @unittest.skip("Test has a segmentation fault on torch 1.8.0")
    def test_export_to_onnx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        model = GPTSANJapaneseModel(config_and_inputs[0]).to(torch_device)
        with tempfile.TemporaryDirectory() as tmpdirname:
            torch.onnx.export(
                model,
                (config_and_inputs[1], config_and_inputs[3], config_and_inputs[2]),
                f"{tmpdirname}/gptsan_japanese_test.onnx",
                export_params=True,
                opset_version=9,
    ...
    def test_generate_with_head_masking(self):
        ...
        model = GPTSANJapaneseForConditionalGeneration(config).eval()
        model.to(torch_device)
        ...
        for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
            ...
            # Explicitly pass decoder_head_mask as it is required from GPTSAN_JAPANESE model when head_mask specified
            if name == "head_mask":
...
class GPTSANJapaneseEncoderOnlyModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def get_large_model_config(self):
        return GPTSANJapaneseConfig.from_pretrained("switch_base_8")
    ...
    def create_and_check_model(self, config, input_ids, attention_mask):
        model = GPTSANJapaneseEncoderModel(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids=input_ids,
    ...
    def create_and_check_model_fp16_forward(self, config, input_ids, attention_mask):
        model = GPTSANJapaneseEncoderModel(config=config).to(torch_device).half().eval()
...
class GPTSANJapaneseEncoderOnlyModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (GPTSANJapaneseEncoderModel,) if is_torch_available() else ()
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = GPTSANJapaneseEncoderOnlyModelTester(self)
        self.config_tester = ConfigTester(self, config_class=GPTSANJapaneseConfig, d_model=37)
...
@require_torch
class TestAsymmetricGPTSANJapanese(unittest.TestCase):
    def build_model_and_check_forward_pass(self, **kwargs):
        tester = GPTSANJapaneseModelTester(self, **kwargs)
    ...
    def test_small_decoder(self):
        # num_hidden_layers is passed to GPTSANJapaneseConfig as num_layers
...
class SwitchTransformerRouterTest(unittest.TestCase):
    ...
    def test_equivalency_token_chose_masked_router(self):
        r"""
        This test tests the equivalency between the `GPTSANJapaneseTop1Router`
        ...
        model = GPTSANJapaneseTop1Router(self.config)
...
class SwitchTransformerModelIntegrationTests(unittest.TestCase):
    ...
    def test_small_logits(self):
        ...
        of the first batch.
        """
        model = GPTSANJapaneseModel.from_pretrained("Tanrei/GPTSAN-japanese", torch_dtype=torch.bfloat16).to(
            torch_device
        )
    ...
    def test_small_generate(self):
        ...
        model = GPTSANJapaneseForConditionalGeneration.from_pretrained(
            "Tanrei/GPTSAN-japanese", torch_dtype=torch.bfloat16
        ).eval()
