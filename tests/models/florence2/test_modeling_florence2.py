...
from transformers import Florence2Config, is_torch_available
...
class Florence2ModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_florence2_inputs_dict(config, input_ids, decoder_input_ids)
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = Florence2Model(config=config).get_decoder().to(torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = Florence2Model(config=config).to(torch_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = Florence2Encoder.from_pretrained(tmpdirname).to(torch_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = Florence2Decoder.from_pretrained(tmpdirname).to(torch_device)
...
@require_torch
class Florence2HeadTests(unittest.TestCase):
    vocab_size = 99
    ...
    def test_sequence_classification_forward(self):
        ...
        model = Florence2ForSequenceClassification(config)
    ...
    def test_question_answering_forward(self):
        ...
        model = Florence2ForQuestionAnswering(config)
        model.to(torch_device)
        outputs = model(
            input_ids=input_ids,
    ...
    def test_lm_forward(self):
        ...
        lm_model = Florence2ForConditionalGeneration(config)
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = Florence2ForConditionalGeneration(config).to(torch_device)
    ...
    def test_tokenization(self):
        tokenizer = BartTokenizer.from_pretrained("facebook/florence2-large")
        ...
        for ex, desired_result in zip(examples, fairseq_results):
            florence2_toks = tokenizer.encode(ex, return_tensors="pt").squeeze()
            assert_tensors_close(desired_result.long(), florence2_toks, prefix=ex)

    @require_torch_fp16
    def test_generate_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data()
        attention_mask = input_ids.ne(1).to(torch_device)
        model = Florence2ForConditionalGeneration(config).eval().to(torch_device)
        model.half()
...
class Florence2ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    # Florence2ForSequenceClassification does not support inputs_embeds
    def test_inputs_embeds(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()

        for model_class in (Florence2Model, Florence2ForConditionalGeneration, Florence2ForQuestionAnswering):
            model = model_class(config)
            model.to(torch_device)
            model.eval()
...
class FastIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tok(self):
        return BartTokenizer.from_pretrained("facebook/florence2-large")

    @cached_property
    def xsum_1_1_model(self):
        return Florence2ForConditionalGeneration.from_pretrained("sshleifer/distilflorence2-xsum-1-1")
...
@require_sentencepiece
@require_tokenizers
class Florence2ModelIntegrationTests(unittest.TestCase):
    @cached_property
    ...
    def test_inference_no_head(self):
        model = Florence2Model.from_pretrained("facebook/florence2-large").to(torch_device)
    ...
    def test_base_mask_filling(self):
        pbase = pipeline(task="fill-mask", model="facebook/florence2-base")
    ...
    def test_large_mask_filling(self):
        plarge = pipeline(task="fill-mask", model="facebook/florence2-large")
        src_text = [" I went to the <mask>."]
    ...
    def test_mnli_inference(self):
        ...
        model = AutoModelForSequenceClassification.from_pretrained("facebook/florence2-large-mnli").to(
            torch_device
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = Florence2ForConditionalGeneration.from_pretrained("facebook/florence2-large-xsum").to(torch_device)
    ...
    def test_xsum_config_generation_params(self):
        config = Florence2Config.from_pretrained("facebook/florence2-large-xsum")
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = Florence2ForConditionalGeneration.from_pretrained("facebook/florence2-large-cnn").to(torch_device)
        tok = BartTokenizer.from_pretrained("facebook/florence2-large")
    ...
    @slow
    def test_contrastive_search_florence2(self):
        article = (
            " New York (CNN)When Liana Barrientos was 23 years old, she got married in Westchester County, New York. A"
            " year later, she got married again in Westchester County, but to a different man and without divorcing"
            " her first husband.  Only 18 days after that marriage, she got hitched yet again. Then, Barrientos"
        ...
        outputs = florence2_model.generate(input_ids, penalty_alpha=0.5, top_k=5, max_length=64, num_beams=1)
        generated_text = florence2_tokenizer.batch_decode(outputs, skip_special_tokens=True)
...
class Florence2StandaloneDecoderModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = Florence2Decoder(config=config).to(torch_device).eval()
        # first forward pass
...
@require_torch
class Florence2StandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Florence2Decoder, Florence2ForCausalLM) if is_torch_available() else ()
    fx_comptatible = True
    test_pruning = False
    is_encoder_decoder = False
    ...
    def setUp(
        ...
        self.model_tester = Florence2StandaloneDecoderModelTester(self, is_training=False)
        self.config_tester = ConfigTester(self, config_class=Florence2Config)
