...
class NomicBertModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = NomicBertLMHeadModel(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_decoder_model_past_large_inputs(
        ...
        config.is_decoder = True
        config.add_cross_attention = True
        model = NomicBertLMHeadModel(config=config).to(torch_device).eval()
...
@require_torch
class NomicBertModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            NomicBertModel,
            NomicBertLMHeadModel,
    ...
    pipeline_model_mapping = (
        {
            ...
            "text-generation": NomicBertLMHeadModel,
