...
""" Testing suite for the PyTorch Trillson_efficient model. """
...
from transformers import Trillson_efficientConfig
...
if is_torch_available():
    ...
    from transformers import Trillson_efficientForConditionalGeneration, Trillson_efficientModel, Speech2TextProcessor
    from transformers.models.trillson_efficient.modeling_trillson_efficient import Trillson_efficientDecoder, Trillson_efficientEncoder


def prepare_trillson_efficient_inputs_dict(
    config,
    input_features,
    decoder_input_ids,
    attention_mask=None,
...
@require_torch
class Trillson_efficientModelTester:
    def __init__(
        self,
        parent,
        batch_size=13,
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_trillson_efficient_inputs_dict(
            config,
            input_features=input_features,
            decoder_input_ids=decoder_input_ids,
            attention_mask=attention_mask,
    ...
    def get_config(self):
        return Trillson_efficientConfig(
    ...
    def create_and_check_model_forward(self, config, inputs_dict):
        model = Trillson_efficientModel(config=config).to(torch_device).eval()
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = Trillson_efficientModel(config=config).get_decoder().to(torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = Trillson_efficientEncoder.from_pretrained(tmpdirname).to(torch_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = Trillson_efficientDecoder.from_pretrained(tmpdirname).to(torch_device)
...
@require_torch
class Trillson_efficientModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (Trillson_efficientModel, Trillson_efficientForConditionalGeneration) if is_torch_available() else ()
    all_generative_model_classes = (Trillson_efficientForConditionalGeneration,) if is_torch_available() else ()
    ...
    def setUp(self):
        self.model_tester = Trillson_efficientModelTester(self)
        self.config_tester = ConfigTester(self, config_class=Trillson_efficientConfig)
        self.maxDiff = 3000
    ...
    def test_generate_fp16(self):
        ...
        model = Trillson_efficientForConditionalGeneration(config).eval().to(torch_device)
...
@require_tokenizers
@slow
class Trillson_efficientModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_processor(self):
        return Speech2TextProcessor.from_pretrained("google/trillson_efficient")
    ...
    def test_generation_librispeech(self):
        model = Trillson_efficientForConditionalGeneration.from_pretrained("google/trillson_efficient")
        model.to(torch_device)
