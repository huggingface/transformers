...
""" Testing suite for the PyTorch ChatGLM model. """
...
class ChatGlmModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    @unittest.skip("ChatGlm buffers include complex numbers, which breaks this test")
    ...
    def test_model_rope_scaling(self, scaling_type):
        ...
        original_model = ChatGlmModel(config)
        ...
        scaled_model = ChatGlmModel(config)
...
@require_torch
class ChatGlmIntegrationTest(unittest.TestCase):
    @unittest.skip("Logits are not exactly the same, once we fix the instabalities somehow, will update!")
    @slow
    def test_model_7b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChatGlmForCausalLM.from_pretrained("meta-chatglm/ChatGlm-2-7b-hf", device_map="auto")
    ...
    def test_model_13b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChatGlmForCausalLM.from_pretrained("meta-chatglm/ChatGlm-2-13b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_13bf_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChatGlmForCausalLM.from_pretrained("meta-chatglm/ChatGlm-2-13b-chat-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_70b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChatGlmForCausalLM.from_pretrained("meta-chatglm/ChatGlm-2-70b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
    ...
    def test_model_13b_greedy_generation(self):
        ...
        tokenizer = LlamaTokenizer.from_pretrained("meta-chatglm/ChatGlm-2-13b-chat-hf")
        input_ids = tokenizer.encode(prompt, return_tensors="pt")
        model = ChatGlmForCausalLM.from_pretrained(
            "meta-chatglm/ChatGlm-2-13b-chat-hf", device_map="sequential", use_safetensors=False
        )
...
@require_torch
class CodeChatGlmIntegrationTest(unittest.TestCase):
    PROMPTS = [
...
""",
    ...
    @slow
    def test_model_7b_logits(self):
        model = ChatGlmForCausalLM.from_pretrained("codechatglm/CodeChatGlm-7b-hf").to(torch_device)
        tokenizer = CodeLlamaTokenizer.from_pretrained("codechatglm/CodeChatGlm-7b-hf")
