...
@require_torch
class EfficientvitsamConfigTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_model_init(self):
        model = EfficientvitsamModel(EfficientvitsamConfig())
        self.assertEqual(model.vision_encoder.backbone.width_list, [32, 64, 128, 256, 512])


@slow
@require_torch
@require_vision
class EfficientvitsamParityTest(unittest.TestCase):
    ...
    def _load_image():
        ...
        return Image.open(BytesIO(requests.get(EfficientvitsamParityTest.image_url, timeout=60).content)).convert(
            "RGB"
        )

    @staticmethod
    def _load_checkpoint():
        return hf_hub_download("mit-han-lab/efficientvit-sam", "efficientvit_sam_l0.pt")

    def test_conversion_key_mapping_loads_cleanly(self):
        import torch
        ...
        model = EfficientvitsamModel(get_config("efficientvit-sam-l0"))
        missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)

        self.assertEqual(missing_keys, [])
        self.assertEqual(unexpected_keys, [])

    def test_image_processor_matches_upstream_transform(self):
        import torch
        ...
        expected = upstream_model.transform(image_array).unsqueeze(0)

        image_processor = EfficientvitsamImageProcessor()
        actual = image_processor(image, return_tensors="pt")["pixel_values"]

        torch.testing.assert_close(actual, expected, rtol=0.0, atol=0.0)

    def test_parity_against_upstream_predictor(self):
        import torch
        ...
        predictor = EfficientViTSamPredictor(upstream_model)
        predictor.set_image(image_array)
        _, upstream_iou, upstream_low_res = predictor.predict(
        ...
        inputs = processor(
            images=image,
            input_points=[self.point_coords.tolist()],
            input_labels=[self.point_labels.tolist()],
            return_tensors="pt",
        )
        ...
        torch.testing.assert_close(outputs.iou_scores[0, 0], torch.tensor(upstream_iou), rtol=1e-4, atol=1e-4)
        torch.testing.assert_close(outputs.pred_masks[0, 0], torch.tensor(upstream_low_res), rtol=5e-4, atol=5e-4)
...
# See the License for the specific language governing permissions and
# limitations under the License.
"""Testing suite for the PyTorch Efficientvitsam model."""
...
pytest.skip("Legacy SAM-wrapper tests are obsolete after the MIT EfficientViT-SAM port.", allow_module_level=True)

from transformers import EfficientvitsamConfig, EfficientvitsamMaskDecoderConfig, EfficientvitsamPromptEncoderConfig, EfficientvitsamVisionConfig, pipeline
...
if is_torch_available():
    ...
    from transformers import EfficientvitsamModel, EfficientvitsamProcessor, EfficientvitsamVisionModel
...
class EfficientvitsamPromptEncoderTester:
    def __init__(
        self,
        hidden_size=32,
        input_image_size=24,
...
class EfficientvitsamMaskDecoderTester:
    def __init__(
        self,
        hidden_size=32,
        hidden_act="relu",
...
class EfficientvitsamModelTester:
    def __init__(
        ...
        self.prompt_encoder_tester = EfficientvitsamPromptEncoderTester()
        self.mask_decoder_tester = EfficientvitsamMaskDecoderTester()
...
class EfficientvitsamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {"feature-extraction": EfficientvitsamModel, "mask-generation": EfficientvitsamModel} if is_torch_available() else {}
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=EfficientvitsamConfig, has_text_modality=False, common_properties=common_properties
        )
    ...
    def test_model_from_pretrained(self):
        ...
        model = EfficientvitsamModel.from_pretrained(model_name)
...
@slow
class EfficientvitsamModelIntegrationTest(unittest.TestCase):
    def tearDown(self):
        super().tearDown()
    ...
    def test_inference_mask_generation_no_point(self):
        model = EfficientvitsamModel.from_pretrained("facebook/sam-vit-base")
        processor = EfficientvitsamProcessor.from_pretrained("facebook/sam-vit-base")
