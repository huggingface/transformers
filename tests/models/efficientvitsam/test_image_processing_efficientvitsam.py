...
from transformers import EfficientvitsamImageProcessor, EfficientvitsamImageProcessorPil
from transformers.testing_utils import require_torch, require_vision


@require_torch
@require_vision
class EfficientvitsamImageProcessorTest(unittest.TestCase):
    def test_default_and_pil_processors_match(self):
        from PIL import Image

        image = Image.fromarray(np.random.default_rng(0).integers(0, 256, size=(321, 517, 3), dtype=np.uint8))

        default_processor = EfficientvitsamImageProcessor()
        pil_processor = EfficientvitsamImageProcessorPil()

        default_outputs = default_processor(image, return_tensors="pt")
        pil_outputs = pil_processor(image, return_tensors="pt")

        np.testing.assert_allclose(
            default_outputs["pixel_values"].numpy(),
            pil_outputs["pixel_values"].numpy(),
            rtol=0.0,
            atol=0.0,
        )
        np.testing.assert_array_equal(
            default_outputs["reshaped_input_sizes"].numpy(),
            pil_outputs["reshaped_input_sizes"].numpy(),
        )

    def test_post_process_masks_upsamples_from_prompt_size(self):
        import torch

        processor = EfficientvitsamImageProcessor()
        masks = [torch.zeros(1, 3, 256, 256)]
        output_masks = processor.post_process_masks(
            masks=masks,
            original_sizes=[(321, 517)],
            reshaped_input_sizes=[(318, 512)],
            binarize=False,
        )

        self.assertEqual(output_masks[0].shape, (1, 3, 321, 517))
...
pytest.skip("Legacy SAM-wrapper image-processor tests are obsolete after the MIT EfficientViT-SAM port.", allow_module_level=True)


class EfficientvitsamImageProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=7,
