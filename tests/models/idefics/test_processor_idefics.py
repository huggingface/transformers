...
from transformers import (AutoProcessor, IdeficsImageProcessor,
                          IdeficsProcessor, LlamaTokenizerFast,
...
class IdeficsProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    ...
    def prepare_prompts(self):
        ...
        num_images = 2
        images = [
            np.random.randint(255, size=(3, 30, 400), dtype=np.uint8)
    ...
    def test_processor(self):
        ...
        input_processor = processor(
            text=prompts, return_tensors="pt", padding="longest"
        )
    ...
    def test_tokenizer_decode(self):
        ...
        processor = IdeficsProcessor(
            tokenizer=tokenizer, image_processor=image_processor, return_tensors="pt"
        )
    ...
    def test_tokenizer_padding(self):
        ...
        self.assertListEqual(
            max_length["attention_mask"][-1].tolist(), predicted_attention_masks[1]
        )
        self.assertListEqual(
            longest["attention_mask"][-1].tolist(), predicted_attention_masks[0]
        )
    ...
    def test_tokenizer_left_padding(self):
        ...
        prompts = [[prompt] for prompt in self.prepare_prompts()[2]]
        max_length = processor(
            text=prompts, padding="max_length", truncation=True, max_length=20
        )
        longest = processor(
            text=prompts, padding="longest", truncation=True, max_length=30
        )
