...
class MptModelTester:
    ...
    def get_large_model_config(self):
        return MptConfig.from_pretrained("bigscience/mpt")
...
class MptModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def test_simple_generation(self):
        ...
        path_560m = "mosaicml/mpt-7b"
        model = MptForCausalLM.from_pretrained(path_560m, use_cache=True, revision="gs555750").cuda()
        model = model.eval()
        tokenizer = MptTokenizer.from_pretrained(path_560m)
    ...
    def test_batch_generation(self):
        ...
        model = MptForCausalLM.from_pretrained(path_560m, use_cache=True, revision="gs555750").cuda()
        model = model.eval()
        tokenizer = MptTokenizer.from_pretrained(path_560m, padding_side="left")
...
@require_torch
class MptEmbeddingTest(unittest.TestCase):
    """
    ...
    def test_embeddings(self):
        ...
        model = MptForCausalLM.from_pretrained(self.path_bigscience_model, torch_dtype="auto")
        model.eval()
    ...
    def test_hidden_states_transformers(self):
        ...
        model = MptModel.from_pretrained(self.path_bigscience_model, use_cache=False, torch_dtype="auto").to(
            torch_device
        )
        model.eval()
    ...
    def test_logits(self):
        cuda_available = torch.cuda.is_available()
        model = MptForCausalLM.from_pretrained(self.path_bigscience_model, use_cache=False, torch_dtype="auto").to(
            torch_device
        )  # load in bf16
        model.eval()
