...
from transformers import MptTokenizer
...
@require_tokenizers
class MptTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    slow_tokenizer_class = None
    rust_tokenizer_class = MptTokenizer
    tokenizer_class = MptTokenizer
    ...
    def setUp(self):
        super().setUp()
        tokenizer = MptTokenizer.from_pretrained("bigscience/tokenizer")
        tokenizer.save_pretrained(self.tmpdirname)

    def get_rust_tokenizer(self, **kwargs):
        kwargs.update(self.special_tokens_map)
        return MptTokenizer.from_pretrained(self.tmpdirname, **kwargs)
        ...
        for tokenizer, pretrained_name, kwargs in self.tokenizers_list:
            with self.subTest(f"{tokenizer.__class__.__name__} ({pretrained_name})"):
                ...
                try:
                    ...
                    self.fail("Mpt Tokenizer should be able to deal with padding")
    ...
    def test_pretrained_model_lists(self):
        # The test has to be overriden because MPT uses ALiBi positional embeddings that does not have
