...
class MusicgenStereoIntegrationTests(unittest.TestCase):
    ...
    def test_generate_unconditional_greedy(self):
        ...
        EXPECTED_VALUES_LEFT = torch.tensor(
            [
                ...
                -0.0011, -0.0015, -0.0018, -0.0032, -0.0060, -0.0082, -0.0097, -0.0099,
            ]
        )
        EXPECTED_VALUES_RIGHT = torch.tensor(
            [
                0.0038,  0.0029,  0.0031,  0.0032,  0.0031,  0.0032,  0.0030,  0.0019,
    ...
    def test_generate_text_audio_prompt(self):
        ...
        EXPECTED_VALUES_LEFT = torch.tensor(
            [
                 0.2536,  0.2009,  0.1471,  0.0896,  0.0306, -0.0200, -0.0501, -0.0728,
                -0.0833, -0.0857, -0.0868, -0.0885, -0.0865, -0.0868, -0.0745, -0.0430,
            ]
        )
        EXPECTED_VALUES_RIGHT = torch.tensor(
            [
                 0.1695,  0.1213,  0.0732,  0.0238, -0.0265, -0.0705, -0.0936, -0.1104,
                -0.1164, -0.1140, -0.1105, -0.1083, -0.1028, -0.1006, -0.0901, -0.0614,
            ]
        )
        # fmt: on
