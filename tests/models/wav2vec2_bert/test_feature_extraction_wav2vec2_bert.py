...
from transformers import Wav2Vec2BERTFeatureExtractor, is_speech_available
...
@require_torch
class Wav2Vec2BERTFeatureExtractionTester(unittest.TestCase):
    def __init__(
        self,
        parent,
        batch_size=7,
...
@require_torch
class Wav2Vec2BERTFeatureExtractionTest(SequenceFeatureExtractionTestMixin, unittest.TestCase):
    feature_extraction_class = Wav2Vec2BERTFeatureExtractor if is_speech_available() else None

    def setUp(self):
        self.feat_extract_tester = Wav2Vec2BERTFeatureExtractionTester(self)
    ...
    def test_call_numpy(self):
        ...
        self.assertTrue(input_values.shape[-1] == feature_extractor.feature_size * feature_extractor.stride)
    ...
    def test_attention_mask(self):
        ...
        self.assertTrue(attention_mask.shape[-1] == input_values.shape[1])
    ...
    def test_call_torch(self):
        ...
        # Test feature size
        input_values = feature_extractor(pt_speech_inputs, padding=True, return_tensors="pt").input_values
        self.assertTrue(input_values.ndim == 3)
        self.assertTrue(input_values.shape[0] == 3)
        self.assertTrue(input_values.shape[-1] == feature_extractor.feature_size * feature_extractor.stride)

        # Test not batched input
        encoded_sequences_1 = feature_extractor(speech_inputs[0], return_tensors="pt").input_values
        encoded_sequences_2 = feature_extractor(pt_speech_inputs[0], return_tensors="pt").input_values
        self.assertTrue(torch.allclose(encoded_sequences_1, encoded_sequences_2, atol=1e-3))

        # Test batched
        encoded_sequences_1 = feature_extractor(speech_inputs, return_tensors="pt").input_values
        encoded_sequences_2 = feature_extractor(pt_speech_inputs, return_tensors="pt").input_values
        for enc_seq_1, enc_seq_2 in zip(encoded_sequences_1, encoded_sequences_2):
    ...
    def test_integration(self):
        ...
        feature_extractor(input_speech, return_tensors="pt").input_values[0, 5, :30]
        self.assertEqual(input_values.shape, (1, 279, 160))
        self.assertTrue(torch.allclose(input_values[0, 5, :30], EXPECTED_INPUT_FEATURES, atol=1e-4))
