...
from transformers import ChameleonConfig, is_torch_available, set_seed
...
class ChameleonIntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        ...
        model = ChameleonForCausalLM.from_pretrained("meta-chameleon/meta-chameleon/chameleon-hf", device_map="auto")
    ...
    def test_model_13bf_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChameleonForCausalLM.from_pretrained("meta-chameleon/Chameleon-2-13b-chat-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
        # Expected mean on dim = -1
    ...
    def test_model_70b_logits(self):
        input_ids = [1, 306, 4658, 278, 6593, 310, 2834, 338]
        model = ChameleonForCausalLM.from_pretrained("meta-chameleon/Chameleon-2-70b-hf", device_map="auto")
        out = model(torch.tensor(input_ids))
