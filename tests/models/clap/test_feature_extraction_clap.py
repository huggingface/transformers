...
class ClapFeatureExtractionTest(SequenceFeatureExtractionTestMixin, unittest.TestCase):
    ...
    def test_integration_fusion_short_input(self):
        ...
        for padding, EXPECTED_VALUES, idx_in_mel in zip(
            ...
            input_features = feature_extractor(
                input_speech, return_tensors="pt", padding=padding
            ).input_features
            self.assertEqual(input_features.shape, (1, 4, 1001, 64))

            self.assertTrue(
                torch.allclose(
                    input_features[0, 0, idx_in_mel[0]], EXPECTED_VALUES[0], atol=1e-4
                )
            )
            self.assertTrue(
                torch.allclose(
                    input_features[0, 0, idx_in_mel[1]], EXPECTED_VALUES[1], atol=1e-4
                )
            )
    ...
    def test_integration_fusion_long_input(self):
        ...
        for padding, EXPECTED_VALUES, block_idx in zip(
            ...
            self.assertTrue(
                torch.allclose(
                    input_features[0, block_idx, MEL_BIN], EXPECTED_VALUES, atol=1e-3
                )
            )
    ...
    def test_integration_rand_trunc_long_input(self):
        ...
        for padding, EXPECTED_VALUES, seed in zip(
            ...
            self.assertTrue(
                torch.allclose(
                    input_features[0, 0, MEL_BIN], EXPECTED_VALUES, atol=1e-4
                )
            )
