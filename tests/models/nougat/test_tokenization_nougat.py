...
from transformers.models.nougat.tokenization_nougat_fast import (
    markdown_compatible, normalize_list_like_lines)
from transformers.testing_utils import (require_levenshtein, require_nltk,
                                        require_tokenizers)
...
class NougatTokenizationTest(TokenizerTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason="NougatTokenizerFast does not have tokenizer_file in its signature"
    )
    ...
    @unittest.skip(
        reason="NougatTokenizerFast directly inherits from PreTrainedTokenizerFast"
    )
...
class MarkdownCompatibleTest(unittest.TestCase):
    ...
    def test_url_conversion(self):
        ...
        expected_output = (
            "Visit my website at [https://www.example.com](https://www.example.com)"
        )
        self.assertEqual(markdown_compatible(input_text), expected_output)
...
class NougatPostProcessingTest(unittest.TestCase):
    ...
    def test_correct_tables_basic(self):
        input_str = (
            "\\begin{table} \\begin{tabular}{l l}  & \\ \\end{tabular} \\end{table}"
        )
        expected_output = (
    ...
    def test_postprocess_as_nougat_no_markdown(self):
        ...
        self.assertEqual(
            self.tokenizer.post_process_single(input_str, fix_markdown=False),
            expected_output,
        )
