...
from transformers import (ConditionalDetrConfig, ResNetConfig,
...
class ConditionalDetrModelTester:
    ...
    def create_and_check_conditional_detr_model(
    ...
    def create_and_check_conditional_detr_object_detection_head_model(
        self, config, pixel_values, pixel_mask, labels
    ):
...
@require_torch
class ConditionalDetrModelTest(
    ...
    pipeline_model_mapping = (
        {
            "image-feature-extraction": ConditionalDetrModel,
            "object-detection": ConditionalDetrForObjectDetection,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=ConditionalDetrConfig, has_text_modality=False
        )
    ...
    def test_conditional_detr_object_detection_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_conditional_detr_object_detection_head_model(
            *config_and_inputs
        )
    ...
    @unittest.skip(
        reason="Conditional DETR does not have a get_input_embeddings method"
    )
    ...
    def test_different_timm_backbone(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if model_class.__name__ == "ConditionalDetrForObjectDetection":
                ...
                self.assertEqual(
                    len(
                        model.conditional_detr.model.backbone.conv_encoder.intermediate_channel_sizes
                    ),
                    3,
                )
            else:
...
class ConditionalDetrModelIntegrationTests(unittest.TestCase):
    ...
    def default_image_processor(self):
        return (
            ConditionalDetrImageProcessor.from_pretrained(
    ...
    def test_inference_no_head(self):
        model = ConditionalDetrModel.from_pretrained(
            "microsoft/conditional-detr-resnet-50"
        ).to(torch_device)
        ...
        expected_slice = torch.tensor(
            [
                [0.4222, 0.7471, 0.8760],
                [0.6395, -0.2729, 0.7127],
                [-0.3090, 0.7642, 0.9529],
            ]
        ).to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_slice_logits = torch.tensor(
            [
                [-10.4372, -5.7558, -8.6764],
                [-10.5410, -5.8704, -8.0590],
                [-10.6827, -6.3469, -8.3923],
            ]
        ).to(torch_device)
        torch.testing.assert_close(
        ...
        expected_scores = torch.tensor([0.8330, 0.8313, 0.8039, 0.6829, 0.5355]).to(
            torch_device
        )
        expected_labels = [75, 17, 17, 75, 63]
        expected_slice_boxes = torch.tensor([38.3089, 72.1022, 177.6293, 118.4512]).to(
            torch_device
        )
