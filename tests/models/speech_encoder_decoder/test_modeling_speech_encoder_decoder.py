...
from transformers.testing_utils import (require_deterministic_for_xpu,
                                        require_torch, require_torch_sdpa,
                                        slow, torch_device)
...
if is_torch_available():
    ...
    from transformers import (BertLMHeadModel, SpeechEncoderDecoderConfig,
...
class EncoderDecoderMixin:
    ...
    def test_sdpa_can_dispatch_composite_models(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            self.assertTrue(
                model_sdpa.encoder.config._attn_implementation == encoder_attn
            )
            self.assertTrue(
                model_sdpa.decoder.config._attn_implementation == decoder_attn
            )
            ...
            if encoder_attn == "sdpa" and decoder_attn == "sdpa":
                ...
                self.assertTrue(
                    model_sdpa_explicit.config._attn_implementation == "sdpa"
                )
            else:
...
class Speech2TextBertModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    @unittest.skip(
        reason="Cannot save full model as Speech2TextModel != Speech2TextEncoder"
    )
    def test_encoder_decoder_model_from_pretrained_configs(self):
        pass
