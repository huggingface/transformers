    ...
    def test_model_matches_original_20b(self):
        ...
        with torch.no_grad():
            for i in range(12):
                ...
                torch.testing.assert_close(logprobs, logprob_differences, atol=1e-1, rtol=1e-1)
...
I am a language model, not a human being"""
        ...
        with torch.no_grad():
            for i in range(12):
                ...
                print(f"Generated token: {repr(decoded_token)}, logprob: {selected_logprobs}, logprob differences: {logprob_differences}")
                torch.testing.assert_close(selected_logprobs, original_logprobs[i], atol=1e-1, rtol=1e-1)
