...
def _quantization_config(format):
    """A config shaped like the ones RedHat / Moonshot publish: `Linear` targets minus an ignore list."""
    return {
...
def _build_checkpoints(directory):
    """Write a tiny mxfp4-pack-quantized MoE checkpoint and its dequantized twin.
    ...
    compressor = BaseCompressor.load_from_registry(format)
    ...
    def compress(weight):
        ...
        scale = torch.exp2(torch.floor(torch.log2(groups.abs().amax(-1) / 6.0))).clamp(min=2.0**-127)
        compressed = compressor.compress({"weight": weight, "weight_scale": scale}, scheme)
        dequantized = compressor.decompress(dict(compressed), scheme)["weight"].to(weight.dtype)
        return compressed["weight_packed"], compressed["weight_scale"], dequantized
    ...
    for key, value in model.state_dict().items():
        ...
        for expert in range(config.num_experts):
            ...
            for name, weight in parts.items():
                packed, scale, dense = compress(weight.contiguous())
                packed_state[f"{prefix}.{expert}.{name}.weight_packed"] = packed
                packed_state[f"{prefix}.{expert}.{name}.weight_scale"] = scale
    ...
    for path, state, quantization_config in (
        (packed_dir, packed_state, _quantization_config(format)),
...
def _expert_weight_bytes(model):
    """Bytes held by the expert projections, whether they are dense parameters or packed swizzled ones."""
    total = 0
    for name, module in model.named_modules():
        if not name.endswith(".experts"):
            continue
        for proj in ("gate_up_proj", "down_proj"):
            for attribute in (proj, f"{proj}_swizzled", f"{proj}_swizzled_scales"):
...
class PackedExpertsTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory()
        cls.packed_dir, cls.dense_dir = _build_checkpoints(cls.directory.name)
    ...
    def test_experts_stay_packed_by_default(self):
        ...
        # The dense parameter is replaced by the swizzled packed bytes, kept as plain parameters so
        # device moves and offloading hooks keep managing them.
        ...
        self.assertEqual(experts.config._experts_implementation, "mxfp4")
    ...
    def test_packed_matches_dequantized_reference(self):
        ...
        # The kernels accumulate differently from a dense bf16 matmul, so this is a tolerance not an equality.
