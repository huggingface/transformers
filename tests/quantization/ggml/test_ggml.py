...
class GgufIntegrationTests(unittest.TestCase):
    ...
    bloom_model_id = "RichardErkhov/bigscience_-_bloom-560m-gguf"
    ...
    q4_bloom_model_id = "bloom-560m.Q4_0.gguf"
    ...
    def test_bloom_q4_0(self):
        tokenizer = AutoTokenizer.from_pretrained(self.bloom_model_id, gguf_file=self.q4_bloom_model_id)
        model = AutoModelForCausalLM.from_pretrained(
            self.bloom_model_id,
            gguf_file=self.q4_bloom_model_id,
            device_map="auto",
            torch_dtype=torch.float16,
        )
