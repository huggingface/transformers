...
def main():
    ...
    gen_cfg = GenerationConfig(max_new_tokens=MAX_NEW, do_sample=False, pad_token_id=tok.eos_token_id or 0)
    ...
    print(
        f"{len(PROMPTS)} prompts × {MAX_NEW} new tokens (target) — "
    ...
    print("  sample req-0 first 60 chars:", tok.decode(res[list(res.keys())[0]].generated_tokens)[:60])
