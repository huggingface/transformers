...
_ = m.generate_batch(
    inputs=inputs[:1], generation_config=gen_cfg, continuous_batching_config=cb_cfg, progress_bar=False, warmup=True
)
torch.mps.synchronize()
t0 = time.perf_counter()
res = m.generate_batch(inputs=inputs, generation_config=gen_cfg, continuous_batching_config=cb_cfg, progress_bar=False)
torch.mps.synchronize()
dt = time.perf_counter() - t0
total_new = sum(len(r.generated_tokens) for r in res.values())
print(
    f"{len(PROMPTS)} prompts × ≤{MAX_NEW} tok — {total_new} actual gen tokens in {dt:.2f}s = {total_new / dt:.1f} tok/s aggregate"
)
print("  sample req-0 first 80 chars:", repr(tok.decode(res[list(res.keys())[0]].generated_tokens)[:80]))
