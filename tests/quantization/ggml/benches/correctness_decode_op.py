...
def run(use_decode):
    ...
    import transformers.integrations.gguf_linear as g
    ...
    model = (
        Auto.from_pretrained(repo, gguf_file="Qwen1.5-MoE-A2.7B_q4_k_m.gguf", gguf_linear=True, dtype=torch.float32)
        .to("mps")
        .eval()
    )
...
print(
    f"top-1 match: {top5_a.indices[0] == top5_b.indices[0]} ({top5_a.indices[0].item()} vs {top5_b.indices[0].item()})"
)
