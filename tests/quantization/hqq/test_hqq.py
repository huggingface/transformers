...
from transformers.testing_utils import (require_accelerate, require_hqq,
                                        require_torch_gpu,
...
class HQQSerializationTest(unittest.TestCase):
    ...
    def test_model_serialization_dynamic_quant_with_skip(self):
        ...
        self.assertEqual(
            isinstance(model.model.layers[1].mlp.down_proj, torch.nn.Linear), True
        )
        self.assertEqual(
            model.model.layers[1].self_attn.v_proj.quant_config["weight_quant_params"][
                "nbits"
            ],
            4,
        )
        self.assertEqual(
            model.model.layers[1].mlp.gate_proj.quant_config["weight_quant_params"][
                "nbits"
            ],
            3,
        )
