# coding=utf-8
# Copyright 2018 Phobert Authors.
#
...
if is_torch_available():
    ...
    from transformers.modeling_phobert import PHOBERT_PRETRAINED_MODEL_ARCHIVE_LIST
...
class PhobertModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    class PhobertModelTester(object):
        def __init__(
            self,
            parent,
            batch_size=13,
        ...
        def create_and_check_phobert_for_sequence_classification(
            ...
            model = PhobertForSequenceClassification(config)
            model.to(torch_device)
            model.eval()
            loss, logits = model(
        ...
        def create_and_check_phobert_for_token_classification(
            ...
            config.num_labels = self.num_labels
            model = PhobertForTokenClassification(config=config)
            model.to(torch_device)
            model.eval()
            loss, logits = model(
        ...
        def create_and_check_phobert_for_multiple_choice(
            self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
        ):
            config.num_choices = self.num_choices
            model = PhobertForMultipleChoice(config=config)
            model.to(torch_device)
            model.eval()
    ...
    def setUp(self):
        self.model_tester = PhobertModelTest.PhobertModelTester(self)
        self.config_tester = ConfigTester(self, config_class=PhobertConfig, hidden_size=37)
    ...
    def test_for_multiple_choice(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_phobert_for_multiple_choice(*config_and_inputs)

    @slow
    def test_model_from_pretrained(self):
        for model_name in PHOBERT_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = PhobertModel.from_pretrained(model_name)
