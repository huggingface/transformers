...
if is_torch_available():
    ...
    from transformers import (
        ViltForCausalLM,
        ViltForMaskedLM,
        ViltForMultipleChoice,
        ViltForQuestionAnswering,
        ViltForSequenceClassification,
...
class ViltModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = ViltForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = ViltForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = ViltForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = ViltForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class ViltModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (ViltForCausalLM,) if is_torch_available() else ()
...
class ViltModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = ViltForMaskedLM.from_pretrained("naver/vilt-base-patch32")
