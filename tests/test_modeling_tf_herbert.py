...
from transformers import HerbertConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.modeling_tf_herbert import (
...
class TFHerbertModelTester:
    ...
    def create_and_check_bert_model(
        ...
        model = TFHerbertModel(config=config)
    ...
    def create_and_check_bert_for_masked_lm(
        ...
        model = TFHerbertForMaskedLM(config=config)
        inputs = {
            "input_ids": input_ids,
    ...
    def create_and_check_bert_for_sequence_classification(
        ...
        model = TFHerbertForSequenceClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_bert_for_multiple_choice(
        ...
        model = TFHerbertForMultipleChoice(config=config)
    ...
    def create_and_check_bert_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = TFHerbertForTokenClassification(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
    ...
    def create_and_check_bert_for_question_answering(
        ...
        model = TFHerbertForQuestionAnswering(config=config)
        inputs = {
            "input_ids": input_ids,
            "attention_mask": input_mask,
            "token_type_ids": token_type_ids,
...
@require_tf
class TFHerbertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        self.model_tester = TFHerbertModelTester(self)
        self.config_tester = ConfigTester(self, config_class=HerbertConfig, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        model = TFHerbertModel.from_pretrained("jplu/tiny-tf-bert-random")
        self.assertIsNotNone(model)

    def test_custom_load_tf_weights(self):
        model, output_loading_info = TFHerbertForTokenClassification.from_pretrained(
