...
def prepare_video(
    num_frames, num_channels, width=10, height=10, numpify=False, torchify=False
):
...
class ImageProcessingTestMixin:
    ...
    def test_fast_is_faster_than_slow(self):
        ...
        if (
            ...
            self.skipTest(
                reason="Skipping speed test as one of the image processors is not defined"
            )
    ...
    def test_image_processor_to_json_file(self):
        for image_processing_class in self.image_processor_list:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                image_processor_second = image_processing_class.from_json_file(
                    json_file_path
                )
    ...
    def test_image_processor_from_and_save_pretrained(self):
        for image_processing_class in self.image_processor_list:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                image_processor_second = image_processing_class.from_pretrained(
                    tmpdirname
                )
    ...
    def test_image_processor_save_load_with_autoimageprocessor(self):
        for i, image_processing_class in enumerate(self.image_processor_list):
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                use_fast = i == 1
                image_processor_second = AutoImageProcessor.from_pretrained(
                    tmpdirname, use_fast=use_fast
                )
    ...
    def test_save_load_fast_slow(self):
        ...
        dict_slow_0 = {
            key: dict_slow_0[key] for key in set(dict_slow_0) & set(dict_slow_1)
        }
        dict_slow_1 = {
            key: dict_slow_1[key] for key in set(dict_slow_0) & set(dict_slow_1)
        }
        ...
        dict_fast_0 = {
            key: dict_fast_0[key] for key in set(dict_fast_0) & set(dict_fast_1)
        }
        dict_fast_1 = {
            key: dict_fast_1[key] for key in set(dict_fast_0) & set(dict_fast_1)
        }
        # check that all additional keys are None, except for `default_to_square` and `data_format` which are only set in fast processors
        self.assertTrue(
            all(
    ...
    def test_cast_dtype_device(self):
        for image_processing_class in self.image_processor_list:
            if self.test_cast_dtype is not None:
                ...
                encoding = image_processor(image_inputs, return_tensors="pt").to(
                    torch.float16
                )
                self.assertEqual(encoding.pixel_values.device, torch.device("cpu"))
                ...
                with self.assertRaises(TypeError):
                    _ = image_processor(image_inputs, return_tensors="pt").to(
                        torch.bfloat16, "cpu"
                    )
    ...
    def test_can_compile_fast_image_processor(self):
        if self.fast_image_processing_class is None:
            self.skipTest(
                "Skipping compilation test as fast image processor is not defined"
            )
        ...
        output_eager = image_processor(
            input_image, device=torch_device, return_tensors="pt"
        )

        image_processor = torch.compile(image_processor, mode="reduce-overhead")
        output_compiled = image_processor(
            input_image, device=torch_device, return_tensors="pt"
        )

        torch.testing.assert_close(
            output_eager.pixel_values,
            output_compiled.pixel_values,
            rtol=1e-4,
            atol=1e-4,
        )
...
class AnnotationFormatTestMixin:
    ...
    def test_processor_can_use_legacy_annotation_format(self):
        ...
        fixtures_path = (
            pathlib.Path(__file__).parent / "fixtures" / "tests_samples" / "COCO"
        )
            ...
            with self.subTest(annotation_format):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    image_processor_second = (
                        self.image_processing_class.from_pretrained(tmpdirname)
                    )
