...
# Skip this test in that case to avoid OSError: [WinError 1314]
def _can_create_symlinks():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            link_file = Path(tmpdir) / "test_link.txt"
            link_file.symlink_to(test_file)
            return link_file.exists() and link_file.read_text() == "test"
...
@pytest.mark.skipif(
    ...
    reason="Platform cannot create symlinks (Windows without Developer Mode/admin)"
)
