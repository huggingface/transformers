...
class ModelUtilsTest(TestCasePlus):
    ...
    def test_model_from_pretrained_meta_device(self):
        def is_on_meta(model_id, dtype):
            with torch.device("meta"):
                ...
                return all(
                    value.device.type == "meta" for value in model.state_dict().values()
                )
            ...
            for module in model.modules():
                if "Attention" in module.__class__.__name__:
                    self.assertEqual(
                        ...
                        mistral_attention_classes[requested_attn_implementation],
                    )
    ...
    def test_model_from_pretrained_from_mlx(self):
        ...
        with torch.no_grad():
            ...
            self.assertTrue(
                torch.allclose(outputs_from_saved["logits"], outputs["logits"])
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with LoggingLevel(logging.INFO):
                with CaptureLogger(logger) as cl1:
                    _, loading_info = TestModelGamma.from_pretrained(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with LoggingLevel(logging.INFO):
                with CaptureLogger(logger) as cl2:
                    _, loading_info = TestModelBeta.from_pretrained(
                        tmp_dir, config=config, output_loading_info=True
                    )
