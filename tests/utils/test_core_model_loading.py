...
class TestWeightGlobMatching(unittest.TestCase):
    ...
    def test_exact_match(self):
        self.assertEqual(
            match_glob("embed_tokens.weight", self.alt_digits, self.map_digits),
            "embed_tokens.weight"
        )
    ...
    def test_digits_only_star_rejects_nondigits(self):
        ...
        self.assertIsNone(
            match_glob("model.layers.a.mlp.gate_up_proj.weight", self.alt_digits, self.map_digits)
        )
    ...
    def test_no_match(self):
        self.assertIsNone(
            match_glob("model.layers.0.mlp.up_proj.weight", self.alt_digits, self.map_digits)
        )
    ...
    def test_leftmost_alternative_wins_for_overlapping_patterns(self):
        ...
        self.assertEqual(
            match_glob("model.layers.0.mlp.gate_up_proj.weight", alt, mapping),
    ...
    def test_anchor_full_match_only(self):
        ...
        self.assertIsNone(
            match_glob("foo.model.layers.0.mlp.gate_up_proj.weight.bar", self.alt_any, self.map_any)
        )
    ...
    def test_large_batch_performance_smoke(self):
        ...
        self.assertEqual(
            match_glob(key, alt, mapping),
            "model.layers.*.mlp.block57.weight"
        )
