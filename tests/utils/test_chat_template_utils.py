...
class JsonSchemaGeneratorTest(unittest.TestCase):
    ...
    def test_single_element_tuple_fails(self):
        ...
        # Single-element tuples should just be the type itself, orlist[type] for variable-length inputs
        with self.assertRaises(TypeHintParsingException):
            get_json_schema(fn)
    ...
    def test_ellipsis_type_fails(self):
        ...
        # Variable length inputs should be specified withlist[type], nottuple[type, ...]
        with self.assertRaises(TypeHintParsingException):
            get_json_schema(fn)
