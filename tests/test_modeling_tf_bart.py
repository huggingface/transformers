...
from .utils import CACHE_DIR, slow, torch_device
...
if is_tf_available():
    ...
    from transformers.modeling_tf_bart import (
...
class BARTModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_advanced_inputs(self):
        ...
        def _check_var(module):
            ...
            self.assertAlmostEqual(tf.std(module.weight).item(), config.init_std, 2)
...
class TFBartHeadTests(unittest.TestCase):
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = TFBartForConditionalGeneration(config).to(torch_device)
        context = tf.Tensor([[71, 82, 18, 33, 46, 91, 2], [68, 34, 26, 58, 30, 2, 1]]).long().to(torch_device)
        summary = tf.Tensor([[82, 71, 82, 18, 2], [58, 68, 2, 1, 1]]).long().to(torch_device)
        loss, logits, enc_features = lm_model.forward(input_ids=context, decoder_input_ids=summary, lm_labels=summary)
        expected_shape = (*summary.shape, config.vocab_size)
        self.assertEqual(logits.shape, expected_shape)

    def test_generate_beam_search(self):
        input_ids = tf.Tensor([[71, 82, 2], [68, 34, 2]]).long().to(torch_device)
        config = BartConfig(
            vocab_size=self.vocab_size,
            d_model=24,
            encoder_layers=2,
    ...
    def test_shift_tokens_right(self):
        input_ids = tf.Tensor([[71, 82, 18, 33, 2, 1, 1], [68, 34, 26, 58, 30, 82, 2]]).long()
    ...
    def test_generate_fp16(self):
        ...
        model = TFBartForConditionalGeneration(config).eval().to(torch_device).half()
    ...
    def test_base_model_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data(output_past=False)
        attention_mask = input_ids.ne(1).to(torch_device)
        lm_model = TFBartForConditionalGeneration(config).eval().to(torch_device).half()
    ...
    def test_prepare_bart_decoder_inputs(self):
        ...
        expected_just_padding_mask = tf.Tensor(
...
def _long_tensor(tok_lst):
    return tf.Tensor(tok_lst, dtype=torch.long, device=torch_device,)
...
class TFBartModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = TFBartModel.from_pretrained("bart-large").to(torch_device)
    ...
    def test_mnli_inference(self):
        ...
        expected_slice = tf.Tensor([[0.1907, 1.4342, -1.0289]]).to(torch_device)
    ...
    def test_cnn_summarization_same_as_fairseq_easy(self):
        hf = TFBartForConditionalGeneration.from_pretrained("bart-large-cnn", output_past=True,).to(torch_device)
