...
class TFBartModelIntegrationTest(unittest.TestCase):
    ...
    def test_xsum_1_1_generation(self):
        hf = TFBartForConditionalGeneration.from_pretrained("sshleifer/distilbart-xsum-1-1", from_pt=True)
        tok = self.tok
        ...
        EXPECTED_SUMMARY_SHORTER = " The International Criminal Court (ICC) has announced that it is set to take over as part of an investigation into allegations of war crimes."

        dct = tok(ARTICLE, return_tensors="tf")
        generated_ids = hf.generate(**dct)
        result = tok.batch_decode(generated_ids, skip_special_tokens=True)[0]
        self.assertEqual(EXPECTED_SUMMARY_SHORTER, result)
    ...
    def test_cnn_summarization_same_as_fairseq_hard(self):
        ...
        hypotheses_batch = hf.generate(**dct)
