...
class ProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_prophetnet_pretrained_checkpoint_hidden_states(self):
        ...
        # notice the bos of encoder_ids will be removed in the model
    ...
    def test_xprophetnet_pretrained_checkpoint_hidden_states(self):
        model = ProphetNetForConditionalGeneration.from_pretrained("microsoft/xprophetnet-large-wiki100-cased")
        model.to(torch_device)

        # encoder-decoder outputs
        # notice the bos of encoder_ids will be removed in the model
        encoder_ids = torch.tensor([[2, 17, 96208, 103471, 2]]).to(torch_device)
    ...
    def test_xprophetnet_ntg_inference(self):
        ...
        input_ids = tokenizer(
            [EN_SENTENCE, RU_SENTENCE, ZH_SENTENCE], padding=True, max_length=256, return_tensors="pt"
        ).input_ids
    ...
    def test_prophetnet_cnndm_inference(self):
        ...
        input_ids = tokenizer([ARTICLE_TO_SUMMARIZE], max_length=512, return_tensors="pt").input_ids
        input_ids = input_ids.to(torch_device)
        ...
        input_ids = tokenizer([ARTICLE_TO_SUMMARIZE], max_length=100, return_tensors="pt").input_ids
        input_ids = input_ids.to(torch_device)
