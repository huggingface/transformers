...
if is_torch_available():
    ...
    from transformers.models.imagegpt.modeling_imagegpt import IMAGEGPT_PRETRAINED_MODEL_ARCHIVE_LIST
...
class ImageGPTModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = ImageGPTForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = ImageGPTForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = ImageGPTForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = ImageGPTForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = ImageGPTForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = ImageGPTForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class ImageGPTModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=ImageGPTConfig, hidden_size=37)
...
class ImageGPTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = ImageGPTForMaskedLM.from_pretrained("openai/imagegpt-small")
