...
class CharacterBertModelTester:
    ...
    def create_and_check_for_next_sequence_prediction(
        ...
        model = CharacterBertForNextSentencePrediction(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_pretraining(
        ...
        model = CharacterBertForPreTraining(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_multiple_choice(
        ...
        model.to(torch_device)
        model.eval()
        multiple_choice_inputs_ids = input_ids.unsqueeze(1).expand(-1, self.num_choices, -1, 50).contiguous()
...
class CharacterBertModelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = CharacterBertConfigTester(self, config_class=CharacterBertConfig, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        for model_name in CHARACTER_BERT_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = CharacterBertModel.from_pretrained(model_name)
...
class CharacterBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        expected_shape = torch.Size((1, 6, mlm_vocab_size))
