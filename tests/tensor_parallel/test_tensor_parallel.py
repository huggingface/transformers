...
@is_tensor_parallel_test
class TestRouterParallel(TestCasePlus):
    class MockDeviceMesh:
        def __init__(self, ep_size, ep_rank):
    ...
    def test_router_parallel_preserves_shape(self):
        num_experts = 8
        top_k = 2
        seq_len = 4
        ep_size = 2
        ...
        router_scores = torch.rand(seq_len, top_k).abs() + 0.1  # positive scores
        router_indices = torch.tensor([[0, 5], [2, 7], [1, 6], [3, 4]])

        router_parallel = RouterParallel()
        mock_module = SimpleNamespace(num_experts=num_experts)

        for ep_rank in range(ep_size):
            mesh = self.MockDeviceMesh(ep_size=ep_size, ep_rank=ep_rank)
            _, new_scores, new_indices = router_parallel._prepare_output_fn(
                mock_module, (router_logits.clone(), router_scores.clone(), router_indices.clone()), mesh
            )
            self.assertEqual(new_scores.shape, (seq_len, top_k), "router_scores shape changed — Bug 1 regression")
            self.assertEqual(new_indices.shape, (seq_len, top_k), "router_indices shape changed")

    def test_router_parallel_score_index_pairing(self):
        num_experts = 128
        top_k = 8
        seq_len = 3
        ep_size = 4
        ...
        router_logits = torch.randn(seq_len, num_experts)
        router_scores = torch.rand(seq_len, top_k).abs() + 0.01  # all positive
        # Spread indices across all EP ranks
        router_indices = torch.tensor(
            [
                [5, 42, 67, 100, 12, 88, 31, 120],
                [0, 33, 64, 96, 15, 55, 77, 111],
                [28, 60, 90, 125, 3, 40, 70, 105],
            ]
        )

        router_parallel = RouterParallel()
        mock_module = SimpleNamespace(num_experts=num_experts)
        sentinel = num_local  # 32
        ...
        for ep_rank in range(ep_size):
            ...
            # Flatten like expert forwards impls
            flat_scores = new_scores.reshape(-1)
            flat_indices = new_indices.reshape(-1)

            # After remapping, scores and indices must remain element-wise paired.
            # For every position i: if index[i] is a valid local expert, score[i] > 0.
            # If index[i] is the sentinel, score[i] == 0. This is what expert forwards
            # rely on when they flatten both with reshape(-1).

            # Must be same length
            self.assertEqual(len(flat_scores), len(flat_indices), "Flattened scores/indices length mismatch")
            self.assertEqual(len(flat_scores), seq_len * top_k)

            for i in range(len(flat_indices)):
                if flat_indices[i] == sentinel:
                    self.assertEqual(flat_scores[i].item(), 0.0, f"Sentinel at position {i} should have score 0.0")
                else:
                    self.assertGreater(
                        flat_scores[i].item(), 0.0, f"Valid expert at position {i} should have score > 0"
                    )
                    self.assertLess(
                        flat_indices[i].item(), num_local, f"Local index {flat_indices[i]} >= num_local {num_local}"
                    )

    def test_grouped_gemm_shard_tensor_uses_rank_not_device(self):
        class MockMesh:
            def size(self):
                return 2
        ...
        full_weight = torch.randn(num_experts, intermediate, hidden)
        empty_param = torch.empty(num_experts, intermediate, hidden)

        # Simulate GPU 2 with TP-local rank 0
        layer = GroupedGemmParallel(device_mesh=MockMesh(), rank=0, empty_param=empty_param)
        sharded = layer.shard_tensor(full_weight, tensor_idx=None, device="cpu")

        # Should get experts 0-63 (rank 0), not 128-191 (device index 2)
        expected_local = num_experts // 2
        self.assertEqual(sharded.shape[0], expected_local)
        self.assertTrue(torch.equal(sharded, full_weight[:expected_local]))

    def test_grouped_mm_experts_sentinel_handling(self):
        try:
            from transformers.integrations.moe import grouped_mm_experts_forward
        except ImportError:
            self.skipTest("moe module not available")

        if not hasattr(torch.nn.functional, "grouped_mm") and not hasattr(torch, "_grouped_mm"):
            self.skipTest("torch.grouped_mm not available (requires torch >= 2.10)")

        num_experts = 4
        hidden_dim = 16
        intermediate_dim = 8
        sentinel = num_experts  # 4

        # Build a minimal experts module
        module = SimpleNamespace(
            num_experts=num_experts,
            gate_up_proj=torch.randn(num_experts, 2 * intermediate_dim, hidden_dim, dtype=torch.bfloat16),
            down_proj=torch.randn(num_experts, hidden_dim, intermediate_dim, dtype=torch.bfloat16),
            has_gate=True,
            has_bias=False,
            is_transposed=False,
            act_fn=torch.nn.SiLU(),
            _apply_gate=lambda proj_out: torch.nn.SiLU()(proj_out.chunk(2, dim=-1)[0]) * proj_out.chunk(2, dim=-1)[1],
        )

        # 2 tokens, top_k=3, with sentinels at non-local positions
        hidden_states = torch.randn(2, hidden_dim, dtype=torch.bfloat16)
        top_k_index = torch.tensor([[0, 2, sentinel], [1, sentinel, 3]])
        top_k_weights = torch.tensor([[0.5, 0.3, 0.0], [0.4, 0.0, 0.2]], dtype=torch.bfloat16)

        output = grouped_mm_experts_forward(module, hidden_states, top_k_index, top_k_weights)

        self.assertEqual(output.shape, (2, hidden_dim))
        self.assertTrue(output.isfinite().all(), f"Output contains NaN or Inf: {output}")
