    ...
    def test_tp_plan_valid_layer_patterns(self):
        ...
        for plan in valid_plans:
            with warnings.catch_warnings(record=True) as w:
                ...
                layer_warnings = [warning for warning in w 
                                if "Layer pattern" in str(warning.message) and "does not match any parameters" in str(warning.message)]
                
                # Should not have layer pattern warnings for valid patterns
                self.assertEqual(len(layer_warnings), 0, 
