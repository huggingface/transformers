...
class MuonOptimizersTest(unittest.TestCase):
    ...
    def test_muon_with_aux_adam_initialization(self):
        ...
        param_groups = [
            dict(params=hidden_weights, use_muon=True, lr=0.02, weight_decay=0.01, momentum=0.95),
            dict(params=hidden_gains_biases + nonhidden_params, use_muon=False,
                 lr=3e-4, betas=(0.9, 0.95), weight_decay=0.01, eps=1e-8),
        ]
    ...
    def test_muon_with_aux_adam_param_group_validation(self):
        ...
        with self.assertRaises(AssertionError):
            param_groups = [dict(params=[w1], lr=0.02)]
            MuonWithAuxAdam(param_groups)
        
        # Test invalid keys for Muon group
        with self.assertRaises(AssertionError):
            param_groups = [dict(params=[w1], use_muon=True, lr=0.02, invalid_key=True)]
            MuonWithAuxAdam(param_groups)
        
        # Test invalid keys for Adam group
        with self.assertRaises(AssertionError):
            param_groups = [dict(params=[w2], use_muon=False, lr=3e-4, invalid_key=True)]
            MuonWithAuxAdam(param_groups)
    ...
    def test_muon_default_values(self):
        ...
        param_groups = [
            dict(params=[w1], use_muon=True),
            dict(params=[w2], use_muon=False),
        ]
