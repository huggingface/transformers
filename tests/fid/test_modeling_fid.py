...
if is_torch_available() and is_datasets_available() and is_faiss_available():
    ...
    from transformers import (
        ...
        FiDModel,
        FiDRetriever,
        FiDSequenceForGeneration,
        FiDTokenForGeneration,
        RagTokenizer,
    )
...
class FiDTestMixin:
    ...
    all_model_classes = (
        (FiDModel, FiDTokenForGeneration, FiDSequenceForGeneration)
        if is_torch_available() and is_datasets_available() and is_faiss_available()
        else ()
    )
    ...
    def get_retriever(self, config):
        ...
        with patch("transformers.models.fid.retrieval_fid.load_dataset") as mock_load_dataset:
    ...
    def check_model_with_end2end_retriever(
        ...
        for model_class in [FiDTokenForGeneration, FiDSequenceForGeneration]:
...
@require_torch
@require_retrieval
class FiDDPRBartTest(FiDTestMixin, unittest.TestCase):
    @cached_property
...
@require_torch
@require_retrieval
class FiDDPRT5Test(FiDTestMixin, unittest.TestCase):
    @cached_property
    def config_and_inputs(self):
        question_encoder_tester = DPRModelTester(self)
        dpr_config_and_inputs = question_encoder_tester.prepare_config_and_inputs()
...
@require_tokenizers
@require_torch_non_multi_gpu
class FiDModelIntegrationTests(unittest.TestCase):
    @cached_property
    def sequence_model(self):
        return (
            FiDSequenceForGeneration.from_pretrained_question_encoder_generator(
    ...
    def token_model(self):
        return (
            FiDTokenForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base", "facebook/bart-large-cnn"
            )
            .to(torch_device)
            .eval()
    ...
    @slow
    def test_fid_sequence_inference(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        )
        fid_retriever = FiDRetriever(
            fid_config,
            question_encoder_tokenizer=fid_question_encoder_tokenizer,
            generator_tokenizer=fid_decoder_tokenizer,
        )

        fid_sequence = self.sequence_model
        fid_sequence.set_retriever(fid_retriever)

        input_ids = fid_question_encoder_tokenizer(
            "who sings does he love me with reba", return_tensors="pt"
        ).input_ids
        decoder_input_ids = fid_decoder_tokenizer("Linda Davis", return_tensors="pt").input_ids
    ...
    @slow
    def test_fid_token_inference(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        fid_token = self.token_model
        fid_token.set_retriever(fid_retriever)
    ...
    @slow
    def test_fid_token_generate_beam(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        output_ids = fid_token.generate(
            input_ids,
            decoder_start_token_id=fid_token.generator.config.decoder_start_token_id,
            num_beams=2,
            num_return_sequences=2,
        )
        # sequence generate test
        output_text_1 = fid_decoder_tokenizer.decode(output_ids[0], skip_special_tokens=True)
        output_text_2 = fid_decoder_tokenizer.decode(output_ids[1], skip_special_tokens=True)
    ...
    @slow
    def test_fid_sequence_generate_beam(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        output_ids = fid_sequence.generate(
            input_ids,
            decoder_start_token_id=fid_sequence.generator.config.decoder_start_token_id,
            num_beams=2,
            num_return_sequences=2,
        )
        # sequence generate test
    ...
    def test_data_questions(self):
        return [
            ...
            "how many episodes are there in dfidon ball z",
        ]

    @slow
    def test_fid_sequence_generate_batch(self):
        tokenizer = RagTokenizer.from_pretrained("facebook/fid-sequence-nq")
        retriever = FiDRetriever.from_pretrained(
            "facebook/fid-sequence-nq", index_name="exact", use_dummy_dataset=True
        )
        fid_sequence = FiDSequenceForGeneration.from_pretrained("facebook/fid-sequence-nq", retriever=retriever).to(
            torch_device
        )
    ...
    @slow
    def test_fid_sequence_generate_batch_from_context_input_ids(self):
        tokenizer = RagTokenizer.from_pretrained("facebook/fid-sequence-nq")
        retriever = FiDRetriever.from_pretrained(
            "facebook/fid-sequence-nq", index_name="exact", use_dummy_dataset=True
        )
        ...
        question_hidden_states = fid_sequence.question_encoder(input_ids, attention_mask=attention_mask)[0]
        docs_dict = retriever(
    ...
    @slow
    def test_fid_token_generate_batch(self):
        tokenizer = RagTokenizer.from_pretrained("facebook/fid-nq-base")
        retriever = FiDRetriever.from_pretrained("facebook/fid-nq-base", index_name="exact", use_dummy_dataset=True)
        fid_token = FiDTokenForGeneration.from_pretrained("facebook/fid-nq-base", retriever=retriever).to(torch_device)
...
@require_torch
@require_retrieval
class FiDModelSaveLoadTests(unittest.TestCase):
    def get_fid_config(self):
        question_encoder_config = AutoConfig.from_pretrained("facebook/dpr-question_encoder-single-nq-base")
        generator_config = AutoConfig.from_pretrained("facebook/bart-large-cnn")
        return FiDConfig.from_question_encoder_generator_configs(
    ...
    @slow
    def test_fid_sequence_from_pretrained(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            fid_sequence = FiDSequenceForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base",
                "facebook/bart-large-cnn",
                retriever=fid_retriever,
                config=fid_config,
            ).to(torch_device)
            # check that the from pretrained methods work
            fid_sequence.save_pretrained(tmp_dirname)
            fid_sequence.from_pretrained(tmp_dirname, retriever=fid_retriever)
            fid_sequence.to(torch_device)
        ...
        fid_sequence = FiDSequenceForGeneration(
            config=fid_config, question_encoder=question_encoder, generator=generator, retriever=fid_retriever
        )
        fid_sequence.to(torch_device)
    ...
    @slow
    def test_fid_token_from_pretrained(self):
        fid_config = self.get_fid_config()
        fid_decoder_tokenizer = BartTokenizer.from_pretrained("facebook/bart-large-cnn")
        fid_question_encoder_tokenizer = DPRQuestionEncoderTokenizer.from_pretrained(
            "facebook/dpr-question_encoder-single-nq-base"
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            fid_token = FiDTokenForGeneration.from_pretrained_question_encoder_generator(
                "facebook/dpr-question_encoder-single-nq-base",
                "facebook/bart-large-cnn",
                retriever=fid_retriever,
                config=fid_config,
            ...
            # check that the from pretrained methods work
            fid_token.save_pretrained(tmp_dirname)
            fid_token.from_pretrained(tmp_dirname, retriever=fid_retriever)
            fid_token.to(torch_device)

            self.assertTrue(fid_token.question_encoder.config.max_length == 200)
            self.assertTrue(fid_token.generator.config.max_length == 200)
        ...
        question_encoder = AutoModel.from_pretrained("facebook/dpr-question_encoder-single-nq-base")
        generator = AutoModelForSeq2SeqLM.from_pretrained("facebook/bart-large-cnn")
        fid_token = FiDTokenForGeneration(
            config=fid_config, question_encoder=question_encoder, generator=generator, retriever=fid_retriever
        )
        fid_token.to(torch_device)
