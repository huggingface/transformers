...
from .utils_generation import GPT2_PRETRAINED_MODEL_GENERATION_TEST_CASES, GPT2_PADDING_TOKENS
        ...
        def check_language_generation(self, model_generation_test_cases, model_archive_map, lm_model):
            for model_name in list(model_archive_map.keys()):
                if model_name in model_generation_test_cases:
                    ...
                    output_ids = model.generate(input_ids, bos_token_id=self.bos_token_id, pad_token_id=self.pad_token_id, eos_token_ids=self.eos_token_id, max_length=self.max_length, do_sample=self.do_sample, num_return_sequences=self.num_return_sequences, num_beams=self.num_beams)
                    ipdb.set_trace()
...
#    @slow
    def test_gpt2_generation(self):
        ...
        self.model_tester.check_language_generation(GPT2_PRETRAINED_MODEL_GENERATION_TEST_CASES, GPT2_PRETRAINED_MODEL_ARCHIVE_MAP, GPT2LMHeadModel)
