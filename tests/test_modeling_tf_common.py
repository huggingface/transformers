...
class TFModelTesterMixin:
    ...
    def test_pt_tf_model_equivalence(self, allow_missing_keys=False):
        ...
        for model_class in self.all_model_classes:
            ...
            if not set(tf_inputs_dict_with_labels.keys()).symmetric_difference(
                tf_inputs_dict.keys()
            ):
    ...
    def _test_xla_generate(self, **generate_kwargs):
        def _generate_and_check_results(model, inputs, is_input_ids):
            ...
            diff = [[], []]
            for _generated, _generated_xla in zip(
                generated.tolist(), generated_xla.tolist()
            ):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            inputs = (
                inputs_dict["input_ids"]
                if is_input_ids
                else inputs_dict["input_features"]
            )
            ...
            for var_name in ["max_position_embeddings", "max_target_positions"]:
                ...
                if (
                    attr is not None
                    and attr < seq_len + generate_kwargs["max_new_tokens"]
                ):
                    try:
                        setattr(
                            config,
                            var_name,
                            seq_len + generate_kwargs["max_new_tokens"],
                        )
