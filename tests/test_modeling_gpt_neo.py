...
if is_torch_available():
    ...
    from transformers.models.gpt_neo.modeling_gpt_neo import GPT_NEO_PRETRAINED_MODEL_ARCHIVE_LIST
...
class GPTNeoModelTester:
    ...
    def create_and_check_for_masked_lm(
        ...
        model = GPTNeoForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = GPTNeoForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = GPTNeoForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = GPTNeoForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class GPTNeoModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=GPTNeoConfig, hidden_size=37)
...
@require_torch
class GPTNeoModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_masked_lm(self):
        model = GPTNeoForMaskedLM.from_pretrained("gpt_neo_xl")
