...
class GenerationIntegrationTestsMixin:
    ...
    def test_validate_generation_inputs(self):
        ...
        input_ids = tokenizer(
            encoder_input_str, return_tensors=return_tensors
        ).input_ids
        ...
        valid_model_kwargs = {
            "attention_mask": create_tensor_fn(np.zeros_like(input_ids))
        }
        model.generate(input_ids, **valid_model_kwargs)

    def test_custom_logits_processor(self):
        model_cls = self.framework_dependent_parameters["AutoModelForSeq2SeqLM"]
        logits_processor_list_cls = self.framework_dependent_parameters[
            "LogitsProcessorList"
        ]
        min_length_logits_processor_cls = self.framework_dependent_parameters[
        ...
        bart_model = model_cls.from_pretrained(
        ...
        logits_processor.append(
            min_length_logits_processor_cls(min_length=10, eos_token_id=0)
        )
    ...
    def test_transition_scores_greedy_search(self):
        ...
        input_ids = tokenizer(
            articles, return_tensors=return_tensors, padding=True
        ).input_ids
        if is_pt:
    ...
    def test_encoder_decoder_generate_attention_mask(self):
        ...
        input_ids_batched = tokenizer(
            articles, padding=True, return_tensors=return_tensors
        ).input_ids
        if is_pt:
            model = model.to(torch_device)
            input_ids = input_ids.to(torch_device)
    ...
    def test_eos_token_id_int_and_list_beam_search(self):
        ...
        padded_correct_condition = expectation < len(generated_tokens[0]) and all(
            token == model.config.pad_token_id
            for token in generated_tokens[0][expectation:]
        )
    ...
    def test_generate_vision2text_conditioning(self):
        ...
        pixel_values = floats_tensor((2, 3, 30, 30))
        conditioning_input = create_tensor_fn(
            [[10], [10]]
