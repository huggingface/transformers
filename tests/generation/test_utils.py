...
class GenerationIntegrationTests(unittest.TestCase, GenerationIntegrationTestsMixin):
    ...
    def test_biased_generation(self):
        # PT-only test: TF doesn't have a SequenceBiasLogitsProcessor equivalent
        ...
        self.assertEqual(
            summary_text,
            "PG&E scheduled the blackouts in response to forecasts for high winds amid dry conditions. Nearly 800 "
            "thousand customers were scheduled to be affected by the shutoffs.",
        )

        # Use bias to avoid "800 thousand" (use "800,000" instead) and replace "shutoffs" by "shutfoo"
        tokenizer_with_prefix_space = AutoTokenizer.from_pretrained("facebook/bart-large-cnn", add_prefix_space=True)

        def _get_tokens_as_tuple(word):
            return tuple(tokenizer_with_prefix_space([word], add_special_tokens=False).input_ids[0])

        sequence_bias = {
            # negative bias -> less likely to appear
            _get_tokens_as_tuple("thousand"): -10.0,
            # positive bias -> more likely to appear
            # shutfoo is really odd, so we need a bigger bias to make it happen
            _get_tokens_as_tuple("shutfoo"): 100.0,
        }

        biased_ids = model.generate(inputs["input_ids"], num_beams=2, min_length=0, sequence_bias=sequence_bias)
        biased_text = tokenizer.batch_decode(biased_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[
            0
        ]
        self.assertEqual(
            biased_text,
            "PG&E scheduled the blackouts in response to forecasts for high winds amid dry conditions. Nearly 800,000 "
            "customers were scheduled to be affected by the shutfoo.",
        )
