...
class TFLogitsProcessorTest(unittest.TestCase):
    ...
    def test_suppress_tokens_at_begin_logits_processor(self, use_xla):
        ...
        input_ids = tf.convert_to_tensor(
            [[11, 17, 15, 8], [14, 0, 19, 5], [13, 11, 18, 19], [11, 12, 16, 15]]
        )
        scores = self._get_uniform_logits(batch_size, vocab_size)
        scores = logits_processor(input_ids, scores, cur_len)
        self.assertFalse(tf.math.reduce_any(tf.math.is_inf((scores))))
        ...
        self.assertTrue(
            tf.math.reduce_all(
                tf.math.is_inf(tf.gather(scores, begin_suppress_tokens, axis=1))
            )
        )
    ...
    def test_suppress_tokens_logits_processor(self, use_xla):
        ...
        logits_processor = TFSuppressTokensLogitsProcessor(
        ...
        cur_len = 5
        input_ids = tf.convert_to_tensor(
            [[0, 10, 19, 6, 3], [17, 4, 8, 17, 2], [7, 1, 11, 6, 15], [5, 8, 13, 16, 0]]
        )
        scores = self._get_uniform_logits(batch_size, vocab_size)
        scores = logits_processor(input_ids, scores, cur_len)
        self.assertTrue(
            tf.math.reduce_all(
                tf.math.is_inf(tf.gather(scores, suppress_tokens, axis=1))
            )
        )
        self.assertFalse(
            tf.math.reduce_any(tf.math.is_inf(tf.gather(scores, keep_tokens, axis=1)))
        )
    ...
    def test_force_tokens_logits_processor(self, use_xla):
        ...
        tf.debugging.assert_near(
            tf.gather(scores, [force_token_map[cur_len]], axis=1), 0.0
        )

        non_forced_inds = [
            i for i in range(vocab_size) if i != force_token_map[cur_len]
        ]
        self.assertTrue(
            tf.math.reduce_all(
