...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        warped_prob_sharp = jax.nn.softmax(
            temp_dist_warper_sharper(input_ids, scores.copy(), cur_len=None), axis=-1
        )
        warped_prob_smooth = jax.nn.softmax(
            temp_dist_warper_smoother(input_ids, scores.copy(), cur_len=None), axis=-1
        )
    ...
    def test_top_k_dist_warper(self):
        ...
        ramp_logits = np.broadcast_to(
            np.arange(vocab_size)[None, :], (batch_size, vocab_size)
        ).copy()
        ...
        self.assertListEqual(
            jnp.isinf(scores[1]).tolist(), 2 * [True] + 3 * [False] + 5 * [True]
        )

        # check special case
        length = 5
        top_k_warp_safety_check = FlaxTopKLogitsWarper(
            top_k=1, filter_value=0.0, min_tokens_to_keep=3
        )

        ramp_logits = np.broadcast_to(
            np.arange(length)[None, :], (batch_size, length)
        ).copy()
    ...
    def test_min_length_dist_processor(self):
        ...
        scores_before_min_length = min_dist_processor(
    ...
    def test_forced_eos_token_logits_processor(self):
        ...
        logits_processor = FlaxForcedEOSTokenLogitsProcessor(
            max_length=max_length, eos_token_id=eos_token_id
        )
    ...
    def test_no_repeat_ngram_dist_processor(self):
        ...
        self.assertListEqual(
            jnp.isinf(filtered_scores_2_gram).tolist(),
            [[False, True, True], [True, False, False]],
        )

        # 3-gram would forbid no token at 1st batch and 1st token (0) at 2nd batch
        self.assertListEqual(
            jnp.isinf(filtered_scores_3_gram).tolist(),
    ...
    def test_processor_list(self):
        ...
        min_dist_proc = FlaxMinLengthLogitsProcessor(
            min_length=10, eos_token_id=eos_token_id
        )
        bos_dist_proc = FlaxForcedBOSTokenLogitsProcessor(bos_token_id=bos_token_id)
        eos_dist_proc = FlaxForcedEOSTokenLogitsProcessor(
            max_length=max_length, eos_token_id=eos_token_id
        )
