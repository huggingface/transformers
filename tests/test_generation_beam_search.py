...
class ConstrainedBeamSearchTester:
    def __init__(
        ...
        self.constraints = [
            PhrasalConstraint(ids_tensor((1, 2), self.vocab_size)[0])
        ]
    ...
    def prepare_inputs(self):
        constraint_states = [
            ConstraintListState(self.constraints)
        ...
        next_indices = ids_tensor((self.batch_size, 2 * self.num_beams), self.num_beams).to(torch_device)
        next_scores, _ = (-floats_tensor((self.batch_size, 2 * self.num_beams)).to(torch_device)).sort(descending=True)
        return (input_ids, next_tokens, next_indices, next_scores, constraint_states)
    ...
    def check_beam_scorer_update(self, input_ids, next_tokens, next_indices, next_scores, constraint_states):
        # check too many eos tokens
        constrained_beam_scorer = self.prepare_beam_scorer()
        print("---1next_scores", next_scores)

        tokens = next_tokens.clone()
        tokens[0, :] = self.eos_token_id

        with self.parent.assertRaises(ValueError):
            constrained_beam_scorer.process(input_ids, next_scores, tokens, next_indices, constraint_states, eos_token_id=self.eos_token_id)
        ...
        beam_outputs = constrained_beam_scorer.process(
            input_ids, next_scores, tokens, next_indices, constraint_states, eos_token_id=self.eos_token_id
        )
        output_scores = beam_outputs["next_beam_scores"]
        output_tokens = beam_outputs["next_beam_tokens"]
        output_indices = beam_outputs["next_beam_indices"]
...
class ConstrainedBeamSearchTest(unittest.TestCase):
    ...
    def test_beam_scorer_update(self):
        inputs = self.constrained_beam_search_tester.prepare_inputs()
        self.constrained_beam_search_tester.check_beam_scorer_update(*inputs)

    # def test_beam_scorer_finalize(self):
    #     inputs = self.constrained_beam_search_tester.prepare_inputs()
    #     self.constrained_beam_search_tester.check_beam_scores_finalize(*inputs)
