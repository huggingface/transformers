...
class BertModelTester:
    ...
    def create_and_check_tracing_for_masked_lm(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
    ...
    def create_and_check_model_tracing_for_causal_lm_as_decoder(
        self,
        config,
        input_ids,
        token_type_ids,
        ...
        logits_close = torch.allclose(result.logits, traced_result["logits"], atol=1e-2)
        loss_close = torch.allclose(result.loss, traced_result["loss"], atol=1e-3)
        self.parent.assertTrue(logits_close and loss_close)

    def create_and_check_tracing_for_next_sequence_prediction(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
    ...
    def create_and_check_tracing_for_pretraining(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
        ...
        traced = symbolic_trace(
            model,
            input_names=["input_ids", "attention_mask", "token_type_ids", "labels", "next_sentence_label"],
            batch_size=self.batch_size,
            seqlen=self.seq_length,
        )
        traced_result = traced(
        ...
        prediction_logits_close = torch.allclose(result.prediction_logits, traced_result["prediction_logits"])
        seq_relationship_logits_close = torch.allclose(
            result.seq_relationship_logits, traced_result["seq_relationship_logits"]
        )
        loss_close = torch.allclose(result.loss, traced_result["loss"])
        self.parent.assertTrue(prediction_logits_close and seq_relationship_logits_close and loss_close)

    def create_and_check_tracing_for_question_answering(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
    ...
    def create_and_check_tracing_for_sequence_classification(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
    ...
    def create_and_check_tracing_for_token_classification(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
    ...
    def create_and_check_tracing_for_multiple_choice(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        if not is_torch_fx_available():
            self.parent.assertTrue(True)
...
class BertModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_tracing_for_causal_lm(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs_for_decoder()
        self.model_tester.create_and_check_tracing_for_causal_lm(*config_and_inputs)

    def test_tracing_for_masked_lm(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_masked_lm(*config_and_inputs)

    def test_tracing_for_causal_lm_decoder(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs_for_decoder()
        self.model_tester.create_and_check_model_tracing_for_causal_lm_as_decoder(*config_and_inputs)

    def test_tracing_for_next_sequence_prediction(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_next_sequence_prediction(*config_and_inputs)

    def test_tracing_for_pretraining(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_pretraining(*config_and_inputs)

    def test_tracing_for_question_answering(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_question_answering(*config_and_inputs)

    def test_tracing_for_sequence_classification(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_sequence_classification(*config_and_inputs)

    def test_tracing_for_token_classification(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_token_classification(*config_and_inputs)

    def test_tracing_for_multiple_choice(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_tracing_for_multiple_choice(*config_and_inputs)
