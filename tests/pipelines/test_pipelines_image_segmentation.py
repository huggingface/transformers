...
class ImageSegmentationPipelineTests(unittest.TestCase):
    model_mapping = dict(
        (
            list(MODEL_FOR_IMAGE_SEGMENTATION_MAPPING.items())
            if MODEL_FOR_IMAGE_SEGMENTATION_MAPPING
            else []
        )
        + (
            MODEL_FOR_SEMANTIC_SEGMENTATION_MAPPING.items()
            if MODEL_FOR_SEMANTIC_SEGMENTATION_MAPPING
            else []
        )
        + (
            MODEL_FOR_INSTANCE_SEGMENTATION_MAPPING.items()
            if MODEL_FOR_INSTANCE_SEGMENTATION_MAPPING
            else []
        )
    )
    ...
    def run_pipeline_test(self, image_segmenter, examples):
        ...
        self.assertEqual(
            [
                {
                    "score": ANY(float, type(None)),
                    "label": ANY(str),
                    "mask": ANY(Image.Image),
                }
            ]
            * n,
            outputs,
        ...
        for single_output in outputs:
            for output_element in single_output:
                compare_pipeline_output_to_hub_spec(
                    output_element, ImageSegmentationOutputElement
                )
    ...
    def test_small_model_pt(self):
        ...
        output = image_segmenter(
            "http://images.cocodataset.org/val2017/000000039769.jpg", subtask="instance"
        )
        for o in output:
            o["mask"] = mask_to_test_readable(o["mask"])
        self.assertEqual(
        ...
        #
        output = image_segmenter(
            "http://images.cocodataset.org/val2017/000000039769.jpg", subtask="semantic"
        )
        ...
        expected_masks = [
            Image.open(requests.get(image, stream=True).raw) for image in expected_masks
        ]
    ...
    def test_threshold(self):
        ...
        outputs = image_segmenter(
            "http://images.cocodataset.org/val2017/000000039769.jpg", threshold=0.999
        )
        # Shortening by hashing
        for o in outputs:
            o["mask"] = mask_to_test_readable(o["mask"])
    ...
    def test_maskformer(self):
        ...
        image_segmenter = pipeline(
            "image-segmentation", model=model, image_processor=image_processor
        )
