...
class FeatureExtractionPipelineTests(unittest.TestCase, metaclass=PipelineTestCaseMeta):
    ...
    def test_small_model_flax(self):
        feature_extractor = pipeline(
            task="feature-extraction", model="Shubhamai/tiny-random-distilbert", framework="flax"
        )
        outputs = feature_extractor("This is a test")
        self.assertEqual(
            nested_simplify(outputs),
    ...
    @require_flax
    def test_tokenization_small_model_flax(self):
        feature_extractor = pipeline(
            task="feature-extraction", model="Shubhamai/tiny-random-distilbert", framework="flax"
        )
        # test with empty parameters
    ...
    @require_flax
    def test_return_tensors_flax(self):
        feature_extractor = pipeline(
            task="feature-extraction", model="Shubhamai/tiny-random-distilbert", framework="flax"
        )
        outputs = feature_extractor("This is a test", return_tensors=True)
        self.assertTrue(isinstance(outputs, jnp.ndarray))
