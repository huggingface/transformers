...
<<<<<<< HEAD
    ...
    def _test_normal_with_special_tokens_mask(self, no_pad_features, pad_features):
        ...
        tf_data_collator = TFDataCollatorForLanguageModeling(
            tokenizer, padding_length=None, batch_size=2, special_tokens_mask=[0] * 5 + [1] * 5
        )

        tf_data_collator_with_padding = TFDataCollatorForLanguageModeling(
            tokenizer, padding_length=16, batch_size=2, special_tokens_mask=[0] * 11 + [1] * 5
        )

        tf_dataset_no_pad = tf.data.Dataset.from_tensor_slices(no_pad_features)
        tf_dataset_pad = tf.data.Dataset.from_tensor_slices(tf.ragged.stack(pad_features))
        ...
        mask_data_collator = TFDataCollatorForLanguageModeling(
            tokenizer, batch_size=2, special_tokens_mask=[0] * 5 + [1] * 5
        )
    ...
    def _test_normal_without_special_tokens_mask(self, no_pad_features, pad_features):
        ...
        tf_data_collator = TFDataCollatorForLanguageModeling(tokenizer, batch_size=2)

        tf_data_collator_with_padding = TFDataCollatorForLanguageModeling(tokenizer, padding_length=16, batch_size=2)
        ...
        no_token_collator = TFDataCollatorForLanguageModeling(tokenizer, padding_length=5, batch_size=2)
        ...
        set_seed(42)  # For reproducibility
        tokenizer = BertTokenizer(self.vocab_file)
        mask_data_collator = TFDataCollatorForLanguageModeling(tokenizer, batch_size=2)
        batch = mask_data_collator(tf_dataset_no_pad)
    ...
    def test_data_collator_for_language_modeling(self):
        ...
        pad_features = [tf.constant(range(5)), tf.constant(range(10)), tf.constant(range(5)), tf.constant(range(10))]
        self._test_normal_without_special_tokens_mask(no_pad_features, pad_features)
        self._test_normal_with_special_tokens_mask(no_pad_features, pad_features)
    ...
    def test_nsp(self):
        tokenizer = BertTokenizer(self.vocab_file)
        features = {
            "input_ids": [[0, 1, 2, 3, 4] for _ in range(2)],
            "token_type_ids": [[0, 1, 2, 3, 4] for _ in range(2)],
            "next_sentence_label": [i for i in range(2)],
        }
        data_collator = TFDataCollatorForLanguageModeling(tokenizer, batch_size=2)
    ...
    def test_sop(self):
        ...
        data_collator = TFDataCollatorForLanguageModeling(tokenizer, batch_size=2, padding_length=8)
        tf_dataset_pad = tf.data.Dataset.from_tensor_slices(features)
        batch = data_collator(tf_dataset_pad)
