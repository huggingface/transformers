...
class BigScience176BModelTester:
    ...
    def create_and_check_bigscience176b_model(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
    ...
    def create_and_check_bigscience176b_model_past(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        model = BigScience176BModel(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_double_lm_head_model(
        ...
        model = BigScience176BDoubleHeadsModel(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_bigscience176b_for_sequence_classification(
        self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args
    ):
        config.num_labels = self.num_labels
        model = BigScience176BForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=sequence_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.num_labels))

    def create_and_check_bigscience176b_for_token_classification(
        self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args
    ):
        config.num_labels = self.num_labels
        model = BigScience176BForTokenClassification(config)
        model.to(torch_device)
        model.eval()
...
class BigScience176BModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (
        (BigScience176BModel, BigScience176BLMHeadModel, BigScience176BDoubleHeadsModel, BigScience176BForSequenceClassification, BigScience176BForTokenClassification)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (BigScience176BLMHeadModel, BigScience176BDoubleHeadsModel) if is_torch_available() else ()
    all_parallelizable_model_classes = (BigScience176BLMHeadModel, BigScience176BDoubleHeadsModel) if is_torch_available() else ()
    fx_compatible = False
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == "BigScience176BDoubleHeadsModel":
    ...
    def test_bigscience176b_double_lm_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_double_lm_head_model(*config_and_inputs)

    def test_bigscience176b_sequence_classification_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_bigscience176b_for_sequence_classification(*config_and_inputs)

    def test_bigscience176b_token_classification_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_bigscience176b_for_token_classification(*config_and_inputs)

    def test_bigscience176b_gradient_checkpointing(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_forward_and_backwards(*config_and_inputs, gradient_checkpointing=True)

    def test_bigscience176b_scale_attn_by_inverse_layer_idx(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs(scale_attn_by_inverse_layer_idx=True)
        self.model_tester.create_and_check_forward_and_backwards(*config_and_inputs)

    def test_bigscience176b_reorder_and_upcast_attn(self):
...
class BigScience176BModelLanguageGenerationTest(unittest.TestCase):
    ...
    @slow
    def test_lm_generate_bigscience176b_with_reorder_and_upcast_attn(self):
        self._test_lm_generate_bigscience176b_helper(reorder_and_upcast_attn=True)

    @slow
    def test_lm_generate_bigscience176b_with_scale_attn_by_inverse_layer_idx(self):
        self._test_lm_generate_bigscience176b_helper(scale_attn_by_inverse_layer_idx=True, verify_outputs=False)
