...
from transformers import PhobertConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.modeling_tf_phobert import (
...
@require_tf
class TFPhobertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    class TFPhobertModelTester(object):
        def __init__(
            self,
            parent,
            batch_size=13,
        ...
        def create_and_check_phobert_model(
            ...
            model = TFPhobertModel(config=config)
        ...
        def create_and_check_phobert_for_masked_lm(
            ...
            model = TFPhobertForMaskedLM(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        ...
        def create_and_check_phobert_for_sequence_classification(
            ...
            model = TFPhobertForSequenceClassification(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
            (logits,) = model(inputs)
            result = {
                "logits": logits.numpy(),
        ...
        def create_and_check_phobert_for_token_classification(
            ...
            config.num_labels = self.num_labels
            model = TFPhobertForTokenClassification(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
            (logits,) = model(inputs)
            result = {
                "logits": logits.numpy(),
        ...
        def create_and_check_phobert_for_question_answering(
            ...
            model = TFPhobertForQuestionAnswering(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
        ...
        def create_and_check_phobert_for_multiple_choice(
            ...
            model = TFPhobertForMultipleChoice(config=config)
    ...
    def setUp(self):
        self.model_tester = TFPhobertModelTest.TFPhobertModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_name in ["vinai/phobert-base"]:
            model = TFPhobertModel.from_pretrained(model_name)
