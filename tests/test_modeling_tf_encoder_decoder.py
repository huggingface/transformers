...
class TFEncoderDecoderMixin:
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        input_ids_dict = self.prepare_config_and_inputs()
        config = input_ids_dict.pop("config")
        decoder_config = input_ids_dict.pop("decoder_config")
        config.output_hidden_states = True
        decoder_config.output_hidden_states = True
        inputs_dict = {
            k: v
            for k, v in input_ids_dict.items()
            if k in ["input_ids", "decoder_input_ids", "attention_mask", "decoder_attention_mask"]
        }
