...
def _to_local(tensor):
    ...
    if hasattr(tensor, "to_local"):
        ...
        if isinstance(tensor, DTensor) and any(p.is_partial() for p in tensor.placements):
...
def _load_tp_and_reference_models(model_path, model_class, enable_sequence_parallel=False):
    ...
    """
    distributed_config = DistributedConfig(tp_plan="auto", enable_sequence_parallel=enable_sequence_parallel)
    model_tp = model_class.from_pretrained(
        model_path, distributed_config=distributed_config, attn_implementation="eager"
    )
    dist.barrier()

    device = model_tp.device
    model_ref = model_class.from_pretrained(model_path, attn_implementation="eager")
