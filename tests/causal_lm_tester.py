...
class CausalLMModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin):
    ...
    def test_model_rope_with_layer_types(self):
        ...
        flat_rope_dict = config.rope_parameters.pop(type_to_drop)  # save for later
        ...
        config.rope_parameters = flat_rope_dict
...
def _config_supports_rope_scaling(config: PretrainedConfig) -> bool:
    ...
    main_config_has_rope = hasattr(config, "rope_parameters") and hasattr(config, "rope_theta")
    sub_config_has_rope = any(
        hasattr(getattr(config, sub_config), "rope_parameters") or hasattr(getattr(config, sub_config), "rope_theta")
