...
class PipelineTestCaseMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        for configuration, model_architecture in dct.get("model_mapping", {}).items():
            ...
            for tokenizer_class in tokenizer_classes:
                if tokenizer_class is not None and tokenizer_class.__name__.endswith("Fast"):
                    test_name = (
                        f"test_pt_{configuration.__name__}_{model_architecture.__name__}_{tokenizer_class.__name__}"
                    )
