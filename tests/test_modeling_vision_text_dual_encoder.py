...
class VisionTextDualEncoderModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = VisionTextDualEncoderForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = VisionTextDualEncoderForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = VisionTextDualEncoderForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = VisionTextDualEncoderForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = VisionTextDualEncoderForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = VisionTextDualEncoderForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class VisionTextDualEncoderModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (VisionTextDualEncoderForCausalLM,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = VisionTextDualEncoderModelTester(self)
...
@require_torch
class VisionTextDualEncoderModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_masked_lm(self):
        model = VisionTextDualEncoderForMaskedLM.from_pretrained("brand-new-bert-base-cased")
