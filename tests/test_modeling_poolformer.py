...
class PoolFormerModelTester:
    ...
    def create_and_check_for_causal_lm(
        ...
        model = PoolFormerForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_masked_lm(
        ...
        model = PoolFormerForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
    ...
    def create_and_check_for_question_answering(
        ...
        model = PoolFormerForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = PoolFormerForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = PoolFormerForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = PoolFormerForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
class PoolFormerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (PoolFormerForCausalLM,) if is_torch_available() else ()

    def setUp(self):
        self.model_tester = PoolFormerModelTester(self)
        self.config_tester = ConfigTester(self, config_class=PoolFormerConfig, hidden_size=37)
...
class PoolFormerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        model = PoolFormerForMaskedLM.from_pretrained("poolformer_s12")
