...
""" Testing suite for the PyTorch Wav2Vec2Conformer model. """
...
class Wav2Vec2ConformerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_mask_feature_prob_ctc(self):
        model = Wav2Vec2ConformerForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-wav2vec2_conformer", mask_feature_prob=0.2, mask_feature_length=2
        )
        model.to(torch_device).train()
        processor = Wav2Vec2Processor.from_pretrained(
            "hf-internal-testing/tiny-random-wav2vec2_conformer", return_attention_mask=True
        )
    ...
    def test_mask_time_prob_ctc(self):
        model = Wav2Vec2ConformerForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-wav2vec2_conformer", mask_time_prob=0.2, mask_time_length=2
        )
        model.to(torch_device).train()
        processor = Wav2Vec2Processor.from_pretrained(
            "hf-internal-testing/tiny-random-wav2vec2_conformer", return_attention_mask=True
    ...
    def test_model_from_pretrained(self):
        model = Wav2Vec2ConformerModel.from_pretrained("facebook/wav2vec2-conformer-large-rel-pos")
        self.assertIsNotNone(model)


@require_torch
class Wav2Vec2ConformerRobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            Wav2Vec2ConformerForCTC,
            Wav2Vec2ConformerModel,
    ...
    def setUp(self):
        self.model_tester = Wav2Vec2ConformerModelTester(
    ...
    def test_model_for_pretraining(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        model = Wav2Vec2ConformerForPreTraining(config).to(torch_device)
    ...
    def test_mask_time_feature_prob_ctc_single_batch(self):
        model = Wav2Vec2ConformerForCTC.from_pretrained(
            "hf-internal-testing/tiny-random-wav2vec2_conformer",
            mask_time_prob=0.2,
...
class Wav2Vec2ConformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_integration(self):
        ...
        model.to(torch_device)
        feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/wav2vec2_conformer-base")
        input_speech = self._load_datasamples(2)
    ...
    def test_loss_pretraining(self):
        model = Wav2Vec2ConformerForPreTraining.from_pretrained(
            "facebook/wav2vec2_conformer-base",
    ...
    @require_pyctcdecode
    @require_torchaudio
    def test_wav2vec2_conformer_with_lm(self):
