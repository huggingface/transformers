...
def test_transformer_flash_attention(seq_len=2)->None:
    ...
    sp_size = 2
    dp_size = 1
    ds_engine, _, _, _ = initialize(model=model, config_params={"train_batch_size": batch_size,
                    "data_parallel_size": dp_size, "sequence_parallel_size": sp_size},)

    assert is_deepspeed_sp_enabled() == True
    ...
    input_tensor = torch.randn(batch_size, seq_len, num_heads, hidden_dim, device=ds_engine.device)
    ...
    assert output_tensor.shape == (batch_size, seq_len, num_heads, hidden_dim)
    ...
    ds_engine, _, _, _ = initialize(model=model, config_params={"train_batch_size": batch_size,
                    "data_parallel_size": dp_size, "sequence_parallel_size": sp_size},)
    assert is_deepspeed_sp_enabled() == False
    ...
    assert output_tensor_no_sp.shape == (batch_size, seq_len, num_heads, hidden_dim)
