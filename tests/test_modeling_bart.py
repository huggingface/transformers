...
class BartModelIntegrationTests(unittest.TestCase):
    ...
    @staticmethod
    def _load_generation_model(checkpoint_name):
        model = BartForConditionalGeneration.from_pretrained(checkpoint_name).to(torch_device)
        if torch_device == 'cuda':
            model = model.half()
        return model
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = self._load_generation_model("facebook/bart-large-xsum").to(torch_device)
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf =self._load_generation_model("facebook/bart-large-cnn")
