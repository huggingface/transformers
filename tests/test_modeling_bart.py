...
class BARTModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_advanced_inputs(self):
        ...
        model = BartModel(config).to(default_device).eval()
...
class BartTranslationTests(unittest.TestCase):
    ...
    def test_enro_generate(self):
        ...
        translated_tokens = model.generate(input_ids=inputs["input_ids"].to(default_device), num_beams=5, )
        decoded = [
    ...
    def test_mbart_fast_forward(self):
        ...
        lm_model = BartForConditionalGeneration(config).to(default_device)
        context = torch.Tensor([[71, 82, 18, 33, 46, 91, 2], [68, 34, 26, 58, 30, 2, 1]]).long().to(default_device)
        summary = torch.Tensor([[82, 71, 82, 18, 2], [58, 68, 2, 1, 1]]).long().to(default_device)
...
class BartHeadTests(unittest.TestCase):
    ...
    def test_lm_forward(self):
        config, input_ids, batch_size = self._get_config_and_data()
        lm_labels = ids_tensor([batch_size, input_ids.shape[1]], self.vocab_size).to(default_device)
    ...
    def test_generate_beam_search(self):
        input_ids = torch.Tensor([[71, 82, 2], [68, 34, 2]]).long().to(default_device)
        config = BartConfig(
            vocab_size=self.vocab_size,
            d_model=24,
            encoder_layers=2,
    ...
    def test_generate_fp16(self):
        ...
        model = BartForConditionalGeneration(config).eval().to(default_device).half()
    ...
    def test_base_model_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data()
        attention_mask = input_ids.ne(1).to(default_device)
        lm_model = BartForConditionalGeneration(config).eval().to(default_device).half()
        lm_model(input_ids, attention_mask=attention_mask)

    def test_default_generate_kwargs(self):
        config, input_ids, _ = self._get_config_and_data()
        model = BartForConditionalGeneration(config).eval().to(default_device)
...
def _long_tensor(tok_lst):
    return torch.tensor(tok_lst, dtype=torch.long, device=default_device, )
...
class BartModelIntegrationTests(unittest.TestCase):
    ...
    def test_mnli_inference(self):
        ...
        expected_slice = torch.Tensor([[0.1907, 1.4342, -1.0289]]).to(default_device)
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = BartForConditionalGeneration.from_pretrained("bart-large-xsum").to(default_device)
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = BartForConditionalGeneration.from_pretrained("bart-large-cnn").to(default_device)
        tok = BartTokenizer.from_pretrained("bart-large")
