# coding=utf-8
# Copyright 2018 Bertweet Authors.
#
...
class BertweetModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    class BertweetModelTester(object):
        def __init__(
            self,
            parent,
            batch_size=13,
        ...
        def create_and_check_bertweet_model(
            ...
            model = BertweetModel(config=config)
        ...
        def create_and_check_bertweet_for_masked_lm(
            ...
            model = BertweetForMaskedLM(config=config)
            model.to(torch_device)
            model.eval()
        ...
        def create_and_check_bertweet_for_question_answering(
            ...
            model = BertweetForQuestionAnswering(config=config)
            model.to(torch_device)
            model.eval()
        ...
        def create_and_check_bertweet_for_sequence_classification(
            ...
            model = BertweetForSequenceClassification(config)
            model.to(torch_device)
            model.eval()
            loss, logits = model(
        ...
        def create_and_check_bertweet_for_token_classification(
            ...
            config.num_labels = self.num_labels
            model = BertweetForTokenClassification(config=config)
            model.to(torch_device)
            model.eval()
            loss, logits = model(
        ...
        def create_and_check_bertweet_for_multiple_choice(
            self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
        ):
            config.num_choices = self.num_choices
            model = BertweetForMultipleChoice(config=config)
            model.to(torch_device)
            model.eval()
    ...
    def setUp(self):
        self.model_tester = BertweetModelTest.BertweetModelTester(self)
        self.config_tester = ConfigTester(self, config_class=BertweetConfig, hidden_size=37)
    ...
    def test_for_multiple_choice(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_bertweet_for_multiple_choice(*config_and_inputs)

    @slow
    def test_model_from_pretrained(self):
        for model_name in BERTWEET_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = BertweetModel.from_pretrained(model_name)
