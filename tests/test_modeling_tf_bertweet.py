...
from transformers import BertweetConfig, is_tf_available
...
if is_tf_available():
    ...
    from transformers.modeling_tf_bertweet import (
...
@require_tf
class TFBertweetModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    class TFBertweetModelTester(object):
        def __init__(
            self,
            parent,
            batch_size=13,
        ...
        def create_and_check_bertweet_for_sequence_classification(
            ...
            model = TFBertweetForSequenceClassification(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
            (logits,) = model(inputs)
            result = {
                "logits": logits.numpy(),
        ...
        def create_and_check_bertweet_for_token_classification(
            ...
            config.num_labels = self.num_labels
            model = TFBertweetForTokenClassification(config=config)
            inputs = {"input_ids": input_ids, "attention_mask": input_mask, "token_type_ids": token_type_ids}
            (logits,) = model(inputs)
            result = {
                "logits": logits.numpy(),
        ...
        def create_and_check_bertweet_for_multiple_choice(
            ...
            model = TFBertweetForMultipleChoice(config=config)
    ...
    def setUp(self):
        self.model_tester = TFBertweetModelTest.TFBertweetModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_name in ["vinai/bertweet-base"]:
            model = TFBertweetModel.from_pretrained(model_name)
