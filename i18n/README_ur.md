...
🤗 ٹرانسفارمرز مختلف طریقوں جیسے کہ متن، بصارت، اور آڈیو پر کام کرنے کے لیے ہزاروں پری ٹرینڈ ماڈلز فراہم کرتے ہیں۔
...
🤗 Transformers کو condaا کے ذریعے اس طرح انسٹال کیا جا سکتا ہے:

```shell script
conda install conda-forge::transformers
```

> **_نوٹ:_** `huggingface` چینل سے `transformers` انسٹال کرنا اب منسوخ ہے۔

Flax، PyTorch یا TensorFlow کی انسٹالیشن صفحات دیکھیں کہ آپ انہیں conda کے ساتھ کیسے انسٹال کر سکتے ہیں۔

> **_نوٹ:_** Windows پر، آپ کو کیشنگ کے فوائد حاصل کرنے کے لیے Developer Mode کو فعال کرنے کے لیے کہا جا سکتا ہے۔ اگر یہ آپ کے لیے ممکن نہیں ہے تو براہ کرم ہمیں [اس مسئلے](https://github.com/huggingface/huggingface_hub/issues/1062) میں آگاہ کریں۔
