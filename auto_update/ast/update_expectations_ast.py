...
class ExpectationsUpdater:
    ...
    def _build_target_keys(self) -> list:
        ...
        Returns:
            List of tuples to try: [(device, precision), (device, None), (None, None)]
        """
        keys = []
        
        # First try: exact match (device, precision)
        if self.precision is not None:
            keys.append((self.device_type, self.precision))
        
        # Second try: device with any precision (device, None)
        keys.append((self.device_type, None))
        
        # Third try: fallback default (None, None)
    ...
    def parse_captured_info(self, captured_file: Path) -> Tuple[str, int, str, Any]:
        ...
        # Pattern: assert_close(actual, expected_variable, ...)
    ...
    def find_expectations_assignment(self, file_path: Path, variable_name: str, assertion_line: int) -> Optional[Tuple[int, str]]:
        """
    ...
    def find_dict_key_line(self, lines: list, start_line: int, target_key: tuple) -> Optional[int]:
        ...
        # Could be: ("cuda", 8) or ('cuda', 8) or (None, None)
        key_patterns = []
        ...
        # Search forward from start_line
