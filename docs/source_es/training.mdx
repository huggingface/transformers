...
Como ya sabes, necesitas un tokenizador para procesar el texto e incluir una estrategia para el padding y el truncamiento, manejando cualquier tamaño de una variable secuencial. Para procesar tu dataset en un solo paso, utiliza el método 🤗 Datasets [`map`](https://huggingface.co/docs/datasets/process.html#map) para aplicar una función de preprocesamiento sobre todo el dataset:
...
🤗 Transformers proporciona una clase [`Trainer`] optimizada para el entrenamiento de modelos 🤗 Transformers, haciendo más fácil el inicio del entrenamiento sin necesidad de escribir manualmente su propio ciclo. La API del [`Trainer`] soporta una amplia gama de opciones de entrenamiento y características como el logging, el gradient accumulation y el mixed precision.

Comienza cargando tu modelo y especifica el número de labels previstas. A partir del [Card Dataset](https://huggingface.co/datasets/yelp_review_full#data-fields) Yelp Review, que como ya sabemos tiene 5 labels:
...
Crea un objeto [`Trainer`] con su modelo, argumentos de entrenamiento, conjuntos de datos de entrenamiento y de prueba, y su función de evaluación:
...
A continuación, aplique fine-tuning a su modelo llamando a [`~transformers.Trainer.train`]:
...
El [`Trainer`] se encarga del ciclo de entrenamiento y permite aplicar fine-tuning a un modelo en una sola línea de código. Para los usuarios que prefieren escribir su propio ciclo de entrenamiento, también puedes aplicar fine-tuning a un modelo 🤗 Transformers en PyTorch nativo.
...
Cargue su modelo con el número de labels previstas:
...
Genial, ¡ahora estás listo para entrenar! 🥳

### Ciclo de entrenamiento

Para hacer un seguimiento del progreso del entrenamiento, utiliza la librería [tqdm](https://tqdm.github.io/) para añadir una barra de progreso sobre el número de pasos de entrenamiento:
...
- [🤗 Transformers Notebooks](notebooks) contiene varios notebooks sobre cómo aplicar fine-tuning un modelo para tareas específicas en PyTorch y TensorFlow.