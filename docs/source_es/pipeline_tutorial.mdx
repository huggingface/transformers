...
Echa un vistazo a la documentación del [`pipeline`] para obtener una lista completa de tareas admitidas.
...
1. Comienza creando un [`pipeline`] y especifica una tarea de inferencia:
...
2. Pasa tu texto de entrada a el [`pipeline`]:
...
Si tienes mas de una entrada, pásala como una lista:
...
El [`pipeline`] acepta cualquier modelo de [Model Hub](https://huggingface.co/models). Hay etiquetas en Model Hub que te permiten filtrar por modelo que te gustaría utilizar para tu tarea. Una vez que hayas elegido un modelo apropiado, cárgalo con la clase `AutoModelFor` y [`AutoTokenizer'] correspondientes. Por ejemplo, carga la clase [`AutoModelForCausalLM`] para una tarea de modelado de lenguaje causal:
...
Crea un [`pipeline`] para tu tarea y especifica el modelo y el tokenizador que has cargado:
...
Pasa tu texto de entrada a [`pipeline`] para generar algún de texto:
...
La flexibilidad del [`pipeline`] significa que también se puede extender a tareas de audio.

Por ejemplo, clasifiquemos la emoción de un breve fragmento del famoso discurso de John F. Kennedy ["We choose to go to the Moon"](https://en.wikipedia.org/wiki/We_choose_to_go_to_the_Moon). Encuentra un modelo de [audio classification](https://huggingface.co/models?pipeline_tag=audio-classification) en Model Hub para el reconocimiento de emociones y cárgualo en el [`pipeline`]:
...
Pasa el archivo de audio a el [`pipeline`]:
...
Especifica tu tarea de visión y pasa tu imagen al clasificador. La imagen puede ser un enlace o una ruta local a la imagen. Por ejemplo, ¿qué especie de gato se muestra a continuación?
