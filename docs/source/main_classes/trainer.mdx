...
You can do the same with the `deepspeed` launcher if you have it installed:

```bash
deepspeed --num_gpus 2 trainer-program.py ...
```
You don't need to use [the Deepspeed integration](deepspeed) to use the `deepspeed` launcher:
