...
which should make the non-stop training as close as possible to "stop and resume" style of training.

However, due to various default non-deterministic pytorch settings this might not fully work. If you want full
determinism please refer to `this document <Controlling sources of randomness>`__. As explained in the document, that
some of those settings that make things determinstic (.e.g., `torch.backends.cudnn.deterministic`) may slow things
down, therefore you can enable those if you need to.
