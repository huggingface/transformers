...
The idea behind gradient accumulation is to instead of calculating the gradients for the whole batch at once to do it in smaller steps. The way we do that is to calculate the gradients iteratively in smaller batches by doing a forward and backward pass through the model and accumulating the gradients in the process. When enough gradients are accumulated we run the model's optimization step. This way we can save GPU memory but in turn we are adding more forward and backward passes which can slow down the training.
...
We can see that this saved some more memory but at the same time training became a bit slower. A general rule of thumb is that gradient checkpointing slows down training by about 20%. Let's have a look at another method with which we can regain some speed and also reduce the memory footprint: mixed precision training.

## FP16 Training

The idea of mixed precision training is that no all variables need to be stored in full (32-bit) floating point precision. If we can reduce the precision the variales and their computations are faster and use less memory. The main advantage comes from saving the activations in half (16-bit) precision. Although the gradients are also computed in half precision they are converted back to full precision for the optimization step so no memory is saved here. For this reason this approach is also called mixed precision training. Enabling mixed precision training is also just a matter of setting the `fp16` flag to `True`:
...
We can see that this is almost twice as fast as the vanilla training while also requiring less GPU memory. Let's add it to the mix of the previous methods:
...
adam_bnb_optim = bnb.optim.Adam8bit(optimizer_grouped_parameters, **optimizer_kwargs)
```
...
So far we have used the `Trainer` to run the experiments but a more flexible alternative to that approach is to use [`accelerate`](https://huggingface.co/docs/accelerate/). With `accelerate` you have full control over the training loop and can essentially write the loop in pure PyTorch with some minor modifications. In turn it allows you to easily scale across different infrastructures such as CPUs, GPUs, TPUs, or distributed multi-GPU setups without chaning any code. Let's see what it takes to implement all of the above tweaks in accelerate. We can still use the `TrainingArguments` to wrap the training settings:
...
This concludes this guide for scaling the training of large models. There is a lot more going on under the hood than we looked at and the ["Performance and Scalability: How To Fit a Bigger Model and Train It Faster" section](https://huggingface.co/docs/transformers/performance) goes into much more detail about the introduced optimization techniques we introduced.