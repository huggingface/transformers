...
يُنشئ التلخيص نسخةً مختصرةً من مستند أو مقال ويلتقط جميع المعلومات المهمة. إلى جانب الترجمة، يُعدّ مثالاً آخر على مهمة يمكن صياغتها كمهمة تسلسل إلى تسلسل. يمكن أن يكون التلخيص:
...
ابدأ بتحميل مجموعة فرعية أصغر من مشاريع قوانين ولاية كاليفورنيا من مجموعة بيانات BillSum من مكتبة 🤗 Datasets:
...
دالة المعالجة المسبقة التي تريد إنشاؤها تحتاج إلى:

1. إضافة بادئة للمُدخل بمُطالبة حتى يعرف T5 أن هذه مهمة تلخيص. تتطلب بعض النماذج القادرة على مهام البرمجة اللغوية العصبية المتعددة مطالبات لمهام مُحددة.
2. استخدام وسيطة الكلمة الرئيسية `text_target` عند ترميز التصنيفات.
3. اقتطاع التسلسلات بحيث لا يزيد طولها عن الحد الأقصى الذي تم تعيينه بواسطة معامل `max_length`.
...
يُعد تضمين مقياس أثناء التدريب مفيدًا غالبًا لتقييم أداء نموذجك. يمكنك تحميل طريقة تقييم بسرعة باستخدام مكتبة 🤗 [Evaluate](https://huggingface.co/docs/evaluate/index). لهذه المهمة، قم بتحميل مقياس [ROUGE](https://huggingface.co/spaces/evaluate-metric/rouge) (انظر [الجولة السريعة](https://huggingface.co/docs/evaluate/a_quick_tour) الخاصة بـ 🤗 Evaluate لمعرفة المزيد حول كيفية تحميل وحساب مقياس):
