...
configurations and a great versatility in use-cases. Below, a few examples are presented. We split them up according to
their modality: language (for tasks such as question answering, sequence classification, named entity recognition), vision
(for tasks such as image classification, object detection), audio (for tasks such as audio classification, keyword spotting)
and tasks that involve multiple modalities (e.g. vision and text).
...
- Direct model use: Less abstractions, but more flexibility and power via a direct access to a tokenizer, feature extractor or processor
...
Image classification is the task of classifying an image according to a given number of classes. An example of a famous
image classification dataset/benchmark is [ImageNet](https://image-net.org/), which consists of 14 million images, each labeled with one of 1,000 possible classes.
If you would like to fine-tune a model on a image classification dataset, you may leverage the [run_image_classification.py](https://github.com/huggingface/transformers/tree/master/examples/pytorch/image-classification) script.

Here is an example of using pipelines to do image classification. It leverages a Vision Transformer (ViT) fine-tuned on ImageNet, hence it can classify an image into one of 1,000 possible classes.

This returns a list of the top 5 labels with corresponding scores. Let's check the label with the highest score, as follows:
...
>>> print(f"label: {result['label']}, with score: {round(result['score'], 4)}")
label: Egyptian cat, with score: 0.9374
```

### Object detection

Object detection is the task of detecting objects in an image. Models typically output a bounding box and a corresponding class label for every detected
object in an image. An example of a famous object detection dataset is [COCO](https://cocodataset.org/#home) (which actually also includes many other tasks such as segmentation and captioning).

Here is an example of using pipelines to do object detection. It leverages a DETR model fine-tuned on COCO.
Note that it does require to have [timm](https://github.com/rwightman/pytorch-image-models) installed, as DETR leverages a convolutional backbone from that package.

Similar to the [image-classification pipeline](#image-classification), it returns the top 5 detected objects in the image by default. 
Let's check the object detected with the highest score, as follows:
...
>>> result = classifier(image)[0]
>>> print(f"detected: {result['label']}, with score: {round(result['score'], 4)}. Bounding box: {result['box']}")
detected: remote, with score: 0.9982. Bounding box: {'xmin': 40, 'ymin': 70, 'xmax': 175, 'ymax': 117}
```
...
We do have several Transformer-based models in the library that combine several modalities at the same time. Examples of these include:

- [TAPAS](./model_doc/tapas), which can be used for table question answering
- [LayoutLM](./model_doc/layoutlm), [LayoutLMv2](./model_doc/layoutlmv2), [LayoutXLM](./model_doc/layoutxlm), which can be used for intelligent document processing tasks
- [TrOCR](./model_doc/trocr), which can be used for optical character recognition
- [Perceiver](./model_doc/perceiver), which can be used for basically any discriminative task you can think of, e.g. video classification.

We refer to the documentation of the models above in case you want to learn more about them. Below, we illustrate how you can leverage the 
[table-question-answering pipeline](./main_classes/pipelines/#TableQuestionAnsweringPipeline) for quick inference on a Pandas dataframe, as follows:
