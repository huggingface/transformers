...
마땅한 모델을 찾을 수 없는 경우 데이터를 기반으로 사전 훈련된 모델을 파인튜닝해야 합니다. 파인튜닝 방법에 대한 자세한 내용은 [파인튜닝 튜토리얼](./training)을 참조하세요. 사전 훈련된 모델을 파인튜닝한 후에는 모델을 Hub의 커뮤니티와 공유하여 머신러닝 민주화에 기여해주세요! 🤗
