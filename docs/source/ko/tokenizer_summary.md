...
이전보다 낫습니다. 하지만, `"Don't"`의 토큰화 결과를 보면 좋지 않습니다.
...
이 토큰화 방법은 텍스트를 더 작은 묶음(chunk)로 분할하는 가장 직관적인 방법이지만, 대규모 텍스트 코퍼스에 대해서는 문제가 발생할 수 있습니다.
...
가능한 모든 기본 문자를 포함하는 기본 어휘의 크기는 굉장히 질 수 있습니다. (예: 모든 유니코드 문자를 기본 문자로 간주하는 경우)
...
transformers에서 제공하는 센텐스피스 토크나이저를 사용하는 모든 모델은 unigram과 함께 사용됩니다. 
