...
- 텍스트에서 클래스를 구분하려면 마침표를 사용하세요. 예: "a cat. a dog."
- 여러 클래스를 사용할 때(예: `"a cat. a dog."`), [`GroundingDinoProcessor`]의 `post_process_grounded_object_detection`을 사용하여 출력을 후처리하세요. `post_process_object_detection`에서 반환되는 라벨은 prob > threshold인 모델 차원의 인덱스를 나타내기 때문입니다.
