...
이 가이드에서는 더 나은 대규모 언어 모델(LLM) 프롬프트를 작성하고 다양한 자연어 처리(NLP) 작업을 해결하는 데 도움이 되는 프롬프트 엔지니어링 모범 사례를 다룹니다. 다음 내용을 배우게 됩니다:

- [프롬프팅의 기초](#basics-of-prompting)
- [대규모 언어 모델(LLM) 프롬프팅의 모범 사례](#best-practices-of-llm-prompting)
...
* [대규모 언어 모델(LLM)을 이용한 생성](../llm_tutorial)
...
현대의 대부분의 대규모 언어 모델(LLM)은 디코더 전용 트랜스포머입니다. 예를 들어 [LLaMA](../model_doc/llama), 
[Llama2](../model_doc/llama2), [Falcon](../model_doc/falcon), [GPT2](../model_doc/gpt2) 등이 있습니다. 그러나 [Flan-T5](../model_doc/flan-t5)와 [BART](../model_doc/bart)와 같은 인코더-디코더 트랜스포머 대규모 언어 모델(LLM)을 접할 수도 있습니다.

인코더-디코더 스타일 모델은 일반적으로 출력이 입력에 크게 의존하는 생성 작업에 사용됩니다. 예를 들어, 번역과 요약 작업에 사용됩니다. 디코더 전용 모델은 다른 모든 유형의 생성 작업에 사용됩니다.

파이프라인을 사용하여 대규모 언어 모델(LLM)으로 텍스트를 생성할 때, 어떤 유형의 대규모 언어 모델(LLM)을 사용하고 있는지 아는 것이 중요합니다. 왜냐하면 이들은 서로 다른 파이프라인을 사용하기 때문입니다.
...
🤗 Hub에서 최근 사용 가능한 대부분의 대규모 언어 모델(LLM) 체크포인트는 두 가지 버전으로 제공됩니다: 기본 버전과 지시(또는 채팅) 버전입니다. 예를 들어, [`tiiuae/falcon-7b`](https://huggingface.co/tiiuae/falcon-7b)와 [`tiiuae/falcon-7b-instruct`](https://huggingface.co/tiiuae/falcon-7b-instruct)가 있습니다.

기본 모델은 초기 프롬프트가 주어졌을 때 텍스트를 완성하는 데 탁월하지만, 지시를 따라야 하거나 대화형 사용이 필요한 자연어 처리(NLP) 작업에는 이상적이지 않습니다. 이때 지시(채팅) 버전이 필요합니다. 이러한 체크포인트는 사전 훈련된 기본 버전을 지시사항과 대화 데이터로 추가 미세 조정한 결과입니다. 이 추가적인 미세 조정으로 인해 많은 자연어 처리(NLP) 작업에 더 적합한 선택이 됩니다.  

[`tiiuae/falcon-7b-instruct`](https://huggingface.co/tiiuae/falcon-7b-instruct)를 사용하여 일반적인 자연어 처리(NLP) 작업을 해결하는 데 사용할 수 있는 몇 가지 간단한 프롬프트를 살펴보겠습니다.

### 자연어 처리(NLP) 작업 [[nlp-tasks]]
...
이제 파이프라인을 통해 모델을 로드했으니, 프롬프트를 사용하여 자연어 처리(NLP) 작업을 해결하는 방법을 살펴보겠습니다.
...
결과적으로, 출력에는 우리가 지시사항에서 제공한 목록에서 선택된 분류 레이블이 포함되어 있으며, 이는 정확한 것입니다!
<Tip>
...
개체명 인식(Named Entity Recognition, NER)은 텍스트에서 인물, 장소, 조직과 같은 명명된 개체를 찾는 작업입니다. 프롬프트의 지시사항을 수정하여 대규모 언어 모델(LLM)이 이 작업을 수행하도록 해보겠습니다. 여기서는 `return_full_text = False`로 설정하여 출력에 프롬프트가 포함되지 않도록 하겠습니다:

```python
>>> torch.manual_seed(1) # doctest: +결과_무시
...
대규모 언어 모델(LLM)이 수행할 수 있는 또 다른 작업은 번역입니다. 이 작업을 위해 인코더-디코더 모델을 사용할 수 있지만, 여기서는 예시의 단순성을 위해 꽤 좋은 성능을 보이는 Falcon-7b-instruct를 계속 사용하겠습니다. 다시 한 번, 모델에게 영어에서 이탈리아어로 텍스트를 번역하도록 지시하는 기본적인 프롬프트를 작성하는 방법은 다음과 같습니다:

```python
>>> torch.manual_seed(2) # doctest: +결과_무시
...
```python
>>> torch.manual_seed(3) # doctest: +결과_무시
...
질의 응답 작업을 위해 프롬프트를 다음과 같은 논리적 구성요소로 구조화할 수 있습니다: 지시사항, 맥락, 질문, 그리고 모델이 답변 생성을 시작하도록 유도하는 선도 단어나 구문(`"답변:"`):

```python
>>> torch.manual_seed(4) # doctest: +결과_무시
...
```python
>>> torch.manual_seed(5) # doctest: +결과_무시
...
## 대규모 언어 모델(LLM) 프롬프트 작성의 모범 사례 [[best-practices-of-llm-prompting]]
...
* 프롬프트의 시작 부분이나 맨 끝에 지시사항을 배치하세요. 대규모 컨텍스트를 다룰 때, 모델들은 어텐션 복잡도가 이차적으로 증가하는 것을 방지하기 위해 다양한 최적화를 적용합니다. 이로 인해 모델이 프롬프트의 중간보다 시작이나 끝 부분에 더 주의를 기울일 수 있습니다.
...
* [퓨샷(Few-shot) 프롬프팅](#few-shot-prompting) 및 [생각의 사슬(Chain-of-thought, CoT)](#chain-of-thought) 같은 고급 기술을 사용하십시오.
* 프롬프트의 견고성을 평가하기 위해 다른 모델로 테스트하십시오.
