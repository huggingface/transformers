...
이는 에이전트가 완료해야 하는 최종 미완성 예제입니다. 미완성 예제는 실제 사용자 입력에 따라 동적으로 만들어집니다. 
...
`.chat`을 실행하면 사용자의 입력 또는 *task*가 미완성된 양식의 예시로 캐스팅됩니다:
```text
Human: <user-input>\n\nAssistant:
```
...
기본 도구에는 매우 정확한 이름과 설명이 있어야 하지만 특정 사용 사례에 맞게 툴의 설명이나 이름을 변경하는 것이 도움이 될 수도 있습니다. 
