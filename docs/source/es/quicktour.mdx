...
```py
>>> resultados = clasificador(["Estamos muy felices de mostrarte la biblioteca de 🤗 Transformers.", "Esperamos que odies lo mucho que te gusta 😈."])
>>> for resultado in resultados:
...     print(f"label: {resultado['label']}, con score: {round(resultado['score'], 4)}")
label: POS, con score: 0.9917
label: NEG, con score: 0.9981
```
...
>>> dataset = datasets.load_dataset("PolyAI/minds14", name="es-ES", split="train")  # doctest: +IGNORE_RESULT
```

Puedes pasar el dataset entero al pipeline:

```py
>>> archivos = dataset["path"]
>>> reconocedor_de_voz(archivos[:4])
[{'text': 'ahora buenas e a ver  tengo un problema como vuestra aplicación resulta que que quiero hacer una transferencia bancaria a una cuenta conocida pero me da error la aplicación a ver que a ver que puede ser'},
 {'text': 'la aplicación no cargue salda de mi nueva cuenta'},
 {'text': 'hola tengo un problema con la aplicación no carga y y tampoco veo que carga el saldo de mi cuenta nueva dice que la aplicación está siendo reparada y ahora no puedo aceder a mi cuenta no necesito inmediatamente'},
 {'text': 'ora buena la aplicación no se carga la viladad no carga el saldo de mi cuenta nueva dice que la villadenta siendo reparada y oro no puede hacer a mi cuenta'}]
```
