...
La biblioteca 🤗 Transformers está diseñada para ser fácilmente ampliable. Cada modelo está completamente codificado en una 
subcarpeta determinada del repositorio sin abstracción, por lo que puedes copiar fácilmente un archivo del modelo y ajustarlo 
...
Ilustraremos todo esto en un modelo ResNet, envolviendo la clase ResNet del [biblioteca timm](https://github.com/rwightman/pytorch-image-models/tree/master/timm) en un [`PreTrainedModel`].
...
restricciones provienen del hecho de que una `PretrainedConfig` tiene más campos que los que está configurando. Al recargar una 
`config` con el método `from_pretrained`, esos campos deben ser aceptados por su `config` y luego enviados a la superclase.

Definir un `model_type` para tu configuración (en este caso `model_type="resnet"`) no es obligatorio, a menos que quieras
registrar tu modelo con las clases automáticas (ver última sección).
...
directamente tu configuración en Hub.
...
Puedes hacer que ty modelo devuelva lo que quieras, pero devolver un diccionario como lo hicimos para 
`ResnetModelForImageClassification`, con la pérdida incluida cuando se pasan las etiquetas, hará que tu modelo se pueda 
...
Ahora que tenemos nuestra clase modelo, vamos a crear una:
...
tutorial, usaremos la versión preentrenada de resnet50d. Dado que nuestro modelo es solo un envoltorio a su alrededor, 
será fácil transferir esos pesos:
...
Primero, asegúrate de que tu modelo esté completamente definido en un archivo `.py`. Puedes basarse en importaciones 
...
del archivo para importar desde el paquete `transformers`.

</Tip>

Ten en cuenta que puedes reutilizar (o subclasificar) una configuración/modelo existente.
...
Luego puedes ingresar a tu propio espacio (o una organización de la que sea miembro) de esta manera:
...
malware (consulte la documentación de [seguridad del Hub](https://huggingface.co/docs/hub/security#malware-scanning) para 
...
También se recomienda encarecidamente pasar un _hash_ de confirmación como una "revisión" para asegurarse de que el autor 
...
tu biblioteca para obtener los modelos personalizados (al contrario de descargar automáticamente el código del modelo del Hub).
...
en cualquier clase del modelo automático debe coincidir con `config_class ` de esos modelos.
