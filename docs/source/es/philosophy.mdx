...
- Que sea tan fácil y rápida de utilizar como sea posible:
  ...
  - Por encima de esas tres clases estandar, la biblioteca proporciona dos APIs: [`pipeline`] para usar rápidamente un modelo (junto a su configuracion y tokenizer asociados)
  ...
  - Como consecuencia, esta biblioteca NO es una caja de herramientas modular de bloques individuales para redes neuronales. Si quieres extender/construir sobre la biblioteca,
    usa simplemente los módulos regulares de Python/PyTorch/TensorFlow/Keras y emplea las clases estandar de la biblioteca como punto de partida para reutilizar funcionalidades
...
- `from_pretrained()` permite generar un modelo/configuración/tokenizer a partir de una versión preentrenada proporcionada ya sea por
  la propia biblioteca (los modelos soportados se pueden encontrar en [Model Hub](https://huggingface.co/models)) o
