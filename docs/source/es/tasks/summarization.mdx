...
La sumarización crea una versión más corta de un documento o un artículo que resume toda su información importante. Junto con la traducción, es un ejemplo de una tarea que puede ser formulada como una tarea secuencia a secuencia. La sumarización puede ser:
...
Esta guía te mostrará cómo puedes hacer fine-tune del modelo [T5](https://huggingface.co/t5-small) sobre el subset de proyectos de ley del estado de California, dentro del dataset [BillSum](https://huggingface.co/datasets/billsum) para hacer sumarización abstractiva.

<Tip>

Consulta la [página de la tarea](https://huggingface.co/tasks/summarization) de sumarización para obtener más información sobre sus modelos, datasets y métricas asociadas.

</Tip>

## Carga el dataset BillSum

Carga el dataset BillSum de la biblioteca 🤗 de datasets:
...
1. Agregar un prefijo al input, una clave para que T5 sepa que se trata de una tarea de sumarización. Algunos modelos capaces de múltiples tareas de NLP requieren una clave que indique la tarea específica.
...
Si no estás familiarizado con el proceso de realizar fine-tuning sobre un modelo con [`Trainer`], ¡mira el tutorial básico [aquí](../training#finetune-with-trainer)!
...
2. Pasarle los argumentos de entrenamiento a [`Seq2SeqTrainer`] junto con el modelo, dataset y el data collator.
3. Realizar la llamada [`~Trainer.train`] para realizar el fine-tuning sobre tu modelo.
...
<tf>
Para realizar el fine-tuning de un modelo en TensorFlow, comienza por convertir tus datasets al formato `tf.data.Dataset` con [`~datasets.Dataset.to_tf_dataset`]. Especifica los inputs y etiquetas en `columns`, si es necesario mezclar el dataset, el tamaño de lote y el data collator:
...
Si no estás familiarizado con realizar fine-tuning de modelos con Keras, ¡mira el tutorial básico [aquí](training#finetune-with-keras)!
...
Para un ejemplo con mayor profundidad de como hacer fine-tune de un modelo para sumarización, échale una mirada a la
