...
a la librería 🤗 Transformers.

En primer lugar, debes decidir las entradas crudas que tu pipeline podrá recibir. Pueden ser cadenas, bytes crudos,
...
(por ejemplo, los archivos de audio pueden ser nombres de archivo, URLs o bytes puros).
...
Puedes especificar un modelo por defecto si lo deseas, en cuyo caso debe venir con una versión específica (que puede ser el nombre de una rama o hash de commit, en este caso usamos `"abcdef"`), así como el tipo:
...
Si quieres contribuir tu pipeline a la librería 🤗 Transformers, tendrás que añadirlo a un nuevo módulo en el
...
salida tenga el mismo tipo que el tipo de salida del pipeline.
