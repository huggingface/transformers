...
Una vez que hayas configurado la versión correcta de la librería, ve a la carpeta de ejemplo de tu elección e instala los requisitos específicos del ejemplo:
...
<frameworkcontent>
<pt>
El script de ejemplo descarga y preprocesa un conjunto de datos de la librería 🤗 [Datasets](https://huggingface.co/docs/datasets/). Luego, el script ajusta un conjunto de datos con [Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) en una arquitectura que soporta la tarea de resumen. El siguiente ejemplo muestra cómo ajustar [T5-small](https://huggingface.co/t5-small) en el conjunto de datos [CNN/DailyMail](https://huggingface.co/datasets/cnn_dailymail). El modelo T5 requiere un argumento adicional `source_prefix` debido a cómo fue entrenado. Este aviso le permite a T5 saber que se trata de una tarea de resumen.
...
## Entrenamiento distribuido y precisión mixta

[Trainer](https://huggingface.co/docs/transformers/main_classes/trainer) admite un entrenamiento distribuido y precisión mixta, lo que significa que también puedes usarlo en un script. Para habilitar ambas características:

- Agregue el argumento `fp16` para habilitar la precisión mixta.
...
<frameworkcontent>
<pt>
Las Unidades de Procesamiento de Tensor (TPUs) están diseñadas específicamente para acelerar el rendimiento. PyTorch admite TPU con el compilador de aprendizaje profundo [XLA](https://www.tensorflow.org/xla) (consulta [aquí](https://github.com/pytorch/xla/blob/master/README.md) para obtener más detalles). Para usar una TPU, inicie el script `xla_spawn.py` y use el argumento `num_cores` para establecer la cantidad de núcleos de TPU que deseas usar.
...
<tf>
Las Unidades de Procesamiento de Tensor (TPUs) están diseñadas específicamente para acelerar el rendimiento. TensorFlow utiliza una [`TPUStrategy`](https://www.tensorflow.org/guide/distributed_training#tpustrategy) para entrenar en TPUs. Para usar una TPU, pase el nombre del recurso de la TPU al argumento `tpu`
...
El script de resumen admite conjuntos de datos personalizados siempre que sean un archivo CSV o JSON Line. Cuando uses tu propio conjunto de datos, necesitas especificar varios argumentos adicionales:
...
Un script de resumen que utilice un conjunto de datos personalizado se vería así:
...
A veces, es una buena idea ejecutar tu secuencia de comandos en una cantidad menor de ejemplos para asegurarse de que todo funcione como se espera antes de comprometerse con un conjunto de datos completo, lo que puede demorar horas en completarse. Utiliza los siguientes argumentos para truncar el conjunto de datos a un número máximo de muestras:
...
No todos los scripts de ejemplo admiten el argumento `max_predict_samples`. Puede que desconozcas si tu secuencia de comandos admite este argumento, agrega el argumento `-h` para verificar:
...
Todos los scripts pueden cargar tu modelo final en [Model Hub](https://huggingface.co/models). Asegúrate de haber iniciado sesión en Hugging Face antes de comenzar:
...
Para darle a tu repositorio un nombre específico, usa el argumento `push_to_hub_model_id` para añadirlo. El repositorio se incluirá automáticamente en tu _namespace_.
