...
[PyTorch](https://pytorch.org/), [TensorFlow](https://www.tensorflow.org/), [JAX](https://jax.readthedocs.io/en/latest/) এর জন্য আধুনিক মেশিন লার্নিং

🤗 Transformers আপনাকে সহজে প্রি-ট্রেইন্ড আধুনিক মডেলগুলি ডাউনলোড ও প্রশিক্ষণ করার API এবং টুল প্রদান করে। প্রি-ট্রেইন্ড মডেল ব্যবহারে কম কম্পিউটিং খরচ ও কম কার্বন নিঃসরণ হয়, এবং শুরু থেকে মডেল ট্রেন করার সময় ও রিসোর্স বাঁচে। আমাদের মডেলগুলি নানা ক্ষেত্রের কাজ সমর্থন করে।

📝 **প্রাকৃতিক ভাষা প্রক্রিয়াকরণ**: টেক্সট ক্লাসিফিকেশন, নাম্বার সনাক্তকরণ, প্রশ্নোত্তর, ভাষা মডেলিং, সারাংশ, অনুবাদ, মাল্টিপল চয়েস QA, টেক্সট জেনারেশন  
🖼️ **কম্পিউটার ভিশন**: ছবি শ্রেণিবিন্যাস, অবজেক্ট ডিটেকশন, সেগমেন্টেশন  
🗣️ **অডিও**: স্বয়ংক্রিয় স্পিচ রিকগনিশন, অডিও শ্রেণিবিন্যাস  
🐙 **মাল্টিমোডাল**: টেবিল QA, OCR, স্ক্যান করা ডকুমেন্ট থেকে তথ্য বের করা, ভিডিও ক্লাসিফিকেশন, ভিজ্যুয়াল প্রশ্নোত্তর

🤗 Transformers PyTorch, TensorFlow এবং JAX-এর মধ্যে ইন্টারঅপারেবিলিটি সাপোর্ট করে। আপনি সহজেই মডেলের বিভিন্ন ধাপে ভিন্ন ফ্রেমওয়ার্ক ব্যবহার করতে পারেন। যেমন, কোডের মাত্র ৩টি লাইন দিয়ে মডেল প্রশিক্ষণ করে তারপর অন্য ফ্রেমওয়ার্কে ইনফারেন্স করতে পারবেন। প্রোডাকশন-এ ব্যবহারের জন্য মডেল ONNX বা TorchScript ফরম্যাটেও রপ্তানি করা যায়।

কমিউনিটিতে যোগ দিতে চাইলে [Hub](https://huggingface.co/models), [Forum](https://discuss.huggingface.co/), [Discord](https://discord.com/invite/JfAtkvEtRb) পরিদর্শন করুন!
...
আমাদের ডকুমেন্টেশন প্রধানত ৫টি ভাগে বিভক্ত:

- **শুরু করুন**-এ লাইব্রেরি সম্পর্কে সংক্ষেপে জানতে এবং ইনস্টলেশন পদ্ধতি পাবেন।
- **টিউটোরিয়াল**-এ লাইব্রেরি ব্যবহার শেখার জন্য বিস্তারিত ও সহজ গাইড পাবেন।
- **হাউ-টু গাইড**-এ ভাষা মডেলিংয়ের জন্য প্রি-ট্রেইন্ড মডেল ফাইন-টিউনিং, নিজস্ব মডেল তৈরি ও শেয়ার ইত্যাদির নির্দিষ্ট গাইডলাইন পাবেন।
- **কনসেপ্ট গাইড**-এ 🤗 Transformers ডিজাইন অতীতের দর্শন, বিভিন্ন মডেল বা টাস্ক পেছনের ধারণা ও ব্যাখ্যা পাবেন।
- **API**-তে সকল ক্লাস ও ফাংশন সম্পর্কে ব্যাখ্যা আছে।

  - **মূল ক্লাস**-এ configuration, model, tokenizer, pipeline প্রভৃতি গুরুত্বপূর্ণ ক্লাসের বিস্তারিত বর্ণনা।
  - **মডেল**-এ প্রতিটি অন্তর্ভুক্ত মডেলের সঙ্গে সংশ্লিষ্ট ক্লাস/ফাংশন বিস্তারিত।
  - **ইন্টার্নাল ইউটিলিটি**-তে গভীরভাবে ব্যবহৃত ইউটিলিটি ক্লাস ও ফাংশনের ব্যাখ্যা।
...
নীচের টেবিলটি লাইব্রেরিতে অন্তর্ভুক্ত প্রতিটি মডেলের সমর্থন-সংক্রান্ত অবস্থা দেখায়। টোকেনাইজেশন Python (যার আরেক নাম "slow") অথবা 🤗 Tokenizers (যার আরেক নাম "fast") দিয়ে করা হচ্ছে কি না; এবং (Flax-এর মাধ্যমে) Jax, PyTorch, TensorFlow ফ্রেমওয়্যারগুলোর মধ্যে কোনগুলো সমর্থন করে, তা এখানে দেখানো হয়েছে।
