...
Before you can fine-tune a pretrained model, download a dataset and prepare it for training. The previous tutorial showed you how to process data for training, and now is an opportunity to put those skills to the test!
...
First, you need a [`DataCollator`](main_classes/data_collator) to assemble your tensors into a batch. You can use [`DataCollatorWithPadding`] to also apply a padding strategy. This will pad each input to the longest length in each batch (also known as _dynamic padding_), which will speed up your training. Make sure you specify `return_tensors` to return TensorFlow tensors:
...
Next, manually post-process `tokenized_dataset` to prepare it for training.
