...
🤗 Transformers status: as of this writing none of the models supports full-PP. GPT2 and T5 models have naive inter-layer model parallelism support. The main obstacle is being unable to convert the models to `nn.Sequential` and have all the inputs to be Tensors. This is because currently the models include many features that make the conversion very complicated, and will need to be removed to accomplish that.
...
- [OSLO](https://github.com/tunib-ai/oslo) is under discussion about integration with the Transformers.
...
🤗 Transformers status:
- [OSLO](https://github.com/tunib-ai/oslo) is under discussion about integration with Transformers.
