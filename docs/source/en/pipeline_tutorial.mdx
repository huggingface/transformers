...
Pipelines alleviate some complexity of batching, because for some pipelines, 
a single item (like a long audio file) need to be chunked into multiple parts
for processing by a model. Pipeline [abstract that away for you](./main_classes/pipelines#pipeline-chunk-batching)
