...
On Blackwell (SM100+), the DeepGEMM experts kernels require UE8M0 scales. A checkpoint with plain `float32` scales (`scale_fmt="float"`) raises a `ValueError`. Use a `scale_fmt="ue8m0"` checkpoint, or run the experts on `grouped_mm` or `batched_mm`, which read `float32` scales directly. Hopper (SM90+) consumes `float32` scales on the DeepGEMM path without conversion. See the [Experts backends](../experts_interface) guide for the experts backend options.
