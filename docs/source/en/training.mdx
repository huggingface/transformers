...
Keras understands. There are two approaches you can take here - if your dataset is small, you can just convert the whole
thing to Numpy arrays and pass everything to Keras.
...
Next we load our tokenizer and tokenize the data as Numpy arrays:
...
Finally, we load, compile and fit our model:
...
Note that we don't pass a loss argument to our models when we `compile()` them! Hugging Face models will automatically
...
This approach is great when it works, but for larger datasets you might find it starting to become a problem. Why?
...
If you want to avoid this, you can load your data as a `tf.data.Dataset` instead. Although you can write your own
`tf.data` pipeline if you want, we have two convenience methods for doing this - [`~datasets.Dataset.to_tf_dataset`]
and [`TFPreTrainedModel.prepare_tf_dataset`]. We recommend `prepare_tf_dataset()` in most cases because it is simpler
and can automatically infer which columns from the dataset to load by comparing them against the input names for the
model. The `to_tf_dataset()` method is more low-level, and is useful when you want to exactly control how your
dataset is created.

Before you can use this method, you will need to add the tokenizer outputs to your dataset as columns, as in
...
columns have been added, we can now stream batches from the dataset and add padding on the fly, which greatly
...
The reason we pass the tokenizer to `prepare_tf_dataset` is so that it can correctly pad batches as they're loaded.
...
Once we've created a `tf.data.Dataset` we can simply compile and fit our model as before:
