...
- **Patches** (`_*_PATCHES`, installed via `patch_attrs`) reversibly swap an attribute
  (a `torch` op, an ExecuTorch internal, an instance method) for the duration of the
  export. Originals are restored on exit, even if the body raises.
- **Fixes** (`_*_FIXES`, applied via `apply_fx_node_fixes` / `apply_onnx_ir_fixes` /
  `fix_fx_graph`) mutate the in-progress graph or program in place. There's no revert —
  they're meant to permanently repair the artifact before the next pipeline step.

Every patch / fix sits in a module-level registry. Adding a new one is *write a function
...
| ----- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **1** | **Torch patches** (`_TORCH_PATCHES`, `_ONNX_PATCHES`)       | `# ── Stage 1: Torch patches ──`                                                                                                 | During `torch.export` / `torch.onnx.export`      | Reversible (`patch_attrs`)           | Monkey-patches PyTorch ops (`where`, `unsqueeze`, `scaled_dot_product_attention`, `searchsorted`, …) plus the private ONNX `_prepare_exported_program_for_export` hook so the FX fixes run immediately after `run_decompositions`. Each `_patch_*` closes over the original. | Define `_patch_*(original)` and append `(target, attr, factory)` to the appropriate list.    |
| **2** | **FX graph fixes** (`_FX_NODE_FIXES`)                       | `# ── Stage 2: FX graph fixes ──`                                                                                                | After `torch.export`, before `torch.onnx.export` | In-place (`apply_fx_node_fixes`)     | Per-node rewrites on the `GraphModule` to drop or replace nodes the ONNX decomposer can't lower (alias ops, in-place views, `_assert_*`, dead comparisons, in-place `triu_`, `fill_diagonal_`, `sort(stable=True)`). DCE runs automatically at the end of the walk.         | Define `_fix_*(gm, node) → bool` (return `True` to consume) and append to `_FX_NODE_FIXES`.  |
...
| **4** | **ONNX IR fixes** (`_IR_FIXES` / `apply_onnx_ir_fixes`)     | `# ── Stage 4: ONNX IR fixes ──`                                                                                                 | After `torch.onnx.export` returns                | In-place (`apply_onnx_ir_fixes`)     | Post-export rewrites on the `ONNXProgram` IR to work around ORT validation/runtime bugs (e.g. forcing `TopK(sorted=True)`). Applied to both the top-level graph and every function.                                                                                         | Implement `_fix_ir_*(graph_like)` and append to `_IR_FIXES`.                                 |
...
| **2** | **Torch op patches** (`_TORCH_PATCHES`)                            | `# ── Stage 2: Torch op patches ──`                                                                                                          | During `torch.export` tracing                           | Reversible (`patch_attrs`)           | Replaces ops the ExecuTorch backends can't accept (`split_copy`, `chunk`, `topk(k>dim)`, non-divisible `avg_pool2d`, `dropout`, in-place `view`, GQA-shaped SDPA) with decomposed equivalents. Same factory pattern as ONNX stage 1.                                                                      | Define `_patch_*(original)` and append `(target, attr, factory)` to `_TORCH_PATCHES`.                                        |
| **3** | **FX graph fixes** (`_FX_PROGRAM_FIXES`, `_FX_NODE_FIXES`)         | `# ── Stage 3: FX graph fixes ──`                                                                                                            | After `torch.export`, before `to_edge_transform_and_lower` | In-place (`fix_fx_graph`)         | Two ordered lists. `_FX_PROGRAM_FIXES` are `(exported_program) → None` callables that need program-level context (range_constraints, graph_signature, state_dict): widen `int_oo` upper bounds, fill missing placeholder `meta["val"]`. `_FX_NODE_FIXES` are per-node fixers driven by `apply_fx_node_fixes`: swap Python sym ops, rewrite `pow` as `mul` chain, normalize amax/max negative dim, force contiguous clone. | Program-level: append `(exported_program) → None` to `_FX_PROGRAM_FIXES`. Per-node: append `(gm, node) → bool` to `_FX_NODE_FIXES`. |
| **4** | **Upstream-pass softenings** (`_EXECUTORCH_PATCHES`)               | `# ── Stage 4: Upstream-pass softenings ──`                                                                                                  | During `to_edge_transform_and_lower` / `to_executorch`  | Reversible (`patch_attrs`)           | Reversibly swaps ExecuTorch internals that crash on legitimate dynamic-shape patterns: `SpecPropPass.update_placeholder_tensor_specs`, `PruneEmptyTensorsPass.remove_empty_tensors_from_cat`, `eval_upper_bound`, `dim_order_from_stride` (rebound on every importer), XNNPACK squeeze/unsqueeze define-node, complex-dtype validator, edge-dialect sym-op allowlist. | Define `_patch_*(original)` and append `(target, attr, factory)` to `_EXECUTORCH_PATCHES`.                                   |
