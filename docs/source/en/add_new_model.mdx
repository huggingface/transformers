...
🤗 Transformers is a very opinionated library with specific design choices. From our experience, we've
found that these fundamental design choices and philosophies are crucial to efficiently scale 🤗
...
- Don't reinvent the wheel! Most of the code you will add for the new model already exist
  somewhere in 🤗 Transformers. Take some time to find similar existing models and tokenizers you can copy
...
- Ask for help when you're stuck! Models are the core components of 🤗 Transformers so we are more
...
1. Fork the 🤗 Transformers [repository](https://github.com/huggingface/transformers) by clicking on the **Fork** button on the
...
Now you have set up a development environment to port `BrandNewBert` to 🤗 Transformers!
...
models into 🤗 Transformers - trying to make complex and new deep learning technology accessible to **everybody**.
...
into the original repository and also when you start writing the 🤗 Transformers implementation of the model. Only at
the very end, when the model has already been successfully ported to 🤗 Transformers, should you verify that the model
...
- At a later stage when you're comparing the original model to the Hugging Face implementation, you can automatically
  verify that each individual component matches it's corresponding component in the 🤗 Transformers implementation
...
We expect every model added to 🤗 Transformers pass a couple of integration tests. This means the original model and
the reimplemented version in 🤗 Transformers should give the exact same output up to a precision of 0.001! Since it is
...
As such, you'll almost certainly compare the intermediate outputs of the 🤗 Transformers model multiple times against
...
- Use the smallest pretrained checkpoint you can find. The smaller the checkpoint, the faster your debug cycle becomes.
  ...
  randomly initialized weights and save those weights for comparison with the 🤗 Transformers version of your model.
...
Once you're comfortable with a debugging strategy and confident you can efficiently debug the original model, you're ready to start writing some new code in 🤗 Transformers!

### Port BrandNewBert to 🤗 Transformers

Start by going to the clone of your 🤗 Transformers' fork:
...
existing model, you can just reuse the entire model architecture of the existing model. You just need to add a [conversion script](add_new_model#write-a-conversion-script) to convert the checkpoint of `BrandNewBert` from the original repository to a checkpoint compatible with your 🤗 Transformers implementation of `BrandNewBert`.
...
request ([WIP] Add `BrandNewBert`) on the main 🤗 Transformers repository so that you and the Hugging Face team can work
...
Ok, now you're really ready to start coding! 😊
...
layer, and so on. Again, it is often useful to look at the architecture of existing models with similiar architectures in 🤗 Transformers to
...
has to work at this point is that you can instantiate the 🤗 Transformers implementation of `BrandNewBert`. The
...
the original repository to a checkpoint compatible with your 🤗 Transformers implementation of
`BrandNewBert`. It is not advised to write the conversion script from scratch, but rather, look through
existing conversion scripts in 🤗 Transformers for one that has been used to convert a similar model written in
...
most likely because you used incorrect parameters in `BrandNewBertConfig()`, have a wrong architecture in the 🤗
Transformers implementation, you have a bug in the `init()` functions of one of the components of the 🤗 Transformers
...
the 🤗 Transformers model. Once you've correctly loaded the checkpoint into the 🤗 Transformers implementation, save
...
It is very likely the 🤗 Transformers implementation and the original model implementation don't give the exact
...
The final step to make sure the 🤗 Transformers implementation works correctly is to ensure the outputs are
equivalent to a precision of `1e-3`. Ensure the output shapes are identical. For example, `outputs.shape` should yield the same value for your 🤗 Transformers implementation and the original
...
The best way to fix the problem is usually to compare the `forward()` pass of the original implementation and the 🤗
...
in the 🤗 Transformers implementation. From our experience, a simple and efficient way to do this is to add many print statements
...
Congratulations, you're done with the most difficult part! The remaining work should be a cakewalk. 😊
...
fully comply with the required design yet. To make sure the implementation is fully compatible with 🤗 Transformers, all
...
you used earlier to implement the model in 🤗 Transformers. A template of the integration tests has already been added by
...
Next, we should add the tokenizer of `BrandNewBert`. Usually, the tokenizer is equivalent or very similar to an existing tokenizer in 🤗 Transformers.

It is very important to find and extract the original tokenizer file and load it into the 🤗
...
a functional tokenization script that uses the original repository, create an analogous script for 🤗 Transformers. It should look similar to this:
...
When the `input_ids` of the original repository 🤗 Transformers yield the same values, add a tokenizer test file. Just like the modeling test files of `BrandNewBert`, the tokenization test files should contain a couple of hard-coded integration tests.

### Run end-to-end integration tests

Having added the tokenizer, you should also add a couple of end-to-end integration tests (for both the model and the
tokenizer) to `tests/models/brand_new_bert/test_modeling_brand_new_bert.py` in 🤗 Transformers. This test should show
the 🤗 Transformers implementation works as expected on a meaningful example depending on the task your model is
...
correct and includes all necessary inputs and outputs. For more details, take a look at our [detailed guide](https://github.com/huggingface/transformers/tree/main/docs#writing-documentation---specification) about writing documentation and docstrings. It is always to good keep in mind that documentation is just as important as the code in 🤗 Transformers since the documentation is usually the first impression the community gets about your model.
...
There are a couple of other very strict design tests in 🤗 Transformers that might still be failing, and it'll show up in
...
You have now finished the coding part, congratulations, you are awesome! 😎
...
`BrandNewBert`. The [`~PreTrainedMode.push_to_hub`] method (available to all 🤗 Transformers models), is a quick and efficient way to push your checkpoint to the Hub:
...
You're done programming now and can move to the last step, which is getting your PR merged into the `main` 🤗 Transformers repository. It is worth taking some time to give your finished
...
This is a major open-source contribution and you should be very proud of your work and achievement. We hope you will continue to contribute to 🤗 Transformers in the future if this process didn't scare you off! But now is the time to tell everyone about your new model:
