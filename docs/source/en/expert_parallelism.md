...
Expert parallelism only shards the experts. Everything else (attention, embeddings, norms) and its optimizer state is replicated on every expert-parallel rank, which is what limits the model size you can train. Set `fsdp_size` together with `tp_size` to shard the rest of the model with [FSDP2](./fsdp) on a second mesh dimension.

```py
distributed_config = DistributedConfig(
    tp_size=8,  # expert parallel size
...
The model is loaded on a 2-D `(fsdp, tp)` device mesh: the experts are sharded across the `tp` dimension, all other parameters are fully sharded across the `fsdp` dimension, and `tp_size * fsdp_size` must equal the number of processes. The experts see the same batch on every `fsdp` rank, so their gradients are averaged over that dimension by the [`Trainer`] (FSDP2 only reduces what it shards). Nothing else changes: train with the [`Trainer`] as usual, and `save_model` gathers the sharded weights and writes a regular checkpoint.
...
> [!WARNING]
> Resuming from a checkpoint is not supported yet for models sharded at load time.
