...
You can push a quantized model on the Hub by naively using `push_to_hub` method. This will first push the quantization configuration file, then push the model weights.
Make sure to use `bitsandbytes>=0.37.2` to be able to use this feature. 
...
model = AutoModelForCausalLM.from_pretrained("ybelkada/bloom-560m-8bit")
```
