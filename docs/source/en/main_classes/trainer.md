...
If you're looking to fine-tune a language model like Llama-2 or Mistral on a text dataset using autoregressive techniques, consider using [`~trl.SFTTrainer`]. This trainer is specially optimized for this particular task. On the other hand, the [`Trainer`] is a more versatile option, suitable for a broader spectrum of tasks.
