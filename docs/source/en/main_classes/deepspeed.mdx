...
1. replace `torchrun` with `deepspeed`.
...
```bash
torchrun --nproc_per_node=2 your_program.py <normal cl args>
```
...
Unlike, `torchrun` where you have to specify how many GPUs to use with `--nproc_per_node`, with the
...
In fact, you can continue using `torchrun` with DeepSpeed as long as you don't need to use
...
Note deepspeed expects that nodes in multi-node environment have shared storage by default.
This default behavior can be overriden in deepspeed config file [checkpoint_options](https://www.deepspeed.ai/docs/config-json/#checkpoint-options)
by setting
```json
{
  "checkpoint": {
