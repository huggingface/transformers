...
The `accelerate.state.AcceleratorState` is used to integrate Deepspeed into the 🤗 Transformers core
...
When not using [`Trainer`], to efficiently deploy DeepSpeed ZeRO-3, you must call static method
`AcceleratorState.set_deepspeed_config` before instantiating the model.

If you're using Deepspeed ZeRO-1 or ZeRO-2 you don't need to do anything.
...
AcceleratorState.set_deepspeed_config(ds_config)
