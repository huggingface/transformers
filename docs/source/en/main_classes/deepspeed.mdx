...
Starting from `transformers>4.27.2`, if `synced_gpus` isn't set to a non-`None` value, it'll be set to `True` automatically if these conditions are detected. But you can still override the value of `synced_gpus` if need to.
