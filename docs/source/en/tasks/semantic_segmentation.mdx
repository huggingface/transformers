...
- validation data transformations that only transposes the images, since computer vision models in transformers expect channels-first layout
...
>>> metric_callback = KerasMetricCallback(metric_fn=compute_metrics, eval_dataset=tf_eval_dataset, batch_size=batch_size, label_cols=["labels"])

>>> push_to_hub_callback = PushToHubCallback(
...     output_dir="scene_segmentation",
