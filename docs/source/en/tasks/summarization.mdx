...
Come up with some text you'd like to summarize. For T5, you need to prefix your input depending on the task you're working on. In other words, for summarization you should prefix your input as shown below:
...
<frameworkcontent>
<pt>
Tokenize the text and return the `input_ids` as PyTorch tensors (you can use the model you just finetuned or a pretrained model for summarization):
...
Use the [`~transformers.generation_utils.GenerationMixin.generate`] method to generate the summarization. Check out the [Generation](./main_classes/text_generation) to learn more about the different text generation strategies, and parameters for controlling generation.
...
<tf>
Tokenize the text and return the `input_ids` as TensorFlow tensors (you can use the model you just finetuned or a pretrained model for summarization):
