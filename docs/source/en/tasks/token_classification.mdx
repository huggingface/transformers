...
<frameworkcontent>
<pt>
Tokenize the text and return PyTorch tensors (you can use the model you just finetuned or a pretrained model for NER):
...
Get the class with the highest probability for each token, and use the model's `id2label` mapping to convert it to a label:
...
<tf>
Tokenize the text and return TensorFlow tensors (you can use the model you just finetuned or a pretrained model for NER):
...
>>> model = TFAutoModelForTokenClassification.from_pretrained("philschmid/distilroberta-base-ner-conll2003")
>>> logits = model(**inputs).logits
```
