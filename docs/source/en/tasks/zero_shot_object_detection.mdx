...
a [ViT](./model_doc/vit)-like Transformer to get multi-modal visual and text features. CLIP learns to
...
Pass the inputs through the model, post process and visualize the results. As the images are resized by the image processor
before being fed to the model, you need to use the `post_process_object_detection` method to make sure the predicted bounding
boxes have the correct coordinates related to the original image:
...
For batch processing, you need to pass text queries as a nested list to the processor and images as lists of PIL images,
or PyTorch tensors, or NumPy arrays.

```py
>>> images = [image, im]
>>> text_queries = [["human face", "rocket", "nasa badge", "star-spangled banner"], ["hat", "book", "sunglasses"]]
...
In addition to zero-shot object detection with text queries, OwlViT offers image guided object detection. In this case
you can use an image query to find similar objects in the target image.
Unlike, text queries, only single example image is allowed.
...
For predictions, instead of passing the inputs to model, pass them to `model.image_guided_detection()`. Draw the predictions
