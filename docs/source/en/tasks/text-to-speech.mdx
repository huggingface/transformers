...
is [SpeechT5](model_doc/speecht5). SpeechT5 is pre-trained on a combination of speech-to-text and text-to-speech data, 
allowing it to learn a unified space of hidden representations shared by both text and speech. This means that the same 
pre-trained model can be fine-tuned for different tasks. Furthermore, SpeechT5 supports multiple speakers through x-vector 
speaker embeddings. 
...
It is worth noting that VoxPopuli or any other automated speech recognition (ASR) dataset may not be the most suitable 
option for training TTS models. The features that make it beneficial for ASR, such as excessive background noise, are 
typically undesirable in TTS. Nonetheless, it can be challenging to find top-quality, multilingual, and multi-speaker TTS 
datasets as compared to ASR datasets.
...
20968 examples should be sufficient for fine-tuning. SpeechT5 expects audio data to have a sampling rate of 16 kHz, make sure the examples in the dataset meet this requirement:
...
However, in Dutch a character such as **à** is used to stress syllables, so we can simply replace it with a without changing 
its meaning. The tokenizer supports some characters with diacritical marks, such as **é**, but not all of them. Let's get 
the tokenizer part of the processor:
...
for numbers, and in `normalized_text` the numbers are written out as text. For that reason we'll use `normalized_text`.

To identify unsupported tokens, we'll extract all unique characters in the dataset using the `SpeechT5Tokenizer` which 
...
By taking the difference between these sets, we can find the characters that are in the dataset but not in the tokenizer.
...
Next, let's define a function to map these characters to valid tokens and then run it on the dataset. No need to handle 
space, that's already replaced by `▁` in the tokenizer.
...
Now that we have dealt with special characters in the text, it's time to get our attention to the audio.
...
To speed up training, let's limit ourselves to speakers that have between 100 and 400 examples. This also makes the 
...
each audio file. We'll skip this step in this guide.

### Speaker embeddings

To allow the TTS model to distinguish between multiple speakers, we'll need to create a speaker embedding for each example. 
The speaker embedding is an additional input into the model that captures a particular speaker's voice characteristics.
To create the speaker embeddings, we use the [spkrec-xvect-voxceleb](https://huggingface.co/speechbrain/spkrec-xvect-voxceleb) 
model from SpeechBrain. The following defines a function `create_speaker_embedding()` that takes an input audio waveform 
...
training examples are in Dutch. We're assuming here that this model will still make reasonable speaker embeddings, but 
this may not be true. For better results, you may want to train an X-vector model on the target speech first.

### Processing the dataset

Finally, let's process the data into the format the model expects. We'll create a `prepare_dataset` function takes in a single example 
and uses the `SpeechT5Processor` object to tokenize the input text and load the target audio into a log-mel spectrogram. 
It also adds the speaker embeddings as an additional input.
...
We should remove those from the dataset. In fact, to allow for larger batch sizes we'll remove anything over 200 tokens.
...
Next, let's create a basic train/test split: 
...
We need to define a custom collator to combine multiple examples into a batch. This will pad shorter sequences with padding 
...
Create the `Trainer` object by combining the model, dataset, and data collator.
...
And with that, we're ready to start training! Training will take several hours depending on your GPU. Depending on your GPU, 
...
Pick an example, here we'll use an example from the test dataset. Obtain a speaker embedding. 
...
sometimes what the model outputs doesn't even sound like speech at all. A lot of this appears to be related to the speaker 
