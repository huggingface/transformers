...
<frameworkcontent>
<tf>
Create a preprocessing function to resize the images and return the `pixel_values`, which are required inputs to the model.
...
Apply the preprocessing transformations over the entire dataset using 🤗 Datasets [`~datasets.Dataset.map`] method.
Set `batched=True` to speed things up by processing multiple elements of the dataset at once:

```py
>>> food = food.map(process, batched=True)
```
...
If you aren't familiar with fine-tuning a model with the [`Trainer`], take a look at the basic tutorial [here](../training#train-with-pytorch-trainer)!
...
<Tip>
If you are unfamiliar with fine-tuning a model with Keras, check out the <a href="https://huggingface.co/docs/transformers/main/en/training#train-a-tensorflow-model-with-keras">basic tutorial</a> first!
</Tip>
...
To compute the accuracy from the predictions use a [Keras callback](https://huggingface.co/docs/transformers/main/en/tasks/./main_classes/keras_callbacks).
...
In order to push your trained Keras model to Hub, you need to build it first. When building a Keras model, you must
provide the model's input shape. In this case, we can get it using image processor's attributes. Note that depending on
the pretrained model you are fine-tuning, the corresponding image processors may have other dictionary keys
(e.g. `shortest_edge` instead of `height` and `width`).

```py
>>> from huggingface_hub import push_to_hub_keras

>>> model.build((None, None, image_processor.size["height"], image_processor.size["width"]))
>>> output_dir = "food_classifier"
>>> push_to_hub_keras(model, repo_id=f"MariaK/{output_dir}")
```

Finally, if you want to use this model for inference with [`pipeline`], or re-use it for further fine-tuning,
you need to upload the following files to Hub:
- `preprocessor_config.json` that you can save from the image_processor you have used during the training
- model's weights in `h5` format, and model's `config.json` that you can save by calling the `save_pretrained()` method on the model
...
...     path_in_repo="",
...     repo_id=f"MariaK/{output_dir}",
...     ignore_patterns="**/logs/*.txt",
... )
```

Congratulations! Your model has been shared on 🤗Hub, and you can use it for inference!
</tf>
</frameworkcontent>
...
>>> classifier(image)
[{'score': 0.9209485054016113, 'label': 'beignets'},
 {'score': 0.0064862510189414024, 'label': 'bruschetta'},
 {'score': 0.0054233744740486145, 'label': 'prime_rib'},
 {'score': 0.005338452290743589, 'label': 'pork_chop'},
 {'score': 0.005172972567379475, 'label': 'chicken_wings'}]
 ```
...
>>> im_processor = AutoImageProcessor.from_pretrained("MariaK/food_classifier")
>>> inputs = im_processor(image, return_tensors="tf")
```
