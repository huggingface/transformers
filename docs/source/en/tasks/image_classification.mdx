...
Apply the preprocessing transformations over the entire dataset using 🤗 Datasets `map()` function.
...
Next, create a batch of examples using `DefaultDataCollator`. Unlike other data collators in 🤗 Transformers, the
...
<Tip>
If you are not familiar with fine-tuning a model with Keras, take a look at the [basic tutorial](https://huggingface.co/docs/transformers/main/en/tasks/../training#train-a-tensorflow-model-with-keras)!
</Tip>

To fine-tune a model in TensorFlow, start by setting up an optimizer function, learning rate schedule, and some training hyperparameters:
...
Then you can load ViT with `TFAutoModelForImageClassification` along with the label mappings:
...
provide the model's input shape. In this case we can get it using image processor's attributes.
...
Finally, if you want to use this model for inference with Transformers Pipeline, or re-use it for further fine-tuning,
you need the following files to Hub:
