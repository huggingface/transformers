...
Start by loading a smaller subset of the [Food-101](https://huggingface.co/datasets/food101) dataset from the 🤗 Datasets library. This'll give you a chance to experiment and make sure everythings works before spending more time training on the full dataset.
...
- An `image` field containing a PIL image of the food item.
- A `label` field containing the food item's class.
...
Now you can convert the label id to a label name for more information:
...
Each food class - or label - corresponds to an id; `79` indicates a prime rib in the example above.
...
Apply some image transformations to the images to make the model more robust against overfitting. Here you'll use torchvision's [`transforms`](https://pytorch.org/vision/stable/transforms.html) module. Crop a random part of the image, resize it, and normalize it with the image mean and standard deviation:
