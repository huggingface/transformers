...
<frameworkcontent>
<pt>
Tokenize each prompt and candidate answer pair and return PyTorch tensors (you can use the model you just finetuned or a pretrained model for multiple choice). You should also create some `labels`:
...
>>> model = AutoModelForMultipleChoice.from_pretrained("distilbert-base-cased-distilled-squad")
...
<tf>
Tokenize each prompt and candidate answer pair and return TensorFlow tensors (you can use the model you just finetuned or a pretrained model for multiple choice).
