...
The [`Trainer`] hides columns - `example_id` and `offset_mapping` - that aren't needed by the model so you'll need to use the [`~datasets.Dataset.set_format`] method to retrieve those columns:
...
Now load the [SQuAD](https://huggingface.co/spaces/evaluate-metric/squad) metric from the 🤗 [Evaluate](https://huggingface.co/docs/evaluate/index) library (see the 🤗 Evaluate [quick tour](https://huggingface.co/docs/evaluate/a_quick_tour) to learn more about how to load and compute a metric):
...
Format your predictions into the expected format and pass the predictions and labels to [`~evaluate.EvaluationModule.compute`] to calculate the SQuAD metric:
