...
Here’s an example using [LLaMA-2](https://huggingface.co/meta-llama/Llama-2-7b-chat-hf). If you’re using a different chat-style model, be careful with how `tokenizer.apply_chat_template` processes messages. It might cut out important tokens depending on how the jinja template is written. For example, some models use special `<think> ... </think>` tokens during reasoning, and these could get lost during re-encoding, causing indexing issues. You might need to manually remove or adjust extra tokens from the completions to keep things stable.
