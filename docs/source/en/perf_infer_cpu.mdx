...
for PyTorch < 1.14.0. JIT-mode could benefit models like "Question and Answer", whose forward parameter order matches the tuple input order in jit.trace.
If we meet case like "text classification", whose forward parameter order does not match the tuple input order in jit.trace, jit.trace will fail and we are capturing this with the exception here to make it fallback and logging is used to notify users.
