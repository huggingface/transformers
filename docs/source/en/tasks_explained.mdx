...
3. About half of the feature vectors are randomly masked, and the masked feature vector is fed to a *context network*. The context network is a Transformer encoder that also adds relative positional embeddings.
...
1. Use a modern CNN, like [ConvNeXT](model_doc/convnext), which relies on convolutional layers but adopts modern network designs.
2. Use a Transformer architecture to split an image into a sequence of patches and process them in parallel.
...
1. An image is split into square non-overlapping patches. If you had a 224x224 pixel image, you could split it into 196 16x16 image patches. Just like how text is tokenized into words, an image is "tokenized" into a sequence of patches. Each patch is then flattened into a single vector.

    The patches are linearly transformed into *patch embeddings* by a 2D convolution to create the proper input dimensions.

2. A *learnable embedding* - a special `[CLS]` token - is added to the beginning of the patch embeddings just like in the BERT model. The final hidden state of the `[CLS]` token is used as the input to the attached classification head; other outputs are ignored. This token helps the model learn how to encode a representation of the image.
...
Mask2Former is a universal architecture for solving all types of segmentation tasks. Traditional segmentation models classifies all the pixels in an image, but Mask2Former frames the task as a *mask classification* problem. Mask classification groups pixels into *N* segments, and predicting *N* masks and their corresponding class label.
...
1. A standard [ResNet101](model_doc/resnet) backbone accepts an image and creates a low-resolution image feature map with 3 consecutive 3x3 convolutions.
