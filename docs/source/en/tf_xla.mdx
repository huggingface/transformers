...
In order to run the forward pass with an XLA-compiled function, we’d need to:
...
To enable XLA-accelerated generation within 🤗 Transformers, you need to have the correct version of `transformers` installed. You can install it by running:
