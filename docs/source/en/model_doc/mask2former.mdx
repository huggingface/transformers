...
A follow up for Mask2Former paper was also released called [Mask2Former for Video Instance Segmentation](https://arxiv.org/abs/2112.10764) by Bowen Cheng, Anwesa Choudhuri, Ishan Misra, Alexander Kirillov, Rohit Girdhar, Alexander G. Schwing. Through this paper, the authors have tried to show that Mask2Former model can achieve SoTA performance for video instance segmentation as well without any architectural changes.
...
- To get the final segmentation result for video instance segmentation task, you can call [`~Mask2FormerImageProcessor.post_process_video_instance_segmentation`]. This task can also be solved using output from [`Mask2FormerForUniversalSegmentation`].
