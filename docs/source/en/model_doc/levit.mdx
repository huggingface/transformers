...
- Compared to ViT, LeViT models uses additional distillation head to effectively learn from a teacher (which, in the
  LeViT paper, is a ResNet like-model). The distillation head is learned through backpropagation under supervision of
  from ResNet like-model. They also use inspiration from convolution neural networks to use activation maps with decreasing
  resolutions to increase the efficiency.
...
- All released checkpoints were pre-trained and fine-tuned on  [ImageNet-1k](http://www.image-net.org/challenges/LSVRC/2012/) 
  ...
  pre-training.
- The authors of LeViT released 5 trained LeViT models, which you can directly plug into [`LevitModel`] or [`LevitModelForImageClassification`]. 
  ...
  (while only using ImageNet-1k for pre-training). The 5 variants available  are (all in 224x224 image size):
