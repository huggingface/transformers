...
## Randomely initializing [`EncoderDecoderModel`] from model configurations.

[`EncoderDecoderModel`] can be randomly initialized from an encoder and a decoder config. In the following example, we should how to do this using the default [`BertModel`] configuration for both the encoder and the decoder.
...
To do so, the `EncoderDecoderModel` class provides a [`EncoderDecoderModel.from_encoder_decoder_pretrained`] function.
...
To load fine-tuned checkpoints of the `EncoderDecoderModel` class, ['EncoderDecoderModel`] provides the `from_pretrained(...)` method just like any other model architectures in Transformers.
...
Once the model is created,it can be finetuned simliar to Bart,T5 or any other encoder-decoder model.
