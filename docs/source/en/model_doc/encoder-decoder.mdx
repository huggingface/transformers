...
## Initialising the [`EncoderDecoderModel`] using only architecture

In this we only take the architecture from the existing model, not the weights. In the below code
we make both encoder and decoder architecture same as bert architecture.
...
## Initialising the [`EncoderDecoderModel`] using models from the hub

In this we initialize both architecture and weights of existing model. In the below example
we make bert based encoder and decoder.
...
## Loading the existing [`EncoderDecoderModel`]

This is to load existing EncoderDecoder Model from local/hub.
...
Once the model is created,it can be finetuned simliar to Bart,T5 or any other EncoderDecoder model.
...
>>> labels = tokenizer("the eiffel tower surpassed the washington monument to become the tallest structure in the world. \
 ...
 ... building by 5. 2 metres ( 17 ft ) and is the second tallest free - standing structure in paris.", return_tensors="pt").input_ids
