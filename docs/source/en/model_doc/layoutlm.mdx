...
_Pre-training techniques have been verified successfully in a variety of NLP tasks in recent years. Despite the
...
(from 93.07 to 94.42)._

Tips:

- In addition to _input_ids_, [`~transformers.LayoutLMModel.forward`] also expects the input `bbox`, which are
...
- A notebook on how to [`fine-tune LayoutLM on the FUNSD dataset`](https://colab.research.google.com/github/NielsRogge/Transformers-Tutorials/blob/master/LayoutLM/Add_image_embeddings_to_LayoutLM.ipynb).

- A notebook on how to [`fine-tune LayoutLMForSequenceClassification on the RVL-CDIP dataset`](https://colab.research.google.com/github/NielsRogge/Transformers-Tutorials/blob/master/LayoutLM/Fine_tuning_LayoutLMForSequenceClassification_on_RVL_CDIP.ipynb).

- A blog post on how to [Fine-tuning LayoutLM for document-understanding using Hugging Face Transformers](https://www.philschmid.de/fine-tuning-layoutlm).

- A notebook on how to [ fine-tune the LayoutLM model on the FUNSD dataset](https://colab.research.google.com/github/NielsRogge/Transformers-Tutorials/blob/master/Fine_tuning_LayoutLMForTokenClassification_on_FUNSD.ipynb).
