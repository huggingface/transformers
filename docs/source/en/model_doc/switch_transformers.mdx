...
The Switch Transformer model uses a sparse T5 encoder-decoder architure, where the MLP are replace by a Mixture of Experts (MoE). A routing mecanism (top 1 in this case) associates each token to one of the expert, where each expert is a dense MLP. While switch tranformers have a lot more weights than their equivalent dense models, the sparsity allows better scaling and better finetuning performance at scale.
