...
The model is a GPT2 model with some additional optimizations, the list of optimizations are:

## Technical details

- Use `gelu_pytorch_tanh` istead of classic `gelu`.
...
- Pre-allocate the key-value cache during inference. Optional, huge speedup (2-3x) for large sequence lengths but disabled by default because it adds a lot of constraints on the generation process.
- Pad the key length to a multiple of 8 (with pre-allocated KV cache only)
...
## GPTBigCode specific outputs

[[autodoc]] models.gpt_bigcode.modeling_gpt_bigcode.GPTBigCodeDoubleHeadsModelOutput
...
[[autodoc]] GPTBigCodeDoubleHeadsModel
    - forward
