...
- Added support for Multi-Query Attention, with two variants. `MULTI_QUERY_1` keeps the query, key and value in a single tensor and is faster for inference (generation), but not for training (needs an extra copy). `MULTI_QUERY_2` separates the query from the key and value and avoids the copy in training, but is slower for generation because it involves two matrix multiplications.
