...
that improves [Vision Transformer (ViT)](https://arxiv.org/abs/2010.11929) in performance and efficiency by introducing convolutions
into ViT to yield the best of both designs.
...
- Cvt models are regular Vision Transformers, but trained with convolutions. It outperform the [original model (ViT)](vit) 
  ...
  [`ViTFeatureExtractor`] by [`CvtFeatureExtractor`] and
  [`ViTForImageClassification`] by [`CvtForImageClassification`]).
