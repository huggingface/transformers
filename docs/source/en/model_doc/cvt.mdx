...
The Cvt model was proposed in [CvT: Introducing Convolutions to Vision Transformers](https://arxiv.org/abs/2103.15808)  by 
Haiping Wu, Bin Xiao, Noel Codella, Mengchen Liu, Xiyang Dai, Lu Yuan and Lei Zhang. Convolutional vision Transformer (CvT),
that improves [Vision Transformer (ViT)](https://arxiv.org/abs/2010.11929) in performance and efficiency by introducing convolutions into ViT to yield the best of both designs.
...
- Cvt models are regular Vision Transformers, but trained with convolutions. It outperform the [original model (ViT)](vit) 
  when fine-tuned on ImageNet-1K and CIFAR-100. You can check out demo notebooks regarding inference as well as
  fine-tuning on custom data [here](https://github.com/NielsRogge/Transformers-Tutorials/tree/master/VisionTransformer) (you can just replace [`ViTFeatureExtractor`] by [`CvtFeatureExtractor`] and [`ViTForImageClassification`] by [`CvtForImageClassification`]).
