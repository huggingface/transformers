...
[Wav2Vec2](https://arxiv.org/abs/2006.11477) is a powerful speech model that learns speech representations from raw audio through self-supervised learning. What makes it unique is its ability to learn from unlabeled speech data and then be fine-tuned on small amounts of labeled data to achieve state-of-the-art results. For example, it can achieve 1.8/3.3 WER on Librispeech clean/other test sets using all labeled data, and even with just 10 minutes of labeled data, it achieves 4.8/8.2 WER when pre-trained on 53k hours of unlabeled data.

You can find all the original Wav2Vec2 checkpoints under the [Wav2Vec2](https://huggingface.co/models?other=wav2vec2) collection.

> [!TIP]
> Click on the Wav2Vec2 models in the right sidebar for more examples of how to apply Wav2Vec2 to different speech tasks like automatic speech recognition (ASR) and audio classification.

The example below demonstrates how to use Wav2Vec2 for automatic speech recognition with [`Pipeline`] or the [`AutoModel`] class.
...
```bash
transformers-cli transcribe facebook/wav2vec2-base-960h path/to/audio.wav
```
...
>>> visualizer = AttentionMaskVisualizer("facebook/wav2vec2-base-960h")
