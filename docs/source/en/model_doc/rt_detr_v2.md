...
The model is meant to be used on images resized to a size 640x640 with the corresponding ImageProcessor. Reshaping to other sizes will generally degrade performance. Also, batching images of variable size is not supported in the current implementation.
```py
>>> import torch
>>> import requests
