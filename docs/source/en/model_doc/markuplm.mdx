...
This is the simplest case, in which the processor (actually the feature extractor) will use the feature extractor to
get all nodes and xpaths from the HTML.
...
"""
encoding = processor(
    html_string, return_tensors="pt"
)  # you can also add all tokenizer parameters here such as padding, truncation
print(encoding.keys())
# dict_keys(["input_ids", "attention_mask", "token_type_ids", "xpath_subs_seq", "xpath_tags_seq"])
```

**Use case 2: web page classification (training, inference) + token classification (inference), parse_html=False**

In case one already has all nodes and xpaths, one doesn't need the feature extractor. In that case, one should
provide the nodes and corresponding xpaths themselves to the processor.
...
print(encoding.keys())
# dict_keys(["input_ids", "attention_mask", "token_type_ids", "xpath_subs_seq", "xpath_tags_seq", "labels"])
```
...
For question answering tasks (such as WebSRC), you can provide a question to the processor. If you want to
...
xpaths = ["/html/body/div/li[1]/div/span", "/html/body/div/li[1]/div/span", "html/body", "html/body/div"]
question = "What's his name?"
encoding = processor(html_string, nodes, xpaths=xpaths, questions=question, return_tensors="pt")
print(encoding.keys())
# dict_keys(["input_ids", "attention_mask", "token_type_ids", "xpath_subs_seq", "xpath_tags_seq"])
```
