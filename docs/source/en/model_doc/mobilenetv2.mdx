...
The MobileNetV2 model was proposed in [TODO](https://arxiv.org/abs/2110.02178) by TODO. MobileNetV2 introduces a new layer that replaces local processing in convolutions with global processing using transformers.
...
- One can use [`MobileNetV2FeatureExtractor`] to prepare images for the model. Note that if you do your own preprocessing, the pretrained checkpoints expect images to be in BGR pixel order (not RGB).
