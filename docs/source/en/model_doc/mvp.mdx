...
The MVP model was proposed in [MVP: Multi-task Supervised Pre-training for Natural Language Generation](https://github.com/RUCAIBox/MVP) by Tianyi Tang, Junyi Li, Wayne Xin Zhao and Ji-Rong Wen.
...
This model was contributed by [Tianyi Tang](https://huggingface.co/StevenTang).
The detailed information and instructions can be found [here](https://github.com/RUCAIBox/MVP).

## Examples
For summarization, you can use MVP or MVP with summarization-specific prompts.
...
For data-to-text generation, you can use MVP or multi-task pre-trained variants.
```python
...
>>> model = MvpForConditionalGeneration.from_pretrained("RUCAIBox/mvp", use_prompt=True, lightweight_tuning=True)
>>> # lightweight tuning with task-specific prompts
>>> model = MvpForConditionalGeneration.from_pretrained("RUCAIBox/mtl-data-to-text", lightweight_tuning=True)
>>> # original Prefix-tuning
>>> model = MvpForConditionalGeneration.from_pretrained(
...     "facebook/bart-large", use_prompt=True, lightweight_tuning=True
... )
>>> # the number of trainable parameters
>>> sum(p.numel() for p in model.parameters() if p.requires_grad)
61823328

>>> # MVP without lightweight tuning
>>> model = MvpForConditionalGeneration.from_pretrained("RUCAIBox/mvp")
>>> # the number of trainable parameters
>>> sum(p.numel() for p in model.parameters() if p.requires_grad)
406293504
