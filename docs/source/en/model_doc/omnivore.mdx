...
- The model is trained on three datasets [ImageNet-1k](http://www.image-net.org/challenges/LSVRC/2012/), [Kinetics](https://www.deepmind.com/open-source/kinetics) and [SUN RGB-D](https://rgbd.cs.princeton.edu/) respectively. 
  There are five checkpoints available are trained on said dataset. In single epoch for the model consisted on one epoch each for ImageNet-1k and Kinetics with ten epochs for SUN RGB-D.
