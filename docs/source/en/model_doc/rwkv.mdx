...
The RWKV model was proposed in [this repository](https://github.com/BlinkDL/RWKV-LM)

It suggests a tweak in the traditional Transformer attention to make it linear. This way, the model is akin to a Recurrent Neural Network (RNN): passing inputs for timestamp 0 and timestamp 1 together is the same as passing inputs at timestamp 0, then inputs at timestamp 1 along with the state of timestamp 0 (see example below).

This can be more computationally efficient than a regular Transformer, and allows for input token sequences of any length (even if the model uses a fixed context length for training).
...
In a traditional autoregressive Transformer, attention is written as

$$O = \hbox{softmax}(QK^{T} / \sqrt{d}) V$$

where \\(Q\\), \\(K\\) and \\(V\\) are matrices of shape `seq_len x hidden_size` named the Query, Key, and Value (in reality, these are matrices with higher dimensionality to account for a batch dimension and an attention head dimension. In this instance, we are only interested in the last two dimensions, which is where the matrix product is taken, so for the sake of simplicity we only consider those two). The product \\(QK^{T}\\) then has shape `seq_len x seq_len` and we can take the maxtrix product with \\(V\\) to get the output \\(O\\) of the same shape as the others.  
...
where \\(R\\) is a new matrix called Receptance by the author, \\(K\\) and \\(V\\) are the same Key and Value matrices. \\(\sigma\\) is the sigmoid activation function. \\(W\\) is a new vector that represents the position of an input token, and is given by
...
The actual recurrent formula used is marginally more complex: for numerical stability, we don't want to compute exponentials of big numbers. Usually the softmax is not computed as is, but the exponential of the maximum term is divided of the numerator and denominator:
