...
[Tensor parallelism](./perf_infer_gpu_multi) splits a model layer across devices. Pass a [`~distributed.DistributedConfig`] to [`~PreTrainedModel.from_pretrained`] to enable it. The same config also covers FSDP2 and expert parallelism.
