...
[Tensor parallelism](./perf_infer_gpu_multi) splits a model layer across devices. Set the number of devices with `DistributedConfig(tp_size=N)` and pass it to [`~PreTrainedModel.from_pretrained`] to enable it. To place whole layers on different devices instead, use [pipeline parallelism for inference](./pipeline_parallel_inference) with `DistributedConfig(pp_size=N)`. You can set both together for a PP+TP mesh.
