...
adding TensorFlow compatibility to an existing model is simpler than to [add a new model from scratch](add_new_model)!
...
is no small feat, but opefully this guide will make less of a rollercoaster 🎢 and more of a walk in the park 🚶.
...
Before you dive deeper, it is recommended that you check the following resources if you're new in 🤗 Transformers:
...
Are you unsure whether the model you wish to use already has a corresponding TensorFlow architecture, so you can jump
straight to the weight conversion section? Check the `model_type` field of the `config.json` of your model of choice
...
- Ask for help when you're stuck! The 🤗 Transformers is here to help, and we've probably found solutions to the same
...
haven't your eyes set on a specific architecture, asking the 🤗 Transformers team for suggestions is a great way to
...
🤗 Transformers but is lacking weights, feel free to jump straight into the weight conversion section of this page.
...
- 1. Make sure you are logged into your Hugging Face account in your terminal. You can log in using the command
   `huggingface-cli login` (you find your access tokens [here](https://huggingface.co/settings/tokens))
...
might come across errors compaining about mismatches between PyTorch and TensorFlow. You might even decide open the
