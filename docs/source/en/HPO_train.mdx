...
[Optuna](https://optuna.org/),[Sigopt](https://sigopt.com/),[Raytune](https://docs.ray.io/en/latest/tune/index.html),[wandb](https://wandb.ai/site/sweeps).
...
Define mode_init callback if you need to re-initialize the model per trial,and pass it when `Trainer Object` is created, take following as example
```py
...
Create a `Trainer object` with your model, model_init function, training arguments, training and test datasets, and evaluation function:
...
Currently, Hyperparameter search for DDP is enabled for optuna and sigopt. Only rank0 will generate the search trial and pass the argument to other ranks.