...
```py
agent.run("Read the following text out loud")
```
| **Input**                                                                                                               | **Output**                                   |
|-------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|
...
Tools are very simple: they're a single function, with a name, and a description. An image captioning tool would be 
designed as follows:

```py
class ImageCaptioningTool:
    description = "This is a tool that captions images ..."

    inputs = ["image"]
    outputs = ["text"]

    def __call__(self, image) -> str:
        # Call the image captioning model
        image_caption = self.model(image)
        return image_caption
```

We then use these tools description to prompt the agent. Through the prompt, we show the agent how it would leverage 
tools in order to perform what was requests in the query.
...
image_generator = load_tool("text-to-image", repo_id="huggingface-tools/text-to-image")
