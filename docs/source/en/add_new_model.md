...
model's [`~PreTrainedModel.save_pretrained`] will automatically call
config's [`~PretrainedConfig.save_pretrained`], so that both model and configuration are saved.
