...
Use [`DataCollatorWithPadding`] to batch the examples together. It will also pad your text, so they are a uniform length. While it is possible to pad your text in the `tokenizer` function by setting `padding=True`, it is more efficient to only pad the text to the length of the longest element in its batch. This is known as *dynamic padding* and is enabled by [`DataCollatorWithPadding`].
...
Start by batching the processed examples together with dynamic padding using [`DataCollatorWithPadding`]. Dynamic padding is more efficient because it only pads text to the longest element in its batch. Make sure you set `return_tensors="tf"` to return `tf.Tensor`'s instead of PyTorch tensors!
