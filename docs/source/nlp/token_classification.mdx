...
Unless you are very familiar with this dataset, the `ner_tags` aren't too informative right now. Access the label names of the `ner_tags` for more information:
...
Use 🤗 Datasets [`map`](https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.map) function to tokenize and align the labels over the entire dataset. You can also set `batched=True` to process multiple elements of the dataset at once for faster preprocessing:

```py
>>> tokenized_wnut = wnut.map(tokenize_and_align_labels, batched=True)
```

Use [`DataCollatorForTokenClassification`] to batch the examples together. It will also pad your text and labels, so they are a uniform length. While it is possible to pad your text and labels in the `tokenizer` function by setting `padding=True`, it is more efficient to only pad the text and labels to the length of the longest element in its batch. This is known as *dynamic padding* and is enabled by [`DataCollatorForTokenClassification`].
...
Start by batching the processed examples - text and labels - together with dynamic padding using [`DataCollatorForTokenClassification`]. Dynamic padding is more efficient because it only pads the text and labels to the longest element in its batch. Make sure you set `return_tensors="tf"` to return `tf.Tensor`'s instead of PyTorch tensors!
