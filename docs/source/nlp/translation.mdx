...
The `translation` field is a dictionary with the English and French translations of the text.
...
1. Prefix the input with a prompt to tell T5 this is a translation task. This prefix is specific to T5, and other translation models like MarianMT do not require prompting.
...
Use [`DataCollatorForSeq2Seq`] to batch the examples together. It will also pad your text and labels, so they are a uniform length. While it is possible to pad your text and labels in the `tokenizer` function by setting `padding=True`, it is more efficient to only pad the text and labels to the length of the longest element in its batch. This is known as *dynamic padding* and is enabled by [`DataCollatorForSeq2Seq`].
...
Start by batching the processed examples together with dynamic padding using [`DataCollatorForSeq2Seq`]. Dynamic padding is more efficient because it only pads text to the longest element in its batch. Make sure you set `return_tensors="tf"` to return `tf.Tensor`'s instead of PyTorch tensors!
