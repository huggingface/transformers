...
- I files del modello devono esssere il più self-contained possibile cosicché quando leggi il codice di uno specifico modello, idealmente, 
...
Come regola generale, vogliamo esssere sicuri che un nuovo modello dipenda solo da [`PreTrainedModel`]. Le funzionalità
...
Similmente al modello, la configurazione eredita le funzionalità base di serializzazione e deserializzazione da 
...
`src/transformers/models/brand_new_bert/modeling_brand_new_bert.py` avrà sia la stessa arhcitettura di BERT se é un 
...
uno simile al vostro modello. Di solito é abbastanza fare una copia di uno script gia esistente e adattarlo al vostro caso. 
...
Una volta sistemati i test comuni, dobbiamo assicurarci che il vostro lavoro sia correttamente testato cosicchè:
...
A volte capita che mancano delle informazioni nella docstring o alcuni nomi sbagliati, questo farà fallire i tests sopra. 
