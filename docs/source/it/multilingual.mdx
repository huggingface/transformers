...
Il seguente modello MBart può essere usato per compiti multilingua:
...
MBart fforza l'id del target language come primo token generato per tradurre nel target language. Imposta il parametro `forced_bos_token_id` a `en` nel metodo `generate` per tradurre in inglese:
...
Se stai usando il checkpoint `facebook/mbart-large-50-many-to-one-mmt`, non hai bisogno di forzare l'id del target language come primo token generato altrimenti l'uso è lo stesso.