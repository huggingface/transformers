...
```py
>>> risultati = classificatore(["Siamo molto felici di mostrarti la libreria 🤗 Transformers.", "Speriamo te non la odierai."])
...
>>> riconoscitore_vocale = pipeline("automatic-speech-recognition", model="radiogroup-crits/wav2vec2-xls-r-1b-italian-doc4lm-5gram")
```
