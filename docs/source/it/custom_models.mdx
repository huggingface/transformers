...
Transformers, e come condividerlo con la community (assieme al relativo codice) così che tutti possano usarlo, anche
...
Quando ricarichi una config da un metodo `from_pretrained`, questi campi devono essere accettati dal tuo config e
...
non voglia registrare il modello con le auto classi (vedi l'ultima sezione).

Una volta completato, puoi facilmente creare e salvare la tua configurazione come faresti con ogni altra configurazione
di modelli della libreria. Ecco come possiamo creare il config di un resnet50d e salvarlo:
...
non è obbligatoria, a meno che tu non voglia registrare il modello con le auto classi (vedi l'ultima sezione).

<Tip>

Se il to modello è molto simile a un modello all'interno della libreria, puoi ri-usare la stessa configurazione di quel modello.
...
Se stai copiando i file un modello dalla libreria, dovrai sostituire tutti gli import relativi in cima al file con import del 
...
`save_pretrained` e registrare in modo corretto con una Auto classe (specialmente per i modelli). Utilizza semplicemente:
...
per cui devi specificare quale delle Auto classi è quella corretta per il tuo modello.

Successivamente, creiamo i modelli e il config come abbiamo fatto in precedenza:
...
Adesso, per inviare il modello all'Hub, assicurati che di essere loggato. Lancia dal tuo terminal:
...
Puoi usare ogni configurazione, modello o tokenizer con file di codice personalizzati nella sua repositoruy 
