...
Inoltre, raccomandiamo fortemente di passare un hash del commit come `revision` per assicurarti che l'autore del modello 
non abbiano aggiornato il codice con alcune nuove righe dannose (a meno che non ti fidi completamente degli autori del modello):
