...
```py
>>> pt_model = DistilBertForSequenceClassification.from_pretrained("path/verso/il-nome-magnifico-che-hai-scelto", from_tf=True)
...
```py
>>> tf_model = TFDistilBertForSequenceClassification.from_pretrained("path/verso/il-nome-magnifico-che-hai-scelto", from_pt=True)
```
...
...     output_dir="./il_path_dove_salvare_il_tuo_modello", tokenizer=tokenizer, hub_model_id="il-tuo-username/il-mio-bellissimo-modello"
... )
```
...
O magari ti piacerebbe aggiungere la versione di TensorFlow del tuo modello PyTorch a cui hai fatto fine-tuning:
...
- Seleziona il/la **owner** del repository. Questo puoi essere te o qualunque organizzazioni di cui fai parte.
