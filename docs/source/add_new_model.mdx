...
own regarding how code should be writte :-)
...
1. The forward pass of your model should be fully written in the modeling file while being fully independent of other
   ...
   for a good example).
1. The code should be fully understandable, even by a non-native English speaker. This means you should pick
...
1. More generally we prefer longer explicit code to short magical one.
1. Avoid subclassing `nn.Sequential` in PyTorch but subclass `nn.Module` and write the forward pass, so that anyone
   using your code can quickly debug it by adding print statement or breaking points
1. Your function signature should be type-annotated. For the rest, good variable names is way more readable and
