...
personalizar seu laço de treinamento de PyTorch nativo para poder treinar em ambientes distribuídos.
...
`Accelerator` detectará automáticamente o tipo de configuração distribuída que está disponível e inicializará todos os
...
Passe todos os objetos relevantes para o treinamento ao método [`prepare`](https://huggingface.co/docs/accelerate/accelerator.html#accelerate.Accelerator.prepare).
...
Por último, substitua o `loss.backward()` padrão em teu laço de treinamento com o método [`backward`](https://huggingface.co/docs/accelerate/accelerator.html#accelerate.Accelerator.backward) do 🤗 Accelerate:
