...
Inclusive, se não tens experiência com alguma modalidade específica ou não compreende o código que forma os modelos,
...
Qualquer parâmetro adicional para sua tarefa também pode ser incluído no [`pipeline`]. A tarefa `text-generation` tem um método
...
que te permitem filtrar pelo modelo que você gostaria de usar para sua tarefa. Uma vez que tiver escolhido o modelo apropriado,
...
Crie uma [`pipeline`] para sua tarefa e especifíque o modelo e o tokenizador que foram carregados:
...
Especifique sua tarefa de visão e passe sua imagem ao classificador.
