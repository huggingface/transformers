...
Get up and running with 🤗 Transformers! Start using the [`pipeline`] for rapid inference, and quickly load a pretrained model and tokenizer with [AutoClasses](./model_doc/auto) to solve your text, vision or audio task.
...
[`pipeline`] is the easiest way to use a pretrained model on a given task.
...
* Name entity recognition (NER): label each word with the entity it represents (person, place, location, etc.).
...
In the following example, we will look at how to use the [`pipeline`] for sentiment analysis.
...
Import the [`pipeline`] with the task you want to complete:
...
Create a [`pipeline`] with the task you want to solve for, and the model you want to use. Setting the `device` argument to `0` allows you to place the tensors on a CUDA device:
...
Load the dataset (see the 🤗 Datasets [Quick Start](https://huggingface.co/docs/datasets/quickstart.html#) for more details) you want to iterate over:
...
Now iterate over the dataset with the pipeline. `KeyDataset` retrieves the item in the dictionary returned by the dataset:
...
The [`pipeline`] can accommodate any model from the Model Hub, making it easy to adapt the [`pipeline`] for other use-cases. For example, if you'd like a model capable of handling French text, use the tags on the [Model Hub](https://huggingface.co/models) to filter for an appropriate model. In this case, we will use a multilingual [BERT model](https://huggingface.co/nlptown/bert-base-multilingual-uncased-sentiment) fine-tuned for sentiment analysis:

```py
>>> model_name = "nlptown/bert-base-multilingual-uncased-sentiment"
```

Use the `AutoClasses` to load the pretrained model and it's associated tokenizer (more on the `AutoClasses` below):
...
Then you can specify the model and tokenizer, and apply the `classifier` on your target text:
...
Under the hood, the [`AutoTokenizer`] and [`AutoModel`] classes work together to power the [`pipeline`]. The [AutoClasses](./model_doc/auto) are shortcuts that automatically retrieves the model architecture of a pretrained model from it's name or path. You only need to select the appropriate [`AutoModel`] for your task and it's associated tokenizer with [`AutoTokenizer`]. 

Let's return to our example and see how we can use the `AutoClasses` to replicate the results of the [`pipeline`].

### Using the AutoTokenizer

A tokenizer is responsible for preprocessing text in a way that is understandable by the model. First, the tokenizer will split a given text into words called *tokens*. There are multiple rules that govern the tokenization process, including how to split a word and at what level (learn more [here](./tokenizer_summary)). The most important thing to remember though is you need to instantiate the tokenizer with the name of the model to ensure you're using the same tokenization rules a model was pretrained with.
...
Just like the [`pipeline`], the tokenizer can accept a list of inputs. In addition, the tokenizer can also pad and truncate the text to return a batch with uniform length:
...
🤗 Transformers provides a simple and unified way to load pretrained instances, which means you can load an [`AutoModel`] like you would load an [`AutoTokenizer`]. The only difference is selecting the correct [`AutoModel`] for the task. Since we are doing text - or sequence - classification, load the [`AutoModelForSequenceClassification`]. The TensorFlow equivalent is simply [`TFAutoModelForSequenceClassification`]:
...
<Tip>
See the [task summary](./task_summary) for which [`AutoModel`] to use for which task.
</Tip>
...
The model outputs the model's final activations in the `logits` attribute. Apply the softmax function to the `logits` to retrieve the probabilities:
...
<Tip>
PyTorch model outputs are special dataclasses so their attributes are autocompleted in an IDE.
The model outputs also behave like a tuple or a dictionary (e.g., you can index with an integer, a slice or a string) in which
case the attributes that are `None` are ignored.
</Tip>
