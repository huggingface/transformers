...
Let's return to our example and see how we can use an `AutoClass` to replicate the results of the [`pipeline`].

### Using the AutoTokenizer

A tokenizer is responsible for preprocessing text in a way that is understandable by the model. First, the tokenizer will split the text into words called *tokens*. There are multiple rules that govern the tokenization process, including how to split a word and at what level (learn more [here](./tokenizer_summary)). The most important thing to remember though is you need to instantiate the tokenizer with the same model name to ensure you're using the same tokenization rules a model was pretrained with.
