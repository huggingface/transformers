...
Load the dataset you want to iterate over:
...
Now iterate over the dataset with the pipeline. This should be just as fast as running a custom loop on a GPU.
...
The model outputs the model's final activations in the `logits` attribute. Apply the softmax activation to the `logits` to retrieve the predictions:
...
<Tip>
All 🤗 Transformers models (PyTorch or TensorFlow) return the model activations *before* the final activation
...
Once your model is fine-tuned, you can save it with its tokenizer using `save_pretrained`:
...
When you are ready to use the model again, reload it with `from_pretrained`:
