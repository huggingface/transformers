...
2. Typically, when you call the processor, you call the feature extractor. Since you also want to tokenize text, instruct the processor to call the tokenizer instead with a context manager.
...
Unlike other data collators, `DataCollatorWithPadding` needs to apply a different padding method to `input_values` and `labels`. You can apply a different padding method with a context manager:
...
Load Wav2Vec2 with [`AutoModelForCTC`]. Save GPU memory by calculating the mean of the loss reduction instead of the sum, and specify the padding token:

```py
>>> from transformers import AutoModelForCTC, TrainingArguments, Trainer

>>> model = AutoModelForCTC.from_pretrained("facebook/wav2vec-base", ctc_loss_reduction="mean", pad_token_id=processor.tokenizer.pad_token_id,)
```
...
For a more in-depth example of how to fine-tune a model for automatic speech recognition, take a look at the corresponding [PyTorch notebook](https://colab.research.google.com/github/huggingface/notebooks/blob/master/examples/speech_recognition.ipynb).
