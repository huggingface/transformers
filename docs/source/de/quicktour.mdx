...
Für mehr Details über die [`pipeline`] und assoziierte Aufgaben, siehe in die Dokumentation [hier](./main_classes/pipelines).
...
Die [`pipeline`] kann auch über einen ganzen Datensatz iterieren. Starte mit der Installation der [🤗 Datasets](https://huggingface.co/docs/datasets/) Bibliothek:

```bash
pip install datasets 
```

Erstelle eine [`pipeline`] mit der Aufgabe die du lösen möchtest und dem Modell welches du nutzen möchtest.
...
Als nächstes lade den Datensatz (siehe 🤗 Datasets [Quick Start](https://huggingface.co/docs/datasets/quickstart.html) für mehr Details) welches du nutzen möchtest. Zum Beispiel lade den [MInDS-14](https://huggingface.co/datasets/PolyAI/minds14) Datensatz:
