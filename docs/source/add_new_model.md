...
🤗Transformers repository and ideally also of the model's original repository.
...
## Adding brand_new_bert to 🤗Transformers

For exemplary purposes, we will call the model that we will add to 🤗Transformers `"brand_new_bert"`.
...
#### Paper
  ...
  - What time of model is brand_new_bert? BERT-like encoder-only model? GPT2-like decoder-only model? BART-like encoder-decoder model?
...
```bash
model = brand_new_bertModel.load_pretrained_checkpoint(/path/to/checkpoint/)
...
the generated files `src/transformers/models/brand_new_bert/modeling_brand_new_bert.py`
and `src/transformers/models/brand_new_bert/configuration_brand_new_bert.py`.

Now you can finally start coding :). The generated code in `src/transformers/models/brand_new_bert/modeling_brand_new_bert.py` will
...
`src/transformers/models/brand_new_bert/modeling_brand_new_bert.py` until you feel like all the 
...
```python
model = brand_new_bertModel.from_pretrained(/path/to/converted/checkpoint/folder)
...
🤗Transformers. 
 ...
 As a final step, it is recommended to add one large integration tests to `tests/test_modeling_brand_new_bert.py` that does some end-to-end
...
Next, make sure that the docstring added to `src/transformers/models/brand_new_bert/modeling_brand_new_bert.py` is correct and included all necessary
