...
Besides using gradient accumulation when a larger batch size is needed and can't normally be fit into the gpu's memory, enabling can also provide a 20-30% speed up as it runs the optimizer less often, e.g. see benchmarks for [RTX-3090](https://github.com/huggingface/transformers/issues/14608#issuecomment-1004392537)
...
One way to use significantly less GPU memory is to enabled "Gradient Checkpointing" (also known as "activation checkpointing"). When enabled, a lot of memory can be freed at the cost of small decrease in the training speed due to recomputing parts of the graph during back-propagation. The slow down will depend on the model but quite often it is around 20-30%.
