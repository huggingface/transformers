...
- Supervised training
  ...
  :obj:`decoder_attention_mask` to ignore padded tokens from the loss (see the `Flax summarization script
...
method takes care of encoding the input and feeding the encoded hidden states via cross-attention layers to the decoder and auto-regressively
