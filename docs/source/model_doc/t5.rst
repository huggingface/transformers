...
- T5 is an encoder-decoder model pre-trained on a multi-task mixture of unsupervised and supervised tasks and for which
  ...
  for summarization: *summarize: ...*. According to `this forum post
  <https://discuss.huggingface.co/t/t5-finetuning-tips/684>`__, task prefixes matter when (1) doing multi-task training
  (2) your task is similar or related to one of the supervised tasks used in T5's pre-training mixture (see Appendix D
  of the `paper <https://arxiv.org/pdf/1910.10683.pdf>`__ for the task prefixes used).

- T5 is trained using the :obj:`CrossEntropyLoss`. During training,
...
- Supervised training
  ...
  token id's of :obj:`labels` are replaced by -100, such that they are not taken into account by the loss function. We
  also pass :obj:`attention_mask` as additional input to the model, which makes sure that padding tokens of the inputs
  are ignored. The code example below illustrates all of this.
