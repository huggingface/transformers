...
- During training, the authors of DETR did find it helpful to use auxiliary losses in the decoder, especially to help
  ...
  [`~transformers.MaskFormerConfig`] to `True`, then prediction feedforward neural networks and Hungarian losses
...
- To get the final segmentation, depending on the task, you can call [`~transformers.MaskFormerFeatureExtractor.post_process_semantic_segmentation`] or [`~transformers.MaskFormerFeatureExtractor.post_process_panoptic_segmentation`].
...
[[autodoc]] MaskFormerPretrainedModel
