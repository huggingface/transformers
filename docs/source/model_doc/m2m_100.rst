...
M2M100 is a multilingual encoder-decoder (seq-to-seq) model primarily intended for translation task. As the model is
multilingual it expects the sequences in a different format. A special language id token is used as prefix in both the
source and target text. The source text format is is :obj:`[lang_code] X [eos]`, where :obj:`lang_code` is source
...
- Generation
    ...
    To generate using the M2M100 multilingual translation models, :obj:`eos_token_id` is used as the
    ...
    The following example shows how to translate between Hindi to French and Chinese to English using the
