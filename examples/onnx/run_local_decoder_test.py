...
class DecoderWithPastWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids, *flat_past, encoder_attention_mask=None):
        """
        Args:
            input_ids: [batch, seq_len] - typically single token at each step
            encoder_attention_mask: optional [batch, enc_seq] (may be None -> use buffer)
            flat_past: flattened list of past key/value tensors for self-attention,
                       in the order: past_0_self_key, past_0_self_value, past_1_self_key, ...
                       Each past tensor shape expected: [batch, n_heads, past_seq_len, head_dim]
        """
        ...
        # If flat_past contains any 4-D tensors and has non-zero past_seq dim -> treat as provided past
        provided_past = any(_is_valid_past_tensor(x) for x in flat_past)

        if provided_past:
            # We expect 2 tensors per layer (self_k, self_v)
            # Validate length
            expected_len = self.n_layers * 2
            if len(flat_past) < expected_len:
                # If fewer were provided, pad with zeros
            ...
            for i in range(self.n_layers):
                # self past from flat_past (key, value)
                self_k = padded[i * 2]
                self_v = padded[i * 2 + 1]
            ...
            # First step: build empty self-attention cache
            for i in range(self.n_layers):
                cross_k = getattr(self, f'cross_k_{i}')
                cross_v = getattr(self, f'cross_v_{i}')
        ...
        # Flatten output past (only self-attention parts)
        flat_output = [logits]
...
with torch.no_grad():
    decoder_input_ids_torch = torch.tensor(
        [[tokenizer.lang_code_to_id["ar_AR"]]])
...
dummy_encoder_mask = torch.ones(1, 64, dtype=torch.long)
...
print(" Exported decoder_with_past.onnx")

# ONNX inference
print("\n Loading ONNX model...")
session = ort.InferenceSession("decoder_with_past.onnx")
...
lang_id = tokenizer.lang_code_to_id["ar_AR"]
# Start with <decoder_start> then the forced BOS language token, as in HF generate
decoder_input_ids = np.array([[decoder_start_id, lang_id]], dtype=np.int64)
generated_ids = [decoder_start_id, lang_id]

# Get actual input/output names
input_names = [i.name for i in session.get_inputs()]
output_names = [o.name for o in session.get_outputs()]

# Initialize empty past
past_kvs = {}
...
for out_name in output_names:
    if out_name.startswith('present_'):
        # present_0_self_key -> past_0_self_key
        past_name = out_name.replace('present_', 'past_')
        if past_name in input_names:
            present_to_past[out_name] = past_name

print(f"Mapping {len(present_to_past)} present->past connections")

for step in range(15):
    # Prepare inputs: recompute from scratch each step with full history and empty past
    ...
    # No cache reuse (ensures parity with HF for this test)
    past_kvs = {}

    if step < 5:
        print(
            f"Step {step}: token={next_token} '{tokenizer.decode([next_token])}' past_shape={outputs[1].shape if len(outputs) > 1 else 'N/A'}")
