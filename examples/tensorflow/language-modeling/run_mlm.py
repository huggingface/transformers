...
def main():
    ...
    with training_args.strategy.scope():
        ...
        tf_train_dataset = tf.data.Dataset.from_generator(train_gen, output_signature=train_sig).batch(
            batch_size=num_replicas * training_args.per_device_train_batch_size, drop_remainder=True
        )
        eval_gen = partial(
            sample_generator, eval_dataset, tokenizer, batch_size=training_args.per_device_eval_batch_size
        )
        eval_sig = {
            feature: tf.TensorSpec(shape=(None,), dtype=tf.int64)
        ...
        try:
            train_perplexity = math.exp(history["loss"])
        except OverflowError:
            train_perplexity = math.inf
        try:
            validation_perplexity = math.exp(history["val_loss"])
        except OverflowError:
            validation_perplexity = math.inf
        logger.info(f"  Final train loss: {history['loss']:.3f}")
        logger.info(f"  Final train perplexity: {train_perplexity:.3f}")
        logger.info(f"  Final train accuracy: {history['accuracy']:.3f}")
        logger.info(f"  Final validation loss: {history['val_loss']:.3f}")
        logger.info(f"  Final validation perplexity: {validation_perplexity:.3f}")
        logger.info(f"  Final validation accuracy: {history['val_accuracy']:.3f}")
        # endregion
