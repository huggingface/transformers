...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            loss = outputs.loss
            # Use actual batch size (from pixel_values or input_ids) instead of configured batch size
            # to correctly handle the last partial batch
            batch_size = batch["pixel_values"].size(0) if "pixel_values" in batch else batch["input_ids"].size(0)
