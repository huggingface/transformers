...
check_min_version("5.8.0.dev0")

logging.basicConfig(level=logging.INFO)
logger = get_logger(__name__)

require_version("datasets>=2.0.0", "To fix: pip install -r examples/pytorch/zero-shot/requirements.txt")
...
def evaluation_loop(
    ...
    for step, batch in enumerate(tqdm(dataloader, disable=not accelerator.is_local_main_process)):
        ...
        predictions = processor.post_process_grounded_object_detection(
            outputs, input_ids, threshold=0.15, text_threshold=0.1, target_sizes=image_size
        )
