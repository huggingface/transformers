...
def compute_metrics(
    ...
    Args:
        ...
        threshold (float, optional): Threshold to filter predicted boxes by confidence. Defaults to 0.15.
    ...
    # During validation a deterministic text prompt (the concatenation of all class names following the
    # `id2label` ordering) is used for every image, so we rebuild the corresponding `input_ids` here
    # instead of collecting them from the model inputs. These are required by the processor to map the
    # predicted token spans back to class labels.
    text_prompt = concat_func(id2label)
    input_ids = processor(text=text_prompt, return_tensors="pt").input_ids
    ...
    for batch, target_sizes in zip(predictions, image_sizes):
        ...
        # the same text prompt is used for every image, so repeat `input_ids` over the batch dimension
        batch_input_ids = input_ids.repeat(len(batch_logits), 1)
...
def main():
    ...
    validation_transform_batch = partial(
        ...
        id2label=id2label,
        label2id=label2id,
        # use a deterministic prompt for evaluation so metrics are computed against a fixed label ordering
        random_text_prompt=False,
    )
