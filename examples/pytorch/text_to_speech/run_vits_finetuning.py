...
def compute_val_metrics_and_losses(
    ...
    loss_kl = kl_loss(prior_latents, posterior_lg, prior_means, prior_lg, labels_mask)
...
def main():
    ...
    per_device_train_batch_size = training_args.per_device_train_batch_size if training_args.per_device_train_batch_size else 1
    total_batch_size = per_device_train_batch_size * accelerator.num_processes * training_args.gradient_accumulation_steps
    ...
    if training_args.do_step_schedule_per_epoch:
        gen_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(gen_optimizer, gamma=training_args.lr_decay, last_epoch=-1)
        disc_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(disc_optimizer, gamma=training_args.lr_decay, last_epoch=-1)
    else:
    ...
    for epoch in range(first_epoch, training_args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model, discriminator):
                ...
                train_losses = [
                    l + losses[i].item() / training_args.gradient_accumulation_steps for (i, l) in enumerate(train_losses)
                ]
            ...
            if do_eval:
                ...
                for step, batch in enumerate(eval_dataloader):
                    ...
                    padded_attn, specs, target_specs = accelerator.pad_across_processes(
                        [attn.squeeze(1), specs, batch["labels"]], dim=1
                    )
                    padded_attn, specs, target_specs = accelerator.pad_across_processes(
