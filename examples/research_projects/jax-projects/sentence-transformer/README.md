...
# Text-Text dual encoder model training examples

> Note: This example is experimental and might not give the best possible results

The following example showcases how to train a CLIP like text-text dual encoder model
using a pre-trained text and text encoder using the JAX/Flax backend.

Such a model can be used for natural language similarity comparison and potentially zero-shot sentence search.
The model is inspired by the [CLIP](https://openai.com/blog/clip/) approach, introduced by Alec Radford et al. and [Sentence-BERT](https://arxiv.org/abs/1908.10084).
The idea is to train a siamese text encoder to project the representation of 2 similar sentences into the same embedding space, such that the similarity is ranked and learned to create good embeddings.

JAX/Flax allows you to trace pure functions and compile them into efficient, fused accelerator code on both GPU and TPU.
Models written in JAX/Flax are **immutable** and updated in a purely functional
way which enables simple and efficient model parallelism.

## TODO : NEEDS MODIFICATION BELOW
