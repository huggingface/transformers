    ...
    if target_column_name == "translation":
        trainer = Seq2SeqTrainer(
            ...
            preprocess_logits_for_metrics=(
                asr_logits_argmax if training_args.predict_with_generate else None
            ),
            compute_metrics=(
                compute_asr_metric if training_args.predict_with_generate else None
            ),
            train_dataset=(
        ...
        trainer = Trainer(
            ...
            compute_metrics=(
                compute_asr_metric if is_text_target else compute_classification_metric
            ),
            train_dataset=(
                vectorized_datasets["train"] if training_args.do_train else None
            ),
