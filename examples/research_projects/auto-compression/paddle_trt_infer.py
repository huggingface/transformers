...
class Predictor(object):
    ...
    def create_predictor(cls, args):
        config = paddle.inference.Config(args.model_name_or_path + "model.pdmodel",
                                         args.model_name_or_path + "model.pdiparams")
        if args.device == "gpu":
    ...
    def predict(self, dataset, collate_fn, args):
        ...
        data_loader = DataLoader(dataset, collate_fn=collate_fn, batch_size=args.batch_size, num_workers=0, shuffle=False, drop_last=True)
        ...
        if args.perf:
            ...
            for i, data in enumerate(data_loader()):
                ...
                predictions = np.argmax(np.array(output[0]), axis=-1) if not is_regression else np.squeeze(np.array(output[0]))
