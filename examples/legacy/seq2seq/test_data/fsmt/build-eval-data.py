...
def get_all_data(pairs, n_objs):
    ...
    for src, tgt in pairs:
        ...
        src_lines = subprocess.run(cmd, check=False, stdout=subprocess.PIPE).stdout.decode("utf-8").splitlines()
        cmd = f"sacrebleu -t wmt19 -l {pair} --echo ref".split()
        tgt_lines = subprocess.run(cmd, check=False, stdout=subprocess.PIPE).stdout.decode("utf-8").splitlines()
