...
class MyNewModel2DecoderLayer(GradientCheckpointLayer):
    def __init__(self, config: MyNewModel2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.input_layernorm = MyNewModel2RMSNorm(config.hidden_size)
        self.post_attention_layernorm = MyNewModel2RMSNorm(config.hidden_size)
...
class MyNewModel2RotaryEmbedding(nn.Module):
    def __init__(
        self,
        device=None,
        config: Optional[MyNewModel2Config] = None,
    ):
        super().__init__()
        self.rope_kwargs = {}
