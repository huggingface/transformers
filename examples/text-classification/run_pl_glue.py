...
class GLUETransformer(BaseTransformer):
        ...
        if os.path.exists(cached_dataset_file) and not self.hparams.overwrite_cache:
            ...
            for split in splits:
                ...
                self.dataset[split].set_format(
                    type='torch',
                    columns=cols_to_keep if not split.startswith('test') else cols_to_keep + ['idx']
                )
    ...
    def load_dataset(self, mode: str, batch_size: int) -> Union[Tuple[DataLoader], DataLoader]:
        ...
        if self.hparams.task == 'mnli' and mode in ['validation', 'test']:
            return [
                DataLoader(
                    self.dataset[mode + '_matched'],
                    batch_size=batch_size,
                    num_workers=self.hparams.num_workers,
                ),
                DataLoader(
                    self.dataset[mode + '_mismatched'],
                    batch_size=batch_size,
                    num_workers=self.hparams.num_workers,
                )
            ]
...
def main(args):
    ...
    if args.do_predict:
        ...
        if args.task == 'mnli':
            ...
            df_mismatched = pd.DataFrame({'idx': model.idxs['mismatched'], 'prediction': model.predictions['mismatched']})
