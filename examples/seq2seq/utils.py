...
def calculate_rouge(
    ...
    Args:
        ...
        aggregate_determinstic: if False do the typical bootstrap resampling of scores. Otherwise take the average.
            Defaults to True, if False this function returns a collections.defaultdict[metric: list of values for each
            observation for each subscore]``
...
def parse_numeric_n_bool_cl_kwargs(unparsed_args: List[str]) -> Dict[str, Union[int, float, bool]]:
    """
    Parse an argv list of unspecified command line args to a dict. Assumes all values are either numeric or boolean in
...
def check_output_dir(args, expected_items=0):
    ...
    `expected_items`: normally 0 (default) - i.e. empty dir, but in some cases a few files are expected (e.g. recovery
    from OOM)
    """
    if (
