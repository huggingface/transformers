...
if __name__ == "__main__":
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            s[-1] = (s[-1] - 0.0) / 1.0
            a[-1] = np.concatenate([np.ones((1, max_len - tlen, config.act_dim)) * -10., a[-1]], axis=1)
    ...
    result = model(states, actions, rewards, rtg[:,:-1], timesteps, attention_mask=attention_mask,)
